...
namespace VAL
{
    ...
    void State::setNew(const effect_lists *is)
    {
        ...
        for (list<simple_effect *>::const_iterator i = is->add_effects.begin(); i != is->add_effects.end();
             ++i)
        {
    ...
    State::State(Validator *const v, const effect_lists *is) : tolerance(v->getTolerance()), vld(v), time(0.0)
    {
        setNew(is);
    };
