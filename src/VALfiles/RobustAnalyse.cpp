...
namespace VAL
{
    ...
    map<const plan_step *, const plan_step *> RobustPlanAnalyser::varyPlanTimestamps(plan *aplan,
                                                                                     const plan *p,
                                                                                     double &variation)
    {
    ...
    map<const plan_step *, const plan_step *> RobustPlanAnalyser::varyPlanTimestampsMax(plan *aplan,
                                                                                        const plan *p,
                                                                                        double &variation)
    {
        map<const plan_step *, const plan_step *> planStepMap;
    ...
    map<const plan_step *, const plan_step *> RobustPlanAnalyser::varyPlanTimestampsAccum(plan *aplan,
                                                                                          const plan *p,
                                                                                          double &variation)
    {
        double accumTime = 0;
    ...
    map<const plan_step *, const plan_step *> RobustPlanAnalyser::varyPlanTimestampsDelay(plan *aplan,
                                                                                          const plan *p,
                                                                                          double &variation)
    {
        double accumTime = 0;
    ...
    RobustPlanAnalyser::varyPlanTimestampsBoundary(plan *aplan, const plan *p, double &variation, int runNo)
    {
        map<const plan_step *, const plan_step *> planStepMap;
        if (robustMetric == MAX)
        {
    ...
    map<const plan_step *, const plan_step *> RobustPlanAnalyser::varyPlanTimestampsBoundaryMax(
    ...
    map<const plan_step *, const plan_step *> RobustPlanAnalyser::varyPlanTimestampsBoundaryAccum(
        plan *aplan, const plan *p, double &variation, int runNo)
    {
        double accumTime = 0;
        map<const plan_step *, const plan_step *> planStepMap;
    ...
    map<const plan_step *, const plan_step *> RobustPlanAnalyser::varyPlanTimestampsBoundaryDelay(
        plan *aplan, const plan *p, double &variation, int runNo)
    {
        double accumTime = 0;
        map<const plan_step *, const plan_step *> planStepMap;
    ...
    {
        Validator *v = new Validator(derivRules, tolerance, typeC, operators, initialState, p, metric,
                                     stepLength, durative, current_analysis->the_domain->constraints,
    ...
    void RobustPlanAnalyser::analyseRobustness()
    {
        ...
        {
            runAnalysis(robustMeasure, noTestPlans, true, numberOfInvalidPlans, numberOfErrorPlans, false,
                        latex);
        }

        if (calcActionRobustness || calcPNERobustness)
        {
            robustDist = UNIFORM;  // best to stick to uniform distribution for calculating robustnesses
        }
        ...
        if (LaTeX)
        {
            displayAnalysisLaTeX(noTestPlans, numberOfInvalidPlans, numberOfErrorPlans,
                                 actionRobustnessOfPlan, actionRobustnessBound, pneRobustnessOfPlan,
                                 pneRobustnessBound);
        }
        else
        {
            displayAnalysis(noTestPlans, numberOfInvalidPlans, numberOfErrorPlans, actionRobustnessOfPlan,
                            actionRobustnessBound, pneRobustnessOfPlan, pneRobustnessBound);
        }
    };

    void RobustPlanAnalyser::runAnalysis(double &variation, int &numberTestPlans, bool recordFailures,
                                         int &numberOfInvalidPlans, int &numberOfErrorPlans, bool allValid,
                                         bool latexAdvice)
    {
        ...
        {
            map<const plan_step *, const plan_step *> planStepMap;  // we need to keep track of which actions
                                                                    // fail
            ...
            {
                planStepMap = varyPlanTimestampsBoundary(testPlan, p, variation, testNo);
            }
            else
            {
            ...
            for (vector<plan_step *>::iterator ps = timedIntitialLiteralActions.begin();
                 ps != timedIntitialLiteralActions.end(); ++ps)
            {
                testPlan->push_back(*ps);
            };

            testPlanValidator = new Validator(
                derivRules, tolerance, typeC, operators, initialState, testPlan, metric, stepLength, durative,
                current_analysis->the_domain->constraints, current_analysis->the_problem->constraints);
                    ...
                    if (!unSatConds.empty())
                    {
                                     ...
                                     dynamic_cast<const UnsatInvariant *>(firstError))
                        {
                            theAction = unsatinv->action;
                                     ...
                                     dynamic_cast<const UnsatDurationCondition *>(firstError))
                        {
                        ...
                        if (theAction != 0 || unsatGoal)
                        {
                            ...
                            if (!unsatGoal)
                            {
                                aPlanStep = theAction->getPlanStep();
                            }
                            ...
                            if (aPlanStep != 0 || unsatGoal)
                            {
                                ...
                                else
                                {
                                    bool lx = LaTeX;
                                    LaTeX = latexAdvice;
                                    record[planStepMap[aPlanStep]] = InvalidActionReport(
                                        1, firstError->getDisplayString(), firstError->getAdviceString());
                                    LaTeX = lx;
                                };
                            }
                            else
    ...
    // how much can the time stamps vary, so that we can be 95% sure that the plan will always be valid (well
    // at least 95%)
    void RobustPlanAnalyser::calculateActionRobustness(double &robustnessOfPlan, double &robustnessBound)
    {
        ...
        do
        {
            ...
            if (lowerBound == 0)
            {
                if (upperBound < lowerBoundLowerBound)
                {
                    robustnessFound = true;
                }
            }
            else
            {
    ...
    void RobustPlanAnalyser::calculatePNERobustness(double &robustnessOfPlan, double &robustnessBound)
    {
        ...
        do
        {
            ...
            runAnalysis(robustMeasure, numberOfTestPlans, false, noOfInvalidPlans, noOfErrorPlans, true,
                        false);
    ...
    void RobustPlanAnalyser::displayAnalysis(int noTestPlans, int numberOfInvalidPlans,
                                             int numberOfErrorPlans, double actionRobustnessOfPlan,
                                             double actionRobustnessBound, double pneRobustnessOfPlan,
                                             double pneRobustnessBound)
    {
        ...
        double upperCritialValTDistrib =
            getUpperCritialValueTDistribution(0.05, noTests - 1);  // we need to look this up in a table, this
                                                                   // is for alpher = 0.05 and N = noTests-1
        ...
        if (!calcActionRobustness && !calcPNERobustness)
        {
            ...
            {
                cout << noTestPlans - numberOfErrorPlans - numberOfInvalidPlans << " plans are valid from ";
            }
            cout << noTestPlans - numberOfErrorPlans << " plans for each action timestamp +-" << robustMeasure
                 << " and each PNE +-" << RobustPNEJudder << ".\n\n";

            if (estProbValidPlan != 1 && estProbValidPlan != 0)
            {
                cout << "There is a 95% chance that the plan has a valid execution with probability in the "
                        "range "
                     << estProbValidPlan * 100.0 << " +-" << errorLimit * 100.0 << ".\n";
            }
            else if (estProbValidPlan == 1)
            {
                cout << "There is a 99% chance that the plan will be valid with probability of at least "
                     << 100 * pow(0.01, (1.0 / (noTestPlans - numberOfErrorPlans))) << "%.\n";
            }
            else if (estProbValidPlan == 0)
            {
                cout << "There is a 99% chance that the plan will fail with probability of at least "
                     << 100 * pow(0.01, (1.0 / (noTestPlans - numberOfErrorPlans))) << "%.\n";
            }
        };
        ...
        if (calcActionRobustness)
        {
            ...
            {
                *report << "The plan has an action timestamp robustness greater than the length of the plan, "
                        << getMaxTime(p);
            }
            else
            {
        ...
        if (calcPNERobustness)
        {
            *report << "The plan has a PNE robustness in the range " << pneRobustnessOfPlan << " +-"
                    << pneRobustnessBound;
        ...
        *report << "\n";
        ...
        if (Verbose && numberOfInvalidPlans != 0)
        {
            ...
            {
                cout << unsatisfiedGoal << " plans failed because the goal is not satisfied.\n";
            }
            ...
            {
                cout << "1 plan did not execute due errors in the validation process.\n";
            }
            else if (numberOfErrorPlans != 0)
            {
                cout << numberOfErrorPlans
            ...
            for (map<const plan_step *, InvalidActionReport>::const_iterator d = record.begin();
                 d != record.end(); ++d)
            {
                string theTime;
                if (d->first != 0)
                ...
                cout << " ";
                for (map<string, pair<int, string>>::const_iterator fr = d->second.failReasons.begin();
    ...
    void RobustPlanAnalyser::displayAnalysisLaTeX(int noTestPlans, int numberOfInvalidPlans,
                                                  int numberOfErrorPlans, double actionRobustnessOfPlan,
                                                  double actionRobustnessBound, double pneRobustnessOfPlan,
                                                  double pneRobustnessBound)
    {
        ...
        *report << "\\subsection{Plan Robustness Report}\n";
        ...
        double upperCritialValTDistrib = getUpperCritialValueTDistribution(
            0.05, noTests - 1);  // we need to look this up in a table, this is for alpher = 0.05
        double errorLimit = (upperCritialValTDistrib * standardDev) / (sqrt(double(noTests)));
        ...
        *report << "\\begin{itemize}\n";
        if (!calcActionRobustness && !calcPNERobustness)
        {
            ...
            *report << noTestPlans - numberOfErrorPlans << " plans for each action timestamp $\\pm$"
                    << robustMeasure << " and each PNE $\\pm$" << RobustPNEJudder << ".\\\\\n";

            *report << "\\item ";
            if (estProbValidPlan != 1 && estProbValidPlan != 0)
            {
                *report << "There is a 95$\\%$ chance that the plan has a valid execution with probability "
                           "in the range "
                        << estProbValidPlan * 100.0 << " $\\pm$" << errorLimit * 100.0 << ".\\\\\n";
            }
            else if (estProbValidPlan == 1)
            {
                *report << "There is a 99$\\%$ chance that the plan will be valid with probability of at "
                           "least "
                        << 100 * pow(0.01, (1.0 / (noTestPlans - numberOfErrorPlans))) << "$\\%$.\\\\\n";
            }
            else if (estProbValidPlan == 0)
            {
                *report << "There is a 99$\\%$ chance that the plan will fail with probability of at least "
                        << 100 * pow(0.01, (1.0 / (noTestPlans - numberOfErrorPlans))) << "$\\%$.\\\\\n";
            }
        };
        ...
        {
            *report << "\\item ";
            *report << "The plan has a PNE robustness in the range " << pneRobustnessOfPlan << "$\\pm$"
                    << pneRobustnessBound;
            if (robustMeasure > 0)
            {
                *report << " when action timestamps may vary up to " << robustMeasure << ".\\\\\n";
        ...
        *report << "\\end{itemize}\n";
        ...
        if (numberOfInvalidPlans != 0)
        {
            ...
            *report << "\\begin{tabbing}\n {\\bf Failures} \\= {\\bf Time} \\qquad \\= {\\bf "
            ...
            if (unsatisfiedGoal == 1)
            {
                *report << "1 \\> \\> Plan failed because the goal is not satisfied.\\\\\n";
            }
            else if (unsatisfiedGoal != 0)
            {
                *report << unsatisfiedGoal
            ...
            if (numberOfErrorPlans == 1)
            {
                *report << "1  \\> \\> plan did not execute due errors in the validation process.\\\\\n";
            }
            else if (numberOfErrorPlans != 0)
            {
                *report << numberOfErrorPlans
                        << "  \\> \\> plans did not execute due errors in the validation process.\\\\\n";
            }
            ...
            *report << "\\setcounter{figure}{0}\n";
            ...
            FEGraph accumPerErrorGraphL("Accumulative percentage of plans failing by these times", 0.0,
                                        100.0);
            int prevValue = 0;
            int accPrevValue = 0;
            ...
            for (map<double, int>::const_iterator g = noErrors.begin(); g != noErrors.end(); ++g)
            {
                ...
                accumPerErrorGraph.discons[g->first] =
                    make_pair(accPerPrevValue, accPerPrevValue + ((g->second) / double(noTests)) * 100.0);
