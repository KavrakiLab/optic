...
namespace Inst
{
    ...
    public:
        ...
        const VAL::func_term *toFuncTerm()
        {
            if (!realisation)
            {
                ...
                for (VAL::parameter_symbol_list::const_iterator i = func->getArgs()->begin();
        ...
        void write(ostream &o) const
        {
            ...
            transform(func->getArgs()->begin(), func->getArgs()->end(), FlexiblePrint<string>(o, " ", ""),
                      PNEParametersOutput(*env));
            o << ")";
        };
    ...
    public:
        ...
        const VAL::proposition *toProposition()
        {
            ...
            {
                VAL::parameter_symbol_list *pl = new VAL::parameter_symbol_list;
                for (VAL::parameter_symbol_list::iterator i = prop->args->begin(); i != prop->args->end();
                     ++i)
                {
                    pl->push_back((*env)[*i]);
                };
        ...
        {
            o << "(" << prop->head->getName();
            transform(prop->args->begin(), prop->args->end(), FlexiblePrint<string>(o, " ", ""),
    ...
    {
        CreatedLiteral(const VAL::proposition *p, VAL::FastEnvironment *e) : Literal(p, e){};
    ...
    struct Purifier
    {
        const S *operator()(const S *s) const
        {
            return s;
        };
    };
    ...
    {
        const pred_symbol *operator()(const pred_symbol *p) const
        {
    ...
    {
        const func_symbol *operator()(const func_symbol *f) const
        {
    ...
    public:
        PrimitiveEvaluatorConstructor<LitStoreEvaluator>(LiteralStore &lits) : literals(lits){};

        PrimitiveEvaluator *operator()(bool &v, bool &u, bool &w, bool &x) const
        {
    ...
    public:
        ...
        {
            o << "(" << op->name->getName();
            std::transform(op->parameters->begin(), op->parameters->end(), FlexiblePrint<string>(o, " ", ""),
        ...
        static int howManyLiteralsOfAnySort()
        {
        ...
        static int howManyPNEsOfAnySort()
        {
            return pnes.size();
        }
        ...
        static OpStore::iterator opsBegin()
        {
            return instOps.begin();
        };

        static OpStore::iterator opsEnd()
        {
            return instOps.end();
        };

        static PNEStore::iterator pnesBegin()
        {
            return pnes.begin();
        };

        static PNEStore::iterator pnesEnd()
        {
            return pnes.end();
        };

        static LiteralStore::iterator literalsBegin()
        {
            return literals.begin();
        };

        static LiteralStore::iterator literalsEnd()
        {
            return literals.end();
        };
        ...
        static instantiatedOp *getInstOp(int i)
        {
            return instOps[i];
        };
        ...
        static OpStore::iterator from(int k)
        {
            return opsBegin() + k;
        };

        const VAL::operator_ *forOp() const
        {
            return op;
        };

        VAL::FastEnvironment *getEnv()
        {
            return env;
        };

        static Literal *getLiteral(Literal *l)
        {
            return literals.insert(l);
        };

        static Literal *findLiteral(Literal *l)
        {
            return literals.find(l);
        };
        ...
        static PNE *getPNE(PNE *p)
        {
            return pnes.insert(p);
        };

        static PNE *findPNE(PNE *p)
        {
            return pnes.find(p);
        };
        ...
        const VAL::operator_symbol *getHead() const
        {
            return op->name;
        };
        ...
        public:
            ...
            bool operator==(const PNEEffectsIterator &i) const
            {
                return effts == i.effts &&
                       (effs == i.effs || (effts == efftsend && effs == effsend && i.effs == i.effsend));
            };
    ...
    public:
        ...
        static int howMany()
        {
            return instDrvs.size();
        };

        static void createAllLiterals(VAL::problem *p, VAL::TypeChecker *tc);
        void collectLiterals(VAL::TypeChecker *tc);

        static DrvStore::iterator drvsBegin()
        {
            return instDrvs.begin();
        };

        static DrvStore::iterator drvsEnd()
        {
            return instDrvs.end();
        };
        ...
        static instantiatedDrv *getInstDrv(int i)
        {
            return instDrvs[i];
        };
        ...
        static DrvStore::iterator from(int k)
        {
            return drvsBegin() + k;
        };

        const VAL::derivation_rule *forDrv() const
        {
            return op;
        };
        ...
        const instantiatedDrvUtils::index *getHead() const
        {
            return &localHead;
        };
