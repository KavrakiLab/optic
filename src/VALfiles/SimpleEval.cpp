...
namespace Inst
{
    ...
    void InitialStateEvaluator::setInitialState()
    {
             ...
             i != current_analysis->the_problem->initial_state->add_effects.end(); ++i)
        {
            ...
            {
                // cout << "Recording " << (*i)->prop->head->getName() << "-stemmed fact in the initial state,
                // ptr " << (*i)->prop->head << "\n";
    ...
    void SimpleEvaluator::visit_simple_goal(simple_goal *s)
    {
        if (EPS(s->getProp()->head)->getParent() == this->equality)
        {
            ...
            unknownTrue = false;
            unknownFalse = false;
            valueTrue = ((*f)[s->getProp()->args->front()] == (*f)[s->getProp()->args->back()]);
            valueFalse = !valueTrue;
    ...
    void SimpleEvaluator::visit_qfied_goal(qfied_goal *p)
    {
        ...
        int i = 0;
        int c = 1;
        for (var_symbol_list::const_iterator pi = p->getVars()->begin(); pi != p->getVars()->end(); ++pi, ++i)
        {
        ...
        while (vals[i] != ends[i])
        {
            ...
            // used. In practice, this should not be a problem because a quantified effect presumably uses the
            // variables it quantifies.
            ...
            else
            {
                if (reallyTrue())
                {
                    if (SimpleEvaluator::verbose)
                    {
                        cout << "Tautologous child of exists\n";
                    }
                    return;
                }
                uTrue = uTrue || unknownTrue;
    ...
    void SimpleEvaluator::visit_conj_goal(conj_goal *c)
    {
        ...
        valueTrue = true;
        valueFalse = false;
        for (goal_list::const_iterator i = c->getGoals()->begin(); i != c->getGoals()->end(); ++i)
        {
            (*i)->visit(this);
            if (reallyFalse())
            {
                if (SimpleEvaluator::verbose)
                {
                    cout << "Contradictory child of and\n";
                }
                return;
            }
            uTrue = uTrue || unknownTrue;
    ...
    void SimpleEvaluator::visit_disj_goal(disj_goal *d)
    {
        ...
        for (goal_list::const_iterator i = d->getGoals()->begin(); i != d->getGoals()->end(); ++i)
        {
            (*i)->visit(this);
            if (reallyTrue())
            {
    ...
    void SimpleEvaluator::visit_action(action *op)
    {
        if (op->precondition)
        {
            if (SimpleEvaluator::verbose)
            {
                cout << "Visiting operator preconditions\n";
            }
            op->precondition->visit(this);
            if (SimpleEvaluator::verbose)
            {
    ...
    void SimpleEvaluator::visit_func_term(func_term *s)
    {
        ...
        if (efs->isStatic())
        {
            isFixed = true;
            pair<bool, double> pv =
                efs->getInitial(makeIterator(f, s->getArgs()->begin()), makeIterator(f, s->getArgs()->end()));
            if (pv.first)
            {
                nvalue = pv.second;
