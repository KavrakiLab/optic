...
namespace VAL
{
    ...
    CompoundPropStore::CompoundPropStore(int c, vector<pair<pddl_type *, vector<const pddl_type *>>> &tps,
    ...
    PropStore *extended_pred_symbol::records() const
    {
        if (!props)
        {
            const_cast<extended_pred_symbol *>(this)->props = parent->buildPropStore(
                const_cast<extended_pred_symbol *>(this), types.begin(), types.end());  // types);
        };
        return props;
    };
    ...
    vector<double> extended_pred_symbol::getTimedAchievers(Environment *f, const proposition *prop) const
    {
        ...
        for (map<double, PropStore *>::const_iterator i = timedInitials.begin(); i != timedInitials.end();
             ++i)
        {
    ...
    {
        return records.partialGet(makeIterator(f, prop->args->begin()), makeIterator(f, prop->args->end()));
    };
