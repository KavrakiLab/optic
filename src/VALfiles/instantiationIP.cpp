...
void instantiatedOp::instantiate(const operator_ *op, const problem *prb, TypeChecker &tc)
{
    ...
    int i = 0;
    int c = 1;
    for (var_symbol_list::const_iterator p = op->parameters->begin(); p != op->parameters->end(); ++p, ++i)
    {
...
public:
    Collector(const operator_ *o, FastEnvironment *f, LiteralStore &l, LiteralStore &ps, LiteralStore &as,
              LiteralStore &ds)
