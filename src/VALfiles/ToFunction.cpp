...
namespace SAS
{
        ...
        for (Range::const_iterator i = range.begin(); i != range.end(); ++i)
        {
            ...
            for (ValuesUnion::const_iterator j = i->begin(); j != i->end(); ++j)
            {
                ...
                for (; k != j->end(); ++k)
                {
                    ...
                    {
                        int c = 0;
                        for (parameter_symbol_list::iterator pr = got->args->begin(); pr != got->args->end();
                             ++pr, ++c)
                        {
                            if (c != wanted->aPosn())
                            {
        ...
        for (Range::const_iterator i = vvs.getRange().begin(); i != vvs.getRange().end(); ++i)
        {
            ...
            for (ValuesUnion::const_iterator j = i->begin(); j != i->end(); ++j)
            {
                ...
                for (; k != j->end(); ++k)
                {
                    ...
                    // we know that all the propositions will have to be accounted for by the state we
                    // identify. So, we'll count how many propositions are accounted for and then check that
                    // they were all handled by the end.
                    Property *wanted = (*k)->getProp();
                    if (!wanted)
                    {
                ...
                if (cnt)  // == relevantConds)
                {
                    ...
                    // and so we can't be in any other, unless it shares the common set of propositions. We
                    // could carry on and see if we can find another match.
                    //				if(cnt < (int)conds.size()) cout << "Still some conditions to account
                    // for\n";
                    break;
                }
                else
    ...
    public:
        ...
        ConditionGatherer(const ConditionGatherer &cg)
          : gathered(cg.gathered.size()), values(cg.values), theStatics(cg.theStatics), others(){};
        ...
        virtual void visit_proposition(proposition *p)
        {
            ...
            for (unsigned int i = 0; i < p->args->size(); ++i, ++ps)
            {
                ...
                // The following assumes that a property will be single valued for all parameters (of any
                // types) that may instantiate a particular position. This looks for any one single valued
                // property matching the property from the proposition (which can be for a more general type
                // than the properties that were originally marked as single valued).
                vector<Property *> ms;
                ...
                for (vector<Property *>::const_iterator prp = ms.begin(); prp != ms.end(); ++prp)
                {
                    ...
                    if ((*prp)->isSingleValued())
                    {
                        ...
                        gathered[(static_cast<const IDsymbol<var_symbol> *>(*ps)->getId())].push_back(
                            make_pair(TPS(p->head)->property(i), p));
                        break;
                    };
                };
            };
        ...
        {
            for (pc_list<simple_effect *>::iterator i = p->add_effects.begin(); i != p->add_effects.end();
                 ++i)
            {
                (*i)->visit(this);
            };
        ...
        void collect(const operator_ *op, FunctionStructure *fs, bool stateForAll, VMap &valueFor)
        {
            ...
            SASOUTPUT
            {
                cout << (stateForAll ? "Precondition" : "Postcondition") << " states for " << *TAS(op->name)
                     << "\n";
            };
            int c = 0;
            for (var_symbol_list::const_iterator ps = op->parameters->begin(); ps != op->parameters->end();
                 ++ps, ++c)
            {
                if (!stateForAll && !TAS(op->name)->hasRuleFor(c))
                {
                    continue;
                }
                ...
                if (!fs->hasFluent((*ps)->type))
                {
                    ...
                    for (vector<const pddl_type *>::const_iterator xx = tmptps.begin(); xx != tmptps.end();
                         ++xx)
                    {
                        if (fs->hasFluent(*xx))
                        {
                ...
                else
                {
                    ...
                    while (ttp)
                    {
                        ...
                        {
                            tps.push_back(ttp);
                            //						cout << "Pushing type " << ttp->getName() << " for " <<
                            //(*ps)->getName() << "\n";
                        };
                        ttp = ttp->type;
                    };
            ...
            SASOUTPUT
            {
                ...
                {
                    cout << "\nStatic conditions:\n";
                    for (vector<proposition *>::const_iterator s = theStatics.begin(); s != theStatics.end();
                         ++s)
                    {
                        cout << "(" << (*s)->head->getName();
    ...
    void FunctionStructure::processActions()
    {
             ...
             i != current_analysis->the_domain->ops->end(); ++i)
        {
            ...
            /*		for(TIMactionSymbol::RCiterator r = TAS((*i)->name)->begin();r !=
               TAS((*i)->name)->end();++r)
                    {
                        cout << **r << "\n";
                    };
    ...
    struct ValueStruct
    {
        ...
        ValueStruct(const pddl_type *p, const ValuesUnion &v, const PropertyState *s) : tp(p), vu(v), ps(s){};
    ...
    void FunctionStructure::normalise()
    {
             ...
             op != current_analysis->the_domain->ops->end(); ++op)
        {
                 ...
                 ps != (*op)->parameters->end(); ++ps, ++c)
            {
                ...
                for (TIMactionSymbol::RCiterator r = TAS((*op)->name)->begin(); r != TAS((*op)->name)->end();
                     ++r)
                {
                    if ((*r)->paramNum() == c)
                    {
                        ...
                        if (tps.empty())
                        {
                            tps.push_back((*ps)->type);
                        }
                        vector<const pddl_type *> rtps;
                        for (vector<const pddl_type *>::const_iterator xx = tps.begin(); xx != tps.end();
                             ++xx)
                        {
                            if (hasFluent(*xx))
                            {
                        ...
                        for (vector<const pddl_type *>::const_iterator tp = rtps.begin(); tp != rtps.end();
                             ++tp)
                        {
                                 ...
                                 vu != frep.find(*tp)->second.getRange().end(); ++vu)
                            {
                                ...
                                if (v != (*vu).end())
                                {
                                    ...
                                    {
                                        //									cout << "Rule " << **r << " applies
                                        //to partial state description in "
                                        //											<< *(v.forState()) <<
                ...
                // The following block seems unnecessary because the action processing handles the multiple
                // subtypes case (although there could still be unforeseen problems with that, so we'll keep
    ...
    void FunctionStructure::restructure(const vector<ValueStruct> &toReduce,
                                        const vector<ValueStruct> &toLeave, const PropertyState *ps)
    {
        /*
        ...
        */
        ...
        for (vector<ValueStruct>::const_iterator i = toReduce.begin(); i != toReduce.end(); ++i)
        {
            ...
            //		cout << "Planning to replace original ValuesUnion " << i->vu << " with " << newvu1 << "
            // and
            //"
            //				<< newvu2 << "\n";
    ...
    void ValueStructure::update(const ValuesUnion &oldvu, const ValuesUnion &newvu1,
    ...
    {
        for (vector<pair<const PropertyState *, Values>>::iterator i = valuesUnion.begin();
             i != valuesUnion.end(); ++i)
        {
                 ...
                 j != EPS(others[i]->head)->posPresEnd(); ++j)
            {
                for (int k = startFor(j->op); k != endFor(j->op); ++k)
                {
                    if (!(--(unsatisfiedPrecs[k])))
                    {
                        ...
                        sasActionTemplates[j->op]->enact(instantiatedOp::getInstOp(k)->getEnv(), reachables,
                                                         others);
                        activated = true;
                    };
                };
