...
namespace VAL
{
    ...
    public:
        ...
        virtual pair<vector<pair<intervalEnd, intervalEnd>>, vector<CoScalar>>
        ...
        virtual bool checkInvariant(const Comparison *comp, const State *s, CoScalar t,
                                    bool rhsIntervalOpen) const = 0;
    };
    ...
    public:
        ...
        static void setAccuracy(CoScalar ac)
        {
            accuracy = ac / 10;
        };
    ...
    public:
        ...
        vector<CoScalar> getRoots(CoScalar t) const;  // on interval(0,t) within accuracy
        pair<vector<pair<intervalEnd, intervalEnd>>, vector<CoScalar>> isolateRoots(CoScalar t,
                                                                                    CoScalar accuracy) const;
        CoScalar evaluate(CoScalar t) const;
        bool isLinear() const;
        ...
        unsigned int getNoTerms(CoScalar endInt) const;  // number of terms to take from power series for
                                                         // approx poly
    ...
    public:
        ...
        map<double, CoScalar> getPoints() const
        {
            return points;
        };
    ...
    {
        // for diff eqn:   dy/dt = p(t) (m - y) - k(t),   for some poly p(t) and some constant m and discharge
        // k(t)
        const Polynomial *poly;
        CoScalar m;
    ...
    public:
        ...
        BatteryCharge(const Polynomial *p, CoScalar m0, vector<pair<const CtsFunction *, bool>> d,
                      CoScalar t0, CoScalar t1, CoScalar y0, CoScalar accuracy)
        ...
        BatteryCharge(CoScalar off, map<double, CoScalar> pts, const Polynomial *p, CoScalar m0,
                      vector<pair<const CtsFunction *, bool>> d)
          : NumericalSolution(pts, off), poly(p), m(m0), discharge(d){};

        ~BatteryCharge(){
            // if(poly != 0) delete poly;
            //     for(vector<pair<const CtsFunction *,bool> >::const_iterator i = discharge.begin(); i !=
            //     discharge.end(); ++i)
            //  {
            //     delete i->first;
            //  };
