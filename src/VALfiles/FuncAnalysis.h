    ...
    public:
        ...
        FValue initially() const
        {
            ...
            if (!seenPos)
            {
                if (!seenZero)
                {
                    return (cval = E_NEGATIVE);
                }
                if (seenNeg)
                {
                    return (cval = E_NONPOS);
                }
                return (cval = E_ZERO);
            }
            else if (!seenNeg)
        ...
        void write(ostream &o) const
        {
            ...
            else if (assigns.empty() && scalers.empty())
            {
                ...
                o << "Assigns:\n";
                for (Updates::const_iterator i = assigns.begin(); i != assigns.end(); ++i)
                {
                    if (i->first)
                    {
                        o << "\t" << i->first->name->getName() << "\n";
                ...
                o << "Increasers:\n";
                for (Updates::const_iterator i = increasers.begin(); i != increasers.end(); ++i)
                {
                    if (i->first)
                    {
                        o << "\t" << i->first->name->getName() << "\n";
                ...
                o << "Decreasers:\n";
                for (Updates::const_iterator i = decreasers.begin(); i != decreasers.end(); ++i)
                {
                    if (i->first)
                    {
                        o << "\t" << i->first->name->getName() << "\n";
                ...
                o << "Scalers:\n";
                for (Updates::const_iterator i = scalers.begin(); i != scalers.end(); ++i)
                {
                    if (i->first)
                    {
                        o << "\t" << i->first->name->getName() << "\n";
                ...
                if (!continuous.empty())
                {
                    ...
                    for (Updates::const_iterator i = continuous.begin(); i != continuous.end(); ++i)
                    {
                        if (i->first)
                        {
                            o << "\t" << i->first->name->getName() << "\n";
            ...
            for (vector<assignment *>::const_iterator i = initials.begin(); i != initials.end(); ++i)
            {
                o << "\t(" << getName();
                for (parameter_symbol_list::const_iterator j = (*i)->getFTerm()->getArgs()->begin();
...
#define EFT(x) static_cast<extended_func_symbol *>(const_cast<func_symbol *>(x))
    ...
    private:
        ...
        template <class TI>
        void doExplore(std::set<func_symbol *> &explored, vector<extended_func_symbol *> &tsort, bool invert,
                       IGraph &inverted, extended_func_symbol *fn, TI, TI);

        void exploreFrom(std::set<func_symbol *> &explored, vector<extended_func_symbol *> &tsort,
