...
namespace VAL
{
    ...
    Intervals::Intervals(vector<pair<intervalEnd, intervalEnd>> ints) : intervals(ints)
    {
        ...
        for (vector<pair<intervalEnd, intervalEnd>>::iterator i = intervals.begin(); i != intervals.end();
             ++i)
        {
            if (((i->first.first > i->second.first) ||
                 ((i->first.first == i->second.first) && (!(i->first.second) || !(i->second.second)))))
            {
    ...
    void Intervals::write(ostream &o) const
    {
        ...
        else
        {
            ...
            for (vector<pair<intervalEnd, intervalEnd>>::const_iterator i = intervals.begin();
                 i != intervals.end();)
            {
                if (i->first.second)
                {
    ...
    Polynomial Polynomial::diff() const
    {
        ...
        for (map<unsigned int, CoScalar>::const_iterator i = coeffs.begin(); i != coeffs.end(); ++i)
        {
            if (i->first != 0)
            {
                ans.setCoeff((i->first) - 1, (i->second) * (i->first));
            }
        };
    ...
    CoScalar newtonsMethod(const Polynomial &p, CoScalar startPt, CoScalar accuracy)
    {
        ...
        int limit = 50 + int(1 / accuracy);  // should normally be within accurrcy < 10 steps, so limit is
                                             // more than enough
        int i = 0;
        ...
        for (; i < limit; ++i)
        {
            ...
            if ((approxRoot - lastApproxRoot < accuracy) && (approxRoot - lastApproxRoot > -accuracy))
            {
                break;
            }
    ...
    vector<CoScalar> Polynomial::getRoots(CoScalar t) const
    {
        ...
        else if (testPoly.getDegree() == 2)
        {
            ...
            if (b * b - 4 * a * c >= 0)
            {
                ...
                if (root2 > 0 && root2 < t && root1 != root2)
                {
        ...
        else
        {
            pair<vector<pair<intervalEnd, intervalEnd>>, vector<CoScalar>> isolExact =
                testPoly.isolateRoots(t, Polynomial::accuracy);

            for (vector<pair<intervalEnd, intervalEnd>>::iterator i = isolExact.first.begin();
        ...
        for (vector<CoScalar>::iterator i = copyRoots.begin(); i != copyRoots.end(); ++i)
        {
            if (*i > 0.01)
            {
                roots.push_back(*i);  // tolerance value
            }
        };
        copyRoots.clear();
    ...
    Polynomial Polynomial::getGCD(const Polynomial &p) const
    {
        ...
        if ((gcd.getDegree() == 0) && (gcd.getCoeff(0) == 0))
        {
            gcd.setCoeff(0, 1);
        }
    ...
    unsigned int descartesBound(const Polynomial &p)
    {
        ...
        if (noSignChanges == 0 && extraVal == 1)
        {
            ...
            if (p.evaluate(0) * p.evaluate(1) < 0)
            {
                noSignChanges = 1;
            }
        }
    ...
    // test for infinity and NaN, returns true if there are no dodgey numbers ie infinity and NaN, (note
    // coeffs value 0 not stored)
    ...
    CoScalar findRootUsingNewtonsMethod(const Polynomial &p, CoScalar intStart, CoScalar intEnd,
                                        CoScalar accuracy)
    {
        CoScalar theRoot = -1;
        int step = 1;

        // use newtons method by symatically choosing different starting points, bisecting the interval(s)
        // each time the starting points will be dense on the interval at infinity but stop once within degree
        // of accuracy should normally find root at first point anyway - using only 10 steps of newtons method
        while (theRoot == -1 && pow(0.5, step - 1) > accuracy)
        {
            for (int j = 0; j < pow(2, step - 1); ++j)
            {
                theRoot = newtonsMethod(
                    p, (intEnd - intStart) * (pow(0.5, step) + j * pow(0.5, step + 1)) + intStart, accuracy);
            };

            if (theRoot <= intStart || theRoot >= intEnd)
            {
                theRoot = -1;
            }
            step++;
    ...
    pair<vector<pair<intervalEnd, intervalEnd>>, vector<CoScalar>> descartesAlg(const Polynomial &p,
                                                                                CoScalar accuracy = 0)
    {
        ...
        do
        {
            ...
            {
                if ((intEnd - intStart) < accuracy)  // maybe repeated root or curve that is very close to
                                                     // zero
                {
                    isol.push_back(make_pair(make_pair(intStart, false), make_pair(intEnd, false)));
                }
    ...
    Intervals getIntervalsFromRoots(vector<CoScalar> roots, const CtsFunction *ctsFtn, CoScalar t,
                                    bool strict)
    {
        Intervals theAns;
    ...
    Intervals Polynomial::getIntervals(const Comparison *comp, const State *s, CoScalar t) const
    {
        ...
        bool strict =
            (comp->getComparison()->getOp() == E_GREATER) || (comp->getComparison()->getOp() == E_LESS);
    ...
    {  // f(t) = K e^(poly) + c   for    f # offSet, where # is >,  <=, > or >=
       // return getApproxPoly(t).getIntervals(comp,s,t);
        ...
        bool strict =
            (comp->getComparison()->getOp() == E_GREATER) || (comp->getComparison()->getOp() == E_LESS);
        // bool greater = (comp->getComparison()->getOp() == E_GREATER) || (comp->getComparison()->getOp() ==
        // E_GREATEQ);
        ...
        if (!rootsExist)
        {
            if (comp->evaluateAtPoint(s))  // no roots so is either satisfied or not over whole interval - so
                                           // just evaluate one pt
            {
                theAns.intervals.push_back(make_pair(make_pair(0, true), make_pair(t, true)));
            };
            return theAns;
    ...
    Intervals NumericalSolution::getIntervals(const Comparison *comp, const State *s, CoScalar t) const
    {
        ...
        bool strict =
            (comp->getComparison()->getOp() == E_GREATER) || (comp->getComparison()->getOp() == E_LESS);
        bool greater =
            (comp->getComparison()->getOp() == E_GREATER) || (comp->getComparison()->getOp() == E_GREATEQ);
        ...
        if ((j->second > offSet && greater) || (j->second < offSet && !greater))
        {
            ...
            while (j != points.end())
            {
                if ((positive && j->second < offSet) || (!positive && j->second > offSet) ||
            ...
            {
                endPoint = lastTime +
                           (j->first - lastTime) * (fabs(lastPoint) / (fabs(lastPoint) + fabs(j->second)));
            }
            else
            {
                endPoint = lastTime;
        ...
        {  // point at j is not satisfied to begin with
            ...
            if (!startPointDefined)
            {
                ...
                {
                    startPt =
                        make_pair(lastTime + (j->first - lastTime) *
    ...
    bool Polynomial::checkInvariant(const Comparison *comp, const State *s, CoScalar t,
            ...
            else if (degree > 1)
            {
                ...
                else
                {
                    ...
                    if (strict)
                    {
                        ...
                        if (!rtsExist)
                        {
                            return evaluate(t / 2.0) > 0;
                        }
                        return !rtsExist;
                    };

                    pair<vector<pair<intervalEnd, intervalEnd>>, vector<CoScalar>> isolExact =
                        removeRepeatedRoots().isolateRoots(t);
    ...
    bool Exponential::checkInvariant(const Comparison *comp, const State *s, CoScalar t,
                                     bool rhsIntervalOpen) const
    {
        if (comp->getComparison()->getOp() == E_EQUALS)
        {
        ...
        else
        {
            if (strict)
            {
                ...
                if (!rtsExist)
                {
                    return evaluate(t / 2.0) - offSet > 0;
                }
                return !rtsExist;
            };
            ...
            for (vector<CoScalar>::const_iterator i = roots.begin(); i != roots.end(); ++i)
            {
                if (evaluate((previousPt + (*i)) / 2) - offSet < 0)
                {
                    return false;
                }
                previousPt = *i;
    ...
    bool NumericalSolution::checkInvariant(const Comparison *comp, const State *s, CoScalar t,
                                           bool rhsIntervalOpen) const
    {
        bool strict =
            (comp->getComparison()->getOp() == E_GREATER) || (comp->getComparison()->getOp() == E_LESS);
        ...
        // check initial point
        map<double, CoScalar>::const_iterator j = points.begin();
        if ((greater && (j->second - offSet < 0)) || (!greater && (j->second - offSet > 0)))
        {
            return false;
        }
        ...
        for (; j != points.end();)
        {
            ...
            if ((greater && (j->second - offSet < 0) || (j->second - offSet <= 0 && !strict)) ||
                (!greater && (j->second - offSet > 0) || (j->second - offSet >= 0 && !strict)))
            {
                return false;
            }
        };
    ...
    {
        CoScalar dis = 0;
        for (vector<pair<const CtsFunction *, bool>>::const_iterator i = discharge.begin();
             i != discharge.end(); ++i)
        {
            if (i->second)
            {
