...
namespace SAS
{
    ...
    public:
        ...
        const pddl_type *getType() const
        {
            return forThis;
        };

        PropertySpace *getSpace() const
        {
            return pspace;
        };
    ...
    public:
        TypesHolder(const pddl_type *pt, const PropertyState *ps, PropertySpace *psp, Property *p,
    ...
    public:
        ...
        {
            if (values.size() != vals.values.size())
            {
                return false;
            }
            for (unsigned int i = 0; i < values.size(); ++i)
            {
                if (!(*(values[i]) == *(vals.values[i])))
                {
                    return false;
                }
            };
        ...
        bool equivalent(const Values &vals) const
        {
            ...
            for (unsigned int i = 0; i < values.size(); ++i)
            {
                if (!values[i]->equivalent(vals.values[i]))
                {
                    return false;
                }
            };
    ...
    public:
        ...
        public:
            ...
            const PropertyState *forState() const
            {
                return (i->first);
            };
        };
        ...
        const_iterator end() const
        {
            return const_iterator(valuesUnion.end());
        };

        size_t size() const
        {
            return valuesUnion.size();
        };
    ...
    public:
        void push_back(const ValuesUnion &vu)
        {
            range.push_back(vu);
        };
    ...
    public:
        ...
        virtual VElement *build(FastEnvironment *fe)
        {
            return this;
        };

        virtual void showValue(ostream &o) const
        {
            write(o);
        };
    ...
    public:
        ...
        VarElement *copy() const
        {
            return new VarElement(*this);
        };
        ...
        const parameter_symbol *getVar() const
        {
            return var;
        };
    };
    ...
    public:
        ...
        ObElement *copy() const
        {
            return new ObElement(*this);
        };
    ...
    public:
        ...
        WildElement *copy() const
        {
            return new WildElement(*this);
        };
    ...
    public:
        ...
        PlaceHolderElement *copy() const
        {
            return new PlaceHolderElement(ph);
        };
    ...
    public:
        ValueElement(const PropertyState *p, const vector<VElement *> &vs) : pst(p), value(vs){};
        ...
        const PropertyState *getPS() const
        {
            return pst;
        };

        VElement *operator[](int i)
        {
            return value[i];
        };
        ...
        {
            if (value.size() != vel->value.size())
            {
                return false;
            }
            for (size_t i = 0; i < value.size(); ++i)
            {
                if (!value[i]->matches(vel->value[i], fenv))
                {
                    return false;
                }
            };
    ...
    public:
        ...
        bool tryMatchedPre(int k, instantiatedOp *iop, const var_symbol *var, SASActionTemplate *sasact,
                           ValueRep *vrep);
        void buildLayers();
        ...
        {
            return sasActionTemplates.begin();
        };

        iterator end() const
        {
            return sasActionTemplates.end();
        };
    };
