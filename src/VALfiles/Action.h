...
namespace VAL
{
    ...
    {
        safeaction(operator_symbol *nm, var_symbol_list *ps, goal *pre, effect_lists *effs,
                   var_symbol_table *st)
    ...
    protected:
        ...
        bool handleEffects(Ownership &o, EffectsRecord &e, const State *s, const effect_lists *effs,
                           const Environment &env, bool markPreCons) const;
        bool handleEffects(Ownership &o, EffectsRecord &e, const State *s, const effect_lists *effs,
    ...
    public:
        ...
        virtual void addTriggeredEvents(vector<const Action *> &triggeredEvents,
    ...
    public:
        InvariantAction(Validator *v, StartAction *sa, const action *a, const const_symbol_list *bs,
    ...
    public:
        DurativeActionElement(Validator *v, const action *a, const const_symbol_list *bs, double d,
                              const goal_list *ds, const InvariantAction *inv, const CtsEffectAction *ctsEff,
                              const vector<const CondCommunicationAction *> &cas, const plan_step *ps = 0)
          : Action(v, a, bs, ps), duration(d), durs(ds), invariant(inv), ctsEffects(ctsEff), condActions(cas)
        {
    ...
    public:
        ...
        StartAction(Validator *v, const action *a, const const_symbol_list *bs, const conj_goal *inv,
                    effect_lists *elc, double d, const goal_list *ds,
                    const vector<const CondCommunicationAction *> &cas,
          ...
          : DurativeActionElement(
                ...
                (inv->getGoals()->empty()) ? 0 :  // do not create invariant action if no invariants to check!
                                             new InvariantAction(v, this,
                                                                 ...
                                                                 bs, ps),
                (elc->assign_effects.empty() && elc->forall_effects.empty() && ccas.empty()) ?
                    0 :  // similarly do not create cts effect action if no cts effects!
                ...
                cas, ps)
        {
            if (inv->getGoals()->empty())
            {
                delete inv;
            }
            if (elc->assign_effects.empty() && elc->forall_effects.empty() && ccas.empty())
            {
                delete elc;
            }
        };
        ...
        void addTriggeredEvents(vector<const Action *> &triggeredEvents,
                                vector<const Action *> &oldTriggeredEvents,
                                vector<const StartAction *> &triggeredStartProcesses,
                                vector<const EndAction *> &triggeredEndProcesses) const;
    ...
    public:
        EndAction(Validator *v, const action *a, const const_symbol_list *bs, const StartAction *sa, double d,
                  const goal_list *ds, const plan_step *ps = 0)
    ...
    const Environment buildBindings(const operator_ *a, const T &bs)
    {
        ...
        for (var_symbol_list::iterator i = a->parameters->begin(); i != a->parameters->end(); ++i, ++j)
        {
            bindings[*i] = *j;
        };
        return bindings;
