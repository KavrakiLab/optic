...
namespace Inst
{
    ...
    public:
        BVEvaluator(PlanGraph &p, VAL::FastEnvironment *fe) : bval(0), pg(p), fenv(fe), continuous(false){};
    ...
    public:
        ...
        virtual void visit_simple_goal(simple_goal *s)
        {
            ...
            {
                evaluation = ((*f)[s->getProp()->args->front()] == (*f)[s->getProp()->args->back()]);
        ...
        virtual void visit_comparison(comparison *c)
        {
            ...
            switch (c->getOp())
            {
                ...
                case E_EQUALS:
                    evaluation =
                        (!bvres->gotLB() || bvres->getLB() <= 0) && (!bvres->gotUB() || bvres->getUB() >= 0);
                    break;
                default:
                    break;
            };
    ...
    public:
        SpikeSupporter(Spike<PropEntry> &s1, Spike<FluentEntry> &s2, FastEnvironment *fe, ActEntry *a,
                       GraphFactory *mf)
          : spes(s1)
          , sfes(s2)
          , f(fe)
        ...
        virtual void visit_simple_goal(simple_goal *s)
        {
            if (EPS(s->getProp()->head)->getParent() != this->equality)
            {
                ...
                if (eid)
                {
                    if (context && s->getPolarity() == E_NEG || !context && s->getPolarity() == E_POS)
                    {
    ...
    BoundedValue *PlanGraph::update(BoundedValue *bv, const VAL::expression *exp, const VAL::assign_op op,
    ...
    void FluentEntry::addUpdatedBy(ActEntry *ae, const VAL::expression *expr, const VAL::assign_op op,
                                   PlanGraph *pg)
    {
    ...
    PlanGraph::PlanGraph(GraphFactory *f)
      : myFac(f), inactive(instantiatedOp::opsBegin(), instantiatedOp::opsEnd())
    {
    ...
    struct IteratingActionChecker : public VisitController
    {
        ...
        {
            for (VAL::pc_list<assignment *>::iterator i = effs->assign_effects.begin();
    ...
    ActEntry::ActEntry(instantiatedOp *io) : theact(io), iterating(false), atype(ATOMIC), dur(0)
    {
