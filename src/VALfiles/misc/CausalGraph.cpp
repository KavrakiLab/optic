...
namespace SAS
{
    ...
    CausalGraph::CausalGraph()
    {
        ...
        for (FunctionStructure::iterator i = fs.begin(); i != fs.end(); ++i)
        {
            ...
            for (SASActionTemplate::iterator j = i->second->precondsBegin(); j != i->second->precondsEnd();
                 ++j)
            {
                pres.insert(j->second.begin(), j->second.end());
            };
            for (SASActionTemplate::iterator j = i->second->postcondsBegin(); j != i->second->postcondsEnd();
                 ++j)
            {
