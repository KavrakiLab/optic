...
namespace SAS
{
    ...
    bool SegmentRep::growOneLevel(const pddl_type *pt, const TIMobjectSymbol *tob, FunctionStructure *fs)
    {
        ...
        for (int i = last; i < sz; ++i)
        {
                 ...
                 j != SASActionTemplate::findOps(pt, seg).end(); ++j)
            {
                ...
                for (OpStore::iterator iop = instantiatedOp::from(k); k != ke; ++k, ++iop)
                {
                    ...
                    activated |= fs->tryMatchedPre(k, *iop, j->first, j->second, valuereps[i]);
                };
            };
        };
        return activated;
    ...
    bool SASActionTemplate::checkPre(FunctionStructure *fs, FastEnvironment *fenv, const var_symbol *v,
                                     ValueRep *vrep)
    {
        //	cout << "Checking " << *this << "\n";
        for (vector<ValueRep *>::iterator i = preconditions[v].begin(); i != preconditions[v].end(); ++i)
        {
            if (vrep->matches(*i, fenv))
            {
                return true;
            }
        };
