...
namespace SAS
{
    ...
    public:
        ValueRep(const pddl_type *pt, int s, ValueElement *v) : ptp(pt), segment(s), vel(v){};
        ...
        const ValueElement *getValue() const
        {
            return vel;
        };
    ...
    public:
        ...
        bool growOneLevel(const pddl_type *pt, const TIMobjectSymbol *tob, FunctionStructure *fs);

        void cap()
        {
            levelcounts.push_back(valuereps.size());
        };
    };
    ...
    public:
        RangeRep(const pddl_type *pt, const TIMobjectSymbol *tob, vector<ValueElement *> &vels)
          : group(pt), object(tob)
        {
    ...
    typedef map<const pddl_type *, vector<vector<pair<const var_symbol *, SASActionTemplate *>>>> PreMap;
    ...
    public:
        SASActionTemplate(operator_ *o, const VMap &pres, const VMap &posts, const vector<proposition *> &sts,
                          const vector<proposition *> &opres, const vector<proposition *> &oposts)
          : op(o)
          , preconditions(pres)
          ...
          , otherposts(oposts)
        {
            for (VMap::const_iterator i = pres.begin(); i != pres.end(); ++i)
            {
                for (vector<ValueRep *>::const_iterator j = i->second.begin(); j != i->second.end(); ++j)
                {
                    ...
                    preMap[i->first->type][(*j)->getSegment()].push_back(make_pair(i->first, this));
                };
            };
        ...
        iterator precondsBegin() const
        {
            return preconditions.begin();
        };

        iterator precondsEnd() const
        {
            return preconditions.end();
        };

        iterator postcondsBegin() const
        {
            return postconditions.begin();
        };

        iterator postcondsEnd() const
        {
            return postconditions.end();
        };
