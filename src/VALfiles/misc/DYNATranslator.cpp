...
namespace VAL
{
    ...
    void DYNATranslator::write_forall_effect(ostream &o, const forall_effect *p)
    {
        ...
        if (p->getEffects()->cond_effects.empty())
        {
            ...
            for (var_symbol_table::const_iterator i = p->getVars()->begin(); i != p->getVars()->end(); ++i)
            {
                ...
                {
                    o << "(or ";
                    for (pddl_type_list::const_iterator j = i->second->either_types->begin();
        ...
        {
            if (p->getEffects()->cond_effects.size() == 1 && p->getEffects()->add_effects.empty() &&
                p->getEffects()->del_effects.empty() && p->getEffects()->forall_effects.empty() &&
                p->getEffects()->timed_effects.empty())
            {
                o << "(when (and ";
                for (var_symbol_table::const_iterator i = p->getVars()->begin(); i != p->getVars()->end();
                     ++i)
                {
                    if (i->second->type)
                    {
    ...
    {
        if (p->ts == filter || p->ts == E_CONTINUOUS)
        {
            o << *(p->effs);
        }
    };
    ...
    void DYNATranslator::write_effect_lists(ostream &o, const effect_lists *p)
    {
        bool tt = (p->add_effects.size() + p->del_effects.size() + p->forall_effects.size() +
                       p->cond_effects.size() + p->assign_effects.size() + p->timed_effects.size() >
                   1);
    ...
    void collect_symbols(var_symbol_list &vs, const expression *e)
    {
        ...
        {
            for (parameter_symbol_list::const_iterator i = f->getArgs()->begin(); i != f->getArgs()->end();
                 ++i)
            {
    ...
    bool State::nextBinding(Environment &bs, const func_term *fe)
    {
        ...
        if (ps == fe->getArgs()->end())
        {
            return false;
        }
        const var_symbol *v = dynamic_cast<const var_symbol *>(*ps);
        const_symbol *c = const_cast<const_symbol *>(bs[v]);
        set<const_symbol *>::iterator i = records[fe->getFunction()][x].find(c);
        if (i == records[fe->getFunction()][x].end())
        {
            cout << "Funny thing\n";
        }
        ++i;
    ...
    {
        int x = 0;
        for (parameter_symbol_list::const_iterator i = fe->getArgs()->begin(); i != fe->getArgs()->end();
             ++i, ++x)
        {
            const var_symbol *ps = dynamic_cast<const var_symbol *>(*i);
            if (bs.find(ps) == bs.end())
            {
                if (records[fe->getFunction()][x].empty())
                {
                    return false;
                }
