...
namespace Planner
{
    ...
    PartialPlan::PartialPlan()
      ...
      , planHead(SearchSpace::instance().getVal(), current_analysis->the_domain->ops,
                 SearchSpace::instance().getDummyPlan())
      , currentStep(planHead.begin())
      , possibleActions(
            SearchSpace::instance().getPG().applicableActions(SearchSpace::instance().getVal(), &current))
      , relevantActions(
            SearchSpace::instance().getPG().relevantActions(SearchSpace::instance().getVal(), &current))
      , waited(false)
      , pending(0){};
    ...
    void PartialPlan::selfCommit()
    {
        ...
        {
            Action *a = new Action(SearchSpace::instance().getVal(), (*i)->getIO()->forOp(),
                                   (*i)->getIO()->getEnv()->getCore());
    ...
    {
        pending = new Happening(SearchSpace::instance().getVal(), current.getTime() + t,
                                vector<pair<double, Action *>>());
    ...
    double PartialPlan::timeToTrigger()
    {
        ...
        for (vector<Inst::ActEntry *>::const_iterator i = relevantActions.begin(); i != relevantActions.end();
             ++i)
        {
            cout << **i << "\n";
        };
    ...
    void PartialPlan::initialWait(double t)
    {
        ...
        SearchSpace::instance().getVal()->getEvents().triggerInitialEvents(SearchSpace::instance().getVal(),
                                                                           t);
    ...
    void PartialPlan::write(ostream &o) const
    {
        ...
        for (map<const FuncExp *, ActiveFE *>::const_iterator i =
                 currentStep.getActiveCtsEffects()->activeFEs.begin();
        ...
        o << currentStep.getActiveCtsEffects()->ctsEffects
          << "\nInvs: " << currentStep.getExecutionContext()->invariants << "\nPlanHead: ";
        planHead.show(o);
        o << "\nState: " << current << "\nApplicable actions: ";
        for (vector<Inst::ActEntry *>::const_iterator i = possibleActions.begin(); i != possibleActions.end();
             ++i)
        {
            o << **i << " ";
        };
