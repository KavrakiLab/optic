                  ...
                  double tolerance, bool lengthDefault, bool giveAdvice)
{
    ...
    while (argcount < argc)
    {
        ...
        PlanRepair pr(timedInitialLiteralActions, deadLine, derivRules, tolerance, tc,
                      current_analysis->the_domain->ops, current_analysis->the_problem->initial_state,
                      the_plan, planNoTimedLits, current_analysis->the_problem->metric, lengthDefault,
                      current_analysis->the_domain->isDurative(), current_analysis->the_problem->the_goal,
                      current_analysis);

        PlanExecutionTracker pet(State(&(pr.getValidator()), current_analysis->the_problem->initial_state),
                                 &(pr.getValidator()));
    ...
    if (!rnk.empty())
    {
        ...
        if (current_analysis->the_problem->metric && current_analysis->the_problem->metric->opt == E_MINIMIZE)
        {
            if (LaTeX)
            {
...
int main(int argc, char *argv[])
{
    ...
    const DerivationRules *derivRules =
        new DerivationRules(current_analysis->the_domain->drvs, current_analysis->the_domain->ops);
