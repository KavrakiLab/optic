...
namespace VAL
{
    ...
    PinguPlanGen::PinguPlanGen(char *name)
      : VisitController(), lastAt("theStart"), count(0), pingu(0), lastMoved(0), mustDelay(true)
    {
    ...
    void PinguPlanGen::doThis(string a, string p)
    {
        ...
        ++count;
        cout << "(action (name " << a << "er) (position " << pp.first << " " << pp.second
             << " 0) (state blocker) (time " << count << "))\n";
        mustDelay = true;
    };
    ...
    void PinguPlanGen::visit_plan_step(plan_step *ps)
    {
        ...
        else if (s == "jump" || s == "block" || s == "float" || s == "bomb" || s == "climb")
        {
            doAction(s, ps);
        }
