...
namespace VAL
{
    ...
    {
        if (act->name != ps.op_sym)
        {
            return false;
        }

        var_symbol_list::const_iterator i = act->parameters->begin();
        for (const_symbol_list::const_iterator j = ps.params->begin(); j != ps.params->end(); ++j, ++i)
        {
    ...
    void Action::addTriggeredEvents(vector<const Action *> &triggeredEvents,
    ...
    void EndAction::addTriggeredEvents(vector<const Action *> &triggeredEvents,
                                       vector<const Action *> &oldTriggeredEvents,
                                       vector<const StartAction *> &triggeredStartProcesses,
                                       vector<const EndAction *> &triggeredEndProcesses) const
    {
    ...
    void StartAction::addTriggeredEvents(vector<const Action *> &triggeredEvents,
                                         vector<const Action *> &oldTriggeredEvents,
                                         vector<const StartAction *> &triggeredStartProcesses,
                                         vector<const EndAction *> &triggeredEndProcesses) const
    {
    ...
    void InvariantAction::addErrorRecord(double t, const State *s) const
    {
        try
        {
            vld->getErrorLog().addUnsatInvariant(t - pre->getEndOfInterval(), t, pre->getIntervals(s), this,
                                                 s);
        }
    ...
    bool DurativeActionElement::confirmPrecondition(const State *s) const
    {
        ...
        if (Robust)  // test the duration constraint according to the original duration given in the plan
        {
            ...
            testTolerance = 0.001;  // 0.00000001; //1e-08, to within acceptable limits for calculations,
        ...
        for (goal_list::const_iterator i = durs->begin(); i != durs->end(); ++i)
        {
            ...
            switch (c->getOp())
            {
                ...
                case E_GREATEQ:
                    ...
                    if (!test && Verbose)
                    {
                        ...
                        *report << "Tolerance of " << d - testDuration << " required for " << this << "\n";
                    };
                    break;
                case E_LESSEQ:
                    ...
                    if (!test && Verbose)
                    {
                        ...
                        *report << "Tolerance of " << testDuration - d << " required for " << this << "\n";
                    };
                    break;
                case E_EQUALS:
                    ...
                    if (!test && Verbose)
                    {
                        ...
                        *report << "Tolerance of "
                                << ((testDuration > d) ? testDuration - d : d - testDuration)
                                << " required for " << this << "\n";
                    };
                    break;
                default:
            ...
            if (!test)
            {
                ...
                if (ErrorReport)
                {
                    ...
                    s->getValidator()->getErrorLog().addUnsatDurationCondition(time, this, s,
                                                                               fabs(d - testDuration));
                };
                return false;
            };
        ...
        for (goal_list::const_iterator i = durs->begin(); i != durs->end(); ++i)
        {
            ...
            switch (c->getOp())
            {
                case E_GREATER:
                    ...
                    if (!test)
                    {
                        *report << "Failed duration constraint: Increase duration by at least "
                                << d - testDuration;
                ...
                case E_LESS:
                    ...
                    if (!test)
                    {
                        *report << "Failed duration constraint: Decrease duration by at least "
                                << testDuration - d;
                        if (LaTeX)
                        {
                            *report << "\\\\";
    ...
    // violated?
    bool Action::constructEffects(Ownership &o, EffectsRecord &e, const State *s, bool markPreCons) const
    {
    ...
    struct FAEhandler
    {
        ...
        bool handle(bool markPreCons)
        {
            ...
            {
                // cout << "Handling " << (*j)->getName() << " = " << (*k)->getName() << " and i: " <<
                // (i==endpt) << "\n";
                bds[*j] = *k;
    ...
    bool Action::handleEffects(Ownership &o, EffectsRecord &e, const State *s, const effect_lists *effs,
                               const Environment &bds, bool markPreCons) const
    {
    ...
    void buildForAllCondActions(Validator *vld, const durative_action *da, const const_symbol_list *params,
                                goal_list *gls, goal_list *gli, goal_list *gle, effect_lists *locels,
                                ...
                                vector<const CondCommunicationAction *> &condActions, Environment *env)
    {
        ...
        if (i == vars->end())
        {
            ...
            condActions.push_back(
                new CondCommunicationAction(vld, da, params, gls, gli, gle, locels, locele, env));
        }
        else
        {
            ...
            for (vector<const_symbol *>::iterator j = vals.begin(); j != vals.end(); ++j)
            {
                ...
                (*env)[v] = *j;
                buildForAllCondActions(vld, da, params, gls, gli, gle, locels, locele, vars, i, condActions,
                                       env);
            };
            --i;
        }
    ...
    CondCommunicationAction::CondCommunicationAction(Validator *v, const durative_action *a,
                                                     const const_symbol_list *bs, goal_list *gs,
                                                     goal_list *gi, goal_list *ge, effect_lists *es,
                                                     effect_lists *el)
      : Action(
            v,
            new safeaction(a->name, a->parameters, new conj_goal(const_cast<goal_list *>(ge)), el, a->symtab),
            bs)
      , status(true)
    ...
    CondCommunicationAction::~CondCommunicationAction()
    {
        ...
        if (cg)
        {
            const_cast<goal_list *>(cg->getGoals())->clear();
        }
    ...
    void EndAction::adjustContextInvariants(ExecutionContext &ec) const
    {
        if (invariant != 0)
        {
            invariant->setRhsIntervalOpen(true);
        }
        for (vector<const CondCommunicationAction *>::const_iterator i = condActions.begin();
             i != condActions.end(); ++i)
        {
    ...
    void StartAction::adjustContextInvariants(ExecutionContext &ec) const
    {
        if (invariant != 0)
        {
            invariant->setRhsIntervalOpen(false);
        }
        for (vector<const CondCommunicationAction *>::const_iterator i = condActions.begin();
             i != condActions.end(); ++i)
        {
    ...
    bool CondCommunicationAction::constructEffects(Ownership &o, EffectsRecord &e, const State *s,
                                                   bool markPreCons) const
    {
        return true;
    };
    ...
    void CtsEffectAction::displayCtsFtns() const
    {
        ...
        for (map<const FuncExp *, ActiveFE *>::const_iterator afe = ace->activeFEs.begin();
    ...
    bool CtsEffectAction::constructEffects(Ownership &o, EffectsRecord &e, const State *s,
                                           bool markPreCons) const
    ...
    const Action *StartAction::partner() const
    {
        return otherEnd;
    };

    const Action *InvariantAction::partner() const
    {
        return start;
    };

    const Action *CtsEffectAction::partner() const
    {
        return start;
    };

    const Action *CondCommunicationAction::partner() const
    {
        return start;
    };
