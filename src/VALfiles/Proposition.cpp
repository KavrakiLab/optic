...
namespace VAL
{
                           ...
                           const pair<intervalEnd, intervalEnd> &int1)
    {
        ...
        someIntervals.erase(std::remove(someIntervals.begin(), someIntervals.end(), int1),
                            someIntervals.end());

        return isPointInInterval(p, someIntervals);
    };

    pair<intervalEnd, intervalEnd> getIntervalFromPt(intervalEnd p,
                                                     ...
                                                     const pair<intervalEnd, intervalEnd> &int1)
    {
        ...
        {
            someIntervals.push_back(int1);  // if repeated make sure not all occurances are deleted
        }

        for (vector<pair<intervalEnd, intervalEnd>>::const_iterator i = someIntervals.begin();
             i != someIntervals.end(); ++i)
        {
            if ((p.first >= i->first.first) && (p.first <= i->second.first))
            {
                // check point is not at end of interval where end is open(assuming point is at start of
                // interval)
    ...
    Intervals setUnion(const Intervals &ints1, const Intervals &ints2)
    {
        ...
        for (vector<pair<intervalEnd, intervalEnd>>::const_iterator i = ints1.intervals.begin();
             i != ints1.intervals.end(); ++i)
        {
            theIntervals.push_back(*i);
        }

        for (vector<pair<intervalEnd, intervalEnd>>::const_iterator i1 = ints2.intervals.begin();
        ...
        // find upper limit
        for (vector<pair<intervalEnd, intervalEnd>>::iterator i2 = theIntervals.begin();
             i2 != theIntervals.end(); ++i2)
        {
            if (upperLimit < i2->second.first)
            {
                upperLimit = i2->second.first + 1;
            }
        };
        ...
        for (unsigned int count = 0; count != theIntervals.size(); ++count)
        {
            ...
            for (vector<pair<intervalEnd, intervalEnd>>::iterator i = theIntervals.begin();
                 i != theIntervals.end(); ++i)
            {
                if ((i->first.first >= progress) && (i->first.first <= startPt.first))
                {
                    if (!((i->second.first == aInterval.second.first) &&
                          (aInterval.second.second)))  // check new interval is adding anything (ie not a
                                                       // point at a closed end)
                    {
            ...
            if (startPt.first == upperLimit)
            {
                break;
            }

            // set closure of start point
            for (vector<pair<intervalEnd, intervalEnd>>::iterator i1 = theIntervals.begin();
            ...
            for (unsigned int count2 = 0; count2 != theIntervals.size() + 1; ++count2)
            {
                     ...
                     i != theIntervals.end(); ++i)
                {
                    if (((isPointInInterval(i->first.first, aInterval) &&
                          !((i->first.first == aInterval.second.first) && !(i->first.second) &&
                            !(aInterval.second.second))) ||
                 ...
                 i != theIntervals.end(); ++i)
            {
                if ((i->first.first >= progress) && (i->first.first <= startPt.first))
                {
                    if (!((i->first.first == aInterval.first.first) &&
                          (i->second.first == aInterval.second.first)))  // check not adding a single point
                                                                         // again
                    {
    ...
    Intervals setComplement(const Intervals &ints, double endPoint)
    {
        ...
        for (vector<pair<intervalEnd, intervalEnd>>::const_iterator i1 = ints.intervals.begin();
    ...
    pair<bool, bool> hasDP(const goal *g, Environment *env, const Validator *vld, bool dp, bool comp)
    {
        ...
        if (dynamic_cast<const simple_goal *>(g))
        {
            ...
            for (map<string, pair<const goal *, const var_symbol_table *>>::const_iterator i =
                     derivPreds.begin();
                 i != derivPreds.end(); ++i)
    ...
    bool ConjGoal::evaluate(const State *s, vector<const DerivedGoal *> DPs) const
    {
        ...
        for (vector<const Proposition *>::const_iterator i1 = conditions.begin(); i1 != conditions.end();
             ++i1)
        {  // cout << **i1 <<" conj\n";
            if (!(*i1)->evaluate(s, DPs))
            {
                return false;
            }
        };
    ...
    ConjGoal::markOwnedPreconditions(const Action *a, Ownership &o, ownership w) const
    {
        for (vector<const Proposition *>::const_iterator i = gs.begin(); i != gs.end(); ++i)
        {
            if (!(*i)->markOwnedPreconditions(a, o, w))
            {
                return false;
            }
        }
    ...
    set<const SimpleProposition *> ConstraintGoal::getLiterals() const
    {
        ...
        if (trigger)
        {
            ...
            for (set<const SimpleProposition *>::const_iterator i = literals1.begin(); i != literals.end();
                 ++i)
            {
                literals.insert(*i);
            };
    ...
    void ConstraintGoal::write(ostream &o) const
    {
        ...
        switch (constraint->getCons())
        {
            ...
            case E_SOMETIMEBEFORE:
                o << "sometime-before " << *(getTrigger()) << " " << *(getRequirement());
                break;
            case E_ALWAYSWITHIN:
                ...
                break;
            case E_HOLDDURING:
                o << "hold-during " << getFrom() << " " << getDeadline() << " " << getRequirement();
                break;
            case E_HOLDAFTER:
    ...
    bool DisjGoal::evaluate(const State *s, vector<const DerivedGoal *> DPs) const
    {  // cout << gs.size() << " disj size\n";
        ...
        if (rnk.second == -1)
        {
            ...
            for (vector<const Proposition *>::const_iterator i = conditions.begin(); i != conditions.end();
                 ++i)
            {
                if ((*i)->evaluate(s, DPs))
                {
        ...
        try
        {
                 ...
                 ++i)
            {
                ...
                if ((theAns.intervals.size() == 1) && (theAns.intervals.begin()->first.first == 0) &&
        ...
        catch (InvariantDisjError ide)
        {
            if (InvariantWarnings)
            {
                if (LaTeX)
                {
                    s->getValidator()->addInvariantWarning(getPropString(s) +
                                                           ",   for values of $t$ in $( 0 , " +
            ...
            else
            {
                if (LaTeX)
                {
                    *report << "\\\\\n \\> \\listrow{Invariant: " + getPropString(s) + ", $t \\in ( 0 , " +
                                   toString(endOfInterval) + " )$}\\\\\n";
                }
                else
                {
    ...
    bool DisjGoal::markOwnedPreconditions(const Action *a, Ownership &o, ownership w) const
    {
        for (vector<const Proposition *>::const_iterator i = gs.begin(); i != gs.end(); ++i)
        {
            if (!(*i)->markOwnedPreconditions(a, o, E_PRE))
            {
                return false;
            }
        }
    ...
    bool SimpleProposition::evaluate(const State *s, vector<const DerivedGoal *> DPs) const
    {
    ...
    bool SimpleProposition::markOwnedPreconditions(const Action *a, Ownership &o, ownership w) const
    {
        return o.markOwnedPrecondition(a, this, w);
    };

    bool DerivedGoal::markOwnedPreconditions(const Action *a, Ownership &o, ownership w) const
    {
    ...
    bool Comparison::evaluate(const State *s, vector<const DerivedGoal *> DPs) const
    {
        if (ctsFtn == 0)
        {
            return evaluateAtPoint(s);
        }
      ...
      if(someIntervals.intervals.size() != 1 || (*someIntervals.intervals.begin()).first.first != 0 ||
      (*someIntervals.intervals.begin()).second.first != endOfInterval) return false; else return true;
          */
    };

    bool Comparison::markOwnedPreconditions(const Action *a, Ownership &o, ownership w) const
    {
        return o.markOwnedPreconditionFEs(a, comp->getLHS(), bindings) &&
               o.markOwnedPreconditionFEs(a, comp->getRHS(), bindings);
    };

    bool ImplyGoal::evaluate(const State *s, vector<const DerivedGoal *> DPs) const
    {
    ...
    bool ImplyGoal::markOwnedPreconditions(const Action *a, Ownership &o, ownership w) const
    {
        return (ant->markOwnedPreconditions(a, o, E_PRE) && cons->markOwnedPreconditions(a, o, E_PRE));
    };

    bool NegGoal::evaluate(const State *s, vector<const DerivedGoal *> DPs) const
    {
        return !(p->evaluate(s, DPs));
    };

    bool NegGoal::markOwnedPreconditions(const Action *a, Ownership &o, ownership w) const
    ...
    string Comparison::getPropString(const State *s) const
    {
        ...
        if (LaTeX)
        {
            ans = "$" + toString(ctsFtn) + " " + op + " " + toString(offSet) + "$ for $t\\in$ (0," +
                  toString(endOfInterval);
            if (rhsIntervalOpen)
            {
                ans += ")";
    ...
    set<const SimpleProposition *> ConjGoal::getLiterals() const
    {
        ...
        for (vector<const Proposition *>::const_iterator i = gs.begin(); i != gs.end(); ++i)
        {
            ...
            for (set<const SimpleProposition *>::const_iterator j = someLiteralsPNEs.begin();
    ...
    set<const SimpleProposition *> ImplyGoal::getLiterals() const
    {
        ...
        for (set<const SimpleProposition *>::const_iterator j = someLiterals.begin(); j != someLiterals.end();
             ++j)
        {
            literalspnes.insert(*j);
        }
    ...
    Intervals DerivedGoal::getIntervals(const State *s) const
    {
        ...
        if ((j->first.first == 0) && (j->first.second == true) && (j->second.first == endOfInterval) &&
    ...
    Intervals Comparison::getIntervals(const State *s) const
    {
        ...
        // cout << " \\\\ \\> The ctsFtn $"<<*ctsFtn<<"$ is satisfied on "<<theAns<<" until
        // "<<endOfInterval<<"\\\\\n";
        return theAns;
    };
    ...
    bool isExpressionConstant(const expression *e, const ActiveCtsEffects *ace, const Environment &bs,
                              CoScalar endInt)
    {
        ...
        {
            return isExpressionConstant(dynamic_cast<const minus_expression *>(e)->getLHS(), ace, bs,
                                        endInt) &&
        ...
        {
            return isExpressionConstant(dynamic_cast<const plus_expression *>(e)->getLHS(), ace, bs,
                                        endInt) &&
        ...
        {
            return isExpressionConstant(dynamic_cast<const uminus_expression *>(e)->getExpr(), ace, bs,
                                        endInt);
        };
    ...
    void Comparison::setUpComparisons(const ActiveCtsEffects *ace, bool rhsOpen)
    {
        ...
        if (const func_term *fexpression = dynamic_cast<const func_term *>(comp->getLHS()))
        {
            ...
            {  // cout << " cts ftn = "<<  *(i->second->ctsFtn) <<"\n";
                ...
                if (fndefined)
                {
                    if (isExpressionConstant(comp->getRHS(), ace, bindings, endOfInterval))  // check is
                                                                                             // constant!
                    {
        ...
        else if (const func_term *fexpression = dynamic_cast<const func_term *>(comp->getRHS()))
        {
            ...
            if (i != ace->activeFEs.end())
            {
                ...
                if (fndefined)
                {
                    if (isExpressionConstant(comp->getLHS(), ace, bindings, endOfInterval))  // check is
                                                                                             // constant!
                    {
        ...
        if (defined)
        {
            ...
            else if (expon != 0)
            {
                ...
                if ((comp->getOp() == E_GREATER) || (comp->getOp() == E_GREATEQ) ||
                    (comp->getOp() == E_EQUALS))
                {
                    exponential = new Exponential(expon->getK(), new Polynomial(*expon->getPolynomial()),
                                                  expon->getc());
    ...
    pair<int, int> SimpleProposition::rank() const
    {
        return make_pair(0, 0);
    };
    ...
    const Proposition *PropositionFactory::buildProposition(const goal *g, bool buildNewLiterals,
                                                            const State *state)
    {
    ...
    */
    ...
    string DerivedGoal::getDPName() const
    {
        ...
        for (parameter_symbol_list::const_iterator i = prop->args->begin(); i != prop->args->end(); ++i)
        {
            if (dynamic_cast<const var_symbol *>(*i))
    ...
    void DerivedGoal::resetLists(const State *s)
    {
        ...
        /* set<const FuncExp *> changedPNEs = s->getChangedPNEs();
         ...
         else
         {
            ...
            for(set<const SimpleProposition *>::const_iterator cl = changedLiterals.begin(); cl !=
         changedLiterals.end(); ++cl)
            {
                ...
                for(map<string,bool>::iterator e = evals.begin(); e != evals.end(); ++e)
                {
                 ...
                 //if the literal affects the DP then remove it from the cache since its value may have
         changed set<const SimpleProposition *>::const_iterator j = (i->second).find(*cl); if(j !=
         (i->second).end())
                 {
    ...
    DerivedGoal::evaluate(const State *s, vector<const DerivedGoal *> DPs) const
    {
        ...
        if (ace != 0)
        {
            const_cast<Proposition *>(deriveFormula)->setUpComparisons(ace, rhsOpen);
        }
        ...
        if (visited(dpName))
        {
            for (vector<const DerivedGoal *>::iterator i = DPs.begin(); i != DPs.end(); ++i)
            {
                (*i)->setRevisit(true);
            }
            return false;
    ...
    void QfiedGoal::create() const
    {
        ...
        {
            props.push_back(
                vld->pf.buildProposition(qg->getGoal(), *(env->copy(vld)), createLiterals, &vld->getState()));
        }
        else
        {
    ...
    {
        bool disjConjEval;
        // get list of instansiated parameters for the qfied prop, so for forall(?z ?y), we have a grounded
        // list for every z? and ?y
        set<var_symbol *> svs = getVariables(qg);
        vector<const_symbol_list *> constantsList = defineUndefinedParameters(
            newBlankConstSymbolList(const_cast<var_symbol_list *>(qg->getVars()), vld),
        ...
        for (vector<const_symbol_list *>::iterator k = constantsList.begin(); k != constantsList.end(); ++k)
        {
            ...
            const Proposition *propToCheck = vld->pf.buildProposition(
                qg->getGoal() /* aGoal */, bindings, false, s);  // cout << *propToCheck <<" \n";
    ...
    {
        // create new bindings from the subset of bindings that a derived predicate has based on the actions
        // bindings
    ...
    const Proposition *PropositionFactory::buildProposition(const goal *g, const Environment &bs,
                                                            bool buildNewLiterals, const State *state)
    {
        ...
        if (const conj_goal *cg = dynamic_cast<const conj_goal *>(g))
        {
            ...
            std::transform(cg->getGoals()->begin(), cg->getGoals()->end(), std::back_inserter(gs),
        ...
        if (const disj_goal *dg = dynamic_cast<const disj_goal *>(g))
        {
            ...
            std::transform(dg->getGoals()->begin(), dg->getGoals()->end(), std::back_inserter(gs),
                           buildProp(this, bs, buildNewLiterals, state));
        ...
        if (const simple_goal *sg = dynamic_cast<const simple_goal *>(g))
        {
            ...
            if (dg != 0)
            {
                ...
                if (visited(dpName))
                {
                    ...
                    {
                        return new NegGoal(
                            new neg_goal(new simple_goal(const_cast<proposition *>(sg->getProp()), E_POS)),
                ...
                const DerivedGoal *newDP = new DerivedGoal(
                    sg->getProp(),
                    buildProposition(dg, *(buildBindings(vst, sg, bs).copy(vld)), buildNewLiterals, state),
                    bs);
                ...
                {
                    newProp = new NegGoal(
                        new neg_goal(new simple_goal(const_cast<proposition *>(sg->getProp()), E_POS)), newDP,
                        bs);
                };
            ...
            else
            {
                ...
                // if we do not want to create literals create true or false literals - which is state
                // dependent of course, so must be used immediatley
                ...
                else
                {
                    ...
                    if (i1 != literals.end())
                    {
                        literalisTrue = i1->second->evaluate(state);
                    }
                };
                if (sg->getPolarity() != E_POS)
                {
        ...
        {
            return new PreferenceGoal(vld, p, buildProposition(p->getGoal(), bs, buildNewLiterals, state),
                                      bs);
        };
    ...
    */
    ...
    const_symbol_list *SimpleProposition::getConstants(var_symbol_list *variables, parameter_symbol_list *psl,
                                                       Validator *vld) const
    {
                ...
                if (*ps == *vs)
                {
                    ...
                    else
                    {
                        theConstants->push_back(
                            const_cast<const_symbol *>(dynamic_cast<const const_symbol *>(*i)));
                    };
                    variableFound = true;
                };
                ++ps;
    ...
    string Comparison::getExprnString(const expression *e, const Environment &bs) const
    {
        ...
        if (dynamic_cast<const mul_expression *>(e))
        {
            if (LaTeX)
            {
                return "(" + getExprnString(dynamic_cast<const mul_expression *>(e)->getLHS(), bs) +
                       " $\\times$ " + getExprnString(dynamic_cast<const mul_expression *>(e)->getRHS(), bs) +
                       ")";
            }
        ...
        if (const func_term *ft = dynamic_cast<const func_term *>(e))
        {
            ...
            for (parameter_symbol_list::const_iterator i = ft->getArgs()->begin(); i != ft->getArgs()->end();
                 ++i)
            {
                s += " ";
    ...
    string Comparison::getExprnString(const expression *e, const Environment &bs, const State *s) const
    {
        ...
        if (const special_val_expr *sp = dynamic_cast<const special_val_expr *>(e))
        {
            if (sp->getKind() == E_TOTAL_TIME)
            {
                if (s->getValidator()->durativePlan())
                {
                    return toString(s->getTime());
                }
...

    {
        ...
        if (!evaluate(s))
        {
            apl->changeAdvice(true, this, true);
        }
    ...
    const AdviceProposition *Comparison::getAdviceProp(const State *s) const
    {
        if (evaluate(s))
        {
            return new AdvicePropositionComp(false, this, "", false);
        }
        ...
        if (ctsFtn != 0)
        {
            apc = new AdvicePropositionComp(true, this,
                                            "Invariant " + getPropString(s) + " is only satisfied on " +
    ...
    const AdviceProposition *ConjGoal::getAdviceProp(const State *s) const
    {
        ...
        for (vector<const Proposition *>::const_iterator i = gs.begin(); i != gs.end(); ++i)
        {
            if (!((*i)->evaluate(s)))
            {
                apc->addAdviceProp((*i)->getAdviceProp(s));
            }
        };
    ...
    const AdviceProposition *SimpleProposition::getAdviceNegProp(const State *s) const
    {
        ...
        if (evaluate(s))
        {
            apl->changeAdvice(true, this, false);
        }
    ...
    const AdviceProposition *Comparison::getAdviceNegProp(const State *s) const
    {
        if (!evaluate(s))
        {
            return new AdvicePropositionComp(false, this, "", true);
        }
        AdvicePropositionComp *apc;
    ...
    const AdviceProposition *DisjGoal::getAdviceNegProp(const State *s) const
    {
        ...
        for (vector<const Proposition *>::const_iterator i = gs.begin(); i != gs.end(); ++i)
        {
            if ((*i)->evaluate(s))
            {
                apc->addAdviceProp((*i)->getAdviceNegProp(s));
            }
        };
    ...
    const AdviceProposition *ImplyGoal::getAdviceNegProp(const State *s) const
    {
        ...
        if (!(ant->evaluate(s)))
        {
            apc->addAdviceProp(ant->getAdviceProp(s));
        }
        if (cons->evaluate(s))
        {
            apc->addAdviceProp(cons->getAdviceNegProp(s));
        }
