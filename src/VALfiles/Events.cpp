...
namespace VAL
{
    ...
    Events::~Events()
    {
        ...
        for (vector<const Action *>::iterator i = oldTriggeredEvents.begin(); i != oldTriggeredEvents.end();
             ++i)
        {
            delete *i;
        }

        for (map<const StartAction *, pair<const Proposition *, const_symbol_list *>>::iterator j =
    ...
    bool Events::isProcessActive(process *pro, const_symbol_list *csl) const
    {
        ...
        for (map<const StartAction *, pair<const Proposition *, const_symbol_list *>>::const_iterator i =
                 activeProcesses.begin();
    ...
    const EndAction *addEndProcess(vector<const Action *> &processes, const StartAction *sa,
                                   const_symbol_list *csl, Validator *v)
    {
        // create end action to end the process
        const operator_ *pro = sa->getAction();
        action *dae = new safeaction(pro->name, pro->parameters, new conj_goal(new goal_list()),
    ...
    void Events::updateHappeningTime(double t)
    {
        if (lastHappeningTime != t)
        {
            ...
            triggeredProcesses.clear();  // to ensure a process is not triggered and untriggered at same time
                                         // point due to rounding errors
            untriggeredProcesses.clear();  // to ensure a processes is not untriggered and then retriggered at
                                           // the same time point due to rounding when triggering processes by
                                           // discrete change!
        };
        lastHappeningTime = t;
    };
    ...
    bool Events::triggerInitialEvents(Validator *v, double firstHappeningTime)
    {
        ...
        if (ace->hasCtsEffects() && (isOK || ContinueAnyway))  // if processes have been activated in the
                                                               // intitial state then there may be events to
                                                               // trigger and we need to update values before
                                                               // the first happening
        {
    ...
    bool checkPreconditionsAreNotSatisfied(const State *s, const vector<const Action *> &events)
    {
        for (vector<const Action *>::const_iterator i = events.begin(); i != events.end(); ++i)
        {
            if ((*i)->getPrecondition()->evaluate(s))
            {
                if (LaTeX)
                {
                    *report << "\\> Event " << *i << " does not falsify its precondition!\\\\\n";
                }
                else if (Verbose)
                {
    ...
    bool Events::triggerDiscreteEvents(Validator *v, bool init)
    {
        ...
        // trigger events: build up list of events to be triggered then add to a happening and execute the
        // happening then test for more events
        bool eventTriggered;
        do
        {
            ...
            for (vector<event *>::iterator e = ungroundEvents.begin(); e != ungroundEvents.end(); ++e)
            {
                ...
                else
                {
                    ...
                    {
                        *report << "Event preconditions are not permitted to be satisfied in the initial "
                                   "state!\n";
                        if (!ContinueAnyway)
                        {
                            return false;
                     ...
                     p != listOfParameters.end(); ++p)
                {
                    ...
                    else
                    {
                        const Action *anAction =
                            new Action(v, *e, *p);  // include precondition to check for mutex events? yes
    ...
    const StartAction *newStartProcessAction(process *pro, const_symbol_list *csl, Validator *v)
    {
        ...
        action *das = new safeaction(pro->name, pro->parameters, new conj_goal(new goal_list()),
                                     new effect_lists(), pro->symtab);
        return new StartAction(v, das, csl, new conj_goal(new goal_list()), elc, 0.0, new goal_list(),
    ...
    bool Events::triggerDiscreteProcesses(Validator *v)
    {
        ...
        if (time > 0)
        {
            ...
            for (map<const StartAction *, pair<const Proposition *, const_symbol_list *>>::iterator ap =
                     activeProcesses.begin();
                 ap != activeProcesses.end(); ++ap)
            {
                const_cast<Proposition *>(ap->second.first)->resetCtsFunctions();  // ensure we are checking
                                                                                   // the precondition at a
                                                                                   // point not on an interval
            ...
            for (vector<const StartAction *>::iterator ee = eraseElements.begin(); ee != eraseElements.end();
                 ++ee)
            {
        ...
        for (vector<process *>::const_iterator p = ungroundProcesses.begin(); p != ungroundProcesses.end();
             ++p)
        {
            ...
            else
            {
                listOfParameters = getParametersDiscreteFinal(
                    (*p)->precondition, *p,
                    v->getState());  // need to check for all changes after the event cascade, this is the
                                     // best way, only consider the process if something has changed
            };
            // cout << "Process " << (*p)->name->getName() << " with " << listOfParameters.size() << "\n";

            // add processes to an event happening and remove repetitions
            for (vector<const_symbol_list *>::iterator pa = listOfParameters.begin();
                 pa != listOfParameters.end(); ++pa)
            {
                if (!isProcessActive(
                        *p, *pa) /*&& !isProcessUntriggered(*p,*pa)*/)  // no need to check if it has not been
                                                                        // untriggered, because above we now
                                                                        // check that the process is triggered
                                                                        // by something that has changed
                {
                    const StartAction *sa = newStartProcessAction(*p, *pa, v);
                    processes.push_back(sa);
                    // only add the precondition to the list of triggered processes, as we already know it to
                    // be satisfied when applied
                    const Proposition *prop =
                        v->pf.buildProposition((*p)->precondition, *(buildBindings(*p, **pa).copy(v)));
    ...
    bool Events::triggerContinuousEvents(Validator *v, bool init)
    {
        ...
        ace->setLocalUpdateTime(localTime);  // set value of LocalUpdateTime - time since last regular
                                             // happening
        ...
        for (vector<const Action *>::iterator e = acts.begin(); e != acts.end(); ++e)
        {
            ...
            if (!eventIntervals.intervals.empty())
            {
                ctsTriggeredEvents.push_back(make_pair(
                    *e, eventIntervals.intervals.begin()->first));  // intervals are constructed left to right
                                                                    // so this is the minimum point
            }
            else
            {
                delete *e;
        ...
        for (vector<process *>::iterator pro = ungroundProcesses.begin(); pro != ungroundProcesses.end();
             ++pro)
        {
                 ...
                 p != listOfParameters.end(); ++p)
            {
                if (!isProcessActive(*pro, *p) /*&& !isProcessUntriggered(*pro,*p)*/)
                {
                    ...
                    const Proposition *prop =
                        v->pf.buildProposition((*pro)->precondition, *(buildBindings(*pro, **p).copy(v)));
                    ...
                    if (!eventIntervals.intervals.empty())
                    {
                        ...
                        {
                            ctsTriggeredEvents.push_back(
                                make_pair(sa, startTime));  // intervals are constructed left to right so this
        ...
        for (vector<pair<const Action *, intervalEnd>>::iterator i = ctsTriggeredEvents.begin();
             i != ctsTriggeredEvents.end(); ++i)
        {
            ...
            {
                eventTriggerTime = i->second.first;
                for (vector<const Action *>::iterator k = eventsToTrigger.begin(); k != eventsToTrigger.end();
                     ++k)
                {
                    delete *k;
                }
        ...
        // remember the no loops rule, i.e. an event cannot be triggered more than once at a given time point,
        // only the same time point as last trigger loop thro' events check if they have already been
        // triggered (unless a process)
        ...
        if (triggerEventsExist && isOK)
        {
            ...
            eventHappening->adjustActiveCtsEffects(*ace);  // update cts effects for interval after event
                                                           // happening
            ...
            for (vector<const Action *>::const_iterator j = eventsToTrigger.begin();
                 j != eventsToTrigger.end(); ++j)
            {
                (*j)->addTriggeredEvents(events, oldTriggeredEvents, triggeredStartProcesses,
            ...
            for (vector<const Action *>::const_iterator te = events.begin(); te != events.end(); ++te)
            {
                triggeredEvents.insert((*te)->getName0());
            }

            // add triggered processes to list of triggered processes
            for (vector<const StartAction *>::const_iterator trs = triggeredStartProcesses.begin();
            ...
            for (map<const StartAction *, pair<const Proposition *, const_symbol_list *>>::iterator sp =
                     startProcesses.begin();
                 sp != startProcesses.end(); ++sp)
            {
                sp->second.first->destroy();
                delete sp->second.first;
            };

            // remove untriggered processes from list of triggered processes
            for (vector<const EndAction *>::const_iterator trs = triggeredEndProcesses.begin();
                 trs != triggeredEndProcesses.end(); ++trs)
            {
                const StartAction *stAction = endStartMap[*trs];
                map<const StartAction *, pair<const Proposition *, const_symbol_list *>>::iterator ap =
    ...
    const vector<const_symbol_list *> getParametersDiscrete(goal *g, operator_ *op, const State &s, bool neg)
    {
        ...
        if (const comparison *comp = dynamic_cast<const comparison *>(g))
        {
            ...
            for (set<const func_term *>::iterator pne = pnes.begin(); pne != pnes.end(); ++pne)
            {
                ...
                for (set<const FuncExp *>::const_iterator fe = changedPNEs.begin(); fe != changedPNEs.end();
                     ++fe)
                {
                        ...
                        (*fe)->checkConstantsMatch((*pne)->getArgs()))  //+check consts match!
                    {
                             ...
                             j != op->parameters->end(); ++j)
                        {
                            ...
                            const_symbol *cs =
                                para == "" ? 0 : s.getValidator()->getAnalysis()->const_tab.symbol_get(para);
        ...
        if (const simple_goal *sg = dynamic_cast<const simple_goal *>(g))
        {
            ...
            for (set<const SimpleProposition *>::const_iterator sp = changedLiterals.begin();
                 sp != changedLiterals.end(); ++sp)
            {
                    ...
                    (*sp)->checkConstantsMatch(sg->getProp()->args))
                {
                    ...
                    for (var_symbol_list::iterator j = op->parameters->begin(); j != op->parameters->end();
                         ++j)
                    {
        ...
        {
            return getParametersDiscrete(const_cast<goal *>(dynamic_cast<const neg_goal *>(g)->getGoal()), op,
                                         s, !neg);
        };
    ...
    {
        // get list of instansigated parameters for the qfied prop, so for forall(?z ?y), we have a grounded
        // list for every z? and ?y
        set<var_symbol *> svs = getVariables(qg);
        vector<const_symbol_list *> constantsList = defineUndefinedParameters(
            newBlankConstSymbolList(const_cast<var_symbol_list *>(qg->getVars()), v),
    ...
    const vector<const_symbol_list *> getParametersCts(goal *g, operator_ *op, Validator *v, bool neg,
                                                       bool atAPoint)
    {
        vector<const_symbol_list *> listOfparameters;
            ...
            for (set<const func_term *>::iterator pne = pnes.begin(); pne != pnes.end(); ++pne)
            {
                     ...
                     fe != v->getActiveCtsEffects()->activeFEs.end(); ++fe)
                {
                        ...
                        fe->first->checkConstantsMatch((*pne)->getArgs()))
                    {
                        ...
                        // any undefined parameters will be ground out and checked later, unless we are
                        // checking at a point for mutex checks with actions at the same time.
                        if (!atAPoint)
                        {
                            listOfparameters.push_back(parametersForEvent);
                        }
                        ...
                        {
                            set<var_symbol *> svs = getVariables(comp);
                            alistOfparameters = defineUndefinedParametersPropVar(parametersForEvent, op, v, g,
                                                                                 false, neg, svs, true);

                            for (vector<const_symbol_list *>::const_iterator l = alistOfparameters.begin();
        ...
        if (dynamic_cast<const neg_goal *>(g))
        {
            return getParametersCts(const_cast<goal *>(dynamic_cast<const neg_goal *>(g)->getGoal()), op, v,
                                    !neg);
        };
    ...
    const vector<const_symbol_list *> getParameters(goal *g, operator_ *op, Validator *v, bool discrete,
                                                    bool neg, bool atAPoint)
    {
        ...
        if ((!neg && (dynamic_cast<const conj_goal *>(g))) || (neg && (dynamic_cast<const disj_goal *>(g))))
        {
            ...
            for (pc_list<goal *>::const_iterator i = goalList->begin(); i != goalList->end(); ++i)
            {
                ...
                {
                    alistOfparameters = getParametersList(g, op, v, someParameters, neg, discrete, atAPoint);
                }
        ...
        if ((!neg && (dynamic_cast<const disj_goal *>(g))) || (neg && (dynamic_cast<const conj_goal *>(g))))
        {
            ...
            for (pc_list<goal *>::const_iterator i = goalList->begin(); i != goalList->end(); ++i)
            {
                ...
                for (vector<const_symbol_list *>::const_iterator l = someParameters.begin();
        ...
        if (const qfied_goal *qg = dynamic_cast<const qfied_goal *>(g))
        {
            ...
            if (discrete)
            {
                listOfparameters = getParametersDiscrete(goalToCheck, op, v->getState(), neg);
            }
            else
            {
    ...
    // find the list of parameters lists that satisfy g from the given list of parameter lists lop (psi as in
    // paper)
    const vector<const_symbol_list *> getParametersList(goal *g, operator_ *op, Validator *v,
                                                        const vector<const_symbol_list *> &lop, bool neg,
                                                        bool discrete, bool atAPoint)
    {
        vector<const_symbol_list *> listOfparameters;
        ...
        if (const comparison *comp = dynamic_cast<const comparison *>(g))
        {
            // if checking for ctsly triggered events we need to assume any comps with ctsly changing pnes are
            // true at this stage. The comps are checked later, so do not do any checking here, it is done
            // later (which will also include any comps with no ctsly changing PNEs that may be given here)
            ...
            vector<const_symbol_list *> listOfparameters =
                defineUndefinedParametersPropVar(someParameters, op, v, g, false, neg, svs, atAPoint);
        ...
        if (const simple_goal *sg = dynamic_cast<const simple_goal *>(g))
        {
            // look in state, look up all simple props that are true with this name. Get a list of constant
            // symbols corresponding to the true literals. Create a list of partially ground parameters from
            // these and lop
            string literalName = sg->getProp()->head->getName();
                    ...
                    if (i->second && (i->first->getPropName() == literalName))
                    {
                        if (i->first->checkParametersConstantsMatch(sg->getProp()->args))
                        {
                            addToListOfParameters(
                                ...
                                i->first->getConstants(op->parameters, sg->getProp()->args, v));
                        }
                    };
                };
            }
            else
            {
                ...
                listOfparameters =
                    defineUndefinedParametersPropVar(someParameters, op, v, g, isDerivedPred, neg, svs);
            };
        ...
        if (dynamic_cast<const neg_goal *>(g))
        {
            return getParametersList(const_cast<goal *>(dynamic_cast<const neg_goal *>(g)->getGoal()), op, v,
                                     lop, !neg, discrete, atAPoint);
        };
        ...
        if ((!neg && (dynamic_cast<const conj_goal *>(g))) || (neg && (dynamic_cast<const disj_goal *>(g))))
        {
            ...
            vector<goal *> negCompGoals;  // do negative ones (and comparisons) later as these often cause a
                                          // lot of work, try to reduce list first
            // loop thro' conjuncts
            for (pc_list<goal *>::const_iterator i = goalList->begin(); i != goalList->end(); ++i)
            {
                if (containsNegationComp(*i, neg))
                    ...
                    if (!aListOfparametersTemp.empty())
                    {
                        aListOfparameters = getParametersList(
                            *i, op, v, aListOfparametersTemp, neg, discrete,
                            atAPoint);  // cout << " size of temp = "<<aListOfparameters.size()<<"\n";
            ...
            for (vector<goal *>::const_iterator j = negCompGoals.begin(); j != negCompGoals.end(); ++j)
            {
                if (!aListOfparametersTemp.empty())
                {
                    aListOfparameters = getParametersList(*j, op, v, aListOfparametersTemp, neg, discrete,
    ...
    void addToListOfParameters(vector<const_symbol_list *> &vcsl, const vector<const_symbol_list *> &lop,
        ...
        for (vector<const_symbol_list *>::const_iterator i = lop.begin(); i != lop.end(); ++i)
        {
            ...
            for (const_symbol_list::const_iterator k = (*i)->begin(); k != (*i)->end(); ++k)
            {
                if ((*j))
                {
                    ...
                    {
                        satisfied = false;  // the literal(or other) is not satisfied by previously defined
                                            // parameters so not add to list of parameters
                    }
                };
    ...
    const vector<const_symbol_list *> defineUndefinedParameters(const_symbol_list *csl,
                                                                var_symbol_list *variables, Validator *vld,
                                                                const set<var_symbol *> &svs)
    {
        ...
        // loop thro definedLists until every parameter is ground, adding to definedLists when grounding a
        // variable and other undefineds remain
        ...
        } while (true);  // end of looping thro defined lists, for given parameter, move to next parameter if
                         // considered parameter set was last in list when chosen
        ...
        */
        ...
        /*  cout << finalDefinedLists.size() << " results\n";
          ...
          for(vector<const_symbol_list*>::const_iterator k = finalDefinedLists.begin(); k !=
          finalDefinedLists.end() ; ++k)
          {
             for(const_symbol_list::iterator l = (*k)->begin(); l != (*k)->end(); ++l)
             {
               cout << (*l)->getName();
    ...
    const vector<const_symbol_list *> defineUndefinedParameters(const vector<const_symbol_list *> &vcsl,
                                                                operator_ *op, Validator *vld,
                                                                const set<var_symbol *> &svs)
    {
        ...
        for (vector<const_symbol_list *>::const_iterator csl = vcsl.begin(); csl != vcsl.end(); ++csl)
        {
            ...
            for (vector<const_symbol_list *>::const_iterator i = someDefinedLists.begin();
    ...
    const vector<const_symbol_list *> checkParametersProp(const vector<const_symbol_list *> &vcsl,
                                                          operator_ *op, Validator *vld, goal *g, bool neg,
                                                          bool dp, bool compError)
    {
        vector<const_symbol_list *> definedLists;
        ...
        for (vector<const_symbol_list *>::const_iterator gp = vcsl.begin(); gp != vcsl.end(); ++gp)
        {
            ...
            else
            {
                ...
                catch (BadAccessError &e)
                {
                    ...
                    {
                        propSatisfied = true;  // if a PNE is not defined, then no problem, the event is
    ...
    const vector<const_symbol_list *> defineUndefinedParametersPropVar(const_symbol_list *csl, operator_ *op,
                                                                       Validator *vld, goal *g, bool dp,
                                                                       bool neg, const set<var_symbol *> &svs,
                                                                       bool compError)
    {
        ...
        if (!svs.empty())
        {
            groundListOfParameters = defineUndefinedParameters(csl, op->parameters, vld, svs);
        }
        else
        {
    ...
    const vector<const_symbol_list *>
    defineUndefinedParametersPropVar(const vector<const_symbol_list *> &vcsl, operator_ *op, Validator *vld,
                                     goal *g, bool dp, bool neg, const set<var_symbol *> &svs, bool compError)
    {
        vector<const_symbol_list *> definedLists;
        vector<const_symbol_list *> someDefinedLists;
    ...
    const goal *copyGoal(const goal *g)
    {
        ...
        if (dynamic_cast<const qfied_goal *>(g))
        {
            ...
            for (map<string, var_symbol *>::const_iterator i = qg->getSymTab()->begin();
    ...
    const expression *copyExpression(const expression *e)
    {
        ...
        {
            return new div_expression(const_cast<expression *>(copyExpression(fexpression->getLHS())),
        ...
        {
            return new minus_expression(const_cast<expression *>(copyExpression(fexpression->getLHS())),
                                        const_cast<expression *>(copyExpression(fexpression->getRHS())));
        };

        if (const mul_expression *fexpression = dynamic_cast<const mul_expression *>(e))
        {
            return new mul_expression(const_cast<expression *>(copyExpression(fexpression->getLHS())),
                                      const_cast<expression *>(copyExpression(fexpression->getRHS())));
        };

        if (const plus_expression *fexpression = dynamic_cast<const plus_expression *>(e))
        {
            return new plus_expression(const_cast<expression *>(copyExpression(fexpression->getLHS())),
                                       const_cast<expression *>(copyExpression(fexpression->getRHS())));
        };
        ...
        {
            parameter_symbol_list *newpsl = new parameter_symbol_list();
            for (parameter_symbol_list::const_iterator i = fexpression->getArgs()->begin();
