...
namespace VAL
{
    ...
    Happening::Happening(Validator *v, const vector<pair<double, Action *>> &as, double timeEndPlan)
      : vld(v), time(0.0), actions(), eventHappening(false), realHappening(false), afterPlan(false)
    {
        ...
        realHappening =
            (find_if(actions.begin(), actions.end(), mem_fun(&Action::isRegAction)) != actions.end());
    };
    ...
    Happening::Happening(Validator *v, vector<const Action *> acts, bool eve)
      ...
      , realHappening(false)
      , afterPlan(false){
            // afterPlan = time > timeEndPlan; may need later for processes, then events may be after the main
            // plan std::transform(acts.begin(),acts.end(),std::back_inserter(actions));

        };

    // for creating event happenings
    Happening::Happening(Validator *v, vector<const Action *> acts, double t, bool eve)
      : vld(v)
      , time(t)
      , actions(acts)
      , eventHappening(eve)
    ...
    bool Happening::canHappen(const State *s) const
    {
        ...
        else
        {
            ...
            if (Verbose)
            {
                for (vector<const Action *>::const_iterator act = actions.begin(); act != actions.end();
                     ++act)
                {
            ...
            return true;  // no need to check event preconditions, these are all satisfied! (if not then there
                          // is an error calculating the events!)
        };
        ...
        for (vector<const Action *>::const_iterator a = actions.begin(); a != actions.end(); ++a)
        {
...

            {
                ...
                if (Verbose || ErrorReport)
                {
                    (*a)->addErrorRecord(time, s);
                }
                if (!ContinueAnyway)
                {
                    return false;
    ...
    bool Happening::applyTo(State *s) const
    {
        ...
        vector<const Action *> eventsForMutexCheck =
            vld->getEvents().getEventsForMutexCheck();  // this is in case an event is triggered at the same
                                                        // time as an action
        if (!eventsForMutexCheck.empty())
        {
            ...
            for (; b != eventsForMutexCheck.end();)
            {
                ...
                for (vector<const Action *>::const_iterator c = actions.begin(); c != actions.end(); ++c)
                {
                    ...
                    {
                        repeated = true;  // there will be only a few possible values of b and c...
                    }
                };
                if (repeated)
                {
            ...
            for (vector<const Action *>::const_iterator b2 = eventsForMutexCheck.begin();
        ...
        {
            if (!(*a1)->constructEffects(own, effs, s, markPreCons))
            {
                return false;
            }
        };
        ...
        for (vector<const Action *>::const_iterator b1 = eventsForMutexCheck.begin();
    ...
    {
        for (vector<const Action *>::const_iterator a = invariants.actions.begin();
             a != invariants.actions.end(); ++a)
        {
            if (InvariantAction *ia = dynamic_cast<InvariantAction *>(const_cast<Action *>(*a)))
            {
                ia->setActiveCtsEffects(ace);
                         ...
                         dynamic_cast<CondCommunicationAction *>(const_cast<Action *>(*a)))
            {
...

    {
        if (a != 0)
        {
            ctsEffects.actions.push_back(a);
        }
    };
    ...
    {
        // return a new happening with only one ctseffectaction that has a pointer to the ace so that they may
        // all be updated
    ...
    void ActiveCtsEffects::addActiveFEs(bool reCalc)
    {
             ...
             a != ctsEffects.actions.end(); ++a)
        {
            ...
            // status will be true when displaying plan so if there is a possiblity of nonlinear effects an
            // exception will be thrown loop thro' conditional effects if they are active add them to the
            // effect list
        ...
        for (map<const FuncExp *, ActiveFE *>::iterator a1 = activeFEs.begin();
             ...
             a1 != activeFEs.end(); ++a1)
        {
            for (vector<pair<pair<const expression *, bool>, const Environment *>>::const_iterator i =
    ...
    void ActiveFE::addParentFEs(const ActiveCtsEffects *ace, const expression *e, const Environment *bs)
    {
...

        {
            ...
            for (map<const FuncExp *, ActiveFE *>::const_iterator j = ace->activeFEs.begin();
    ...
    bool ActiveFE::appearsInEprsn(const ActiveCtsEffects *ace, const expression *e,
                                  const Environment *bs) const
    {
        ...
        {
            if (appearsInEprsn(ace, bexp->getLHS(), bs))
            {
                return true;
            }
            if (appearsInEprsn(ace, bexp->getRHS(), bs))
            {
                return true;
            }
        };
        ...
        if (const uminus_expression *uexp = dynamic_cast<const uminus_expression *>(e))
        {
            if (appearsInEprsn(ace, uexp->getExpr(), bs))
            {
                return true;
            }
        };
    ...
    void ActiveCtsEffects::buildAFECtsFtns()
    {
        ...
        for (map<const FuncExp *, ActiveFE *>::iterator i = activeFEs.begin(); i != activeFEs.end(); ++i)
        {
            ...
            for (vector<const ActiveFE *>::iterator j = i->second->parentFEs.begin();
        ...
        // sift out dependencies for diff equations that are self dependent and depend on another from the
        // list
        for (vector<ActiveFE *>::iterator i = loopDepTempActiveFEs.begin(); i != loopDepTempActiveFEs.end();
             ++i)

        {
            bool otherDepend = false;
            for (vector<const ActiveFE *>::iterator j = (*i)->parentFEs.begin(); j != (*i)->parentFEs.end();
                 ++j)
            {
        ...
        // sift out loop dependencies for exp functions from above list (d f_2/dt = f_1, where f_1 from above
        // list)
             ...
             i != listToTopSortTempActiveFEs.end(); ++i)
        {
            if ((*i)->parentFEs.size() > 0)
            {
                ...
                for (vector<ActiveFE *>::iterator j = loopDepActiveFEs.begin(); j != loopDepActiveFEs.end();
                     ++j)
                {
        ...
        for (vector<ActiveFE *>::iterator i = loopDepActiveFEs.begin(); i != loopDepActiveFEs.end(); ++i)
        {
            ...
            //		for(vector<pair< pair<const expression *,bool> ,const Environment *> >::const_iterator j =
    ...
    bool isConstLinearChangeExpr(const ExprnPair &exp, const map<const FuncExp *, ActiveFE *> activeFEs,
                                 Validator *vld)
    {
    ...
    bool isConstant(const expression *exp, const Environment *env,
                    const map<const FuncExp *, ActiveFE *> activeFEs, Validator *vld)
    {
    ...
    const CtsFunction *ActiveCtsEffects::buildExp(const ActiveFE *afe)
    {
        ...
        else if (const mul_expression *me = dynamic_cast<const mul_expression *>(rateExprn))
        {
            ...
            {
                simpleExp = false;
                if (afe->parentFEs.size() == 1)
                {
                    fexp = (*afe->parentFEs.begin())->fe;
                }
        ...
        if (simpleExp)  // simple in the sense that does not depend on other PNES that are exps
        {
            ...
            if (dynamic_cast<const func_term *>(FEExprn))
            {
                ...
                // We can also deal with the special case where we have two expressions in the exprns list for
                // this activeFE and one is constant, A, and the other is B.f for constant B.
                if (constExpA)
                {
    ...
    const CtsFunction *ActiveCtsEffects::buildNumericalSoln(const ActiveFE *afe)
    {
        ...
        const BatteryCharge *newBatteryCharge =
            new BatteryCharge(new Polynomial(bPoly), mValue, discharge, 0, localUpdateTime,
    ...
    {
        // return true;
        for (map<const FuncExp *, ActiveFE *>::const_iterator i = activeFEs.begin(); i != activeFEs.end();
             ++i)
        {
            // cout << i->second->fe<<" fe chking\n";
            if (!(i->second->parentFEs.empty()))
            {
                return false;
            }
        };
    ...
    {
        el->add_effects.insert(el->add_effects.begin(), more->add_effects.begin(), more->add_effects.end());
        el->del_effects.insert(el->del_effects.begin(), more->del_effects.begin(), more->del_effects.end());
        el->forall_effects.insert(el->forall_effects.begin(), more->forall_effects.begin(),
                                  more->forall_effects.end());
        el->cond_effects.insert(el->cond_effects.begin(), more->cond_effects.begin(),
                                more->cond_effects.end());
        el->assign_effects.insert(el->assign_effects.begin(), more->assign_effects.begin(),
    ...
    Plan::Plan(Validator *v, const operator_list *ops, const plan *p) : vld(v), timeToProduce(p->getTime())
    {
        if (p->empty())
        {
            return;
        ...
        else
        {
            while (i != planStructure.end())
            {
                timedActionSeq::iterator j = find_if(i, planStructure.end(), sameTime(i->first));
                timedActionSeq vs(i, j);
                happenings.push_back(new Happening(vld, vs, d));
                i = j;
            };
    ...
    void handleDAeffects(const effect_lists *efcts, effect_lists *els, effect_lists *ele, effect_lists *elc)
    {
             ...
             i1 != efcts->forall_effects.end(); ++i1)
        {
            ...
            {
                els->forall_effects.push_back(
                    new forall_effect(elsfa, const_cast<var_symbol_list *>((*i1)->getVarsList()),
            ...
            {
                elc->forall_effects.push_back(
                    new forall_effect(elcfa, const_cast<var_symbol_list *>((*i1)->getVarsList()),
                                      const_cast<var_symbol_table *>((*i1)->getVars())));
            };
            ...
            {
                ele->forall_effects.push_back(
                    new forall_effect(elefa, const_cast<var_symbol_list *>((*i1)->getVarsList()),
                                      const_cast<var_symbol_table *>((*i1)->getVars())));
            };
        };
    };
    ...
    struct handleDAConditionalEffects
    {
        ...
        void operator()(const forall_effect *fa)
        {
            if (!fa->getEffects()->cond_effects.empty())
            {
                ...
                handleDAConditionalEffects hDAc = for_each(
                    fa->getEffects()->cond_effects.begin(), fa->getEffects()->cond_effects.end(),
                ...
                if (!hDAc.ctsCondActions.empty())
                {
                    ...
                    cout << "This is really complex stuff! Unfortunately, VAL doesn't even try to deal with "
                            "it.\n";
                ...
                {
                    forall_effect *fas = new forall_effect(lels, new var_symbol_list(*(fa->getVarsList())),
                ...
                {
                    forall_effect *fae = new forall_effect(lele, new var_symbol_list(*(fa->getVarsList())),
                                                           new var_symbol_table());
        ...
        void operator()(const cond_effect *ce)
        {
            ...
            if (!(locelc->assign_effects.empty() &&
                  locelc->forall_effects.empty()))  // cts effects will always be assign effects or forall
            {
                ctsCondActions.push_back(new CondCommunicationAction(vld, da, params, gls, gli, gle, 0,
                                                                     locelc));  // cts cond actions
                ...
                && locele->assign_effects.empty() && gli->empty() && gle->empty())
            {
                ...
                return;
                       ...
                       locels->forall_effects.empty()  // if start effs and other effects
            ...
            if (vars)
            {
                buildForAllCondActions(vld, da, params, gls, gli, gle, locels, locele, vars, vars->begin(),
                                       condActions, &bs);
            }
            else
            {
                condActions.push_back(
                    new CondCommunicationAction(vld, da, params, gls, gli, gle, locels, locele));
            }
        };
    };

    void Plan::planBuilder::handleDurativeAction(const durative_action *da, const const_symbol_list *params,
                                                 double start, double duration, const plan_step *ps)
    {
        ...
        hDAc = for_each(da->effects->forall_effects.begin(), da->effects->forall_effects.end(), hDAc);
        ...
        StartAction *sa = new StartAction(vld, das, params, inv, elc, duration, ds, hDAc.condActions,
                                          hDAc.ctsCondActions, ps);
    ...
    Polynomial getPoly(const expression *e, const ActiveCtsEffects *ace, const Environment &bs,
                       CoScalar endInt)
    {
    ...
    Polynomial getPoly(const expression *e, bool inc, const ActiveCtsEffects *ace, const Environment &bs,
                       CoScalar endInt)
    {
        if (inc)
        {
    ...
    double Plan::timeOf(const Action *a) const
    {
        ...
        {
            if (find((*h)->getActions()->begin(), (*h)->getActions()->end(), a) != (*h)->getActions()->end())
            {
