...
namespace Inst
{
    ...
    bool varFree(const VAL::parameter_symbol_list *pl)
    {
        ...
        {
            if (!dynamic_cast<const VAL::const_symbol *>(*i))
            {
                return false;
            }
        }
        ...
        if (hpsIsStatic)
        {
            ...
            for (; epsItr != epsEnd; ++epsItr)
            {
                if ((*epsItr)->contains(f, s->getProp()))
                {
                    ...
                    {
                        cout << s->getProp()->head->getName() << " is static, and in the EPS " << *epsItr
                             << std::endl;
                    }
                    valueTrue = true;
                    valueFalse = false;
            ...
            if (!epsMatch)
            {
                valueTrue = (InitialStateEvaluator::init0State.find(s->getProp()->head) !=
    ...
    public:
        ...
        /** @Return <code>true</code> if no JPDCs are defined from this object (i.e. from its corresponding
         * parameter). */
         ...
         *                    parameters if this parameter is assigned this value.  Each entry of this map
         *                    is a pair consisting of:
         *                      - the index of another operator parameter, i.e. one other than
         * <code>paramID</code>, and where parameter 0 is the first parameter.
         *                      - a set of values that can be given to that parameter
         *
         ...
         *  @param otherParam The ID of another parameter of the operator (i.e. <code>thisParam !=
         * otherParam</code>)
         *  @param otherVal   A value of <code>otherParam</code> that is permissible if the parameter
         * corresponding to this object is given the value <code>thisVal</code>.
         *
         */
        inline void addRestriction(const int &thisParam, const int &thisVal, const int &otherParam,
                                   const int &otherVal)
        {
         ...
         *  @return  An iterator pointing to the JPDCs from that value - returns <code>end()</code> if
         * undefined.
         */
    ...
    protected:
        /**
         * A method to trim redundant elements from the targets of JPDCs.  If a JPDC records that setting
         * parameter <code>a</code> to the value <code>b</code> means that parameter <code>c</code> could take
         * the value <code>d</code>, then if <code>d</code> is removed from the domain of <code>c</code>, it
         * is redundant, and hence should also be removed from the JPDC.
         *
         * @param oItr  An iterator pointing to the JPDCs arising when some value has been given to some
         * parameter. From this, all redundant restrictions on other parameters' values will be eliminated.
         */
        void trim(JPDCData::iterator &oItr)
        {
            ...
            for (; trimItr != trimEnd; ++trimItr)
            {
                ...
                {
                    set<int> isect;
                    std::set_intersection(domains[trimItr->first].second.begin(),
    ...
    public:
         ...
         *  one of the inputs.
         *
         *  @param dest    One of the input set pairs, to be overwritten by the output of the
         * intersection/union operation
         *  @param other   The other input set pair for the operation
         *  @param doUnion If <code>true</code>, the union of the two sets is taken.  Otherwise, the
         ...
         *  @see domains
         */
        static bool mergePair(pair<bool, set<int>> &dest, const pair<bool, set<int>> &other,
                              const bool &doUnion)
        {
            bool toReturn = false;
            if (dest.first)
        ...
        void mergeInJPDCs(const int &paramID, const JPDCData &newData)
        {
            ...
            {
                if (nItr->first < oItr->first)
                {  // if this parameter now has restrictions from the value nItr->first, but it didn't used to
                    if (mergeDebug)
                    {
                    ...
                    else
                    {
                        if (mergeDebug)
                        {
                            if (domItr != domEnd)
                            {
                                cout << " - However, next possible legal value of this parameter would be "
                                     << *domItr << ", so discarding restrictions\n";
                            }
                            else
                            {
                                cout << " - However, there are no more legal values of this parameter - "
                ...
                else if (oItr->first < nItr->first)
                {  // not changing anything about how the value oItr->first affects other parameters
                    if (mergeDebug)
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (mergeDebug)
                            {
                                if (domItr != domEnd)
                                {
                                    cout << " - However, next possible legal value of this parameter would "
                                            "be "
                                         << *domItr << ", so discarding restrictions\n";
                                }
                                else
                ...
                {  // now intersect - have multiple restrictions

                    if (mergeDebug)
                    {
                        cout << "Merging restrictions stemming from setting the parameter to " << oItr->first
                             << endl;
                    }
                    if (!domainIsFull)
                    {
                    ...
                    {
                        if (naItr->first < oaItr->first)
                        {  // this parameter now has restrictions from nItr->first onto naItr->first
                            if (mergeDebug)
                            {
                        ...
                        else if (oaItr->first < naItr->first)
                        {  // no change on restrictions from oItr->first onto oaItr->first
                            ++oaItr;
                        }
                        else
                        {
                            if (mergeDebug)
                            {
                                cout << "  * Intersecting restrictions on the value of parameter "
                                     << (naItr->first + 1) << endl;
    ...
    */
    ...
    protected:
        ...
        /** The symbols used in the action definition for each parameter (e.g. <code>?a</code>,
         * <code>?b</code>) */
          ...
          , equality(VAL::current_analysis->pred_tab.symbol_probe("="))
        {
            {
                int i = 0;
                for (typename T::const_iterator p = parameters->begin(); p != parameters->end(); ++p, ++i)
                {
                    //   symbols[i] = *p;
                    ...
                    if (instantiatedValues.find((*p)->type) == instantiatedValues.end())
                    {
                        ...
                        catch (TypeException e)
                        {
                            ...
                            cerr << "-------------------------------------------------------------\n";
                            cerr << "Unfortunately, a type error has been encountered in your domain and "
                                    "problem file,\n";
                            cerr << "and the planner has to terminate.  Specifically, for parameter "
                                 << (i + 1) << "\n";
                    ...
                    possibleParameterValues[i].insert(possibleParameterValues[i].end(),
    ...
    public:
        ...
        virtual void visit_simple_goal(VAL::simple_goal *s)
        {
            ...
            if (!hpsIsStatic)
            {
                if (jpdcDebug)
                {
                    cout << " - Found a non-static fact " << s->getProp()->head->getName() << " at " << hps
                         << std::endl;
                }
                if (!doUnion.front())
                {
                ...
                for (; groundItr != groundEnd; ++groundItr, ++gi)
                {
                    bool keep = true;
                    {
                        ...
                        for (int aff = 0; wwItr != wwEnd; ++wwItr, ++aff)
                        {
                            ...
                            if (hasToMatch[aff])
                            {
                                // Simple case: precondition of the action said (connected ?x table), and this
                                // fact says (connected ?a ?b) where ?b != table - no way this static fact can
                                // support this action
                            ...
                            else
                            {
                                ...
                                {
                                    // no way this argument of the fact can be used as a binding for the
                                    // relevant action parameter
                                    keep = false;
                                    break;
                                }
                                ...
                                if (isect)
                                {  // if we're keeping only the intersection  (i.e. doing conjunction over
                                   // facts)
                                    if (oldSets[aff] && !oldSets[aff]->first)
                                    {  // if the old choices are defined, and are non-empty
                                        if (oldSets[aff]->second.find(asInt) == oldSets[aff]->second.end())
                                        {  // if this binding has already been pruned
                                            keep = false;
                                            break;
                                        }
                                    }
                    ...
                    if (keep)
                    {  // If the fact is consistent with permissible parameter values
                        if (jpdcDebug || debug)
                        {
                        ...
                        if (isect)
                        {
                            ...
                            {
                                if (scratch[aff] != -1)
                                {  // if it wasn't a const parameter
                                    newSets[aff].second.insert(scratch[aff]);  // add it to the new domain of
                                    ...
                                    if (aff == affWithBiggestParam)
                                    {
                                        ...
                                        for (int aff2 = 0; aff2 < affects; ++aff2)
                                        {
                                            ...
                                            if (pIdx2 < 0)
                                            {
                                                continue;  // if it's a constant
                                            }
                                            if (pIdx2 >= pIdx1)
                                            {
                                                continue;  // only store backwards jpdcs (see comments on the
                                                           // jpdcs variable definition
                                            }
                                            if (jpdcDebug)
                                            {
            ...
            if (isect)
            {
                ...
                {
                    if (oldSets[aff])
                    {  // if we previously had some options for this parameter
                        const int paramID = parameterIndex[aff];
                        if (newSets[aff].second.size() == possibleParameterValues[paramID].size())
                        {
                            // if the new options are just all entities of the right type, there's not a lot
                            // of point taking the intersection with this lower down, so we'll clear the set
                            // and set the bool to true (pretending it's never been touched)
                            newSets[aff].second.clear();
                            newSets[aff].first = true;
                        }

                        if (ParameterDomainsAndConstraints::mergePair(domainStack.front().domains[paramID],
                                                                      newSets[aff], doUnion.front()))
                        {
            ...
            if (!haveSpecial)
            {
                ...
                for (int v = 0; v < varCount; ++v)
                {
                    if (panic.front().find(v) != panic.front().end())
                    {
                        ...
                        if (ParameterDomainsAndConstraints::mergePair(second->domains[v], panicPair,
                                                                      doUnion.front()))
                        {
                            updateFrom.insert(v);
                        };
                    ...
                    else
                    {
                        if (ParameterDomainsAndConstraints::mergePair(
                                second->domains[v], domainStack.front().domains[v], doUnion.front()))
                        {
                            updateFrom.insert(v);
                        };
                    }
        ...
        /** Stub code to visit numeric goals.  For now, these are ignored - any domain constraints are
         * determined solely from the logical preconditions. */
        ...
        /**
         * Obtain an iterator pointing to the first possible combination of parameter bindings.  Call this
         * once the actions' preconditions have been visited by this class.
         *
    ...
    protected:
        ...
        virtual void checkParameterOptionRemains(const int &paramID)
        {
            ...
            {
                const int lookedUp = parameterValuesToIndices[paramID]
    ...
    public:
        DerivationRuleParameterDomainConstraints(const VAL::derivation_rule *const ruleIn,
                                                 VAL::TypeChecker &tc)
          : ParameterDomainConstraints(ruleIn->get_head()->args, ruleIn->get_head()->head->getName(), tc)
          , rule(ruleIn)
        {
            propagate();
        }
    ...
    protected:
         ...
         *  used to restrict the values of lower-indexed parameters.  Its JPDCs are applied to the possible
         * values of earlier parameters (recording the new sets in <code>allowedValuesForParameter</code>. The
         * procedure is essentially recursive, but to support iteration rather than having to explicitly
         * enumerate and then return the possible bindings, it is preempted, and enough information is stored
         * in <code>allowedValuesForParameter</code> and <code>popSet</code> to allow it to pick up from where
         * it left off, and update from the next value of the specified parameter each time it is called.
         *
        ...
        bool rollover(int x)
        {
            ...
            while (x)
            {
                ...
                if (xJPDC != pds.jpdcs[x].end())
                {
                    ...
                    for (; cItr != cEnd; ++cItr)
                    {
                        if (allowedValuesForParameter[cItr->first].front().second == whole)
                        {
                            if (debug)
                            {
                                cout << "  * First time a restriction has been put on " << (1 + cItr->first)
                                     << ", using the JPDC set\n";
                            }
                            allowedValuesForParameter[cItr->first].push_front(
                                make_pair(const_cast<set<int> *>(&(cItr->second)), borrowed));
                        ...
                        else
                        {
                            if (debug)
                            {
                                cout << "  * Taking intersection between existing JPDC set and new one for "
                                        "parameter "
                                     << (1 + cItr->first) << endl;
                            }
                            ...
                            std::set_intersection(oldSet->begin(), oldSet->end(), cItr->second.begin(),
                                                  cItr->second.end(),
                        ...
                        if (allowedValuesForParameter[cItr->first].front().first->empty())
                        {
                            if (debug)
                            {
                                cout << "  x Parameter set for " << (1 + cItr->first) << " became empty\n";
                            }
                            keepValue = false;
                            break;
                        }
                ...
                if (keepValue)
                {
                    if (debug)
                    {
                        cout << "Keeping this value of the parameter, moving on to " << x << endl;
                    }
                    --x;
    ...
    public:
        ...
        /**
         * Return whether bindings are currently defined - <code>false</code> if no further bindings exist
         * beyond those considered thus far, <code>true</code> otherwise.
         */
        ...
        /**
         *  Move onto the next parameter binding.  In the simple case, there are more values to consider in
         * the domain of the first parameter, in which case the iterator over these values is incremented, and
         * the next binding can be considered.  If this is not the case, then the next parameter
         * <code>x</code> with remaining values is determined and <code>rollover(x)</code> is called.
         *  @see rollover
         */
        void next()
        {
            ...
            while (valItrs[x] == valEnds[x])
            {
                if (debug)
                {
                    cout << "Finished options for parameter " << (x + 1) << endl;
                }
                ++x;
                if (x == varCount)
                {
                    if (debug)
                    {
                        cout << "No more parameter options left, bailing out\n";
                    }
                    valid = false;
                    return;
                }
    ...
    void instantiatedOp::instantiate(const VAL::operator_ *op, const VAL::problem *prb, VAL::TypeChecker &tc)
    {
        ...
        if (insistOnOp && insistOnOp == op)
        {
            for (int p = 0; p < opParamCount; ++p)
            {
                ...
                if (pItr == pEnd)
                {
                    std::cout << "Error during instantiating the named operator: parameter " << p + 1
        ...
        while (options->isValid())
        {
            ...
            else if (insistOnOp && insistOnOp == op)
            {
                bool allMatched = true;
                int p = 0;
                for (var_symbol_list::const_iterator a = op->parameters->begin(); a != op->parameters->end();
                     ++a, ++p)
                {
                    if (e[*a] != insistOnOpParameters[p])
                    {
    ...
    void instantiatedDrv::instantiate(const VAL::derivation_rule *op, const VAL::problem *prb,
                                      VAL::TypeChecker &tc)
    {
    ...
    public:
        ...
        Collector(const VAL::derivation_rule *o, FastEnvironment *f, LiteralStore &l, PNEStore &p,
                  VAL::TypeChecker *t = 0)
          : tc(t)
          , adding(true)
          , op(0)
        ...
        virtual void visit_qfied_goal(qfied_goal *p)
        {
            ...
            for (var_symbol_list::const_iterator pi = p->getVars()->begin(); pi != p->getVars()->end();
                 ++pi, ++i)
            {
                // cout << "Symbol goes at " << static_cast<const IDsymbol<var_symbol>*>(*pi)->getId() <<
                // endl;
    ...
    bool instantiatedOp::isGoalMetByEffect(const VAL::effect_lists *effs, const Literal *lit)
    {
        ...
        for (pc_list<VAL::simple_effect *>::const_iterator i = effs->add_effects.begin();
             i != effs->add_effects.end(); ++i)
        {
            if (isGoalMetByEffect(*i, lit))
            {
                return true;
            }
        };
        for (pc_list<VAL::forall_effect *>::const_iterator i = effs->forall_effects.begin();
        ...
        for (pc_list<VAL::cond_effect *>::const_iterator i = effs->cond_effects.begin();
        ...
        for (pc_list<VAL::timed_effect *>::const_iterator i = effs->timed_effects.begin();
