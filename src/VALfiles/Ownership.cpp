...
namespace VAL
{
        ...
        if (po != propOwner.end())
        {
            if (po->second.first != a && po->second.second != E_ADD)
            {
                ...
                if (Verbose)
                {
                    if (LaTeX)
                    {
                        *report << " \\> \\listrow{Mutex violation: \\action{" << *a << "} (adds \\exprn{"
                                << *p << "})";
                        if (po->second.first)
                        {
                ...
                if (ErrorReport)
                {
                    vld->getErrorLog().addMutexViolation(vld->getCurrentHappeningTime(), a, po->second.first,
                                                         &(vld->getState()));
                }
                return false;
            };
            ...
            switch (po->second.second)
            {
                ...
                case E_DEL:
                    ...
                    if (Verbose)
                    {
                        ...
                        {
                            cout << "WARNING: " << *a << " adds and deletes the literal " << *p << "\n";
                        };
                    };
                    return true;
                case E_ADD:
                    ...
                    if (Verbose)
                    {
                        if (po->second.first == a)
                        {
                            ...
                            {
                                *report << " \\> \\listrow{WARNING: \\action{" << *a
                                        << "} adds the literal \\exprn{" << *p << "} twice}\\\\\n";
                            }
                            else
                            {
                                cout << "WARNING: " << *a << " adds the literal " << *p << " twice\n";
                            };
                        }
                        else
                        {
                            if (LaTeX)
                            {
                                *report << " \\> \\listrow{WARNING: \\action{" << *a << "} and \\action{"
                                        << *(po->second.first) << "} both add the literal \\exprn{" << *p
                                        << "}}\\\\\n";
                            }
                            else
                            {
        ...
        if (po != propOwner.end())
        {
            if (po->second.first != a && po->second.second != E_DEL)
            {
                ...
                if (Verbose)
                {
                    if (LaTeX)
                    {
                        *report << " \\> \\listrow{Mutex violation: \\action{" << a << "} (deletes \\exprn{"
                                << *p << "})";
                        if (po->second.first)
                        {
                            *report << " and \\exprn{" << po->second.first << "}}\\\\";
            ...
            switch (po->second.second)
            {
                case E_NPRE:
                    if (Verbose)
                    {
                        ...
                        {
                            cout << "WARNING: " << *a << " deletes a false precondition literal " << *p
                                 << "\n";
                        };
                    };
                case E_PPRE:
                ...
                case E_DEL:
                    ...
                    if (Verbose)
                    {
                        if (po->second.first == a)
                        {
                            ...
                            {
                                *report << " \\> \\listrow{WARNING: \\action{" << *a
                                        << "} deletes the literal \\exprn{" << *p << "} twice}\\\\\n";
                            }
                            else
                            {
                                cout << "WARNING: " << *a << " deletes the literal " << *p << " twice\n";
                            };
                        }
                        else
                        {
                            ...
                            {
                                *report << " \\> \\listrow{WARNING: \\action{" << *a << "} and \\action{"
                                        << *(po->second.first) << "} both delete the literal \\exprn{" << *p
                                        << "}}\\\\\n";
                            }
                            else
                            {
    ...
    {
        map<const SimpleProposition *, pair<const Action *, ownership>>::const_iterator po =
            propOwner.find(p);

        if (po != propOwner.end())
        {
            if (po->second.second == E_PRE || po->second.second == E_NPRE || po->second.second == E_PPRE)
            {
                if (po->second.first != a)
                {
            ...
            else
            {
                if (Verbose && po->second.first != a)
                {
                    if (LaTeX)
                    {
                        *report << " \\> \\listrow{Mutex violation: \\action{" << a << "} (requires \\exprn{"
                                << p << "})}\\\\\n";
                    }
                    else
                    {
        ...
        if (const func_term *fe = dynamic_cast<const func_term *>(e))
        {
            ...
            if (FEOwner.find(fexp) != FEOwner.end())
            {
                ...
                else
                {
                    if (LaTeX)
                    {
                        *report << " \\> \\listrow{Mutex violation: \\action{" << a << "} (requires \\fexprn{"
                                << *fexp << "})}\\\\\n";
                    }
                    else
                    {
                    ...
                    if (ErrorReport)
                    {
                        vld->getErrorLog().addMutexViolation(vld->getCurrentHappeningTime(), a, 0,
                                                             &(vld->getState()));
                    }
                    return false;
                };
    ...
    bool Ownership::markOwnedEffectFE(const Action *a, const FuncExp *fe, assign_op aop, const expression *e,
                                      const Environment &bs)
    {
        if (!markOwnedPreconditionFEs(a, e, bs))
        {
            if (Verbose)
            {
                if (LaTeX)
                {
                    *report << " \\> \\listrow{Conflict over ownership of resource: \\fexprn{" << *fe
                            << "} wanted by \\action{" << a << "}}\\\\\n";
                }
                else
                {
            ...
            switch (FEOwner[fe].second)
            {
                ...
                case E_ADD:
                    ...
                    if (Verbose)
                    {
                        ...
                        {
                            cout << "WARNING: " << *a << " assigns to expression " << *fe << "twice\n";
                        };
                    };
                    return true;
                ...
                case E_ASSIGNMENT:
                    ...
                    if (Verbose)
                    {
                        if (LaTeX)
                        {
                            *report << " \\> \\listrow{\\action{" << *a << "} assigns to expression \\fexprn{"
                                    << *fe << "} twice}\\\\\n";
                        }
                        else
                        {
