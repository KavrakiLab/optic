...
namespace VAL
{
    ...
    double FuncExp::evaluate(const State *s) const
    {
        ...
        {
            ans += RobustPNEJudder * (1 - 2 * getRandomNumberUniform());  // if not robustness testing this
    ...
    {
        int parameterNo = 1;
        for (parameter_symbol_list::const_iterator i = fe->getArgs()->begin(); i != fe->getArgs()->end(); ++i)
        {
