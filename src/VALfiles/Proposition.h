...
namespace VAL
{
    ...
    public:
        ...
        virtual bool evaluate(const State *s,
                              vector<const DerivedGoal *> = vector<const DerivedGoal *>()) const = 0;
        ...
        virtual bool evaluateAtPointWithinError(
            const State *s, vector<const DerivedGoal *> = vector<const DerivedGoal *>()) const;
    ...
    public:
        ...
        const_symbol_list *getConstants(var_symbol_list *variables, parameter_symbol_list *psl,
                                        Validator *vld) const;
    ...
    public:
        FalseProposition(const Environment &bs, bool tp = false) : Proposition(bs), trueProp(tp){};
    ...
    public:
        ...
        bool evaluateAtPointWithinError(const State *s,
    ...
    public:
        ImplyGoal(const imply_goal *i, const Proposition *a, const Proposition *c, const Environment &bs)
    ...
    public:
        ConstraintGoal(Validator *v, const constraint_goal *cg, const Proposition *t, const Proposition *r,
    ...
    */
    ...
    public:
        ...
        ~PropositionFactory()
        {
            for (map<string, const SimpleProposition *>::iterator i = literals.begin(); i != literals.end();
                 ++i)
            {
                delete (i->second);
            }
        ...
        const Proposition *buildProposition(const goal *g, const Environment &bs,
                                            bool buildNewLiterals = true, const State *state = 0);
        const Proposition *buildProposition(const goal *g, bool buildNewLiterals = true,
                                            const State *state = 0);
    };
