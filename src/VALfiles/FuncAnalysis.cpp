...
namespace VAL
{
    ...
    template <class TI>
    void FuncAnalysis::doExplore(set<func_symbol *> &explored, vector<extended_func_symbol *> &tsort,
    ...
    void FuncAnalysis::exploreFrom(set<func_symbol *> &explored, vector<extended_func_symbol *> &tsort,
                                   bool invert, IGraph &inverted, func_symbol *fn)
    {
        extended_func_symbol *efn = EFT(fn);
        if (invert)
        {
            doExplore(explored, tsort, invert, inverted, efn, efn->getDeps().begin(), efn->getDeps().end());
        }
        else
        {
            doExplore(explored, tsort, invert, inverted, efn, inverted[efn].begin(), inverted[efn].end());
        };
        tsort.push_back(efn);
    };
    ...
    void extended_func_symbol::applyUpdates()
    {
        ...
        for (Updates::iterator j = increasers.begin(); j != increasers.end() && cval != E_ALL; ++j)
        {
            ...
            if (FAverbose && cval == E_ALL &&
                find(preconds.begin(), preconds.end(), pair<operator_ *, derivation_rule *>(j->first, 0)) !=
                    preconds.end())
            {
        ...
        for (Updates::iterator j = decreasers.begin(); j != decreasers.end() && cval != E_ALL; ++j)
        {
            ...
            if (FAverbose && cval == E_ALL &&
                std::find(preconds.begin(), preconds.end(),
                          pair<operator_ *, derivation_rule *>(j->first, 0)) != preconds.end())
            {
                cout << "But note that a precondition applies\n";
            };
        };
