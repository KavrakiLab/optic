...
namespace VAL
{
    ...
    {
        for (std::set<const pddl_type *>::const_iterator i = pts.begin(); i != pts.end(); ++i)
        {
    ...
    struct matchFunc
    {
        ...
        bool operator()(const func_decl *fd) const
        {
            if (!fd)
            {
                if (Verbose)
                {
                    *report << "Problematic function declaration!\n";
                }
                ParseFailure pe;
                throw(pe);
            };
            ...
            if (!fdf)
            {
                if (Verbose)
                {
                    *report << *fd << " problematic function declaration!\n";
                }
                ParseFailure pe;
                throw(pe);
            };
    ...
    bool TypeChecker::subType(const pddl_typed_symbol *tp1, const pddl_typed_symbol *tp2)
    {
        ...
        if (tp1->type)
        {
            ...
            else
            {
                ...
                if (Verbose)
                {
                    *report << tp2->getName() << " has bad type definition\n";
                }
                TypeException te;
                throw(te);
            };
        ...
        else
        {
            ...
            if (!tp1->either_types)
            {
                if (Verbose)
                {
                    *report << tp1->getName() << " badly typed!\n";
                }
                TypeException te;
                throw(te);
            };
    ...
    {
        if (!isTyped || !thea->the_domain->predicates)
        {
            return true;
        }
        pred_decl_list::const_iterator prd = std::find_if(
            thea->the_domain->predicates->begin(), thea->the_domain->predicates->end(), matchPred(p->head));
    ...
    bool TypeChecker::typecheckActionInstance(const plan_step *p)
    {
        ...
        operator_list::const_iterator op =
            std::find_if(thea->the_domain->ops->begin(), thea->the_domain->ops->end(), matchOp(p->op_sym));
        ...
        for (const_symbol_list::const_iterator i = p->params->begin(); i != e; ++i, ++arg)
        {
            if (!subType(*i, *arg))
            {
                if (Verbose)
                {
                    *report << "Type problem in action " << *p << "\n";
                }
                return false;
            };
        };
    ...
    bool TypeChecker::typecheckGoal(const goal *g)
    {
        ...
        {
            if (cg->getGoals()->end() ==
                std::find_if(cg->getGoals()->begin(), cg->getGoals()->end(), badchecker(this)))
            {
                return true;
            }
            return false;
        ...
        {
            if (dg->getGoals()->end() ==
                std::find_if(dg->getGoals()->begin(), dg->getGoals()->end(), badchecker(this)))
            {
                return true;
            }
            return false;
    ...
    bool TypeChecker::typecheckEffect(const effect *e)
    {
        ...
        {
            return typecheckGoal(ce->getCondition()) && typecheckEffects(ce->getEffects());
        };
    ...
    bool TypeChecker::typecheckEffects(const effect_lists *es)
    {
        ...
        return (es->add_effects.end() ==
                std::find_if(es->add_effects.begin(), es->add_effects.end(), badchecker(this))) &&
               (es->del_effects.end() ==
                std::find_if(es->del_effects.begin(), es->del_effects.end(), badchecker(this))) &&
               (es->forall_effects.end() ==
                std::find_if(es->forall_effects.begin(), es->forall_effects.end(), badchecker(this))) &&
               (es->cond_effects.end() ==
                std::find_if(es->cond_effects.begin(), es->cond_effects.end(), badchecker(this))) &&
               (es->assign_effects.end() ==
                std::find_if(es->assign_effects.begin(), es->assign_effects.end(), badchecker(this))) &&
               (es->timed_effects.end() ==
                std::find_if(es->timed_effects.begin(), es->timed_effects.end(), badchecker(this)));
    };
    ...
    bool TypeChecker::typecheckAction(const operator_ *act)
    {
        ...
        if (Verbose)
        {
            *report << "Type-checking " << act->name->getName() << "\n";
        }
        if (!typecheckGoal(act->precondition))
        {
            if (Verbose)
            {
                *report << "Conditions fail type-checking.\n";
            }
            return false;
        }
        if (!typecheckEffects(act->effects))
        {
            if (Verbose)
            {
                *report << "Effects fail type-checking.\n";
            }
            return false;
        }
        if (const durative_action *da = dynamic_cast<const durative_action *>(act))
        {
            if (!typecheckGoal(da->dur_constraint))
            {
                if (Verbose)
                {
                    *report << "Duration constraint fails type-checking.\n";
                }
                return false;
            };
        };
        if (Verbose)
        {
            *report << "...action passes type checking.\n";
        }
        return true;
    };
    ...
    bool TypeChecker::typecheckDerivationRule(const derivation_rule *d)
    {
        ...
        if (Verbose)
        {
            *report << "Type-checking derivation rule for " << (d->get_head()->head->getName()) << "\n";
        }
    ...
    bool TypeChecker::typecheckDomain()
    {
        ...
        return thea->the_domain->ops->end() == std::find_if(thea->the_domain->ops->begin(),
                                                            thea->the_domain->ops->end(), badchecker(this)) &&
               thea->the_domain->drvs->end() == std::find_if(thea->the_domain->drvs->begin(),
                                                             thea->the_domain->drvs->end(),
                                                             badchecker(this)) &&
               (!thea->the_domain->constraints || typecheckGoal(thea->the_domain->constraints));
    };
    ...
    bool TypeChecker::typecheckProblem()
    {
        ...
        if (thea->the_problem->the_goal && !typecheckGoal(thea->the_problem->the_goal))
        {
            if (Verbose)
            {
                *report << "Type-checking goal failed\n";
            }
            return false;
        }
        if (!typecheckEffects(thea->the_problem->initial_state))
        {
            if (Verbose)
            {
                *report << "Type-checking initial state failed\n";
            }
            return false;
        }
        if (thea->the_problem->constraints && !typecheckGoal(thea->the_problem->constraints))
        {
            if (Verbose)
            {
                *report << "Type-checking constraints failed\n";
            }
            return false;
        }
        return true;
    ...
    vector<const_symbol *> TypeChecker::range(const var_symbol *v)
    {
        ...
        for (const_symbol_table::const_iterator i = thea->const_tab.begin(); i != thea->const_tab.end(); ++i)
        {
            if (subType(i->second, v))
            {
