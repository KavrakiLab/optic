 ...
 * Stephen
 * Cresswell - PDDL Parser
 *
 * This file is part of VAL, the PDDL validator.
 *
 * VAL is free software: you
 ...
 * along
 * with VAL.  If not, see <http://www.gnu.org/licenses/>.
 *
...
namespace VAL
{
    ...
    string getName(plan_step *ps)
    {
        ...
        for (typed_symbol_list<const_symbol>::const_iterator j = ps->params->begin(); j != ps->params->end();
             ++j)
        {
    ...
    DerivationRules::DerivationRules(const derivations_list *d, const operator_list *o) : ops(o), derivPreds()
    {
        ...
        {
            map<string, pair<const goal *, const var_symbol_table *>>::iterator dp =
                derivPreds.find((*i)->get_head()->head->getName());
            if (dp != derivPreds.end())
            {
                if (dp->second.second->size() != (*i)->get_head()->args->size())
                {
                    ...
                    *report << "Derived predicates of the same name must have the same number of "
                               "parameters!\n";
                    if (LaTeX)
                    {
                        *report << "\\end{document}\n";
                ...
                for (parameter_symbol_list::iterator j = (*i)->get_head()->args->begin();
            ...
            else
            {
                ...
                char count = 41;  // to ensure ordering of bindings is correct when vst is looped thro which
                                  // is a map which orders its loop with its key, (we dont want the name of
                                  // parameter just var sym address and its order of appearance)

                for (parameter_symbol_list::const_iterator j = (*i)->get_head()->args->begin();
                     j != (*i)->get_head()->args->end(); ++j)
                {
                    if (dynamic_cast<const var_symbol *>(*j))
                    {
    ...
    // this changes the bindings of a DP to match the bindings of its disjuctive counterpart as given by the
    // map
    ...
    bool DerivationRules::stratification() const
    {
        ...
        for (pc_list<derivation_rule *>::const_iterator i = drvs->begin(); i != drvs->end(); ++i)
        {
            if (analyseDPs[make_pair(*i, *i)] == 2)
            {
                return false;
            }
        };

        if (!Verbose || drvs->size() == 0)
        {
            return true;
        }
        ...
        while (true)
        {
            ...
            for (map<derivation_rule *, unsigned int>::const_iterator i = remaining.begin();
                 i != remaining.end(); ++i)
            {
            ...
            for (map<derivation_rule *, unsigned int>::const_iterator j = remaining.begin();
                 j != remaining.end(); ++j)
            {
...

                {
                    ...
                    for (map<derivation_rule *, unsigned int>::const_iterator i = remaining.begin();
                         i != remaining.end(); ++i)
                    {
                        if (remaining[i->first] == 1 && analyseDPs[make_pair(i->first, j->first)] == 2)
                        {
                            ijr2 = false;
                        }
                    };
            ...
            for (vector<derivation_rule *>::const_iterator k = toRemove.begin(); k != toRemove.end(); ++k)
            {
                remaining[*k] = 0;
            }
        ...
        if (LaTeX)
        {
            *report << "\\subsection{Stratification}\n";
            for (vector<pair<unsigned int, vector<string>>>::const_iterator i = stratification.begin();
    ...
    const goal *DerivationRules::NNF(const goal *gl) const
    {
        ...
        if (dynamic_cast<const neg_goal *>(g))
        {
            ...
            if (dynamic_cast<const conj_goal *>(ng))
            {
                ...
                for (pc_list<goal *>::const_iterator i = cg->getGoals()->begin(); i != cg->getGoals()->end();
                     ++i)
                {
            ...
            if (dynamic_cast<const disj_goal *>(ng))
            {
                ...
                for (pc_list<goal *>::const_iterator i = dg->getGoals()->begin(); i != dg->getGoals()->end();
                     ++i)
                {
                    goal *agoal = const_cast<goal *>(NNF(new neg_goal(new goal(*const_cast<goal *>(*i)))));
                    gl->push_back(agoal);
            ...
            if (dynamic_cast<const qfied_goal *>(ng))
            {
                ...
                {
                    ans = new qfied_goal(
                        E_FORALL, new var_symbol_list(*const_cast<var_symbol_list *>(qg->getVars())),
                ...
                {
                    ans = new qfied_goal(
                        E_EXISTS, new var_symbol_list(*const_cast<var_symbol_list *>(qg->getVars())),
                        const_cast<goal *>(NNF(new neg_goal(const_cast<goal *>(qg->getGoal())))),
                        new var_symbol_table(*const_cast<var_symbol_table *>(qg->getSymTab())));
                }
...

        {
            ...
            if (qg->getQuantifier() == E_EXISTS)
            {
                ans = new qfied_goal(E_EXISTS,
                                     new var_symbol_list(*const_cast<var_symbol_list *>(qg->getVars())),
                                     const_cast<goal *>(NNF(const_cast<goal *>(qg->getGoal()))),
                                     new var_symbol_table(*const_cast<var_symbol_table *>(qg->getSymTab())));
            }
            else
            {
                ans = new qfied_goal(E_FORALL,
                                     new var_symbol_list(*const_cast<var_symbol_list *>(qg->getVars())),
                                     const_cast<goal *>(NNF(const_cast<goal *>(qg->getGoal()))),
                                     new var_symbol_table(*const_cast<var_symbol_table *>(qg->getSymTab())));
            }
    ...
    {
        for (pc_list<simple_effect *>::const_iterator e1 = efflist->add_effects.begin();
             e1 != efflist->add_effects.end(); ++e1)
        {
            if (isDerivedPred((*e1)->prop->head->getName()))
            {
                return false;
            }
        };

        for (pc_list<simple_effect *>::const_iterator e2 = efflist->del_effects.begin();
             e2 != efflist->del_effects.end(); ++e2)
        {
            if (isDerivedPred((*e2)->prop->head->getName()))
            {
                return false;
            }
        };

        for (pc_list<forall_effect *>::const_iterator e3 = efflist->forall_effects.begin();
             e3 != efflist->forall_effects.end(); ++e3)
        {
            if (!effects((*e3)->getEffects()))
            {
                return false;
            }
        };

        for (pc_list<cond_effect *>::const_iterator e4 = efflist->cond_effects.begin();
             e4 != efflist->cond_effects.end(); ++e4)
        {
            if (!effects((*e4)->getEffects()))
            {
                return false;
            }
        };

        for (pc_list<assignment *>::const_iterator e5 = efflist->assign_effects.begin();
    ...
    bool DerivationRules::effects() const
    {
        ...
        {
            for (pc_list<simple_effect *>::const_iterator e1 = (*i)->effects->add_effects.begin();
            ...
            for (pc_list<simple_effect *>::const_iterator e2 = (*i)->effects->del_effects.begin();
            ...
            for (pc_list<forall_effect *>::const_iterator e3 = (*i)->effects->forall_effects.begin();
            ...
            for (pc_list<cond_effect *>::const_iterator e4 = (*i)->effects->cond_effects.begin();
            ...
            for (pc_list<assignment *>::const_iterator e5 = (*i)->effects->assign_effects.begin();
    ...
    DerivationRules::~DerivationRules()
    {
        ...
        for (map<string, pair<const goal *, const var_symbol_table *>>::iterator i = derivPreds.begin();
             i != derivPreds.end(); ++i)
        {
    ...
    bool Validator::execute()
    {
        ...
        while (thisStep != theplan.end())
        {
            ...
            if (isOK || ContinueAnyway)
            {
                ...
                if (isReg && events.hasEvents())
                {
                    events.updateEventsForMutexCheck(this);
                }
                ...
                if (!finalInterestingState && thisStep != theplan.end() && (*thisStep)->isAfterPlan())
                {
    ...
    struct comparePS
    {
        ...
        {
            if (ps1->start_time < ps2->start_time)
            {
                return true;
            }
    ...
    void Validator::displayInitPlanLaTeX(const plan *p) const
    {
        ...
        {
            if (!((*i)->start_time_given))
            {
                (*i)->start_time = ++countNoTimeStamp;
            }
            vps.push_back(*i);
        };
        ...
        *report << "Plan size: " << vps.size();
        if (p->getTime() >= 0)
        {
            *report << "\\\\Planner run time: " << p->getTime();
        }
...

    {
        if (stepLength)
        {
            return stepcount;  // Default is step count.
        }
    ...
    bool Validator::graphsToShow() const
    {
        ...
        for (map<const FuncExp *, FEGraph *>::const_iterator i = graphs.begin(); i != graphs.end(); ++i)
        {
            ...
            if ((disconts != i->second->discons.end()) || (conts != i->second->points.end()))
            {
                return true;
            }
        };
    ...
    void FEGraph::drawLaTeXAxis(double maxTime) const
    {
        ...
        *report << "\\put(" << -0.2 + ff << "," << timeAxisV << "){ 0 }\n";
        ...
        {
            *report << "\\put(" << ((*i) / maxTime) * graphMaxH * 0.9 + ff << "," << timeAxisV
                    << "){ \\line(0,-1){0.05} }\n";
        };
    };
...

    {
        if (minValue == 0 && maxValue == 0)
        {
            ...
            for (map<double, pair<double, double>>::const_iterator i1 = discons.begin(); i1 != discons.end();
                 ++i1)

            {
                if (i1->second.first > maxValue)
                {
                    maxValue = i1->second.first;
                }
                if (i1->second.first < minValue)
                {
                    minValue = i1->second.first;
                }
                if (i1->second.second > maxValue)
                {
                    maxValue = i1->second.second;
                }
                if (i1->second.second < minValue)
                {
                    minValue = i1->second.second;
                }
            };
    ...
    void FEGraph::drawLaTeXLine(double t1, double y1, double t2, double y2, double maxTime) const
    {
        ...
        if ((minValue > 0 && (y1 < minValue || y2 < minValue)) || y1 > maxValue || y2 > maxValue)
        {
            return;
        }
        ...
        if ((lt2 - lt1 > 0.0041))
        {
            *report << "\\qbezier(" << lt1 << "," << ly1 << ")(" << (lt1 + lt2) / 2 << "," << midValue << ")("
                    << lt2 << "," << ly2 << ")\n";
        };
    };
    ...
    void FEGraph::displayLaTeXGraph(double maxTime)
    {
        ...
        if ((disconts == discons.end()) && (conts == points.end()))
        {
            return;
        }
            ...
            if ((conts != points.end()) && (conts->first < disconts->first))
            {
                ...
                for (; conts->first <= disconts->first;)
                {
                    ...
                    if ((conts == points.end()) || (conts->first > disconts->first))
                    {
                        if (!ctsActivity)
                        {
                            drawLaTeXLine(prevTime, prevValue, disconts->first, disconts->second.first,
                                          maxTime);
                        }
                        else
                        {
                            drawLaTeXLine(nextTime, nextValue, disconts->first, disconts->second.first,
                                          maxTime);
                        }
...

    {
        ...
        int totalEls = 0;
        for (map<int, map<int, GanttElement *>>::const_iterator r = chartRows.begin(); r != chartRows.end();
             ++r)
        {
        ...
        for (map<int, map<int, GanttElement *>>::const_iterator r1 = chartRows.begin(); r1 != chartRows.end();
             ++r1)
        {
            for (map<int, GanttElement *>::const_iterator ge = r1->second.begin(); ge != r1->second.end();
                 ++ge)
            {
                if (ge->second->start != ge->second->end)
                {
                    ...
                    for (int n = 2;; ++n)
                    {
                        ...
                        else if ((length < smallElement / n) && (length >= smallElement / (n + 1)))
                        {
                            ++smallElCount[n];
                            break;
                        };
    ...
    void Gantt::drawLaTeXGantt(const Plan &p, int noPages, int noPageRows)
    {
        ...
        for (int pg = 1; pg <= noOfPages; ++pg)
        {
...

            {
                ...
                if (pg == 1)
                {
                    numRowsPg1 = endRow - startRow + 1;
                }

                if (endRow > numRows || pgr == noOfPageRows)
                {
                    endRow = numRows;
                }

                drawLaTeXGantt((maxTime / noOfPages) * (pg - 1), (maxTime / noOfPages) * pg, startRow, endRow,
                               numRowsPg1);
            };
        };
    };
    ...
    vector<string> Gantt::getSigObjs(const Action *a)
    {
        ...
        for (var_symbol_list::const_iterator i = a->getAction()->parameters->begin();
             i != a->getAction()->parameters->end(); ++i)
        {
            ...
            if (ii != sigObjs.end())
            {
                ...
                if (j == usedSigObjs.end())
                {
                    usedSigObjs.push_back(par);
                }
            };
        };
            ...
            if (i.isRegular())
            {
                ...
                for (vector<const Action *>::const_iterator a = actions.begin(); a != actions.end(); ++a)
                {
                    if (!(dynamic_cast<EndAction *>(const_cast<Action *>(*a))))
                    {
                        ...
                        for (map<int, map<int, GanttElement *>>::const_iterator r = chartRows.begin();
                             r != chartRows.end(); ++r)
                        {
                            ...
                            if (ge->second->end < (*i)->getTime())
                            {
                                ...
                                {
                                    vector<string>::iterator fso = std::find(ge->second->sigObjs.begin(),
                        ...
                        else
                        {
...

                            {
                                ...
                                // find the largest last time smaller than start time of this action from rows
                                // - dont choose row with sig objs
                                for (map<int, map<int, GanttElement *>>::const_iterator r = chartRows.begin();
                                     r != chartRows.end(); ++r)
                                {
                                    map<int, GanttElement *>::const_iterator ge = r->second.end();
                                    ...
                                    if ((ge->second->end > largestTime) &&
                                        (ge->second->end < (*i)->getTime()) && (getSigObj(r->first) == ""))
                                    {
                                        largestTime = ge->second->end;
                                        row = r->first;
                                    };
    ...
    string Gantt::getSigObj(int r)
    {
        ...
        for (map<int, GanttElement *>::const_iterator ge = chartRows[r].begin(); ge != chartRows[r].end();
             ++ge)
        {
            for (vector<string>::const_iterator so = sigObjs.begin(); so != sigObjs.end(); ++so)
            {
                vector<string>::const_iterator fso =
                    std::find(ge->second->sigObjs.begin(), ge->second->sigObjs.end(), *so);
                if (fso != ge->second->sigObjs.end())
                {
    ...
    void Gantt::shuffleRows()
    {
        ...
        if ((chartRows.size() < 3) || (usedSigObjs.size() == 0))
        {
            return;
        }
    ...
    void Gantt::displayKey()
    {
        ...
        for (map<int, map<int, GanttElement *>>::const_iterator i = chartRows.begin(); i != chartRows.end();
             ++i)
        {
    ...
    string Gantt::getColour(int row)
    {
        ...
        if (r != 0 || b != 0 || g != 0)
        {
            ans = "[rgb]{" + toString(r * pow(0.7, cyc)) + "," + toString(g * pow(0.75, cyc)) + "," +
                  toString(b * pow(0.65, cyc)) + "}";
        }
        else
        {
            return "";
    ...
    void Gantt::drawLaTeXGantt(double startTime, double endTime, int startRow, int endRow, int numRows)
    {
        ...
        *report << "\\put(" << ff + 0.02 - 0.12 << ", " << 0 << "){\\begin{sideways} " << startTime
                << " \\end{sideways}}\n";
        *report << "\\put(" << ff << "," << graphV * 0.9 << "){ \\line(-1,0){0.05} }\n";
        *report << "\\put(" << ff - 0.12 << "," << graphV * 0.9 + 0.05 << "){\\begin{sideways} " << endTime
                << " \\end{sideways}}\n";
             ...
             ++i)
        {
            if (i->first >= startRow && i->first <= endRow)
            {
                ...
                *report << "\\normalcolor\n";
                for (map<int, GanttElement *>::const_iterator j = i->second.begin(); j != i->second.end();
                     ++j)
                {
                ...
                for (map<int, GanttElement *>::const_iterator j1 = i->second.begin(); j1 != i->second.end();
                     ++j1)
                {
                          ...
                          || ((j1->second->start > endTime) && (j1->second->end > endTime))))
                    {
                        ...
                        {
                            drawLaTeXDAElement(j1->second, i->first - startRow + 1, j1->first, startTime,
                                               endTime, numRows);
                        }

                        else
                        {
                            drawLaTeXElement(j1->second, i->first - startRow + 1, j1->first, startTime,
                                             endTime, numRows);
                        };
                    };
                };
    ...
    void Gantt::drawRowNums(int startRow, int endRow, int numRows)
    {
        ...
        {
            *report << "\\put(" << spacing * (r - startRow + 0.5) << "," << 0 - 0.15 << "){\\begin{sideways}"
                    << r << " \\end{sideways}}\n";
        };
    };

    void Gantt::drawLaTeXDAElement(const GanttElement *ge, int row, int pos, double startTime, double endTime,
                                   int numRows) const
    {
        double ff = -0.0515;
        double ff2 = 0.005;
        ...
        {
            *report << "\\put(" << p1.first << "," << p1.second << "){\\framebox(" << height << "," << length
...

        {
            ...
            *report << "\\put(" << ff + p1.first << "," << p1.second << "){ \\line(0,1){" << length << "} }\n"
                    ...
                    << "} }\n"
                    << "\\put(" << ff + p1.first + height / 2 << "," << p1.second + length / 2
                    << "){ \\begin{sideways} " << pos << "  \\end{sideways} }\n";
        }
    ...
    void Gantt::drawLaTeXElement(const GanttElement *ge, int row, int pos, double startTime, double endTime,
                                 int numRows) const
    {
        int numBoxes;
        ...
        *report << "\\put(" << p4.first << "," << p4.second << "){\\framebox(" << boxSize << "," << boxSize
                << ")[l]{ \\begin{sideways} {\\tiny " << pos << "  } \\end{sideways}}}\n";
    ...
    void PlanRepair::advice(ErrorLog &el)
    {
        ...
        for (vector<const UnsatCondition *>::const_iterator i = el.getConditions().begin();
    ...
    void PlanRepair::firstPlanAdvice()
    {
        ...
        if (ErrorReport && getUnSatConditions().size() != 0)
        {
            repairPlan();
        }
    };
    ...
    pair<double, double> getSlideLimits(const plan *aPlan, string actionName, double currentTime,
                                        double deadLine)
    {
        ...
        if (endTime != -1)
        {
            ...
            if (slideLimits.first < (endSlideLimits.first - duration) && (endSlideLimits.first != startTime))
            {
                slideLimits.first = endSlideLimits.first - duration;
            }
            if (!(slideLimits.second < (endSlideLimits.second - duration) && (slideLimits.second != endTime)))
            {
    ...
    double getMaxTime(const plan *aPlan)
    {
        ...
        for (pc_list<plan_step *>::const_iterator i = aPlan->begin(); i != aPlan->end(); ++i)
        {
            if ((*i)->start_time > maxTime)
            {
                maxTime = (*i)->start_time;
            }
            if ((*i)->duration_given && (((*i)->start_time + (*i)->duration) > maxTime))
            {
    ...
    void changeActionTime(const plan *aPlan, string actionName, double currentTime,
                          double newTime)  //,include double current time
    {
        for (pc_list<plan_step *>::const_iterator i = aPlan->begin(); i != aPlan->end(); ++i)
        {  //  cout <<  getName(*i) << " == " << actionName <<" && "<< (*i)->start_time <<" ==
           //  "<<currentTime<<"\n";
            if (getName(*i) == actionName && (*i)->start_time == currentTime)
            {
                (*i)->start_time = newTime;
    ...
    void PlanRepair::repairPlan()
    {
        ...
        if (firstPlanStep != 0)
        {
            ...
            if (!planRepaired)
            {
                planRepaired = shakePlan(repairingPlan, firstPlanStep,
        ...
        for (vector<plan_step *>::const_iterator i = timedInitialLiteralActions.begin();
             i != timedInitialLiteralActions.end(); ++i)
        {
            if (!((*i)->op_sym->getName().length() > 28 && (*i)->op_sym->getName().substr(0, 28) == "Timed "
                                                                                                    "Initial "
                                                                                                    "Literal "
                                                                                                    "Action"))
            {
    ...
    // for every action after and including firstAction shift the start time by shiftTime, taking into account
    // that this plan is already shifted starttime is the original time of the first action
    void moveActionTimes(const plan *aPlan, const plan_step *firstAction, double startTime,
    ...
    bool PlanRepair::slideEndOfPlan(const plan *repairingPlan, const plan_step *firstAction)
    {
        ...
        while (searching)
        {
            shiftTime = shiftMax * aNumber(counter++);  // start time w.r.t. the first action in the end
                                                        // section of actions
            ...
            for (vector<plan_step *>::iterator ps = timedInitialLiteralActions.begin();
            ...
            planRepairValidator =
                new Validator(v.getDerivRules(), v.getTolerance(), typeC, operators, initialState, testPlan,
                              metric, stepLength, durative, current_analysis->the_domain->constraints,
            ...
            if (!anError && (planRepairValidator->getErrorLog().getConditions().size() == 0) &&
            ...
            if (counter >= 8191 ||
                ((shiftTime - lastShiftTime) < v.getTolerance() && (shiftTime - lastShiftTime) > 0))
            {
                searching = false;
            }
    ...
    */
    map<const plan_step *, const plan_step *> varyPlanTimestampsUniform(plan *aplan, const plan *p,
                                                                        double &variation, double deadLine)
    {
        ...
        for (pc_list<plan_step *>::iterator ps = aplan->begin(); ps != aplan->end(); ++ps, ++origPlanStep)
        {
            ...
            else if ((*ps)->duration_given && (((*ps)->start_time + (*ps)->duration) > deadLine))
            {
    ...
    bool PlanRepair::shakePlan(const plan *repairingPlan, const plan_step *firstAction, double variation)
    {
        ...
        while (searching)
        {
                ...
                (planRepairValidator->checkGoal(theGoal)))
            {
                ...
                for (map<const plan_step *, const plan_step *>::iterator i = planStepMap.begin();
                     i != planStepMap.end(); ++i)
                {
    ...
    bool PlanRepair::repairPlanOneActionAtATime(const plan *repairingPlan, const plan_step *firstAction)
    {
        ...
        // action that is not fixed after an action which is fixed; only try to repair an action twice if some
        // action has been fixed in the meantime.
        ...
        while (continueRepairing)
        {
            ...
            {
                if (fixOneAction.second.first)
                {
                    actionNotFixed = fixOneAction.first;
                }
    ...
    set<const Action *> PlanRepair::getUniqueFlawedActions(Validator *vld)
    {
        ...
        for (vector<const UnsatCondition *>::const_iterator j = unsatPlanRepairConditions.begin();
             j != unsatPlanRepairConditions.end(); ++j)
        {
            const Action *anAction = 0;

            if (const UnsatInvariant *usi = dynamic_cast<const UnsatInvariant *>(*j))
            {
                anAction = usi->action;
            }
            else if (const UnsatPrecondition *usp = dynamic_cast<const UnsatPrecondition *>(*j))
            {
                anAction = usp->action;
            }
    ...
    pair<const plan_step *, pair<bool, bool>> PlanRepair::repairPlanOneAction(const plan *repairingPlan,
                                                                              const plan_step *firstAction)
    {
        ...
        while (true)
        {
            ...
            plan *testPlan2 = new plan(*testPlan);  // we can delete the test plan now without deleting the
                                                    // timed initial literal actions
            ...
            if (actionFixed || (counter >= 1023) ||
                ((slideLimits.second - slideLimits.first) * bNumber < v.getTolerance()))
            {
    ...
    vector<plan_step *> getTimedInitialLiteralActions(analysis *current_analysis)
    {
        ...
        if (current_analysis->the_problem->initial_state->timed_effects.size() != 0)
        {
            ...
            for (pc_list<timed_effect *>::const_iterator e =
                     current_analysis->the_problem->initial_state->timed_effects.begin();
                 e != current_analysis->the_problem->initial_state->timed_effects.end(); ++e)
            {
                operator_symbol *timed_initial_lit =
                    current_analysis->op_tab.symbol_put("Timed Initial Literal Action " + toString(count++));

                action *timed_initial_lit_action =
                    new action(timed_initial_lit, new var_symbol_list(), new conj_goal(new goal_list()),
                               (*e)->effs, new var_symbol_table());
    ...
    // compared with earlier versions (VAL-3.2 in particular)
    {
     ...
     //if a duractive action invariant error log is split into two then only consider one of them, as only
    interested in action vector<const UnsatCondition *> unsatConditionsTemp = getUnSatConditions();
     vector<const UnsatCondition *> unsatConditions;
     bool addToConds;
     for(vector<const UnsatCondition *>::const_iterator r = unsatConditionsTemp.begin(); r !=
    unsatConditionsTemp.end(); ++r)
     {
         addToConds = true;
         for(vector<const UnsatCondition *>::const_iterator s = unsatConditions.begin(); s !=
    unsatConditions.end(); ++s)
         {
     ...
     //loop thro' unsatisfied conditions looking for unsatisfied invariant conditions that can be 'slided
    around' and try and repair plan for(vector<const UnsatCondition *>::const_iterator i =
    unsatConditions.begin(); i != unsatConditions.end(); )
     {
            if(const UnsatInvariant * usi = dynamic_cast<const UnsatInvariant*>(*i))
            {
               ...
               pair<double,double> slideLimits = usi->getSlideLimits(repairingPlan,v.getTolerance());//times
    the start point can be slid without it or endpoint of durative action crossing another endpoint
                //cout << "Slide limits "<< actionName <<" = "<< slideLimits.first <<" , "<<
    slideLimits.second <<"\n";

               if(slideLimits.first != usi->startTime || slideLimits.second != usi->startTime)
               {
                //search for a new place for the durative action, use the repaired plan for the next
    unsatisfied condition

                int startingPoint = 1;
                int limit =  int((slideLimits.second - slideLimits.first) / (usi->endTime - usi->startTime)
    + 1.5 + 2.0);
                //cout << limit <<" =limit\n";
    ...
    unsatPlanRepairConditions.begin(); j != unsatPlanRepairConditions.end(); )
                          {
                                 ...
                                 {
                                      //since action is only sliding about between other actions the durative
    action is not split into > 1 errorlog entries if(actionName == usi2->action->getName0()) break;
                                 };
                                 ...
                                 if(j == unsatPlanRepairConditions.end())
                                 {
                                    ...
                                    continueRepairingTemp = true; //ie this condition is repaired so may be
    possible to repair other errors satConditions.push_back(*i);
                     ...
                     if(((slideLimits.second - slideLimits.first)*bNumber < v.getTolerance()) ||
    continueRepairingTemp || startingPoint > limit) break; else {
    ...
    continueValidator->getErrorLog().getConditions(); for(vector<const UnsatCondition *>::const_iterator r =
    unsatConditionsTemp.begin(); r != unsatConditionsTemp.end(); ++r)
               {
                   addToConds = true;
                   for(vector<const UnsatCondition *>::const_iterator s = unsatConditions.begin(); s !=
    unsatConditions.end(); ++s)
    ...
    toBeDeletedValidator = continueValidator;
     ...
     {
         if(LaTeX) *report << "\\subsubsection{Similar Valid Plan}\n Failed to find a valid similar plan
    {\\begin{rotate}{270}:-( \\end{rotate}}"; else cout << "\nFailed to find a valid similar plan :-(\n";
