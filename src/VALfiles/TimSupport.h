...
namespace TIM
{
    ...
    public:
        ...
        ObjectIt oBegin()
        {
            return exhibitors.begin();
        };

        ObjectIt oEnd()
        {
            return exhibitors.end();
        };
        ...
        int familySize() const
        {
            return EPS(predicate)->arity();
        };
        ...
        const VAL::extended_pred_symbol *root() const
        {
            return EPS(predicate);
        };
    ...
    public:
        ...
        template <class TI>
        TIMpredSymbol(pred_symbol *p, TI s, TI e) : extended_pred_symbol(p, s, e), props(e - s)
        {
            for_each(props.begin(), props.end(), setUpProps(this));
        ...
        bool checkMutex(TI sa, TI ea, TIMpredSymbol *pb, TI sb, TI eb)
        {
            ...
            if (pb == this)
            {
                for (vector<pair<int, int>>::const_iterator p = i->second.begin(); p != i->second.end(); ++p)
                {
...
#define cTPS(x) const_cast<TIMpredSymbol *>(static_cast<const TIMpredSymbol *>(x))
    ...
    public:
        ...
        const vector<VAL::proposition *> &getInits() const
        {
            return initialps;
        };
...
#define TOB(x) static_cast<TIM::TIMobjectSymbol *>(x)
    ...
    {
    private:
        typedef multiset<Property *, std::less<Property *>> Properties;
    ...
    public:
        ...
        {
            vector<Property *> props;
            transform(s, e, inserter(props, props.begin()), bind2nd(mem_fun(&Property::getBaseProperty), pt));
        ...
        PSIterator begin() const
        {
            return properties.begin();
        };

        PSIterator end() const
        {
        ...
        PropertyState *adjust(const PropertyState *del, const PropertyState *add)
        {
            ...
            vector<Property *> ps;
            set_difference(properties.begin(), properties.end(), del->properties.begin(),
                           del->properties.end(), inserter(ps, ps.begin()));
    ...
    public:
        ...
        PropertySpace(Property *p) : states(), rules(), properties(1, p), objects(), isStateValued(true){};
        void checkStateValued();

        void merge(PropertySpace *ps)
        {
            copy(ps->states.begin(), ps->states.end(), inserter(states, states.end()));
            copy(ps->rules.begin(), ps->rules.end(), inserter(rules, rules.end()));
            copy(ps->properties.begin(), ps->properties.end(), inserter(properties, properties.end()));
            copy(ps->objects.begin(), ps->objects.end(), inserter(objects, objects.end()));
        ...
        OIterator obegin() const
        {
            return objects.begin();
        };

        OIterator oend() const
        {
            return objects.end();
        };
    ...
    public:
        TransitionRule(TIMAnalyser *t, VAL::operator_ *o, int v, PropertyState *e, PropertyState *l,
                       PropertyState *r, opType ty = INSTANT);

        TransitionRule(TIMAnalyser *t, VAL::derivation_rule *o, int v, PropertyState *e, PropertyState *l,
                       PropertyState *r, opType ty = INSTANT);
        ...
        {
            o << (*enablers) << " => " << (*lhs) << " -> " << (*rhs)
              << (isAttribute() ? " attribute rule: " : "") << (isIncreasing() ? "increasing" : "")
        ...
        const PropertyState *getLHS() const
        {
            return lhs;
        };

        const PropertyState *getRHS() const
        {
            return rhs;
        };

        const PropertyState *getEnablers() const
        {
            return enablers;
        };

        const VAL::operator_ *byWhat() const
        {
            return op;
        };
    ...
    public:
        ...
        const vector<VAL::pred_symbol *> &getIgnores() const
        {
            return toIgnore;
        };
        ...
        virtual void visit_durative_action(VAL::durative_action *p)
        {
            ...
            if (inserting)
            {
                ...
                timed_goal *tg =
                    new timed_goal(new simple_goal(new proposition(nm, p->parameters), E_POS), E_OVER_ALL);
                if (p->precondition)
                {
    ...
    public:
        ...
        RCiterator begin() const
        {
            return rules.begin();
        };

        RCiterator end() const
        {
            return rules.end();
        };
...
#define TASc(x) static_cast<const TIM::TIMactionSymbol *const>(x)
    ...
    public:
        ...
        {
            return propspaces.begin();
        };

        const_iterator pend() const
        {
            return propspaces.end();
        };

        const_iterator abegin() const
        {
            return attrspaces.begin();
        };

        const_iterator aend() const
        {
            return attrspaces.end();
        };

        const_iterator sbegin() const
        {
            return staticspaces.begin();
        };

        const_iterator send() const
        {
            return staticspaces.end();
        };
    };
    ...
    struct mRec
    {
        ...
        {
            return (first < m.first || (first == m.first && second < m.second) || opt < m.opt);
        };
    };
...
#define MEX(x) dynamic_cast<TIM::MutexStore *>(x)
    ...
    {
    public:
        TIMaction(VAL::operator_symbol *nm, VAL::var_symbol_list *ps, VAL::goal *pre, VAL::effect_lists *effs,
...
#define TDA(x) dynamic_cast<TIM::TIMdurativeAction *>(x)
    ...
    struct mutRec
    {
        ...
        bool operator<(const mutRec &m) const
        {
            return (first < m.first ||
                    (first == m.first &&
                     (second < m.second ||
                      (second == m.second && (one < m.one || (one == m.one && other < m.other))))));
        };
    };
    ...
    public:
        ...
        unsigned int getMutexes(VAL::operator_ *A, TI sa, TI ea, TI sb, TI eb)
        {
            ...
            for (set<mutRec>::const_iterator i = argPairs.begin(); i != argPairs.end(); ++i)
            {
                ...
                {
                    //			cout << "Mutex for " << i->first << " " << **(getIx(sa,i->first)) << " and "
                    //<< i->second
                    //					<< " " << **(getIx(sb,i->second)) << " " << i->one << " " << i->other
                    //<<
                    //"
                    //"
        ...
        bool selfMutex(TI sa, TI ea)
        {
            for (set<mutRec>::const_iterator i = argPairs.begin(); i != argPairs.end(); ++i)
            {
                if (i->first != i->second && *(getIx(sa, i->first)) == *(getIx(sa, i->second)))
                {
                    return true;
                };
            };
    ...
    {
    public:
        virtual VAL::action *buildAction(VAL::operator_symbol *nm, VAL::var_symbol_list *ps, VAL::goal *pre,
                                         VAL::effect_lists *effs, VAL::var_symbol_table *st)
        {
