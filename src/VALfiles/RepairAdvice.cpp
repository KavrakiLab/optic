...
namespace VAL
{
    ...
    void ErrorLog::displayReport() const
    {
        ...
        for (vector<const UnsatCondition *>::const_iterator i = conditions.begin(); i != conditions.end();
             ++i)
        {
            (*i)->display();
        };
    ...
    void ErrorLog::addUnsatInvariant(double st, double e, Intervals ints, const Action *a, const State *s,
                                     bool rootError)
    {
