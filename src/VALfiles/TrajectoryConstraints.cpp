...
#define MESSAGE(x)                                                                                           \
    ...
    public:
        ...
        CollectProps(TrajectoryConstraintsMonitor &tcm) : isPref(false), share(false), tjm(tcm){};
        ...
        void visit_constraint_goal(constraint_goal *cg)
        {
            ...
            const Proposition *q = cg->getTrigger() ?
                                       (!e1 ? tjm.vld->pf.buildProposition(cg->getTrigger()) :
                                              tjm.vld->pf.buildProposition(cg->getTrigger(), *e1, true, 0)) :
                                       0;
    ...
    bool TrajectoryConstraintsMonitor::checkAtState(const State &s)
    {
        ...
        for (Deadlines::const_iterator i = within.begin(); i != within.end(); ++i)
        {
            ...
            {
                MESSAGE("Failed to meet deadline on " << *(i->second) << " (" << i->first << ")");
        ...
        for (PropositionPairs::const_iterator i = sometimeAfter.begin(); i != sometimeAfter.end(); ++i)
        {
            if ((i->first)->evaluate(&s))
            {
                MESSAGE("Triggered a requirement for " << *(i->second) << " after " << *(i->first) << " at "
                                                       << s.getTime());
                PropMonitor pm(i->second);
                passOn(pm, *i);
        ...
        for (Propositions::const_iterator i = never.begin(); i != never.end(); ++i)
        {
            if ((*i)->evaluate(&s))
            {
                MESSAGE("Constraint violated by satisfaction of " << **i << " at " << s.getTime());
                if (i->violationSerious(&s))
                {
                    return false;
                }
        ...
        for (PropositionPairs::const_iterator i = sometimeBefore.begin(); i != sometimeBefore.end(); ++i)
        {
            ...
            if ((i->first)->evaluate(&s))
            {
                MESSAGE(*(i->first) << " satisfied without " << *(i->second) << " sometime before");
                if (i->violationSerious(&s))
                {
                    return false;
                }
