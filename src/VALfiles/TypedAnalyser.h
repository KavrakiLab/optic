...
namespace VAL
{
    ...
    public:
        ...
        virtual void notify(void (extended_pred_symbol::*f)(operator_ *, const proposition *), operator_ *o,
                            const proposition *p){};
    ...
    public:
        ...
        {
            return records.get(makeIterator(f, p->args->begin()), makeIterator(f, p->args->end()));
        };
    ...
    public:
        ...
        pddl_type *operator*()
        {
            ...
            for (int i = 0; i < far; ++i)
            {
                ...
                //			cout << "Seeing set size " << n << " far: " << far << " count: " << count << " x:
                //"
                //<< x << "\n";
                if (n >= 2)
                {
            ...
            return const_cast<pddl_type *>(n >= 2 ? (*tpsSets)[far].second[x] : (*tpsSets)[far].first);
        };
    };
    ...
    public:
        CompoundPropStore(int c, vector<pair<pddl_type *, vector<const pddl_type *>>> &tps, TMap &t,
                          extended_pred_symbol *e, Associater *a);
        ...
        PropInfo *get(const proposition *p) const
        {
            SimplePropStore *s = records.get(typeIt(p->args->begin()), typeIt(p->args->end()));
            if (s)
            {
                return s->get(p);
            }
        ...
        void write(ostream &o) const
        {
            for (vector<SimplePropStore *>::const_iterator i = stores.begin(); i != stores.end(); ++i)
            {
                (*i)->write(o);
            };
        };

        void notify(void (extended_pred_symbol::*f)(operator_ *, const proposition *), operator_ *o,
                    const proposition *p);
     ...
     *
     *  Because multiple <code>extended_pred_symbol</code>s are created for each PDDL predicate, care must be
     * taken when interpreting the results of methods such as <code>appearsStatic()</code> - this will, for
     * instance, report whether a given typed variant of the predicate is static, not necessarily whether the
     * base PDDL predicate is static.  To ascertain properties of the base predicate as a whole, use
     * <code>getParent()</code> then with this iterate through all the derived
     * <code>extended_pred_symbol</code>s.
     *
    ...
    public:
        ...
        {
            int i = 0;
            for (parameter_symbol_list::iterator j = p->args->begin(); j != p->args->end(); ++j, ++i)
            {
                types[i] = (*j);  //->type;
            };
#ifdef STATICHACK
        ...
        Types::const_iterator tcBegin() const
        {
            return types.begin();
        };

        Types::const_iterator tcEnd() const
        {
            return types.end();
        };
        ...
        OpProps::const_iterator posPresEnd() const
        {
            return pospreconds.end();
        };

        OpProps::const_iterator addsBegin() const
        {
            return adds.begin();
        };

        OpProps::const_iterator addsEnd() const
        {
            return adds.end();
        };

        OpProps::const_iterator delsBegin() const
        {
            return dels.begin();
        };

        OpProps::const_iterator delsEnd() const
        {
            return dels.end();
        };
        ...
        void writeName(ostream &o) const
        {
            ...
            for (Types::const_iterator i = types.begin(); i != types.end(); ++i)
            {
                ...
                if ((*i)->type)
                {
                    ...
                    for (vector<const pddl_type *>::const_iterator x = ls.begin(); x != ls.end(); ++x)
                    {
                        o << (*x)->getName() << " ";
                    };
                    o << "] ";
        ...
        void write(ostream &o) const
        {
            o << "\nReport for: ";
            writeName(o);
            o << "\n------------\nInitial: " << initialState << " Goal: " << posgoalState << " positive/ "
              << neggoalState << " negative\nInitial state records:\n";
            records()->write(o);
            o << "\nPreconditions:\n";
            ...
            for (OpProps::const_iterator i = pospreconds.begin(); i != pospreconds.end(); ++i)
            {
                if (i->op)
                {
                    o << "\t" << i->op->name->getName() << "\n";
                ...
                if (i->drv)
                {
                    o << "\t" << i->drv->get_head()->head->getName() << "\n";
                }
            };
            if (!negpreconds.empty())
            {
                o << "-ve:\n";
                for (OpProps::const_iterator i = negpreconds.begin(); i != negpreconds.end(); ++i)
                {
                    if (i->op)
                    {
                        o << "\t" << i->op->name->getName() << "\n";
    ...
    struct EPSBuilder
    {
        ...
        virtual extended_pred_symbol *operator()(pred_symbol *nm, EPSTypeIterator s, EPSTypeIterator e)
        {
...
#define EPS(x) static_cast<VAL::extended_pred_symbol *>(const_cast<VAL::pred_symbol *>(x))
    ...
    public:
        holding_pred_symbol(const string &nm)
          : pred_symbol(nm), a(new NodeAssociater()), preds(), baseStores(), timedBases(){};
        ...
        PropStore *buildPropStore(extended_pred_symbol *e, const Types &tps, double t)
        {
#ifdef TADEBUG
        ...
        /** An iterator through the set of <code>extended_pred_symbol</code> objects for predicates of this
         * object's type. */
    ...
    public:
...
#ifdef TADEBUG
                            cout << "Recording precondition on " << p->getProp()->head->getName()
                                 << std::endl;
#endif
                        }
                        if (drv)
                        {
                            EPS(p->getProp()->head)->addPosPre(drv, p->getProp());
                        }
                    }
                    else
                    {
...
#ifdef TADEBUG
                            cout << "Recording negative precondition on " << p->getProp()->head->getName()
                                 << std::endl;
#endif
                        }
                        if (drv)
                        {
                            EPS(p->getProp()->head)->addNegPre(drv, p->getProp());
                        }
                    };
                }
            };
        ...
        virtual void visit_simple_effect(simple_effect *p)
        {
            ...
            else
            {
...
#ifdef TADEBUG
                        cout << "Recording add of " << p->prop->head->getName() << " at "
                             << EPS(p->prop->head) << std::endl;
...
#ifdef TADEBUG
                        cout << "Recording delete of " << p->prop->head->getName() << " at "
                             << EPS(p->prop->head) << std::endl;
                        assert(!EPS(p->prop->head)->isStatic());
#endif
                    };
                ...
                {
                    cout << "Effect on " << p->prop->head->getName() << " at " << EPS(p->prop->head)
                         << " has been filtered\n";
                };
            };
        };
        ...
        virtual void visit_domain(domain *p)
        {
            ...
            for (vector<durative_action *>::iterator i = das.begin(); i != das.end(); ++i)
            {
                ...
                if (tg)
                {
                    c = dynamic_cast<const comparison *>(tg->getGoal());
                }
                if (c)
                {
                    if (c->getOp() == E_EQUALS)
        ...
        virtual void visit_func_term(func_term *p)
        {
            ...
            else
            {
                if (op)
                {
                    EFT(p->getFunction())->addPre(op);
                }
                if (drv)
                {
                    EFT(p->getFunction())->addPre(drv);
                }
            }
        };
    };
