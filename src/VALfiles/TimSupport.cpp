...
namespace TIM
{
    ...
    TransitionRule::TransitionRule(TIMAnalyser *t, operator_ *o, int v, PropertyState *e, PropertyState *l,
    ...
    TransitionRule::TransitionRule(TIMAnalyser *t, derivation_rule *o, int v, PropertyState *e,
                                   PropertyState *l, PropertyState *r, opType ty)
      : tan(t)
      , op(0)
      , drv(o)
      , opt(ty)
    ...
    struct checkNotApplicable
    {
        ...
        checkNotApplicable(const_symbol *c) : tos(static_cast<TIMobjectSymbol *>(c)){};

        bool operator()(Property *p)
        {
            return find_if(p->begin(), p->end(), not1(bind2nd(mem_fun(&PropertySpace::contains), tos))) !=
                   p->end();
        };
    };
    ...
    private:
        ...
        void findValid()
        {
            ...
            {
                if (find_if(trule->enablers->begin(), trule->enablers->end(), checkNotApplicable(*obit)) ==
                    trule->enablers->end())
                {
                    break;
                }
    ...
    public:
        RuleObjectIterator(TransitionRule *tr) : trule(tr), obit(trule->objects.begin())
        {
            findValid();
        };
    ...
    struct extendWithIncrRule
    {
        ...
        void operator()(TransitionRule *tr)
        {
            if (tr->isIncreasing())
            {
                extended = for_each(tr->beginEnabledObjects(), tr->endEnabledObjects(), recordObjectIn(ps));
            };
        };
    ...
    struct extendWithStateRule
    {
        ...
        void operator()(TransitionRule *tr)
        {
            ...
            if (p && got.find(p) == got.end())
            {
                ...
                if (i != got.end())
                {
                    OUTPUT cout << *p << " is a superset of a state we already have: " << **i << "\n";
                }
                else
                {
                    got.insert(p);
    ...
    struct process_argument
    {
        ...
        template <class T>
        process_argument(TIMAnalyser *t, T *g) : ta(t), timps(findPred<T>(g)), arg(0), prp(0){};
    ...
    {
        process_argument_in_goal(TIMAnalyser *t, simple_goal *g) : process_argument(t, g){};
    ...
    {
        process_argument_in_effect(TIMAnalyser *t, simple_effect *g) : process_argument(t, g){};
    ...
    {
        process_argument_in_derivation_effect(TIMAnalyser *t, derivation_rule *g) : process_argument(t, g){};
    ...
    {
        process_constant_in_goal(TIMAnalyser *t, simple_goal *g) : process_argument(t, g){};
    ...
    {
        process_constant_in_initial(TIMAnalyser *t, simple_effect *g) : process_argument(t, g, g->prop){};
    ...
    {
        if (EPS(predicate)->getParent() != eps->getParent())
        {
            return false;
        }
        // cout << "A: " << *pt << "\n";
    ...
    bool Property::equivalent(const Property *p) const
    {
        ...
        if (posn != p->posn || EPS(predicate)->getParent() != EPS(p->predicate)->getParent())
        {
            return false;
        }
        return true;
    ...
    void TIMAnalyser::close(set<Property *> &seed, const pddl_type *pt)
    {
        ...
        while (adding)
        {
            ...
            for (TRules::const_iterator r = trules.begin(); r != trules.end(); ++r)
            {
                ...
                {
                    copy((*r)->getRHS()->begin(), (*r)->getRHS()->end(), inserter(seed, seed.begin()));
    ...
    {
        if (!EPS(predicate)->getParent())
        {
            return const_cast<Property *>(this);
        }
        TIMpredSymbol *tps = static_cast<TIMpredSymbol *>(EPS(predicate)->getParent()->find(
            predicate, makeTT(EPS(predicate)->tBegin(), posn, pt), makeTT(EPS(predicate)->tEnd(), 0, 0)));
    ...
    void TIMAnalyser::visit_simple_effect(simple_effect *p)
    {
        if (initially)
        {
            for_each(p->prop->args->begin(), p->prop->args->end(), process_constant_in_initial(this, p));
        }
        else
        {
            for_each(p->prop->args->begin(), p->prop->args->end(), process_argument_in_effect(this, p));
        };
    };
    ...
    void TIMAnalyser::insertPre(int v, Property *p)
    {
        ...
        if (!rules[v])
        {
            if (op)
            {
                rules[v] = new ProtoRule(this, op, v, isDurative ? (atStart ? START : END) : INSTANT);
            }
            if (drv)
            {
                rules[v] = new ProtoRule(this, drv, v, isDurative ? (atStart ? START : END) : INSTANT);
            }
        }
        rules[v]->insertPre(p);
    };
    ...
    void ProtoRule::addRules(TRules &trules)
    {
        ...
        vector<Property *> es;
        set_difference(enablers.begin(), enablers.end(), dels.begin(), dels.end(), inserter(es, es.begin()));
        vector<Property *> is;
        set_intersection(dels.begin(), dels.end(), adds.begin(), adds.end(), inserter(is, is.begin()));
        // Problem here: if we have a constant we still need to know what it is...
        parameter_symbol *v =
            (var >= 0 ? (op ? getAt(op->parameters, var) : getAt(drv->get_head()->args, var)) : 0);
        ...
        {
            assert(op);
            // don't worry, cannot ever be true for derivation rules: have no delete effects, so is empty,
            // dels empty....
            for (vector<Property *>::iterator i = is.begin(); i != is.end(); ++i)
            {
                ...
                for (vector<const pddl_type *>::const_iterator pt = types.begin(); pt != types.end(); ++pt)
                {
                ...
                enablers.insert(find_if(enablers.begin(), enablers.end(), bind2nd(greater<Property *>(), p)),
                                p);
            };
            ...
            if (adds.size() > is.size() || dels.size() > is.size())
            {
                ...
                enablers.clear();
                merge(es.begin(), es.end(), is.begin(), is.end(), inserter(enablers, enablers.begin()));
                es.swap(enablers);
            }
            else
            {
    ...
    void TIMAnalyser::setUpSpaces()
    {
        ...
        transform(pts.begin(), pts.end(), inserter(propspaces, propspaces.begin()), spaceSet);
    ...
    */
    ...
    void TransitionRule::assembleMutex(TransitionRule *tr)
    {
        if (op)
        {
            OUTPUT cout << "Mutex caused by rules: " << *this << " (" << this->byWhat()->name->getName()
                        << ") and " << *tr << " (" << tr->byWhat()->name->getName() << ")\n";
    ...
    void mutex::write(ostream &o) const
    {
        for (set<mutRec>::const_iterator i = argPairs.begin(); i != argPairs.end(); ++i)
        {
            ...
            {
                o << "Mutex for '" << op1->name->getName() << "' and '" << op2->name->getName()
    ...
    void MutexStore::additionalMutexes()
    {
        ...
        // "\n";
        for_each(conditions.begin(), conditions.end(), addMx(dynamic_cast<operator_ *>(this)));
        // Work through all the invariant mRecs of this action
        for_each(invariants.begin(), invariants.end(), addMx(dynamic_cast<operator_ *>(this)));
    };
    ...
    bool PropertySpace::examine(vector<PropertySpace *> &as)
    {
        ...
        if (std::accumulate(rules.begin(), rules.end(), true, checkRule))
        {
            OUTPUT cout << "\nPotential pseudo space...\n"
                        << "This will cause problems in several uses of TIM - tell Derek to get on with "
                           "fixing it!\n"
                        << *this << "\n";
        };
        // #endif
    ...
    struct getConditionally
    {
        ...
        getConditionally(bool c, Property *p, TI pt, TI term) : cond(c), prop(p), pit(pt), terminus(term)
        {
            while (pit != terminus && (c ? (*pit == prop) : (*pit != prop)))
            {
                ++pit;
            }
        };
        ...
        {
            ++pit;
            while (pit != terminus && (cond ? (*pit == prop) : (*pit != prop)))
            {
                ++pit;
            }
            return *this;
    ...
    {
        PropertyState *p1 = retrieve(tan, getIt(false, p, begin(), end()), getIt(false, p, end(), end()));
        PropertyState *p2 = retrieve(tan, getIt(true, p, begin(), end()), getIt(true, p, end(), end()));
    ...
    TransitionRule::TransitionRule(TransitionRule *tr, PropertyState *e, PropertyState *l, PropertyState *r)
      : tan(tr->tan), op(tr->op), opt(tr->opt), var(tr->var), enablers(e), lhs(l), rhs(r), objects(){};
    ...
    void splitStates(PStates &states, Property *p, PStates &xstates)
    {
        ...
        for (PStates::iterator i = states.begin(); i != e; ++i)
        {
            ...
            if (!pp.first->empty())
            {
                xstates.insert(pp.first);
            }
            if (!pp.second->empty())
            {
                newStates.insert(pp.second);
            }
        };
    ...
    set<PropertySpace *> TIMAnalyser::relevant(pddl_type *tp)
    {
        ...
        for (vector<PropertySpace *>::iterator i = propspaces.begin(); i != propspaces.end(); ++i)
        {
