...
void usage()
{
    cout << "VAL: The PDDL+ plan validation tool\n"
         ...
         << "    -r <n> <p> <m> -- Analyse the plan for its robustness, each action timestamp to within a "
            "(float) value of n, each PNE to within a (float value) of p, for m test plans.\n"
         << "    -ra <p>    -- Calculate robustness of plan with respect to varying action timestamps, "
            "whilst varying PNEs to within a (float value) of p (default p = 0).\n"
         << "    -rp <n>    -- Calculate robustness of plan with respect to varying PNEs, whilst varying "
            "action timestamps to within a (float value) of n (default n = 0).\n"
         << "    -rm <x>    -- Set metric for robustness testing: x = m, maximum; x = a, accumulative; x = "
            "d, delay. (default x = m).\n"
         << "    -rd <x>    -- Set distribution for robustness testing: x = u, uniform; x = n, normal; x = "
            "p, psuedo-normal. (default x = u).\n"
         << "    -j         -- When varying the values of PNEs also vary for event preconditions. (default = "
            "false)\n"
         ...
         << "    -p <n> <m> -- Number of pages for LaTeX Gantt chart (n = across time axis, m = across "
            "rows).\n"
...
vector<plan_step *> getTimedInitialLiteralActions()
{
    ...
    if (an_analysis.the_problem->initial_state->timed_effects.size() != 0)
    {
        ...
        for (pc_list<timed_effect *>::const_iterator e =
             ...
             e != an_analysis.the_problem->initial_state->timed_effects.end(); ++e)
        {
            operator_symbol *timed_initial_lit =
                an_analysis.op_tab.symbol_put("Timed Initial Literal Action " + toString(count++));
...
void executePlans(int &argc, char *argv[], int &argcount, TypeChecker &tc, const DerivationRules *derivRules,
                  double tolerance, bool lengthDefault, bool giveAdvice)
{
    Ranking rnk;
    Ranking rnkInv;
    ...
    while (argcount < argc)
    {
        ...
        // add actions that are not to be moved to the timed intitial literals otherwise to the plan to be
        // repaired i.e. pretend these actions are timed initial literals
    ...
    if (!rnk.empty())
    {
        ...
        if (an_analysis.the_problem->metric && an_analysis.the_problem->metric->opt == E_MINIMIZE)
        {
            if (LaTeX)
            {
...
void analysePlansForRobustness(int &argc, char *argv[], int &argcount, TypeChecker &tc,
                               const DerivationRules *derivRules, double tolerance, bool lengthDefault,
                               bool giveAdvice, double robustMeasure, int noTestPlans, bool car, bool cpr,
                               RobustMetric robm, RobustDist robd)
{
    ...
    while (argcount < argc)
    {
        ...
        RobustPlanAnalyser rpa(robustMeasure, noTestPlans, derivRules, tolerance, tc,
                               an_analysis.the_domain->ops, an_analysis.the_problem->initial_state, the_plan,
                               an_analysis.the_problem->metric, lengthDefault,
                               an_analysis.the_domain->isDurative(), an_analysis.the_problem->the_goal,
...
int main(int argc, char *argv[])
{
    try
    {
        ...
        const DerivationRules *derivRules =
            new DerivationRules(an_analysis.the_domain->drvs, an_analysis.the_domain->ops);
        ...
        if (Robust)
        {
            analysePlansForRobustness(argc, argv, argcount, tc, derivRules, tolerance, lengthDefault,
