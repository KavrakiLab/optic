...
int main(int argc, char *argv[])
{
    ...
    /*
       ...
       *(dynamic_cast<holding_pred_symbol*>(current_analysis->pred_tab.symbol_probe("near"))->pBegin());
        ...
        cout << *(instantiatedOp::getInstOp(string("drive"),Iterator<const_symbol *
       const,strP,&getConst>(args), Iterator<const_symbol * const,strP,&getConst>(args+3))) << "\n";
    */
    ...
    for (pred_decl_list::const_iterator ci = predicates->begin(); ci != predicates->end(); ci++)
    {
        ...
        for (holding_pred_symbol::PIt i = hps->pBegin(); i != hps->pEnd(); ++i)
        {
            ...
            tps->write(std::cout);
            std::cout << "\nIs definitely static " << tps->isDefinitelyStatic() << tps->isStatic()
                      << std::endl;
            std::cout << ", ";
        }
    }
