...
namespace VAL
{
    ...
    public:
        BaseGen(UINT32 seed = 0)
        {
            if (!seed)
            {
                seed = static_cast<UINT32>(time(0));
            }
            jc_ = seed;
            z_ = (jc_ = CM_ * jc_ + CA_);
            w_ = (jc_ = CM_ * jc_ + CA_);
    ...
    public:
        ...
        IntGen(const IntGen &uni)
          : gen_(uni.gen_), low_(uni.low_), delta_(uni.delta_), shift_(uni.shift_), mask_(uni.mask_)
        {
            bit_ = 0;
            word_ = gen_();
        }
    ...
    public:
        explicit NormalGen(UINT32 seed = 0, double m = 0, double v = 1) : gen_(seed), mean_(m), stdv_(v)
        {
            if (stdv_ < 0)
            {
                stdv_ = -stdv_;
        ...
        NormalGen(const NormalGen &nrm) : gen_(nrm.gen_), mean_(nrm.mean_), stdv_(nrm.stdv_)
        {
            ;
        }
        ...
        double operator()() const
        {
            ...
            return ((v > 0) ? (mean_ + stdv_ * (2.506628 + x)) : (mean_ + stdv_ * (-2.506628 - x)));
        }
