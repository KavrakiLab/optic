...
namespace VAL
{
    ...
    void TypeStripWriteController::write_problem(ostream &o, const problem *p)
    {
        ...
        if (dom->constants)
        {
            for (const_symbol_list::const_iterator i1 = dom->constants->begin(); i1 != dom->constants->end();
                 ++i1)
            {
                // o << "(" << (*i)->type->getName() << " " << (*i)->getName() << ") ";
                for (pddl_type_list::const_iterator t = dom->types->begin(); t != dom->types->end(); ++t)
    ...
    void TypeStripWriteController::write_constraint_goal(ostream &o, const constraint_goal *cg)
    {
        ...
        switch (cg->getCons())
        {
            ...
            case E_SOMETIME:
                ...
                break;
            case E_WITHIN:
                o << "within " << (cg->getDeadline()) << " " << *(cg->getRequirement());
                break;
            case E_ATMOSTONCE:
                o << "at-most-once " << *(cg->getRequirement());
                break;
            case E_SOMETIMEAFTER:
                o << "sometime-after " << *(cg->getTrigger()) << " " << *(cg->getRequirement());
                break;
            case E_SOMETIMEBEFORE:
                o << "sometime-before " << *(cg->getTrigger()) << " " << *(cg->getRequirement());
                break;
            case E_ALWAYSWITHIN:
                o << "always-within " << (cg->getDeadline()) << " " << *(cg->getTrigger()) << " "
                  << *(cg->getRequirement());
                break;
            case E_HOLDDURING:
                o << "hold-during " << cg->getFrom() << " " << cg->getDeadline() << " "
                  << *(cg->getRequirement());
                break;
            case E_HOLDAFTER:
                o << "hold-after " << cg->getFrom() << " " << *(cg->getRequirement());
                break;
            default:
                break;
        };
