...
namespace VAL
{
    ...
    void LaTeXSupport::LaTeXBuildGraph(ActiveCtsEffects *ace, const State *s)
    {
             ...
             fe != ace->activeFEs.end(); ++fe)
        {
            ...
            // setup initial value if nec, must be already to update cts so can assume defined in prob file
            // here
            ...
            else
            {
                ...
                if (const NumericalSolution *ns = dynamic_cast<const NumericalSolution *>(fe->second->ctsFtn))
                {
                    ...
                    for (map<double, CoScalar>::const_iterator j = thePoints.begin(); j != thePoints.end();
                         ++j)
                    {
                        if (j->first > ace->localUpdateTime)
                        {
                            break;
                        }
    ...
    void LaTeXSupport::LaTeXHeader()
    {
        *report << "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n"
                ...
                << "\\newcommand{\\headingtimehappening}{{\\bf Time} \\qquad \\= {\\bf "
                   "Happening}\\\\[0.8ex]}\n"
                ...
                << "\\newcommand{\\assignmentcts}[3]{\\listrow{Updating \\fexprn{#1} (#2) by #3 for "
                   "continuous update.}}\n"
                ...
                << "\\newcommand{\\scaledown}[3]{\\listrow{Scaling down \\fexprn{#1} (#2) by a factor of "
                   "#3.}}\n"
    ...
    void LaTeXSupport::LaTeXGantt(Validator *v)
    {
        ...
        for (vector<string>::iterator go = ganttObjectsAndTypes.begin(); go != ganttObjectsAndTypes.end();
             ++go)
        {
            ...
            if (type != NULL)
            {
                     ...
                     o != current_analysis->the_problem->objects->end(); ++o)
                {
                    if ((*o)->type == type)
                    {
                        ganttObjects.push_back((*o)->getName());
                    }
                };
            }
            else
