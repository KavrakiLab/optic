...
namespace VAL
{
    ...
    public:
        Update(const FuncExp *f, assign_op ao, FEScalar v) : fe(f), aop(ao), value(v){};
    ...
    typedef pair<pair<const expression *, bool>, const Environment *> ExprnPair;  // bool is true if
                                                                                  // increasing
    ...
    // the following class contains all the active cts effects that are to be updated before each regular
    // happening at the same point in time
    struct ActiveCtsEffects
    {
        Happening ctsEffects;  // contains all the active cts effects, length of time is given from the last
                               // happening
        map<const FuncExp *, ActiveFE *> activeFEs;  // contains all the active FEs and their dependencies on
                                                     // other FEs, also how to update the FEs
    ...
    public:
        ...
        private:
            ...
            double currenttime;  // currenttime will always be the time of the state prevailing.
                                 //(Last distinct time! May be different type of happening happenings with
                                 // same time)
        ...
        public:
            ...
            {
                for (map<const FuncExp *, ActiveFE *>::iterator j = ace.activeFEs.begin();
            ...
            void handleCtsHappening()
            {
                ...
                ace.setLocalUpdateTime(currenttime - (*i)->getTime());  // set value of LocalUpdateTime - time
                ...
                {
                    ace.setLocalUpdateTime(currenttime - (*i)->getTime());  // set value of LocalUpdateTime -
