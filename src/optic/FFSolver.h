...
namespace Planner
{
    ...
    public:
        FakeFFEvent(StartEvent *const e, instantiatedOp *a, const int &pw, const double &dMin,
                    const double &dMax)
    ...
    private:
        FFEvent * toUpdate;
    public:
            ImplicitFFEvent(FFEvent * const e, instantiatedOp * a, const int & pw, const double & dMin, const
    double & dMax) : FFEvent(a,pw,dMin,dMax), toUpdate(e)
        {
    ...
    */
    ...
    public:
        ...
        ExtendedMinimalState(const set<int> &f, const vector<double> &sMin, const vector<double> &sMax,
                             ...
                             const map<int, set<int>> &sa, const double &ts, const int &nt,
                             const unsigned int &pl, const vector<AutomatonPosition::Position> &ps,
                             const double &ppv, const double *tdrStatus, const double &sc)
          : decorated(new MinimalState(f, sMin, sMax, /*psa, */ sa, ps, ppv, tdrStatus, sc, nt, pl))
          , timeStamp(ts)
          , stepBeforeTIL(-1)
          , tilFanIn(0)
          , hasBeenDominated(false)
        {
        }

        ExtendedMinimalState()
          : decorated(new MinimalState())
          , timeStamp(0.0)
          , stepBeforeTIL(-1)
          , tilFanIn(0)
          , hasBeenDominated(false)
        ...
        //      return (nextTIL == o.nextTIL && first == o.first && secondMin == o.secondMin && secondMax ==
        //      o.secondMax && startedActions == o.startedActions && invariants == o.invariants &&
        //      fluentInvariants == o.fluentInvariants && stepBeforeTIL == o.stepBeforeTIL && tilFanIn ==
        //      o.tilFanIn && tilComesBefore == o.tilComesBefore && queueEqual(startEventQueue,
        //      o.startEventQueue) && fabs(timeStamp - o.timeStamp) < 0.0005);
        //  }
                                          ...
                                          double minDur = 0.0, double maxDur = 0.0) const
        {
            return new ExtendedMinimalState(*this,
                                            decorated->applyAction(a, minTimestamps, newDummySteps,
        ...
        {
            decorated->applyActionLocally(a, minTimestamps, newDummySteps, constraintsSatisfied, minDur,
                                          maxDur);
        }
    ...
    struct Solution
    {
        ...
        Solution();
        void update(const list<FFEvent> &newPlan, const TemporalConstraints *const newCons,
                    const double &newMetric);
    };
    ...
    public:
        ...
        public:
            ...
            HTrio(const double &hvalue, const double &msIn, const double &mseIn, const int &planLength,
                  const char *
#ifndef NDEBUG
                      diagnosisIn
#endif
              ...
              , makespan(msIn)
              , makespanEstimate(mseIn)
              , admissibleCostEstimate(0.0)
#ifndef NDEBUG
    ...
    private:
        ...
        static HTrio calculateHeuristicAndCompressionSafeSchedule(
            ExtendedMinimalState &theState, ExtendedMinimalState *prevState, set<int> &goals,
            set<int> &goalFluents, list<ActionSegment> &helpfulActions, list<FFEvent> &header,
            list<FFEvent> &now, const int &stepID, map<double, list<pair<int, int>>> *justApplied = 0,
            double tilFrom = 0.001);

        static HTrio calculateHeuristicAndSchedule(
            ExtendedMinimalState &theState, ExtendedMinimalState *prevState, set<int> &goals,
            set<int> &goalFluents, ParentData *const p, list<ActionSegment> &helpfulActions,
            pair<bool, double> &currentCost, list<FFEvent> &header, list<FFEvent> &now, const int &stepID,
            bool considerCache = false, map<double, list<pair<int, int>>> *justApplied = 0,
            double tilFrom = 0.001);
         ...
         */
        static ExtendedMinimalState *applyActionToState(ActionSegment &theAction,
         ...
         */
        static bool stateHasProgressedBeyondItsParent(const ActionSegment &theAction,
                                                      const ExtendedMinimalState &parent,
                                                      const ExtendedMinimalState &child);

        static void evaluateStateAndUpdatePlan(auto_ptr<SearchQueueItem> &succ, ExtendedMinimalState &state,
        ...
        //  static void justEvaluateNotReuse(auto_ptr<SearchQueueItem> & succ, RPGHeuristic* rpg,
        //  ExtendedMinimalState & state, ExtendedMinimalState * prevState, set<int> & goals, set<int> &
        //  goalFluents, list<ActionSegment> & helpfulActionsExport, list<FFEvent> & extraEvents,
        //  list<FFEvent> & header, HTrio & bestNodeLimitHeuristic, list<FFEvent> *& bestNodeLimitPlan, bool &
        //  bestNodeLimitGoal, bool & stagnant, map<double, list<pair<int,int> > > * justApplied, double
        //  tilFrom=0.001);

        //  static bool checkTSTemporalSoundness(RPGHeuristic* const rpg, ExtendedMinimalState & theState,
        //  const int & theAction, const VAL::time_spec & ts, const double & incr, int oldTIL=-1);

        static void pairDummyStepsWithRealSteps(MinimalState &theState, int &currStepID,
                                                list<FFEvent> &header, list<FFEvent> &nowList,
                                                const list<pair<int, FFEvent>> &newDummySteps);

        static bool precedingActions(ExtendedMinimalState &theState, const ActionSegment &actionSeg,
                                     list<ActionSegment> &alsoMustDo, int oldTIL = -1, double moveOn = 0.001);

        static bool checkTemporalSoundness(ExtendedMinimalState *prevState, ExtendedMinimalState &theState,
                                           const ActionSegment &actionSeg, int oldTIL = -1,
                                           double moveOn = 0.001);

        static void makeJustApplied(map<double, list<pair<int, int>>> &justApplied, double &tilFrom,
                                    ExtendedMinimalState &state, const bool &lastIsSpecial);

        static double evaluateMetric(const MinimalState &theState, const list<FFEvent> &plan,
                                     const bool printMetric = true);

        /** @brief Find whether to carry on searching after a given goal state.
         *
         * @return First value: <code>true</code> if to carry on searching.  Second value: <code>true</code>
         * if the state has room for improvement (in the case of preferences).
         */
        static pair<bool, bool> carryOnSearching(const MinimalState &theState, const list<FFEvent> &plan,
    ...
    public:
        ...
        // static list<instantiatedOp*> * solveSubproblem(LiteralSet & startingState, vector<pair<PNE*,
        // double> > & startingFluents, SubProblem* const s);
