...
namespace Planner
{
    ...
    void MinimalState::numericGoalHoldsFromSteps(const int &gID, const list<int> &stepIDs)
    {
        if (!stepsFromWhichNumericGoalsHold)
        {
...
#ifdef MEMDEBUG
            cout << "Creating new array to hold numeric goal achievement times for " << this << " at "
                 << stepsFromWhichNumericGoalsHold << endl;
#endif
            ...
            for (int i = 0; i < numGoalArrayCount; ++i)
            {
                ...
                {
                    cout << "- Fatal internal error: memset has failed to fill entry " << i << " with 0"
                         << endl;
                }
            }
#endif
    ...
    MinimalState::MinimalState(const StateFacts &f, const vector<double> &sMin, const vector<double> &sMax,
                               const map<int, set<int>> &sa, const vector<AutomatonPosition::Position> &ps,
                               const double &ppv, const double *tdrStatus, const int nt,
                               const unsigned int pl, const unsigned int ae
#ifdef STOCHASTICDURATIONS
                               ,
      ...
      , startedActions(sa)
      , preferenceStatus(ps)
      , prefPreconditionViolations(ppv)
      , lowerBoundOnTimeDependentRewardFacts(
            tdrStatus ? new double[NumericAnalysis::getFactsInTimeDependentRewards().size()] : 0)
      , planLength(pl)
      , actionsExecuting(ae)
      , nextTIL(nt)
...
statusOfTemporalPreferences(psa ? psa->clone() : 0)*/
    {
        if (tdrStatus)
        {
            memcpy(lowerBoundOnTimeDependentRewardFacts, tdrStatus,
                   sizeof(double) * NumericAnalysis::getFactsInTimeDependentRewards().size());
        }
#ifdef STOCHASTICDURATIONS
    ...
    MinimalState::MinimalState(const set<int> &f, const vector<double> &sMin,
...
statusOfTemporalPreferences(psa ? psa->clone() : 0)*/
    ...
    MinimalState::MinimalState(const MinimalState &other, const int extendBy)
      ...
      , lowerBoundOnTimeDependentRewardFacts(
            other.lowerBoundOnTimeDependentRewardFacts ?
                new double[NumericAnalysis::getFactsInTimeDependentRewards().size()] :
                0)
      ,  // cost(other.cost),
      ...
      , temporalConstraints(
            globalTransformer->cloneTemporalConstraints(other.temporalConstraints, extendBy)) /*,
statusOfTemporalPreferences(other.statusOfTemporalPreferences ? other.statusOfTemporalPreferences->clone() :
0)*/
    {
        if (other.lowerBoundOnTimeDependentRewardFacts)
        {
            memcpy(lowerBoundOnTimeDependentRewardFacts, other.lowerBoundOnTimeDependentRewardFacts,
                   sizeof(double) * NumericAnalysis::getFactsInTimeDependentRewards().size());
        }
#ifdef STOCHASTICDURATIONS
    ...
    MinimalState::MinimalState()
      : prefPreconditionViolations(0.0)
    ...
    void MinimalState::printState(ostream &cout) const
    {
        ...
        //         cout << "\nPreference violations:" << statusOfTemporalPreferences->getWithinViolations() <<
        //         endl;
        //     }
    ...
    //         if (data[j] != AutomatonPosition::satisfied && data[j] !=
