...
namespace Planner
{
    ...
    void Globals::markThatActionsInPlanHaveToBeKept()
    {
        ...
        for (int idebug = 0, i = 0; planItr != planEnd; ++planItr, ++i, ++idebug)
        {
            ...
            instantiatedOp *const currOp = instantiatedOp::findInstOp(
                currStep->op_sym, currStep->params->begin(), currStep->params->end());
            if (!currOp)
            {
                cout << "Exiting: step " << idebug
                     << " in the input plan uses an action that the instantiation code did not create.\n";
            ...
            cout << "Marking that " << *currOp << " (" << ID
                 << ") must not be eliminated by the preprocessor\n";
        }
    }
    ...
    void Globals::eliminatedAction(const int &i, const char *synopsis)
    {
        ...
        {
            cout << "Error in preprocessor.  Pruned operator " << i
                 << ", but should not have done.  Reason given was:\n";
    ...
    void Globals::checkActionBounds(const int &actID, const double &startBegin, const double &startEnd)
    {
        ...
        if (actionMustBeAbleToStartAtTime[actID] < startBegin ||
            actionMustBeAbleToStartAtTime[actID] > startEnd)
        {
            cout << "Error in preprocessor.  Action " << actID << " must be able to start at "
                 << actionMustBeAbleToStartAtTime[actID] << ", which is not in the preprocessed range ["
                 << startBegin << "," << startEnd << "]\n";
            exit(1);
        }
    }
                                    ...
                                    const double &endBegin, const double &endEnd)
    {
        ...
        {
            cout << "Error in preprocessor.  Action " << actID << " must be able to end at "
                 << actionMustBeAbleToEndAtTime[actID] << ", which is not in the preprocessed range ["
                 << endBegin << "," << endEnd << "]\n";
            exit(1);
        }
    }
    ...
    void Globals::checkActionDurationBounds(const int &actID, const double &durMin, const double &durMax)
    {
        ...
        {
            cout << "Error in preprocessor.  Action " << actID << " must be able to have the duration "
                 << inputDur << ", but has been limited to [" << durMin << "," << durMax << "]\n";
            exit(1);
        }
    }
