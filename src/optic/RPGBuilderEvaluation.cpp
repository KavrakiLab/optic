...
namespace Planner
{
     ...
     */
    EpsilonResolutionTimestamp
    earliestPointForNumericPrecondition(const RPGBuilder::RPGNumericPrecondition &p,
    ...
    EpsilonResolutionTimestamp
    earliestPointForNumericPrecondition(const RPGBuilder::RPGNumericPrecondition &p,
                                        const MinimalState &theState, const vector<double> &stepTimes)
    {
        static const int varCount = RPGBuilder::getPNECount();
        static int stepID;
    ...
    /** @brief A struct representing the intermediate goals to achieve at a layer during RPG solution
     * extraction.
     *
     ...
     *  - Action ends that must be added (the ends of actions that have been started in the plan, but not yet
     * finished)
     *
    ...
    struct RPGRegress
    {
         ...
         */
        void requestNumericThreshold(const int &variable, const double &threshold,
                                     const EpsilonResolutionTimestamp &weight)
        {
            const pair<map<int, pair<double, EpsilonResolutionTimestamp>>::iterator, bool> insPair =
                numericValueGreaterThan.insert(make_pair(variable, make_pair(threshold, weight)));
    ...
    public:
        RPGRegressionMap() : forbidLaterThan(EpsilonResolutionTimestamp::infinite())
        {
        }
        ...
        RPGRegress &operator[](const EpsilonResolutionTimestamp &t)
        {
            ...
            {
                cerr << "Fatal internal error: in solution extraction, satisfying a numeric precondition at "
                        "layer "
                     << t << ", after previously satisfied one at layer " << forbidLaterThan
                     << " - need to work strictly backwards\n";
    ...
    struct SupportingAction
    {
        ...
        SupportingAction(const int &actionID, const Planner::time_spec &timeSpecifier,
          ...
          : actID(actionID), ts(timeSpecifier), howManyTimes(applyThisManyTimes), tilR(tilRelevance)
        {
        }
    };
    ...
    struct ActionAndHowManyTimes
    {
        ...
        ActionAndHowManyTimes(const int &act, const Planner::time_spec &timeSpec, const int &lim,
                              const double &dmin, const double &dmax, const bool alreadyIn = false)
          : actID(act)
          , ts(timeSpec)
          , howManyTimes(lim)
          , minDur(dmin)
          , maxDur(dmax)
          , gradientsFinishAt(EpsilonResolutionTimestamp::infinite())
          , gradientNeedsStarting(true)
          , alreadyInThePlan(alreadyIn)
        {
        ...
        {
            if (!alreadyInThePlan && other.alreadyInThePlan)
            {
                return true;
            }
            if (alreadyInThePlan && !other.alreadyInThePlan)
    ...
    struct DelayedGradientDescriptor : public ActionAndHowManyTimes
    {
        ...
        DelayedGradientDescriptor(const int &act, const Planner::time_spec &timeSpec, const double &dmax,
                                  const pair<int, double> &effect)
    ...
    public:
        ...
        typedef map<EpsilonResolutionTimestamp, FactLayerEntry>::const_reverse_iterator
    ...
    public:
        ...
        struct RecordedAssignment
        {
            ...
            RecordedAssignment() : act(0), eff(-1), maximiseEffect(true)
            {
            }
        ...
        protected:
             ...
             *
             * - If this variable is set to <code>true</code>, then
             * <code>assignmentEffectThatGaveThisValue</code> and
             * <code>effectsThatIncreasedThisVariable</code> are not initialised.
             ...
             *  - Entries <code>[pneCount..(pneCount-1)]</code> store the negation of the minimum value of
             * each PNE
             *  - Entries from <code>2 * pneCount</code> onwards store the maximum values of artificial
             * variables.
             *
             ...
             */
            map<int, map<double, list<const ActionAndHowManyTimes *>>>
                integratedEffectsDirectlyBeforeThisLayer;
             ...
             */
            void supplyDetails(const FluentLayerEntry &f, const EpsilonResolutionTimestamp &timeDifference,
                               const bool &doApplyGradients, const bool &ignorableLayer)
            {
             ...
             *
             *  - If <code>val > 0</code>, then the gradient on the upper bound on <code>var</code> is
             * increased.
             *  - If <code>val < 0</code>, then the gradient on the negative lower bound on <code>var</code>
             * is increased.
             *
             *  In both cases, the index of the affected variable is added to the set <code>varChanged</code>,
             * and the value of the affected variable is increased by epsilon times the gradient.
             */
            void startGradient(const int &var, const double &val, set<int> &varChanged)
            {
                ...
                else if (val < 0)
                {
                    ...
                    if (fabs(alter) < 0.0000001)
                    {
                        if (!previouslyZero)
                        {
                            gradientBecomesZeroOn(var + pneCount);
                        }
                    }
                    else
                    {
                        if (previouslyZero)
                        {
                            gradientBecomesNonZeroOn(var + pneCount);
                        }
                    }
                }
            }
        ...
        public:
            ...
            FluentLayerEntry(const vector<double> &v) : internalValues(v), gradients(v.size(), 0.0)
            {
            ...
            FluentLayerEntry(const FluentLayerEntry &f, const EpsilonResolutionTimestamp &timeDifference,
                             const bool &doApplyGradients, const bool &ignorableLayer)
            {
            ...
            void keepAssignmentIfBetter(const ActionAndHowManyTimes *act, const int &effID,
                                        const bool &maxEffect, const int &var, const double &val,
                                        set<int> &varChanged)
            {
            ...
            void applyIncrease(const int &effID, const int &var, const double &minVal, const double &maxVal,
                               const int &howManyTimes, set<int> &varChanged)
            {
             ...
             *
             *  @param var  The variable for which the gradient changes (either a positive or negative
             * variable)
            ...
            void markEffectsToRevisit(const set<int> &toRevisit, const vector<double> &maxNeeded)
            {
                ...
                {
                    if (instaneousEffectsToRevisitAfterThisLayer.find(*effItr) !=
                        instaneousEffectsToRevisitAfterThisLayer.end())
                    {
                        continue;
                    }
            ...
            void markIntegratedOrGradientEffectVariableToRevisit(const int &toRevisit,
                                                                 const vector<double> &maxNeeded)
            {
                if (integratedOrGradientEffectsToRevisitAfterThisLayer.find(toRevisit) !=
                    integratedOrGradientEffectsToRevisitAfterThisLayer.end())
                {
                    return;
                }
            ...
            map<int, map<double, list<const ActionAndHowManyTimes *>>> &
            getIntegratedEffectsDirectlyBeforeThisLayer()
            {
                return integratedEffectsDirectlyBeforeThisLayer;
            }

            map<int, map<double, list<const ActionAndHowManyTimes *>>> &
            getGradientEffectsDirectlyBeforeThisLayer()
            {
            ...
            double willBecomeSatisfiedAfterDelay(const RPGBuilder::RPGNumericPrecondition &currPre) const
            {
                ...
                if (currPre.op == VAL::E_GREATEQ)
                {
                    ...
                    if (debug)
                    {
                        cout << "Current LHS value is " << currV << ", RHS is " << currPre.RHSConstant
                             << ", so with a gradient of " << currG << " that will mean a delay of "
                             << ((currPre.RHSConstant - currV) / currG) << endl;
                    }
                ...
                else
                {
                    ...
                    if (debug)
                    {
                        cout << "Current LHS value is " << currV << ", RHS is " << currPre.RHSConstant
                             << ", so with a gradient of " << currG << " that will mean a delay of "
                             << ((currPre.RHSConstant - currV) / currG) + EPSILON << endl;
                    }
    ...
    protected:
        ...
        /** @brief The type of the map used to hold the variable and gradient values at each point in time
         * across the RPG. */
        typedef map<EpsilonResolutionTimestamp, FluentLayerEntry *> LayerMap;

        /** @brief The values of the numeric variables, and the active gradients, at each point in time across
         * the RPG. */
        LayerMap layers;
        ...
        inline FluentLayerEntry *newFluentLayerEntry(const FluentLayerEntry *const previousFL,
                                                     const EpsilonResolutionTimestamp &timeDifference,
                                                     const bool &applyGradients, const bool &ignorableLayer)
        {
            static FluentLayerEntry *newLayer;
         ...
         */
        LayerMap::iterator addLayer(LayerMap::const_iterator &startAt,
                                    const EpsilonResolutionTimestamp &timeDifference)
        {
            EpsilonResolutionTimestamp newTime = startAt->first;
            newTime += timeDifference;

            const pair<LayerMap::iterator, bool> insPair =
                layers.insert(make_pair(newTime, newFluentLayer()));
        ...
        {
            /*const list<int> & numericEffs = (toUse->ts == Planner::E_AT_START ?
            RPGBuilder::getStartEffNumerics()[toUse->actID] : RPGBuilder::getEndEffNumerics()[toUse->actID]);
            ...
            for (; effItr != effEnd; ++effItr) {
                ...
                for (int pass = 0; pass < 2; ++pass) {
                    ...
                    for (int rhsC = 0; rhsC < 2; ++rhsC) {
                        ...
                        const pair<map<int,double>::iterator,bool> insPair =
            alreadyAchieved.insert(make_pair(v,rhs));
        ...
        /** @brief For each variable ID, the actions in the planning graph that have ever had an integrated
         * effect of the magnitude given.
         */
        vector<map<double, list<ActionAndHowManyTimes *>>> actionsThatHaveHadThisIntegratedEffect;

        /** @brief For each variable ID, the actions in the planning graph that have ever had a gradient
         * effect of the magnitude given.
         */
        vector<map<double, list<ActionAndHowManyTimes *>>> actionsThatHaveHadThisGradientEffect;

        /** @brief The set of facts true at the current layer, due to gradients having been started in the
         * past.
         *
        ...
        /** @brief For convenience - for each effect ID, the actions with that effect in the most recent
         * action layer. */
         ...
         *  are used to record details about the layer at which it is currently set to become true due to
         * active gradients.  Each entry is a pair:
         ...
         */
        typedef pair<map<EpsilonResolutionTimestamp, set<int>>::iterator, set<int>::iterator>
            PreconditionToSatisfactionLayerPair;
        ...
        /** @brief The earliest point at which an effect can act upon/a precondition can refer to a given
         * variable.
         *
        ...
        void aboutToRecordAnEffect(const EpsilonResolutionTimestamp &effectAppearsInLayer)
        {
            if (effectsAffectThisLayer == layers.end())
            {
                ...
                if (effectInputs == layers.end())
                {
                    ...
                    cout << "Internal error: current effect layer is " << effectAppearsInLayer
                         << ", but no fluent values are defined for " << priorLayer << endl;
                    exit(1);
                }
#endif

                effectsAffectThisLayer = addLayer(effectInputs, EpsilonResolutionTimestamp::epsilon());
                mostRecentInstantaneousEffects =
                    &(effectsAffectThisLayer->second->instantaneousEffectsDirectlyBeforeThisLayer);
                mostRecentIntegratedEffects =
                    &(effectsAffectThisLayer->second->integratedEffectsDirectlyBeforeThisLayer);
                mostRecentGradientEffects =
                    &(effectsAffectThisLayer->second->gradientEffectsDirectlyBeforeThisLayer);
            ...
            else
            {
                ...
                {
                    cout << std::setprecision(8) << std::fixed;
                    cout << "Internal error: current effect layer is " << effectsAffectThisLayer->first
                         << ", but asked for an effect to go in " << effectAppearsInLayer << endl;
                    exit(1);
                }
#endif
            }
    ...
    public:
        /** @brief Placeholder constructor.  The real initialisation is done in
         * <code>setFactLayerZero()</code>.
         */
        FluentLayers(const vector<pair<double, double>> *const payloadIn)
          : payloadDurationBounds(payloadIn)
          , debug(Globals::globalVerbosity & 64 || Globals::globalVerbosity & 128)
        {
         ...
         */
        void setFactLayerZero(const vector<double> &values,
                              const vector<EpsilonResolutionTimestamp> &cannotReferToVariableUntilTime)
        {
            assert(layers.empty());
         ...
         *
         *  This is used to update for the CTS effects of currently executing actions, and find which facts
         * are true in the state being evaluated.  Otherwise, it probably shouldn't be used.
         *
         ...
         * @param isDueToGradients    If set to <code>true</code>, then the gradients will cause facts to
         * become true EPSILON after the time returned.
         * @return  A pair, where:
         *          -  the first value is the timestamp of a layer to visit (or <code>DBL_MAX</code> if none
         * is needed)
         *           - the secound value is <code>true</code> if this is epsilon after the previous layer
         * (this is used to prevent rounding errors)
         */
        pair<EpsilonResolutionTimestamp, bool> nextLayerWouldBeAt(const EpsilonResolutionTimestamp &currTS,
                                                                  bool &isToRevisitEffects,
                                                                  bool &isDueToGradients)
        {
            const map<EpsilonResolutionTimestamp, set<int>>::const_iterator ptItr =
                preconditionsBecomingTrueAtTime.begin();
            ...
            if (layerWithEffectsToBeRevisited != layers.end())
            {
                ...
                if (debug)
                {
                    cout << COLOUR_light_blue
                         << "Need to recalculate fluents at epsilon after the previous fluent layer\n"
                         << COLOUR_default;
                }
            ...
            if (debug)
            {
                if (gapIsEpsilon)
                {
                    cout << COLOUR_light_blue << "Numeric preconditions become true at " << ptItr->first
                         << ", so returning (" << toReturn << ",true)\n"
                         << COLOUR_default;
                }
                else
                {
                    cout << COLOUR_light_blue << "Numeric preconditions become true at " << ptItr->first
                         << ", so returning (" << toReturn << ",false)\n"
                         << COLOUR_default;
                }
            }
        ...
        void createThenIgnore(const EpsilonResolutionTimestamp &ts)
        {
            assert(layerWithEffectsToBeRevisited == layers.end() ||
                   (layerWithEffectsToBeRevisited->first == ts));
            ...
            if (debug)
            {
                cout << "Asked to create layer at " << ts << ", previous is at " << lastEntry->first
                     << ", so advance beyond previous fluent layer is " << timeDifference << endl;
            }
            if (!timeDifference.isZero())
            {
                if (debug)
                ...
                layers.insert(lastEntry, make_pair(ts, newFluentLayerEntry(lastEntry->second, timeDifference,
                                                                           true, true)));
            }
            else
            {
        ...
        void recordEffectsThatAreNowToBeRevisited(FactLayerMap &factLayers)
        {
            ...
            if (debug)
            {
                cout << "> Revisiting effects in layer " << layerWithEffectsToBeRevisited->first
                     << ", so creating output layer epsilon later\n";
            }
            ...
            mostRecentInstantaneousEffects =
                &(effectsAffectThisLayer->second->getInstantaneousEffectsDirectlyBeforeThisLayer());
            mostRecentIntegratedEffects =
                &(effectsAffectThisLayer->second->getIntegratedEffectsDirectlyBeforeThisLayer());
            mostRecentGradientEffects =
                &(effectsAffectThisLayer->second->getGradientEffectsDirectlyBeforeThisLayer());

            const set<int> &revisit =
                layerWithEffectsToBeRevisited->second->getInstantaneousEffectsToRevisitAfterThisLayer();

            {
                ...
                for (; rvItr != rvEnd; ++rvItr)
                {
                    ...
                    dest.insert(dest.end(), actionsThatHaveHadThisEffect[*rvItr].begin(),
                                actionsThatHaveHadThisEffect[*rvItr].end());
                }
            }

            const set<int> &revisit2 = layerWithEffectsToBeRevisited->second
                                           ->getIntegratedOrGradientEffectsToRevisitAfterThisLayer();
                ...
                for (int pass = 0; pass < 2; ++pass)
                {
                    ...
                    for (; rv2Itr != rv2End; ++rv2Itr)
                    {
                        map<double, list<const ActionAndHowManyTimes *>> &outerdest =
                            (pass ? (*mostRecentGradientEffects)[*rv2Itr] :
                                    (*mostRecentIntegratedEffects)[*rv2Itr]);

                        map<double, list<ActionAndHowManyTimes *>> &src =
                            (pass ? actionsThatHaveHadThisGradientEffect[*rv2Itr] :
                                    actionsThatHaveHadThisIntegratedEffect[*rv2Itr]);
                                ...
                                else
                                {
                                        ...
                                        EpsilonResolutionTimestamp::infinite())
                                    {
                                        if ((*actItr)->gradientNeedsStarting)
                                        {
                                            ...
                                            *actItr =
                                                &newEntry;  // from now on, use this version which doesn't
                                                            // trigger an increase of the perpetual gradient
                                        }
                                    ...
                                    else
                                    {
                                        ...
                                        EpsilonResolutionTimestamp extraTime(
                                            (*actItr)->howManyTimes * ((*actItr)->maxDur), false);

                                        EpsilonResolutionTimestamp endAt = effectsAffectThisLayer->first;
                                        endAt -=
                                            EpsilonResolutionTimestamp::epsilon();  // the time of the action
                                                                                    // layer in which the
                                                                                    // effect began
                                        endAt += extraTime;  // plus the maximum sequential time of
                                                             // applications of this action

                                        if (factLayers.empty() ||
                                            factLayers.begin()->first > (*actItr)->gradientsFinishAt)
                                        {
                                            ...
                                            {
                                                factLayers[endAt]
                                                    .gradientFinishes.insert(make_pair(*rv2Itr, 0.0))
                                                    .first->second += effItr->first;
                                            }
                                            else
                                            {
                                                factLayers[endAt]
                                                    .gradientFinishes
                                                    .insert(
                                                        make_pair(*rv2Itr + FluentLayerEntry::pneCount, 0.0))
                                                    .first->second -= effItr->first;
                                            }
                                        ...
                                        else
                                        {
                                            ...
                                            if (effItr->first > 0)
                                            {
                                                factLayers[(*actItr)->gradientsFinishAt]
                                                    .gradientFinishes.insert(make_pair(*rv2Itr, 0.0))
                                                    .first->second -= effItr->first;
                                                factLayers[endAt]
                                                    .gradientFinishes.insert(make_pair(*rv2Itr, 0.0))
                                        ...
                                        *actItr = &newEntry;  // from now on, use the version with the most
                                                              // recent endAt record
                                        dest.push_back(*actItr);
                                    }
                                }
        ...
        void recordConsequencesOfActiveGradients(const EpsilonResolutionTimestamp &newFactsAppearsInLayer)
        {
            ...
            const map<EpsilonResolutionTimestamp, set<int>>::iterator nextFacts =
                preconditionsBecomingTrueAtTime.begin();
        ...
        /** @brief Record that a given integrated continuous numeric effect can be applied a number of times,
         * on behalf of the specified action.
         *
         ...
         * @param effID                 The ID of the effect (an index into
         * <code>RPGBuilder::getNumericEff()</code>)
         */
        void recordIntegratedNumericEffect(const ActionAndHowManyTimes &action,
                                           const EpsilonResolutionTimestamp &effectAppearsInLayer,
                                           const pair<int, double> &effID)
        {
            if (debug)
            {
            ...
            if (debug)
            {
                cout << COLOUR_light_green
                     << "Added integrated effect: " << *(RPGBuilder::getPNE(effID.first));
                if (effID.second > 0)
                {
        ...
        /** @brief Record that a given gradient continuous numeric effect can be applied a number of times, on
         * behalf of the specified action.
         *
         * If the action can only be applied finitely often, the fact that the effect has to finish is
         * recorded in <code>factLayers</code>.
         *
         * @param action                Details on the action, including how many times it can be applied
         * @param effectAppearsInLayer  The layer in which the consequences of the effect appear
         * @param effID                 The ID of the effect (an index into
         ...
         */
        void recordGradientNumericEffect(const ActionAndHowManyTimes &action,
                                         const EpsilonResolutionTimestamp &effectAppearsInLayer,
                                         const pair<int, double> &effID, FactLayerMap &factLayers)
        {
            ...
            if (debug)
            {
                cout << COLOUR_light_green << "Added gradient effect: d"
                     << *(RPGBuilder::getPNE(effID.first));
                if (effID.second > 0)
                {
            ...
            if (action.howManyTimes == INT_MAX)
            {
                return;
            }
            if (action.maxDur == DBL_MAX)
            {
                return;
            }

            EpsilonResolutionTimestamp endAt = effectAppearsInLayer;
            endAt -= EpsilonResolutionTimestamp::epsilon();  // the time of the action layer in which the
                                                             // effect began
            endAt += EpsilonResolutionTimestamp(action.howManyTimes * action.maxDur,
                                                false);  // plus the maximum sequential time of applications
                                                         // of this action, rounded up

            gc.back().gradientsFinishAt = endAt;

            if (effID.second > 0)
            {
                factLayers[endAt].gradientFinishes.insert(make_pair(effID.first, 0.0)).first->second +=
                    effID.second;
            }
            else
            {
                factLayers[endAt]
                    .gradientFinishes.insert(make_pair(effID.first + FluentLayerEntry::pneCount, 0.0))
                    .first->second -= effID.second;
            }
        }

        /** @brief Record that a given numeric effect can be applied a number of times, on behalf of the
         * specified action.
         *
         * @param action                Details on the action, including how many times it can be applied
         ...
         * <code>RPGBuilder::getNumericEff()</code>)
         */
        void recordInstantaneousNumericEffect(const ActionAndHowManyTimes &action,
                                              const EpsilonResolutionTimestamp &effectAppearsInLayer,
                                              const int &effID)
        {
            if (debug)
            {
                cout << "Recording effect " << effID << " affecting fact layer " << effectAppearsInLayer
                     << endl;
            }
            ...
            if (debug)
            {
                cout << COLOUR_light_blue << "Added effect: " << RPGBuilder::getNumericEff()[effID]
                     << COLOUR_default << endl;
            }
            gc.push_back(action);
        ...
        /** @brief Apply any effects recorded in the most recent action layer, supplied via calls to
         * <code>recordInstantaneousNumericEffect()</code>.
         *
         ...
         *  @param newNumericPreconditions  If new numeric preconditions have become true, their indices are
         * pushed onto this list
         *  @param maxNeeded                The maximum amount of each variable needed to satisfy the
         * preconditions
         */
        void applyRecentlyRecordedEffects(vector<EpsilonResolutionTimestamp> &achievedInLayer,
                                          list<int> &newNumericPreconditions, const vector<double> &maxNeeded)
        {
            if (effectsAffectThisLayer == layers.end())
            {
                return;
            ...
            if (debug)
            {
                cout << "Applying the numeric effects noted in this layer; effects affect "
                     << effectsAffectThisLayer->first << endl;
            }
            ...
            {
                map<int, map<double, list<const ActionAndHowManyTimes *>>>::const_iterator effItr =
                    mostRecentGradientEffects->begin();
                const map<int, map<double, list<const ActionAndHowManyTimes *>>>::const_iterator effEnd =
                    mostRecentGradientEffects->end();
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    ...
                    map<double, list<const ActionAndHowManyTimes *>>::const_iterator magItr =
                        effItr->second.begin();
                    const map<double, list<const ActionAndHowManyTimes *>>::const_iterator magEnd =
                        effItr->second.end();
                    ...
                    for (; magItr != magEnd; ++magItr)
                    {
                        ...
                        const list<const ActionAndHowManyTimes *>::const_iterator actEnd =
                            magItr->second.end();
                        ...
                        for (; actItr != actEnd; ++actItr)
                        {
                            if ((*actItr)->gradientNeedsStarting)
                            {
                                ...
                                effectsAffectThisLayer->second->startGradient(effItr->first, magItr->first,
                                                                              variableChanged);
                            }
                        }
                    }
                    if (debug)
                    {
                        ...
                        const double &l = effectsAffectThisLayer->second
                                              ->getGradients()[effItr->first + FluentLayerEntry::pneCount];
                        if (l == 0)
                        {
                            cout << "0.0";
                        }
                        ...
                        cout << "," << effectsAffectThisLayer->second->getGradients()[effItr->first] << "]"
                             << endl;
                    }
                }
                ...
                {
                    avsToRecalculate.insert(RPGBuilder::getVariableDependencies(*vcItr).begin(),
                                            RPGBuilder::getVariableDependencies(*vcItr).end());
                }
                ...
                if (debug)
                {
                    ...
                    {
                        cout << "Gradients of AV " << RPGBuilder::getArtificialVariable(*avItr) << " is now "
                             << effectsAffectThisLayer->second->getGradients()[*avItr] << endl;
                    }
                }
...

            {
                ...
                map<int, map<double, list<const ActionAndHowManyTimes *>>>::const_iterator effItr =
                    mostRecentIntegratedEffects->begin();
                const map<int, map<double, list<const ActionAndHowManyTimes *>>>::const_iterator effEnd =
                    mostRecentIntegratedEffects->end();
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    ...
                    for (; magItr != magEnd; ++magItr)
                    {
                        ...
                        for (; actItr != actEnd; ++actItr)
                        {
                            ...
                            assert((*actItr)->howManyTimes);
                            effectsAffectThisLayer->second->applyIncrease(
                                -1, effItr->first, magItr->first, magItr->first, (*actItr)->howManyTimes,
                                variableChanged);
                        }
                        if (debug)
                        {
                    ...
                    if (debug)
                    {
                        ...
                        cout << "Bounds are now [";
                        if (effectsAffectThisLayer->second
                                ->values()[affectedVar + FluentLayerEntry::pneCount] == DBL_MAX)
                        {
                            cout << "-inf,";
                        }
                        else
                        {
                            cout << -effectsAffectThisLayer->second
                                         ->values()[affectedVar + FluentLayerEntry::pneCount]
                                 << ",";
                        }
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                ...
                map<int, list<const ActionAndHowManyTimes *>>::const_iterator effItr =
                    mostRecentInstantaneousEffects->begin();
                const map<int, list<const ActionAndHowManyTimes *>>::const_iterator effEnd =
                    mostRecentInstantaneousEffects->end();

                for (; effItr != effEnd; ++effItr)
                {
                    RPGBuilder::RPGNumericEffect &currEff = RPGBuilder::getNumericEff()[effItr->first];
                    if (currEff.isAssignment != (pass == 1))
                    {
                        continue;
                    }
                    ...
                    for (; actItr != actEnd; ++actItr)
                    {
                        const double maxVal = currEff.evaluate(effectInputs->second->values(),
                                                               (*actItr)->minDur, (*actItr)->maxDur);
                        const double minVal = currEff.evaluateMin(effectInputs->second->values(),
                                                                  (*actItr)->minDur, (*actItr)->maxDur);
                        if (currEff.isAssignment)
                        {
                            if (debug)
                            {
                                ...
                                else if (effectsAffectThisLayer->second
                                             ->values()[affectedVar + FluentLayerEntry::pneCount] == -DBL_MAX)
                                {
                                    cout << "inf,";
                                }
                            ...
                            effectsAffectThisLayer->second->keepAssignmentIfBetter(
                                *actItr, effItr->first, true, affectedVar, maxVal, variableAssignedTo);
                            effectsAffectThisLayer->second->keepAssignmentIfBetter(
                                *actItr, effItr->first, false, affectedVar, minVal, variableAssignedTo);
                            if (debug)
                            {
                                cout << "Bounds are now [";
                                if (effectsAffectThisLayer->second
                        ...
                        else
                        {
                            if (debug)
                            {
                                cout << currEff << " gives us " << minVal << "," << maxVal << " of "
                                     << *(RPGBuilder::getPNE(currEff.fluentIndex)) << endl;
                            }
                            effectsAffectThisLayer->second->applyIncrease(effItr->first, affectedVar, minVal,
                                                                          maxVal, (*actItr)->howManyTimes,
                                                                          variableChanged);
                            // effectsAffectThisLayer->second->applyIncrease(effItr->first, affectedVar,
                            // minVal, (*actItr)->howManyTimes, variableChanged);
            ...
            // pull in preconditions that gradients have satisfied, but double-check this is definitely the
            // case
...

            {
                ...
                {
                    if (achievedInLayer[*preItr] != EpsilonResolutionTimestamp::undefined())
                    {
                        continue;
                    }

                    PreconditionToSatisfactionLayerPair &forecastPair =
                        layerAtWhichPreconditionBecomesTrue[*preItr];

                    const RPGBuilder::RPGNumericPrecondition &currPre =
                        RPGBuilder::getNumericPreTable()[*preItr];

                    satisfactionDelay =
                        effectsAffectThisLayer->second->willBecomeSatisfiedAfterDelay(currPre);
                    ...
                    if (satisfactionDelay == 0.0)
                    {
                        ...
                        if (debug)
                        {
                            cout << COLOUR_yellow << "Precondition " << *preItr << ", " << currPre
                                 << ", becomes true in layer " << effectsAffectThisLayer->first
                                 << COLOUR_default << endl;
                        }
                    ...
                    else if (satisfactionDelay < DBL_MAX)
                    {
                        ...
                        const EpsilonResolutionTimestamp earliestPOPoint =
                            earliestPointForNumericPrecondition(currPre, earliestNumericPOTimes);
                        ...
                        if (forecastPair.first == preconditionsBecomingTrueAtTime.end())
                        {
                            ...
                            forecastPair.first =
                                preconditionsBecomingTrueAtTime.insert(make_pair(futureLayer, set<int>()))
                                    .first;
                            forecastPair.second = forecastPair.first->second.insert(*preItr).first;

                            if (debug)
                            {
                                cout << COLOUR_blue << "Precondition " << *preItr << ", " << currPre
                                     << ", becomes true in layer " << futureLayer << COLOUR_default << endl;
                            }
                        }
                        else
                        {
                            map<EpsilonResolutionTimestamp, set<int>>::iterator newForecastItr =
                                preconditionsBecomingTrueAtTime.find(futureLayer);
                            if (newForecastItr != forecastPair.first)
                            {
                                ...
                                if (debug)
                                {
                                    cout << COLOUR_blue << "Revised: precondition " << *preItr << ", "
                                         << currPre << ", becomes true in layer " << futureLayer
                                         << COLOUR_default << endl;
                                }
                            }
                        }
                    ...
                    {
                        // was forecast to become true, but isn't any longer - a gradient in its favour has
                        // expired
...

            {
                ...
                for (; vcItr != vcEnd; ++vcItr)
                {
                    effectsAffectThisLayer->second->markEffectsToRevisit(
                        revisitInstantaneousEffectIfVariableValueChanges[*vcItr], maxNeeded);
                }
            }

            {
                ...
                {
                    effectsAffectThisLayer->second->markEffectsToRevisit(
                        revisitInstantaneousEffectIfVariableIsAssignedTo[*atItr], maxNeeded);

                    if (*atItr >= RPGBuilder::getPNECount())
                    {
                        if (!actionsThatHaveHadThisIntegratedEffect[*atItr - RPGBuilder::getPNECount()]
                                 .empty())
                        {
                            effectsAffectThisLayer->second->markIntegratedOrGradientEffectVariableToRevisit(
                                *atItr - RPGBuilder::getPNECount(), maxNeeded);
                        }
                    }
                    else
                    {
            ...
            if (!effectsAffectThisLayer->second->getInstantaneousEffectsToRevisitAfterThisLayer().empty() ||
                !effectsAffectThisLayer->second->getIntegratedOrGradientEffectsToRevisitAfterThisLayer()
                     .empty())
            {
                layerWithEffectsToBeRevisited = effectsAffectThisLayer;
                if (debug)
                {
                    cout << "Noting that " << layerWithEffectsToBeRevisited->first
                         << " has effects that should be revisited\n";
                }
            }
            else
            {
                if (debug)
                {
                    cout << "Noting that " << effectsAffectThisLayer->first
                         << " has no effects that should be revisited\n";
                }
            }
         ...
         *
         * @param pre             The numeric precondition (an index into
         * <code>RPGBuilder::getNumericPreTable()</code>
        ...
        void requestNumericPrecondition(const int &pre, const EpsilonResolutionTimestamp &achievedAt,
                                        const EpsilonResolutionTimestamp &forActionLayer,
                                        RPGRegressionMap &goalsAtLayer,
                                        const EpsilonResolutionTimestamp &tilR)
        {
            if (debug)
            {
                if (((forActionLayer - EpsilonResolutionTimestamp::epsilon()) - currentSubgoalLayer->first) >
                    EpsilonResolutionTimestamp::zero())
                {
                    cout << "It appears there is a cycle in the RPG solution extraction\n";
                    cout << "Previously, we were asking for the preconditions of actions at "
                         << currentSubgoalLayer->first << endl;
                    cout << "However, the action for which preconditions are being requested now is at "
                         << forActionLayer << endl;
                }
            }
            // check we are definitely going backwards through the RPG
            assert((-currentSubgoalLayer->first + (forActionLayer - EpsilonResolutionTimestamp::epsilon())) <=
                   EpsilonResolutionTimestamp::zero());
            ...
            if (debug)
            {
                ...
                {
                    cout << "Asking for a precondition, " << pre << ", that became true at time "
                         << achievedAt << ", but there is no fluent layer recorded for that time\n";
                }
            }
            ...
            const double threshold =
                (currPre.op == VAL::E_GREATER ? currPre.RHSConstant + 0.00001 : currPre.RHSConstant);
            ...
            if (debug)
            {
                cout << "\t\t\t - Translated to a threshold " << currPre.LHSVariable << " >= " << threshold
                     << endl;
            }
        }
         ...
         *
         * @param pre             The numeric goal (an index into
         * <code>RPGBuilder::getNumericPreTable()</code>
         * @param achievedAt      The fact layer at which this precondition was satisfied
         * @param goalsAtLayer    The record of goals kept during solution extraction, to be updated
         *                        by this function.
                         ...
                         RPGRegressionMap &goalsAtLayer)
        {
            ...
            const double threshold =
                (currPre.op == VAL::E_GREATER ? currPre.RHSConstant + 0.001 : currPre.RHSConstant);

            RPGRegress &addTo = goalsAtLayer[achievedAt];

            if (debug)
            {
                cout << " - Requesting " << currPre.LHSVariable << " >= " << threshold << " at time "
                     << achievedAt << endl;
            }
            addTo.requestNumericThreshold(currPre.LHSVariable, threshold,
                                          EpsilonResolutionTimestamp::infinite());
        }
         ...
         *
         */
        void recordSideEffects(const int &act, const Planner::time_spec &ts,
                               const EpsilonResolutionTimestamp &actLayer)
        {
            ...
            ActionAndHowManyTimes tmp(act, ts, 1, (*payloadDurationBounds)[act].first,
                                      (*payloadDurationBounds)[act].second);
         ...
         */
        void satisfyNumericPreconditionsAtLayer(
            RPGRegressionMap::const_iterator &currentLayer, RPGRegressionMap &goalsAtLayer,
            list<pair<EpsilonResolutionTimestamp, SupportingAction>> &actionsUsed)
        {
            if (currentLayer->second.numericValueGreaterThan.empty())
            {
                return;
            }
            if (currentLayer->first <= EpsilonResolutionTimestamp::zero())
            {
                return;
            }
            ...
            if (debug)
            {
                cout << COLOUR_light_blue << "Satisfying numeric preconditions in layer "
                     << currentLayer->first << COLOUR_default << endl;
            }
            ...
            map<int, pair<double, EpsilonResolutionTimestamp>>::const_iterator thresholdItr =
                currentLayer->second.numericValueGreaterThan.begin();
            const map<int, pair<double, EpsilonResolutionTimestamp>>::const_iterator thresholdEnd =
                currentLayer->second.numericValueGreaterThan.end();
            ...
            for (; thresholdItr != thresholdEnd; ++thresholdItr)
            {
                ...
                /*{
                    ...
                    if (aaItr != alreadyAchieved.end()) {
                        ...
                        if (debug) {
                            cout << " * becomes variable " << thresholdItr->first << " >= " << needToAchieve
                << endl;
                        }
                    }
                }*/
                ...
                const double valueInPreviousLayer =
                    previousFluentLayer->second->values()[thresholdItr->first];
                ...
                if (needToAchieve <= valueInPreviousLayer)
                {
                    if (debug)
                    {
                        cout << "- Could have " << needToAchieve << " of " << thresholdItr->first
                             << " in the previous layer, " << previousFluentLayer->first << endl;
                    }
                    ...
                    if (debug)
                    {
                        cout << "  * Earliest possible point with sufficient is layer " << rollback->first
                             << endl;
                    }

                    if (rollback->first > EpsilonResolutionTimestamp::zero())
                    {
                        goalsAtLayer[rollback->first].requestNumericThreshold(
                            thresholdItr->first, needToAchieve, thresholdItr->second.second);
                    }
                    continue;
                }
                ...
                {
                    const RecordedAssignment *const assignmentUsed =
                        preconditionsIn->second->assignmentAppliedToVariable(thresholdItr->first);

                    // if it was done with assignment...
                    if (assignmentUsed && assignmentUsed->act)
                    {
                        const RPGBuilder::RPGNumericEffect &currEff =
                            RPGBuilder::getNumericEff()[assignmentUsed->eff];

                        const ActionAndHowManyTimes *const toUse = assignmentUsed->act;

                        actionsUsed.push_back(
                            make_pair(currentLayer->first, SupportingAction(toUse->actID, toUse->ts, 1,
                                                                            thresholdItr->second.second)));

                        if (debug)
                        {
                            cout << "  * Using assignment by "
                                 << *(RPGBuilder::getInstantiatedOp(toUse->actID)) << endl;
                        }
                        ...
                        if (currEff.size)
                        {
                            ...
                            breakApart(currEff.variables, currEff.weights, currEff.size, currEff.constant,
                                       assignmentUsed->maximiseEffect,
                                       thresholdItr->second.first - contributionToIncreaseFromDur,
                                       previousFluentLayer, (*payloadDurationBounds)[toUse->actID].first,
                                       (*payloadDurationBounds)[toUse->actID].second, subTerms);

                            if (!subTerms.empty())
                            {  // duration dependent variables don't induce subterms
                                ...
                                for (; tItr != tEnd; ++tItr)
                                {
                                    prev.requestNumericThreshold(tItr->first, tItr->second,
                                                                 thresholdItr->second.second);
                                }
                            }
                        }
                ...
                if (!assignmentMatched)
                {
                    ...
                    {
                        const RPGBuilder::ArtificialVariable &currAV =
                            RPGBuilder::getArtificialVariable(thresholdItr->first);
                        double avRemaining = thresholdItr->second.first;

                        /*for (int s = 0; s < currAV.size; ++s) {
                            const map<int,double>::const_iterator aaItr =
                        alreadyAchieved.find(currAV.fluents[s]); if (aaItr != alreadyAchieved.end()) {
                                avRemaining -= aaItr->second * currAV.weights[s];
                            }
                        }*/

                        breakApart(currAV.fluents, currAV.weights, currAV.size, currAV.constant, true,
                                   avRemaining, preconditionsIn, EPSILON, DBL_MAX, subTerms);
                    }
                    ...
                    for (; tItr != tEnd; ++tItr)
                    {
                        ...
                        if (debug)
                        {
                            ...
                            {
                                cout << "increasing " << *(RPGBuilder::getPNE(tItr->first)) << ": must have "
                                     << residual << " by now, given " << acceptableMinimum << " previously\n";
                            }
                            else
                            {
                                cout << "decreasing "
                                     << *(RPGBuilder::getPNE(tItr->first - FluentLayerEntry::pneCount))
                                     << ": must have " << -residual << " by now, given " << -acceptableMinimum
                                     << " previously\n";
                            }
                        }
                        ...
                        {
                            const RecordedAssignment *const assignmentUsed =
                                preconditionsIn->second->assignmentAppliedToVariable(tItr->first);
                            ...
                            if (assignmentUsed && assignmentUsed->act)
                            {
                                ...
                                actionsUsed.push_back(make_pair(
                                    currentLayer->first, SupportingAction(toUse->actID, toUse->ts, 1,
                                                                          thresholdItr->second.second)));
                                ...
                                if (assignmentUsed->maximiseEffect)
                                {
                                    for (int s = 0; s < currEff.size; ++s)
                                    {
                                        if (currEff.variables[s] == -3)
                                        {
                                            contributionToIncreaseFromDur +=
                                                (currEff.weights[s] * toUse->minDur);
                                        }
                                        else if (currEff.variables[s] == -19)
                                        {
                                            contributionToIncreaseFromDur +=
                                                (currEff.weights[s] * toUse->maxDur);
                                        }
                                    }
                                }
                                else
                                ...
                                if (currEff.size)
                                {
                                    ...
                                    // they hold values sufficient for the desired result
                                    list<pair<int, double>> subTerms;
                                    breakApart(currEff.variables, currEff.weights, currEff.size,
                                               currEff.constant, assignmentUsed->maximiseEffect,
                                               thresholdItr->second.first - contributionToIncreaseFromDur,
                                               previousFluentLayer,
                                               (*payloadDurationBounds)[toUse->actID].first,
                                               (*payloadDurationBounds)[toUse->actID].second, subTerms);
                        ...
                        {
                            const set<int> *const instantaneousEffs =
                                preconditionsIn->second->getInstantaneousEffectsThatIncreasedVariable(
                                    tItr->first);
                            ...
                            if (instantaneousEffs)
                            {
                                ...
                                for (; effItr != effEnd && residual > acceptableMinimum; ++effItr)
                                {
                                    ...
                                    const list<const ActionAndHowManyTimes *> &actions =
                                        preconditionsIn->second
                                            ->getInstantaneousEffectsDirectlyBeforeThisLayer()[*effItr];
                                    ...
                                    list<const ActionAndHowManyTimes *>::const_iterator aItr =
                                        actions.begin();
                                    const list<const ActionAndHowManyTimes *>::const_iterator aEnd =
                                        actions.end();
                                    ...
                                    for (; aItr != aEnd && residual > acceptableMinimum; ++aItr)
                                    {
                                        if (debug)
                                        {
                                            cout << "          > Residual: " << residual
                                                 << ", Acceptable Minimum: " << acceptableMinimum << endl;
                                        }

                                        // for (int pass = 0; pass < 2 && residual > acceptableMinimum;
                                        // ++pass) {

                                        {
                                            const double minEff =
                                                currEff.evaluateMin(previousFluentLayer->second->values(),
                                                                    (*aItr)->minDur, (*aItr)->maxDur);
                                            const double maxEff =
                                                currEff.evaluate(previousFluentLayer->second->values(),
                                                                 (*aItr)->minDur, (*aItr)->maxDur);
                                                    ...
                                                    (minEff > 0 || maxEff > 0))
                                            {
                                                ...
                                                if (minRatherThanMax)
                                                {
                                                    for (int s = 0; s < currEff.size; ++s)
                                                    {
                                                        ...
                                                        {
                                                            contributionToIncreaseFromDur +=
                                                                (currEff.weights[s] * (*aItr)->minDur);
                                                        }
                                                        else if (currEff.variables[s] == -19)
                                                        {
                                                            contributionToIncreaseFromDur +=
                                                                (currEff.weights[s] * (*aItr)->maxDur);
                                                        }
                                                    }
                                                }
                                                else
                                                ...
                                                if (debug)
                                                {
                                                    cout << "          + Effect can be obtained "
                                                         << applyTimes << " time(s) using ";
                                                    ...
                                                    cout << *(RPGBuilder::getInstantiatedOp((*aItr)->actID))
                                                         << ", yielding an effect value of " << localEff
                                                         << endl;
                                                }
                                                if (tItr->first >= FluentLayerEntry::pneCount)
                                                {
                                                    ...
                                                    {
                                                        const double at =
                                                            (-(residual - acceptableMinimum)) / localEff;
                                                        applyTimes = at;
                                                        if (at > applyTimes)
                                                        {  // round up the number of times we need apply the
                                                           // action
                                                            ++applyTimes;
                                                        }
                                                        residual -= -(applyTimes * localEff);
                                                    }
                                                    else if (applyTimes * localEff <=
                                                             (-(residual - acceptableMinimum)))
                                                    {
                                                        const double at =
                                                            (-(residual - acceptableMinimum)) / localEff;
                                                        applyTimes = at;
                                                ...
                                                if (debug)
                                                {
                                                    cout << "Going to apply it " << applyTimes
                                                         << " times, leaving a residual of " << residual
                                                         << endl;
                                                }
                                                ...
                                                if (localEff == DBL_MAX)
                                                {
                                                    ...
                                                    {
                                                        *magnitudeUpdate = residual - acceptableMinimum -
                                                                           contributionToIncreaseFromDur;
                                                    }
                                                }
                                                else
                                                {
                                                    if (*used)
                                                    {
                                                        ...
                                                        {
                                                            if (*magnitudeUpdate <
                                                                localEff - contributionToIncreaseFromDur)
                                                            {
                                                                *magnitudeUpdate =
                                                                    localEff - contributionToIncreaseFromDur;
                                                            }
                                                ...
                                                actionsUsed.push_back(make_pair(
                                                    currentLayer->first,
                                                    SupportingAction((*aItr)->actID, (*aItr)->ts, applyTimes,
                                                                     thresholdItr->second.second)));

                                                recordSideEffects(*aItr, applyTimes,
                                                                  previousFluentLayer->second->values());
                                    ...
                                    if (maxUsed)
                                    {
                                        ...
                                        breakApart(currEff.variables, currEff.weights, currEff.size,
                                                   currEff.constant, true, maximumEffectMagnitudeNeeded,
                                                   previousFluentLayer, 0.001, DBL_MAX, inputSubTerms);

                                        if (!inputSubTerms.empty())
                                        {  // duration dependent variables don't induce subterms
                                            RPGRegress &prev = goalsAtLayer[previousFluentLayer->first];

                                            list<pair<int, double>>::const_iterator t2Itr =
                                                inputSubTerms.begin();
                                            const list<pair<int, double>>::const_iterator t2End =
                                                inputSubTerms.end();

                                            for (; t2Itr != t2End; ++t2Itr)
                                            {
                                                prev.requestNumericThreshold(t2Itr->first, t2Itr->second,
                                                                             thresholdItr->second.second);
                                            }
                                        }
                                    }
                                    ...
                                    if (minUsed)
                                    {
                                        list<pair<int, double>> inputSubTerms;
                                        breakApart(currEff.variables, currEff.weights, currEff.size,
                                                   currEff.constant, false, -minimumEffectMagnitudeNeeded,
                                                   previousFluentLayer, 0.001, DBL_MAX, inputSubTerms);
                            ...
                            if (residual > acceptableMinimum)
                            {
                                ...
                                map<int, map<double, list<const ActionAndHowManyTimes *>>> &ieffs =
                                    preconditionsIn->second->getIntegratedEffectsDirectlyBeforeThisLayer();

                                map<int, map<double, list<const ActionAndHowManyTimes *>>>::const_iterator
                                    ieItr;
                                ...
                                else
                                {
                                    ...
                                    map<double, list<const ActionAndHowManyTimes *>>::const_iterator effItr =
                                        ieItr->second.begin();
                                    const map<double, list<const ActionAndHowManyTimes *>>::const_iterator
                                        effEnd = ieItr->second.end();
                                    ...
                                    for (; effItr != effEnd && residual > acceptableMinimum; ++effItr)
                                    {
                                        ...
                                        for (; aItr != aEnd && residual > acceptableMinimum; ++aItr)
                                        {
                                            ...
                                            if (nInt > (*aItr)->howManyTimes)
                                            {
                                                ...
                                                actionsUsed.push_back(
                                                    make_pair(currentLayer->first,
                                                              SupportingAction((*aItr)->actID, (*aItr)->ts,
                                                                               (*aItr)->howManyTimes,
                                                                               thresholdItr->second.second)));
                                                recordSideEffects(*aItr, (*aItr)->howManyTimes,
                                                                  previousFluentLayer->second->values());
                                            }
                                            else
                                            {
                                                ...
                                                actionsUsed.push_back(make_pair(
                                                    currentLayer->first,
                                                    SupportingAction((*aItr)->actID, (*aItr)->ts, nInt,
                                                                     thresholdItr->second.second)));
                                                recordSideEffects(*aItr, nInt,
                                                                  previousFluentLayer->second->values());
                                            }
                                        }
                                    }
                            ...
                            if (residual > acceptableMinimum)
                            {
                                ...
                                {
                                    map<int, map<double, list<const ActionAndHowManyTimes *>>> &geffs =
                                        layerWithGradientOnThisVar->second
                                            ->getGradientEffectsDirectlyBeforeThisLayer();
                                    if (keepPositive)
                                    {
                                    ...
                                    if (geItr != geffs.end())
                                    {
                                        map<double, list<const ActionAndHowManyTimes *>>::const_iterator
                                            effItr = geItr->second.begin();
                                        const map<double, list<const ActionAndHowManyTimes *>>::const_iterator
                                            effEnd = geItr->second.end();
                                    ...
                                    if (rightWay)
                                    {
                                        if (debug)
                                        {
                                            cout << "    + Suitable gradient effects in layer "
                                                 << layerWithGradientOnThisVar->first << endl;
                                        }
                                        break;
                                    }
                                    else
                                    {
                                        if (debug)
                                        {
                                            cout << "    + No suitable gradient effects in layer "
                                                 << layerWithGradientOnThisVar->first << endl;
                                        }
                                ...
                                for (; effItr != effEnd && residual > acceptableMinimum; ++effItr)
                                {
                                    if (keepPositive)
                                    {
                                        if (effItr->first < 0)
                                        {
                                            if (debug)
                                            {
                                                cout << "    * Ignoring decrease with gradient "
                                                     << effItr->first << endl;
                                            }
                                            else
                                            {
                                                cout << "    * Keeping increase with gradient "
                                                     << effItr->first << endl;
                                            }
                                            continue;
                                        }
                                    ...
                                    else
                                    {
                                        if (effItr->first > 0)
                                        {
                                            if (debug)
                                            {
                                                cout << "    * Ignoring increase with gradient "
                                                     << effItr->first << endl;
                                            }
                                            else
                                            {
                                                cout << "    * Keeping decrease with gradient "
                                                     << effItr->first << endl;
                                            }
                                    ...
                                    for (; aItr != aEnd && residual > acceptableMinimum; ++aItr)
                                    {
                                        const EpsilonResolutionTimestamp durationOfEffect =
                                            ((*aItr)->gradientsFinishAt < preconditionsIn->first ?
                                                 (*aItr)->gradientsFinishAt :
                                                 preconditionsIn->first) -
                                            layerWithGradientOnThisVar->first;

                                        const double overThatTime = durationOfEffect.toDouble() * eff;

                                        const double needed =
                                            ceil(durationOfEffect.toDouble() / ((*aItr)->maxDur));

                                        if (debug)
                                        {
                                            cout << "       - Using " << needed << " of "
                                                 << *(RPGBuilder::getInstantiatedOp((*aItr)->actID)) << endl;
                                        }
                                        ...
                                        if (!(*aItr)->alreadyInThePlan)
                                        {
                                            actionsUsed.push_back(make_pair(
                                                layerWithGradientOnThisVar->first,
                                                SupportingAction((*aItr)->actID, (*aItr)->ts, needed,
                                                                 thresholdItr->second.second)));
                                            recordSideEffects(*aItr, needed,
                                                              layerBeforeGradientOnThisVar->second->values());
                                        }
                                    }
                                }
                            }
                            ...
                            if (residual > borrowFactLayerZeroValues()[tItr->first])
                            {
                                ...
                                ++rollback;
                                goalsAtLayer[rollback->first].requestNumericThreshold(
                                    tItr->first, residual, thresholdItr->second.second);
                            }
                        }
                    }
                }
        ...
        void breakApart(const vector<int> &variables, const vector<double> &weights, const int &size,
                        const double &constant, const bool &useMaxVariables, double threshold,
                        const LayerMap::const_iterator &reachableValues, const double &minDur,
                        const double &maxDur, list<pair<int, double>> &subTerms)
        {
            if (debug)
            {
                cout << "Breaking apart ";
            ...
            for (int s = 0; s < size; ++s)
            {
                ...
                if (w < 0.0)
                {
                    ...
                    if (v < 0)
                    {
                        ...
                        {
                            cerr << "Fatal error: referring to something other than a state variable or "
                                    "?duration in a precondition or effect\n";
                            exit(1);
                        }
                    }
                    else
            ...
            for (int s = 0; s < size && accumulatedLHS < threshold; ++s)
            {
                ...
                if (accumulatedLHS + (values[v] * w) >= threshold)
                {
                    if (debug)
                    {
                        cout << "That does the rest: asking for " << v
                             << " >= " << ((threshold - accumulatedLHS) / w) << endl;
                    }
    ...
    struct DotDetails
    {
        ...
        void addNumericFactNode(const double &t, const int &pre, const bool inState = false)
        {
            ...
            {
                const RPGBuilder::ArtificialVariable &currAV =
                    RPGBuilder::getArtificialVariable(currPre.LHSVariable);

                double localRHS = currPre.RHSConstant - currAV.constant;

                if (currAV.size == 1 && (currAV.fluents[0] >= pneCount) &&
                    (currAV.fluents[0] < (2 * pneCount)))
                {
        ...
        void addActionNode(const double &t, const int &act, const Planner::time_spec &ts)
        {
            ...
            {
                const list<Literal *> &pres =
                    (ts == Planner::E_AT_START ?
                         RPGBuilder::getProcessedStartPropositionalPreconditions()[act] :
                         RPGBuilder::getEndPropositionalPreconditions()[act]);
            ...
            {
                const list<int> &numPres =
                    (ts == Planner::E_AT_START ? RPGBuilder::getProcessedStartRPGNumericPreconditions()[act] :
                                                 RPGBuilder::getEndRPGNumericPreconditions()[act]);
        ...
        void printLayerDef(ostream &o, const bool &factLayer, const double &atTime, const int &idx,
                           const ClusterDef &def) const
        {
    ...
    ostream &operator<<(ostream &o, const DotDetails &d)
    {
        ...
        while (flItr != flEnd && alItr != alEnd)
        {
            if (alItr->first - flItr->first > 0.0005)
            {
                ...
                if (cluster)
                {
                    o << "\tc" << cluster - 1 << " -> c" << cluster << " [ltail=cluster_" << cluster - 1
                      << ", lhead=cluster_" << cluster << "];\n";
                }
                ++flItr;
            }
    ...
    {
    public:
        Private(const bool &b, vector<list<Literal *>> *atse, vector<list<Literal *>> *atee,
                vector<list<pair<int, Planner::time_spec>>> *eta, vector<list<Literal *>> *atsne,
                vector<list<Literal *>> *atene, vector<list<pair<int, Planner::time_spec>>> *neta,
                vector<list<pair<int, Planner::time_spec>>> *pta, vector<list<Literal *>> *atsp,
                vector<list<Literal *>> *ati, vector<list<Literal *>> *atep,
                vector<list<RPGBuilder::NumericEffect>> *atnuse,
                vector<list<RPGBuilder::NumericEffect>> *atnuee, vector<list<int>> *atrnuse,
                vector<list<int>> *atrnuee, vector<list<int>> *atnusp, vector<list<int>> *atnui,
                vector<list<int>> *atnuep, vector<list<int>> *atpnuep, vector<int> *iusp, vector<int> *iuip,
                vector<int> *iuep, vector<EpsilonResolutionTimestamp> *ail,
                vector<EpsilonResolutionTimestamp> *ailr, vector<pair<int, Planner::time_spec>> *ab,
                vector<pair<int, Planner::time_spec>> *abr, vector<EpsilonResolutionTimestamp> *negail,
                vector<EpsilonResolutionTimestamp> *negailr, vector<pair<int, Planner::time_spec>> *negab,
                vector<pair<int, Planner::time_spec>> *negabr, vector<EpsilonResolutionTimestamp> *nail,
                vector<EpsilonResolutionTimestamp> *nailr, vector<ActionFluentModification *> *nab,
                vector<ActionFluentModification *> *nabr, vector<int> *iunsp, vector<int> *iuni,
                vector<int> *iunep, vector<RPGBuilder::RPGNumericPrecondition> *rnp,
                vector<RPGBuilder::RPGNumericEffect> *rne, vector<list<pair<int, Planner::time_spec>>> *ppta,
                vector<list<pair<int, Planner::time_spec>>> *nppta, vector<list<Literal *>> *atpsp,
                vector<int> *iupsp, vector<int> *iupsnp, list<pair<int, Planner::time_spec>> *pla,
          ...
          , deleteArrays(b)
          , expandFully(false)
          , doneIntegration(false)
          , evaluateDebug(false)
        {
            earliestPropositionPOTimes =
                vector<EpsilonResolutionTimestamp>(ail->size(), EpsilonResolutionTimestamp::undefined());
            earliestNumericPOTimes = vector<EpsilonResolutionTimestamp>(
                RPGBuilder::getPNECount(), EpsilonResolutionTimestamp::undefined());
        ...
        struct EndPrecRescale
        {
            ...
            EndPrecRescale(const int &v, const double &off, const double &tc, const double &dur)
              : var(v), offset(off), totalchange(tc), duration(dur){};

            EndPrecRescale(){};

            EndPrecRescale(const EndPrecRescale &e, const double &remaining)
              : var(e.var)
              , offset(e.offset)
              , totalchange(e.totalchange * (remaining / duration))
              , duration(remaining){};
        ...
        public:
            ...
            /** @brief This contains the layer in which the goal/trigger (0/1) of the indexed preference
             * became true. */
            vector<vector<EpsilonResolutionTimestamp>> preferencePartTrueAtLayer;

            /** @brief Preferences violated by each start action.  First part of pair: preconditions.  Second
             * part of pair: itself. */
            vector<ActionViolationData> startActionCosts;

            /** @brief Preferences violated by each end action.  First part of pair: preconditions.  Second
             * part of pair: itself. */
            ...
            /** @brief Status of each preference. If it's unsatisfied in the RPG, it has to be unsatisfied in
             * the LP, too. */
            vector<AutomatonPosition::Position> optimisticPrefStatus;

#ifdef POPF3ANALYSIS
            /* The actions waiting for the costs of their preconditions to be reduced, before they're
             ...
             *
             * Each entry on the list pairs an action ID with <code>true</code> if it's not appeared in the
             * RPG before, or <code>false</code> otherwise.
             */
            ...
            void nextThingToVisit(const EpsilonResolutionTimestamp &currTS, NextFactLayer &next)
            {
                ...
                if (debug)
                {
                    cout << "Working out when the next thing to happen is, based on a current time of "
                         << currTS << endl;
                }

                const map<EpsilonResolutionTimestamp, list<pair<int, bool>>>::iterator eaItr =
                    endActionsAtTime.begin();
                if (eaItr != endActionsAtTime.end())
                {
                    ...
                    if (debug)
                    {
                        cout << "Next action layer could occur alongside actions applicable due to fact "
                                "layer "
                             << next.timestamp << endl;
                    }
                ...
                if (flItr != factLayers.end())
                {
                    ...
                    if (isEpsilonLater)
                    {
                        if (debug)
                        {
                            cout << "Next fact layer would be " << flItr->first
                                 << ", i.e. epsilon after now - keeping\n";
                        }
                    ...
                    {  // if the facts are greater than epsilon ahead, and so is the other option so far (if
                       // any)
                        if (next.timestamp == flItr->first)
                        {
                            ...
                            if (debug)
                            {
                                cout << "Next fact layer would be at " << flItr->first
                                     << ", i.e. the same time as the end actions\n";
                            }
                        }
                        else if (flItr->first < next.timestamp)
                        {
                            ...
                            {
                                cout << "Next fact layer would be at " << flItr->first
                                     << ", i.e. sooner than the end actions, so not visiting those for now\n";
                            }
                        ...
                        else
                        {
                            ...
                            {
                                cout << "Next fact layer would be " << flItr->first
                                     << ", later than the end actions need\n";
                            }
                        }
                    }
                }
                ...
                const pair<EpsilonResolutionTimestamp, bool> fluentLayersWantToRevisit =
                    fluentLayers.nextLayerWouldBeAt(currTS, isToRecalculate, isDueToGradients);
                ...
                if (fluentLayersWantToRevisit.first != EpsilonResolutionTimestamp::infinite())
                {
                    if (fluentLayersWantToRevisit.second)
                    {
                        ...
                        if (next.timestamp.isEpsilonAhead(currTS))
                        {
                            if (debug)
                            {
                                ...
                                {
                                    cout << "Also revisiting preconditions that have become true due to "
                                            "gradients, epsilon from now\n";
                                }
                            }
                        }
                        else
                        {
                            if (debug)
                            {
                                if (next.revisitInstantaneousNumericEffects)
                                {
                                    cout << "Revisiting effects at the time epsilon from now (" << currTS
                                         << " -> " << fluentLayersWantToRevisit.first
                                         << "), ignoring other later options\n";
                                }
                                else
                                {
                                    cout << "Also revisiting preconditions that have become true due to "
                                            "gradients, epsilon from now; ignoring other later options\n";
                                }
                            }
                            next.endActionsAppearingAtThisTime = endActionsAtTime.end();
                            next.newFactsAtThisTime = factLayers.end();
                    ...
                    else
                    {
                        if (next.timestamp.isEpsilonAhead(currTS))
                        {
                            if (debug)
                            {
                                cout << "Ignoring fluent-layer update at " << fluentLayersWantToRevisit.first
                                     << ", is later than the other options that are only epsilon ahead\n";
                            }
                        ...
                        else
                        {
                            if (fluentLayersWantToRevisit.first > next.timestamp)
                            {
                                if (debug)
                                {
                                    cout << "Ignoring fluent-layer update at "
                                         << fluentLayersWantToRevisit.first
                                         << ", is later than the other options\n";
                                }
                            }
                            else if (fluentLayersWantToRevisit.first == next.timestamp)
                            {
                                ...
                                if (debug)
                                {
                                    cout << "Fluent-layer update at " << fluentLayersWantToRevisit.first
                            ...
                            else
                            {
                                ...
                                {
                                    cout << "Fluent-layer update at " << fluentLayersWantToRevisit.first
                                         << ", earlier than the other options\n";
                                }
                            }
                        }
                    }
            ...
            map<int, pair<EpsilonResolutionTimestamp, EpsilonResolutionTimestamp>>
                propositionMustBeDeletedAddedAfter;

            list<pair<set<int>, set<int>>> gc;

            /** @brief The cost of deleting a fact (from always constraints).  Set details which prefs would
             * be broken.
             */
            ...
            /** @brief A map from preferences to which (applicable) actions would violate them, were they to
             * be applied.
             */
             ...
             *
             * Right now this is only used to set min/max action durations on actions with duration-dependent
             * costs.
             */
            ...
            BuildingPayload(const MinimalState &theState, const list<StartEvent> *seq, vector<int> &spc,
                            vector<int> &epc, vector<int> &nspc, vector<int> &nepc, const int &easSize,
                            const int goalCount, const vector<double> &mtsIn, const double &tsIn,
                            list<ActionSegment> &haIn, const double &costLimit)
              : startState(theState)
              , startEventQueue(seq)
              , startPreconditionCounts(spc)
              , endPreconditionCounts(epc)
              , numericStartPreconditionCounts(nspc)
              , numericEndPreconditionCounts(nepc)
              ...
              ,
#endif
              vCount(theState.secondMin.size())
              , avCount(RPGBuilder::getAVCount())
              , minTimestamps(mtsIn)
              , stateTS(tsIn, true)
              , helpfulActions(haIn)
                                ...
                                for (int pass = 0; pass < 3; ++pass) {
                                    ...
                                    switch(pass) {
                                        case 0:
                                        {
                                            ...
                                            loopOn =
                   &(RPGBuilder::getStartPropositionalPreconditions()[currAct]); numLoopOn =
                   &(RPGBuilder::getStartPreNumerics()[currAct]); break;
                                        }
                                        ...
                                        case 1:
                                        {
                                            ...
                                            loopOn =
                   &(RPGBuilder::getInvariantPropositionalPreconditions()[currAct]); numLoopOn =
                   &(RPGBuilder::getInvariantNumerics()[currAct]); break;
                                        }
                                        ...
                                        case 2:
                                        {
                                            if (ts == Planner::E_AT_START) {
                                                ...
                                                offset = 0.001;
                                                loopOn =
                   &(RPGBuilder::getEndPropositionalPreconditions()[currAct]); numLoopOn =
                   &(RPGBuilder::getEndPreNumerics()[currAct]);
                                            }
                                            break;
                                        }
                                    ...
                                    if (loopOn) {
                                        ...
                                        for (; preItr != preEnd; ++preItr) {
                                            ...
                                            const double poTS =
                   RPGHeuristic::Private::earliestPropositionPOTimes[ID] + offset; if (debug) { if (pass == 0)
                   { cout << " " << *(*preItr) << "s=" << poTS; } else if (pass == 1) { cout << " " <<
                   *(*preItr) << "i=" << poTS; } else { cout << " " << *(*preItr) << "e=" << poTS;
                                                }
                                            }
                                    ...
                                    if (numLoopOn) {
                                        ...
                                        for (; preItr != preEnd; ++preItr) {
                                            RPGBuilder::RPGNumericPrecondition & currPre =
                   RPGBuilder::getNumericPreTable()[*preItr]; const double poTS =
                   RPGHeuristic::Private::earliestPointForNumericPrecondition(currPre) + offset; if (debug) {
                                                if (pass == 0) {
                                ...
                                for (int pass = 0; pass < passCount; ++pass) {
                                    ...
                                    const list<Literal*> * const addLoop = (pass ?
                   &(RPGBuilder::getEndPropositionAdds()[currAct]) :
                   &(RPGBuilder::getStartPropositionAdds()[currAct])); const list<Literal*> * const delLoop =
                   (pass ? &(RPGBuilder::getEndPropositionDeletes()[currAct]) :
                   &(RPGBuilder::getStartPropositionDeletes()[currAct])); const list<int> * const numLoop =
                   (pass ? &(RPGBuilder::getEndEffNumerics()[currAct]) :
                   &(RPGBuilder::getStartEffNumerics()[currAct]));

                                    {
                                        ...
                                        for (; preItr != preEnd; ++preItr) {
                                            const int ID = (*preItr)->getID();
                                            const map<int, pair<double,double> >::iterator poRestrictItr =
                   referTo->propositionMustBeDeletedAddedAfter.find(ID); if (poRestrictItr !=
                   referTo->propositionMustBeDeletedAddedAfter.end()) { const double poTS =
                   poRestrictItr->second.second + offset; #ifdef MDIDEBUG if (debug) { if (pass == 0) { cout
                   << " " << *(*preItr) << "s+=" << poTS; } else { cout << " " << *(*preItr) << "e-=" << poTS;
                                                    }
                                                }
                                                #endif
                                                if (poTS > value) value = poTS;
                   ...
                   referTo->propositionMustBeDeletedAddedAfter.find(ID); if (poRestrictItr !=
                   referTo->propositionMustBeDeletedAddedAfter.end()) { const double poTS =
                   poRestrictItr->second.first + offset; #ifdef MDIDEBUG if (debug) { if (pass == 0) { cout <<
                   " " << *(*preItr) << "s-=" << poTS; } else { cout << " " << *(*preItr) << "e-=" << poTS;
                                                    }
                                                }
                                                #endif
...

                                    {
                                        ...
                                        for (; preItr != preEnd; ++preItr) {
                                            const RPGBuilder::RPGNumericEffect & currEff =
                   RPGBuilder::getNumericEff()[*preItr]; const double poTS =
                   RPGHeuristic::Private::earliestPointForNumericEffect(currEff) + offset; #ifdef MDIDEBUG if
                   (debug) { if (pass == 0) { cout << " " << *preItr << "sne=" << poTS; } else { cout << " "
                   << *preItr << "ene=" << poTS;
                                                }
                                            }
                                            #endif
                                ...
                                if (RPGBuilder::getRPGDEs(currAct).empty()) {
                                    const double poTS =
                   earliestPointForDuration(*(RPGBuilder::getRPGDEs(currAct)[0])) + (ts == Planner::E_AT_START
                   ? 0.001 : offsetToEarlier);
...
#define MAKESTARTMDI(dest, a) MaxDependentInfo dest = a
        ...
        EpsilonResolutionTimestamp getEarliestAchievedAt(const int &factID)
        {
            ...
            {
                cerr << "Warning: asking for the earliest achiever for " << *(RPGBuilder::getLiteral(factID))
                     << ", which never appeared in the RPG\n";
            }
         ...
         *
         * This function is used during solution extraction to find which action to use to achieve a given
         * fact.
         *
         ...
         * @param  returnAchievedIn   After execution, contains the layer in which to add the proposition as
        ...
        void getAchieverDetails(const int &factID, const EpsilonResolutionTimestamp &layerTS,
                                EpsilonResolutionTimestamp &returnAchievedIn,
                                pair<int, Planner::time_spec> &returnAchiever)
        {
#ifdef POPF3ANALYSIS
            if (Globals::globalVerbosity & 16)
            {
                cout << achieverDetails[factID].size() << " achievers, from "
                     << achieverDetails[factID].front().getLayer() << " to "
                     << achieverDetails[factID].back().getLayer() << ",";
                cout.flush();
            }
            ...
            while (abItr->getLayer() > layerTS)
            {
                ...
                if (abItr == achieverDetails[factID].rend())
                {
                    ...
                    cerr << "Internal error evaluating state no. " << RPGHeuristic::statesEvaluated
                         << ": asking for precondition " << factID << ", "
                         << *(RPGBuilder::getLiteral(factID)) << ", at time " << layerTS
                         << ", but the achievers were:\n";
                    ...
                    for (; abfItr != abfEnd; ++abfItr)
                    {
                        ...
                        {
                            cout << "end of " << *(RPGBuilder::getInstantiatedOp(abfItr->getAchiever().first))
                                 << endl;
                        }
                        else
                        {
                            cout << "start of "
                                 << *(RPGBuilder::getInstantiatedOp(abfItr->getAchiever().first)) << endl;
                        }
                    }
                    exit(1);
                }
            ...
            // cout << "Using achiever that adds fact in layer " << abItr->layer << " to support a need in "
            // << layerTS << endl;
...
//}
                                ...
                                const EpsilonResolutionTimestamp &cTime)
        {
            ...
            const vector<NumericAnalysis::NumericLimitDescriptor> &limits =
                NumericAnalysis::getGoalNumericUsageLimits();
            ...
            if (localDebug)
            {
                cout << "Computing dynamic costs on " << *(RPGBuilder::getInstantiatedOp(actID)) << " at "
                     << cTime << endl;
            }
            ...
            for (; costItr != costEnd; ++costItr)
            {
                if ((*costItr)->start > cTime)
                {
                    if (localDebug)
                    {
                        cout << "Outcome [" << (*costItr)->start << "," << (*costItr)->end
                             << "] comes later\n";
                    }
                    continue;
                }
                ...
                list<NumericAnalysis::EffectDeterminedByTime *>::const_iterator deltaItr =
                    (*costItr)->limitCosts.begin();
                const list<NumericAnalysis::EffectDeterminedByTime *>::const_iterator deltaEnd =
                    (*costItr)->limitCosts.end();
                ...
                for (; deltaItr != deltaEnd; ++deltaItr)
                {
                    if ((*deltaItr)->m == 0.0)
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "Increases limit " << (*deltaItr)->y << " by " << (*deltaItr)->c
                                 << " towards cap of "
                                 << NumericAnalysis::getGoalNumericUsageLimits()[(*deltaItr)->y].limit
                                 << endl;
                        }
                    }
                    else
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "Increases limit " << (*deltaItr)->y << " by "
                                 << (*deltaItr)->m * cTime.toDouble() + (*deltaItr)->c << " towards cap of "
                                 << NumericAnalysis::getGoalNumericUsageLimits()[(*deltaItr)->y].limit
                                 << endl;
                        }
                    }
        ...
        void addPreconditionCost(ActionViolationData &costData, const int &currAct,
                                 const Planner::time_spec &currTS,
                                 const EpsilonResolutionTimestamp &factLayerTS, BuildingPayload *payload)
        {
            ...
            {
                const list<Literal *> &actionPreconditionlist =
                    (currTS == Planner::E_AT_START ? (*actionsToProcessedStartPreconditions)[currAct] :
                ...
                {
                    const vector<int> &actPrefs =
                        (currTS == Planner::E_AT_START ? RPGBuilder::getStartPreferences()[currAct] :
                                                         RPGBuilder::getEndPreferences()[currAct]);
                    ...
                    {
                        const NNF_Flat *const f =
                            payload->initialUnsatisfiedPreferenceConditions[actPrefs[p]][0];
                        ...
                        if (f)
                        {
                            if (!f->isSatisfied())
                            {
                                ...
                                // cout << "Applying " << *(RPGBuilder::getInstantiatedOp(currAct)) << " would
                                // mean violating precondition pref " <<
                                // RPGBuilder::getPreferences()[actPrefs[p]].name << ":" << actPrefs[p] << ",
                                // as it is not currently satisfied - "; cout << "cost of violation = " <<
                                // prefCosts[actPrefs[p]] << endl;
                            }
                        }
                        else
                        {
        ...
        void addEffectCost(ActionViolationData &costData, const int &currAct,
                           const Planner::time_spec &currTS, BuildingPayload *payload)
        {
            if (RPGBuilder::getPreferences().empty())
            {
                costData.precViolationCost = 0.0;
...

            {
                list<Literal *> &addEffects =
                    (currTS == Planner::E_AT ?
                         ...
                         (currTS == Planner::E_AT_START ? (*actionsToStartEffects)[currAct] :
                                                          (*actionsToEndEffects)[currAct]));
                ...
                for (; addEffItr != addEffEnd; ++addEffItr)
                {
                    ...
                    map<int, AddingConstraints>::const_iterator addConsItr =
                        payload->prefCostOfAddingFact.find(currEff);
                    if (addConsItr == payload->prefCostOfAddingFact.end())
                    {
                        continue;
                    }

                    allViols.insert(addConsItr->second.addingWillViolate.begin(),
                                    addConsItr->second.addingWillViolate.end());
                    ...
                    if (Globals::globalVerbosity & 32768)
                    {
                        for (; vItr != vEnd; ++vItr)
                        {
                            cout << "Add effect " << *(*addEffItr) << " Would violate "
                                 << RPGBuilder::getPreferences()[*vItr].name << ":" << *vItr << endl;
                        }
                    }
                }
            }

            {
                list<Literal *> &delEffects =
                    (currTS == Planner::E_AT ?
                         ...
                         (currTS == Planner::E_AT_START ? (*actionsToStartNegativeEffects)[currAct] :
                                                          (*actionsToEndNegativeEffects)[currAct]));
                ...
                for (; delEffItr != delEffEnd; ++delEffItr)
                {
                    ...
                    map<int, set<int>>::const_iterator delConsItr =
                        payload->prefCostOfDeletingFact.find(currEff);
                    if (delConsItr == payload->prefCostOfDeletingFact.end())
                    {
                        continue;
                    }
                    ...
                    if (Globals::globalVerbosity & 32768)
                    {
                        ...
                        for (; vItr != vEnd; ++vItr)
                        {
                            cout << "Delete effect " << *(*delEffItr) << " Would violate "
                                 << RPGBuilder::getPreferences()[*vItr].name << ":" << *vItr << endl;
                        }
                    }
                }
            ...
            std::set_difference(allViols.begin(), allViols.end(), costData.preconditionsMeanViolating.begin(),
                                costData.preconditionsMeanViolating.end(),
                                insert_iterator<set<int>>(costData.effectsMeanViolating,
                                                          costData.effectsMeanViolating.begin()));
            ...
            costData.overallViolations.insert(costData.effectsMeanViolating.begin(),
                                              costData.effectsMeanViolating.end());
        }

        CostedAchieverDetails::AchieverProposalResult
        proposeNewAchiever(const int &factID, const int &actID, const Planner::time_spec &ts,
                           const ActionViolationData &prefCosts, const bool &openEnd,
                           const EpsilonResolutionTimestamp &cTime)
        {
#ifdef POPF3ANALYSIS
            ...
            if (ts == Planner::E_AT || currentCosts.size() == 0)
            {
                ...
                achieverDetails[factID].push_back(CostedAchieverDetails(
                    cTime, make_pair(actID, ts), currentCosts, PreferenceSetAndCost(true), true));
                return (achieverDetails[factID].size() > 1 ?
                            CostedAchieverDetails::replaced_existing_achiever :
                            CostedAchieverDetails::first_achiever);
            }
            const vector<NumericAnalysis::NumericLimitDescriptor> &limits =
                NumericAnalysis::getGoalNumericUsageLimits();
            ...
            for (int pPass = (openEnd ? 1 : 0); pPass < 2; ++pPass)
            {
                ...
                const list<Literal *> &pres = (pPass == 1 ? (*actionsToEndPreconditions)[actID] :
                                                            (*actionsToProcessedStartPreconditions)[actID]);
                list<Literal *>::const_iterator pItr = pres.begin();
                const list<Literal *>::const_iterator pEnd = pres.end();

                for (; pItr != pEnd; ++pItr)
                {
                    list<CostedAchieverDetails>::const_reverse_iterator cItr =
                        achieverDetails[(*pItr)->getStateID()].rbegin();
                ...
                {
                    // equivalent to an imaginary fact that the action has started: max of its preconditions,
            ...
            {
                const vector<double> &addOn = (ts == Planner::E_AT_END ? endEffectsOnResourceLimits[actID] :
                                                                         startEffectsOnResourceLimits[actID]);
            ...
            {
                const vector<double> &addOn =
                    (ts == Planner::E_AT_END ? dynamicEndEffectsOnResourceLimits[actID] :
                                               dynamicStartEffectsOnResourceLimits[actID]);
            ...
            for (int ul = 0; ul < ulCount; ++ul)
            {
                double offset = (NumericAnalysis::getGoalNumericUsageLimits()[ul].optimisationLimit ?
                                     prefCosts.overallViolationCost :
                                     0.0);
                switch (limits[ul].op)
                {
                    case E_GREATER:
            ...
            if (achieverDetails[factID].empty())
            {
                ...
                achieverDetails[factID].push_back(CostedAchieverDetails(
                    cTime, make_pair(actID, ts), actCosts,
                    PreferenceSetAndCost(prefCosts.overallViolationCost, prefCosts.overallViolations),
                    false));
                return CostedAchieverDetails::first_achiever;
            }

            // otherwise, do an admissible intersection approximation of the costs compared to the best
            // recorded achiever
            ...
            anyImprovement = (anyImprovement || (achieverDetails[factID].back().preferenceCosts.cost >
                                                 prefCosts.overallViolationCost));
            ...
            {
                achieverDetails[factID].push_back(CostedAchieverDetails(
                    cTime, achieverDetails[factID].back().getAchiever(), admissibleCosts,
                    PreferenceSetAndCost(prefCosts.overallViolationCost, prefCosts.overallViolations),
                    costFree));
            ...
            achieverDetails[factID].push_back(CostedAchieverDetails(
                cTime, achieverDetails[factID].back().getAchiever(), admissibleCosts,
                PreferenceSetAndCost(prefCosts.overallViolationCost, prefCosts.overallViolations), costFree));
        ...
        void populateActionFluentLookupTable()
        {
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                const vector<list<int>> *const nowOn =
                    (pass ? actionsToNumericEndPreconditions : actionsToProcessedStartNumericPreconditions);
                ...
                for (int a = 0; a < actCount; ++a)
                {
                    ...
                    for (; pItr != pEnd; ++pItr)
                    {
                        ...
                        for (int side = 0; side < 2; ++side)
                        {
                            ...
                            {
                                const RPGBuilder::ArtificialVariable &currAV =
                                    RPGBuilder::getArtificialVariable(var);
            ...
            {
                const vector<list<int>> *const nowOn =
                    (pass ? actionsToRPGNumericEndEffects : actionsToRPGNumericStartEffects);
        ...
        void metricHasChanged()
        {
...

            {
                ...
                if (dumpActCosts)
                {
                    cout << "Created space for cost details of " << maxPermissibleCostOfAFact.size()
                         << " facts, and " << nanCosts.size() << " costs\n";
                }
                ...
                if (!RPGBuilder::getPreferences().empty())
                {
                    const vector<list<LiteralCellDependency<pair<int, bool>>>> *const pToP =
                        PreferenceHandler::getPreconditionsToPrefs();
                ...
                for (int l = 0; l < ulCount; ++l)
                {
                    switch (limits[l].op)
                    {
                        ...
                        case VAL::E_GREATER:
                        {
                            ...
                            for (; vItr != vEnd; ++vItr)
                            {
                                varToLowerResourceLimitVariable[vItr->first].insert(
                                    make_pair(l, vItr->second));
                            }
                            break;
                        }
                        default:
                        {
                            cerr << "Internal error: should not have limits specified as anything other than "
                                    ">= or >\n";
                            exit(1);
                        }
                    }
...

            {
                ...
                for (int gID = 0; gID < igcSize; ++gID)
                {
                    ...
                    {
                        isAGoalFactWithIndependentAchivementCosts[literalGoalVector[gID]->getStateID()] =
                            true;
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                const vector<list<int>> *const nowOn =
                    (pass ? actionsToRPGNumericEndEffects : actionsToRPGNumericStartEffects);
                vector<vector<double>> &costsGoIn =
                    (pass ? endEffectsOnResourceLimits : startEffectsOnResourceLimits);
                const vector<list<NumericAnalysis::CostAtTime *> *> &dynamicCosts =
                    (pass ? NumericAnalysis::getTimeDependentEndCosts() :
                            NumericAnalysis::getTimeDependentStartCosts());
                ...
                for (int a = 0; a < actCount; ++a)
                {
                    ...
                    for (; eItr != eEnd; ++eItr)
                    {
                        ...
                        if (!currEff.isAssignment)
                        {
                            // resource limits are only detectable from constant-valued increase/decrease
                            // effects or we can fake it with bounded durations so check that is the case
                            // first
                            ...
                            else if (currEff.size == 1)
                            {
                                ...
                                {
                                    evaluatesTo.first = evaluatesTo.second =
                                        currEff.constant;  // start with the constant offset

                                    assert(currEff.weights[0] >= 0.0);
                                    evaluatesTo.first +=
                                        currEff.weights[0] *
                                        RPGBuilder::getOpMinDuration(a, -1);  // this is okay - is working at
                                                                              // a global level
                                    evaluatesTo.second +=
                                        currEff.weights[0] *
                                        RPGBuilder::getOpMaxDuration(a, -1);  // this is okay - is working at
                                                                              // a global level
                                    ...
                                    if (dumpActCosts)
                                    {
                                        cout << "Duration-dependent effect: " << currEff << " evaluates to "
                                             << evaluatesTo.first << ", " << evaluatesTo.second << endl;
                                    }
                                }
                            ...
                            if (evaluatesSafely)
                            {
                                ...
                                map<int, set<pair<int, double>>>::const_iterator lsItr =
                                    varToLowerResourceLimitVariable.find(currEff.fluentIndex);
                                if (lsItr != varToLowerResourceLimitVariable.end())
                                {
                                    ...
                                    for (; lItr != lEnd; ++lItr)
                                    {
                                        if (lItr->second > 0)
                                        {
                                            ...
                                            if (dumpActCosts)
                                            {
                                                cout << "Cost of " << *(RPGBuilder::getInstantiatedOp(a))
                                                     << " gone up by " << evaluatesTo.second * lItr->second
                                                     << endl;
                                            }
                                        }
                                        else if (lItr->second < 0)
                                        {
                                            ...
                                            {
                                                cout << "Cost of " << *(RPGBuilder::getInstantiatedOp(a))
                                                     << " gone up by " << evaluatesTo.first * lItr->second
                                                     << endl;
                                            }
                                        }
                                        else
                    ...
                    for (; ccItr != ccEnd; ++ccItr)
                    {
                        ...
                        {
                            const list<Literal *> &pres =
                                (ipass ? RPGBuilder::getEndPropositionalPreconditions()[a] :
                            ...
                            if (toUse > -DBL_MAX)
                            {
                                const double newPreLimit = toUse - costsGoIn[a][*ccItr];
                                {
                                    ...
                                    for (; preItr != preEnd; ++preItr)
                                    {
                                        vector<double> &toUpdate =
                                            maxPermissibleCostOfAFact[(*preItr)->getStateID()];
        ...
        void integrateContinuousEffects()
        {
            ...
            for (int lda = 0; lda < loopLim; ++lda)
            {
                ...
                if (currLD)
                {
                    ...
                    if (!mustIntegrate)
                    {
                        mustIntegrate = (RPGBuilder::rogueActions[lda] == RPGBuilder::OT_NORMAL_ACTION &&
                                         !RPGBuilder::isSelfMutex(lda) &&
                                         RPGBuilder::howManyTimesOptimistic(lda, refState) > 1);
                    }

                    const double dur = (RPGBuilder::rogueActions[lda] == RPGBuilder::OT_PROCESS ?
                                            DBL_MAX :
                                            RPGBuilder::getOpMaxDuration(lda, -1));
                    const int effCount = currLD->vars.size();

                    for (int eff = 0; eff < effCount; ++eff)
                    {
                        if (mustIntegrate ||
                            cannotUseAsGradients.find(currLD->vars[eff]) != cannotUseAsGradients.end())
                        {
                            if (evaluateDebug)
                            {
                            ...
                            if (evaluateDebug)
                            {
                                cout << " " << integratedCTSEffectChange[lda].back() << " of "
                                     << *(RPGBuilder::getPNE(integratedCTSEffectVar[lda].back())) << endl;
                            }
                        }
                        else
                        {
                            ...
                            if (evaluateDebug)
                            {
                                cout << "d" << *(RPGBuilder::getPNE(gradientCTSEffectVar[lda].back()))
                                     << "/dt = " << gradientCTSEffectChange[lda].back() << endl;
                            }
                        }
                    }
        ...
        BuildingPayload *spawnNewPayload(const MinimalState &theState, const double &costLimit,
                                         const list<StartEvent> *seq, const vector<double> &minTimestamps,
                                         const double &stateTS, list<ActionSegment> &haIn)
        {
            static const int easSize = initialUnsatisfiedEndPreconditions->size();

            BuildingPayload *const toReturn = new BuildingPayload(
                theState, seq, *(initialUnsatisfiedProcessedStartPreconditions),
                *(initialUnsatisfiedEndPreconditions),
                *(initialUnsatisfiedProcessedStartNumericPreconditions),
                *(initialUnsatisfiedNumericEndPreconditions), easSize, goals.size() + goalFluents.size(),
...

            {
                ...
                {
                    cout << "Fluent table bounds for fluents in the range " << 0 << ".."
                         << toReturn->vCount - 1
                         << ".  Number of non-static PNEs: " << instantiatedOp::howManyNonStaticPNEs()
                         << endl;
                }
         ...
         * @param timeAtWhichValueIsDefined    If the state numeric bound was immediately after the last
         * relevant action, this is that time point.
         * @param gradientsToStart             Updated by this function to contain gradient effects to start
         * corresponding to actions already in the plan that have not yet finished.
         */
        void giveUsTheEffectsOfExecutingActions(
            BuildingPayload *const payload, const vector<double> &timeAtWhichValueIsDefined,
...
// actions' as a proper gradient.
                        ...
                        if (RPGHeuristic::makeCTSEffectsInstantaneous)
                        {
                            cerr << "Error: making continuous effects instantaneous (as in the the Colin "
                                    "IJCAI paper) is incompatible with the revised approach to state "
                                    "variable bounds for heuristic purposes.\n";
                            exit(1);
                        }
#endif

                        gradientsToStart[timeAtWhichValueIsDefined[varList[e]]].push_back(
                            DelayedGradientDescriptor(evItr->actID, Planner::E_AT_START, multiplier,
                                                      make_pair(varList[e], changeList[e].constant)));
                    }
                }

                // if (evaluateDebug || eeDebug) cout << "Clearing extra ends attached to " <<
                // *(RPGBuilder::getInstantiatedOp(saItr->first)) << "\n";
            }
        }

        /** @brief Add delayed gradient effects as slope functions rather than using integration.
         */
        void addDelayedGradientEffects(BuildingPayload *const payload,
                                       const map<double, list<DelayedGradientDescriptor>> &delayedGradients)
        {
            if (RPGBuilder::modifiedRPG)
            {
                // popf heuristic, so gradients might not start straight away
                cerr << "The -/ option is currently not implemented for the POPF heuristic - add -c or -T to "
                        "the command line\n";
                exit(1);
            }
            ...
            for (; dgItr != dgEnd; ++dgItr)
            {
                ...
                for (; gItr != gEnd; ++gItr)
                {
                    if (localDebug)
                    {
                        cout << "-- Adding gradient effect from executing action, d"
                             << *(RPGBuilder::getPNE(gItr->gradientEffect.first))
                             << "/dt = " << gItr->gradientEffect.second << " until " << gItr->maxDur;
                        cout << ", from initial bounds ["
                             << payload->startState.secondMin[gItr->gradientEffect.first] << ","
                             << payload->startState.secondMax[gItr->gradientEffect.first] << "]\n";
                    }
                    payload->fluentLayers.recordGradientNumericEffect(
                        *gItr, EpsilonResolutionTimestamp::epsilon(), gItr->gradientEffect,
                        payload->factLayers);
                }
            }
        }
        ...
        void resetAchievedBy(BuildingPayload *payload)
        {
            ...
            *numericAchievedInLayer = (*numericAchievedInLayerReset);
            ...
            PreferenceHandler::getUnsatisfiedConditionCounts(payload->startState,
                                                             payload->initialUnsatisfiedPreferenceConditions,
                                                             payload->becomesUnreachableAtLayer);
            payload->preferencePartTrueAtLayer.resize(
                payload->initialUnsatisfiedPreferenceConditions.size(),
                vector<EpsilonResolutionTimestamp>(2, EpsilonResolutionTimestamp::infinite()));
            ...
            PreferenceHandler::getCostsOfDeletion(payload->startState, payload->prefCostOfDeletingFact,
                                                  payload->prefCostOfChangingNumberA);
            PreferenceHandler::getCostsOfAdding(payload->startState, payload->prefCostOfAddingFact,
                                                payload->prefCostOfChangingNumberB);
        ...
        static EpsilonResolutionTimestamp
        localEarliestPointForNumericPrecondition(const RPGBuilder::RPGNumericPrecondition &p)
        {
        ...
        static EpsilonResolutionTimestamp earliestPointForNumericEffect(const RPGBuilder::RPGNumericEffect &p)
        {
            ...
            if (earliestNumericPOTimes[p.fluentIndex] > TS)
            {
                TS = earliestNumericPOTimes[p.fluentIndex];
            }
        ...
        void recordFactLayerZero(BuildingPayload *const payload)
        {
...

            {
                ...
                if (loopLim)
                {
                    const vector<double> *const maxFluentTable =
                        &(payload->fluentLayers.borrowFactLayerZeroValues());
                    for (int i = 0; i < loopLim; ++i)
                    {
                        if (ignoreNumbers || (*rpgNumericPreconditions)[i].isSatisfied(*maxFluentTable))
                        {
                            numericIsTrueInState[i] = true;
                            const EpsilonResolutionTimestamp poTS =
                                localEarliestPointForNumericPrecondition((*rpgNumericPreconditions)[i]);
                            ...
                            if (evaluateDebug && RPGBuilder::modifiedRPG &&
                                (*numericAchievedInLayer)[i] > EpsilonResolutionTimestamp::zero())
                            {
                                cout << "RPG modified: delaying numeric fact " << i << " to layer "
                                     << (*numericAchievedInLayer)[i] << "\n";
                            }
        ...
        void recordFactLayerZero(BuildingPayload *const payload)
        {
            ...
            if (RPGBuilder::modifiedRPG)
            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    const int factID = FACTA(stateItr);
#ifdef POPF3ANALYSIS
                    achieverDetails[factID].push_back(
                        CostedAchieverDetails(currentCosts, PreferenceSetAndCost(true), true));
                    CostedAchieverDetails &achievedTS = achieverDetails[factID].back();
#else
                    EpsilonResolutionTimestamp *const achievedTS =
                        &((*achievedInLayer)[factID] = EpsilonResolutionTimestamp::zero());
#endif
                    ...
                    if (from.beforeOrAfter == StepAndBeforeOrAfter::BEFORE)
                    {  // special case: initial state is anything achieved /before/ step 0, rather than at
                       // step 0
                        earliestPropositionPOTimes[factID] = EpsilonResolutionTimestamp::zero();
                        // if (!stateItr->second.deletableFrom.empty()) {
                        //     toUpdate =
                        //     (&payload->propositionMustBeDeletedAddedAfter.insert(make_pair(factID,make_pair(0.0,0.0))).first->second);
                        // }
                        // if (!expandFully) cout << "Initial Fact " << *(RPGBuilder::getLiteral(factID)) << "
                        // is true in the initial state\n";
                    }
                    else
                    {
                        const unsigned int dependsOnActionAtStep = from.stepID;
                        const EpsilonResolutionTimestamp actTS(payload->minTimestamps[dependsOnActionAtStep],
                                                               true);
                        earliestPropositionPOTimes[factID] = actTS;
                        // if (!expandFully) cout << "Initial Fact " << *(RPGBuilder::getLiteral(factID)) << "
                        // appears in PO no earlier than action " << dependsOnActionAtStep << ", i.e. t=" <<
                        // earliestPropositionPOTimes[factID] << "\n";

                        toUpdate =
                            &(payload->propositionMustBeDeletedAddedAfter
                                  .insert(make_pair(factID, make_pair(EpsilonResolutionTimestamp::zero(),
                                                                      EpsilonResolutionTimestamp::zero())))
                                  .first->second);
                        ...
                        {
                            /*if (evaluateDebug) cout << "RPG modified: delaying " <<
                            *(RPGBuilder::getLiteral(factID)) << " to layer " << (actTS -
                            ...
                            *achievedTS = actTS - EpsilonResolutionTimestamp::epsilon();
                            ...
                            if (evaluateDebug)
                            {
                                cout << "RPG modified: delaying " << *(RPGBuilder::getLiteral(factID))
                                     << " to layer " << shiftedTS << "\n";
                            }
#ifdef POPF3ANALYSIS
                    ...
                    if (toUpdate)
                    {
                        map<StepAndBeforeOrAfter, bool>::const_iterator invItr =
                            stateItr->second.deletableFrom.begin();
                        const map<StepAndBeforeOrAfter, bool>::const_iterator invEnd =
                            stateItr->second.deletableFrom.end();

                        for (; invItr != invEnd; ++invItr)
                        {
                            const EpsilonResolutionTimestamp actTS(
                                payload->minTimestamps[invItr->first.stepID] -
                                    (invItr->first.beforeOrAfter == StepAndBeforeOrAfter::BEFORE ? 0.001 :
                                                                                                   0.0),
                                true);
                            if (actTS > EpsilonResolutionTimestamp::zero())
                            {
                                ...
                                if (evaluateDebug)
                                {
                                    cout << "RPG modified: cannot delete "
                                         << *(RPGBuilder::getLiteral(factID)) << " until layer " << actTS
                                         << "\n";
                                }
                            }
                        }
            ...
            else
            {
                ...
                const map<int, PropositionAnnotation>::const_iterator stateEnd =
                    payload->startState.first.end();
            ...
            if (RPGBuilder::modifiedRPG)
            {
                map<int, PropositionAnnotation>::const_iterator stateItr =
                    payload->startState.retired.begin();
                const map<int, PropositionAnnotation>::const_iterator stateEnd =
                    payload->startState.retired.end();
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    ...
                    const EpsilonResolutionTimestamp actTS(payload->minTimestamps[dependsOnActionAtStep],
                                                           true);
                    pair<EpsilonResolutionTimestamp, EpsilonResolutionTimestamp> &toUpdate =
                        payload->propositionMustBeDeletedAddedAfter
                            .insert(make_pair(factID, make_pair(EpsilonResolutionTimestamp::zero(),
                                                                EpsilonResolutionTimestamp::zero())))
                            .first->second;
                    ...
                    if (toUpdate.first < actTS)
                    {
                        if (evaluateDebug)
                        {
                            cout << "RPG modified: cannot delete " << *(RPGBuilder::getLiteral(factID))
                                 << " until layer " << actTS << ", due to existing deletor\n";
                        }
                        toUpdate.first = actTS;
                    }
                    if (toUpdate.second < actTS)
                    {
                        if (evaluateDebug)
                        {
                            cout << "RPG modified: cannot add " << *(RPGBuilder::getLiteral(factID))
                                 << " until layer " << actTS << ", due to existing deletor\n";
                        }
                        toUpdate.second = actTS;
                    }
                    ...
                    for (; invItr != invEnd; ++invItr)
                    {
                        const EpsilonResolutionTimestamp actTS(
                            payload->minTimestamps[invItr->first.stepID] -
                                (invItr->first.beforeOrAfter == StepAndBeforeOrAfter::BEFORE ? 0.001 : 0.0),
                            true);
                        if (actTS > EpsilonResolutionTimestamp::zero())
                        {
                            if (toUpdate.first < actTS)
            ...
            if (RPGBuilder::modifiedRPG)
            {
                ...
                const vector<FluentInteraction> &lastStepToTouchPNE =
                    payload->startState.temporalConstraints->lastStepToTouchPNE;
                ...
                for (int i = 0; i < loopLim; ++i)
                {
                    ...
                    if (stepID == -1)
                    {
                        ...
                        earliestNumericPOTimes[i] =
                            EpsilonResolutionTimestamp::zero() - EpsilonResolutionTimestamp::epsilon();
                    }
                    else
                    {
                        if (evaluateDebug)
                        ...
                        {
                            cerr << "Fatal error: for variable " << i << ", timestamp of step " << stepID
                                 << " is " << payload->minTimestamps[stepID] << endl;
                        ...
                        earliestNumericPOTimes[i] =
                            EpsilonResolutionTimestamp(payload->minTimestamps[stepID], true);
                    }
                }
            }
            else
                ...
                if (loopLim)
                {
                    ...
                    for (int i = 0; i < loopLim; ++i)
                    {
                        if (ignoreNumbers || (*rpgNumericPreconditions)[i].isSatisfied(*maxFluentTable))
                        {
                            ...
                            (*numericAchievedBy)[i] = 0;
                            (*numericAchievedInLayer)[i] =
                                (RPGBuilder::modifiedRPG && poTS > EpsilonResolutionTimestamp::zero() ?
                                     (poTS - EpsilonResolutionTimestamp::epsilon()) :
                                     EpsilonResolutionTimestamp::zero());
                            if (evaluateDebug && RPGBuilder::modifiedRPG &&
                                (*numericAchievedInLayer)[i] > EpsilonResolutionTimestamp::zero())
                            {
        ...
        {
            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    ...
                    const list<LiteralCellDependency<pair<int, bool>>> &dependents =
                        (*negativePreconditionsToPrefs)[factID];
                    list<LiteralCellDependency<pair<int, bool>>>::const_iterator depItr = dependents.begin();
                    const list<LiteralCellDependency<pair<int, bool>>>::const_iterator depEnd =
                        dependents.end();

                    for (; depItr != depEnd; ++depItr)
                    {
                        NNF_Flat *const toUpdate =
                            payload->initialUnsatisfiedPreferenceConditions[depItr->dest.first]
                                                                           [depItr->dest.second ? 1 : 0];
        ...
        void addRequirementToHaveSeenTheEndOfAllCurrentlyExecutingActions(BuildingPayload *const payload)
        {
            ...
            for (; saItr != saEnd; ++saItr)
            {
                if (!TemporalAnalysis::canSkipToEnd(saItr->first))
                {
                    ...
                    if (evaluateDebug)
                    {
                        cout << "Insisting on the end of " << saItr->first << " - "
                             << *(RPGBuilder::getInstantiatedOp(saItr->first)) << " - is not skippable\n";
                    }
#ifndef NDEBUG
                ...
                else
                {
                    if (evaluateDebug)
                    {
                        cout << "End of " << *(RPGBuilder::getInstantiatedOp(saItr->first))
                             << " is a skippable\n";
                    }
                }
            }
                ...
                if (gfItr != gfEnd)
                {
                    ...
                    for (; gfItr != gfEnd; ++gfItr)
                    {
                        if (ignoreNumbers || (*rpgNumericPreconditions)[*gfItr].isSatisfied(*maxFluentTable))
                        {
                            if (evaluateDebug)
                            {
                                cout << "\t" << (*rpgNumericPreconditions)[*gfItr]
        ...
        void addTemporalConstraintsFromActiveActions(BuildingPayload *const payload)
        {
            ...
            map<int, pair<EpsilonResolutionTimestamp, EpsilonResolutionTimestamp>>::iterator constrItr =
                payload->propositionMustBeDeletedAddedAfter.begin();
            const map<int, pair<EpsilonResolutionTimestamp, EpsilonResolutionTimestamp>>::iterator constrEnd =
                payload->propositionMustBeDeletedAddedAfter.end();
            ...
            for (; constrItr != constrEnd; ++constrItr)
            {
                ...
                for (int pass = 0; pass < 2; ++pass)
                {
                    const list<pair<int, Planner::time_spec>> &effList =
                        (pass ? (*negativeEffectsToActions)[litID] : (*effectsToActions)[litID]);
                    const EpsilonResolutionTimestamp layer =
                        (pass ? constrItr->second.second : constrItr->second.first) -
                        EpsilonResolutionTimestamp::epsilon();

                    if (layer <= EpsilonResolutionTimestamp::epsilon())
                    {
                        continue;
                    }
                    ...
                    for (; effItr != effEnd; ++effItr)
                    {
                        ...
                        if (RPGBuilder::rogueActions[effItr->first] == RPGBuilder::OT_NORMAL_ACTION)
                        {
                            if (effItr->second == Planner::E_AT_START)
                            {
                                ...
                                {
                                    ++(payload->forbiddenStart.insert(pair<int, int>(effItr->first, 0))
                                           .first->second);
                                }
                            }
                            else
                            {
                                ...
                                {
                                    ++(payload->forbiddenEnd.insert(pair<int, int>(effItr->first, 0))
                                           .first->second);
                                }
                            }
                        }
                    ...
                    if (layerEntry.endOfJustApplied->first.empty() &&
                        layerEntry.endOfJustApplied->second.empty())
                    {
            ...
            {
                const EpsilonResolutionTimestamp layer =
                    earliestNumericPOTimes[v] - EpsilonResolutionTimestamp::epsilon();
                if (layer <= EpsilonResolutionTimestamp::epsilon())
                {
                    continue;
                }
                    ...
                    for (; aItr != aEnd; ++aItr)
                    {
                        if (RPGBuilder::rogueActions[*aItr] == RPGBuilder::OT_NORMAL_ACTION)
                        {
                            if (destSet.insert(*aItr).second)
                            {
                                ...
                                {
                                    ++(payload->forbiddenStart.insert(pair<int, int>(*aItr, 0))
                                           .first->second);
                                }
                            }
                        }
        ...
        bool addTemporalConstraintsFromActiveActions(BuildingPayload *const payload,
                                                     map<double, list<pair<int, int>>> *const justApplied,
                                                     const EpsilonResolutionTimestamp &stateTS,
                                                     const int &nextTIL, const double &tilFrom)
        {
            if (!justApplied)
            {
                if (evaluateDebug)
            ...
            EpsilonResolutionTimestamp TILoffset((nextTIL < tilCount ? tilTimes[nextTIL] - tilFrom : 0.0),
                                                 true);
                ...
                if (insLayer > EpsilonResolutionTimestamp::zero())
                {
                    ...
                    for (; liItr != liEnd; ++liItr)
                    {
                        const int currActID = liItr->first;
                        {
                            ...
                            for (; invItr != invEnd; ++invItr)
                            {
                                ...
                                list<pair<int, Planner::time_spec>> &affected =
                                    (*negativeEffectsToActions)[litID];
                                ...
                                for (; affItr != affEnd; ++affItr)
                                {
                                    if (affItr->second == Planner::E_AT_START)
                                    {
                                        if (evaluateDebug)
                                        {
                                            cout << "Delaying start of " << affItr->first << " to "
                                                 << insLayer << "\n";
                                        }
                                    ...
                                    else
                                    {
                                        ...
                                        if (evaluateDebug)
                                        {
                                            cout << "i) Delaying end of " << affItr->first << " to "
                                                 << insLayer << "\n";
                                        }
                                    }
                                }
                                if (nextTIL < tilCount)
                                {
                                    ...
                                    {
                                        const EpsilonResolutionTimestamp earliestTILtime =
                                            EpsilonResolutionTimestamp(tilTimes[i], true) - TILoffset;

                                        if (earliestTILtime < insLayer)
                                        {
                                            list<int>::iterator effItr =
                                                tilTemporaryNegativeEffects[i].begin();
                                            const list<int>::iterator effEnd =
                                                tilTemporaryNegativeEffects[i].end();

                                            for (; effItr != effEnd; ++effItr)
                                            {
                                                if (*effItr == litID)
                                                {
                                                    TILoffset -= ((insLayer - earliestTILtime) +
                                                                  EpsilonResolutionTimestamp::epsilon());
                                                    if (TILoffset < stateTS)
                                                    {
                                                        if (evaluateDebug)
                                                        {
                                                            cout << "Dead end found: TIL definitely going to "
                                                                    "delete invariant of open action\n";
                                                        }
                                                        return false;
                                                    }
                                                    break;
                    ...
                    if (!affectedStarts.empty() || !affectedEnds.empty())
                    {
                        ...
                        payload->factLayers[insLayer].endOfJustApplied = &(setsToForbid.back());
                        {
                            ...
                            for (; sItr != sEnd; ++sItr)
                            {
                                ...
                                if (evaluateDebug)
                                {
                                    cout << "Start of " << *sItr << " now delayed because of "
                                         << payload->forbiddenStart[*sItr] << " actions\n";
                                }
                            }
                        }
                        {
                            ...
                            for (; sItr != sEnd; ++sItr)
                            {
                                ...
                                if (evaluateDebug)
                                {
                                    cout << "End of " << *sItr << " now delayed because of "
                                         << payload->forbiddenEnd[*sItr] << " actions\n";
                                }
                            }
                        }
                    }
                    if (!affectedEnds.empty())
                    {
                        ...
                        for (; sItr != sEnd; ++sItr)
                        {
                            ...
                            const EpsilonResolutionTimestamp earlierIns =
                                insLayer - EpsilonResolutionTimestamp(actDur, false);
                            if (earlierIns > EpsilonResolutionTimestamp::zero())
                            {
                                ...
                                if (!fle.endOfJustApplied)
                                {
                                    ...
                                    ++(payload->forbiddenStart.insert(pair<int, int>(*sItr, 0))
                                           .first->second);
                                }
                ...
                if (insLayer > EpsilonResolutionTimestamp::zero())
                {
                    ...
                    for (; liItr != liEnd; ++liItr)
                    {
                        ...
                        if (eas.isUndefined())
                        {
                            ...
                            if (evaluateDebug)
                            {
                                cout << "ii) Delaying end of " << currActID << " until " << insLayer << "\n";
                            }
                        }
                    }
                }
        ...
        void performTILInitialisation()
        {
...

            {
                ...
                deadlineAtTime =
                    vector<EpsilonResolutionTimestamp>(loopLim, EpsilonResolutionTimestamp::infinite());
            }

            earliestDeadlineRelevancyStart = vector<EpsilonResolutionTimestamp>(
                initialUnsatisfiedEndPreconditions->size(), EpsilonResolutionTimestamp::undefined());
            earliestDeadlineRelevancyEnd = vector<EpsilonResolutionTimestamp>(
                initialUnsatisfiedEndPreconditions->size(), EpsilonResolutionTimestamp::undefined());
            ...
            for (int i = tilCount - 1; tilItr != tilEnd; ++tilItr, --i)
            {
...

                {
                    ...
                    for (; effItr != effEnd; ++effItr)
                    {
                        ...
                        if ((*effectsToActions)[currEffID].empty() &&
                            addedLater.find(currEffID) == addedLater.end())
                        {
        ...
        void initialiseLatestArrays()
        {
            ...
            if (!initLatestArrays)
            {
                earliestStartAllowed =
                    vector<EpsilonResolutionTimestamp>(easSize, EpsilonResolutionTimestamp::undefined());
                earliestEndAllowed =
                    vector<EpsilonResolutionTimestamp>(easSize, EpsilonResolutionTimestamp::undefined());
                latestStartAllowed =
                    vector<EpsilonResolutionTimestamp>(easSize, EpsilonResolutionTimestamp::undefined());
            ...
            for (int i = 0; i < easSize; ++i)
            {
                latestStartAllowed[i] = EpsilonResolutionTimestamp::infinite();
            }
            for (int i = 0; i < easSize; ++i)
            {
                latestEndAllowed[i] = EpsilonResolutionTimestamp::infinite();
            }
            ...
            if (expandFully)
            {
                for (int i = 0; i < easSize; ++i)
                {
                    earliestStartAllowed[i] = EpsilonResolutionTimestamp::infinite();
                }
                for (int i = 0; i < easSize; ++i)
                {
                    earliestEndAllowed[i] = EpsilonResolutionTimestamp::infinite();
                }
            }
        }

        void addTILsBeforeExpansion(BuildingPayload *const payload, const int &nextTIL,
                                    FactLayerMap &factLayers, const MinimalState &stateBeingEvaluated,
                                    const EpsilonResolutionTimestamp &stateTS, const double &tilFrom)
        {
            if (nextTIL < tilCount)
            {
                static const int easSize = initialUnsatisfiedEndPreconditions->size();
                ...
                for (int i = 0; i < easSize; ++i)
                {
                    earliestDeadlineRelevancyStart[i] = EpsilonResolutionTimestamp::infinite();
                }
                for (int i = 0; i < easSize; ++i)
                {
                    earliestDeadlineRelevancyEnd[i] = EpsilonResolutionTimestamp::infinite();
                }

                set<int> earlier;

                const double TILoffset =
                    (RPGBuilder::modifiedRPG ? 0.000 :
                                               (nextTIL < tilCount ? tilTimes[nextTIL] - tilFrom : 0.0));
                ...
                for (int i = nextTIL; i < tilCount; ++i)
                {
                    ...
                    /*{
                        ...
                        for (; effItr != effEnd; ++effItr) {
                            ...
                            if (currAIL == EpsilonResolutionTimestamp::undefined() &&
                    (*achievedBy)[currEff].first == -1) { // not in initial state if
                    (earlier.insert(currEff).second) { assert(thisTS >= EpsilonResolutionTimestamp::zero());
                                    factLayers[thisTS].TILs.push_back(pair<int, int>(i, currEff));
                                }
                            }
                            #endif
                ...
                for (int i = nextTIL; i < tilCount; ++i)
                {
                    const EpsilonResolutionTimestamp thisTS =
                        EpsilonResolutionTimestamp(tilTimes[i], true) - stateTS;
                    ...
                    for (; effItr != effEnd; ++effItr)
                    {
                        ...
                        for (; ioItr != ioEnd; ++ioItr)
                        {
                            ...
                            {
                                if (latestStartAllowed[actID] > thisTS)
                                {
                                    latestStartAllowed[actID] = thisTS;
                                }
                                const EpsilonResolutionTimestamp maxDur(
                                    RPGBuilder::getOpMaxDuration(actID, 0), false);
                                if (latestEndAllowed[actID] > thisTS + maxDur)
                                {
                                    latestEndAllowed[actID] = thisTS + maxDur;
                                }
                            }
                            else
                            {
                                const EpsilonResolutionTimestamp minDur(payload->actionDurations[actID].first,
                                                                        false);
                                if (latestStartAllowed[actID] > thisTS - minDur)
                                {
                                    latestStartAllowed[actID] = thisTS - minDur;
                                }
            ...
            if (RPGBuilder::modifiedRPG)
            {
                ...
                payload->abstractFactCurrentlyTrue.resize(RPGBuilder::getNegativeEffectsToActions().size(),
                                                          false);
                ...
                for (; factItr != factEnd; ++factItr)
                {
                    ...
                    if (presentItr == stateBeingEvaluated.first.end())
                    {
                        if (abstractTILDebug)
                        {
                            cout << COLOUR_light_blue << "Abstract fact "
                                 << *(RPGBuilder::getLiteral(factItr->first))
                                 << " - not in state, assuming it's expired\n"
                                 << COLOUR_default;
                        }
                        continue;
                    }
                    ...
                    {
                        const double &earliestAsDouble =
                            payload->minTimestamps[presentItr->second.availableFrom.stepID];
                        assert(earliestAsDouble >= 0.0);  // initial state adds should have a 'BEFORE' label

                        // because TILs true initially need to have a 'zero' timestamp for their fact layer
                        // index, TIL facts that are (at x y) are actually put in fact layer x + epsilon
                    ...
                    if (!currentlyFalse)
                    {
                        ...
                        factLayers[earliestPossibleOccurrence].abstractFactBecomesTrue.push_back(
                            factItr->first);
                        if (abstractTILDebug)
                        {
                ...
                {
                    {
                        ...
                        if (firmUpper != DBL_MAX)
                        {
                            ...
                            if (rounded < latestStartAllowed[a])
                            {
                                latestStartAllowed[a] = rounded;
                            }
                        }
                    }
                    {
                        ...
                        {
                            EpsilonResolutionTimestamp rounded(firmUpper, false);
                            if (rounded < latestEndAllowed[a])
                            {
                                latestEndAllowed[a] = rounded;
                            }
                        }
                    }
                }
            ...
            else
            {
                for (int a = 0; a < actCount; ++a)
                {
                    ...
                    }
                    {
                        ...
                        if (latestEndAllowed[a] < EpsilonResolutionTimestamp::zero())
                        {
                            latestStartAllowed[a] = EpsilonResolutionTimestamp::undefined();
                        }
                    }
                }
            }
        ...
        void findApplicableActions(const MinimalState &theState, const double &stateTime,
                                   list<ActionSegment> &applicableActions);
        void filterApplicableActions(const MinimalState &theState, const double &stateTime,
                                     list<ActionSegment> &applicableActions);
        bool testApplicability(const MinimalState &theState, const double &stateTime,
                               const ActionSegment &actID, const bool &fail, const bool &ignoreDeletes);
        ...
        bool sensiblePair(const BuildingPayload *const payload, const pair<int, Planner::time_spec> &achiever,
                          const unsigned int &fact)
        {
            ...
            else
            {
                if (achiever.first < 0)
                {
                    cerr << "\t\tTrying to use a negative indexed action as an achiever for fact "
                         << *(RPGBuilder::getLiteral(fact)) << "\n";
        ...
        void addGoalsForPreferences(BuildingPayload *const payload, RPGRegressionMap &goalsAtLayer)
        {
            ...
            PreferenceHandler::getDesiredGoals(desired, desiredNegative, &desiredNumeric, payload->startState,
                                               payload->initialUnsatisfiedPreferenceConditions,
                                               payload->optimisticPrefStatus, achieverDetails,
                                               *negativeAchievedInLayer, numericAchievedInLayer);

            {
                ...
                for (; dItr != dEnd; ++dItr)
                {
                    ...
                    for (; fItr != fItrEnd; ++fItr)
                    {
                        ...
                        const EpsilonResolutionTimestamp &insLayer =
                            achieverDetails[currGoal].back().getLayer();
                        if (insLayer > EpsilonResolutionTimestamp::zero())
                        {
                            const int achiever = achieverDetails[currGoal].back().getAchiever().first;
                            if (achiever != -1)
                            {
                                goalsAtLayer[insLayer].propositionalGoals.insert(
                                    pair<int, EpsilonResolutionTimestamp>(
                                        currGoal, EpsilonResolutionTimestamp::infinite()));
                                if (evaluateDebug)
                                {
                                    cout << "Preference goal " << *(*fItr)
                                         << " to be achieved in layer with TS " << insLayer << "\n";
                                }
                            }
                            else
                            {
                                assert(payload->startState.first.find(currGoal) !=
                                       payload->startState.first.end());
                                if (evaluateDebug)
                                {
                                    cout << "Preference goal " << *(*fItr)
                                         << " achieved in initial state at layer " << insLayer << "\n";
                                }
                            }
                        }
                        else if (evaluateDebug)
                        {
                            cout << "Preference goal " << *(*fItr)
                                 << " achieved in initial state, not adding to RPG regression\n";
                        }
                    }
                }
            }
                ...
                for (; dItr != dEnd; ++dItr)
                {
                    ...
                    for (; fItr != fItrEnd; ++fItr)
                    {
                        ...
                        if (insLayer > EpsilonResolutionTimestamp::zero())
                        {
                            ...
                            if (achiever != -1)
                            {
                                goalsAtLayer[insLayer].negativePropositionalGoals.insert(
                                    pair<int, EpsilonResolutionTimestamp>(
                                        currGoal, EpsilonResolutionTimestamp::infinite()));
                                if (evaluateDebug)
                                {
                ...
                for (; dNumItr != dNumEnd; ++dNumItr)
                {
                    ...
                    for (; nItr != nEnd; ++nItr)
                    {
                        ...
                        if (numericIsTrueInState[currGoal])
                        {
                            if (evaluateDebug)
                            {
                                cout << "Numeric goal achieved in state, not adding to RPG regression\n";
                            }
                            continue;
                        }
                        ...
                        if (evaluateDebug)
                        {
                            cout << "Numeric goal to be achieved in layer with TS " << insLayer << "\n";
                        }
        ...
        void addExtraPreconditionsForPreferences(BuildingPayload *const payload, const int &act,
                                                 const Planner::time_spec &ts,
                                                 const EpsilonResolutionTimestamp &currTS,
                                                 const EpsilonResolutionTimestamp &tilR,
                                                 RPGRegressionMap &goalsAtLayer)
        {
            map<pair<int, Planner::time_spec>, list<pair<int, bool>>>::const_iterator extras =
                payload->preferencePartsToSatisfyBeforeAction.find(make_pair(act, ts));
            ...
            if (extras != payload->preferencePartsToSatisfyBeforeAction.end())
            {
                ...
                {
                    const EpsilonResolutionTimestamp &trueInLayer =
                        payload->preferencePartTrueAtLayer[prefPItr->first][prefPItr->second ? 1 : 0];

                    if (trueInLayer >= currTS)
                    {
                        if (evaluateDebug)
                        {
                            cout << COLOUR_yellow << "\t\tToo early to manage to satisfy preference "
                                 << prefPItr->first << COLOUR_default << endl;
                        }
                        continue;
                    }
                    else
                    {
                        if (evaluateDebug)
                        {
                            cout << COLOUR_yellow << "\t\tPreference " << prefPItr->first
                                 << " was satisfied in layer " << trueInLayer << " - adding preconditions"
                                 << COLOUR_default << endl;
                        }
                    }
                    ...
                    PreferenceHandler::getPreconditionsToSatisfy(
                        extraPreconditionList, extraNegativePreconditionList, &extraNumericPreconditionList,
                        *prefPItr, currTS, achieverDetails, *negativeAchievedInLayer, numericAchievedInLayer);

                    {
                        ...
                        for (; aplItr != aplEnd; ++aplItr)
                        {
                            ...
                            if (evaluateDebug)
                            {
                                cout << "\t\tPreference precondition " << currPrec << " "
                                     << *(RPGBuilder::getLiteral(currPrec)) << ": ";
                                cout.flush();
                            }
                            ...
                            if (acIn > EpsilonResolutionTimestamp::zero())
                            {
                                if (acBy.first != -1)
                                {
                                    map<int, EpsilonResolutionTimestamp>::iterator insItr =
                                        goalsAtLayer[acIn]
                                            .propositionalGoals
                                            .insert(pair<int, EpsilonResolutionTimestamp>(currPrec, tilR))
                                            .first;
                                    if (insItr->second > tilR)
                                    {
                                        insItr->second = tilR;
                                    }
                                    if (evaluateDebug)
                                    {
                                        cout << "required at time " << acIn << "\n";
                                    }
                                    assert(acIn < currTS);
                                    // isHelpful = false;
                                }
                                else
                                {
                                    if (evaluateDebug)
                                    {
                                        cout << "in initial state at layer " << acIn << "\n";
                                    }
                                }
                            }
                            else
...

                    {
                        ...
                        for (; aplItr != aplEnd; ++aplItr)
                        {
                            ...
                            if (acIn > EpsilonResolutionTimestamp::zero() && !numericIsTrueInState[currPrec])
                            {
                                if (evaluateDebug)
                                {
                                    cout << "\t\tAdding requirement for numeric preference precondition "
                                         << currPrec << ", " << RPGBuilder::getNumericPreTable()[currPrec]
                                         << " at time " << acIn << "\n";
                                }

                                payload->fluentLayers.requestNumericPrecondition(currPrec, acIn, currTS,
                                                                                 goalsAtLayer, tilR);
                                // isHelpful = false;
                            }
                            else
                            {
                                if (evaluateDebug)
                                {
                                    cout << "\t\tNumeric preference precondition " << currPrec << ", "
                                         << RPGBuilder::getNumericPreTable()[currPrec]
        ...
        void extractRP(BuildingPayload *const payload, int &h,
                       list<pair<double, list<ActionSegment>>> &relaxedPlan,
                       pair<int, Planner::time_spec> &earliestTIL, double &makespanEstimate,
                       const vector<EpsilonResolutionTimestamp> &earliestRelevanceOfGoal)
        {
...

            {
                ...
                for (; gsItr != gsEnd; ++gsItr)
                {
                    ...
                    if (insLayer > 0.0)
                    {
                        ...
                        {
#ifdef POPF3ANALYSIS
                            const map<int, int>::const_iterator fToG =
                                RPGBuilder::getLiteralsToGoalIndex().find(*gsItr);
                            assert(fToG != RPGBuilder::getLiteralsToGoalIndex().end());
#ifndef NDEBUG
                            if (earliestRelevanceOfGoal.size() <= fToG->second)
                            {
                                cerr << "Fatal error: fact " << *(RPGBuilder::getLiteral(*gsItr))
                                     << " is down as being goal " << fToG->second
                                     << ", but this does not exist\n";
                                cerr << "Number of literal goals: " << RPGBuilder::getLiteralGoals().size()
                                     << ", size of the local goal set: " << goals.size() << endl;
                                exit(1);
                            }
#endif
                            goalsAtLayer[insLayer].propositionalGoals.insert(
                                pair<int, EpsilonResolutionTimestamp>(currGoal,
                                                                      earliestRelevanceOfGoal[fToG->second]));
#else
                            goalsAtLayer[insLayer].propositionalGoals.insert(
                                pair<int, EpsilonResolutionTimestamp>(
                                    currGoal, EpsilonResolutionTimestamp::infinite()));
                            ...
                            if (evaluateDebug)
                            {
                                cout << "Goal " << *(RPGBuilder::getLiteral(*gsItr))
                                     << " to be achieved in layer with TS " << insLayer << "\n";
                            }
                        }
                        else
            ...
            if (!RPGBuilder::nonTemporalProblem())
            {
                ...
                for (; ueItr != ueEnd; ++ueItr)
                {
                    ...
                    if (evaluateDebug)
                    {
                        cout << "End of " << *(RPGBuilder::getInstantiatedOp(currAct)) << " goes at "
                             << insLayer << endl;
                    }
                    const EpsilonResolutionTimestamp tilR = earliestTILForAction(currAct, false);
                    (goalsAtLayer[insLayer + EpsilonResolutionTimestamp::epsilon()]
                         .actionEnds
                         .insert(pair<int, pair<int, EpsilonResolutionTimestamp>>(
                             currAct, pair<int, EpsilonResolutionTimestamp>(0, tilR)))
                         .first->second.first) += ueItr->second.size();
                    // HACK
                }
            }
            ...
            while (!goalsAtLayer.empty())
            {
                ...
                map<int, EpsilonResolutionTimestamp> &currPositiveGAL =
                    goalsAtLayer.rbegin()->second.propositionalGoals;
                map<int, EpsilonResolutionTimestamp> &currNegativeGAL =
                    goalsAtLayer.rbegin()->second.negativePropositionalGoals;

                if (evaluateDebug)
                {
                    cout << COLOUR_light_green << currTS << ":\n" << COLOUR_default;
                }
                map<int, pair<int, EpsilonResolutionTimestamp>> &actionStarts =
                    goalsAtLayer.rbegin()->second.actionStarts;
                map<int, pair<int, EpsilonResolutionTimestamp>> &actionEnds =
                    goalsAtLayer.rbegin()->second.actionEnds;

                bool alreadyPushed = false;

                {
                    map<int, pair<int, EpsilonResolutionTimestamp>>::iterator asItr = actionStarts.begin();
                    const map<int, pair<int, EpsilonResolutionTimestamp>>::iterator asEnd =
                        actionStarts.end();

                    if (evaluateDebug && asItr != asEnd)
                    {
                        cout << "Adding starts at TS " << currTS << "\n";
                    }
                    ...
                    for (; asItr != asEnd; ++asItr)
                    {
                        if (evaluateDebug)
                        {
                            cout << "\t\tAdding start of " << asItr->first << ": "
                                 << *(RPGBuilder::getInstantiatedOp(asItr->first)) << endl;
                        }

                        EpsilonResolutionTimestamp tilR = earliestTILForAction(asItr->first, true);
                        if (tilR > asItr->second.second)
                        {
                            tilR = asItr->second.second;
                        }

                        if (!alreadyPushed)
                        {
                            relaxedPlan.push_front(pair<double, list<ActionSegment>>(
                                (currTS - EpsilonResolutionTimestamp::epsilon()).toDouble(),
                                list<ActionSegment>()));
                            alreadyPushed = true;
                        }

                        relaxedPlan.front().second.insert(
                            relaxedPlan.front().second.end(), asItr->second.first,
                            ActionSegment(getOp(asItr->first), Planner::E_AT_START, -1, emptyIntList));
                        ++h;
                        ...
                        //              if (currTS == EPSILON) {
                        //                  helpfulActions.push_back(pair<int, Planner::time_spec>(*asItr,
                        //                  Planner::E_AT_START)); if (evaluateDebug) cout << "\t\tIs a
                        //                  helpful action\n";
                        //              }

                        for (int pass = 0; pass < 2; ++pass)
                        {
                            map<int, EpsilonResolutionTimestamp> &currGAL =
                                (pass ? currNegativeGAL : currPositiveGAL);

                            list<Literal *> &actionEffectsList =
                                (pass ? (*actionsToStartNegativeEffects)[asItr->first] :
                                        (*actionsToStartEffects)[asItr->first]);
                            ...
                            {
                                map<int, EpsilonResolutionTimestamp>::iterator cgItr =
                                    currGAL.find((*aelItr)->getStateID());
                                if (cgItr != currGAL.end())
                                {
                                    if (tilR > cgItr->second)
                                    {
                                        tilR = cgItr->second;
                                    }
                                    currGAL.erase(cgItr);
                        ...
                        {
                            list<Literal *> &actionPreconditionlist =
                                (*actionsToProcessedStartPreconditions)[asItr->first];
                            ...
                            for (; aplItr != aplEnd; ++aplItr)
                            {
                                ...
                                if (acIn > EpsilonResolutionTimestamp::zero())
                                {
                                    if (acBy.first != -1)
                                    {
                                        ...
                                        {
                                            cout << "required at time " << acIn
                                                 << " to support current time of " << currTS << "\n";
                                        }
                                        assert(acIn <= currTS);
                                        isHelpful = false;
                                    }
                            ...
                            if (evaluateDebug)
                            {
                                cout << "\t\tPreconditions done.\n";
                            }
                        }

                        if (!ignoreNumbers)
                        {
                            list<int> &actionPreconditionlist =
                                (*actionsToProcessedStartNumericPreconditions)[asItr->first];
                            list<int>::iterator aplItr = actionPreconditionlist.begin();
                            const list<int>::iterator aplEnd = actionPreconditionlist.end();

                            if (evaluateDebug)
                            {
                                cout << "\t\tNumeric preconditions:\n";
                            }
                            ...
                            for (; aplItr != aplEnd; ++aplItr)
                            {
                                ...
                                const EpsilonResolutionTimestamp &acIn = (*numericAchievedInLayer)[currPrec];
                                if (acIn > EpsilonResolutionTimestamp::zero() &&
                                    !numericIsTrueInState[currPrec])
                                {
                                    if (evaluateDebug)
                                    {
                                        cout << "\t\tAdding requirement for numeric precondition " << currPrec
                                             << ", " << RPGBuilder::getNumericPreTable()[currPrec]
                                             << " at time " << acIn << "\n";
                                    }
                                ...
                                else
                                {
                                    if (evaluateDebug)
                                    {
                                        cout << "\t\tNumeric precondition " << currPrec << ", "
                                             << RPGBuilder::getNumericPreTable()[currPrec]
                                             << ", satisfied in initial state\n";
                                    }
                                }
                        ...
                        addExtraPreconditionsForPreferences(payload, asItr->first, Planner::E_AT_START,
                                                            currTS, tilR, goalsAtLayer);

                        if (isHelpful)
                        {
                            payload->helpfulActions.push_front(
                                ActionSegment(RPGBuilder::getInstantiatedOp(asItr->first),
                                              Planner::E_AT_START, -1, emptyIntSet));
                            if (evaluateDebug)
                            {
                                cout << "\t\tIs a helpful action\n";
                            }
                        }
...

                {
                    ...
                    for (; asItr != asEnd; ++asItr)
                    {
                        if (evaluateDebug)
                        {
                            cout << "\t\tAdding end of " << asItr->first << "\n";
                        }
                        ...
                        if (!TemporalAnalysis::canSkipToEnd(asItr->first))
                        {
                            ...
                            const ActionSegment theOp(getOp(asItr->first), Planner::E_AT_END, -1,
                                                      emptyIntList);
                        ...
                        for (int pass = 0; pass < 2; ++pass)
                        {
                            ...
                            list<Literal *> &actionEffectsList =
                                (pass ? (*actionsToEndNegativeEffects)[asItr->first] :
                                        (*actionsToEndEffects)[asItr->first]);
                            list<Literal *>::iterator aelItr = actionEffectsList.begin();
                            const list<Literal *>::iterator aelEnd = actionEffectsList.end();
                        ...
                        {
                            list<Literal *> &actionPreconditionlist =
                                (*actionsToEndPreconditions)[asItr->first];
                            list<Literal *>::iterator aplItr = actionPreconditionlist.begin();
                            const list<Literal *>::iterator aplEnd = actionPreconditionlist.end();
                            ...
                            for (; aplItr != aplEnd; ++aplItr)
                            {
                                ...
                                if (acIn > EpsilonResolutionTimestamp::zero())
                                {
                                    if (acBy.first != -1)
                                    {
                                        map<int, EpsilonResolutionTimestamp>::iterator galItr =
                                            goalsAtLayer[acIn]
                                                .propositionalGoals
                                                .insert(pair<int, EpsilonResolutionTimestamp>(currPrec, tilR))
                                                .first;
                                        if (galItr->second > tilR)
                                        {
                                            galItr->second = tilR;
                                        }
                                        isHelpful = false;
                        ...
                        {
                            list<int> &actionPreconditionlist =
                                (*actionsToNumericEndPreconditions)[asItr->first];
                            list<int>::iterator aplItr = actionPreconditionlist.begin();
                            const list<int>::iterator aplEnd = actionPreconditionlist.end();
                        ...
                        addExtraPreconditionsForPreferences(payload, asItr->first, Planner::E_AT_END, currTS,
                                                            tilR, goalsAtLayer);
                        ...
                        if (isHelpful)
                        {
                            if (payload->startState.startedActions.find(asItr->first) !=
                                payload->startState.startedActions.end())
                            {
                                payload->helpfulActions.push_front(
                                    ActionSegment(RPGBuilder::getInstantiatedOp(asItr->first),
                                                  Planner::E_AT_END, -1, emptyIntSet));
                                if (evaluateDebug)
                                {
                                    cout << "\t\tIs a helpful action\n";
                                }
                ...
                if (!ignoreNumbers)
                {
                    ...
                    list<pair<EpsilonResolutionTimestamp, SupportingAction>> actionsUsed;
                    payload->fluentLayers.satisfyNumericPreconditionsAtLayer(thisLayer, goalsAtLayer,
                                                                             actionsUsed);
                    ...
                    if (!actionsUsed.empty())
                    {
                        ...
                        if (evaluateDebug)
                        {
                            cout << "Adding achievers for numeric goals at TS " << currTS << "\n";
                        }

                        list<pair<EpsilonResolutionTimestamp, SupportingAction>>::const_iterator sapItr =
                            actionsUsed.begin();
                        const list<pair<EpsilonResolutionTimestamp, SupportingAction>>::const_iterator
                            sapEnd = actionsUsed.end();
                        ...
                        for (; sapItr != sapEnd; ++sapItr)
                        {
                            ...
                            if (sapItr->first < currTS)
                            {
                                ...
                                {
                                    map<int, pair<int, EpsilonResolutionTimestamp>>::iterator galItr =
                                        goalsAtLayer[sapItr->first]
                                            .actionStarts
                                            .insert(pair<int, pair<int, EpsilonResolutionTimestamp>>(
                                                saItr->actID, make_pair(saItr->howManyTimes, saItr->tilR)))
                                            .first;
                                    if (galItr->second.second > saItr->tilR)
                                    {
                                        galItr->second.second = saItr->tilR;
                                    }
                                    if (evaluateDebug)
                                    {
                                        cout << "\t\tAdding requirement for start of action at time "
                                             << sapItr->first << "\n";
                                    }
                                }
                                else
                                {
                                    cerr << "Adding actions other than EPSILON earlier (in this case "
                                         << currTS << " - " << sapItr->first
                                         << ") should only occur for gradient actions\n";
                                    exit(1);
                                }
                            ...
                            if (currAchievedBy.second == Planner::E_AT_START)
                            {
                                if (evaluateDebug)
                                {
                                    cout << "\t\tUsing start of " << currAchievedBy.first << " - "
                                         << *(RPGBuilder::getInstantiatedOp(currAchievedBy.first));
                                ...
                                if (evaluateDebug && currTS == EpsilonResolutionTimestamp::epsilon())
                                {
                                    cout << "\t\tIs a helpful action\n";
                                }
                                relaxedPlan.front().second.insert(
                                    relaxedPlan.front().second.end(), saItr->howManyTimes,
                                    ActionSegment(getOp(currAchievedBy.first), Planner::E_AT_START, -1,
                                                  emptyIntList));
                                ...
                                const EpsilonResolutionTimestamp sTIL =
                                    earliestTILForAction(currAchievedBy.first, true);
                                ...
                                for (int pass = 0; pass < 2; ++pass)
                                {
                                    ...
                                    list<Literal *> &actionEffectsList =
                                        (pass ? (*actionsToStartNegativeEffects)[currAchievedBy.first] :
                                                (*actionsToStartEffects)[currAchievedBy.first]);
                                    list<Literal *>::iterator aelItr = actionEffectsList.begin();
                                    const list<Literal *>::iterator aelEnd = actionEffectsList.end();
                                ...
                                {
                                    list<Literal *> &actionPreconditionlist =
                                        (*actionsToProcessedStartPreconditions)[currAchievedBy.first];
                                    list<Literal *>::iterator aplItr = actionPreconditionlist.begin();
                                    const list<Literal *>::iterator aplEnd = actionPreconditionlist.end();
                                    if (evaluateDebug)
                                    {
                                    ...
                                    for (; aplItr != aplEnd; ++aplItr)
                                    {
                                        ...
                                        EpsilonResolutionTimestamp acIn(
                                            EpsilonResolutionTimestamp::undefined());
                                        pair<int, Planner::time_spec> acBy;
                                        ...
                                        getAchieverDetails(currPrec,
                                                           currTS - EpsilonResolutionTimestamp::epsilon(),
                                                           acIn, acBy);
                                        ...
                                        if (acIn > EpsilonResolutionTimestamp::zero())
                                        {
                                            if (acBy.first != -1)
                                            {
                                                map<int, EpsilonResolutionTimestamp>::iterator galItr =
                                                    goalsAtLayer[acIn]
                                                        .propositionalGoals
                                                        .insert(pair<int, EpsilonResolutionTimestamp>(
                                                            currPrec, tilR))
                                                        .first;
                                                if (galItr->second > tilR)
                                                {
                                            ...
                                            else
                                            {
                                                if (evaluateDebug)
                                                {
                                                    cout << "in initial state at time " << acIn << "\n";
                                                }
                                            }
                                        }
                                        else
                                ...
                                {
                                    list<int> &actionPreconditionlist =
                                        (*actionsToProcessedStartNumericPreconditions)[currAchievedBy.first];
                                    list<int>::iterator aplItr = actionPreconditionlist.begin();
                                    const list<int>::iterator aplEnd = actionPreconditionlist.end();
                                    ...
                                    {
                                        const int currPrec = (*aplItr);
                                        const EpsilonResolutionTimestamp &acIn =
                                            (*numericAchievedInLayer)[currPrec];
                                        if (acIn > EpsilonResolutionTimestamp::zero() &&
                                            !numericIsTrueInState[currPrec])
                                        {
                                            if (evaluateDebug)
                                            {
                                                cout << "\t\tAdding requirement for numeric precondition "
                                                     << currPrec << ", "
                                                     << RPGBuilder::getNumericPreTable()[currPrec]
                                                     << " at time " << acIn << "\n";
                                            }
                                            payload->fluentLayers.requestNumericPrecondition(
                                                currPrec, acIn, currTS, goalsAtLayer, tilR);
                                            isHelpful = false;
                                        }
                                        else
                                        {
                                ...
                                addExtraPreconditionsForPreferences(
                                    payload, currAchievedBy.first, Planner::E_AT_START,
                                    currTS - EpsilonResolutionTimestamp::epsilon(), tilR, goalsAtLayer);

                                if (isHelpful)
                                {
                                    payload->helpfulActions.push_front(
                                        ActionSegment(RPGBuilder::getInstantiatedOp(currAchievedBy.first),
                                                      Planner::E_AT_START, -1, emptyIntSet));
                                }

                                EpsilonResolutionTimestamp &oldR =
                                    earliestDeadlineRelevancyStart[currAchievedBy.first];
                                if (oldR > tilR)
                                {
                                    oldR = tilR;
                                }
                            ...
                            else
                            {
                                if (evaluateDebug)
                                {
                                    cout << "\t\tUsing end of " << currAchievedBy.first << " - "
                                         << *(RPGBuilder::getInstantiatedOp(currAchievedBy.first)) << endl;
                                }
                                if (evaluateDebug && currTS == EpsilonResolutionTimestamp::epsilon())
                                {
                                    cout << "\t\tIs a helpful action\n";
                                }
                                relaxedPlan.front().second.push_back(ActionSegment(
                                    getOp(currAchievedBy.first), Planner::E_AT_END, -1, emptyIntList));
                                //                  if (currTS == EPSILON) {
                                //                      helpfulActions.push_back(pair<int,
                                //                      Planner::time_spec>(currAchievedBy->act,
                                //                      Planner::E_AT_END));
                                //                  }
                                h += saItr->howManyTimes;
                                ...
                                const EpsilonResolutionTimestamp sTIL =
                                    earliestTILForAction(currAchievedBy.first, false);
                                EpsilonResolutionTimestamp tilR = saItr->tilR;
                                if (tilR > sTIL)
                                {
                                    tilR = sTIL;
                                ...
                                for (int pass = 0; pass < 2; ++pass)
                                {
                                    ...
                                    list<Literal *> &actionEffectsList =
                                        (pass ? (*actionsToEndNegativeEffects)[currAchievedBy.first] :
                                                (*actionsToEndEffects)[currAchievedBy.first]);
                                    list<Literal *>::iterator aelItr = actionEffectsList.begin();
                                    const list<Literal *>::iterator aelEnd = actionEffectsList.end();
                                ...
                                {
                                    list<Literal *> &actionPreconditionlist =
                                        (*actionsToEndPreconditions)[currAchievedBy.first];
                                    list<Literal *>::iterator aplItr = actionPreconditionlist.begin();
                                    const list<Literal *>::iterator aplEnd = actionPreconditionlist.end();
                                    ...
                                    for (; aplItr != aplEnd; ++aplItr)
                                    {
                                        ...
                                        if (acIn > EpsilonResolutionTimestamp::zero())
                                        {
                                            if (acBy.first != -1)
                                            {
                                                ...
                                                if (evaluateDebug)
                                                {
                                                    cout << "\t\tAdding requirement for " << currPrec << ", "
                                                         << RPGBuilder::getNumericPreTable()[currPrec]
                                                         << " at time " << acIn << "\n";
                                                }
                                                assert(acIn < currTS);
                                    ...
                                    {
                                        map<int, set<int>>::const_iterator saItr =
                                            payload->startState.startedActions.find(currAchievedBy.first);
                                    ...
                                    {
                                        map<int, pair<int, EpsilonResolutionTimestamp>>::iterator galItr =
                                            goalsAtLayer[startActionSchedule[currAchievedBy.first] +
                                                         EpsilonResolutionTimestamp::epsilon()]
                                                .actionStarts
                                                .insert(pair<int, pair<int, EpsilonResolutionTimestamp>>(
                                                    currAchievedBy.first, make_pair(addToThePast, tilR)))
                                                .first;
                                        if (galItr->second.second > tilR)
                                        {
                                            galItr->second.second = tilR;
                                        }
                                        if (evaluateDebug)
                                        {
                                            cout << "\t\tAdding requirement for start of action at time "
                                                 << (startActionSchedule[currAchievedBy.first] +
                                                     EpsilonResolutionTimestamp::epsilon())
                                                 << "\n";
                                        }
                                        isHelpful = false;
                                    ...
                                    else
                                    {
                                        if (evaluateDebug)
                                        {
                                            cout << "\t\tAction has already been started, do not need to "
                                                    "schedule start in RPG\n";
                                        }
                                    }
                                }
                                ...
                                {
                                    list<int> &actionPreconditionlist =
                                        (*actionsToNumericEndPreconditions)[currAchievedBy.first];
                                    list<int>::iterator aplItr = actionPreconditionlist.begin();
                                    const list<int>::iterator aplEnd = actionPreconditionlist.end();
                                ...
                                addExtraPreconditionsForPreferences(
                                    payload, currAchievedBy.first, Planner::E_AT_END,
                                    currTS - EpsilonResolutionTimestamp::epsilon(), tilR, goalsAtLayer);
                                ...
                                EpsilonResolutionTimestamp &oldR =
                                    earliestDeadlineRelevancyEnd[currAchievedBy.first];
                                if (oldR > tilR)
                                {
                                    oldR = tilR;
                                }
                ...
                {
                    map<int, EpsilonResolutionTimestamp> &currSupportingGAL =
                        (galPass ? currNegativeGAL : currPositiveGAL);
                    ...
                    if (evaluateDebug && !currSupportingGAL.empty())
                    {
                        if (galPass)
                        {
                            cout << "Finding achievers for " << currSupportingGAL.size()
                                 << " negative goal(s) at TS " << currTS << "\n";
                        }
                        else
                        {
                            cout << "Finding achievers for " << currSupportingGAL.size() << " goal(s) at TS "
                                 << currTS << "\n";
                        }
                    }
                    ...
                    while (!currSupportingGAL.empty())
                    {
                        ...
                        if (evaluateDebug)
                        {
                            cout << "\tGoal " << nextToAchieve << ", "
                                 << *(RPGBuilder::getLiteral(nextToAchieve)) << "\n";
                        }
                        ...
                        {
                            getAchieverDetails(nextToAchieve,
                                               currTS /* + EpsilonResolutionTimestamp::epsilon()*/,
                                               layerIgnore, currAchievedBy);
                        ...
                        if (currAchievedBy.second == Planner::E_AT_START)
                        {
                            ...
                            {
                                cout << "\t\tUsing start of " << currAchievedBy.first << " - "
                                     << *(RPGBuilder::getInstantiatedOp(currAchievedBy.first)) << " at "
                                     << layerIgnore << "\n";
                            }
                            if (evaluateDebug && currTS == EpsilonResolutionTimestamp::epsilon())
                            {
                            ...
                            relaxedPlan.front().second.push_back(ActionSegment(
                                getOp(currAchievedBy.first), Planner::E_AT_START, -1, emptyIntList));
                            ++h;

                            if (RPGHeuristic::printRPGAsDot)
                            {
                                payload->dot.highlightAction(currAchievedBy.first, Planner::E_AT_START);
                                payload->dot.actionMeetsFactInRP(currAchievedBy.first, Planner::E_AT_START,
                                                                 nextToAchieve);
                            }
...

                            {
                                ...
                                for (; aplItr != aplEnd; ++aplItr)
                                {
                                    ...
                                    getAchieverDetails(
                                        currPrec, currTS - EpsilonResolutionTimestamp::epsilon(), acIn, acBy);
                                    ...
                                    if (acIn > EpsilonResolutionTimestamp::zero())
                                    {
                                        if (acBy.first != -1)
                                        {
                                            map<int, EpsilonResolutionTimestamp>::iterator galItr =
                                                goalsAtLayer[acIn]
                                                    .propositionalGoals
                                                    .insert(
                                                        pair<int, EpsilonResolutionTimestamp>(currPrec, tilR))
                                                    .first;
                                            if (galItr->second > tilR)
                                            {
                                                galItr->second = tilR;
                                            ...
                                            if (evaluateDebug)
                                            {
                                                cout << "adding requirement at time " << acIn << "\n";
                                            }
                                            assert(acIn < currTS);
                                            isHelpful = false;
                                        }
                                        else
                                        {
                                            if (evaluateDebug)
                                            {
                                                cout << "initial state at time " << acIn << "\n";
                                            }
                                        }
                                    }
                                    else
                            ...
                            if (!ignoreNumbers)
                            {
                                ...
                                for (; aplItr != aplEnd; ++aplItr)
                                {
                                        ...
                                        !numericIsTrueInState[currPrec])
                                    {
                                        ...
                                        payload->fluentLayers.requestNumericPrecondition(
                                            currPrec, acIn, currTS - EpsilonResolutionTimestamp::epsilon(),
                                            goalsAtLayer, tilR);
                                        isHelpful = false;
                                    }
                                    else
                                ...
                                payload->fluentLayers.recordSideEffects(
                                    currAchievedBy.first, currAchievedBy.second,
                                    currTS - EpsilonResolutionTimestamp::epsilon());
                            }
                        ...
                        else if (currAchievedBy.second == Planner::E_AT_END)
                        {
                            ...
                            {
                                payload->dot.highlightAction(currAchievedBy.first, Planner::E_AT_END);
                                payload->dot.actionMeetsFactInRP(currAchievedBy.first, Planner::E_AT_END,
                                                                 nextToAchieve);
                            }
                            ...
                            bool isHelpful = true;
                            {
                                ...
                                for (; aplItr != aplEnd; ++aplItr)
                                {
                                    ...
                                    if (acIn > EpsilonResolutionTimestamp::zero())
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                cout << "\t\tPrecondition " << currPrec
                                                     << " in initial state at layer " << acIn << "\n";
                                            }
                                        }
                                    }
                                    else
                                    {
                                        if (evaluateDebug)
                                        {
                                            cout << "\t\tPrecondition " << currPrec << " in initial state\n";
                                        }
                                    }
                                }
                                ...
                                if (theState.startedActions.find(currAchievedBy.first) ==
                                    theState.startedActions.end())
                                {
                                    const EpsilonResolutionTimestamp sas =
                                        startActionSchedule[currAchievedBy.first];
                                    if (sas.isUndefined())
                                    {
                                        ...
                                        if (!TemporalAnalysis::canSkipToEnd(currAchievedBy.first))
                                        {
                                            cerr << "Critical error - trying to schedule goal to before "
                                                    "start of plan\n";
                                            exit(1);
                                        }
                                    }
                                    ...
                                    {
                                        map<int, pair<int, EpsilonResolutionTimestamp>>::iterator galItr =
                                            goalsAtLayer[sas + EpsilonResolutionTimestamp::epsilon()]
                                                .actionStarts
                                                .insert(pair<int, pair<int, EpsilonResolutionTimestamp>>(
                                                    currAchievedBy.first,
                                                    pair<int, EpsilonResolutionTimestamp>(1, tilR)))
                                                .first;
                                        if (galItr->second.second > tilR)
                                        {
                                            galItr->second.second = tilR;
                        ...
                        {  // aha, is a timed initial literal!
                            ...
                            // relaxedPlan.front().second.push_back(ActionSegment(0, Planner::E_AT,
                            // currAchievedBy.first, emptyIntList));
                            ++h;

                            for (int pass = galPass; pass < 2; ++pass)
                            {
                                const list<int> &tilEffectsList =
                                    (pass ? tilTemporaryNegativeEffects[currAchievedBy.first] :
                                            tilEffects[currAchievedBy.first]);
                                map<int, EpsilonResolutionTimestamp> &currGAL =
                                    (pass ? currNegativeGAL : currPositiveGAL);
                            ...
                            addExtraPreconditionsForPreferences(
                                payload, currAchievedBy.first, Planner::E_AT,
                                currTS - EpsilonResolutionTimestamp::epsilon(), tilR, goalsAtLayer);
                        }
                    }
                }
                if (evaluateDebug)
                {
                    cout << "All goals at this TS now satisfied\n";
                }
         ...
         * @param currTS   The time specifier of the action: <code>Planner::E_AT_START</code> or
         * <code>Planner::E_AT_END</code>
         ...
         *
         * @return <code>true</code> if adding the action led to all goals being achieved; <code>false</code>
         * otherwise.
         */
        bool applyNumericEffects(Private::BuildingPayload *const payload, const int &currAct,
                                 const Planner::time_spec &currTS, const EpsilonResolutionTimestamp &nlTime,
                                 const int &limitTo);
         ...
         * otherwise.
         */
        bool applyPropositionalEffects(Private::BuildingPayload *const payload, const int &currAct,
                                       const Planner::time_spec &currTS, const ActionViolationData &prefCosts,
                                       const bool &openEnd,
                                       const EpsilonResolutionTimestamp &nlTime);  //, MaxDependentInfo &
                                                                                   // POtime);

        bool checkPreconditionsAreSatisfied(const int &currAct, const Planner::time_spec &ts,
                                            const EpsilonResolutionTimestamp &layer);

        bool updateActionsForNewLiteralFact(BuildingPayload *const payload, const int &toPropagate,
                                            const EpsilonResolutionTimestamp &factLayerTime,
                                            const bool &isActuallyNew);
        bool updateActionsForNewNumericFact(BuildingPayload *const payload, const int &toPropagate,
                                            const EpsilonResolutionTimestamp &factLayerTime);
        bool applyEndEffectNow(BuildingPayload *const payload, const int &currAct, const bool &openAct,
                               const EpsilonResolutionTimestamp &factLayerTime, const bool &isActuallyNew);

        bool revisitActs(BuildingPayload *payload, const EpsilonResolutionTimestamp &factLayerTime,
                         const list<pair<int, Planner::time_spec>> &actsToVisit);
        bool updatePreferencesForFact(BuildingPayload *const payload, const int &fID, const bool &isALiteral,
                                      const bool &polarity, const bool &isActuallyNew,
                                      const EpsilonResolutionTimestamp &factLayerTime);
        bool updateForPreferencesInBetterPositions(
            BuildingPayload *const payload, const list<pair<int, int>> &preferencePairedWithFactNowFreeToAdd,
            const EpsilonResolutionTimestamp &factLayerTime);

#ifdef POPF3ANALYSIS
        void calculateGoalCost(BuildingPayload *const payload, double *calculatedCost = 0);
        void updateAdditiveCosts(BuildingPayload *const payload, const int &ci,
                                 const EpsilonResolutionTimestamp &deadline, const int &updateForGoal,
                                 double &costToUpdate);
#endif

        bool startShouldBeDelayed(const int &currAct, const EpsilonResolutionTimestamp &factLayerTime,
                                  BuildingPayload *const payload);
    };
    ...
    void RPGHeuristic::doFullExpansion(MinimalState &refState, const list<FFEvent> &dummyPlan)
    {
        ...
        delete getRelaxedPlan(refState, 0, minTimestamps, 0.0, -DBL_MAX, refState.secondMin,
                              refState.secondMax, timeAtWhichValueIsDefined, dummyHelpful, dummyRP,
                              dummyEstimate);
    ...
    RPGHeuristic::RPGHeuristic(
        const bool &b, vector<list<Literal *>> *atse, vector<list<Literal *>> *atee,
        vector<list<pair<int, Planner::time_spec>>> *eta, vector<list<Literal *>> *atsne,
        vector<list<Literal *>> *atene, vector<list<pair<int, Planner::time_spec>>> *neta,
        vector<list<pair<int, Planner::time_spec>>> *pta, vector<list<Literal *>> *atsp,
        vector<list<Literal *>> *ati, vector<list<Literal *>> *atep,
        vector<list<RPGBuilder::NumericEffect>> *atnuse, vector<list<RPGBuilder::NumericEffect>> *atnuee,
        vector<list<int>> *atrnuse, vector<list<int>> *atrnuee, vector<list<int>> *atnusp,
        vector<list<int>> *atnui, vector<list<int>> *atnuep, vector<list<int>> *atpnuep, vector<int> *iusp,
        vector<int> *iuip, vector<int> *iuep, vector<EpsilonResolutionTimestamp> *ail,
        vector<EpsilonResolutionTimestamp> *ailr, vector<pair<int, Planner::time_spec>> *ab,
        vector<pair<int, Planner::time_spec>> *abr, vector<EpsilonResolutionTimestamp> *negail,
        vector<EpsilonResolutionTimestamp> *negailr, vector<pair<int, Planner::time_spec>> *negab,
        vector<pair<int, Planner::time_spec>> *negabr, vector<EpsilonResolutionTimestamp> *nail,
        vector<EpsilonResolutionTimestamp> *nailr, vector<ActionFluentModification *> *nab,
        vector<ActionFluentModification *> *nabr, vector<int> *iunsp, vector<int> *iuni, vector<int> *iunep,
        vector<RPGBuilder::RPGNumericPrecondition> *rnp, vector<RPGBuilder::RPGNumericEffect> *rne,
        vector<list<pair<int, Planner::time_spec>>> *ppta, vector<list<pair<int, Planner::time_spec>>> *nppta,
        vector<list<Literal *>> *atpsp, vector<int> *iupsp, vector<int> *iupsnp,
        list<pair<int, Planner::time_spec>> *pla, list<pair<int, Planner::time_spec>> *onpa)
      : d(new Private(b, atse, atee, eta, atsne, atene, neta, pta, atsp, ati, atep, atnuse, atnuee, atrnuse,
                      atrnuee, atnusp, atnui, atnuep, atpnuep, iusp, iuip, iuep, ail, ailr, ab, abr, negail,
                      negailr, negab, negabr, nail, nailr, nab, nabr, iunsp, iuni, iunep, rnp, rne, ppta,
                      nppta, atpsp, iupsp, iupsnp, pla, onpa)){
...
RPGHeuristic *RPGBuilder::generateRPGHeuristic()
{
    ...
    return new RPGHeuristic(
        false,  // subproblem does not own the arrays
        &actionsToStartEffects, &actionsToEndEffects, &effectsToActions, &actionsToStartNegativeEffects,
        &actionsToEndNegativeEffects, &negativeEffectsToActions, &preconditionsToActions,
        &actionsToStartPreconditions, &actionsToInvariants, &actionsToEndPreconditions,
        &actionsToStartNumericEffects, &actionsToEndNumericEffects, &actionsToRPGNumericStartEffects,
        &actionsToRPGNumericEndEffects, &actionsToRPGNumericStartPreconditions,
        &actionsToRPGNumericInvariants, &actionsToRPGNumericEndPreconditions,
        &actionsToProcessedStartRPGNumericPreconditions, &initialUnsatisfiedStartPreconditions,
        &initialUnsatisfiedInvariants, &initialUnsatisfiedEndPreconditions, &achievedInLayer,
        &achievedInLayerReset, &achievedBy, &achievedByReset, &negativeAchievedInLayer,
        &negativeAchievedInLayerReset, &negativeAchievedBy, &negativeAchievedByReset, &numericAchievedInLayer,
        &numericAchievedInLayerReset, &numericAchievedBy, &numericAchievedByReset,
        &initialUnsatisfiedNumericStartPreconditions, &initialUnsatisfiedNumericInvariants,
        &initialUnsatisfiedNumericEndPreconditions, &rpgNumericPreconditions, &rpgNumericEffects,
        &processedPreconditionsToActions, &processedRPGNumericPreconditionsToActions,
        &actionsToProcessedStartPreconditions, &initialUnsatisfiedProcessedStartPreconditions,
        &initialUnsatisfiedProcessedStartNumericPreconditions, &preconditionlessActions,
...
RPGHeuristic::EvaluationInfo *RPGHeuristic::getRelaxedPlan(
    MinimalState &theState, const list<StartEvent> *startEventQueue, const vector<double> &minTimestamps,
    const double &stateTS, const double &costLimit, const vector<double> &extrapolatedMin,
    const vector<double> &extrapolatedMax, const vector<double> &timeAtWhichValueIsDefined,
    list<ActionSegment> &helpfulActions, list<pair<double, list<ActionSegment>>> &relaxedPlan,
    double &finalPlanMakespanEstimate, map<double, list<pair<int, int>>> *justApplied, double tilFrom)
{
    ...
    if (evaluateDebug)
    {
        cout << "Evaluating a state with " << theState.startedActions.size()
             << " sorts of actions on the go\n";
    }

    // rpprintState(theState);

    auto_ptr<Private::BuildingPayload> payload(
        d->spawnNewPayload(theState, costLimit, startEventQueue, minTimestamps, stateTS, helpfulActions));
    ...
    if (RPGBuilder::getMetric())
    {
        if (RPGBuilder::getMetric()->minimise)
        {
            if (currentPreferenceCost - reachablePreferenceCost >= 0.0001)
            {
                if (evaluateDebug)
                {
                    cout << "-- Adding dummy goal to try to reach the theoretical minimum preference "
                            "violation penalty ("
                         << reachablePreferenceCost << "\n";
                }
                ++payload->unsatisfiedGoals;
            ...
            else
            {
                if (evaluateDebug)
                {
                    cout << "-- Already at the reachable preference cost of " << reachablePreferenceCost
                         << endl;
                }
            }
        }
        else
        {
            if (currentPreferenceCost - reachablePreferenceCost <= 0.0001)
            {
                if (evaluateDebug)
                {
                    cout << "-- Adding dummy goal to try to reach the theoretical maximum preference "
                            "violation penalty ("
                         << reachablePreferenceCost << "\n";
                }
                ++payload->unsatisfiedGoals;
    ...
    if (d->expandFully)
    {
        payload->unsatisfiedGoals = INT_MAX;
    }
    ...
    if ((!payload->unsatisfiedGoals && !payload->unappearedEnds) || blindSearch)
    {
        if (evaluateDebug)
        {
            ...
            {
                cout << "Current cost of state is " << currentPreferenceCost << ", reachable cost is "
                     << reachablePreferenceCost << ", so returning\n";
            }
        }
        ...
        if (RPGBuilder::getMetric() && Globals::optimiseSolutionQuality)
        {
            ...
            {
                double admissibleCost =
                    reachablePreferenceCost /* + definiteWithinCost*/ + RPGBuilder::getMetric()->constant;
                ...
                for (; wItr != wEnd; ++wItr, ++vItr)
                {
                    if (*vItr < 0)
                    {
                        if (*wItr < 0.0)
                        {
                            ...
                            return new EvaluationInfo(fakeH, std::numeric_limits<double>::signaling_NaN(),
                                                      !payload->unsatisfiedGoals && !payload->unappearedEnds);
                        }
                        else
                        {
                ...
                return new EvaluationInfo(fakeH, admissibleCost,
                                          !payload->unsatisfiedGoals && !payload->unappearedEnds);
            }
            else
            {
        ...
        return new EvaluationInfo(fakeH, reachablePreferenceCost,
                                  !payload->unsatisfiedGoals && !payload->unappearedEnds);
    }
    ...
    if (!d->addTemporalConstraintsFromActiveActions(
            payload.get(), justApplied, EpsilonResolutionTimestamp(stateTS, true), nextTIL, tilFrom))
    {
    ...
    d->addTILsBeforeExpansion(payload.get(), nextTIL, payload->factLayers, payload->startState,
                              EpsilonResolutionTimestamp(stateTS, true), tilFrom);
...

    {
        if (evaluateDebug)
        {
            cout << "Considering preconditionless actions\n";
        }
    ...
    if (payload->unsatisfiedGoals || payload->unappearedEnds)
    {
        if (!RPGBuilder::modifiedRPG && RPGBuilder::sortedExpansion)
        {
            ...
            for (; eoItr != eoEnd; ++eoItr)
            {
                ...
                for (; fItr != fEnd; ++fItr)
                {
                    if (d->updateActionsForNewLiteralFact(payload.get(), *fItr,
                                                          EpsilonResolutionTimestamp::zero(), true))
                    {
                        break;
                    }
                }
        ...
        else
        {
            if (evaluateDebug)
            {
                cout << "Considering " << theState.first.size() << " initial propositional facts\n";
            }
            ...
            if (!RPGBuilder::getPreferences().empty())
            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    ...
                    if (TemporalAnalysis::getFactIsAbstract()[factID])
                    {
                        if (evaluateDebug)
                        {
                            cout << "Not yet updating preferences from abstract fact " << factID << ", "
                                 << *(RPGBuilder::getLiteral(factID)) << endl;
                        }
                        continue;
                    }
#ifdef POPF3ANALYSIS
                    ...
                    if (layer.isZero())
                    {
                        if (evaluateDebug)
                        {
                            cout << "Updating preferences from fact " << factID << ", "
                                 << *(RPGBuilder::getLiteral(factID)) << endl;
                        }
                        if (d->updatePreferencesForFact(payload.get(), factID, true, true, true, layer))
                        {
                            break;
                        }
                    }
                    else
                    {
                        if (evaluateDebug)
                        {
                            cout << "Modified RPG: Not updating preferences from fact " << factID << " until "
                                 << layer << "\n";
                        }
                    }
                }
...
// const double poTS = (RPGBuilder::modifiedRPG ? 0.0 : d->earliestPropositionPOTimes[factID]);
                    ...
                    if (layer.isZero())
                    {
                        ...
                        if (evaluateDebug)
                        {
                            cout << "Updating from fact " << factID << " "
                                 << *(RPGBuilder::getLiteral(factID)) << "\n";
                        }
                        if (d->updateActionsForNewLiteralFact(payload.get(), factID, layer, true))
                        {
                            break;
                        }
                    }
                    else
                    {
                        if (evaluateDebug)
                        {
                            cout << "Modified RPG: Not updating from fact " << factID << " until " << layer
                                 << "\n";
                        }
    ...
    if (payload->unsatisfiedGoals || payload->unappearedEnds)
    {
        ...
        if (loopLim)
        {
            if (evaluateDebug)
            {
                cout << "Considering initial numeric facts\n";
            }
                ...
                for (int i = 0; i < loopLim; ++i)
                {
                    ...
                    if (ignoreNumbers || currPre.isSatisfied(*maxFluentTable))
                    {
                        ...
                        if (layer.isZero())
                        {
                            if (evaluateDebug)
                            {
                                cout << "Updating preferences from numeric fact " << i << ":  "
                                     << (*(d->rpgNumericPreconditions))[i] << endl;
                            }
                        ...
                        else
                        {
                            if (evaluateDebug)
                            {
                                cout << "Not updating preferences from numeric fact " << i << ":  "
                                     << (*(d->rpgNumericPreconditions))[i] << " until " << layer << "\n";
                            }
                        }
                    }
                }
                ...
                for (int i = 0; i < loopLim; ++i)
                {
                    ...
                    if (ignoreNumbers || currPre.isSatisfied(*maxFluentTable))
                    {
                        ...
                        if (layer.isZero())
                        {
                            ...
                            if (evaluateDebug)
                            {
                                cout << "Updating from numeric fact " << i << ":  "
                                     << (*(d->rpgNumericPreconditions))[i];
                                cout << "; was achieved in the initial state\n";
                            }
                            if (d->updateActionsForNewNumericFact(payload.get(), i,
                                                                  EpsilonResolutionTimestamp::zero()))
                            {
                                break;
                            }
                        ...
                        else
                        {
                            if (evaluateDebug)
                            {
                                cout << "Modified RPG: Not updating from numeric fact " << i << ":  "
                                     << (*(d->rpgNumericPreconditions))[i] << " until " << layer << "\n";
                            }
                            if (printRPGAsDot)
                            {
...

        {
            ...
            for (; procItr != procEnd; ++procItr)
            {
                ...
                for (int e = 0; e < effCount; ++e)
                {
                    DelayedGradientDescriptor proc(currAct, Planner::E_AT_START, DBL_MAX,
                                                   make_pair(varList[e], changeList[e].constant));
                    payload->fluentLayers.recordGradientNumericEffect(
                        proc, EpsilonResolutionTimestamp::epsilon(), proc.gradientEffect,
                        payload->factLayers);
                }
            }
        }
        ...
        payload->fluentLayers.applyRecentlyRecordedEffects(*(d->numericAchievedInLayer),
                                                           newNumericPreconditions, d->maxNeeded);
        ...
        if (!newNumericPreconditions.empty())
        {
            ...
            if (payload->unsatisfiedGoals || payload->unappearedEnds)
            {
                if (RPGBuilder::modifiedRPG)
                {
                    ...
                    {
                        const EpsilonResolutionTimestamp earliestPOPoint =
                            earliestPointForNumericPrecondition(RPGBuilder::getNumericPreTable()[*preID],
                                                                &(d->earliestNumericPOTimes));
                        ...
                        {
                            payload->factLayers[EpsilonResolutionTimestamp::epsilon()].second.push_back(
                                *preID);
                            if (printRPGAsDot)
                            {
                                payload->dot.addNumericFactNode(
                                    EpsilonResolutionTimestamp::epsilon().toDouble(), *preID);
                            }
                        }
                    }
                }
    ...
    if (evaluateDebug)
    {
        cout << "Unsatisfied goals: " << payload->unsatisfiedGoals
             << ", Unappeared ends: " << payload->unappearedEnds << "\n";
    }

    {
        if (payload->unsatisfiedGoals && payload->unsatisfiedGoals == payload->fakeGoalCount)
        {
            ...
            const bool prefsPreviouslyRemaining =
                (RPGBuilder::getMetric()->minimise ?
    ...
    vector<EpsilonResolutionTimestamp> earliestRelevanceOfGoal(RPGBuilder::getLiteralGoals().size(),
                                                               EpsilonResolutionTimestamp::infinite());
    ...
    if (payload->unsatisfiedGoals || payload->unappearedEnds)
    {
#ifdef POPF3ANALYSIS
        {
            ...
            for (int fID; gItr != gEnd; ++gItr, ++gdItr, ++gID)
            {
                ...
                if (*gdItr != DBL_MAX)
                {
                    payload->factLayers[EpsilonResolutionTimestamp(*gdItr, true)]
                        .literalGoalsWeMustHaveByNow.push_back(fID);
            ...
            map<EpsilonResolutionTimestamp, list<int>>::const_iterator costItr =
                payload->becomesUnreachableAtLayer.begin();
            const map<EpsilonResolutionTimestamp, list<int>>::const_iterator costEnd =
                payload->becomesUnreachableAtLayer.end();

            for (; costItr != costEnd; ++costItr)
            {
                payload->factLayers[costItr->first].preferenceUnreachableIfNotSatisfiedByNow =
                    costItr->second;
...

        {
            ...
            for (int fID; gItr != gEnd; ++gItr, ++gdItr)
            {
                for (int ipass = 0; ipass < 2; ++ipass)
                {
                    ...
                    {
                        payload->factLayers[EpsilonResolutionTimestamp(*gdItr, true)]
                            .numericGoalsWeMustHaveByNow.push_back(fID);
    ...
    while (!nextHappening.empty())
    {
        ...
        {
            payload->fluentLayers.recordConsequencesOfActiveGradients(nextHappening.timestamp +
                                                                      EpsilonResolutionTimestamp::epsilon());
        }

        if (nextHappening.newFactsAtThisTime != payload->factLayers.end() &&
            !RPGBuilder::getPreferences().empty())
        {
            const map<EpsilonResolutionTimestamp, FactLayerEntry>::iterator currFactLayerItr =
                nextHappening.newFactsAtThisTime;
            if (evaluateDebug)
            {
                cout << "FACT LAYER UPDATING PREFERENCES AT TIME " << currFactLayerItr->first << "\n";
            }
...

            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    if (evaluateDebug)
                    {
                        cout << "Updating preferences from fact " << *stateItr << ", "
                             << *(RPGBuilder::getLiteral(*stateItr)) << endl;
                    }
                    if (d->updatePreferencesForFact(payload.get(), *stateItr, true, true, true, cTime) &&
                        breakOnGoalsAndEnds)
                    {
                        break;
                    }
...

            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    if (evaluateDebug)
                    {
                        cout << "Updating preferences from repeated fact " << *stateItr << ", "
                             << *(RPGBuilder::getLiteral(*stateItr)) << endl;
                    }
                    if (d->updatePreferencesForFact(payload.get(), *stateItr, true, true, false, cTime) &&
                        breakOnGoalsAndEnds)
                    {
                        break;
                    }
...

            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    if (evaluateDebug)
                    {
                        cout << "Updating preferences from negative fact " << *stateItr << ", "
                             << *(RPGBuilder::getLiteral(*stateItr)) << endl;
                    }
                    if (d->updatePreferencesForFact(payload.get(), *stateItr, true, false, true, cTime) &&
                        breakOnGoalsAndEnds)
                    {
                        break;
                    }
...

            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    if (evaluateDebug)
                    {
                        cout << "Updating preferences from numeric fact " << *stateItr << ":  "
                             << (*(d->rpgNumericPreconditions))[*stateItr] << endl;
                    }
                    if (d->updatePreferencesForFact(payload.get(), *stateItr, false, true, true, cTime) &&
                        breakOnGoalsAndEnds)
                    {
                        break;
                    }
        ...
        if (nextHappening.endActionsAppearingAtThisTime != payload->endActionsAtTime.end())
        {
            if (evaluateDebug)
            {
                cout << "ACTION LAYER AT TIME " << nextHappening.endActionsAppearingAtThisTime->first << "\n";
            }

            const map<EpsilonResolutionTimestamp, list<pair<int, bool>>>::iterator &eaatItr =
                nextHappening.endActionsAppearingAtThisTime;
            ...
            for (; eaItr != eaEnd; ++eaItr)
            {
                ...
                if (cTime > Private::latestEndAllowed[actToPass])
                {
                    if (evaluateDebug)
                    {
                        cout << "End of action has been cancelled: invariant or one-way end precondition "
                                "deleted by TIL\n";
                    }
                }
                else
                {
                    ...
                    if (eaItr->second)
                    {
                        const map<int, list<int>> &endUses =
                    ...
                    if (abstractFalse > 0)
                    {
                        if (evaluateDebug)
                        {
                            ...
                            cout << "end effect of " << actToPass << " "
                                 << *(RPGBuilder::getInstantiatedOp(actToPass)) << ": " << abstractFalse
                                 << " as some support is false:";
                            const map<int, list<int>> &endUses =
                                TemporalAnalysis::getAbstractFactsUsedByActionEnd();
                    ...
                    else
                    {
                        if (evaluateDebug)
                        {
                            ...
                            cout << "end effect of " << actToPass << " "
                                 << *(RPGBuilder::getInstantiatedOp(actToPass)) << "\n";
                        }
                        ...
                        if (d->applyEndEffectNow(payload.get(), actToPass, actIsOpen, cTime, eaItr->second) &&
                            breakOnGoalsAndEnds)
                        {
                            break;
                        }
        ...
        if (nextHappening.newFactsAtThisTime != payload->factLayers.end())
        {
            ...
            if (evaluateDebug)
            {
                cout << "FACT LAYER AT TIME " << currFactLayerItr->first << "\n";
            }
...

            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    ...
                    if (evaluateDebug)
                    {
                        cout << "Updating from fact " << *stateItr << ", "
                             << *(RPGBuilder::getLiteral(*stateItr)) << endl;
                    }
                    if (d->updateActionsForNewLiteralFact(payload.get(), *stateItr, cTime, true) &&
                        breakOnGoalsAndEnds)
                    {
                        break;
                    }
...

            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    if (evaluateDebug)
                    {
                        cout << "Updating from repeated fact " << *stateItr << ", "
                             << *(RPGBuilder::getLiteral(*stateItr)) << endl;
                    }
                    if (d->updateActionsForNewLiteralFact(payload.get(), *stateItr, cTime, false) &&
                        breakOnGoalsAndEnds)
                    {
                        break;
                    }
...

            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    if (evaluateDebug)
                    {
                        cout << "Updating from numeric fact " << *stateItr << ":  "
                             << (*(d->rpgNumericPreconditions))[*stateItr] << endl;
                    }
            ...
            //             {
            //                 list<pair<int, int> >::iterator stateItr =
            //                 currFactLayerItr->second.TILs.begin(); const list<pair<int, int> >::iterator
            //                 stateEnd = currFactLayerItr->second.TILs.end();
            //
            //                 for (; stateItr != stateEnd; ++stateItr) {
            //                     if (evaluateDebug) cout << "Updating from TIL " << stateItr->first << ",
            //                     fact " << stateItr->second << "\n";
            //
            //                     ActionViolationData & costData = payload->tilCosts[stateItr->first];
            //
            //                     costData.canBeApplied = true;
            //
            //                     d->addPreconditionCost(costData, stateItr->first, Planner::E_AT,
            //                     EpsilonResolutionTimestamp::undefined(), payload.get());
            //                     d->addEffectCost(costData, stateItr->first, Planner::E_AT, payload.get());
            //
            //                     const CostedAchieverDetails::AchieverProposalResult applyResult =
            //                     d->proposeNewAchiever(stateItr->second, stateItr->first, Planner::E_AT,
            //                     costData, false, cTime); if (applyResult !=
            //                     CostedAchieverDetails::not_added) {
            //                         if (d->updateActionsForNewLiteralFact(payload.get(), stateItr->second,
            //                         cTime, (applyResult == CostedAchieverDetails::first_achiever)) &&
            //                         breakOnGoalsAndEnds) break;
            //                     }
            //
            //
            //                 }
...

            {
                ...
                for (; stateItr != stateEnd; ++stateItr)
                {
                    ...
                    if (evaluateDebug)
                    {
                        cout << "Updating from TIL " << *stateItr << " @ "
                             << RPGBuilder::getNonAbstractedTILVec()[*stateItr]->duration
                             << " (cTime = " << cTime << ")" << endl;
                    }

                    d->addPreconditionCost(costData, *stateItr, Planner::E_AT,
                                           EpsilonResolutionTimestamp::undefined(), payload.get());
                    d->addEffectCost(costData, *stateItr, Planner::E_AT, payload.get());

                    EpsilonResolutionTimestamp nlTime = cTime;
                    ++nlTime;
                    if (d->applyPropositionalEffects(payload.get(), *stateItr, Planner::E_AT, costData, false,
                                                     nlTime) &&
                        breakOnGoalsAndEnds)
                    {
                        break;
            ...
            if (!currFactLayerItr->second.startDelayedUntilNow.empty())
            {
                ...
                if (d->updateActionsForNewLiteralFact(payload.get(), -2, cTime, true) && breakOnGoalsAndEnds)
                {
                    break;
                }
            }
            ...
            if (currFactLayerItr->second.endOfJustApplied)
            {
                    ...
                    for (; fItr != fEnd; ++fItr)
                    {
                        ...
                        if (!(--(nrItr->second)))
                        {
                            ...
                            if (!payload->startPreconditionCounts[*fItr])
                            {
                                ...
                                d->noLongerForbidden.push_back(
                                    pair<int, Planner::time_spec>(*fItr, Planner::E_AT_START));
                                spawn = true;
                            }
                        }
                    }
                    ...
                    for (; fItr != fEnd; ++fItr)
                    {
                        ...
                        if (!(--(nrItr->second)))
                        {
                            ...
                            if (!payload->endPreconditionCounts[*fItr])
                            {
                                assert(*fItr < RPGBuilder::getStartPropositionalPreconditions().size());
                                d->noLongerForbidden.push_back(
                                    pair<int, Planner::time_spec>(*fItr, Planner::E_AT_END));
                                spawn = true;
                            }
                        }
                    }
                ...
                if (spawn)
                {
                    if (d->updateActionsForNewLiteralFact(payload.get(), -2, cTime, true) &&
                        breakOnGoalsAndEnds)
                    {
                        break;
                    }
            ...
            if (RPGBuilder::modifiedRPG && (!currFactLayerItr->second.abstractFactBecomesTrue.empty() ||
                                            !currFactLayerItr->second.abstractFactBecomesFalse.empty()))
            {
                // abstract TIL stuff
                d->noLongerForbidden.clear();
...

                {
                    ...
                    const list<int>::iterator delEnd =
                        currFactLayerItr->second.abstractFactBecomesFalse.end();
                    ...
                    for (; delItr != delEnd; ++delItr)
                    {
                        ...
                        {
                            cout << COLOUR_light_blue << "Abstract fact "
                                 << *(RPGBuilder::getLiteral(*delItr)) << " becomes false.";
                            cout.flush();
                        }
                        ...
                        for (; ioItr != ioEnd; ++ioItr)
                        {
                            ...
                            {
                                if (payload->startPreconditionCounts[ioItr->first] ||
                                    payload->numericStartPreconditionCounts[ioItr->first])
                                {
                                    // still not appeared in the TRPG
                                    ++(payload->startPreconditionCounts[ioItr->first]);  // one of its start
                                                                                         // preconditions has
                                                                                         // become unsatisfied
                                    ++uncounted;
                                }
                            ...
                            {
                                if (payload->endPreconditionCounts[ioItr->first] ||
                                    payload->numericEndPreconditionCounts[ioItr->first])
                                {
                                    // still not appeared in the TRPG
                                    ++(payload->endPreconditionCounts[ioItr->first]);  // one of its start
                                                                                       // preconditions has
                                                                                       // become unsatisfied
                                    ++uncounted;
                                }
...

                {
                    ...
                    for (; addItr != addEnd; ++addItr)
                    {
                        ...
                        {
                            cout << COLOUR_light_blue << "Abstract fact "
                                 << *(RPGBuilder::getLiteral(*addItr)) << " becomes true.";
                            cout.flush();
                        }
                        ...
                        for (; ioItr != ioEnd; ++ioItr)
                        {
                            ...
                            {
                                if (payload->startPreconditionCounts[ioItr->first])
                                {  // still being disabled by at least the absence of this fact
                                    // still not appeared in the TRPG
                                    ++counted;
                                    if (!(--(payload->startPreconditionCounts[ioItr->first])) &&
                                        !(payload->numericStartPreconditionCounts[ioItr->first]))
                                    {
                                        d->noLongerForbidden.push_back(
                                            pair<int, Planner::time_spec>(actID, Planner::E_AT_START));
                                        spawn = true;
                                    }
                                }
                            }
                            if (ioItr->second == Planner::E_AT_END || ioItr->second == Planner::E_OVER_ALL)
                            {
                                ...
                                {  // still being disabled by at least the absence of this fact
                                    ...
                                    {
                                        d->noLongerForbidden.push_back(
                                            pair<int, Planner::time_spec>(actID, Planner::E_AT_END));
...
#ifdef POPF3ANALYSIS
            {
                ...
                const list<int>::const_iterator gEnd =
                    currFactLayerItr->second.literalGoalsWeMustHaveByNow.end();
            ...
            if (!failedToMeetDeadline)
            {
                ...
                const list<int>::const_iterator gEnd =
                    currFactLayerItr->second.numericGoalsWeMustHaveByNow.end();
            ...
            if (!failedToMeetDeadline)
            {
                list<int>::const_iterator gItr =
                    currFactLayerItr->second.preferenceUnreachableIfNotSatisfiedByNow.begin();
                const list<int>::const_iterator gEnd =
                    currFactLayerItr->second.preferenceUnreachableIfNotSatisfiedByNow.end();
                ...
                if (costChanged && Globals::bestSolutionQuality > -DBL_MAX)
                {
                    ...
                    {
                        double localCost = RPGBuilder::getMetric()->constant + reachablePreferenceCost +
                                           payload->withinCosts;
                        ...
                        if (localCost <= Globals::bestSolutionQuality)
                        {
                            ...
                            {
                                cout << "Pruning state: within costs alone would mean the solution cost is "
                                     << localCost << " by layer " << currFactLayerItr->first << endl;
                            }
                            failedToMeetDeadline = true;
                        }
                    }
...
#endif
        {
            ...
            if (!newNumericPreconditions.empty())
            {
                ...
                for (; npItr != npEnd; ++npItr)
                {
                    ...
                    {
                        --(payload->unsatisfiedGoals);
                        if (payload->unsatisfiedGoals == payload->fakeGoalCount &&
                            payload->unappearedEnds == 0)
                        {
                ...
                if (RPGBuilder::modifiedRPG)
                {
                    ...
                    for (; preID != preIDEnd; ++preID)
                    {
                        ...
                        {
                            payload
                                ->factLayers[nextHappening.timestamp + EpsilonResolutionTimestamp::epsilon()]
                                .second.push_back(*preID);
                            if (printRPGAsDot)
                            {
                                payload->dot.addNumericFactNode(nextHappening.timestamp.toDouble() + EPSILON,
                                                                *preID);
                            }
                        }
                    }
                ...
                {
                    FactLayerEntry &factsGoAt =
                        payload->factLayers[nextHappening.timestamp + EpsilonResolutionTimestamp::epsilon()];

                    factsGoAt.second.insert(factsGoAt.second.end(), newNumericPreconditions.begin(),
                                            newNumericPreconditions.end());
                }
            }
            else
            {
    ...
    if ((payload->unsatisfiedGoals > payload->fakeGoalCount) || payload->unappearedEnds
#ifdef POPF3ANALYSIS
        || payload->tooExpensive || failedToMeetDeadline
#endif
    )
    {
        if (RPGHeuristic::printRPGAsDot)
        ...
        if (evaluateDebug)
        {
            ...
            cout << "unsatisfiedGoals = " << payload->unsatisfiedGoals << ", fake goals "
                 << payload->fakeGoalCount << ", unappearedEnds = " << payload->unappearedEnds;
#ifdef POPF3ANALYSIS
            ...
            if (!Globals::remainingActionsInPlan.empty())
            {
                ...
                for (; actItr != actEnd; ++actItr)
                {
                    ...
                    if (actItr->second == Planner::E_AT_START)
                    {
                        ...
                        {
                            cout << "Noted number of unsatisfied start preconditions: "
                                 << payload->startPreconditionCounts[actItr->first->getID()] << endl;
                        }
                    ...
                    else
                    {
                        ...
                        {
                            cout << "Noted number of unsatisfied end preconditions: "
                                 << payload->endPreconditionCounts[actItr->first->getID()] << endl;
                        }
                    ...
                    {
                        const list<Literal *> pres =
                            (actItr->second == Planner::E_AT_START ?
                        ...
                        for (; pItr != pEnd; ++pItr)
                        {
                            ...
                            d->getAchieverDetails((*pItr)->getStateID(),
                                                  EpsilonResolutionTimestamp::infinite(), acIn, acBy);
                            ...
                            else
                            {
                                ...
                                {
                                    cout << "\tPrecondition " << *(*pItr) << " was achieved in layer " << acIn
                                         << endl;
                                }
                                else
                                {
                                    cout << "\tPrecondition " << *(*pItr)
                                         << " was true in the partial order at " << acIn << endl;
                                }
                            }
                        }
                    }

                    {
                        ...
                        for (; pItr != pEnd; ++pItr)
                        {
                            ...
                            {
                                cout << "\tNumeric precondition " << RPGBuilder::getNumericPreTable()[*pItr]
                                     << " was never seen. ";
                            }
                            else
                            {
                                ...
                                {
                                    cout << "\tNumeric precondition "
                                         << RPGBuilder::getNumericPreTable()[*pItr]
                                         << " is true in state, PO time "
                                         << (*(d->numericAchievedInLayer))[(*pItr)] << endl;
                                }
                                else
                                {
                                    cout << "\tNumeric precondition "
                                         << RPGBuilder::getNumericPreTable()[*pItr]
                                         << " was achieved at time "
                                         << (*(d->numericAchievedInLayer))[(*pItr)] << endl;
                                }
                            }
                        }
            ...
            for (; gItr != gEnd; ++gItr)
            {
                ...
                {
#endif
                    cout << "Goal " << *gItr << ", " << *(RPGBuilder::getLiteral(*gItr))
                         << ", was never seen. ";
                    assert(!(RPGBuilder::isStatic(RPGBuilder::getLiteral(*gItr)).first));
                    cout << "Possible achievers: " << (*(d->effectsToActions))[*gItr].size() << endl;
                    list<pair<int, Planner::time_spec>>::const_iterator aItr =
                        (*(d->effectsToActions))[*gItr].begin();
                    const list<pair<int, Planner::time_spec>>::const_iterator aEnd =
                        (*(d->effectsToActions))[*gItr].end();
                    ...
                    for (; aItr != aEnd; ++aItr)
                    {
                        if (aItr->second == Planner::E_AT_START)
                        {
                            ...
                            {
                                cout << "- Start of " << aItr->first << ", "
                                     << *(RPGBuilder::getInstantiatedOp(aItr->first)) << ", was never seen\n";
                            }
                            list<Literal *> &actPres =
                                (*(d->actionsToProcessedStartPreconditions))[aItr->first];
                            ...
                            for (; preItr != preEnd; ++preItr)
                            {
                                ...
                                {
                                    d->getAchieverDetails((*preItr)->getStateID(),
                                                          EpsilonResolutionTimestamp::infinite(), acIn, acBy);
                                }
#else
                                d->getAchieverDetails((*preItr)->getStateID(),
                        ...
                        else
                        {
                            if (payload->endActionSchedule[aItr->first].isUndefined() &&
                                payload->openEndActionSchedule[aItr->first].isUndefined())
                            {
                                if (!payload->startActionSchedule[aItr->first].isUndefined())
                                {
                                    cout << "- End of " << aItr->first << ", "
                                         << *(RPGBuilder::getInstantiatedOp(aItr->first))
                                         << ", was never seen\n";
                                }
                                else
                                {
                                    cout << "- End of " << aItr->first << ", "
                                         << *(RPGBuilder::getInstantiatedOp(aItr->first))
                                         << " was never seen (nor was its start)\n";
...
#else
                                        d->getAchieverDetails((*preItr)->getStateID(),
                                                              EpsilonResolutionTimestamp::infinite(), acIn,
                                                              acBy);
#endif

                                        if (acIn.isUndefined())
                                        {
                                            cout << "   * Start precondition " << *(*preItr)
                                                 << " never appeared\n";
                                        }
                                    }
                                }
            ...
            if (startEventQueue)
            {
                ...
                for (; seqItr != seqEnd; ++seqItr)
                {
                    cout << "- End of " << seqItr->actID << ", "
                         << *(RPGBuilder::getInstantiatedOp(seqItr->actID)) << ":\n";
...
#else
                        d->getAchieverDetails((*preItr)->getStateID(), EpsilonResolutionTimestamp::infinite(),
                                              acIn, acBy);
#endif
    ...
    else
    {
        if (payload->fakeGoalCount)
        {
            ...
            if (evaluateDebug)
            {
                cout << "RPG found all goals and ends, but could not satisfy some preferences\n";
            }
        }
        else
        {
            if (evaluateDebug)
            {
                cout << "RPG found all goals and ends, and satisfied all preferences\n";
            }
        }

        if (prefDebug)
        {
            cout << COLOUR_red << "Got preference violations down to "
                 << payload->goalPrefViolationAtLastLayer << COLOUR_default << endl;
        }
    }
    ...
    if (!ignoreNumbers && !timeAtWhichValueIsDefined.empty())
    {
        ...
        for (int i = 0; i < loopLim; ++i)
        {
            ...
            {
                if ((*(d->rpgNumericPreconditions))[i].isSatisfiedWCalculate(extrapolatedMin,
                                                                             extrapolatedMax))
                {
    ...
    if (earliestTIL.first != INT_MAX)
    {
        for (int tilID = theState.nextTIL; tilID <= earliestTIL.first; ++tilID)
        {
            ...
            if (rpItr != rpEnd)
            {
                if (EpsilonResolutionTimestamp(rpItr->first, true) ==
                    EpsilonResolutionTimestamp(tilTime, true))
                {
                    rpItr->second.push_back(
                        ActionSegment((instantiatedOp *)0, Planner::E_AT, tilID, RPGHeuristic::emptyIntList));
                    //++h;
            ...
            else
            {
                ...
                relaxedPlan.back().second.push_back(
                    ActionSegment((instantiatedOp *)0, Planner::E_AT, tilID, RPGHeuristic::emptyIntList));
                //++h;
                if (tilID == theState.nextTIL)
                {
    ...
    if (RPGBuilder::fullFFHelpfulActions)
    {
...

        {
            ...
            for (; haItr != haEnd; ++haItr)
            {
                ...
                for (; eItr != eEnd; ++eItr)
                {
                    const list<pair<int, Planner::time_spec>> &acBy =
                        (*(d->effectsToActions))[(*eItr)->getStateID()];
                    ...
                    for (; candItr != candEnd; ++candItr)
                    {
                        ...
                        {
                            const EpsilonResolutionTimestamp &thisSAS =
                                payload->startActionSchedule[candItr->first];
                            if (thisSAS.isUndefined())
                            {
                                continue;
                            }
                            if (thisSAS > sas)
                        ...
                        {
                            /*                            // first time we've had to work out if this is
                               helpful list<Literal*>::const_iterator depItr = deps->begin(); const
                               list<Literal*>::const_iterator depEnd = deps->end();

                                                        for (; depItr != depEnd; ++depItr) {
                                                            const int factID = (*depItr)->getID();
                                                            const double acIn =
                               (*(d->achievedInLayer))[factID]; if (acIn == -1.0) break; if (acIn > 0.0 &&
                               (*(d->achievedBy))[factID].first == -1) break;
                                                        }
                                                        if (depItr == depEnd) {*/
                            helpfulActions.insert(haNext,
                                                  ActionSegment(RPGBuilder::getInstantiatedOp(candItr->first),
                                                                candItr->second, -1, emptyIntList));
                            /*}*/
                        }
                    }
                }
                ...
                if (haItr->second == Planner::E_AT_START)
                {
                    ...
                    const EpsilonResolutionTimestamp &endStamp =
                        payload->endActionSchedule[haItr->first->getID()];
                    list<Literal *>::const_iterator eItr = effs->begin();
                    const list<Literal *>::const_iterator eEnd = effs->end();
                    ...
                    for (; eItr != eEnd; ++eItr)
                    {
                        ...
                        for (; candItr != candEnd; ++candItr)
                        {
                            if (candItr->second != Planner::E_AT_END)
                            {
                                continue;
                            }
                            if (payload->endActionSchedule[candItr->first] == endStamp)
                            {
                                if (endsIn.insert(candItr->first).second &&
                                    startsIn.insert(candItr->first).second)
                                {
                                    /*const list<Literal*> * deps =
                                    &((*(d->actionsToProcessedStartPreconditions))[candItr->first]);
                                    ...
                                    if (depItr == depEnd) { */
                                    helpfulActions.insert(
                                        haNext, ActionSegment(RPGBuilder::getInstantiatedOp(candItr->first),
                                                              Planner::E_AT_START, -1, emptyIntList));
                                    /*}*/
                                }
                            }
                        }
    ...
    if (RPGHeuristic::orderByDeadlineRelevance && !relaxedPlan.empty())
    {
        if (Globals::globalVerbosity & 1048576)
        {
            ...
            for (; haItr != haEnd; ++haItr)
            {
                ...
                else
                {
                    ...
                    if (haItr->second == Planner::E_AT_END)
                    {
                        cout << "end\n";
                    }
                    if (haItr->second == Planner::E_OVER_ALL)
                    {
                        cout << "middle point " << haItr->divisionID << "\n";
                    }
                }
            }
        }
        ...
        for (; rlItr != rlEnd; ++rlItr)
        {
            ...
            if (RPGBuilder::getNonAbstractedTILVec().empty() ||
                (nextTIL < Private::tilCount && rlItr->second != Planner::E_AT))
            {
                const double w = (rlItr->second == Planner::E_AT_START ?
                                      Private::earliestDeadlineRelevancyStart[rlItr->first->getID()] :
                                      Private::earliestDeadlineRelevancyEnd[rlItr->first->getID()])
                                     .toDouble();
    ...
    if (evaluateDebug || Globals::globalVerbosity & 1048576)
    {
        ...
        for (; rpItr != rpEnd; ++rpItr)
        {
            ...
            for (; rlItr != rlEnd; ++rlItr)
            {
                ...
                {
                    cout << "\t\tTIL " << rlItr->divisionID << " @ "
                         << RPGBuilder::getNonAbstractedTILVec()[rlItr->divisionID]->duration << endl;
                }
            }
        }
    }
    ...
    //         const vector<map<EpsilonResolutionTimestamp, int> > & softDeadlines =
    //         RPGBuilder::getLiteralGoalSoftDeadlines();
    //
    ...
    //             const map<EpsilonResolutionTimestamp, int>::const_iterator costEnd =
    //             softDeadlines[gID].end();
    //
    //             for (; costItr != costEnd && acIn >= costItr->first; ++costItr) {
    //
    //                 if (!(psa[costItr->second] == AutomatonPosition::satisfied) ) {
    //
    //                     assert(psa[costItr->second] == AutomatonPosition::unreachable ||
    //                     psa[costItr->second] == AutomatonPosition::eternallysatisfied); continue;
    //                 }
    //
    //                 const RPGBuilder::Constraint & currPref =
    //                 RPGBuilder::getPreferences()[costItr->second];
    //
    //                 if (evaluateDebug) {
    //                     cout << "\t\tUpdating state as in RPG, fact " << *(*gItr) << " didn't appear until
    //                     " << acIn << ", so at time " << costItr->first << ", violated preference " <<
    //                     currPref.name << ", cost " << currPref.cost << endl;
    //                 }
    //
    ...
    if (RPGBuilder::getMetric())
    {
                                                ...
                                                payload->rpgGoalPrefViolation < reachablePreferenceCost)
        {
            ...
            {
                if (!canBeSatisfied(theState.preferenceStatus[p]))
                {
                    continue;
                }
                if (!isSatisfied(payload->optimisticPrefStatus[p]))
                {
                    ...
                    if (prefDebug)
                    {
                        cout << "Preference " << RPGBuilder::getPreferences()[p].name << " (" << p
                             << ") was never reached\n";
                            ...
                            for (int cc = 0; cc < cellCount; ++cc)
                            {
                                ...
                                if (cells[cc].lit)
                                {
                                    ...
                                    if (cells[cc].polarity)
                                    {
                                        ...
                                        {
                                            oneWasFalse = true;
                                            cout << " - Don't have supporting fact " << *(cells[cc].lit)
                                                 << endl;
                                        }
                                        else
                                        {
                                ...
                                else
                                {
                                    ...
                                    if (cells[cc].polarity)
                                    {
                                        ...
                                        {
                                            oneWasFalse = true;
                                            cout << " - Don't have supporting numeric "
                                                 << RPGBuilder::getNumericPreTable()[fID] << endl;
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        oneWasFalse = true;
                                        //    cout << " - Don't have supporting numeric ¬" <<
                                        //    RPGBuilder::getNumericPrecs()[fID] << endl;
                                        //}
                                    }
                                }
                            }
    ...
    if (RPGBuilder::getMetric() && Globals::optimiseSolutionQuality)
    {
        if (NumericAnalysis::theMetricIsMonotonicallyWorsening())
        {
            ...
            if (RPGHeuristic::alwaysExpandFully && Globals::optimiseSolutionQuality &&
                !d->currentCosts.empty())
            {
            ...
            {
                admissibleCost = payload->rpgGoalPrefViolation /* + definiteWithinCost*/ +
                                 RPGBuilder::getMetric()->constant;
                ...
                for (; wItr != wEnd; ++wItr, ++vItr)
                {
                    if (*vItr < 0)
                    {
                        ...
                        {
                            // Would need upper-bound on makespan
                            return new EvaluationInfo(h, std::numeric_limits<double>::signaling_NaN(),
                                                      realGoalsSatisfied);
                        }
                        else
                        {
...
EpsilonResolutionTimestamp RPGHeuristic::Private::earliestTILForAction(const unsigned int &i,
                                                                       const bool &isStart)
{
    assert(i >= 0);
    ...
    list<Literal *> &inspect =
        (isStart ? (*actionsToProcessedStartPreconditions)[i] : (*actionsToEndPreconditions)[i]);
...
bool RPGHeuristic::Private::applyPropositionalEffects(
    Private::BuildingPayload *const payload, const int &currAct, const Planner::time_spec &currTS,
    const ActionViolationData &prefCosts, const bool &openEnd,
    const EpsilonResolutionTimestamp &nlTime)  //, MaxDependentInfo & POtime)
{
    static const bool updateDebug = Globals::globalVerbosity & 64 || evaluateDebug;

    {
        const list<Literal *> &addEffects =
            (currTS == Planner::E_AT ?
                 RPGBuilder::getNonAbstractedTILVec()[currAct]->addEffects :
                 (currTS == Planner::E_AT_START ? RPGBuilder::getStartPropositionAdds()[currAct] :
                                                  RPGBuilder::getEndPropositionAdds()[currAct]));
        ...
        for (; addEffItr != addEffEnd; ++addEffItr)
        {
            const int currEff = (*addEffItr)->getStateID();
            const CostedAchieverDetails::AchieverProposalResult applyResult =
                proposeNewAchiever(currEff, currAct, currTS, prefCosts, openEnd, nlTime);
            ...
            switch (applyResult)
            {
                case CostedAchieverDetails::not_added:
                {
                    if (updateDebug)
                    {
                        cout << "\t\tFact " << currEff << ", " << *(*addEffItr) << ", was already achieved\n";
                    }
                    break;
                }
                case CostedAchieverDetails::replaced_existing_achiever:
                {
                    ...
                    if (updateDebug)
                    {
                        cout << "\t\tFact " << currEff << ", " << *(*addEffItr) << ", now achieved at "
                             << nlTime << " with lower cost\n";
                    }
                ...
                case CostedAchieverDetails::first_achiever:
                {
                    ...
                    if (updateDebug)
                    {
                        cout << "\t\tFact " << currEff << ", " << *(*addEffItr) << ", is new in " << nlTime
                             << "\n";
                    }
                    if (goals.find(currEff) != gsEnd)
                    {
                            ...
                            payload->unappearedEnds == 0)
                        {
                            ...
                            {
                                // can return true (terminate expansion) if there are no outstanding fake
                                // goals
                                return true;
                            }
                        }
                ...
                case CostedAchieverDetails::same_time_as_first_achiever:
                {
                    ...
                    if (updateDebug)
                    {
                        cout << "\t\tFact " << currEff << ", " << *(*addEffItr) << ", achieved at " << nlTime
                             << " - no later than before, but now with lower cost\n";
                    }
                    if (RPGHeuristic::printRPGAsDot)
                    {
                        payload->dot.addActionToFactEdge(currAct, currTS, currEff);
    ...
    {
        const list<Literal *> &delEffects =
            (currTS == Planner::E_AT ?
                 RPGBuilder::getNonAbstractedTILVec()[currAct]->delEffects :
                 (currTS == Planner::E_AT_START ? RPGBuilder::getStartPropositionDeletes()[currAct] :
                                                  RPGBuilder::getEndPropositionDeletes()[currAct]));
        ...
        for (; delEffItr != delEffEnd; ++delEffItr)
        {
            ...
            if (currAIL.isUndefined())
            {
                if (updateDebug)
                {
                    cout << "\t\tNegative fact " << currEff << " is new\n";
                }
                currAIL = nlTime;
...
bool RPGHeuristic::Private::checkPreconditionsAreSatisfied(const int &currAct, const Planner::time_spec &ts,
                                                           const EpsilonResolutionTimestamp &layer)
{
    {
        const list<Literal *> &precList =
            (ts == Planner::E_AT_START ? (*actionsToProcessedStartPreconditions)[currAct] :
                                         (*actionsToEndPreconditions)[currAct]);
        ...
        for (; pItr != pEnd; ++pItr)
        {
            ...
            if (evaluateDebug)
            {
                cout << "\t\t\t\tPrecondition " << *(*pItr) << " first became true at "
                     << achieverDetails[(*pItr)->getStateID()].front().getLayer() << "\n";
            }
#else
            ...
            if (evaluateDebug)
            {
                cout << "\t\t\t\tPrecondition " << *(*pItr) << " became true at "
                     << (*achievedInLayer)[(*pItr)->getStateID()] << "\n";
            }
#endif
        }
    }
    ...
    {
        const list<int> &precList =
            (ts == Planner::E_AT_START ? (*actionsToProcessedStartNumericPreconditions)[currAct] :
        ...
        for (; pItr != pEnd; ++pItr)
        {
            ...
            if (evaluateDebug)
            {
                cout << "\t\t\t\tNumeric precondition " << (*pItr) << " became true at "
                     << (*numericAchievedInLayer)[(*pItr)] << "\n";
            }
        }
    }
...
bool RPGHeuristic::Private::applyNumericEffects(Private::BuildingPayload *const payload, const int &currAct,
                                                const Planner::time_spec &currTS,
                                                const EpsilonResolutionTimestamp &nlTime, const int &limitTo)
{
    const int actualLimit =
        (limitTo != -1 ? limitTo : RPGBuilder::howManyTimesOptimistic(currAct, payload->startState));
    ...
    ActionAndHowManyTimes details(currAct, currTS, actualLimit, payload->actionDurations[currAct].first,
                                  payload->actionDurations[currAct].second);

    {
        // first, instantaneous numeric effects
        const list<int> &numericEffects =
            (currTS == Planner::E_AT_START ? (*actionsToRPGNumericStartEffects)[currAct] :
    ...
    map<int, list<RPGBuilder::IntegralContinuousEffect>>::const_iterator iceItr =
        RPGBuilder::getActionsToIntegralConditionalEffects().find(currAct);
    ...
    if (currTS == Planner::E_AT_START)
    {
        ...
        if (!integratedCTSEffectChange[currAct].empty())
        {
            ...
            {
                payload->fluentLayers.recordIntegratedNumericEffect(
                    details, nlTime, make_pair(*contChangeVar, *contChangeItr));
            }
        }
        ...
        if (!gradientCTSEffectChange[currAct].empty())
        {
            ...
            {
                payload->fluentLayers.recordGradientNumericEffect(
                    details, nlTime, make_pair(*contChangeVar, *contChangeItr), payload->factLayers);
            }
        }
    }
...
bool RPGHeuristic::Private::updateActionsForNewLiteralFact(
    Private::BuildingPayload *const payload, const int &toPropagate,
    const EpsilonResolutionTimestamp &factLayerTime,
    const bool &decrementRemainingUnsatisfiedPreconditionCounters)
{
    ...
    list<pair<int, Planner::time_spec>> &dependents =
        ((toPropagate == -1) ?
             (*preconditionlessActions) :
             ((toPropagate == -2) ? (noLongerForbidden) : (*processedPreconditionsToActions)[toPropagate]));
    ...
    if (updateDebug)
    {
        cout << "\tAffects " << dependents.size() << " actions\n";
    }
    ...
    for (; depItr != depEnd; ++depItr)
    {
        ...
        if (updateDebug)
        {
            cout << ", " << (startAct ? "start" : "end") << " " << *(RPGBuilder::getInstantiatedOp(currAct))
                 << "\n";
        }
        if (startAct)
        {
            int &spc = startPreconditionCounts[currAct];
            if (!preconditionless && decrementRemainingUnsatisfiedPreconditionCounters)
            {
                --spc;
            }
            if ((!spc && !numericStartPreconditionCounts[currAct]) &&
                forbiddenStart.find(currAct) == forbiddenStart.end() && nlTime < latestStartAllowed[currAct])
            {
                ...
                if (updateDebug)
                {
                    cout << "\tStart of action " << currAct
                         << " is now applicable: " << startPreconditionCounts[currAct] << "/"
                         << numericStartPreconditionCounts[currAct];
                    if (preconditionless)
                    {
                        cout << ", considered preconditionless";
                    }
                    cout << "\n";
                }
                ...
                if (expandFully)
                {
                    earliestStartAllowed[currAct] = factLayerTime;
                }
                ...
                if (applyPropositionalEffects(payload, currAct, Planner::E_AT_START, costData, false,
                                              nlTime /*, POtime*/))
                {
                    return true;
                }
                ...
                if (applyNumericEffects(payload, currAct, Planner::E_AT_START, nlTime, -1))
                {
                    return true;
                }

                {
                    static pair<EpsilonResolutionTimestamp, list<pair<int, bool>>> defaultEntry(
                        EpsilonResolutionTimestamp::undefined(), list<pair<int, bool>>());

                    defaultEntry.first = factLayerTime + EpsilonResolutionTimestamp(
                                                             payload->actionDurations[currAct].first, true);
                    ...
                    if (!endPreconditionCounts[currAct] && !numericEndPreconditionCounts[currAct])
                    {
                        if (updateDebug)
                        {
                            if (decrementRemainingUnsatisfiedPreconditionCounters)
                            {
                                cout << "\t\tiii) End of action can be put at a fact layer sufficiently far "
                                        "in future ("
                                     << defaultEntry.first << "\n";
                            }
                            else
                            {
                                cout << "\t\tiii) End of action can be repeated at a fact layer sufficiently "
                                        "far in future ("
                                     << defaultEntry.first << "\n";
                            }
                        }

                        endActionsAtTime.insert(defaultEntry)
                            .first->second.push_back(
                                make_pair(currAct, decrementRemainingUnsatisfiedPreconditionCounters));
                    }
                }
            ...
            else
            {
                if (updateDebug)
                {
                    cout << "\tStart of action " << currAct << " now only has "
                         << startPreconditionCounts[currAct]
                         << " unsatisfied propositional preconditions and "
                         << numericStartPreconditionCounts[currAct] << " numeric\n";
                }
            }
        }
        else
        {
            ...
            if ((!epc && !numericEndPreconditionCounts[currAct]) &&
                forbiddenEnd.find(currAct) == forbiddenEnd.end() && nlTime < latestEndAllowed[currAct])
            {
                if (RPGBuilder::rogueActions[currAct])
                {
                    cout << "Critical Error: Trying to apply end of action " << currAct << ", "
                         << *(RPGBuilder::getInstantiatedOp(currAct)) << ", which is invalid or irrelevant\n";
                    ...
                    {
                        cout << "\tFound it on the list of actions depending on "
                             << RPGBuilder::getLiteral(toPropagate) << "\n";
                    }
                    assert(!RPGBuilder::rogueActions[currAct]);
                }
                if (updateDebug)
                {
                    cout << "\tEnd of action " << currAct << ", " << *(RPGBuilder::getInstantiatedOp(currAct))
                         << ", is now applicable\n";
                }
                ...
                if (decrementRemainingUnsatisfiedPreconditionCounters)
                {
                    if (expandFully)
                    {
                        earliestEndAllowed[currAct] = factLayerTime;
                    }
                    ...
                    if (insistOnThisEnd)
                    {
                        if (updateDebug)
                        {
                            cout << "\tEnd is insisted upon, had " << unappearedEnds << " remaining\n";
                        }
                            ...
                            EpsilonResolutionTimestamp::zero())
                        {
                            ...
                            if (updateDebug)
                            {
                                cout << "\tNow down to " << unappearedEnds << "\n";
                            }
                        }
                        else
                        {
                            if (updateDebug)
                            {
                                cout << "\tOpen end cannot appear quite yet\n";
                            }
                            insistOnThisEnd = false;
                            endActionsAtTime[openEndActionSchedule[currAct]].push_back(
                                make_pair(-1 - currAct, decrementRemainingUnsatisfiedPreconditionCounters));
                            /*--unappearedEnds; // has still appeared, insofar as plan extraction will happily
                            pick this up at a later layer. if (!unappearedEnds && unsatisfiedGoals ==
                            payload->fakeGoalCount) {
                ...
                for (int epPass = 0; epPass < 2; ++epPass)
                {
                    ...
                    {
                        if (!startPreconditionCounts[currAct] && !numericStartPreconditionCounts[currAct] &&
                            !startActionSchedule[currAct].isUndefined())
                        {
                            ...
                            if (factLayerTime - enforcedEnd >= EpsilonResolutionTimestamp::zero())
                            {
                                ...
                                if (updateDebug)
                                {
                                    cout << "\t\tDoing the update loop because " << factLayerTime
                                         << " >= " << enforcedEnd << endl;
                                }
                            }
                            else
                            {
                                endActionsAtTime[enforcedEnd].push_back(
                                    make_pair(currAct, decrementRemainingUnsatisfiedPreconditionCounters));
                                if (updateDebug)
                                {
                                    cout << "\t\tNot doing the update loop because " << factLayerTime << " < "
                                         << enforcedEnd << endl;
                                }
                            }
                        }
                    ...
                    if (doLoop)
                    {
                        ...
                        if (RPGHeuristic::printRPGAsDot)
                        {
                            ...
                            {
                                payload->dot.addActionNode(factLayerTime.toDouble(), depItr->first,
                                                           depItr->second);
                            }
                        }

                        // MAKEENDMDI(POtime, (limitToOnce ? payload->earliestStartOf[currAct] : 0.0),
                        // RPGBuilder::getOpMinDuration(currAct, 0), currAct);

                        if (applyPropositionalEffects(payload, currAct, Planner::E_AT_END,
                                                      payload->endActionCosts[currAct], limitToOnce,
                                                      nlTime /*, POtime*/))
                        {
                            return true;
                        }
                        ...
                        if (limitToOnce)
                        {
                            ...
                            {
                                if (applyNumericEffects(payload, currAct, Planner::E_AT_END, nlTime,
                                                        saItr->second.size()))
                                {
                                    return true;
                                }
                        ...
                        {
                            if (applyNumericEffects(payload, currAct, Planner::E_AT_END, nlTime, -1))
                            {
                                return true;
                            }
                        }
            ...
            else
            {
                if (updateDebug)
                {
                    ...
                    {
                        cout << "\tEnd of action " << currAct << " at " << factLayerTime << " now only has "
                             << numericEndPreconditionCounts[currAct]
                             << " unsatisfied numeric conditions, and 1 unsatisfied literal condition:";
                        list<Literal *>::const_iterator litItr =
                            RPGBuilder::getEndPropositionalPreconditions()[currAct].begin();
                        const list<Literal *>::const_iterator litEnd =
                            RPGBuilder::getEndPropositionalPreconditions()[currAct].end();
                        ...
                        for (; litItr != litEnd; ++litItr)
                        {
                            ...
                            {
                                if (achieverDetails[(*litItr)->getStateID()].front().getLayer() ==
                                    factLayerTime)
                                {
                                    cout << " now{" << *(*litItr) << "@"
                                         << achieverDetails[(*litItr)->getStateID()].front().getLayer()
                                         << "}";
                                    ++slack;
                                }
                                else if (achieverDetails[(*litItr)->getStateID()].front().getLayer() >
                                         factLayerTime)
                                {
                                    cout << " later{" << *(*litItr) << "@"
                                         << achieverDetails[(*litItr)->getStateID()].front().getLayer()
                    ...
                    {
                        cout << "\tEnd of action " << currAct << " now only has "
                             << numericEndPreconditionCounts[currAct]
                             << " unsatisfied numeric conditions, and " << endPreconditionCounts[currAct]
                             << " unsatisfied literal conditions\n";
                    }
                }
            }
        }
...
bool RPGHeuristic::Private::updateActionsForNewNumericFact(Private::BuildingPayload *const payload,
                                                           const int &toPropagate,
                                                           const EpsilonResolutionTimestamp &factLayerTime)
{
    vector<int> &startPreconditionCounts = payload->startPreconditionCounts;
    vector<int> &endPreconditionCounts = payload->endPreconditionCounts;
    ...
    list<pair<int, Planner::time_spec>> &dependents =
        ((toPropagate >= 0) ? (*processedNumericPreconditionsToActions)[toPropagate] : dummyList);

    list<pair<int, Planner::time_spec>>::iterator depItr = dependents.begin();
    const list<pair<int, Planner::time_spec>>::iterator depEnd = dependents.end();

    if (updateDebug && (toPropagate >= 0))
    {
        cout << "\tAffects " << dependents.size() << " actions\n";
    }
    ...
    for (; depItr != depEnd; ++depItr)
    {
        ...
        if (updateDebug)
        {
            cout << "\tAffects " << currAct << ", " << (startAct ? "start" : "end") << " "
                 << *(RPGBuilder::getInstantiatedOp(currAct)) << "\n";
        }
        if (startAct)
        {
            if (!(--numericStartPreconditionCounts[currAct]) && !startPreconditionCounts[currAct] &&
                forbiddenStart.find(currAct) == forbiddenStart.end() && nlTime < latestStartAllowed[currAct])
            {
                assert(!RPGBuilder::rogueActions[currAct]);
                if (updateDebug)
                {
                    cout << "\tStart of action " << currAct << " is now applicable\n";
                }
                ...
                {
                    const EpsilonResolutionTimestamp endTime =
                        factLayerTime +
                        EpsilonResolutionTimestamp(payload->actionDurations[currAct].first, true);
                    ...
                    if (!endPreconditionCounts[currAct] && !numericEndPreconditionCounts[currAct])
                    {
                        if (updateDebug)
                        {
                            cout << "\t\tEnd of action can be put at fact layer sufficiently far in future ("
                                 << endTime << ")\n";
                        }
            ...
            else
            {
                ...
                {
                    cout << "\tStart of action " << currAct << " now only has "
                         << startPreconditionCounts[currAct] << " unsatisfied propositional preconditions\n";
                }
            }
        }
        else
        {
            ...
            if (!numericEndPreconditionCounts[currAct] && !endPreconditionCounts[currAct] &&
                forbiddenEnd.find(currAct) == forbiddenEnd.end() && nlTime < latestEndAllowed[currAct])
            {
                assert(!RPGBuilder::rogueActions[currAct]);
                if (updateDebug)
                {
                    cout << "\tEnd of action " << currAct << " is now applicable\n";
                }
            ...
            else
            {
                if (updateDebug)
                {
                    ...
                    cout << " now only has " << endPreconditionCounts[currAct]
...
bool RPGHeuristic::Private::applyEndEffectNow(Private::BuildingPayload *const payload, const int &currAct,
                                              const bool &openEnd,
                                              const EpsilonResolutionTimestamp &factLayerTime,
                                              const bool &isActuallyNew)
{
    ...
    // MAKEENDMDI(POtime, (openEnd ? payload->earliestStartOf[currAct] : 0.0),
    // RPGBuilder::getOpMinDuration(currAct, 0), currAct);

    if (applyPropositionalEffects(payload, currAct, Planner::E_AT_END, costData, openEnd,
                                  nlTime /*, POtime*/))
    {
        return true;
    }
    ...
    if (openEnd)
    {
        ...
        if (saItr != startedActions.end())
        {
            if (applyNumericEffects(payload, currAct, Planner::E_AT_END, nlTime, saItr->second.size()))
            {
                return true;
            }
        }
...
bool RPGHeuristic::Private::startShouldBeDelayed(const int &currAct,
                                                 const EpsilonResolutionTimestamp &factLayerTime,
                                                 BuildingPayload *const payload)
{
    ...
    pair<double, double> startBounds(factLayerTime.toDouble(),
                                     TemporalAnalysis::getActionTSBounds()[currAct][0].second);
    pair<double, double> endBounds(factLayerTime.toDouble() + payload->actionDurations[currAct].first,
                                   TemporalAnalysis::getActionTSBounds()[currAct][1].second);
    ...
    if (!keep)
    {
        if (debug)
        {
            cout << COLOUR_light_red << "Not actually getting start of "
                 << *(RPGBuilder::getInstantiatedOp(currAct))
                 << " as it can't appear here due to temporal constraints\n"
                 << COLOUR_default;
        }
        return true;
    }
    ...
    if (debug)
    {
        cout << COLOUR_light_red << "Delayed start of " << *(RPGBuilder::getInstantiatedOp(currAct)) << " to "
             << rounded << " rather than " << factLayerTime << " due to temporal constraints\n"
             << COLOUR_default;
    }
...
bool RPGHeuristic::Private::revisitActs(BuildingPayload *payload,
                                        const EpsilonResolutionTimestamp &factLayerTime,
                                        const list<pair<int, Planner::time_spec>> &actsToVisit)
{
    ...
    for (; depItr != depEnd; ++depItr)
    {
        ...
        if (depItr->second == Planner::E_AT_START)
        {
...

            {
                ...
                defaultEntry.first =
                    factLayerTime + EpsilonResolutionTimestamp(payload->actionDurations[currAct].first, true);

                if (!payload->endPreconditionCounts[currAct] &&
                    !payload->numericEndPreconditionCounts[currAct])
                {
                    payload->endActionsAtTime.insert(defaultEntry)
                        .first->second.push_back(make_pair(currAct, false));
                }
            }
        }
        else if (depItr->second == Planner::E_AT_END)
        {
            ...
            for (int epPass = 0; epPass < 2; ++epPass)
            {
                ...
                {
                    if (!payload->startPreconditionCounts[currAct] &&
                        !payload->numericStartPreconditionCounts[currAct] &&
                        !payload->startActionSchedule[currAct].isUndefined())
                    {
        ...
        else
        {
            ...
            addPreconditionCost(costData, depItr->first, Planner::E_AT,
                                EpsilonResolutionTimestamp::undefined(), payload);
            addEffectCost(costData, depItr->first, Planner::E_AT, payload);

            if (applyPropositionalEffects(payload, depItr->first, depItr->second, costData, false,
                                          nlTime /*, POtime*/))
            {
                return true;
            }
...
bool RPGHeuristic::Private::updatePreferencesForFact(BuildingPayload *const payload, const int &fID,
                                                     const bool &isALiteral, const bool &polarity,
                                                     const bool &isActuallyNew,
                                                     const Planner::EpsilonResolutionTimestamp &factLayerTime)
{
    ...
    assert(isALiteral ? (polarity ? (size_t)fID < preconditionsToPrefs->size() :
                                    (size_t)fID < negativePreconditionsToPrefs->size()) :
...

    {
        ...
        for (; depItr != depEnd; ++depItr)
        {
            NNF_Flat *const toUpdate =
                payload
                    ->initialUnsatisfiedPreferenceConditions[depItr->dest.first][depItr->dest.second ? 1 : 0];
            ...
            if (depItr->dest.first < taskPrefCount)
            {
                ...
                if (currPref.cons == VAL::E_WITHIN && currPref.deadline < factLayerTime.toDouble())
                {
                    if (Globals::globalVerbosity & 32768)
                    {
                        ...
                        cout << "was relevant to within preference " << currPref.name << ":"
                             << depItr->dest.first << ", but " << factLayerTime << " is too late for "
                             << currPref.deadline << COLOUR_default << endl;
                    }
            ...
            if (Globals::globalVerbosity & 32768)
            {
                ...
                cout << "is relevant to preference " << RPGBuilder::getPreferences()[depItr->dest.first].name
                     << ":" << depItr->dest.first << COLOUR_default << endl;
            }
            ...
            if (depItr->dest.first < taskPrefCount)
            {
                if (toUpdate->isSatisfied())
                {
                    ...
                    payload->preferencePartTrueAtLayer[depItr->dest.first][depItr->dest.second ? 1 : 0] =
                        factLayerTime;

                    const AutomatonPosition::Position oldPosn =
                        payload->optimisticPrefStatus[depItr->dest.first];

                    PreferenceHandler::updateCostsAndPreferenceStatus(
                        payload->startState, depItr->dest, payload->optimisticPrefStatus,
                        payload->prefCostOfAddingFact, payload->prefCostOfChangingNumberB, nowCostFreeToAdd);

                    const AutomatonPosition::Position newPosn =
                        payload->optimisticPrefStatus[depItr->dest.first];

                    if (Globals::globalVerbosity & 32768)
                    {
                        cout << "\t\t" << COLOUR_light_red << "Preference status was "
                             << positionName[oldPosn] << ", now " << positionName[newPosn] << COLOUR_default
                             << endl;
                    }
                    ...
                    if (oldPosn == AutomatonPosition::unsatisfied || oldPosn == AutomatonPosition::triggered)
                    {
                        ...
                        {
                            payload->rpgGoalPrefViolation -=
                                RPGBuilder::getPreferences()[depItr->dest.first].cost;

                            if (Globals::globalVerbosity & 32768)
                            {
                                cout << "\t\t" << COLOUR_light_red << "Subtracted "
                                     << RPGBuilder::getPreferences()[depItr->dest.first].cost
                                     << " from cost of reaching goal" << COLOUR_default << endl;
                            }
                            if (RPGBuilder::getMetric())
                            {
                                if (RPGBuilder::getMetric()->minimise)
            ...
            else
            {
                if (toUpdate->isSatisfied())
                {
                    ...
                    {
                        cout << "\t\t" << COLOUR_light_blue
                             << "Precondition preference now satisfied, at time " << factLayerTime << "\n"
                             << COLOUR_default;
                    }
                    payload->preferencePartTrueAtLayer[depItr->dest.first][depItr->dest.second ? 1 : 0] =
                        factLayerTime;

                    const pair<int, Planner::time_spec> &currAct =
                        RPGBuilder::getPreferences()[depItr->dest.first].attachedToOperator;
                    ...
                    ActionViolationData &actCost =
                        (currAct.second == Planner::E_AT_START ? payload->startActionCosts[currAct.first] :
                                                                 payload->endActionCosts[currAct.first]);
                    ...
                    if (actCost.canBeApplied)
                    {
                        ...
                        {
                            canAppearHere = ((!payload->startPreconditionCounts[currAct.first] &&
                                              !payload->numericStartPreconditionCounts[currAct.first]) &&
                                             payload->forbiddenStart.find(currAct.first) ==
                                                 payload->forbiddenStart.end() &&
                                             factLayerTime < latestStartAllowed[currAct.first]);
                        }
                        else if (currAct.second == Planner::E_AT_END)
                        {
                            canAppearHere =
                                ((!payload->endPreconditionCounts[currAct.first] &&
                                  !payload->numericEndPreconditionCounts[currAct.first]) &&
                                 payload->forbiddenEnd.find(currAct.first) == payload->forbiddenEnd.end() &&
                                 factLayerTime < latestEndAllowed[currAct.first]);
                        }
                        else
                        {
                            cerr << "Fatal internal error: precondition preference not at the start or end "
                                    "of an action\n";
                            exit(1);
                        }
                    }
                    ...
                    if (canAppearHere)
                    {
                        const set<int>::iterator pmvItr =
                            actCost.preconditionsMeanViolating.find(depItr->dest.first);
                        ...
                        if (actsVisited.insert(currAct).second)
                        {
                            if (Globals::globalVerbosity & 32768)
                            {
                                cout << "\t\t" << COLOUR_yellow << " - Revisiting "
                                     << *(RPGBuilder::getInstantiatedOp(currAct.first));
                                if (currAct.second == Planner::E_AT_END)
                                {
                                    cout << ", end";
                                }
...
bool RPGHeuristic::Private::updateForPreferencesInBetterPositions(
    BuildingPayload *const payload, const list<pair<int, int>> &preferencePairedWithFactNowFreeToAdd,
    const EpsilonResolutionTimestamp &factLayerTime)
{
    set<pair<int, Planner::time_spec>> actsVisited;
    ...
    {
        const map<int, set<pair<int, Planner::time_spec>>>::iterator reduceCostItr =
            payload->preferenceWouldBeViolatedByAction.find(cfItr->first);
        if (reduceCostItr == payload->preferenceWouldBeViolatedByAction.end())
        {
            if (Globals::globalVerbosity & 32768)
            {
                cout << COLOUR_light_blue "\tNo actions have costs associated with "
                     << RPGBuilder::getPreferences()[cfItr->first].name << ":" << cfItr->first
                     << COLOUR_default << endl;
            }
            continue;
        }
        ...
        for (; actItr != actEnd; ++actItr)
        {
            ...
            ActionViolationData &actCost =
                (actItr->second == Planner::E_AT ?
                     (payload->tilCosts[currAct]) :
                     (actItr->second == Planner::E_AT_START ? payload->startActionCosts[currAct] :
                                                              payload->endActionCosts[currAct]));

            actCost.effectsMeanViolating.erase(cfItr->first);

            if (cfItr->second != -1)
            {
                payload->preferencePartsToSatisfyBeforeAction[*actItr].push_back(
                    make_pair(cfItr->first, (cfItr->second == 1)));
            }
            ...
            if (actCost.canBeApplied)
            {
                if (actItr->second == Planner::E_AT_START)
                {
                    canAppearHere = ((!payload->startPreconditionCounts[currAct] &&
                                      !payload->numericStartPreconditionCounts[currAct]) &&
                ...
                {
                    canAppearHere = ((!payload->endPreconditionCounts[currAct] &&
                                      !payload->numericEndPreconditionCounts[currAct]) &&
                ...
                else if (actItr->second == Planner::E_AT)
                {
                    ...
                    else
                    {
                        ...
                        {
                            const EpsilonResolutionTimestamp tilTS =
                                EpsilonResolutionTimestamp(tilTimes[currAct], false) - payload->stateTS;
                            if (tilTS <= factLayerTime)
                            {  // if the latest point at which the TIL can appear is before now
                                canAppearHere = false;
                            }
                        }
                    }
            ...
            if (canAppearHere)
            {
                if (actCost.preconditionsMeanViolating.find(cfItr->first) ==
                    actCost.preconditionsMeanViolating.end())
                {
...
void RPGHeuristic::Private::updateAdditiveCosts(BuildingPayload *const payload, const int &ci,
                                                const EpsilonResolutionTimestamp &deadline,
                                                const int &updateForGoal, double &costToUpdate)
{
    if (updateForGoal == -1)
    {
        return;
    ...
    for (double localCost; achievedByItr != achievedByEnd; ++achievedByItr)
    {
...

        {
            ...
            if (addItr == addEnd)
            {
                ...
                {
                    if (payload->startState.startedActions.find(*achievedByItr) ==
                            payload->startState.startedActions.end() ||
    ...
    if (localDebug)
    {
        cout << COLOUR_yellow << UAC << "- Additive cost of goal " << *(literalGoalVector[updateForGoal])
             << " on limit " << ci << " = " << costUpdate << COLOUR_default << endl;
    }
...
void RPGHeuristic::Private::calculateGoalCost(BuildingPayload *const payload, double *calculatedCost)
{
    ...
    for (int gID = 0; gID < lgc; ++gID, ++deadlineItr)
    {
        ...
        if (payload->startState.first.find(fID) != payload->startState.first.end())
        {
            if (localDebug & 2)
            {
                cout << COLOUR_light_blue << CGC << "goal fact" << *(literalGoalVector[gID])
                     << " is already true" << endl
                     << COLOUR_default;
            }
            ...
            if (limits[ci].optimisationLimit && addTheMaxCosts)
            {
                if (achiever.costs[ci] > 0.0)
                {
                    ...
                    if (localDebug & 2)
                    {
                        ...
                        cout << ci << ", " << achiever.costs[ci] - currentCosts[ci] << "; total is now "
                             << maxCosts[ci] << endl
                             << COLOUR_default;
                    }
                }
            ...
            else
            {
                if (achiever.costs[ci] > 0.0)
                {
                    if (mostExpensiveGID[ci] == -1 || maxCosts[ci] < achiever.costs[ci])
                    {
                        ...
                        updateAdditiveCosts(payload, ci, roundedDeadline, mostExpensiveGID[ci],
                                            additiveCosts[ci]);
    ...
    for (int ci = 0; ci < costCount; ++ci)
    {
        ...
        if (limits[ci].optimisationLimit && (Globals::bestSolutionQuality > -DBL_MAX || calculatedCost))
        {
            ...
            if (localDebug & 1)
            {
                cout << COLOUR_light_blue << CGC << "Optimisation limit, direct effect on limit is "
                     << localCost << ", split into " << additiveCosts[ci] << " additive costs, and "
                     << maxCosts[ci] << " max costs\n";
            }
            ...
            if (!RPGBuilder::getPreferences().empty())
            {
                if (RPGBuilder::getMetric()->minimise)
                {
                    localCost -= PreferenceHandler::getCurrentCost(
                        payload->startState.prefPreconditionViolations, payload->optimisticPrefStatus);
                }
                else
                {
                    localCost += PreferenceHandler::getCurrentCost(
                ...
                if (localDebug & 1)
                {
                    cout << ", with preferences is " << localCost << ", "
                         << PreferenceHandler::getCurrentViolations(payload->optimisticPrefStatus) << endl;
                }
            }
            if (localDebug & 1)
            {
        ...
        //         if (localCost == 0.0) {
        //             if (localDebug & 1) {
        //                 cout << COLOUR_light_blue << CGC << "final effect on limit " << ci << " is zero:
        //                 ignoring it\n" << COLOUR_default;
        //             }
        //
        ...
        if (limitToUse > -DBL_MAX)
        {
            if (localCost > 0.0)
            {
                switch (limits[ci].op)
                {
                    case E_GREATER:
                    {
                        if (localCost <= limitToUse)
                        {
                            if (localDebug & 1)
                            {
                                cout << COLOUR_light_red << CGC << "final effect on limit " << ci << " is "
                                     << localCost << ", limit is " << limitToUse << ", so too expensive\n"
                                     << COLOUR_default;
                            }
                ...
                if (localDebug & 1)
                {
                    cout << COLOUR_light_green << CGC << "final effect on limit " << ci << " is " << localCost
                         << ", limit is " << limitToUse << ", so okay\n"
                         << COLOUR_default;
                }
            }
...
void RPGHeuristic::findApplicableActions(const MinimalState &theState, const double &stateTime,
                                         list<ActionSegment> &applicableActions)
{
...
bool negativesAreOkay(const list<Literal *> &checkFor, const StateFacts &checkIn)
{
    ...
    for (; cfItr != cfEnd; ++cfItr)
    {
        if (checkIn.find((*cfItr)->getStateID()) != checkIn.end())
        {
            return false;
        }
    }
...
void RPGHeuristic::Private::findApplicableActions(const MinimalState &theState, const double &stateTime,
                                                  list<ActionSegment> &applicableActions)
{
    static const bool debug = (Globals::globalVerbosity & 16);
    ...
    {
        list<pair<int, Planner::time_spec>> &dependents =
            (pass ? *onlyNumericPreconditionActions : *preconditionlessActions);

        if (debug)
        {
            if (pass)
            {
                cout << "Considering the " << dependents.size()
                     << " actions with only numeric preconditions\n";
            }
            else
            {
                cout << "Considering the " << dependents.size()
                     << " propositionally preconditionless actions\n";
            }
        }
        ...
        for (; depItr != depEnd; ++depItr)
        {
            ...
            const bool nonMutex =
                RPGBuilder::stepNeedsToHaveFinished(candidate, theState, candidate.needToFinish);
...

    {
        ...
        for (; stateItr != stateEnd; ++stateItr)
        {
            if (debug)
            {
                cout << "Considering what benefits from " << *(RPGBuilder::getLiteral(FACTA(stateItr)))
                     << " being true\n";
            }

            list<pair<int, Planner::time_spec>> &dependents =
                (*processedPreconditionsToActions)[FACTA(stateItr)];
            ...
            for (; depItr != depEnd; ++depItr)
            {
                const int currAct = depItr->first;
                const Planner::time_spec startOrEnd = depItr->second;
                int &toManipulate = (startOrEnd == Planner::E_AT_START ? startPreconditionCounts[currAct] :
                                                                         endPreconditionCounts[currAct]);
    ...
    for (; fItr != fEnd; ++fItr)
    {
        ...
        vector<list<int>> *const toQuery =
            (fItr->second == Planner::E_AT_START ? actionsToProcessedStartNumericPreconditions :
                                                   actionsToNumericEndPreconditions);
        if (isApplicable)
        {
            ...
            for (; npItr != npEnd; ++npItr)
            {
                if (!((*rpgNumericPreconditions)[*npItr])
                         .isSatisfiedWCalculate(theState.secondMin, theState.secondMax))
                {
                    if (debug)
                    {
                        cout << (*rpgNumericPreconditions)[*npItr] << " isn't satisfied, so "
                             << *(fItr->first) << " isn't applicable\n";
                    }
                    isApplicable = false;
                    break;
                }
        ...
        /*        if (isApplicable && false) { //HACK
                    vector<list<int> > * const checkEffects = (fItr->second == Planner::E_AT_START ?
           actionsToRPGNumericStartEffects : actionsToRPGNumericEndEffects);

                    {
                        ...
                        for (; neItr != neEnd; ++neItr) {
                            ...
                            if (theState.fluentInvariants.find(((*rpgNumericEffects)[*neItr]).fluentIndex) !=
           theState.fluentInvariants.end()) { isApplicable = false; break;
                            }
        ...
        if (isApplicable)
        {
            ...
            if (fItr->second == Planner::E_AT_START)
            {
                ...
                {
                    if (RPGBuilder::isInteresting(fItr->first->getID(), theState.first,
                                                  theState.startedActions))
                    {
                    ...
                    else
                    {
                        if (debug)
                        {
                            cout << "Not okay to apply " << *fItr
                                 << ", as it would not be interesting to do so\n";
                        }
                    }
                }
                else
                {
                    ...
                    {
                        cout << "Not okay to apply " << *fItr
                             << ", as it is currently executing and the planner has been asked to forbid "
        ...
        for (; saItr != saEnd; ++saItr) {
            ...
            if (lEffs) {
                ...
                for (; saOneItr != saOneEnd && saOneItr->first < lLim; ++saOneItr) {
                    applicableActions.push_back(ActionSegment(getOp(saItr->first), VAL::E_OVER_ALL,
    saOneItr->first, emptyIntList));
                }
            }
        }
    }*/
...
bool RPGHeuristic::testApplicability(const MinimalState &theState, const double &stateTime,
                                     const ActionSegment &actID, bool fail, bool ignoreDeletes)
{
    return d->testApplicability(theState, stateTime, actID, fail, ignoreDeletes);
}

bool RPGHeuristic::Private::testApplicability(const MinimalState &theState, const double &stateTime,
                                              const ActionSegment &actID, const bool &fail,
                                              const bool &ignoreDeletes)
{
    if (actID.second == Planner::E_AT_START)
    {
        if (RPGBuilder::noSelfOverlaps)
        {
            ...
            {
                if (theState.startedActions.find(actID.first->getID()) != theState.startedActions.end())
                {
                    return false;
                }
            }
        ...
        {
            if (!TemporalAnalysis::okayToStart(actID.first->getID(), stateTime))
            {
                return false;
            }
        }
        ...
        if (!negativesAreOkay(
                RPGBuilder::getProcessedStartNegativePropositionalPreconditions()[actID.first->getID()],
                theState.first))
        {
            if (fail)
            {
        ...
        if (!ignoreDeletes)
        {
            ...
            for (; fItr != fEnd; ++fItr)
            {
                if (!((*rpgNumericPreconditions)[*fItr])
                         .isSatisfiedWCalculate(theState.secondMin, theState.secondMax))
                {
                    if (fail)
                    {
                        cerr << "Fatal internal error: precondition " << (*rpgNumericPreconditions)[*fItr]
                             << " not satisfied\n";
                        assert(false);
                    }
                    else
    ...
    else if (actID.second == Planner::E_AT_END)
    {
        ...
        {
            if (!TemporalAnalysis::okayToEnd(actID.first->getID(), stateTime))
            {
                return false;
            }
        }
    ...
    {  // til action
        ...
        for (; i <= actID.divisionID; ++i)
        {
            ...
            const bool passesPropositional =
                RPGBuilder::stepNeedsToHaveFinished(candidate, theState, candidate.needToFinish);
            if (!passesPropositional)
            {
                if (fail)
...
void RPGHeuristic::filterApplicableActions(const MinimalState &theState, const double &stateTime,
                                           list<ActionSegment> &applicableActions)
{
    return d->filterApplicableActions(theState, stateTime, applicableActions);
}

void RPGHeuristic::Private::filterApplicableActions(const MinimalState &theState, const double &stateTime,
                                                    list<ActionSegment> &applicableActions)
{
    ...
    if (filterDebug)
    {
        cout << "Filtering applicable actions\n";
    }
    ...
    if (filterDebug)
    {
        cout << "Input consists of " << toFilter.size() << " actions\n";
    }
    ...
    for (; tfItr != tfEnd; ++tfItr)
    {
        ...
        {
            if (!RPGBuilder::noSelfOverlaps ||
                theState.startedActions.find(tfItr->first->getID()) == theState.startedActions.end())
            {
                if (filterDebug)
                {
                    cout << "Considering start of " << *(tfItr->first) << "\n";
                }

                isApplicable =
                    RPGBuilder::isInteresting(tfItr->first->getID(), theState.first, theState.startedActions);

                if (isApplicable)
                {
                    isApplicable = TemporalAnalysis::okayToStart(tfItr->first->getID(), stateTime);
                }
                ...
                if (isApplicable)
                {
                    isApplicable = negativesAreOkay(
                        RPGBuilder::getProcessedStartNegativePropositionalPreconditions()[tfItr->first
                                                                                              ->getID()],
                        theState.first);
                }
            }
        ...
        {
            const map<int, set<int>>::const_iterator saItr =
                theState.startedActions.find(tfItr->first->getID());
            if (saItr != theState.startedActions.end() &&
                TemporalAnalysis::okayToEnd(tfItr->first->getID(), stateTime))
            {
                isApplicable = RPGBuilder::stepNeedsToHaveFinished(*tfItr, theState, tfItr->needToFinish);
            }
            else
            ...
            {
                isApplicable = negativesAreOkay(
                    RPGBuilder::getEndNegativePropositionalPreconditions()[tfItr->first->getID()],
                    theState.first);
            }
        }
    ...
    for (; tnfItr != tnfEnd; ++tnfItr)
    {
        ...
        if (filterDebug)
        {
            cout << "Considering numerically " << *(tnfItr->first) << "\n";
        }

        vector<list<int>> *const toQuery =
            (tnfItr->second == Planner::E_AT_START ? actionsToProcessedStartNumericPreconditions :
                                                     actionsToNumericEndPreconditions);
        {
            ...
            for (; npItr != npEnd; ++npItr)
            {
                ...
                else
                {
                    if (filterDebug)
                    {
                        cout << "\t" << (*rpgNumericPreconditions)[*npItr] << "satisfied\n";
                    }
                }
            }
        }
...
void RPGHeuristic::setGuidance(const char *config)
{
    ...
    else
    {
        ...
        cerr << "\t-gmakectsinstantaneous - make continuous effects instantaneous (as in the Colin IJCAI "
                "paper)\n";
        exit(1);
    }
}
