...
namespace Planner
{
    ...
    const char *positionName[8] = {"satisfied",
                                   "unsatisfied",
                                   "triggered",
                                   "unreachable",
                                   "eternally satisfied",
                                   "satisfied, but goal seen already and still holds",
                                   "satisfied, but if goal seen again, becomes unsatisfied",
                                   "probably satisfied/eternally satisfied"};
    ...
    {
        return (p == AutomatonPosition::satisfied || p == AutomatonPosition::eternallysatisfied ||
                p == AutomatonPosition::probablyeternallysatisfied ||
                p == AutomatonPosition::seenoncealreadyandstillholds ||
                p == AutomatonPosition::seenoncealready);
    }
        ...
        for (uint p = 0; p < taskPrefCount; ++p)
        {
            ...
            switch (prefs[p].cons)
            {
                ...
                case VAL::E_WITHIN:
                {
                    ...
                    {
                        // if within preferences are unreachable or initially satisfied, they'll never
                        // introduce dummy steps that need checking
                ...
                case VAL::E_ALWAYSWITHIN:
                {
                    ...
                    {
                        // if always-within preferences are unreachable, they'll never introduce dummy steps
                        // that need checking
                        temporalPreferences = true;
                    }
                    if (initialAutomataPositions[p] == AutomatonPosition::triggered)
...
#define CDPAIR(x, y, z)                                                                                      \
    ...
    {
        cout << "Populating lookup tables for the dependencies of preference " << pref->name << " (" <<
    prefIdx << ")\n";

        for (int pass = 0; pass < 2; ++pass) {


            int & toIncrement = (pass ? initialUnsatisfiedTriggerPreconditions :
    initialUnsatisfiedGoalPreconditions);
...

            {
                ...
                for (; factItr != factItrEnd; ++factItr) {
                    PreferenceHandler::mappingFromFactsToPreferences[(*factItr)->getID()].push_back(make_pair(prefIdx,
    (pass == 1)));
                    ++toIncrement;
                }
            ...
            }
            {
                ...
                const list<pair<int,int> > & relevantNumerics = (pass ? pref->triggerRPGNum :
    pref->goalRPGNum);

                {
                    cout << "Preference " << pref->name << " (" << prefIdx << ") has " <<
    relevantNumerics.size() << " numeric precondition pairs relevant to its "; if (pass) { cout <<
    "trigger\n"; } else { cout << "goal\n";
                    }
                }
                ...
                for (int cidx = 0; numItr != numEnd; ++numItr, ++cidx) {
                    ...
                    for (int pass = 0; pass < 2; ++pass) {
                        ...
                        PreferenceHandler::mappingFromNumericFactsToPreferences[preIdx].push_back(make_pair(prefIdx,
    (pass == 1)));
                        {
                            cout << (RPGBuilder::getNumericPreTable()[preIdx]) << " is relevant to preference
    " << pref->name << " (" << prefIdx << ")\n";
                        }
                        ++toIncrement;
                    }
    ...
    bool PreferenceFSA::goalOrTriggerIsSatisfied(const MinimalState *theState, const int &partIdx)
    {
        ...
        if (false)
        {  /// This assumes the domain does *not* use ADL - only uncomment it for debugging purposes
            ...
            if (secondOpinion.second)
            {
                ...
                for (; lItr != lEnd; ++lItr)
                {
                    if (!RPGBuilder::getNumericPreTable()[*lItr].isSatisfiedWCalculate(theState->secondMin,
                                                                                       theState->secondMax))
                    {
                        secondOpinion.second = false;
                        break;
        ...
        {
            cout << "Before looking at the current state, the preference " << pref->name << ":" << prefIdx
                 << " would be ";
    ...
    double cheapestRootedAt(list<Literal *> &answer, list<Literal *> &answerNegative,
                            list<int> *answerNumeric, const int &node, const NNF_Flat::Cell *const cells,
                            const bool *cellIsAnAnd, const int *parentIDs, const int &cellCount,
                            const EpsilonResolutionTimestamp &factLayerTime,
                            const vector<list<CostedAchieverDetails>> &literalCosts,
                            const vector<EpsilonResolutionTimestamp> &negativeLiteralAchievedInLayer,
                            const vector<EpsilonResolutionTimestamp> *numericAchievedInLayer)
    {
        ...
        if (cellIsAnAnd[node])
        {
            ...
            for (int c = node + 1; c < cellCount; ++c)
            {
                if (parentIDs[c] == node)
                {
                    const double childCost = cheapestRootedAt(
                        answer, answerNegative, answerNumeric, c, cells, cellIsAnAnd, parentIDs, cellCount,
                        factLayerTime, literalCosts, negativeLiteralAchievedInLayer, numericAchievedInLayer);
        ...
        else
        {
            ...
            for (int c = node + 1; c < cellCount; ++c)
            {
                if (parentIDs[c] == node)
                {
                    ...
                    list<int> tmpNum;
                    const double childCost = cheapestRootedAt(
                        tmp, tmpNegative, (answerNumeric ? &tmpNum : (list<int> *)0), c, cells, cellIsAnAnd,
                        parentIDs, cellCount, factLayerTime, literalCosts, negativeLiteralAchievedInLayer,
            ...
            if (toReturn != DBL_MAX)
            {
                ...
                answerNegative.insert(answerNegative.end(), incumbentNegative.begin(),
                                      incumbentNegative.end());
                if (answerNumeric)
                {
    ...
    void PreferenceFSA::satisfyAtLowCost(
        const NNF_Flat *flattened, const vector<list<CostedAchieverDetails>> &literalCosts,
        const vector<EpsilonResolutionTimestamp> &negativeLiteralAchievedInLayer,
        const vector<EpsilonResolutionTimestamp> *numericAchievedInLayer, list<list<Literal *>> &dest,
        list<list<Literal *>> &destNegative, list<list<int>> *destNumeric)
    {
        ...
        cheapestRootedAt(answer, answerNegative, answerNumeric, 0, cells, cellIsAnAnd, parentIDs, cellCount,
                         EpsilonResolutionTimestamp::infinite(), literalCosts, negativeLiteralAchievedInLayer,
                         numericAchievedInLayer);
    }

    void PreferenceFSA::getPreconditionsToSatisfy(
        list<Literal *> &literals, list<Literal *> &negativeLiterals, list<int> *numericPres,
        const bool &theTrigger, const EpsilonResolutionTimestamp &factLayerTime,
        const vector<list<CostedAchieverDetails>> &literalCosts,
        const vector<EpsilonResolutionTimestamp> &negativeLiteralAchievedInLayer,
        const vector<EpsilonResolutionTimestamp> *numericAchievedInLayer)
    {
        ...
        cheapestRootedAt(literals, negativeLiterals, numericPres, 0, cells, cellIsAnAnd, parentIDs, cellCount,
                         factLayerTime, literalCosts, negativeLiteralAchievedInLayer, numericAchievedInLayer);
    }
    ...
    namespace PrefCommon
    {
                                               ...
                                               PreferenceData *d, const int &partIdx)
        {
            ...
            if (nodeCount != 1)
            {
                ...
                {
                    cout << "Cannot find a single possible trigger - NNF tree contains " << nodeCount
                         << " interior nodes, rather than 1\n";
                }
                return;
            }
            ...
            if (cellIsAnAnd[0])
            {
                if (cellCount > 2)
                {
                    ...
                    if (PreferenceHandler::preferenceDebug || oneThingDebug)
                    {
                        cout << "Cannot find a single possible trigger - NNF tree has an AND at the root, "
                                "and "
                             << (cellCount - 1) << " leaves\n";
                    }
                    return;
            ...
            for (int ci = 1; ci < cellCount; ++ci)
            {
                ...
                if (cells[ci].lit)
                {
                    if (cells[ci].polarity)
                    {
                        ...
                        addingThisWouldTrigger.push_back(
                            ConditionAndPolarity(cells[ci].lit->getStateID(), false, true));
                    }
                    else
                    {
                        if (PreferenceHandler::preferenceDebug || oneThingDebug)
                        ...
                        addingThisWouldTrigger.push_back(
                            ConditionAndPolarity(cells[ci].lit->getStateID(), false, false));
                    }
                }
                else
                {
                                    ...
                                    const bool &polarity)
        {
...

            {
                ...
                else
                {
                    const RPGBuilder::ArtificialVariable &currAV =
                        RPGBuilder::getArtificialVariable(master.LHSVariable);
                ...
                for (int p = 0; p < loopLim; ++p)
                {
                    ...
                    else if (numPres[p].RHSVariable < (2 * PNECount))
                    {
                        const double flipped =
                            (numPres[p].RHSConstant != 0.0 ? -numPres[p].RHSConstant : 0.0);
                        if (masterVar == numPres[p].LHSVariable - PNECount &&
                            (op == VAL::E_LESS || op == VAL::E_LESSEQ))
                        {
                            const VAL::comparison_op flippedOp =
                                (numPres[p].op == VAL::E_GREATER ? VAL::E_LESS : VAL::E_LESSEQ);
                    ...
                    {
                        const RPGBuilder::ArtificialVariable &currAV =
                            RPGBuilder::getArtificialVariable(numPres[p].LHSVariable);
            ...
            static const int pneCount = RPGBuilder::getPNECount();
            {
                ...
                {
                    if (numEffs[e].fluentIndex != masterVar)
                    {
                        continue;
                    }
                    ...
                    {
                        if (condPass == 1 &&
                            NumericAnalysis::getNumericEffectsThatAreInConditionalEffects().find(e) ==
                                NumericAnalysis::getNumericEffectsThatAreInConditionalEffects().end())
                        {
                            continue;
                        }
                        const list<pair<int, Planner::time_spec>> &acts =
                            (condPass ? NumericAnalysis::getNumericEffectsThatAreInConditionalEffects()
                                            .find(e)
                                            ->second :
                        ...
                        for (; fitsInEveryCase && actItr != actEnd; ++actItr)
                        {
                            ...
                            const list<int> &actPres = (actItr->second == Planner::E_AT_END ?
                            ...
                            for (int pass = 0; fitsInEveryCase && pass < 2; ++pass)
                            {
                                ...
                                for (int term = 0; term < numEffs[e].size; ++term)
                                {
                                    ...
                                    switch (numEffs[e].variables[term])
                                    {
                                        case -3:
                                        {
                                            ...
                                            relevantBound =
                                                numEffs[e].weights[term] *
                                                (pass ? localBounds[-3].second : localBounds[-3].first);
                                            break;
                                        }
                                        case -19:
                                        {
                                            ...
                                            relevantBound =
                                                numEffs[e].weights[term] *
                                                (pass ? -localBounds[-3].first : -localBounds[-3].second);
                                            break;
                                        }
                                        default:
                                        {
                                            ...
                                            if (numEffs[e].variables[term] >= pneCount)
                                            {
                                                relevantBound =
                                                    -numEffs[e].weights[term] *
                                                    (pass ? localBounds[numEffs[e].variables[term] - pneCount]
                                                                .first :
                                                            localBounds[numEffs[e].variables[term] - pneCount]
                                                                .second);
                                            }
                                            else
                                            {
                                                relevantBound =
                                                    numEffs[e].weights[term] *
                                                    (pass ? localBounds[numEffs[e].variables[term]].second :
                                                            localBounds[numEffs[e].variables[term]].first);
                                            }
                                        }
                                    }
                                    if (localDebug)
                                    {
                                        cout << "   Contribution from term " << term << " = " << relevantBound
                                             << endl;
                                    }
                                ...
                                else
                                {
                                    const double preValue =
                                        (pass ? localBounds[masterVar].second : localBounds[masterVar].first);
                                    switch (op)
                                    {
                                        case VAL::E_GREATEQ:
                                        {
        ...
        void actionsWhichMeanWeHad(set<pair<int, Planner::time_spec>> &actVariables,
                                   list<ConditionAndPolarity> &addingThisWouldTriggerList)
        {
            ...
            for (; tItr != tEnd; ++tItr)
            {
                ...
                if (!addingThisWouldTrigger.second)
                {
                    ...
                    if (polarity)
                    {
                        const vector<list<pair<int, Planner::time_spec>>> &preToAction =
                            RPGBuilder::getPresToActions();

                        list<pair<int, Planner::time_spec>>::const_iterator wpItr =
                            preToAction[litID].begin();
                        const list<pair<int, Planner::time_spec>>::const_iterator wpEnd =
                            preToAction[litID].end();
                    ...
                    {
                        const vector<list<pair<int, Planner::time_spec>>> &eta =
                            (polarity ? RPGBuilder::getEffectsToActions() :
                                        RPGBuilder::getNegativeEffectsToActions());
                ...
                else
                {
                    ...
                    {
                        const vector<list<pair<int, Planner::time_spec>>> &numPreToAction =
                            RPGBuilder::getProcessedRPGNumericPreconditionsToActions();
                        ...
                        {
                            list<pair<int, Planner::time_spec>>::const_iterator wpItr =
                                numPreToAction[*prItr].begin();
                            const list<pair<int, Planner::time_spec>>::const_iterator wpEnd =
                                numPreToAction[*prItr].end();
                    ...
                    {
                        const vector<list<pair<int, Planner::time_spec>>> &eta =
                            RPGBuilder::getRpgNumericEffectsToActions();
    ...
    AlwaysFSA::AlwaysFSA(const int &idx, RPGBuilder::Constraint *const p,
                         AutomatonPosition::Position &initPosn)
      : PreferenceFSA(idx, p)
    {
    ...
    bool AlwaysFSA::update(MinimalState *state, const pair<bool, bool> &goalOrTriggerChanged,
                           const vector<double> &minTimestamps,
                           list<NNFPreconditionChooser::Supporters> &chosen)
    {
        ...
        if (goalOrTriggerIsSatisfied(state, 0))
        {
            ...
            if (!NNFPreconditionChooser::collectNNFDependencies(*state, pref->d->nodes[0].first,
                                                                chosen.back()))
            {
                cerr << "Fatal internal error: could not satisfy NNF tree of always preference that is "
                        "true\n";
        ...
        else
        {
            ...
            if (alwaysUpdateDebug || PreferenceHandler::preferenceDebug)
            {
                cout << "Preference " << pref->name << " is now violated\n";
            }
            return (pref->cost != DBL_MAX);
        }
    ...
    double AlwaysFSA::currentCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                  const int &flag)
    {
        if (!(goalType & flag))
        {
        ...
        if (preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            if (PreferenceHandler::preferenceDebug)
            {
                cout << pref->name << ") cost is " << pref->cost
                     << " because the always condition has been broken\n";
            }
            return pref->cost;
        }
    ...
    double AlwaysFSA::reachableCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                    const int &flag)
    {
        if (!(goalType & flag))
        {
    ...
    void AlwaysFSA::getCostsOfDeletion(const MinimalState &theState, map<int, set<int>> &factCost,
                                       map<int, map<double, set<int>>> &numChangeCost)
    {
        if (theState.preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            return;
        }
        ...
        for (int c = 0; c < cellCount; ++c)
        {
            ...
            else
            {
                if (cells[c].polarity)
                {
                    ...
                    {
                        double delta =
                            -currPre.RHSConstant - theState.secondMin[currPre.LHSVariable - varCount];
                        if (delta <= 0.0)
                        {
                            delta = 1e-6;
                        }
    ...
    bool AtEndFSA::update(MinimalState *state, const pair<bool, bool> &goalOrTriggerChanged,
                          const vector<double> &minTimestamps,
                          list<NNFPreconditionChooser::Supporters> &chosen)
    {
        if (state->preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            if (PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": update: at-end pref " << pref->name << " is unreachable, cost "
                     << pref->cost << "\n";
        ...
        if (goalOrTriggerIsSatisfied(state, 0))
        {
            if (PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": update: at-end pref " << pref->name << " is currently satisfied, cost "
                     << pref->cost << "\n";
            }

            // we don't need to put dummy steps in for at-ends, because if they're true at the end of the
            // plan, they're true.

            /*chosen.push_back(NNFPreconditionChooser::Supporters(prefIdx,false));
            if (!NNFPreconditionChooser::collectNNFDependencies(*state, pref->d->nodes[0].first,
            chosen.back())) { cerr << "Fatal internal error: could not satisfy NNF tree of at-end
            preference\n"; pref->d->nodes[0].first->write(cerr); cerr << endl; exit(1);
            }*/
        ...
        else
        {
            if (PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": update: at-end pref " << pref->name
                     << " is currently unsatisfied, cost " << pref->cost << "\n";
            }
    ...
    double AtEndFSA::currentCost(const vector<AutomatonPosition::Position> &preferenceStatus, const int &flag)
    {
        ...
        if (!(preferenceStatus[prefIdx] == AutomatonPosition::satisfied))
        {
            if (PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": at-end pref " << pref->name << " is currently unsatisfied, cost "
                     << pref->cost << "\n";
            }
            return pref->cost;
        }
        ...
        if (PreferenceHandler::preferenceDebug)
        {
            cout << prefIdx << ": at-end pref " << pref->name << " is currently satisfied\n";
        }
        return 0.0;
    }

    double AtEndFSA::reachableCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                   const int &flag)
    {
        if (!(goalType & flag))
        {
    ...
    void AtEndFSA::getDesiredGoals(list<list<Literal *>> &desired, list<list<Literal *>> &desiredNegative,
                                   list<list<int>> *desiredNumeric, const MinimalState &startState,
                                   const vector<vector<NNF_Flat *>> &unsatisfiedPreferenceConditions,
                                   const vector<list<CostedAchieverDetails>> &literalCosts,
                                   const vector<EpsilonResolutionTimestamp> &negativeLiteralAchievedInLayer,
                                   const vector<EpsilonResolutionTimestamp> *numericAchievedInLayer)
    {
        if (startState.preferenceStatus[prefIdx] == AutomatonPosition::satisfied ||
            startState.preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            return;
        }
        if (!unsatisfiedPreferenceConditions[prefIdx][0])
        {
            return;
        }
        if (!unsatisfiedPreferenceConditions[prefIdx][0]->isSatisfied())
        {
            return;
        }

        satisfyAtLowCost(unsatisfiedPreferenceConditions[prefIdx][0], literalCosts,
                         negativeLiteralAchievedInLayer, numericAchievedInLayer, desired, desiredNegative,
                         desiredNumeric);
    }

    void AtEndFSA::updateCosts(const MinimalState &initialState, const bool &wasTheTrigger,
    ...
    SometimeFSA::SometimeFSA(const int &idx, RPGBuilder::Constraint *const p,
                             AutomatonPosition::Position &initPosn)
      : PreferenceFSA(idx, p)
    {
    ...
    bool SometimeFSA::update(MinimalState *state, const pair<bool, bool> &goalOrTriggerChanged,
                             const vector<double> &minTimestamps,
                             list<NNFPreconditionChooser::Supporters> &chosen)
    {
            ...
            state->preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            if (sometimeUpdateDebug || PreferenceHandler::preferenceDebug)
            {
                if (state->preferenceStatus[prefIdx] == AutomatonPosition::eternallysatisfied)
                {
                    cout << prefIdx << ": update: sometime pref " << pref->name
                         << " has already been eternally satisfied, cost " << pref->cost << "\n";
                }
                else
                {
                    cout << prefIdx << ": update: sometime pref " << pref->name
                         << " cannot be satisfied from here, cost " << pref->cost << "\n";
                    assert(!goalOrTriggerIsSatisfied(state, 0));
                }
            }
            if (state->preferenceStatus[prefIdx] == AutomatonPosition::eternallysatisfied)
        ...
        if (goalOrTriggerIsSatisfied(state, 0))
        {
            ...
            {
                cout << prefIdx << ": update: sometime pref " << pref->name
                     << " is probably eternally satisfied, cost " << pref->cost << "\n";
            }
        ...
        else
        {
            if (sometimeUpdateDebug || PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": update: sometime pref " << pref->name << " is currently "
                     << positionName[state->preferenceStatus[prefIdx]] << ", cost " << pref->cost << "\n";
            }
        }
    ...
    double SometimeFSA::currentCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                    const int &flag)
    {
        if (!(goalType & flag))
        {
            ...
            preferenceStatus[prefIdx] != AutomatonPosition::satisfied)
        {
            if (PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": sometime pref " << pref->name << " is currently "
                     << positionName[preferenceStatus[prefIdx]] << ", cost " << pref->cost << "\n";
            }
            return pref->cost;
        }
        ...
        if (PreferenceHandler::preferenceDebug)
        {
            cout << prefIdx << ": sometime pref " << pref->name << " is satisfied\n";
        }
        return 0.0;
    }

    double SometimeFSA::reachableCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                      const int &flag)
    {
        if (!(goalType & flag))
        {
    ...
    void SometimeFSA::getUnsatisfiedConditionCounts(const MinimalState &state, vector<NNF_Flat *> &toFill)
    {
        if (state.preferenceStatus[prefIdx] == AutomatonPosition::satisfied ||
            state.preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            // Once we've had a sometime preference, or shown it to be unreachable, we don't need to try to
            // get it again
            return;
        }
        if (!pref->d->flattened[0])
    ...
    void
    SometimeFSA::getDesiredGoals(list<list<Literal *>> &desired, list<list<Literal *>> &desiredNegative,
                                 list<list<int>> *desiredNumeric, const MinimalState &startState,
                                 const vector<vector<NNF_Flat *>> &unsatisfiedPreferenceConditions,
                                 const vector<list<CostedAchieverDetails>> &literalCosts,
                                 const vector<EpsilonResolutionTimestamp> &negativeLiteralAchievedInLayer,
    ...
    void SometimeFSA::updateCosts(const MinimalState &initialState, const bool &wasTheTrigger,
                                  vector<AutomatonPosition::Position> &optimisticAutomataPositions,
                                  map<int, AddingConstraints> &prefCostOfAddingFact,
                                  map<int, map<double, AddingConstraints>> &numChangeCost,
                                  list<pair<int, int>> &preferencesThatNowHaveNoAddCosts)
    ...
    AtMostOnceFSA::AtMostOnceFSA(const int &idx, RPGBuilder::Constraint *const p,
                                 AutomatonPosition::Position &initPosn)
    ...
    bool AtMostOnceFSA::update(MinimalState *state, const pair<bool, bool> &goalOrTriggerChanged,
                               const vector<double> &minTimestamps,
                               list<NNFPreconditionChooser::Supporters> &chosen)
    {
        ...
        if (state->preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            ...
            {
                cout << prefIdx << ": update: at-most-once pref " << pref->name << ":" << prefIdx
                     << " cannot be satisfied from here, cost " << pref->cost << "\n";
            }
            return (pref->cost != DBL_MAX);
        }
        ...
        switch (state->preferenceStatus[prefIdx])
        {
            ...
            case AutomatonPosition::seenoncealreadyandstillholds:
            {
                if (allPresent)
                {
                    ...
                    {
                        cout << prefIdx << ": update: at-most-once pref " << pref->name << ":" << prefIdx
                             << " has been seen once, and still holds\n";
                    }
                    return true;
                }
                else
                {
                    ...
                    {
                        cout << prefIdx << ": update: at-most-once pref " << pref->name << ":" << prefIdx
                             << " has been seen once, but doesn't currently hold\n";
                    }
            ...
            case AutomatonPosition::seenoncealready:
            {
                if (allPresent)
                {
                    ...
                    {
                        cout << prefIdx << ": update: at-most-once pref " << pref->name << ":" << prefIdx
                             << " has been violated - now seen twice - cost = " << pref->cost << endl;
                    }
                    state->preferenceStatus[prefIdx] = AutomatonPosition::unreachable;
                    return (pref->cost != DBL_MAX);
                }
                else
                {
                    ...
                    {
                        cout << prefIdx << ": update: at-most-once pref " << pref->name << ":" << prefIdx
                             << " has been seen once, and still doesn't currently hold\n";
                    }
                    return true;
                }
            }
            default:
            {
                cerr << "Internal error - should not have preference status "
                     << positionName[state->preferenceStatus[prefIdx]] << " for the at-most-once preference "
                     << pref->name << ":" << prefIdx << endl;
                exit(1);
            }
        }
    ...
    double AtMostOnceFSA::currentCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                      const int &flag)
    {
        if (preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            ...
            if (PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": at-most-once pref " << pref->name << " cannot be satisfied, cost "
                     << pref->cost << "\n";
            }
            return pref->cost;
        }
        ...
        if (PreferenceHandler::preferenceDebug)
        {
            cout << prefIdx << ": at-most-once pref " << pref->name
                 << " is cost free: " << positionName[preferenceStatus[prefIdx]] << endl;
        }
        return 0.0;
    }

    double AtMostOnceFSA::reachableCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                        const int &flag)
    {
        if (preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
    ...
    void AtMostOnceFSA::getCostsOfAdding(const MinimalState &theState, map<int, AddingConstraints> &factCost,
                                         map<int, map<double, AddingConstraints>> &numChangeCost)
    {
        if (theState.preferenceStatus[prefIdx] != AutomatonPosition::seenoncealready)
        {
            return;
        }
        if (!addingOneThingCanTrigger)
                ...
                else
                {
                    ...
                    if (currAV.size == 1)
                    {
                        ...
                        else
                        {
                            ...
                            numChangeCost[currAV.fluents[0] - varCount][delta].addingWillViolate.insert(
                                prefIdx);
                        }
                    }
                }
    ...
    SometimeAfterFSA::SometimeAfterFSA(const int &idx, RPGBuilder::Constraint *const p,
                                       AutomatonPosition::Position &initPosn,
                                       const VAL::constraint_sort shouldBe)
      : PreferenceFSA(idx, p), triggerPartCount(0)
    {
    ...
    bool SometimeAfterFSA::update(MinimalState *state, const pair<bool, bool> &goalOrTriggerChanged,
                                  const vector<double> &minTimestamps,
                                  list<NNFPreconditionChooser::Supporters> &chosen)
    {
        ...
        if (state->preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            ...
            {
                cout << prefIdx << ": update: sometime-after pref " << pref->name
                     << " cannot be satisfied from here, cost " << pref->cost << "\n";
            }
            return (pref->cost != DBL_MAX);
        }
        ...
        if (state->preferenceStatus[prefIdx] == AutomatonPosition::satisfied)
        {
            if (goalOrTriggerChanged.second)
            {
                ...
                if (NNFPreconditionChooser::collectNNFDependencies(*state, pref->d->nodes[1].first,
                                                                   chosen.back()) != allPresent)
                {
                    if (allPresent)
                    {
                        cerr << "Fatal internal error: could not satisfy NNF tree trigger of a "
                                "sometime-after preference\n";
                    }
                    else
                    {
                        cerr << "Fatal internal error: could not unsatisfy NNF tree trigger of a "
                                "sometime-after preference\n";
                    }
        ...
        if (goalOrTriggerChanged.first || triggerChanged)
        {
                                                               ...
                                                               chosen.back()) != allPresent)
            {
                if (allPresent)
                {
                    cerr << "Fatal internal error: could not satisfy NNF tree goal of a sometime-after "
                            "preference\n";
                }
                else
                {
                    cerr << "Fatal internal error: could not unsatisfy NNF tree goal of a sometime-after "
                            "preference\n";
                }
                pref->d->nodes[0].first->write(cerr);
                cerr << endl;
            ...
            if (allPresent)
            {
                ...
                {
                    cout << prefIdx << ": update: sometime-after pref " << pref->name
                         << " is now satisfied, cost " << pref->cost << "\n";
                }
                state->preferenceStatus[prefIdx] = AutomatonPosition::satisfied;
            }
            else
            {
                ...
                {
                    cout << prefIdx << ": update: sometime-after pref " << pref->name
                         << " remains unsatisfied, cost " << pref->cost << "\n";
                }
            }
        }
    ...
    double SometimeAfterFSA::currentCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                         const int &flag)
    {
        if (preferenceStatus[prefIdx] == AutomatonPosition::triggered ||
            preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            if (!(goalType & flag))
            {
                return 0.0;
            ...
            if (PreferenceHandler::preferenceDebug)
            {
                if (preferenceStatus[prefIdx] == AutomatonPosition::triggered)
                {
                    cout << prefIdx << ": sometime-after pref " << pref->name
                         << " is currently unsatisfied, cost " << pref->cost << "\n";
                }
                else
                {
                    cout << prefIdx << ": sometime-after pref " << pref->name << " cannot be reached, cost "
                         << pref->cost << "\n";
                }
            }
            return pref->cost;
        ...
        if (PreferenceHandler::preferenceDebug)
        {
            cout << prefIdx << ": sometime-after pref " << pref->name << " is "
                 << positionName[preferenceStatus[prefIdx]] << endl;
        }
        return 0.0;
    }

    double SometimeAfterFSA::reachableCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                           const int &flag)
    {
        if (preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
    ...
    void SometimeAfterFSA::getUnsatisfiedConditionCounts(const MinimalState &state,
                                                         vector<NNF_Flat *> &toFill)
    {
        if (state.preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
    ...
    void SometimeAfterFSA::getCostsOfAdding(const MinimalState &theState,
                                            map<int, AddingConstraints> &factCost,
                                            map<int, map<double, AddingConstraints>> &numChangeCost)
    {
        if (theState.preferenceStatus[prefIdx] == AutomatonPosition::unreachable ||
            theState.preferenceStatus[prefIdx] == AutomatonPosition::triggered)
        {
            return;
        }
#ifdef TEMPORALSAFEAUTOMATONSEMANTICS
        if (theState.preferenceStatus[prefIdx] == AutomatonPosition::satisfied ||
            theState.preferenceStatus[prefIdx] == AutomatonPosition::probablyeternallysatisfied)
        {
            // Temporal safety: if sometime-after a b, then in the non-temporal case, we penalise a until b
            // appears. This isn't makespan-estimate-preserving, so don't do it in POPF.
            return;
        }
#endif
        if (!addingOneThingCanTrigger)
        ...
        if (allPresent)
        {
            /*if (addingThisWouldTrigger.second) {
            RPGBuilder::RPGNumericPrecondition & currPre =
        RPGBuilder::getNumericPreTable()[addingThisWouldTrigger.first]; cout << "No fear from " << pref->name
        << " of satisfying " << currPre << " - is already satisfied\n";
        }*/
            return;
        }
    ...
    void SometimeAfterFSA::updateCosts(const MinimalState &initialState, const bool &wasTheTrigger,
                                       vector<AutomatonPosition::Position> &optimisticAutomataPositions,
                                       map<int, AddingConstraints> &prefCostOfAddingFact,
                                       map<int, map<double, AddingConstraints>> &numChangeCost,
                                       list<pair<int, int>> &preferencesThatNowHaveNoAddCosts)
    {
...
#ifdef TEMPORALSAFEAUTOMATONSEMANTICS
        if (optimisticAutomataPositions[prefIdx] == AutomatonPosition::satisfied ||
            optimisticAutomataPositions[prefIdx] == AutomatonPosition::probablyeternallysatisfied)
        {
            // Temporal safety: if sometime-after a b, then in the non-temporal case, we penalise a until b
            // appears. This isn't makespan-estimate-preserving, so don't do it in POPF.
            return;
        ...
        for (; tItr != tEnd; ++tItr)
        {
            ...
            // cout << "Now no longer have to fear seeing the trigger of " << pref->name << " - the goal is
            // reachable\n";
            ...
            if (tItr->second)
            {
                ...
                if (currPre.LHSVariable < varCount)
                {
                    map<int, map<double, AddingConstraints>>::iterator varItr =
                        numChangeCost.find(currPre.LHSVariable);
                    if (varItr != numChangeCost.end())
                    {
                        ...
                        if (dItr != varItr->second.end())
                        {
                            ...
                            dItr->second.extraGoalsToAvoidViolations.push_back(
                                make_pair(prefIdx, wasTheTrigger));
                        }
                    }
                }
                else if (currPre.LHSVariable < 2 * varCount)
                {
                    double delta =
                        -currPre.RHSConstant - initialState.secondMax[currPre.LHSVariable - varCount];
                    if (delta <= 0.0)
                    {
                        delta = 1e-6;
                    }
                    ...
                    map<int, map<double, AddingConstraints>>::iterator varItr =
                        numChangeCost.find(currPre.LHSVariable - varCount);
                    if (varItr != numChangeCost.end())
                    {
                        map<double, AddingConstraints>::iterator dItr = varItr->second.find(delta);
                        if (dItr != varItr->second.end())
                ...
                else
                {
                    ...
                    if (currAV.size == 1)
                    {
                        ...
                        if (currAV.fluents[0] < varCount)
                        {
                            ...
                            map<int, map<double, AddingConstraints>>::iterator varItr =
                                numChangeCost.find(currAV.fluents[0]);
                            if (varItr != numChangeCost.end())
                            {
                                map<double, AddingConstraints>::iterator dItr = varItr->second.find(delta);
                                if (dItr != varItr->second.end())
                        ...
                        else
                        {
                            ...
                            map<int, map<double, AddingConstraints>>::iterator varItr =
                                numChangeCost.find(currAV.fluents[0] - varCount);
                            if (varItr != numChangeCost.end())
                            {
                                map<double, AddingConstraints>::iterator dItr = varItr->second.find(delta);
                                if (dItr != varItr->second.end())
    ...
    void SometimeAfterFSA::getDesiredGoals(
        list<list<Literal *>> &desired, list<list<Literal *>> &desiredNegative,
        list<list<int>> *desiredNumeric, const MinimalState &startState,
        const vector<vector<NNF_Flat *>> &unsatisfiedPreferenceConditions,
        const vector<list<CostedAchieverDetails>> &literalCosts,
        const vector<EpsilonResolutionTimestamp> &negativeLiteralAchievedInLayer,
        const vector<EpsilonResolutionTimestamp> *numericAchievedInLayer)
    {
        if (startState.preferenceStatus[prefIdx] != AutomatonPosition::triggered)
        {
            return;
        }
        if (!unsatisfiedPreferenceConditions[prefIdx][0])
    ...
    SometimeBeforeFSA::SometimeBeforeFSA(const int &idx, RPGBuilder::Constraint *const p,
                                         AutomatonPosition::Position &initPosn)
      : PreferenceFSA(idx, p), triggerPartCount(0)
    {
    ...
    bool SometimeBeforeFSA::update(MinimalState *state, const pair<bool, bool> &goalOrTriggerChanged,
                                   const vector<double> &minTimestamps,
                                   list<NNFPreconditionChooser::Supporters> &chosen)
    {
        ...
        if (state->preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            ...
            {
                cout << prefIdx << ": update: sometime-before pref " << pref->name
                     << " cannot be satisfied from here, cost " << pref->cost << "\n";
            }
            return (pref->cost != DBL_MAX);
        }
        ...
        if (state->preferenceStatus[prefIdx] == AutomatonPosition::eternallysatisfied)
        {
            ...
            {
                cout << prefIdx << ": update: sometime-before pref " << pref->name
                     << " cannot be unsatisfied from here, cost " << pref->cost << "\n";
            }
            return true;
        }
        ...
        if (goalOrTriggerChanged.second)
        {
            ...
            if (state->preferenceStatus[prefIdx] == AutomatonPosition::probablyeternallysatisfied)
            {
                                                                   ...
                                                                   chosen.back()) != allPresent)
                {
                    if (allPresent)
                    {
                        cerr << "Fatal internal error: could not satisfy trigger of sometime-before "
                                "preference\n";
                    }
                    else
                    {
                        cerr << "Fatal internal error: could not unsatisfy trigger of sometime-before "
                                "preference\n";
                    }
                ...
                if (allPresent)
                {
                    ...
                    {
                        cout << prefIdx << ": update: sometime-before pref " << pref->name
                             << " is okay so long as the earlier satisfaction of its goal holds\n";
                    }
                }
            }
            else
            {
                if (allPresent)
                {
                    ...
                    {
                        cout << prefIdx << ": update: sometime-before pref " << pref->name
                             << " is now broken forever, cost " << pref->cost << "\n";
                    }
                    state->preferenceStatus[prefIdx] = AutomatonPosition::unreachable;
                    return (pref->cost != DBL_MAX);
                }
        ...
        if (goalOrTriggerChanged.first)
        {
                                                               ...
                                                               chosen.back()) != allPresent)
            {
                if (allPresent)
                {
                    cerr << "Fatal internal error: could not satisfy NNF goal of sometime-before "
                            "preference\n";
                }
                else
                {
                    cerr << "Fatal internal error: could not unsatisfy NNF goal of sometime-before "
                            "preference\n";
                }
                pref->d->nodes[0].first->write(cerr);
                cerr << endl;
            ...
            if (allPresent)
            {
                ...
                {
                    cout << prefIdx << ": update: sometime-before pref " << pref->name
                         << " has triggered, probably eternally satisfied\n";
                }
                state->preferenceStatus[prefIdx] = AutomatonPosition::probablyeternallysatisfied;
            }
            else
            {
                if (sometimeBeforeUpdateDebug || PreferenceHandler::preferenceDebug)
                {
                    cout << prefIdx << ": update: sometime-before pref " << pref->name << " still "
                         << positionName[state->preferenceStatus[prefIdx]] << endl;
                }
            }
        }
        return true;
    ...
    double SometimeBeforeFSA::currentCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                          const int &flag)
    {
        if (preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            ...
            if (PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": sometime-before pref " << pref->name << " cannot be satisfied, cost "
                     << pref->cost << "\n";
            }
            return pref->cost;
        }
        ...
        if (PreferenceHandler::preferenceDebug)
        {
            cout << prefIdx << ": sometime-before pref " << pref->name << " is satisfied\n";
        }
        return 0.0;
    }

    double SometimeBeforeFSA::reachableCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                            const int &flag)
    {
        if (preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
    ...
    void SometimeBeforeFSA::getUnsatisfiedConditionCounts(const MinimalState &state,
                                                          vector<NNF_Flat *> &toFill)
    {
        if (state.preferenceStatus[prefIdx] == AutomatonPosition::unreachable ||
            state.preferenceStatus[prefIdx] == AutomatonPosition::eternallysatisfied)
        {
    ...
    void SometimeBeforeFSA::getCostsOfAdding(const MinimalState &theState,
                                             map<int, AddingConstraints> &factCost,
                                             map<int, map<double, AddingConstraints>> &numChangeCost)
    {
        if (theState.preferenceStatus[prefIdx] != AutomatonPosition::satisfied)
        ...
        for (; tItr != tEnd; ++tItr)
        {
            ...
            {
                // cout << "Adding " << *(RPGBuilder::getLiteral(tItr->first)) << " would kill " << pref->name
                // << endl;
                factCost[tItr->first].addingWillViolate.insert(prefIdx);
            }
        }
    ...
    void SometimeBeforeFSA::updateCosts(const MinimalState &initialState, const bool &wasTheTrigger,
                                        vector<AutomatonPosition::Position> &optimisticAutomataPositions,
                                        map<int, AddingConstraints> &prefCostOfAddingFact,
                                        map<int, map<double, AddingConstraints>> &numChangeCost,
                                        list<pair<int, int>> &preferencesThatNowHaveNoAddCosts)
    {
        ...
        for (; tItr != tEnd; ++tItr)
        {
            ...
            {
                // cout << "Now, adding " << *(RPGBuilder::getLiteral(tItr->first)) << " won't kill " <<
                // pref->name << endl;
    ...
    WithinFSA::WithinFSA(const int &idx, RPGBuilder::Constraint *const p,
                         AutomatonPosition::Position &initPosn)
      : PreferenceFSA(idx, p)
    {
    ...
    bool WithinFSA::update(MinimalState *state, const pair<bool, bool> &goalOrTriggerChanged,
                           const vector<double> &minTimestamps,
                           list<NNFPreconditionChooser::Supporters> &chosen)
    {
            ...
            state->preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            if (withinUpdateDebug || PreferenceHandler::preferenceDebug)
            {
                if (state->preferenceStatus[prefIdx] == AutomatonPosition::eternallysatisfied)
                {
                    cout << prefIdx << ": update: within pref " << pref->name
                         << " has already been eternally satisfied, cost " << pref->cost << "\n";
                }
                else
                {
        ...
        if (goalOrTriggerIsSatisfied(state, 0))
        {
            ...
            const pair<bool, double> satisfiedAt = NNFPreconditionChooser::satisfyNNFEarly(
                *state, minTimestamps, pref->d->nodes[0].first, ignore);
            if (satisfiedAt.second < pref->deadline)
            {
                if (withinUpdateDebug || PreferenceHandler::preferenceDebug)
                {
                    cout << COLOUR_light_green << prefIdx << ": update: within pref " << pref->name
                         << " is now satisfied at " << satisfiedAt.second << ", cost " << pref->cost
                         << COLOUR_default << "\n";
                }
                state->preferenceStatus[prefIdx] = AutomatonPosition::satisfied;
            }
            else
            {
                if (withinUpdateDebug || PreferenceHandler::preferenceDebug)
                {
                    cout << COLOUR_light_red << prefIdx << ": update: within pref " << pref->name
                         << " was satisfied now, but definitely not soon enough, so removing this option"
                         << COLOUR_default << "\n";
                }
                chosen.pop_back();
            }
        ...
        else
        {
            if (withinUpdateDebug || PreferenceHandler::preferenceDebug)
            {
                cout << COLOUR_yellow << prefIdx << ": update: within pref " << pref->name << " is currently "
                     << positionName[state->preferenceStatus[prefIdx]] << ", cost " << pref->cost
                     << COLOUR_default << "\n";
            }
        }
    ...
    double WithinFSA::currentCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                  const int &flag)
    {
        if (!(goalType & flag))
        {
            if (PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": within pref " << pref->name
                     << " and the current flag are not related, taking cost as 0\n";
            }
            return 0.0;
        }
            ...
            preferenceStatus[prefIdx] != AutomatonPosition::probablyeternallysatisfied)
        {
            if (PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": within pref " << pref->name << " is currently "
                     << positionName[preferenceStatus[prefIdx]] << ", current cost " << pref->cost << "\n";
            }
            return pref->cost;
        }
        ...
        if (PreferenceHandler::preferenceDebug)
        {
            cout << prefIdx << ": within pref " << pref->name << " is satisfied, current cost 0\n";
        }
        return 0.0;
    }

    double WithinFSA::reachableCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                    const int &flag)
    {
        if (!(goalType & flag))
        {
    ...
    void WithinFSA::getUnsatisfiedConditionCounts(const MinimalState &state, vector<NNF_Flat *> &toFill)
    {
        if (state.preferenceStatus[prefIdx] == AutomatonPosition::satisfied ||
            state.preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            // Once we've had a within preference, or shown it to be unreachable, we don't need to try to get
            // it again
            return;
        }
        if (!pref->d->flattened[0])
    ...
    void WithinFSA::updateCosts(const MinimalState &initialState, const bool &wasTheTrigger,
                                vector<AutomatonPosition::Position> &optimisticAutomataPositions,
                                map<int, AddingConstraints> &prefCostOfAddingFact,
                                map<int, map<double, AddingConstraints>> &numChangeCost,
                                list<pair<int, int>> &preferencesThatNowHaveNoAddCosts)
    ...
    void WithinFSA::becomesUnreachableAtTime(const MinimalState &state,
                                             map<EpsilonResolutionTimestamp, list<int>> &unreachableAtTime)
    {
        if (state.preferenceStatus[prefIdx] == AutomatonPosition::satisfied ||
            state.preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
    ...
    AlwaysWithinFSA::AlwaysWithinFSA(const int &idx, RPGBuilder::Constraint *const p,
                                     AutomatonPosition::Position &initPosn)
      : SometimeAfterFSA(idx, p, initPosn, VAL::E_ALWAYSWITHIN)
    {
        // for now we only support always-withins where a single step triggers them, and definitely triggers
        // them this means the trigger is either a single fact, a single negative fact, or a numeric
        // precondition dependent on a single variable that is never subject to continuous numeric effects

        // TODO check these conditions hold
    }

    bool AlwaysWithinFSA::update(MinimalState *state, const pair<bool, bool> &goalOrTriggerChanged,
                                 const vector<double> &minTimestamps,
                                 list<NNFPreconditionChooser::Supporters> &chosen)
    {
        const bool sometimeAfterUpdateDebug = false;
        ...
        if (state->preferenceStatus[prefIdx] == AutomatonPosition::unreachable)
        {
            if (sometimeAfterUpdateDebug || PreferenceHandler::preferenceDebug)
            {
                cout << prefIdx << ": update: always-within pref " << pref->name
                     << " cannot be satisfied from here, cost " << pref->cost << "\n";
            }
            return (pref->cost != DBL_MAX);
        }
        ...
        if (state->preferenceStatus[prefIdx] == AutomatonPosition::satisfied)
        {
            ...
            if (goalOrTriggerChanged.second)
            {
                                                                   ...
                                                                   chosen.back()) != allPresent)
                {
                    if (allPresent)
                    {
                        cerr << "Fatal internal error: could not satisfy NNF tree trigger of an "
                                "always-within preference\n";
                    }
                    else
                    {
                        cerr << "Fatal internal error: could not unsatisfy NNF tree trigger of an "
                                "always-within preference\n";
                    }
                    pref->d->nodes[1].first->write(cerr);
                    cerr << endl;
        ...
        assert(state->preferenceStatus[prefIdx] == AutomatonPosition::triggered ||
               state->preferenceStatus[prefIdx] == AutomatonPosition::probablyeternallysatisfied);
        ...
        if (goalOrTriggerChanged.first || triggerChanged)
        {
            ...
            if (requirementPresent)
            {
                ...
                {
                    cout << prefIdx << ": update: always-within pref " << pref->name
                         << " is now probably satisfied, cost " << pref->cost << "\n";
                }
                state->preferenceStatus[prefIdx] = AutomatonPosition::probablyeternallysatisfied;
                chosen.push_back(NNFPreconditionChooser::Supporters(prefIdx, false));
                if (!NNFPreconditionChooser::collectNNFDependencies(*state, pref->d->nodes[0].first,
            ...
            else
            {
                if (sometimeAfterUpdateDebug || PreferenceHandler::preferenceDebug)
                {
                    cout << prefIdx << ": update: always-within pref " << pref->name << " remains "
                         << positionName[state->preferenceStatus[prefIdx]] << ", cost " << pref->cost << "\n";
                }
            }
        }
    ...
    void AlwaysWithinFSA::getCostsOfAdding(const MinimalState &theState,
                                           map<int, AddingConstraints> &factCost,
                                           map<int, map<double, AddingConstraints>> &numChangeCost)
    {
    ...
    void AlwaysWithinFSA::updateCosts(const MinimalState &initialState, const bool &wasTheTrigger,
                                      vector<AutomatonPosition::Position> &optimisticAutomataPositions,
                                      map<int, AddingConstraints> &prefCostOfAddingFact,
                                      map<int, map<double, AddingConstraints>> &numChangeCost,
                                      list<pair<int, int>> &preferencesThatNowHaveNoAddCosts)
    ...
    void
    AlwaysWithinFSA::getDesiredGoals(list<list<Literal *>> &desired, list<list<Literal *>> &desiredNegative,
                                     list<list<int>> *desiredNumeric, const MinimalState &startState,
                                     const vector<vector<NNF_Flat *>> &unsatisfiedPreferenceConditions,
                                     const vector<list<CostedAchieverDetails>> &literalCosts,
                                     const vector<EpsilonResolutionTimestamp> &negativeLiteralAchievedInLayer,
    ...
    bool PreconditionPref::update(MinimalState *state, const pair<bool, bool> &,
                                  const vector<double> &minTimestamps,
                                  list<NNFPreconditionChooser::Supporters> &chosen)
    {
        if (!goalOrTriggerIsSatisfied(state, 0))
        {
            if (PreferenceHandler::preferenceDebug)
            {
                cout << "Precondition preference " << pref->name << " not satisfied, cost " << pref->cost
                     << endl;
            }
        ...
        else
        {
            if (PreferenceHandler::preferenceDebug)
            {
                cout << "Precondition preference " << pref->name << " satisfied, cost " << pref->cost << endl;
            }
            chosen.push_back(NNFPreconditionChooser::Supporters(prefIdx, false));
            if (!NNFPreconditionChooser::collectNNFDependencies(*state, pref->d->nodes[0].first,
                                                                chosen.back()))
    ...
    double PreconditionPref::currentCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                         const int &flag)
    {
        exit(1);
        return DBL_MAX;
    ...
    double PreconditionPref::reachableCost(const vector<AutomatonPosition::Position> &preferenceStatus,
                                           const int &flag)
    {
        exit(1);
        return DBL_MAX;
    ...
    void PreconditionPref::getUnsatisfiedConditionCounts(const MinimalState &state,
                                                         vector<NNF_Flat *> &toFill)
    {
        if (pref->d->flattened[0])
        {
                                   ...
                                   list<NNFPreconditionChooser::Supporters> &chosen)
    {
        if (PreferenceHandler::preferenceDebug)
        {
            cout << "Updating preference automata following action application\n";
        }
        ...
        for (int pass = 0; pass < 2; ++pass)
        {
            ...
            for (; fItr != fEnd; ++fItr)
            {
                ...
                list<LiteralCellDependency<pair<int, bool>>>::const_iterator depItr =
                    preconditionsToPrefs[*fItr].begin();
                const list<LiteralCellDependency<pair<int, bool>>>::const_iterator depEnd =
                    preconditionsToPrefs[*fItr].end();
                ...
                for (; depItr != depEnd; ++depItr)
                {
                    ...
                    if (PreferenceHandler::preferenceDebug)
                    {
                        cout << " " << RPGBuilder::getPreferences()[depItr->dest.first].name << ":"
                             << depItr->dest.first;
        ...
        for (int pass = 0; pass < 2; ++pass)
        {
            ...
            {
                list<LiteralCellDependency<pair<int, bool>>>::const_iterator depItr =
                    negativePreconditionsToPrefs[*fItr].begin();
                const list<LiteralCellDependency<pair<int, bool>>>::const_iterator depEnd =
                    negativePreconditionsToPrefs[*fItr].end();
...

        {
            ...
            for (; vItr != vEnd; ++vItr)
            {
                ...
                {
                    const list<int> &affectedA =
                        RPGBuilder::affectsRPGNumericPreconditions(*vItr + offset * pneCount);
                    numericPres.insert(affectedA.begin(), affectedA.end());

                    const list<int> &affectedAVs =
                        RPGBuilder::getVariableDependencies(*vItr + offset * pneCount);
            ...
            {
                list<LiteralCellDependency<pair<int, bool>>>::const_iterator depItr =
                    numericPreconditionsToPrefs[*fItr].begin();
                const list<LiteralCellDependency<pair<int, bool>>>::const_iterator depEnd =
                    numericPreconditionsToPrefs[*fItr].end();
                ...
                for (; depItr != depEnd; ++depItr)
                {
                    ...
                    if (depItr->dest.second)
                    {
                        ...
                        if (PreferenceHandler::preferenceDebug)
                        {
                            cout << "  - Revisiting trigger of "
                                 << RPGBuilder::getPreferences()[depItr->dest.first].name << ":"
                                 << depItr->dest.first << " due to "
                                 << RPGBuilder::getNumericPreTable()[*fItr] << endl;
                        }
                    }
                    else
                    {
                        if (PreferenceHandler::preferenceDebug)
                        {
                            cout << "  - Revisiting requirement of "
                                 << RPGBuilder::getPreferences()[depItr->dest.first].name << ":"
                                 << depItr->dest.first << " due to "
                                 << RPGBuilder::getNumericPreTable()[*fItr] << endl;
                        }
        ...
        for (int p = 0; p < taskPrefCount; ++p)
        {
            ...
            const bool shouldBeTrue =
                automata[p]->update(&state,
                                    make_pair(RPGBuilder::getPreferences()[p].d->flattened[0],
                                              RPGBuilder::getPreferences()[p].d->flattened[1]),
                                    minTimestamps, ignoreChosen);
            if (before != state.preferenceStatus[p] || !shouldBeTrue)
            {
                cerr << COLOUR_light_red << "When double-checking " << RPGBuilder::getPreferences()[p].name
                     << " the position changed from " << positionName[before] << " to "
                     << positionName[state.preferenceStatus[p]];
                if (!shouldBeTrue)
                {
    ...
    double PreferenceHandler::getCurrentCost(const int &specificPreference, const MinimalState &state,
                                             const int flag)
    {
    ...
    double PreferenceHandler::getCurrentCost(const int &specificPreference,
                                             const vector<AutomatonPosition::Position> &positions,
                                             const int flag)
    {
        if (automata[specificPreference])
        {
    ...
    double PreferenceHandler::getCurrentCost(const double &prefPreconditionViolations,
                                             const vector<AutomatonPosition::Position> &positions,
                                             const int flag)
    {
    ...
    double PreferenceHandler::getReachableCost(const int &specificPreference, const MinimalState &state,
                                               const int flag)
    {
        if (automata[specificPreference])
        {
    ...
    double PreferenceHandler::getReachableCost(const int &specificPreference,
                                               const vector<AutomatonPosition::Position> &positions,
                                               const int flag)
    {
        if (automata[specificPreference])
    ...
    double PreferenceHandler::markUnreachables(MinimalState &theState, const list<int> &toMark)
    {
        ...
        for (; tmItr != tmEnd; ++tmItr)
        {
            ...
            if (*tmItr < tpCount)
            {
                if (theState.preferenceStatus[*tmItr] == AutomatonPosition::unreachable)
                {
                    continue;
                }
    ...
    void PreferenceHandler::getUnsatisfiedConditionCounts(
        const MinimalState &theState, vector<vector<NNF_Flat *>> &toFill,
        map<EpsilonResolutionTimestamp, list<int>> &unreachableAtTime)
    {
    ...
    void PreferenceHandler::getCostsOfDeletion(const MinimalState &theState,
                                               map<int, set<int>> &prefCostOfDeletingFact,
                                               map<int, map<double, set<int>>> &prefCostOfChangingNumber)
    {
        const int tpCount = RPGBuilder::getTaskPrefCount();
    ...
    void PreferenceHandler::getCostsOfAdding(
        const MinimalState &theState, map<int, AddingConstraints> &prefCostOfDeletingFact,
    ...
    void PreferenceHandler::updateCostsAndPreferenceStatus(
        const MinimalState &initialState, const pair<int, bool> &whatHasBeenSatisfied,
        vector<AutomatonPosition::Position> &optimisticAutomataPositions,
        map<int, AddingConstraints> &prefCostOfAddingFact,
        map<int, map<double, AddingConstraints>> &prefCostOfChangingNumberB,
        list<pair<int, int>> &preferencesThatNowHaveNoAddCosts)
    {
        ...
        if (whatHasBeenSatisfied.first >= tpCount)
        {
            return;
        }

        assert(automata[whatHasBeenSatisfied.first]);
        automata[whatHasBeenSatisfied.first]->updateCosts(
            initialState, whatHasBeenSatisfied.second, optimisticAutomataPositions, prefCostOfAddingFact,
            prefCostOfChangingNumberB, preferencesThatNowHaveNoAddCosts);
        if (preferenceDebug)
        {
            cout << "\t\tAfter that, optimistic preference status is "
                 << positionName[optimisticAutomataPositions[whatHasBeenSatisfied.first]] << endl;
        }
    }

    void PreferenceHandler::getDesiredGoals(
        list<list<Literal *>> &desired, list<list<Literal *>> &desiredNegative,
        list<list<int>> *desiredNumeric, const MinimalState &startState,
        const vector<vector<NNF_Flat *>> &unsatisfiedPreferenceConditions,
        ...
        const vector<EpsilonResolutionTimestamp> *numericAchievedInLayer)
    {
        ...
        for (int sv = 0; sv < tpCount; ++sv)
        {
            ...
            if (!canBeSatisfied(startState.preferenceStatus[sv]))
            {
                continue;
            }
            if (!isSatisfied(optimisticAutomatonPositions[sv]))
            {
                continue;
            }

            automata[sv]->getDesiredGoals(desired, desiredNegative, desiredNumeric, startState,
                                          unsatisfiedPreferenceConditions, literalCosts,
                                          negativeLiteralAchievedInLayer, numericAchievedInLayer);
        }
    }
    ...
    void PreferenceHandler::substituteRPGNumericPreconditions(
                ...
                if (oldRoot)
                {
                    ...
                    switch (prefs[i].cons)
                    {
                        ...
                        case VAL::E_ATMOSTONCE:
                        {
                            ...
                            desirable = true;
                            // undesirable if we've not yet tripped it, or have tripped it in the past, and
                            // hence keeping it false is a good idea
                            undesirable = true;
                            break;
                        }
                        case VAL::E_SOMETIMEAFTER:
                    ...
                    prefs[i].d->nodes[part] =
                        NNFUtils::substituteRPGNumerics(oldRoot, desirable, undesirable, commonData);
                }
            }
        }
    ...
    void PreferenceHandler::flattenNNF()
    {
        ...
        for (unsigned int i = 0; i < prefCount; ++i)
        {
            for (int part = 0; part < 2; ++part)
            {
                ...
                list<pair<int, int>> &destNum =
                    (part ? prefs[i].debug_triggerRPGNum : prefs[i].debug_goalRPGNum);
    ...
    void PreferenceHandler::collectRequiredFactsInNNF(const int &pref, const bool &wasTheTrigger,
                                                      std::set<int> &requiredPositive,
                                                      std::set<int> &requiredNegative)
    {
    ...
    void PreferenceHandler::collectAllFactsInNNF(const int &pref, const bool &wasTheTrigger,
                                                 std::set<int> &requiredPositive,
                                                 std::set<int> &requiredNegative)
    {
    ...
    bool PreferenceHandler::couldBeBeneficialToOppose(const int &p, const bool &wasTheTrigger)
    {
        ...
        switch (prefs[p].cons)
        {
            ...
            case VAL::E_ALWAYSWITHIN:
            {
                // it can be beneficial to avoid the trigger of a sometime after to avoid having to reach the
                // requirement, or to avoid the trigger of a sometime before to avoid violating the preference
                // if the requirement has not yet been seen
    ...
    void PreferenceHandler::initPTRTable()
    {
        ...
        for (unsigned int i = 0; i < prefCount; ++i)
        {
            ...
            for (int part = 0; part < 2; ++part)
            {
                ...
                if (oldRoot)
                {
                    NNFUtils::buildLiteralsToPreconditions<pair<int, bool>>(
                        d->flattened[part], preconditionsToPrefs, negativePreconditionsToPrefs,
                        numericPreconditionsToPrefs, d->conditionLiterals[part],
                        d->conditionNegativeLiterals[part], d->conditionNums[part], currReference);

                    d->flattened[part]->reset();
                    if (d->flattened[part]->isSatisfied())
                    {
                        defaultTruePrefParts.insert(currReference);
                    }
                    ...
                    for (int c = 0; c < cellCount; ++c)
                    {
                        ...
                        const RPGBuilder::RPGNumericPrecondition &currRNP =
                            RPGBuilder::getNumericPreTable()[rnp];
        ...
        if (preferenceDebug)
        {
            for (unsigned int l = 0; l < litCount; ++l)
            {
                ...
                {
                    list<LiteralCellDependency<pair<int, bool>>>::iterator depItr =
                        (isneg ? negativePreconditionsToPrefs[l].begin() : preconditionsToPrefs[l].begin());
                    const list<LiteralCellDependency<pair<int, bool>>>::iterator depEnd =
                        (isneg ? negativePreconditionsToPrefs[l].end() : preconditionsToPrefs[l].end());
    ...
    void PreferenceHandler::getPreconditionsToSatisfy(
        list<Literal *> &literals, list<Literal *> &negativeLiterals, list<int> *numPres,
        ...
        const vector<EpsilonResolutionTimestamp> *numericAchievedInLayer)
    {
        ...
        automata[satisfied.first]->getPreconditionsToSatisfy(
            literals, negativeLiterals, numPres, satisfied.second, factLayerTime, literalCosts,
            negativeLiteralAchievedInLayer, numericAchievedInLayer);
    }

    void PreferenceHandler::aboutToApply(MinimalState &theState, const ActionSegment &act,
                                         const vector<double> &minTimestamps,
                                         list<NNFPreconditionChooser::Supporters> &chosen)
    {
        ...
        const vector<int> &prePrefs =
            (act.second == Planner::E_AT_START ? RPGBuilder::getStartPreferences()[act.first->getID()] :
        ...
        for (int p = 0; p < ppCount; ++p)
        {
            ...
            {
                cerr << "Fatal internal error: precondition preference update returned false, i.e. it was "
                        "somehow in the :constraints section\n";
                exit(1);
            }
        }
    }
    ...
    bool
    PreferenceHandler::constraintsAreSatisfied(const vector<AutomatonPosition::Position> &automataPositions)
    {
