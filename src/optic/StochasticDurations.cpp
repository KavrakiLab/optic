...
namespace Planner
{
    ...
    {
        cerr << "Found a state dependent duration on " << *op << ", based on " << *pne
             << ".  Due to lazy programming, these are not currently supported.\n";
        exit(1);
    }
    ...
    protected:
        ...
        struct StochasticDurationAndNumberOfTimes
        {
            ...
            StochasticDurationAndNumberOfTimes() : distributionID(-1)
            {
            }
            ...
            {
                if (distributionID < s.distributionID)
                {
                    return true;
                }
                if (distributionID > s.distributionID)
                ...
                if (rateParameter < s.rateParameter)
                {
                    return true;
                }
                if (rateParameter > s.rateParameter)
                ...
                if (numberOfTimes < s.numberOfTimes)
                {
                    return true;
                }
        ...
        /*double sampleTimestamp(vector<FFEvent*> & planVec, const int & stepID, const TemporalConstraints *
        const orderings, const bool & deterministicSoFar) {
            ...
            if (planVec[stepID]->time_spec == VAL::E_AT_START) {
                ...
                for (; cItr != cEnd; ++cItr) {
                    if (deterministicSoFar) {
                        newLatest = (cItr->second ? EPSILON : 0.0) +
        planVec[cItr->first]->stochasticTimestamp->getTimestampForRPGHeuristic(); //sampleTimestamp(planVec,
        cItr->first, orderings); } else { newLatest = (cItr->second ? EPSILON : 0.0) +
        sampleTimestamp(planVec, cItr->first, orderings, false);
                    }
            ...
            for (; cItr != cEnd; ++cItr) {
                if (cItr->first == planVec[stepID]->pairWithStep) {
                    ...
                    if (debug) {
                        cout << " -> corresponding start " << cItr->first << " was at " << newLatest << ", so
        with duration = ";
                    }
        ...
        double sampleSeveralTimestamps(vector<FFEvent *> &planVec, const int &stepID,
                                       const TemporalConstraints *const orderings,
                                       const bool &deterministicSoFar)
        {
            if (stepTimestampSampleDirtyBits[stepID] == currentDirtyBit)
            {
                if (debug)
                {
                    cout << stepID << ": Reusing previous result for step " << stepID << ", returning "
                         << stepTimestampSample[stepID] << endl;
                }
            ...
            if (!cons)
            {
                if (debug)
                {
                    cout << stepID << ": No precedence constraints on step " << stepID << ", returning 0.0"
                         << endl;
                }
                ...
                for (; cItr != cEnd; ++cItr)
                {
                    if (deterministicSoFar)
                    {
                        newLatest = (cItr->second ? EPSILON : 0.0) +
                                    planVec[cItr->first]
                                        ->stochasticTimestamp
                                        ->getTimestampForRPGHeuristic();  // sampleTimestamp(planVec,
                                                                          // cItr->first,
                                                                          // orderings);
                    }
                    else
                    {
                        newLatest = (cItr->second ? EPSILON : 0.0) +
                                    sampleSeveralTimestamps(planVec, cItr->first, orderings, false);
                    }
            ...
            for (; cItr != cEnd; ++cItr)
            {
                if (cItr->first == planVec[stepID]->pairWithStep)
                {
                    ...
                    if (debug)
                    {
                        cout << " -> corresponding start " << cItr->first << " was at " << newLatest
                             << ", so with duration = ";
                    }
                    newLatest += sampleDuration(planVec[stepID]->action->getID());
                    if (debug)
        ...
        double expectedTimestamp(vector<FFEvent *> &planVec, const int &stepID,
                                 const TemporalConstraints *const orderings)
        {
            const map<int, bool> *const cons = orderings->stepsBefore(stepID);
            ...
            if (planVec[stepID]->time_spec == VAL::E_AT_START)
            {
                ...
                for (; cItr != cEnd; ++cItr)
                {
                    ...
                    {
                        newLatest = (cItr->second ? EPSILON : 0.0) +
                                    planVec[cItr->first]->stochasticTimestamp->getTimestampForRPGHeuristic();
                    }
                    else
                    {
                        newLatest = (cItr->second ? EPSILON : 0.0) +
                                    expectedTimestamp(planVec, cItr->first, orderings);
                    }
        ...
        void usage()
        {
            ...
            cerr << "-Mmontecarlo                 - 5000 samples for goal state CDFs; expected durations "
                    "otherwise\n";
            cerr << "-Mmontecarlo[n]              - [n] samples for goal state CDFs; expected durations "
                    "otherwise\n";
            cerr << "-Mmontecarlo[n1][n2]         - [n1] samples for goal state CDFs; [n2] samples "
                    "otherwise\n";
            cerr << "-Mmontecarlo[n1][n2][yes|no] - If 'no', performs the same as -Mmontecarlo[n1][n2].  "
                    "Otherwise,\n";
            cerr << "                               also uses goal sampling for unit goals in non-goal "
                    "states.\n";
        }
    ...
    public:
        MonteCarloDurationManager(const bool &relax)
          : nonGoalSamples(-1)
          , samples(5000)
          , onlyEverUseExpectedValues(relax)
          , useGoalSamplingForUnitGoalAchievement(false)
          , temporaryDistributionSize(0)
        {
            debug = false;
        ...
        MonteCarloDurationManager(const string &params)
          : nonGoalSamples(-1)
          , samples(5000)
          , onlyEverUseExpectedValues(false)
          , useGoalSamplingForUnitGoalAchievement(false)
          , temporaryDistributionSize(0)
        {
            resizeTemporaryStores(10);
        ...
        bool migrateMaxDeadlineConstraints(
            map<double, map<int, double>> &compressionSafeEndEffectOfStepsAtTime,
            double &effectMagnitudeExcluded, list<pair<list<int>, double>> &maxDeadlineConstraints,
            const double &deadline, bool pushedYet)
        {
            ...
            {
                map<double, map<int, double>>::const_iterator csItr =
                    compressionSafeEndEffectOfStepsAtTime.begin();
                const map<double, map<int, double>>::const_iterator csEnd =
                    compressionSafeEndEffectOfStepsAtTime.end();
                                     ...
                                     list<pair<list<int>, double>> &maxDeadlineConstraints)
        {
...

            {
                ...
                for (int fID; gItr != gEnd; ++gItr, ++gdItr, ++gID)
                {
                        ...
                        if (fItr != newState.first.end())
                        {
                            if (prevState.first.find(fID) == prevState.first.end())
                            {
                                ...
                                else
                                {
                                    if (!useGoalSamplingForUnitGoalAchievement)
                                    {
                                        newState.literalGoalHoldsFromStep(gID,
                                                                          fItr->second.availableFrom.stepID);
                                    }

                                    const map<int, double>::iterator insItr =
                                        deadlineConstraints
                                            .insert(make_pair(fItr->second.availableFrom.stepID, *gdItr))
                                            .first;
                            ...
                            {
                                litGoalAlreadyPresent[gID] = true;
                                assert(useGoalSamplingForUnitGoalAchievement ||
                                       newState.stepFromWhichLiteralGoalsHold);
                            }
                        }
                        else
                        {
                    ...
                    {
                        // then, if we don't have it, see if it's been deleted, as that restricts when it
                        // could be added again

                        const map<int, PropositionAnnotation>::const_iterator fItr =
                            newState.retired.find(fID);

                        if (fItr != newState.retired.end())
                        {
                            map<StepAndBeforeOrAfter, bool>::const_iterator afterItr =
                                fItr->second.addableFrom.begin();
                            const map<StepAndBeforeOrAfter, bool>::const_iterator afterEnd =
                                fItr->second.addableFrom.end();

                            for (; afterItr != afterEnd; ++afterItr)
                            {
                                const map<int, double>::iterator insItr =
                                    deadlineConstraints.insert(make_pair(afterItr->first.stepID, *gdItr))
                                        .first;
                ...
                for (int fID; gItr != gEnd; ++gItr, ++gdItr)
                {
                    for (int ipass = 0; ipass < 2; ++ipass, ++gID)
                    {
                        ...
                        const RPGBuilder::RPGNumericPrecondition &currPre =
                            RPGBuilder::getNumericPreTable()[fID];
                        ...
                        if (currPre.isSatisfiedWCalculate(prevState.secondMin, prevState.secondMax))
                        {
                            ...
                            assert(useGoalSamplingForUnitGoalAchievement ||
                                   (prevState.stepsFromWhichNumericGoalsHold &&
                                    prevState.stepsFromWhichNumericGoalsHold[gID]));
                            continue;
                        }
...

                        {
                            ...
                            // first, we forbid the inclusion of compression-safe end numeric effects from the
                            // end of an action that has just been started
                            ...
                            if (var >= 2 * varCount)
                            {
                                ...
                                for (int i = 0; i < currAV.size; ++i)
                                {
                                    ...
                                    const int stepID =
                                        newState.temporalConstraints->lastStepToTouchPNE[varTwo]
                                            .lastInstantaneousEffect;
                                    if (stepID >= 0)
                                    {
                                    ...
                                    pushed = migrateMaxDeadlineConstraints(
                                        newState.temporalConstraints->lastStepToTouchPNE[varTwo]
                            ...
                            else
                            {
                                ...
                                const int stepID = newState.temporalConstraints->lastStepToTouchPNE[var]
                                                       .lastInstantaneousEffect;
                                if (stepID >= 0)
                                {
                                    trueFromStep.push_back(stepID);
                                ...
                                pushed = migrateMaxDeadlineConstraints(
                                    newState.temporalConstraints->lastStepToTouchPNE[var]
                                        .compressionSafeEndEffectOfStepsAtTime,
                            ...
                            if (effectMagnitudeExcluded != 0.0)
                            {
                                ...
                                if (var >= 2 * varCount)
                                {
                                    ...
                                    stateValue = currAV.evaluateWCalculate(newState.secondMin,
                                                                           newState.secondMax, varCount);
                                }
                                ...
                                const bool stillSatisfied = (currPre.op == VAL::E_GREATER ?
                                                                 valueWithExclusions > currPre.RHSConstant :
                                                                 valueWithExclusions >= currPre.RHSConstant);

                                if (!stillSatisfied)
                                {
                                    // we know anything excluded is the end of the compression safe action
                                    // just applied, the effects of which are available after the
                                    // corresponding end (whose step index is one higher than the start)

                                    // cout << "Needed to include the CS end effect of " <<
                                    // *(planVec[appliedStepID]->action) << " to satisfy " << currPre << endl;
                        ...
                        if (pushed)
                        {
                            maxDeadlineConstraints.back().first.insert(
                                maxDeadlineConstraints.back().first.end(), trueFromStep.begin(),
                                trueFromStep.end());
                        }
                        else
                        {
                            ...
                            else if (tfsSize == 1)
                            {
                                ...
                                {
                                    const map<int, double>::iterator insItr =
                                        deadlineConstraints.insert(make_pair(*scItr, *gdItr)).first;
                        ...
                        assert(useGoalSamplingForUnitGoalAchievement ||
                               (newState.stepsFromWhichNumericGoalsHold &&
                                newState.stepsFromWhichNumericGoalsHold[gID]));
                    }
                }
            }
            ...
            if (allGoalsMet && !useGoalSamplingForUnitGoalAchievement)
            {
...

                {
                    ...
                    for (int gID = 0; gItr != gEnd; ++gItr, ++gdItr, ++gID)
                    {
                        ...
                        {
                            const map<int, double>::iterator insItr =
                                deadlineConstraints.insert(make_pair(stepID, *gdItr)).first;
                    ...
                    for (int fID; gItr != gEnd; ++gItr, ++gdItr)
                    {
                        for (int ipass = 0; ipass < 2; ++ipass, ++gID)
                        {
                            ...
                            if (orderAfter[0] == 1)
                            {
                                ...
                                const map<int, double>::iterator insItr =
                                    deadlineConstraints.insert(make_pair(orderAfter[1], *gdItr)).first;
        ...
        /*bool staysWithinGoalDeadlines(const MinimalState & prevState, const MinimalState & newState, const
        ...
        newState.retired.find(fID);
                        ...
                        if (fItr != newState.retired.end()) {
                            ...
                            map<StepAndBeforeOrAfter, bool>::const_iterator afterItr =
        fItr->second.addableFrom.begin(); const map<StepAndBeforeOrAfter, bool>::const_iterator afterEnd =
        fItr->second.addableFrom.end();
                            ...
                            for (; afterItr != afterEnd; ++afterItr) {
                                ...
                                if (step->stochasticTimestamp->getTimestampForRPGHeuristic() > *gdItr) {
                                    ...
                                    // if the condition holds, then the earliest point at which it could be
        added,
        ...
        RPGBuilder::getArtificialVariable(var);
                                ...
                                for (int i = 0; i < currAV.size; ++i) {
                                    ...
                                    const int stepID =
        newState.temporalConstraints->lastStepToTouchPNE[varTwo].lastInstantaneousEffect; if (stepID >= 0) {
        ...
        { return false;
                                ...
                                const int stepID =
        newState.temporalConstraints->lastStepToTouchPNE[var].lastInstantaneousEffect; if (stepID >= 0) {
        ...
        template <typename T>
        void findAllBefore(const int &stepID, int &maxStepID, list<int> &toVisit,
                           const TemporalConstraints *const orderings, T tmpItr, const T &stepItrEnd,
         ...
         *
         * @param theseAreGoalSteps  If <code>true</code> use goal-quality sampling for the steps; otherwise,
         * use the other method specified (expected values, sampling, etc.)
         */
        template <typename T>
                                            ...
                                            const bool &theseAreGoalSteps)
        {
            ...
            {
                // if these aren't goal-achieving steps, and we aren't using sampling for non-goal steps, plug
                // in expected values and call it good.
            ...
            for (; stepItr != stepItrEnd; ++stepItr)
            {
                if (planVec[stepItr->first]->stochasticTimestamp &&
                    (!theseAreGoalSteps ||
                     dynamic_cast<SinglePercentileData *>(planVec[stepItr->first]->stochasticTimestamp)
                         ->hasBeenSampledToGoalAccuracy))
                {
                    if (localdebug)
                    {
                        cout << COLOUR_yellow << "Already have samples of the time to reach step "
                             << stepItr->first << " with the necessary accuracy\n"
                             << COLOUR_default;
                    }
            ...
            while (!numberOfStepsBeforeIt.empty())
            {
                ...
                assert(biggestItr->first <= -1 || !planVec[biggestItr->first]->stochasticTimestamp ||
                       !dynamic_cast<SinglePercentileData *>(planVec[biggestItr->first]->stochasticTimestamp)
                            ->hasBeenSampledToGoalAccuracy);
...

                {
                    ...
                    for (; duItr != duEnd; ++duItr)
                    {
                        if (!planVec[*duItr]->stochasticTimestamp ||
                            !dynamic_cast<SinglePercentileData *>(planVec[*duItr]->stochasticTimestamp)
                                 ->hasBeenSampledToGoalAccuracy)
                        {
                            if (localdebug)
                            {
                ...
                for (; duItr != duEnd; ++duItr)
                {
                    ...
                    planVec[*duItr]->stochasticTimestamp =
                        new SinglePercentileData(dItr->first, theseAreGoalSteps);
                    ...
                    if (localdebug)
                    {
                        cout << "> Sampled CDF for step " << *duItr << " at percentile is " << dItr->first
                             << endl;
                    }
                }
            }
        ...
        bool updateTimestampsOfNewPlanStep(const MinimalState &prevState, MinimalState &newState,
                                           ...
                                           bool &isAGoalState)
        {
            ...
            if (newStep->time_spec == VAL::E_AT_START)
            {
                ...
                if (newStepPairedWith)
                {
                    ...
                    const map<int, bool> *const consTwo =
                        newState.temporalConstraints->stepsBefore(stepID + 1);
                    ...
                    if (simple)
                    {
                        ...
                        newStepPairedWith->stochasticTimestamp =
                            new SinglePercentileData(durCalc, !onlyEverUseExpectedValues);
                        if (localdebug)
                        {
                            cout << "- Corresponding end is simply the duration after it = "
                                 << newStepPairedWith->stochasticTimestamp->getTimestampForRPGHeuristic()
                                 << endl;
                        }
                    }
                    else
            ...
            for (unsigned int s = 0; s < stepCount; ++s)
            {
                ...
                if (relevantStepIDs.find(s) != relevantStepIDs.end())
                {
                    continue;
                }
            ...
            isAGoalState = findDeadlineConstraints(prevState, newState, planVec, stepID, deadlineConstraints,
                                                   maxDeadlineConstraints) &&
                           newState.startedActions.empty();
            ...
            if (!haveAnyConstraints || (!useGoalSamplingForUnitGoalAchievement && !isAGoalState))
            {
                ...
                const bool retVal = updateSeveralTimestampsQuickly(
                    stepsThatNeedUpdating.begin(), stepsThatNeedUpdating.end(), maxDeadlineConstraints,
                ...
                {
                    {
                        ...
                        {
                            if (planVec[dcItr->first]->stochasticTimestamp->getTimestampForRPGHeuristic() >
                                dcItr->second)
                            {
                                return false;
                            }
                    ...
                    {
                        list<pair<list<int>, double>>::const_iterator mdcItr = maxDeadlineConstraints.begin();
                        const list<pair<list<int>, double>>::const_iterator mdcEnd =
                            maxDeadlineConstraints.end();
                        ...
                        for (; mdcItr != mdcEnd; ++mdcItr)
                        {
                            ...
                            for (; sItr != sEnd; ++sItr)
                            {
                                if (planVec[*sItr]->stochasticTimestamp->getTimestampForRPGHeuristic() >
                                    mdcItr->second)
                                {
                                    return false;
                                }
            ...
            const bool checkVal = updateSeveralTimestampsQuickly(
                deadlineConstraints.begin(), deadlineConstraints.end(), maxDeadlineConstraints, planVec,
                newState.temporalConstraints, !onlyEverUseExpectedValues);
...

            {
                ...
                for (; dcItr != dcEnd; ++dcItr)
                {
                    if (localdebug)
                    {
                        cout << "Samplied timestamp of step " << dcItr->first << " = " << dcItr->second
                             << endl;
                    }
                    if (planVec[dcItr->first]->stochasticTimestamp->getTimestampForRPGHeuristic() >
                        dcItr->second)
            ...
            return updateSeveralTimestampsQuickly(tmpMap.begin(), tmpMap.end(), maxDeadlineConstraints,
                                                  planVec, newState.temporalConstraints, false);
        };
        ...
        virtual void prepareTheInitialState(MinimalState &initialState)
        {
            if (useGoalSamplingForUnitGoalAchievement)
            {
                // we only need to do this to mark initially-true goals as 'past achieved' (persisting up to
                // the point at which they're destroyed) for if we're only looking whether the deadlines have
                // been met in complete goal states
...

            {
                ...
                for (int fID; gItr != gEnd; ++gItr, ++gdItr, ++gID)
                {
                    ...
                    // if we get this far, we have the goal, either because it's static, or in the initial
                    // state
    ...
    void setSolutionDeadlineTimeToLatestGoal()
    {
        ...
        for (int pass = 0; pass < 2; ++pass)
        {
            const list<double> &loopOver =
                (pass ? RPGBuilder::getNumericRPGGoalDeadlines() : RPGBuilder::getLiteralGoalDeadlines());
        ...
        */
...

        {
            ...
            while (!current_in_stream.eof())
            {
                ...
                if (!(buffer1 >> bucketID))
                {
                    cout << "Exiting: in distribution file " << filename << ", cannot interpret " << bucket
                         << " as a numeric value for a PDF bucket\n";
                }
                ...
                {
                    cout << "Exiting: in distribution file " << filename << ", cannot interpret "
                         << entryCount << " as an integral value for the size of a PDF bucket\n";
                }
    ...
    public:
        GSLAnalyticPDFSingleParameter(double (*sampleFn)(const gsl_rng *, double),
                                      double (*cdfFn)(double, double), const double &parameterIn,
                                      const double &ev)
          : GSLAnalyticPDF(), parameter(parameterIn), sampleFunction(sampleFn), cdfFunction(cdfFn)
        {
            expectedValue = ev;
        }
    ...
    public:
        GSLAnalyticScaledPDFSingleParameter(double (*sampleFn)(const gsl_rng *, double),
                                            double (*cdfFn)(double, double), const double &parameterIn,
                                            const double &multiplier, const double &ev)
          : GSLAnalyticPDF()
          , scale(multiplier)
          , parameter(parameterIn)
          , sampleFunction(sampleFn)
          , cdfFunction(cdfFn)
        {
            expectedValue = ev;
        }
    ...
    public:
        GSLAnalyticPDFTwoParameter(double (*sampleFn)(const gsl_rng *, double, double),
                                   double (*cdfFn)(double, double, double), const double &parameterAIn,
                                   const double &parameterBIn, const double &ev)
          : GSLAnalyticPDF()
          , parameterA(parameterAIn)
          , parameterB(parameterBIn)
          , sampleFunction(sampleFn)
          , cdfFunction(cdfFn)
        {
            expectedValue = ev;
    ...
    void initialiseDistributions()
    {
        ...
        for (; pneItr != pneEnd; ++pneItr)
        {
            ...
            const double pneValue =
                EFT(currPNE->getHead())->getInitial(currPNE->begin(), currPNE->end()).second;

            if (currPNE->getHead()->getName().find("exponential") == 0)
            {
                PDFsForVariable[currPNE->getGlobalID()] = new GSLAnalyticPDFSingleParameter(
                    &gsl_ran_exponential, &gsl_cdf_exponential_Pinv, pneValue, pneValue);

                if (verbose)
                {
                    cout << "Initialised an exponential distribution based on " << *currPNE << ", rate "
                         << pneValue << endl;
                }
            }
            else if (currPNE->getHead()->getName().find("stochastic-gaussian-mean") == 0)
            {
                const string restOfName = currPNE->getHead()->getName().substr(
                    24);  // 24 = number of characters in 'stochastic-gaussian-mean'
                ...
                for (; pne2Itr != pneEnd; ++pne2Itr)
                {
                    ...
                    if (standardDeviation->getHead()->getName().find("stochastic-gaussian-"
                                                                     "standarddeviation") == 0)
                    {
                        // cout << "Name is " << standardDeviation->getHead()->getName() << ", length " <<
                        // standardDeviation->getHead()->getName().size() << endl;
                        const string restOfThisName = standardDeviation->getHead()->getName().substr(
                            37);  // 37 = number of characters in 'stochastic-gaussian-standarddeviation'

                        if (restOfThisName == restOfName)
                        {
                            VAL::LiteralParameterIterator<VAL::parameter_symbol_list::const_iterator>
                                meanItr = currPNE->begin();
                            const VAL::LiteralParameterIterator<VAL::parameter_symbol_list::const_iterator>
                                meanEnd = currPNE->end();

                            VAL::LiteralParameterIterator<VAL::parameter_symbol_list::const_iterator> sdItr =
                                standardDeviation->begin();
                            const VAL::LiteralParameterIterator<VAL::parameter_symbol_list::const_iterator>
                                sdEnd = standardDeviation->end();
                ...
                if (pne2Itr == pneEnd)
                {
                    cerr << "Fatal error: found a Gaussian distribution mean function, "
                         << currPNE->getHead()->getName()
                         << ", but no standard deviation function stochastic-gaussian-standarddeviation"
                         << restOfName << endl;
                    exit(1);
                }

                const double sd = EFT(standardDeviation->getHead())
                                      ->getInitial(standardDeviation->begin(), standardDeviation->end())
                                      .second;

                PDFsForVariable[currPNE->getGlobalID()] = new GSLAnalyticScaledPDFSingleParameter(
                    &gsl_ran_gaussian, &gsl_cdf_gaussian_Pinv, sd / pneValue, pneValue, pneValue);

                if (verbose)
                {
                    cout << "Initialised a Gaussian distribution based on " << *currPNE
                         << ", mean = " << pneValue << ", standard deviation " << *standardDeviation << " = "
                         << sd << endl;
                }
            }
            ...
            {
                const string restOfName = currPNE->getHead()->getName().substr(
                    24);  // 24 = number of characters in 'stochastic-uniform-lower'
                ...
                for (; pne2Itr != pneEnd; ++pne2Itr)
                {
                    ...
                    if (standardDeviation->getHead()->getName().find("stochastic-uniform-upper") == 0)
                    {
                        ...
                        // standardDeviation->getHead()->getName().size() << endl;
                        const string restOfThisName = standardDeviation->getHead()->getName().substr(
                            24);  // 37 = number of characters in 'stochastic-uniform-upper'
                ...
                if (pne2Itr == pneEnd)
                {
                    cerr << "Fatal error: found a uniform distribution lower-bound function, "
                         << currPNE->getHead()->getName() << ", but could not find stochastic-uniform-upper"
                         << restOfName << endl;
                    exit(1);
                }

                const double upper = EFT(standardDeviation->getHead())
                                         ->getInitial(standardDeviation->begin(), standardDeviation->end())
                                         .second;

                PDFsForVariable[currPNE->getGlobalID()] = new GSLAnalyticPDFTwoParameter(
                    &gsl_ran_flat, &gsl_cdf_flat_Pinv, pneValue, upper, (pneValue + upper) / 2);

                if (verbose)
                {
                    cout << "Initialised a uniform distribution based on " << *currPNE << ", range = ["
                         << pneValue << "," << upper << "]" << endl;
                }
            }
            ...
            {
                VAL::LiteralParameterIterator<VAL::parameter_symbol_list::const_iterator> paramItr =
                    currPNE->begin();
                const VAL::LiteralParameterIterator<VAL::parameter_symbol_list::const_iterator> paramEnd =
                    currPNE->end();

                if (paramItr == paramEnd)
                {
                    cerr << "Error: stochastic-external function type should take exactly 1 argument, a "
                            "filename.\n";
                    exit(1);
                }
