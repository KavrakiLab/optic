...
namespace Planner
{
    ...
    public:
        ...
        StepAndBeforeOrAfter(const BeforeOrAfter &bOrA, const unsigned int &stepIDin)
          : realBeforeOrAfter(bOrA)
          , realStepID(stepIDin)
          , beforeOrAfter(realBeforeOrAfter)
          , stepID(realStepID)
        {
        }
        ...
        bool operator<(const StepAndBeforeOrAfter &o) const
        {
            ...
            if (beforeOrAfter == BEFORE && o.beforeOrAfter == AFTER)
            {
                return true;
            }
            return false;
        ...
        bool operator>(const StepAndBeforeOrAfter &o) const
        {
            ...
            if (beforeOrAfter == AFTER && o.beforeOrAfter == BEFORE)
            {
                return true;
            }
            return false;
    ...
    struct StepAndBeforeOrAfter
    {
        ...
        StepAndBeforeOrAfter() : beforeOrAfter(BEFORE), stepID(0)
        {
        }
...
#define UNSAFETOSKIP false
         ...
         *
         *  @param propositionIsAvailableFrom   Which step the proposition (or its negation) are available
         * from
         *  @param propositionCanBeToggledFrom  <code>PropositionAnnotation::AFTER</code> if available from
         * epsilon after that onwards, <code>PropositionAnnotation::BEFORE</code> otherwise.
         *  @param polarity                     If <code>true</code>, the proposition is available at the time
         * given; if <code>false</code>, its negation is available at the time given.
         */
         ...
         *              can be used.
         */
        PropositionAnnotation(const StepAndBeforeOrAfter &sba) : availableFrom(sba)
        {
    ...
    public:
        ...
        virtual TemporalConstraints *cloneTemporalConstraints(const TemporalConstraints *const,
                                                              const int extendBy = 0) = 0;
        virtual TemporalConstraints *emptyTemporalConstraints() = 0;

        /**
         *  Return the index of the step in the plan that must precede the ends of all actions that have not
         * yet finished.  Default implementation is to return -1: i.e. no such step exists.
         ...
         *  @param minTimestamps[in,out]  The minimum timestamps of the steps to reach s.  Must have room for
         * steps created by the application of the action.
         ...
         *  @param inPlace[in]    If <code>false</code>, a copy of <code>s</code> is made, and <code>a</code>
         * is applied to this. Otherwise, <code>a</code> is applied directly to <code>s</code>.
        ...
        virtual MinimalState *applyAction(MinimalState &s, vector<double> &minTimestamps,
                                          ...
                                          const ActionSegment &a, const bool &inPlace, const double &minDur,
                                          const double &maxDur) = 0;
         ...
         */
        virtual void updateWhenEndOfActionIs(MinimalState &s, const int &actID, const int &stepID,
                                             const double &newTS) = 0;
#endif
#endif
    };
    ...
    typedef map<int, pair<set<int>, set<int>>> StateBFacts;  // first: actions that add the fact; second:
                                                             // actions that require the fact as an invariant
#define FACTA(x) (*x)
...
#define FACTA(x) (x->first)
    ...
    protected:
        ...
        PreferenceStatusArray()
            : data(arraySize ? new AutomatonPosition::Position[arraySize] : (AutomatonPosition::Position*)0),
    cost(0.0) {
        }
    ...
    public:
...
#ifdef TOTALORDERSTATES
        StateFacts first;
        map<int, int> invariants;  // a map from fact IDs to how many non-compression-safe actions have an
                                   // invariant on that fact
         ...
         *
         * Each entry is either >= 0.0, i.e. a legitimate timestamp, or < 0.0, denoting that either the fact
         * has been seen or it's unreachable.
         */
...
#ifndef TOTALORDERSTATES
        MinimalState(const set<int> &f, const vector<double> &sMin,
                     const vector<double> &sMax,  // const PreferenceStatusArray * psa,
                     const map<int, set<int>> &sa, const vector<AutomatonPosition::Position> &ps,
        ...
        MinimalState(const StateFacts &f, const vector<double> &sMin,
                     const vector<double> &sMax,  // const PreferenceStatusArray * psa,
                     const map<int, set<int>> &sa, const vector<AutomatonPosition::Position> &ps,
                     const double &ppv, const double *tdrStatus,  // const double & sc,
                     const int nt = 0, const unsigned int pl = 0, const unsigned int ae = 0
        ...
        /**
         *  Specify a <code>StateTransformer</code> object to use to handle applying actions to states, and
         * updating the recorded temporal constraints.
         *
        ...
        /**
         *  Return a pointer to the <code>StateTransformer</code> object to use to handle applying actions to
         * states, and updating the recorded temporal constraints.
         *
        ...
        MinimalState *applyAction(const ActionSegment &a, vector<double> &minTimestamps,
                                  list<pair<int, FFEvent>> &newDummySteps, bool &constraintsSatisfied,
                                  double minDur = 0.0, double maxDur = 0.0) const
        {
            return globalTransformer->applyAction(*const_cast<MinimalState *>(this), minTimestamps,
                                                  newDummySteps, constraintsSatisfied, a, false, minDur,
                                                  maxDur);
        }
        ...
        {
            globalTransformer->applyAction(*this, minTimestamps, newDummySteps, constraintsSatisfied, a, true,
                                           minDur, maxDur);
        }
