...
namespace Planner
{
    ...
    void CompressionSafeScheduler::assignTimestamps(const MinimalState &s, list<FFEvent> &header,
                                                    list<FFEvent> &now)
    {
        if (!safeToUseThis)
        {
            std::cerr << "Fatal internal error - attempting to use the simple compression-safe scheduler on "
                         "a problem that needs at least the STP solver\n";
        ...
        while (!visit.empty())
        {
            ...
            if (stepsBefore)
            {
                ...
                if (planAsAVector[i]->time_spec == Planner::E_AT_END)
                {
                    ...
                    const double minDurationSinceStart =
                        planAsAVector[pairedWith]->lpTimestamp + planAsAVector[pairedWith]->minDuration;
