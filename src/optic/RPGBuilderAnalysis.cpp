...
namespace Planner
{
    ...
    public:
        ...
        virtual void visit_qfied_goal(qfied_goal *p)
        {
            ...
            vector<VAL::var_symbol *> vars(
                static_cast<const id_var_symbol_table *>(p->getSymTab())->numSyms());
            ...
            int i = 0;
            int c = 1;
            for (var_symbol_list::const_iterator pi = p->getVars()->begin(); pi != p->getVars()->end();
                 ++pi, ++i)
            {
            ...
            while (vals[i] != ends[i])
            {
                // This is inefficient because it creates a copy of the environment even if the copy is never
                // used. In practice, this should not be a problem because a quantified effect presumably uses
                // the variables it quantifies.
    ...
    void RPGBuilder::eliminateDuplicateConstraints(
        const list<RPGBuilder::Constraint> &input, map<string, list<int>> &outputNameMap,
        ...
        VAL::TypeChecker *tc)
    {
...

        {
            ...
            for (int pi = 0; pItr != pEnd; ++pItr, ++pi)
            {
                ...
                if (sig.legal)
                {
                    const map<ConstraintSignature, int>::iterator insItr =
                        signaturesToIDs.insert(make_pair(sig, -1)).first;
                    ...
                    if (insItr->second == -1)
                    {
                        ...
                        //                         noteWithinPreferences->insert(make_pair(nextPrefID,
                        //                         literals[*(sig.goalFacts.begin())]));
                        //                     }
...

        {
            ...
            for (int pi = prefIDstart; pItr != pEnd; ++pItr, ++pi)
            {
                ...
                {
                    cout << "Warning: " << pItr->name << " is a duplicate of " << output[kept[pi]].name
                         << endl;
                }
            }
        }
        ...
        for (int i = 0; i < opCount; ++i)
        {
            if (!rogueActions[i])
            {
                ...
                {  // checked props, now onto numerics
                    // if (criterion == -1) {
                    //   cout << "Action " << i << " - " << *(instantiatedOps[i]) << " is propositionally
                    //   uninteresting once applied: no propositional effects (" <<
                    //   actionsToStartEffects[i].size() << ", " << actionsToEndEffects[i].size() << ")\n";
                    // } else {
                    //   cout << "Action " << i << " - " << *(instantiatedOps[i]) << " is propositionally
                    //   uninteresting once applied: nothing deletes " << *(literals[criterion]) << "\n";
                    // }
                    {
                        ...
                        for (; neItr != neEnd; ++neItr)
                        {
                            ...
                            if (NumericAnalysis::getDominanceConstraints()[currRNE.fluentIndex] ==
                                    NumericAnalysis::E_METRICTRACKING ||
                                NumericAnalysis::getDominanceConstraints()[currRNE.fluentIndex] ==
                            ...
                            else
                            {
                                ...
                                {
                                    if (NumericAnalysis::getDominanceConstraints()[currRNE.fluentIndex] ==
                                        NumericAnalysis::E_SMALLERISBETTER)
                                    {
                                    ...
                                    else if (NumericAnalysis::getDominanceConstraints()[currRNE
                                                                                            .fluentIndex] ==
                                             NumericAnalysis::E_BIGGERISBETTER)
                                    {
                                        if (currRNE.constant > 0.0)
                                        {
                                            allUninteresting = false;
                    ...
                    if (currLE)
                    {
                        ...
                        {
                            if (NumericAnalysis::getDominanceConstraints()[currLE->vars[vc]] ==
                                    NumericAnalysis::E_METRICTRACKING ||
                                NumericAnalysis::getDominanceConstraints()[currLE->vars[vc]] ==
                                    NumericAnalysis::E_IRRELEVANT)
                            {
                            }
                            else
                            {
                                ...
                                else if (NumericAnalysis::getDominanceConstraints()[currLE->vars[vc]] ==
                                         NumericAnalysis::E_BIGGERISBETTER)
                                {
                                    if (currLE->effects[0][vc].constant > 0.0)
                                    {
    ...
    void RPGBuilder::findConcurrentRedundantActions()
    {
        ...
        for (int i = 0; i < opCount; ++i)
        {
            ...
            {
                // already have a one-way fact condition, which is necessarily as strong as the concurrent
                // redundant action condition
                continue;
            }

            // First, think about numbers.  This is the most involved part: we need to make sure the
            // net-effect of this action is non-positive, and that between the start and the end there
            // intermediate values of variables changed at both the start and the end is necessarily worse
            // than if the start effect hadn't occurred.
                ...
                for (; neItr != neEnd; ++neItr)
                {
                    ...
                    else
                    {
                        if (currRNE.size == 0 && !currRNE.isAssignment)
                        {
                            ...
                            else if (NumericAnalysis::getDominanceConstraints()[currRNE.fluentIndex] ==
                                     NumericAnalysis::E_BIGGERISBETTER)
                            {
                                if (currRNE.constant > 0.0)
                                {
                            ...
                            // If we get this far, the start effect is to either increase something that is
                            // better smaller; or decrease something that is better bigger.  So, on its own,
                            // the start is a bad move.  But, record it and revisit later, increase an end
                            // effect gives a net-gain to this action.
                            actionStartEndEffectsOnVariable[currRNE.fluentIndex].addStartEffect(
                                currRNE.constant);
                        }
                        else
                        {
...

            {
                ...
                for (; neItr != neEnd; ++neItr)
                {
                    ...
                    else
                    {
                        if (currRNE.size == 0 && !currRNE.isAssignment)
                        {
                            ConstantNumericEffectStartEndPair &netEffect =
                                actionStartEndEffectsOnVariable[currRNE.fluentIndex];
            ...
            // If we get this far, then the numeric effects of the action are not overall (or intermediately)
            // beneficial Now check for propositions: start and end add effects must be undeletable throughout
            ...
            {
                const list<Literal *> &propositionalAdds = (pass ? RPGBuilder::getEndPropositionAdds()[i] :
                                                                   RPGBuilder::getStartPropositionAdds()[i]);
                ...
                for (int fID; allUninteresting && effItr != effEnd; ++effItr)
                {
                    ...
                    {
                        // checkPass = 0: see if there's already an invariant known to be co-deleted along
                        // with fID checkPass = 1: for invariants whose co-deletion potential is unknown, work
                        // it out
                        ...
                        for (; invItr != invEnd; ++invItr)
                        {
                            ...
                            map<int, map<int, bool>>::const_iterator cdItr =
                                allActionsDeletingADeleteB.find(fID);
                            ...
                            if (plausible && checkPass == 1)
                            {
                                ...
                                actionsDeletingAButNotB.insert(negativeEffectsToActions[fID].begin(),
                                                               negativeEffectsToActions[fID].end());

                                list<pair<int, Planner::time_spec>>::const_iterator bdItr =
                                    negativeEffectsToActions[otherFID].begin();
                                const list<pair<int, Planner::time_spec>>::const_iterator bdEnd =
                                    negativeEffectsToActions[otherFID].end();
    ...
    int RPGBuilder::KShotKShotFormula::getLimit(const MinimalState &s) const
    {
        ...
        {
            const int thisShots =
                (int)((s.secondMax[sfItr->variable] - sfItr->greaterThan) / sfItr->decreaseBy);
    ...
    void RPGBuilder::findSelfMutexes()
    {
        ...
        for (int i = 0; i < opCount; ++i)
        {
            ...
            std::set_difference(
                startDeletes.begin(), startDeletes.end(), startAdds.begin(), startAdds.end(),
            ...
            if (isect.empty())
            {
                // cout << "Action " << *(RPGBuilder::getInstantiatedOp(i)) << " doesn't delete any of its own
                // preconditions at the start; " << overallStartDeletes.size() << " strong start deletes, " <<
                // startPreconditions.size() << " start preconditions\n";
                continue;
            }
            ...
            std::set_intersection(endAdds.begin(), endAdds.end(), isect.begin(), isect.end(),
            ...
            for (; fItr != fEnd; ++fItr)
            {
                ...
                // cout << "Action " << *(RPGBuilder::getInstantiatedOp(i)) << " might be self-mutex due to "
                // << *(*fItr) << std::endl;
        ...
        for (; mutexGroup != mutexGroupEnd; ++mutexGroup)
        {
            ...
            // now we check that the set of actions that add the fact are a subset or equal to those in the
            // set if any aren't accounted for, we can't guarantee a self mutex

            set<int> notAccountedFor;
            std::set_difference(adders.begin(), adders.end(), mutexGroup->second.begin(),
                                mutexGroup->second.end(),
    ...
    void RPGBuilder::kshotInferForAction(const int &i, MinimalState &refState,
                                         LiteralSet &maybeOneShotLiteral, vector<double> &initialFluents,
                                         const int &fluentCount)
    {
        ...
        if (oldKShotLimit > 1)
        {
            {  // first, let's see if there's a simple propositional reason for it being oneshot
...

                {
                    ...
                    for (; adItr != adEnd; ++adItr)
                    {
                        ...
                        {
                            //                      cout << "\t" << *(getLiteral(*adItr)) << " is added by
                            //                      actions, and isn't one-shot\n";
                        }
                    }
                }
            ...
            {
                // next case
                // - precondition says v > c where c is constant, probably 0 in any reasonable encoding, but
                // may as well generalise
                // - v is strictly decreasing (ask TIM)
                // - the action decreases v by some amount d, where d is constant or strictly increasing
                // (again, ask TIM)
                // - and thus, k = a formula (v - c) / d
                // this is weaker than the previous case, as the previous determines a bound of 1 or 0
                // depending on logical information rather than k based on numeric information
...

                {
                    ...
                    {
                        list<int> &currNP =
                            (pass == 0 ? actionsToRPGNumericStartPreconditions[i] :
                                         (pass == 1 ? actionsToRPGNumericInvariants[i] :
                                                      actionsToRPGNumericEndPreconditions[i]));
...

                {
                    ...
                    for (; cpItr != cpEnd; ++cpItr)
                    {
                        ...
                        {
                            list<NumericEffect> &currNE =
                                (pass == 0 ? actionsToStartNumericEffects[i] : actionsToEndNumericEffects[i]);
                                    ...
                                    if (constOnlyCalculate(neItr->formula, initialFluents, decVal))
                                    {
                                        if (neItr->op == VAL::E_DECREASE)
                                        {
                                            if (decVal > 0)
                                            {
                                                ...
                                                {
                                                    decreaseBy +=
                                                        decVal;  // must have a start then end dec effect
                                                }
                                            }
                                        }
                        ...
                        if (foundDecreaser && decreaseBy > 0.000000001)
                        {
                            ...
                            if (EFT(currPNE->getHead())->onlyGoingDown())
                            {
                                ...
                                "; if (RHSConstant) { cout << "k = ceil(" << *(currPNE) << " - " <<
                                (RHSConstant - decreaseBy) << ")"; } else { cout << "k = " << *(currPNE);
                                }
                                if (decreaseBy != 1.0) {
                                    cout << " / " << decreaseBy << "\n";
                                } else {
                                ...
                                shotFormulae.push_back(
                                    ShotCalculator(LHSVariable, (RHSConstant - decreaseBy), decreaseBy));
                            }
                        }
                    }
                }
            ...
            {
                //          cout << "Have an analysis now - says limit is " <<
                //          kShotFormulae[i]->getLimit(refState) << "\n";
            }
            ...
            if (kShotFormulae[i]->getLimit(refState) <= 1)
            {
                ...
                {
                    if (effectsToActions[*tiItr].size() == 1 &&
                        refState.first.find(*tiItr) == refState.first.end())
                    {  // only added by one thing, i.e. this action
                        oneShotLiterals[*tiItr] = true;
                        maybeOneShotLiteral.insert(getLiteral(*tiItr));
                        list<pair<int, Planner::time_spec>> &listToUse =
                            processedPreconditionsToActions[*tiItr];
                        ...
                        for (; affectedItr != affectedEnd; ++affectedItr)
                        {
                            if (affectedItr->second == Planner::E_AT_END)
                            {
                                ...
                                //                          cout << "Has " << *(getLiteral(*tiItr)) << " as a
                                //                          one-shot wrapped literal on which an end is
                                //                          dependent\n";
                            }
                        }
                    }
                    ...
                    {
                        //                  cout << *(getLiteral(*tiItr)) << " isn't one-shot wrapped in the
                        //                  action"; cout << "Is added by " << effectsToActions[*tiItr].size()
                        //                  << " actions\n"; if (refState.first.find(*tiItr) !=
                        //                  refState.first.end()) cout << "Is in initial state\n";
                    }
                }
        ...
        for (; delsItr != delsEnd; ++delsItr)
        {
            if (tilAdds.find(delsItr->first) == tilAdds.end() && effectsToActions[delsItr->first].empty())
            {
                ...
                {  // first pass - collect actions for which it's an invariant
                    ...
                    {
                        if (affItr->second == Planner::E_OVER_ALL)
                        {
                ...
                {  // second pass - for actions having it as an end precondition, but not as an invariant
                    ...
                    for (; affItr != affEnd; ++affItr)
                    {
                        ...
                        {
                            affects.insert(affItr,
                                           pair<int, Planner::time_spec>(affItr->first, Planner::E_OVER_ALL));
                        }
                    }
                }
            }
    ...
    void RPGBuilder::doSomeUsefulMetricRPGInference()
    {
        ...
        while (!maybeOneShotLiteral.empty())
        {
            ...
            for (; litItr != litEnd; ++litItr)
            {
                ...
                {  //
                    ...
                    if (kShotFormulae[actID]->getLimit(refState) <= 1)
                    {
                        ...
                        // cout << "Have shown that literal " << *(*litItr) << " is one shot: only added by "
                        // << *(getInstantiatedOp(actID)) << "\n";
                        list<pair<int, Planner::time_spec>>::iterator depItr =
                            preconditionsToActions[lID].begin();
                        const list<pair<int, Planner::time_spec>>::iterator depEnd =
                            preconditionsToActions[lID].end();
                    ...
                    {
                        //                  cout << *(*litItr) << " isn't one-shot: the single achieving
                        //                  action, " << *(getInstantiatedOp(actID)) << ", can be applied " <<
                        //                  kShotFormulae[actID]->getLimit(refState) << " times\n";
                    }
                }
                else
                {
                    //              if (eta.size() > 1) cout << *(*litItr) << " isn't one shot: is added by "
                    //              << eta.size() << " actions\n";
                }
            }
            ...
            for (; riItr != riEnd; ++riItr)
            {
                ...
                {
                    //              cout << "Revisiting " << *(getInstantiatedOp(*riItr)) << " due to one-shot
                    //              literal\n";
                ...
                else
                {
                    if (initialState.find(*llItr) == initialState.end())
                    {
                        if (effectsToActions[(*llItr)->getStateID()].empty())
                        {
                            ...
                            {
                                ostringstream s;
                                s << "Has a precondition " << *(*llItr)
                                  << " that is false in the initial state, and never added by an action\n";
                                Globals::eliminatedAction(operatorID, s.str().c_str());
                            }
#endif
                            ...
                            if (localDebug)
                            {
                                cout << "Nothing adds " << *(*llItr)
                                     << " and it isn't in the initial state\n";
                            }
                            eliminate = true;
                            break;
                        }
        ...
        if (eliminate)
        {
            ...
            set<int> noLongerGet;
            {
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    ...
                    const bool rv =
                        removeFirst(effectsToActions[effID],
                                    pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
                    if (!rv)
                    {
                        std::cerr << "Fatal internal error: action start-adds " << *(*effItr)
                                  << " but it isn't in effects-to-actions\n";
                        assert(rv);
                    }
...

            {
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    ...
                    const bool rv =
                        removeFirst(negativeEffectsToActions[effID],
                                    pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
                    if (!rv)
                    {
                        std::cerr << "Fatal internal error: action start-deletes " << *(*effItr)
                                  << " but it isn't in negative-effects-to-actions\n";
                        assert(rv);
                    }
                }
                currEffectsList.clear();
...

            {
                ...
                {
                    const int effID = (*effItr)->getStateID();
                    const bool rv = removeFirst(effectsToActions[effID],
                                                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_END));
                    if (!rv)
                    {
                        std::cerr << "Fatal internal error: action end-adds " << *(*effItr)
                                  << " but it isn't in effects-to-actions\n";
                        assert(rv);
                    }
                    if (effectsToActions[effID].empty())
...

            {
                ...
                {
                    const int effID = (*effItr)->getStateID();
                    const bool rv = removeFirst(negativeEffectsToActions[effID],
                                                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_END));
                    if (!rv)
                    {
                        std::cerr << "Fatal internal error: action end-deletes " << *(*effItr)
                                  << " but it isn't in negative-effects-to-actions\n";
                        assert(rv);
                    }
                }
...

            {
                ...
                for (; precItr != precEnd; ++precItr)
                {
                    ...
                    const bool rv =
                        removeFirst(preconditionsToActions[precID],
                                    pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
                    if (!rv)
                    {
                        std::cerr << "Fatal internal error: action has a start pre " << *(*precItr)
                                  << " but it isn't in preconditions-to-actions\n";
                        assert(rv);
                    }
                }
...

            {
                ...
                for (; precItr != precEnd; ++precItr)
                {
                    ...
                    const bool rv =
                        removeFirst(preconditionsToActions[precID],
                                    pair<int, Planner::time_spec>(operatorID, Planner::E_OVER_ALL));
                    if (!rv)
                    {
                        std::cerr << "Fatal internal error: action has an over all " << *(*precItr)
                                  << " but it isn't in preconditions-to-actions\n";
                        assert(rv);
                    }
                }
...

            {
                ...
                for (; precItr != precEnd; ++precItr)
                {
                    ...
                    if (!rv1)
                    {
                        std::cerr << "Fatal internal error: action has an end pre " << *(*precItr)
                                  << " but it isn't in preconditions-to-actions\n";
                        assert(rv1);
                    }

                    const bool rv2 =
                        removeFirst(processedPreconditionsToActions[precID],
                                    pair<int, Planner::time_spec>(operatorID, Planner::E_AT_END));
                    if (!rv2)
                    {
                        std::cerr << "Fatal internal error: action has an end pre " << *(*precItr)
                                  << " but it isn't in processed-preconditions-to-actions\n";
                        assert(rv2);
                    }
                }
                ...
                {
                    const bool rv = removeFirst(preconditionlessActions,
                                                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_END));
                    assert(rv);
                }
                currPreconditionsList.clear();
...

            {
                ...
                {
                    const bool rv =
                        removeFirst(preconditionlessActions,
                                    pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
                    assert(rv);
                }
                ...
                for (; precItr != precEnd; ++precItr)
                {
                    ...
                    if (!rv)
                    {
                        std::cerr << "Fatal internal error: action has a processed start pre " << *(*precItr)
                                  << " but it isn't in processed-preconditions-to-actions\n";
                        assert(rv);
                    }
                }
    ...
    void RPGBuilder::postFilterUnreachableActions()
    {
...

        {
            ...
            {
                list<pair<int, Planner::time_spec>>::iterator depItr =
                    processedPreconditionsToActions[(*rvItr)->getStateID()].begin();
                const list<pair<int, Planner::time_spec>>::iterator depEnd =
                    processedPreconditionsToActions[(*rvItr)->getStateID()].end();
    ...
    {
        globalStaticLiterals =
            vector<pair<bool, bool>>(instantiatedOp::howManyLiteralsOfAnySort(), make_pair(false, false));
        ...
        for (int globalLitID = 0; lsItr != lsEnd; ++lsItr, ++globalLitID)
        {
            ...
            if (!epsMatch)
            {
                valueTrue = (Inst::InitialStateEvaluator::getInit0State().find((*lsItr)->getProp()->head) !=
                             Inst::InitialStateEvaluator::getInit0State().end());
            }
    ...
    void RPGBuilder::findStaticLiterals()
    {
        ...
        for (int i = 0; i < litCount; ++i)
        {
            ...
            {
                globalStaticLiterals[literals[i]->getGlobalID()] =
                    pair<bool, bool>(true, initialState.find(literals[i]) != initialState.end());
        ...
        for (int i = 0; i < opCount; ++i)
        {
            if (!rogueActions[i])
            {
                ...
                }
                {
                    ...
                    if (!toDec && before)
                    {
                        preconditionlessActions.push_back(
                            pair<int, Planner::time_spec>(i, Planner::E_AT_START));
                    }
                }
                {
                    int dummy = 0;
...

                {
                    ...
                    {
                        preconditionlessActions.push_back(
                            pair<int, Planner::time_spec>(i, Planner::E_AT_END));
                    }
                }
                {
                    int dummy = 0;
    ...
    struct NextRegress
    {
        ...
        NextRegress(const int &a, const int &b, const double &c, const set<int> &d)
          : fact(a), fluent(b), ts(c), forGoal(d){};
    ...
    private:
        ...
        bool keepDeadlinesOnPreconditions()
        {
            ...
            const vector<list<pair<int, Planner::time_spec>>> &presToActions =
        ...
        void requestVisitHandler(const int &literal, const int &fluent, const double &timeIn,
                                 set<int> forGoal)
        {
            ...
            map<int, double> &bestSeen =
                (literal != -1 ? literalSeenForGoalAtTime[literal] : fluentSeenForGoalAtTime[fluent]);
    ...
    public:
        CommonRegressionData(const int &lc, const int &fc, const int &ac, const int &dc)
          : fluentCount(fc)
          , literalSeenForGoalAtTime(lc)
          , fluentSeenForGoalAtTime(fc)
          , opAppliedForGoalAtTime(ac)
          , dpAppliedForGoalAtTime(dc)
          , fluentAffectedBy(fc)
          , interestingNumericEffects(rpgNumericEffectsToActions.size(), false)
          , haveDeterminedWhetherAllInterestingEffectsAreAtTheEnd(false){

                {vector<list<pair<int, Planner::time_spec>>>::const_iterator numEffItr =
                     rpgNumericEffectsToActions.begin();
        const vector<list<pair<int, Planner::time_spec>>>::const_iterator numEffEnd =
            rpgNumericEffectsToActions.end();
        ...
        for (int i = 0; numEffItr != numEffEnd; ++numEffItr, ++i)
        {
            ...
            else if (NumericAnalysis::getDominanceConstraints()[affFluent] ==
                     NumericAnalysis::E_BIGGERISBETTER)
            {
...
void regressThroughAction(const int &actID, const Planner::time_spec &ts, const NextRegress &inAidOf)
{
...

    {
        ...
        if (ts == Planner::E_AT_END && atTime != DBL_MAX)
        {
        ...
        switch (pass)
        {
            case 0:
            {
                ...
                else
                {
                    if (atTime != DBL_MAX)
                    {
                        if (debug)
                        {
                            cout << "Duration of " << *(getInstantiatedOp(actID)) << " = "
                                 << getOpMinDuration(actID, -1) << "\n";
                        }
                        atTime -= getOpMinDuration(actID, -1);
                    }
                }
            ...
            case 1:
            {
                ...
                if (ts == Planner::E_AT_START)
                {
                    if (debug)
                    {
                        cout << "Adding pres for invariants from start of " << *(getInstantiatedOp(actID))
                             << " at time ";
                        if (atTime == DBL_MAX)
                        {
                            cout << "infinity\n";
...
void RPGBuilder::pruneIrrelevant(const int &operatorID)
{
...

    {
        ...
        {
            const int effID = (*effItr)->getStateID();
            negativeEffectsToActions[effID].remove(
                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
        }
        currEffectsList.clear();
    }
...

    {
        ...
        {
            const int precID = (*precItr)->getStateID();
            preconditionsToActions[precID].remove(
                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
        }
        currPreconditionsList.clear();
    }
...

    {
        ...
        for (; precItr != precEnd; ++precItr)
        {
            ...
            preconditionsToActions[precID].remove(
                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_END));
            processedPreconditionsToActions[precID].remove(
                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_END));
        }
        ...
        if (currPreconditionsList.empty())
        {
            ...
            onlyNumericPreconditionActions.remove(
                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_END));
        }
        currPreconditionsList.clear();
    }
...

    {
        ...
        for (; precItr != precEnd; ++precItr)
        {
            ...
            rpgNumericPreconditionsToActions[precID].remove(
                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
        }
        currPreconditionsList.clear();
    }
...

    {
        ...
        {
            const int precID = *precItr;
            processedRPGNumericPreconditionsToActions[precID].remove(
                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
        }
...

    {
        ...
        for (; effItr != effEnd; ++effItr)
        {
            rpgNumericEffectsToActions[*effItr].remove(
                pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
        }
...
void RPGBuilder::postFilterIrrelevantActions()
{
    ...
    while (!data.empty())
    {
        ...
        if (currPair.fact != -1)
        {
            ...
            {
                cout << COLOUR_light_blue << "Finding achievers for " << *(getLiteral(currLit))
                     << " at time ";
        ...
        {
            cout << "For some reason, the goal of achieving precisely nothing has worked its way onto the "
                    "regression filter stack\n";
            assert(false);
        }
    }
    ...
    for (int i = 0; i < opCount; ++i)
    {
        if (!rogueActions[i])
        {
            ...
            {
                //              cout << *getInstantiatedOp(i) << " is interesting no later than " <<
                //              startBound << "\n";
                oldStart = startBound;
            }
            if (oldEnd > endBound)
...
void RPGBuilder::separateOptimisationTILs()
{
        ...
        else
        {
            if (ti != newTI)
            {
                ...
                }
                {
                    ...
                    {
                        negativeEffectsToActions[(*elItr)->getStateID()].remove(make_pair(ti, Planner::E_AT));
                        negativeEffectsToActions[(*elItr)->getStateID()].push_back(
                            make_pair(newTI, Planner::E_AT));
                    }
                }
            }
            ++newTI;
...

    {
        ...
        {
            if (normalTimedInitialLiteralsVector[ti]->duration <
                optimisationTimedInitialLiteralsVector[oti]->duration)
            {
...
void RPGBuilder::removePointlessEffects()
{
    ...
    while (!newlyPointlessEffects.empty())
    {
        ...
        for (; fItr != fEnd; ++fItr)
        {
            for (pointless_effect eff = PE_ADDED; eff < PE_DELETED_THEN_ADDED;
                 eff = (eff == PE_ADDED ? PE_DELETED : PE_DELETED_THEN_ADDED))
            {
                list<pair<int, Planner::time_spec>> &eta =
                    (eff == PE_DELETED ? negativeEffectsToActions[(*fItr)->getStateID()] :
                                         effectsToActions[(*fItr)->getStateID()]);

                vector<list<Literal *>> &startEffs =
                    (eff == PE_DELETED ? actionsToStartNegativeEffects : actionsToStartEffects);

                vector<list<Literal *>> &endEffs =
                    (eff == PE_DELETED ? actionsToEndNegativeEffects : actionsToEndEffects);
                ...
                if (Globals::globalVerbosity & 16384)
                {
                    if (!eta.empty())
                    {
                        if (eff == PE_ADDED)
                        {
                            cout << "Add effects on " << *(*fItr) << " [" << (*fItr)->getStateID()
                                 << "] are pointless - number of actions with this precondition: "
                                 << preconditionsToActions[(*fItr)->getStateID()].size() << endl;
                        }
                        else
                        {
                            cout << "Delete effects on " << *(*fItr) << " [" << (*fItr)->getStateID()
                                 << "] are pointless\n";
                        }
                    }
                }
                ...
                assert(
                    (*PreferenceHandler::getNegativePreconditionsToPrefs())[(*fItr)->getStateID()].empty());
                ...
                for (; etaItr != etaEnd; ++etaItr)
                {
                    switch (etaItr->second)
                    {
                        case Planner::E_AT_START:
                        {
                            if (Globals::globalVerbosity & 16384)
                            {
                                cout << " - Removed effect from start of "
                                     << *(RPGBuilder::getInstantiatedOp(etaItr->first)) << endl;
                            }
                            pointless_effect &toUpdate = pointlessStartEffects[etaItr->first]
                                                             .insert(make_pair(*fItr, eff))
                                                             .first->second;
                        ...
                        case Planner::E_AT_END:
                        {
                            if (Globals::globalVerbosity & 16384)
                            {
                                cout << " - Removed effect from end of "
                                     << *(RPGBuilder::getInstantiatedOp(etaItr->first)) << endl;
                            }

                            pointless_effect &toUpdate = pointlessEndEffects[etaItr->first]
                                                             .insert(make_pair(*fItr, eff))
                                                             .first->second;
                            toUpdate = (pointless_effect)(toUpdate | eff);
                        ...
                        case Planner::E_AT:
                        {
                            pointless_effect &toUpdate = pointlessTILEffects[etaItr->first]
                                                             .insert(make_pair(*fItr, eff))
                                                             .first->second;
                            toUpdate = (pointless_effect)(toUpdate | eff);
                            if (toUpdate == PE_ADDED)
                            {
                                const bool rv = removeFirst(
                                    normalTimedInitialLiteralsVector[etaItr->first]->addEffects, *fItr);
                                assert(rv);
                            }
                            else
                            {
                                const bool rv = removeFirst(
                                    normalTimedInitialLiteralsVector[etaItr->first]->delEffects, *fItr);
                                assert(rv);
                            }
                            break;
                        }
                        default:
                        {
                            std::cerr << "Internal error: facts should always be added by one of an action "
                                         "start, an action end, or a TIL\n";
                            exit(1);
                        }
                    }
                }
        ...
        for (; actCheckItr != actCheckEnd; ++actCheckItr)
        {
            if (!RPGBuilder::getStartPropositionAdds()[*actCheckItr].empty())
            {
                if (Globals::globalVerbosity & 16384)
                {
                    cout << "Still keeping " << *(RPGBuilder::getInstantiatedOp(*actCheckItr))
                         << " as it has start add effects\n";
                }
            ...
            if (Globals::globalVerbosity & 16384)
            {
                cout << "i) " << *(RPGBuilder::getInstantiatedOp(*actCheckItr))
                     << " has no start add effects\n";
            }
            ...
            if (!endAdds.empty())
            {
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    const pair<map<int, bool>::iterator, bool> insPair =
                        actionsThatDeleteThisFactNeedIt.insert(make_pair((*effItr)->getStateID(), true));

                    if (insPair.second)
                    {
                        // not worked out whether it follows the 'only deleted immediately after inspection'
                        // idiom

                        const list<pair<int, Planner::time_spec>> &deletors =
                            negativeEffectsToActions[(*effItr)->getStateID()];
                        ...
                        for (; dItr != dEnd; ++dItr)
                        {
                            ...
                            if (pItr == pEnd)
                            {
                                ...
                                if (Globals::globalVerbosity & 16384)
                                {
                                    cout << *(RPGBuilder::getInstantiatedOp(dItr->first)) << " deletes "
                                         << *(*effItr) << " without needing it\n";
                                }
                                insPair.first->second = false;
                                break;
                            }
                    ...
                    else
                    {
                        ...
                        {
                            list<Literal *> &lookat = (enPass ? actionsToEndPreconditions[*actCheckItr] :
                                                                actionsToStartPreconditions[*actCheckItr]);
                        ...
                        if (!everNeeded)
                        {
                            ...
                            {
                                cout << "Still keeping " << *(RPGBuilder::getInstantiatedOp(*actCheckItr))
                                     << " as it end-adds " << *(*effItr)
                                     << " without having needed it earlier\n";
                            }
                        ...
                        else
                        {
                            if (Globals::globalVerbosity & 16384)
                            {
                                cout << "ii) " << *(RPGBuilder::getInstantiatedOp(*actCheckItr))
                                     << " end-adds " << *(*effItr)
                                     << ", but actions that delete this fact need it\n";
                            }
                        }
                    }
                }
            ...
            {
                const list<int> &numericEffs = (pass ? RPGBuilder::getEndEffNumerics()[*actCheckItr] :
                                                       RPGBuilder::getStartEffNumerics()[*actCheckItr]);
                    ...
                    for (; numericEffectsAreOnlyBad && nEffItr != nEffEnd; ++nEffItr)
                    {
                        ...
                        switch (NumericAnalysis::getDominanceConstraints()[currEff.fluentIndex])
                        {
                            case NumericAnalysis::E_NODOMINANCE:
                            {
                                ...
                                {
                                    cout << "Still keeping " << *(RPGBuilder::getInstantiatedOp(*actCheckItr))
                                         << " as it has effects on a non-dominated numeric variable\n";
                                }
                            ...
                            case NumericAnalysis::E_SMALLERISBETTER:
                            {
                                if (currEff.isAssignment || currEff.size > 1)
                                {
                                    ...
                                    // and multi-variable-based effects, based on something other than
                                    // ?duration have non-predicable outcomes
                                    if (Globals::globalVerbosity & 16384)
                                    {
                                        cout << "Still keeping "
                                             << *(RPGBuilder::getInstantiatedOp(*actCheckItr))
                                             << " as it an assignment effect or multi-variable effect on a "
                                                "variable\n";
                                    }
                                ...
                                pair<double, double> effInFormConstantAndDurationCoefficient(currEff.constant,
                                                                                             0.0);
                                ...
                                if (currEff.size)
                                {
                                    ...
                                    else
                                    {
                                        if (Globals::globalVerbosity & 16384)
                                        {
                                            cout << "Still keeping "
                                                 << *(RPGBuilder::getInstantiatedOp(*actCheckItr))
                                                 << " as it has a variable-dependent effect on a variable\n";
                                        }
                                ...
                                // find bounds on this, accounting for anything that happened at the start (if
                                // we're on the second pass, for end effects)

                                if (pass == 1)
                                {
                                    const map<int, pair<double, double>>::const_iterator startEffOnThisVar =
                                        accruedSimpleEffectOnPNE.find(currEff.fluentIndex);

                                    if (startEffOnThisVar != accruedSimpleEffectOnPNE.end())
                                    {
                                        effInFormConstantAndDurationCoefficient.first +=
                                            startEffOnThisVar->second.first;
                                        effInFormConstantAndDurationCoefficient.second +=
                                            startEffOnThisVar->second.second;
                                    }
                                }
                                ...
                                {
                                    // positive duration coefficient - small as possible duration for min, big
                                    // as possible for max
                                    minValueOfEffect += effInFormConstantAndDurationCoefficient.second *
                                                        RPGBuilder::getOpMinDuration(*actCheckItr, 0);
                                    maxValueOfEffect += effInFormConstantAndDurationCoefficient.second *
                                                        RPGBuilder::getOpMaxDuration(*actCheckItr, 0);
                                }
                                else if (effInFormConstantAndDurationCoefficient.second > 0.0)
                                {
                                    // negative duration coefficient - big as possible duration for min, small
                                    // as possible for max
                                    minValueOfEffect += effInFormConstantAndDurationCoefficient.second *
                                                        RPGBuilder::getOpMaxDuration(*actCheckItr, 0);
                                    maxValueOfEffect += effInFormConstantAndDurationCoefficient.second *
                                    ...
                                    NumericAnalysis::E_BIGGERISBETTER)
                                {
                                    ...
                                    // this is a good effect if the maximum effect is positive, i.e. there's
                                    // some way it can muster an increase
                                    if (maxValueOfEffect > 0.0)
                                    {
                                        ...
                                        if (Globals::globalVerbosity & 16384)
                                        {
                                            cout << "Still keeping "
                                                 << *(RPGBuilder::getInstantiatedOp(*actCheckItr))
                                                 << " as it has a beneficial effect on "
                                    ...
                                    else
                                    {
                                        if (pass == 0)
                                        {
                                            ...
                                            // as we want to check if the action has a net gain.  e.g.
                                            // decrease by 2 at start, increase by <= 2 at end is an effect in
                                            // the right direction, but it's not net gain

                                            accruedSimpleEffectOnPNE[currEff.fluentIndex] =
                                                effInFormConstantAndDurationCoefficient;
                                        }
                                    }
                                }
                                else
                                {
                                    ...
                                    // this is a good effect if the minimum effect is negative, i.e. there's
                                    // some way it can muster a decrease
                ...
                {
                    RPGBuilder::LinearEffects *const currLE =
                        RPGBuilder::getLinearDiscretisation()[*actCheckItr];
                    if (currLE)
                    {
                        ...
                        for (int cei = 0; numericEffectsAreOnlyBad && cei < ceCount; ++cei)
                        {
                            switch (NumericAnalysis::getDominanceConstraints()[currLE->vars[cei]])
                            {
                                ...
                                case NumericAnalysis::E_SMALLERISBETTER:
                                {
                                    pair<double, double> effInFormConstantAndDurationCoefficient(
                                        0.0, currLE->effects[0][cei].constant);

                                    const map<int, pair<double, double>>::const_iterator startEffOnThisVar =
                                        accruedSimpleEffectOnPNE.find(currLE->vars[cei]);
                                    ...
                                    if (effInFormConstantAndDurationCoefficient.second > 0.0)
                                    {
                                        // positive duration coefficient - small as possible duration for min,
                                        // big as possible for max
                                        minValueOfEffect += effInFormConstantAndDurationCoefficient.second *
                                                            RPGBuilder::getOpMinDuration(*actCheckItr, 0);
                                        maxValueOfEffect += effInFormConstantAndDurationCoefficient.second *
                                                            RPGBuilder::getOpMaxDuration(*actCheckItr, 0);
                                    ...
                                    {
                                        // negative duration coefficient - big as possible duration for min,
                                        // small as possible for max
                                        minValueOfEffect += effInFormConstantAndDurationCoefficient.second *
                                                            RPGBuilder::getOpMaxDuration(*actCheckItr, 0);
                                        maxValueOfEffect += effInFormConstantAndDurationCoefficient.second *
                                                            RPGBuilder::getOpMinDuration(*actCheckItr, 0);
                                    ...
                                    if (NumericAnalysis::getDominanceConstraints()[currLE->vars[cei]] ==
                                        NumericAnalysis::E_BIGGERISBETTER)
                                    {
                                        // the 'bigger is better' case
                                        // this is a good effect if the maximum effect is positive, i.e.
                                        // there's some way it can muster an increase
                                        if (maxValueOfEffect > 0.0)
                                        {
                                            // possible to have net gain - effect could be good
                                            numericEffectsAreOnlyBad = false;
                                        ...
                                        {
                                            accruedSimpleEffectOnPNE[currLE->vars[cei]] =
                                                effInFormConstantAndDurationCoefficient;
                                        }
                                    }
                                    else
                                    {
                                        // the 'smaller is better' case
                                        // this is a good effect if the minimum effect is negative, i.e.
                                        // there's some way it can muster a decrease
                                        if (minValueOfEffect < 0.0)
                                        {
                                            // possible to have net loss - effect could be good
                                            numericEffectsAreOnlyBad = false;
            ...
            if (numericEffectsAreOnlyBad)
            {
                ...
                // as we aren't clearing the register of this action's start propositional add effects, assert
                // none exist
...

                {
                    ...
                    for (; effItr != effEnd; ++effItr)
                    {
                        ...
                        const bool rv =
                            removeFirst(negativeEffectsToActions[effID],
                                        pair<int, Planner::time_spec>(*actCheckItr, Planner::E_AT_START));
                        assert(rv);
                    }
                    currEffectsList.clear();
                }
...

                {
                    ...
                    for (; effItr != effEnd; ++effItr)
                    {
                        ...
                        const bool rv =
                            removeFirst(negativeEffectsToActions[effID],
                                        pair<int, Planner::time_spec>(*actCheckItr, Planner::E_AT_END));
                        assert(rv);
                    }
                    currEffectsList.clear();
                }
                ...
                for (int pass = 0; pass < 2; ++pass)
                {
                    list<int> &currEffectsList = (pass ? actionsToRPGNumericEndEffects[*actCheckItr] :
                                                         actionsToRPGNumericStartEffects[*actCheckItr]);
                    ...
                    {
                        const bool rv =
                            removeFirst(rpgNumericEffectsToActions[*effItr],
                                        pair<int, Planner::time_spec>(
                                            *actCheckItr, pass ? Planner::E_AT_END : Planner::E_AT_START));
                        assert(rv);
                    }
...

                {
                    ...
                    for (; precItr != precEnd; ++precItr)
                    {
                        ...
                        if (preconditionsToActions[precID].empty() &&
                            literalGoalSet.find(*precItr) == literalGoalSet.end() &&
...

                {
                    ...
                    for (; precItr != precEnd; ++precItr)
                    {
                        ...
                        const bool rv =
                            removeFirst(preconditionsToActions[precID],
                                        pair<int, Planner::time_spec>(*actCheckItr, Planner::E_OVER_ALL));
                        assert(rv);
                        if (preconditionsToActions[precID].empty() &&
                            literalGoalSet.find(*precItr) == literalGoalSet.end() &&
                            (*PreferenceHandler::getPreconditionsToPrefs())[precID].empty() &&
...

                {
                    ...
                    if (currPreconditionsList.empty())
                    {
                        ...
                        {
                            const bool rv =
                                removeFirst(onlyNumericPreconditionActions,
                                            pair<int, Planner::time_spec>(*actCheckItr, Planner::E_AT_END));
                            assert(rv);
                        }
                    }
                ...
                for (int pass = 0; pass < 2; ++pass)
                {
                    list<int> &currPreconditionsList =
                        (pass ? actionsToRPGNumericInvariants[*actCheckItr] :
                                actionsToRPGNumericStartPreconditions[*actCheckItr]);
                    ...
                    {
                        const bool rv =
                            removeFirst(rpgNumericPreconditionsToActions[*precItr],
                                        pair<int, Planner::time_spec>(
                                            *actCheckItr, pass ? Planner::E_OVER_ALL : Planner::E_AT_START));
                        assert(rv);
                    }
...

                {
                    ...
                    for (; precItr != precEnd; ++precItr)
                    {
                        ...
                        assert(rv1);
                        const bool rv2 =
                            removeFirst(processedRPGNumericPreconditionsToActions[*precItr],
                                        pair<int, Planner::time_spec>(*actCheckItr, Planner::E_AT_END));
                        assert(rv2);
                    }
                ...
                {
                    list<int> &currPreconditionsList =
                        actionsToProcessedStartRPGNumericPreconditions[*actCheckItr];
    ...
    for (int litID = 0; litID < factCount; ++litID)
    {
...

        {
            ...
            for (; depItr != depEnd; ++depItr)
            {
                if (depItr->second == Planner::E_OVER_ALL)
                {
                    if (localDebug)
                    {
                        cout << *(RPGBuilder::getLiteral(litID))
                             << " cannot be a semaphore fact - appears as an over all\n";
                    }
                ...
                {
                    const list<Literal *> &simultaneousDels =
                        (depItr->second == Planner::E_AT_START ?
                             actionsToStartNegativeEffects[depItr->first] :
                             actionsToEndNegativeEffects[depItr->first]);
                    ...
                    if (delItr == delEnd)
                    {
                        ...
                        {
                            cout << *(RPGBuilder::getLiteral(litID))
                                 << " cannot be a semaphore fact - required by "
                                 << *(RPGBuilder::getInstantiatedOp(depItr->first))
                                 << " but not then immediately deleted\n";
                        }
                ...
                for (int pass = 0; pass < 2; ++pass)
                {
                    ...
                    const list<Literal *> &subsequentAdd =
                        (pass ? actionsToEndEffects[depItr->first] : actionsToStartEffects[depItr->first]);
                ...
                if (!foundAnAdd)
                {
                    ...
                    if (localDebug)
                    {
                        cout << *(RPGBuilder::getLiteral(litID))
                             << " cannot be a semaphore fact - required by "
                             << *(RPGBuilder::getInstantiatedOp(depItr->first)) << " but not then added\n";
                    }
...

        {
            ...
            for (; affItr != affEnd; ++affItr)
            {
                const list<Literal *> &simultaneousPres =
                    (affItr->second == Planner::E_AT_START ? actionsToStartPreconditions[affItr->first] :
                                                             actionsToEndPreconditions[affItr->first]);

                {
                    ...
                    if (pItr == pEnd)
                    {
                        ...
                        {
                            cout << *(RPGBuilder::getLiteral(litID))
                                 << " cannot be a semaphore fact - deleted by "
                                 << *(RPGBuilder::getInstantiatedOp(affItr->first)) << " but not required\n";
                        }
                        consistentlyASemaphore = false;
                        break;
                    }
                ...
                for (int pass = 0; pass < 2; ++pass)
                {
                    ...
                    const list<Literal *> &subsequentAdd =
                        (pass ? actionsToEndEffects[affItr->first] : actionsToStartEffects[affItr->first]);
                ...
                if (!foundAnAdd)
                {
                    ...
                    if (localDebug)
                    {
                        cout << *(RPGBuilder::getLiteral(litID))
                             << " cannot be a semaphore fact - deleted by "
                             << *(RPGBuilder::getInstantiatedOp(affItr->first)) << " but not then added\n";
                    }
                    consistentlyASemaphore = false;
                    break;
                }
...

        {
            ...
            for (; affItr != affEnd; ++affItr)
            {
                ...
                for (int pass = 0; pass < 2; ++pass)
                {
                    ...
                    const list<Literal *> &simultaneousPres =
                        (pass ? actionsToStartPreconditions[affItr->first] :
                                actionsToEndPreconditions[affItr->first]);
                    ...
                    const list<Literal *> &simultaneousDels =
                        (pass ? actionsToStartNegativeEffects[affItr->first] :
                                actionsToEndNegativeEffects[affItr->first]);
                ...
                if (!foundRequireDeletePair)
                {
                    ...
                    {
                        cout << *(RPGBuilder::getLiteral(litID))
                             << " cannot be a semaphore fact - not required-then-deleted by "
                             << *(RPGBuilder::getInstantiatedOp(affItr->first)) << endl;
                    }
                    consistentlyASemaphore = false;
                    break;
                }
        ...
        for (int rnp = 0; rnp < rnpCount; ++rnp)
        {
            ...
            for (; depItr != depEnd; ++depItr)
            {
                ...
                if (depItr->second == Planner::E_AT_START)
                {
                    ...
                    if (localDebug)
                    {
                        cout << "Required by " << *(RPGBuilder::getInstantiatedOp(depItr->first))
                             << ", start\n";
                    }
                }
                ...
                {
                    std::cerr << "Internal error: have found a precondition that doesn't apply to a start, "
                                 "invariant or end action\n";
                    exit(1);
                }
                ...
                if (depItr->second == Planner::E_OVER_ALL)
                {
                    // also consider that the action needs semaphore facts it requires-deletes at the start
                    // and adds at the end
                    set<int> addedAtStart;

                    {
                        list<Literal *>::const_iterator pItr = actionsToStartEffects[depItr->first].begin();
                        const list<Literal *>::const_iterator pEnd =
                            actionsToStartEffects[depItr->first].end();
                    ...
                    list<Literal *>::const_iterator preItr =
                        actionsToStartPreconditions[depItr->first].begin();
                    const list<Literal *>::const_iterator preEnd =
                        actionsToStartPreconditions[depItr->first].end();
            ...
            for (; vItr != vEnd; ++vItr)
            {
                ...
                if (mapItr == referringToFluentNeedsFacts.end())
                {
                    ...
                    if (localDebug)
                    {
                        cout << "Looks like referring to " << *(RPGBuilder::getPNE(*vItr)) << " needs "
                             << coRequired.size() << " facts\n";
                    }
                }
                else
                {
                    ...
                    {
                        set<int> isect;
                        std::set_intersection(coRequired.begin(), coRequired.end(), mapItr->second.begin(),
                                              mapItr->second.end(),
                                              std::insert_iterator<set<int>>(isect, isect.begin()));
                        ...
                        {
                            cout << COLOUR_light_blue << "Now have no definite guards for "
                                 << *(RPGBuilder::getPNE(*vItr)) << COLOUR_default << endl;
                        }
                    }
                }
            }
        ...
        for (int rne = 0; rne < rneCount; ++rne)
        {
            ...
            for (; depItr != depEnd; ++depItr)
            {
                ...
                if (depItr->second == Planner::E_AT_START)
                {
                    ...
                    if (localDebug)
                    {
                        cout << "Attached to " << *(RPGBuilder::getInstantiatedOp(depItr->first))
                             << ", start\n";
                    }
                }
                else if (depItr->second == Planner::E_AT_END)
                ...
                {
                    std::cerr << "Internal error: have found an instantaneous effect that doesn't apply to a "
                                 "start or end action\n";
                    exit(1);
                }
            ...
            if (mapItr == changingFluentNeedsFacts.end())
            {
                ...
                if (localDebug)
                {
                    cout << "Looks like changing to " << *(RPGBuilder::getPNE(featured)) << " needs "
                         << coRequired.size() << " facts\n";
                }
            }
            else
            {
                if (!mapItr->second.empty())
                {
                    ...
                    if (localDebug && mapItr->second.empty())
                    {
                        cout << COLOUR_light_blue << "Now have no definite guards for effects on "
                             << *(RPGBuilder::getPNE(featured)) << COLOUR_default << endl;
                    }
                }
            }
        }
            ...
            for (int actID = 0; actID < actCount; ++actID)
            {
                ...
                set<int> local;
                {
                    ...
                    for (; preItr != preEnd; ++preItr)
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "CTS effects on " << *(RPGBuilder::getInstantiatedOp(actID))
                                 << ": must hold the semaphore on " << *(*preItr) << endl;
                        }
                        local.insert((*preItr)->getStateID());
                    }
                }
                ...
                for (; vItr != vEnd; ++vItr)
                {
                    ...
                    else
                    {
                        ...
                        {
                            set<int> isect;
                            std::set_intersection(local.begin(), local.end(), mapItr->second.begin(),
                                                  mapItr->second.end(),
                                                  std::insert_iterator<set<int>>(isect, isect.begin()));
                            isect.swap(mapItr->second);
