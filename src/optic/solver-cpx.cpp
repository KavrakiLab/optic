...
void MILPSolverCPX::addCol(const vector<pair<int, double>> &entries, const double &lb, const double &ub,
...
void MILPSolverCPX::setMatrixEntry(const int &row, const int &column, const double &coefficient)
{
    modelcon->data[row].setLinearCoef((*modelvar)[column], coefficient);
}
...
bool MILPSolverCPX::solve(const bool &skipPresolve)
{
    ...
    // cout << "Solving problem with " << cplex->getNcols() << " columns and " << cplex->getNrows() << "
    // rows\n";
    bool toReturn = false;
    try
    {
