...
namespace Planner
{
    ...
    public:
        ExpressionBuilder(list<RPGBuilder::Operand> &formulaIn, VAL::FastEnvironment *f,
                          VAL::TypeChecker *t = 0)
        ...
        bool buildFormula(VAL::expression *e)
        {
            if (debug)
            {
                cout << "Building numeric expression\n";
            }
            valid = true;
            e->visit(this);
            return valid;
        ...
        void visit_int_expression(const int_expression *s)
        {
            if (debug)
            {
                cout << "int term " << s->double_value() << endl;
            }
        ...
        void visit_float_expression(const float_expression *s)
        {
            if (debug)
            {
                cout << "float term " << s->double_value() << endl;
            }
            const double rounded =
                round(s->double_value() / Globals::numericTolerance) * Globals::numericTolerance;
            if (fabs(rounded - s->double_value()) > (Globals::numericTolerance * 0.1) &&
                !toleranceWarningGiven)
            {
                cerr << "Warning: rounding numeric constants such as " << s->double_value()
                     << " to an accuracy of " << Globals::numericTolerance << endl;
            ...
            else
            {
                ...
                else
                {
                        ...
                        realPNE->getHead()->getName().find("stochastic-") == 0)
                    {
                        if (debug)
                        {
                            cout << "Stochastic duration PNE " << *realPNE << ", ID "
                                 << realPNE->getGlobalID() << std::endl;
                        }
                        ...
                        if (realPNE->getStateID() == -1)
                    {
                        if (debug)
                        {
                            cout << "PNE " << *realPNE << ", with static value "
                                 << EFT(realPNE->getHead())
                                        ->getInitial(realPNE->begin(), realPNE->end())
                                        .second
                                 << std::endl;
                        }
                        const double originalValue =
                            EFT(realPNE->getHead())->getInitial(realPNE->begin(), realPNE->end()).second;
                        const double rounded =
                            round(originalValue / Globals::numericTolerance) * Globals::numericTolerance;
                        if (fabs(rounded - originalValue) > (Globals::numericTolerance * 0.1) &&
                            !toleranceWarningGiven)
                        {
                            cerr << "Warning: rounding numeric constants such as " << originalValue
                                 << " to an accuracy of " << Globals::numericTolerance << endl;
        ...
        for (; fItr != fEnd; ++fItr)
        {
            ...
            switch (currMathOp)
            {
                ...
                case RPGBuilder::NE_STOCHASTIC_DURATION_TERM:
                {
                    ...
                    RHS.push_front(
                        EFT(currOperand.durationVar->getHead())
                            ->getInitial(currOperand.durationVar->begin(), currOperand.durationVar->end())
                            .second);
                    break;
                }
#endif
    ...
    double RPGBuilder::ArtificialVariable::evaluateWCalculate(const vector<double> &fluentTable,
                                                              const int &pneCount)
    {
    ...
    double RPGBuilder::ArtificialVariable::evaluateWCalculate(const vector<double> &minFluentTable,
                                                              const vector<double> &maxFluentTable,
                                                              const int &pneCount) const
    {
        // cout << "Evaluating AV " << ID << " of size " << size << "\n";
        double toReturn = constant;
        // cout << "Evaluating AV of size " << size << "\n";
    ...
    bool RPGBuilder::RPGNumericPrecondition::isSatisfiedWCalculate(const vector<double> &maxFluents) const
    {
        ...
        if (localVal == std::numeric_limits<double>::signaling_NaN())
        {
            return false;
        }
        if (op == VAL::E_GREATER)
    ...
    bool RPGBuilder::RPGNumericPrecondition::isSatisfiedWCalculate(const vector<double> &minFluents,
                                                                   const vector<double> &maxFluents) const
    {
        const int pneCount = RPGBuilder::getPNECount();
        int var = LHSVariable;
        if (var < 0)
    ...
    bool RPGBuilder::RPGNumericPrecondition::canBeUnsatisfiedWCalculate(
        const vector<double> &minFluents, const vector<double> &maxFluents) const
    {
        const int pneCount = RPGBuilder::getPNECount();
        int var = LHSVariable;
        // cout << "Precondition based on variable " << var << "\n";
    ...
    RPGBuilder::NumericEffect::NumericEffect(const VAL::assign_op &opIn, const int &fIn,
                                             VAL::expression *formulaIn, VAL::FastEnvironment *f,
                                             VAL::TypeChecker *t)
    ...
    RPGBuilder::NumericPrecondition::NumericPrecondition(const VAL::comparison_op &opIn,
                                                         VAL::expression *LHSformulaIn,
                                                         VAL::expression *RHSformulaIn,
                                                         VAL::FastEnvironment *f, VAL::TypeChecker *t,
                                                         const bool p)
      : op(opIn), valid(true), polarity(p){{ExpressionBuilder builder(LHSformula, f, t);
...
bool RPGBuilder::RPGNumericEffect::operator<(const RPGNumericEffect &e) const
{
    ...
    if (!isAssignment && e.isAssignment)
    {
        return true;
    }
    if (isAssignment && !e.isAssignment)
...
struct InvData
{
    ...
    struct LTAVPointer
    {
        bool operator()(const RPGBuilder::ArtificialVariable *const a,
                        const RPGBuilder::ArtificialVariable *const b) const
        {
            return ((*a) < (*b));
        };
    };
    ...
    struct LTRNPPointer
    {
        bool operator()(const RPGBuilder::RPGNumericPrecondition *const a,
                        const RPGBuilder::RPGNumericPrecondition *const b) const
        {
            return ((*a) < (*b));
        };
    };
    ...
    InvData(vector<ArtificialVariable> &avs, vector<RPGNumericPrecondition> &pres)
      : avReuseInit(false)
      , preReuseInit(false)
      , baseNextAVIndex(-1)
      , nextAVIndex(-1)
      , baseNextPreIndex(-1)
      , nextPreIndex(-1)
      , rpgArtificialVariables(avs)
      , rpgNumericPreconditions(pres){};
    ...
    pair<ArtificialVariable *, bool> insertAV(ArtificialVariable *candidate)
    {
        if (!avReuseInit)
        {
            ...
            for (int s = 0; s < loopLim; ++s)
            {
                ...
                if (rpgArtificialVariables[s].ID > nextAVIndex)
                {
                    nextAVIndex = rpgArtificialVariables[s].ID;
                }
            }
            ++nextAVIndex;
    ...
    pair<const RPGNumericPrecondition *, bool> insertPre(RPGNumericPrecondition *const candidate)
    {
        if (!preReuseInit)
        {
            ...
            for (int s = 0; s < loopLim; ++s)
            {
                ...
                if (rpgNumericPreconditions[s].ID > nextPreIndex)
                {
                    nextPreIndex = rpgNumericPreconditions[s].ID;
                }
            }
            ++nextPreIndex;
        ...
        pair<set<const RPGNumericPrecondition *, LTRNPPointer>::iterator, bool> avrItr =
            preReuse.insert(candidate);
...
bool RPGBuilder::pushInvariantBackThroughStartEffects(
    const RPGBuilder::RPGNumericPrecondition &pre, list<int> &startEffs, RPGBuilder::LinearEffects *ctsEffs,
    InvData &commonData, pair<const RPGBuilder::RPGNumericPrecondition *, bool> &preResult,
    pair<RPGBuilder::ArtificialVariable *, bool> &avResult)
{
        ...
        if (lookup != lhs.end())
        {
            ...
            double localConst = currEff.constant * lookup->second;
            {
                ...
                for (int s = 0; s < loopLim; ++s)
                {
                    ...
                    {
                        localMap.insert(
                            make_pair(currEff.variables[s] - pneCount, -currEff.weights[s] * lookup->second));
                    }
                }
            }
    ...
    if (ctsEffs)
    {
        ...
        for (int v = 0; v < ctsVarCount; ++v)
        {
            ...
            if (lookup != lhs.end())
            {
                if (debug)
                {
                    cout << "- Relevant CTS effect: increase " << *(getPNE(ctsEffs->vars[v])) << " * #t "
                         << ctsEffs->effects[0][v].constant << endl;
                }
    ...
    for (; moItr != moEnd; ++moItr)
    {
        ...
        for (; mergeItr != mergeEnd; ++mergeItr)
        {
            ...
            if (debug)
            {
                cout << "* Changes coeffienct on " << *(getPNE(mergeItr->first)) << " to " << mergeItr->second
                     << endl;
            }
    ...
    if (fabs(rhs) < 0.00000000001)
    {
        rhs = 0.0;
    }

    if (lhs.empty())
    {  // variable-less precondition
...
void RPGBuilder::handleNumericInvariants()
{
...

    {
        ...
        {
            list<pair<int, Planner::time_spec>> &copyTo = processedRPGNumericPreconditionsToActions[i] =
                rpgNumericPreconditionsToActions[i];
    ...
    for (int i = 0; i < opCount; ++i)
    {
        if (!rogueActions[i])
        {
            ...
            list<int> &addToTwo = actionsToProcessedStartRPGNumericPreconditions[i] =
                actionsToRPGNumericStartPreconditions[i];

            // cout << "Before handing action " << i << " had " << addToTwo.size() << " numeric start pres\n";

            set<int> alreadyAtStart;
            alreadyAtStart.insert(actionsToRPGNumericStartPreconditions[i].begin(),
                                  actionsToRPGNumericStartPreconditions[i].end());
            ...
            bool eliminate = false;
            {
                ...
                for (bool first = true; liItr != liEnd; ++liItr, first = false)
                {
                    ...
                    const bool feasible = pushInvariantBackThroughStartEffects(
                        invPre, actionsToRPGNumericStartEffects[i], linearDiscretisation[i], commonData,
                        preResult, avResult);
            ...
            if (eliminate)
            {
                ...
                list<pair<const RPGNumericPrecondition *, bool>>::reverse_iterator preItr =
                    localPres.rbegin();
                const list<pair<const RPGNumericPrecondition *, bool>>::reverse_iterator preEnd =
                    localPres.rend();
            ...
            else
            {
                ...
                for (; preItr != preEnd; ++preItr)
                {
                    ...
                    precToActionMap.insert(make_pair(preItr->first->ID, emptyList))
    ...
    if (commonData.anyNewAVs())
    {
        ...
        for (; avItr != avEnd; ++avItr)
        {
            ...
            for (int aff = 0; aff < afflim; ++aff)
            {
                ...
                if (affVar >= 0)
                {
                    rpgVariableDependencies[affVar].push_back(currAV.ID);
                }
            }
        }
    }
    ...
    if (commonData.anyNewPres())
    {
        ...
        for (; preItr != preEnd; ++preItr)
        {
            ...
            {
                rpgArtificialVariablesToPreconditions[currPre.LHSVariable - 2 * pneCount].push_back(
                    currPre.ID);
                assert(currPre.ID < newPreLimit);
            }
    ...
    {
        list<pair<int, Planner::time_spec>> &destList =
            processedRPGNumericPreconditionsToActions[ptaItr->first];
    ...
    for (int i = 0; i < opCount; ++i)
    {
        if (!rogueActions[i])
        {
            assert(actionsToProcessedStartRPGNumericPreconditions[i].size() ==
                   ((unsigned int)initialUnsatisfiedProcessedStartNumericPreconditions[i]));
        }
    }
...
void RPGBuilder::makeOneSided(pair<list<double>, list<int>> &LHSvariable,
                              pair<list<double>, list<int>> &RHSvariable, const int &negOffset)
{
...

    {
        ...
        while (dlItr != dlEnd)
        {
            ...
            {  // for negative weights
                ...
                {  // keep vars here, but refer to negative instances and flip sign on weight
                    if (*ilItr >= 0)
                    {
                        *ilItr += negOffset;
                    }
    ...
    for (; tItr != tEnd; ++tItr)
    {
        if (tItr->second)
        {
            ...
            result.first.push_back(
                *wItr *
                EFT(tItr->second->getHead())->getInitial(tItr->second->begin(), tItr->second->end()).second);
...
void RPGBuilder::makeDurationWeightedSum(list<Operand> &formula,
    ...
    for (int st = 0; opItr != opEnd; ++opItr, ++st)
    {
        ...
        switch (currMathOp)
        {
            case RPGBuilder::NE_ADD:
            {
                ...
                formulaStack.front().first.insert(formulaStack.front().first.begin(), oldFront.first.begin(),
                                                  oldFront.first.end());
                formulaStack.front().second.insert(formulaStack.front().second.begin(),
                                                   oldFront.second.begin(), oldFront.second.end());
                if (stackDebug)
                {
                    cout << "Result:\n\t";
                    printStackTerm(formulaStack.front().first, formulaStack.front().second);
            ...
            {  // at least one of the terms has to be entirely conflict, otherwise we have var x * var y
                ...
                if (firstIsConst && secondIsConst)
                {
                    ...
                    formulaStack.front().first.push_back(entryIsConst(oldFront).second *
                                                         entryIsConst(oldSecondFront).second);
                }
            ...
            case RPGBuilder::NE_VIOLATION:
            {
                ...
                if (stackDebug)
                {
                    cout << "Preferences with name " << currOperand.isviolated << ": " << vID->second.size()
                         << endl;
                }
...
bool RPGBuilder::processPrecondition(RPGBuilder::BuildingNumericPreconditionData &commonData,
                                     RPGBuilder::NumericPrecondition &currPre, list<int> &conjunctiveList,
                                     list<int> &disjunctiveList, const int &i,
                                     const Planner::time_spec &passTimeSpec)
{
...

    {
        ...
        for (; lhsItr != lhsEnd; ++lhsItr, ++opItr, ++rhsItr)
        {
            ...
            if (!lIsConst)
            {
                ...
                if (lSize > 1)
                {
                    ...
                    ArtificialVariable newAV(commonData.avCount, a, wVector, vVector, constTerm, rConst,
                                             false, false);
                    pair<set<ArtificialVariable>::iterator, bool> insResult =
                        commonData.artificialVariableSet.insert(newAV);
                    if (insResult.second)
                    {
                        ++commonData.avCount;
                    }
                ...
                else if (lSize)
                {
                    ...
                    else
                    {
                        ...
                        assert(lConst > 0.0);  // would be insane otherwise - the negative variables thing
                                               // should have kicked in, and having 0*var as a function is
                                               // just plain silly
                        const double newMaxNeed = rConst / lConst;
                        if (lVar >= 0)
                        {
                            if (newMaxNeed > commonData.localMaxNeed[lVar])
                            {
                                commonData.localMaxNeed[lVar] = newMaxNeed;
                            }
                        }
                    }
                }
            ...
            {
                RPGNumericPrecondition newPrec(commonData.precCount, false, false, lVar, lConst, currOp,
                                               rConst);
                map<RPGNumericPrecondition, list<pair<int, Planner::time_spec>>>::iterator insResult =
                    commonData.rpgNumericPreconditionSet.find(newPrec);
                if (insResult == commonData.rpgNumericPreconditionSet.end())
                {
                    if (debugRPGNum)
                    {
                        cout << "New RPGNumericPrecondition created, ID = " << newPrec.ID << "\n";
                        cout << "lv = " << newPrec.LHSVariable << ", lc = " << newPrec.LHSConstant
                             << ", rv = " << newPrec.RHSVariable << ", rc = " << newPrec.RHSConstant << "\n";
                    }
                    ...
                    commonData.rpgNumericPreconditionSet.insert(
                        pair<RPGNumericPrecondition, list<pair<int, Planner::time_spec>>>(newPrec, tmpList));
                ...
                else
                {
                    if (debugRPGNum)
                    {
                        ...
                        cout << "lv = " << insResult->first.LHSVariable
                             << ", lc = " << insResult->first.LHSConstant
                             << ", rv = " << insResult->first.RHSVariable
                             << ", rc = " << insResult->first.RHSConstant << "\n";
                    }
                    if (i >= 0)
                    {
...
bool RPGBuilder::processPreconditions(RPGBuilder::BuildingNumericPreconditionData &commonData,
                                      ...
                                      int &toIncrement, const int &i, const Planner::time_spec &passTimeSpec)
{
    const bool debugRPGNum = (Globals::globalVerbosity & 16);
...
void RPGBuilder::buildRPGNumericPreconditions()
{
    ...
    for (int i = 0; i < opCount; ++i)
    {
        ...
        for (int pass = 0; pass < 3; ++pass)
        {
            ...
            switch (pass)
            {
                case 0:
                {
                    ...
                    if (debugRPGNum)
                    {
                        cout << "Building RPG Numeric Preconditions for start of " << *(instantiatedOps[i])
                             << "\n";
                    }
                }
                break;
                case 1:
                {
                    ...
                    if (debugRPGNum)
                    {
                        cout << "Building RPG Numeric Preconditions for over all of " << *(instantiatedOps[i])
                             << "\n";
                    }
                }
                break;
                case 2:
                {
                    ...
                    if (debugRPGNum)
                    {
                        cout << "Building RPG Numeric Preconditions for end of " << *(instantiatedOps[i])
                             << "\n";
                    }
                }
                break;
            ...
            if (!processPreconditions(commonData, (*actionsToNumericPreconditions)[i],
                                      (*actionsToRPGNumericPreconditions)[i],
        ...
        {
#ifndef NDEBUG
            const bool durative = (!fixedDurationExpressions[i].empty() ||
                                   !maxDurationExpressions[i].empty() || !minDurationExpressions[i].empty());
#endif
            ...
            for (int ceID = 0; !contradictoryPreconditions && ceItr != ceEnd; ++ceItr, ++prcItr, ++ceID)
            {
                ...
                for (int pass = 0; pass < 3; ++pass)
                {
                    ...
                    {
                        cout << "Looking at " << actionsToNumericPreconditions->size()
                             << " numeric conditions on conditional effect " << ceID << " of " << i << endl;
                    }

                    if (!processPreconditions(commonData, *actionsToNumericPreconditions, dest, unsat, -2,
                                              passTimeSpec))
                    {
                        if (Globals::globalVerbosity & 32)
                        {
                            cout << "Contradictory preconditions on conditional effect " << ceID << " of "
                                 << i << endl;
                        }
                        contradictoryPreconditions = true;
                        break;
        ...
        for (; goalNumItr != goalNumEnd; ++goalNumItr, ++goalDeadItr)
        {
            ...
            if (processPreconditions(commonData, justOne, dest, unsat, -1, Planner::E_AT))
            {
                ...
                if (destSize == 2)
                {
                    if (debugRPGNum)
                    {
                        cout << "Evaluated to pair " << dest.front() << ", " << dest.back() << "\n";
                    }
                ...
                else if (destSize == 1)
                {
                    if (debugRPGNum)
                    {
                        cout << "Evaluated to single goal " << dest.front() << "\n";
                    }
...

    {
        ...
        for (; avsItr != avsEnd; ++avsItr)
        {
            ...
            if (debugRPGNum)
            {
                cout << "Storing AV with ID " << avsItr->ID << " at index " << oIndex << "\n";
            }
            {
    ...
    {
        map<RPGNumericPrecondition, list<pair<int, Planner::time_spec>>>::iterator rnpItr =
            commonData.rpgNumericPreconditionSet.begin();
        const map<RPGNumericPrecondition, list<pair<int, Planner::time_spec>>>::iterator rnpEnd =
            commonData.rpgNumericPreconditionSet.end();
        ...
        for (; rnpItr != rnpEnd; ++rnpItr)
        {
            ...
            if (debugRPGNum)
            {
                ...
                list<pair<int, Planner::time_spec>>::iterator ataItr =
                    rpgNumericPreconditionsToActions[currID].begin();
                const list<pair<int, Planner::time_spec>>::iterator ataEnd =
                    rpgNumericPreconditionsToActions[currID].end();
                for (; ataItr != ataEnd; ++ataItr)
                {
                    cout << "\t" << ataItr->first << " " << *(instantiatedOps[ataItr->first]) << " "
                         << (ataItr->second == Planner::E_AT_START ?
                                 "start" :
                                 (ataItr->second == Planner::E_AT_END ? "end" : "over all"))
                         << "\n";
                }
            }
        }
    ...
    numericAchievedInLayer =
        vector<EpsilonResolutionTimestamp>(commonData.precCount, EpsilonResolutionTimestamp::undefined());
    numericAchievedInLayerReset =
        vector<EpsilonResolutionTimestamp>(commonData.precCount, EpsilonResolutionTimestamp::undefined());
    numericAchievedBy =
        vector<ActionFluentModification *>(commonData.precCount, (ActionFluentModification *)0);
...
void RPGBuilder::buildRPGNumericEffects()
{
    ...
    for (int act = 0; act < opCount; ++act)
    {
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                ...
                for (; neItr != neEnd; ++neItr)
                {
                    if (localDebug)
                    {
                        ...
                        {
                            cout << "Conditional effect of action " << act << " has "
                                 << (pass ? "end" : "start") << " effect:\n";
                        }
                    }
                    ...
                    RPGNumericEffect rpgEffect(effID, currEffect.fluentIndex, currEffect.op == VAL::E_ASSIGN,
                                               wVector, vVector, a, constTerm);

                    pair<map<RPGNumericEffect, list<pair<int, Planner::time_spec>>>::iterator, bool> insItr =
                        rpgNumericEffectSet.insert(
                            pair<RPGNumericEffect, list<pair<int, Planner::time_spec>>>(rpgEffect,
                                                                                        emptyList));
                    ...
                    if (doingMainEffects)
                    {
                        insItr.first->second.push_back(pair<int, Planner::time_spec>(
                            act, (pass ? Planner::E_AT_END : Planner::E_AT_START)));
                        (pass ? actionsToRPGNumericEndEffects : actionsToRPGNumericStartEffects)[act]
                            .push_back(insItr.first->first.ID);
                    }
                    else
                    {
                        (*prcItr).addNumericEffect(insItr.first->first.ID,
                                                   (pass ? Planner::E_AT_END : Planner::E_AT_START), act);
                    }
                    ...
                    if (localDebug)
                    {
                        ...
                        if (insItr.second)
                        {
                            cout << "Effect hasn't been seen before anywhere\n";
                        }
                    }
                }
            }
        ...
        map<RPGNumericEffect, list<pair<int, Planner::time_spec>>>::iterator effItr =
            rpgNumericEffectSet.begin();
        const map<RPGNumericEffect, list<pair<int, Planner::time_spec>>>::iterator effEnd =
            rpgNumericEffectSet.end();
...
struct NiceFormOfPrecondition
{
    ...
    bool isTheOppositeOf(const NiceFormOfPrecondition &other) const
    {
        ...
        if (op == other.op)
        {  // must have >= and >, in some combination
            return false;
        }
...
void RPGBuilder::detectConditionalEffectsThatEncodeIntegralOutcomes()
{
    ...
    for (int act = 0; act < opCount; ++act)
    {
...

        {
            ...
            for (int oc = 0; ceItr != ceEnd; ++ceItr, ++oc)
            {
                ...
                if (outcomes[oc]->getNumericPreconditions().size() != 2)
                {
                    if (localDebug)
                    {
                        cout << *(RPGBuilder::getInstantiatedOp(act))
                             << " doesn't have two numeric preconditions\n";
                    }
                    sound = false;
                    break;
                }
        ...
        // find an end point - do we have an outcome which has a precondition that nothing else has the
        // 'opposite' of
        ...
        for (int oc = 0; oc < outcomeCount; ++oc)
        {
            ...
            list<pair<int, Planner::time_spec>>::const_iterator pItr =
                outcomes[oc]->getNumericPreconditions().begin();
            const list<pair<int, Planner::time_spec>>::const_iterator pEnd =
                outcomes[oc]->getNumericPreconditions().end();
            ...
            for (int pID = 0; pItr != pEnd; ++pItr, ++pID)
            {
                ...
                for (int oc2 = 1; oc2 < outcomeCount; ++oc2)
                {
                    ...
                    list<pair<int, Planner::time_spec>>::const_iterator p2Itr =
                        outcomes[oc2]->getNumericPreconditions().begin();
                    const list<pair<int, Planner::time_spec>>::const_iterator p2End =
                        outcomes[oc2]->getNumericPreconditions().end();
                    ...
                    for (; p2Itr != p2End; ++p2Itr)
                    {
                        ...
                        if (NiceFormOfPrecondition(p2Itr->first).isTheOppositeOf(here))
                        {
                            if (localDebug)
                            {
                                cout << "The opposite of " << RPGBuilder::getNumericPreTable()[pItr->first]
                                     << " is " << RPGBuilder::getNumericPreTable()[p2Itr->first] << endl;
                            }
                            foundOpposite = true;
                            break;
                        }
                ...
                if (!foundOpposite)
                {
                    if (localDebug)
                    {
                        cout << "Not found anything opposite to "
                             << RPGBuilder::getNumericPreTable()[pItr->first] << ", so outcome " << oc
                             << " is an end point\n";
                    }
                    ocEndPoint = oc;
                    ...
                    if (pID == 0)
                    {
                        rightPrecondition.push_back(make_pair(
                            NiceFormOfPrecondition(outcomes[oc]->getNumericPreconditions().back().first),
                            outcomes[oc]->getNumericPreconditions().back().second));
                    }
                    else
                    {
                        rightPrecondition.push_back(make_pair(
                            NiceFormOfPrecondition(outcomes[oc]->getNumericPreconditions().front().first),
        ...
        while (toVisit != -1)
        {
            ...
            for (int pID = 0; pID < 2; ++pID)
            {
                if (localDebug)
                {
                    if (pID)
                    {
                        cout << "At " << nowVisiting << ", outcome " << oc << ", looking to the left of "
                             << RPGBuilder::getNumericPreTable()[leftPrecondition[nowVisiting]
                                                                     .first.originalID]
                             << endl;
                    }
                    else
                    {
                        cout << "At " << nowVisiting << ", outcome " << oc << ", looking to the right of "
                             << RPGBuilder::getNumericPreTable()[rightPrecondition[nowVisiting]
                                                                     .first.originalID]
                             << endl;
                    }
                }
                const pair<NiceFormOfPrecondition, Planner::time_spec> &here =
                    (pID == 0 ? rightPrecondition[nowVisiting] : leftPrecondition[nowVisiting]);
                    ...
                    for (int preID = 0; p2Itr != p2End; ++p2Itr, ++preID)
                    {
                        ...
                        if (comp.isTheOppositeOf(here.first))
                        {
                            if (adjacent[pID] == -1)
                            {
                                if (localDebug)
                                {
                                    cout << "- The opposite is outcome " << oc2 << ": "
                                         << RPGBuilder::getNumericPreTable()[p2Itr->first] << endl;
                                }
            ...
            else if (adjacent[0] != -1 && adjacent[1] != -1)
            {
                ...
                if (ordered.size() == 1 || ordered.size() == outcomeCount)
                {  // at the ends
                    if (localDebug)
                    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "Next in sequence, position " << toVisit << ", is outcome "
                                 << ordered.back() << endl;
                        }
                    }
                }
            ...
            {
                if (ordered.size() != outcomeCount)
                {  // should be on the right hand end
                    if (localDebug)
                    {
            ...
            if (toVisit == nowVisiting + 1)
            {
                int done = 0;
                list<pair<int, Planner::time_spec>>::const_iterator p2Itr =
                    outcomes[ordered.back()]->getNumericPreconditions().begin();
                const list<pair<int, Planner::time_spec>>::const_iterator p2End =
                    outcomes[ordered.back()]->getNumericPreconditions().end();
                ...
                for (; p2Itr != p2End; ++p2Itr)
                {
                    ...
                    else
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "* Its right precondition is "
                                 << RPGBuilder::getNumericPreTable()[p2Itr->first] << endl;
                        }
                    }
                }
        ...
        {
            if (fabs((leftPrecondition[oc].first.rhs - leftPrecondition[oc - 1].first.rhs) - leftDifference) >
                0.0000001)
            {
                sound = false;
            }
        ...
        for (int oc = 0; oc < outcomeCount; ++oc)
        {
            list<pair<int, Planner::time_spec>>::const_iterator effItr =
                outcomes[oc]->getNumericEffects().begin();
            const list<pair<int, Planner::time_spec>>::const_iterator effEnd =
                outcomes[oc]->getNumericEffects().end();

            for (; effItr != effEnd; ++effItr)
            {
                if (effItr->second == Planner::E_AT_START)
                {
                    startEffectsDueToOutcome[oc].insert(
                        make_pair(RPGBuilder::getNumericEff()[effItr->first].fluentIndex, effItr->first));
                }
                else
                {
                    endEffectsDueToOutcome[oc].insert(
                        make_pair(RPGBuilder::getNumericEff()[effItr->first].fluentIndex, effItr->first));
                }
            }
        }
        ...
        for (int pass = 0; sound && pass < 2; ++pass)
        {
            ...
            for (int oc = 1; sound && oc < outcomeCount; ++oc)
            {
                ...
                for (; thisEffItr != thisEffEnd && otherEffItr != otherEffEnd; ++thisEffItr, ++otherEffItr)
                {
                    ...
                    const RPGBuilder::RPGNumericEffect &thisEff =
                        RPGBuilder::getNumericEff()[thisEffItr->second];
                    const RPGBuilder::RPGNumericEffect &otherEff =
                        RPGBuilder::getNumericEff()[otherEffItr->second];
                    ...
                    if (oc == 1)
                    {
                        differenceMap.insert(
                            make_pair(thisEffItr->first, thisEff.constant - otherEff.constant));
                    }
                    else
                    {
        ...
        toUpdate.push_back(IntegralContinuousEffect(
            act, outcomeCount - 1, leftPrecondition[0].first.originalID, leftPrecondition[0].second,
            leftDifference, rightPrecondition[0].first.originalID, rightDifference));
        ...
        {
            cout << *(RPGBuilder::getInstantiatedOp(act))
                 << " has an integral outcome series encoded using conditional effects\n";
        }
    }
}

bool RPGBuilder::IntegralContinuousEffect::satisfied(const Planner::time_spec &ts, const vector<double> &min,
                                                     const vector<double> &max, const double &minDur,
                                                     const double &maxDur,
                                                     pair<int, int> &outcomeBracket) const
{
    ...
    if (debug)
    {
        cout << "Seeing if an ICE attached to " << *(RPGBuilder::getInstantiatedOp(parentAction))
             << " based around " << currPre << " fires\n";
    }
    ...
    else
    {
        ...
        if (currPre.LHSVariable < pneCount)
        {
            ...
            if (debug)
            {
                cout << "+ " << *(RPGBuilder::getPNE(currPre.LHSVariable)) << " : " << minLHS << " to "
                     << evaluatedLHS << endl;
            }
        }
        else if (currPre.LHSVariable < 2 * pneCount)
        ...
        else
        {
            ...
            for (int s = 0; s < currAV.size; ++s)
            {
                if (currAV.fluents[s] == -19)
                {
                    ...
                    if (debug)
                    {
                        cout << "+ " << currAV.weights[s] << " * -?duration : " << -currAV.weights[s] * maxDur
                             << " to " << -currAV.weights[s] * minDur << endl;
                    }
                }
                else if (currAV.fluents[s] == -3)
                {
                    ...
                    if (debug)
                    {
                        cout << "+ " << currAV.weights[s] << " * ?duration : " << currAV.weights[s] * minDur
                             << " to " << currAV.weights[s] * maxDur << endl;
                    }
                }
                else
                {
                    ...
                    if (currAV.fluents[s] < pneCount)
                    {
                        ...
                        if (debug)
                        {
                            cout << "+ " << currAV.weights[s] << " * "
                                 << *(RPGBuilder::getPNE(currAV.fluents[s])) << " : "
                                 << currAV.weights[s] * min[currAV.fluents[s]] << " to "
                                 << currAV.weights[s] * max[currAV.fluents[s]] << endl;
                        }
                    }
                    else
                    {
                        ...
                        {
                            cout << "- " << currAV.weights[s] << " * "
                                 << *(RPGBuilder::getPNE(currAV.fluents[s] - pneCount)) << " : "
                                 << -currAV.weights[s] * max[currAV.fluents[s] - pneCount] << " to "
                                 << -currAV.weights[s] * min[currAV.fluents[s] * pneCount] << endl;
                        }
                    }
                }
            }
    ...
    if (currPre.op == VAL::E_GREATER ? diff > (outcomes * localLeftPreconditionDifference) :
                                       diff >= (outcomes * localLeftPreconditionDifference))
    {
        if (debug)
        {
...
void RPGBuilder::IntegralContinuousEffect::workOutSuitablePreconditionRelaxation()
{
    ...
    const bool fires =
        satisfied(leftPreconditionTS, dummyMin, dummyMax, RPGBuilder::getOpMinDuration(parentAction, -1),
                  RPGBuilder::getOpMaxDuration(parentAction, -1), outcomeBracket);

    if (outcomeBracket.first > 0 || outcomeBracket.second < outcomes)
    {
        cout << "Only keeping outcomes " << outcomeBracket.first << " to " << outcomeBracket.second << " of "
             << *(RPGBuilder::getInstantiatedOp(parentAction)) << endl;
    }
...
void RPGBuilder::IntegralContinuousEffect::getRelaxedStartEffects(list<int> &writeTo,
                                                                  const double &currLayer) const
{
    if (minimalOutcome == -1)
    {
        return;
    ...
    const bool fires =
        satisfied(Planner::E_AT_START, dummyMin, dummyMax, RPGBuilder::getOpMinDuration(parentAction, -1),
                  RPGBuilder::getOpMaxDuration(parentAction, -1), outcomeBracket);
    ...
    if (fires)
    {
        ...
        {
            writeTo.insert(writeTo.end(), startEffectsForOutcome[oc].begin(),
                           startEffectsForOutcome[oc].end());
        }
    }
}

void RPGBuilder::IntegralContinuousEffect::getRelaxedEndEffects(list<int> &writeTo,
                                                                const double &currLayer) const
{
    if (minimalOutcome == -1)
    {
    ...
    const bool fires =
        satisfied(Planner::E_AT_END, dummyMin, dummyMax, RPGBuilder::getOpMinDuration(parentAction, -1),
                  RPGBuilder::getOpMaxDuration(parentAction, -1), outcomeBracket);
...
{
    if (!ms || !Globals::optimiseSolutionQuality)
    {
        return;
    }
    ...
    while (varItr != varEnd)
    {
        if (*varItr == -1)
        {
            ...
            const list<int>::iterator varPrev = varItr;
            ++varItr;
            const list<double>::iterator wPrev = wItr;
            ++wItr;
        ...
        else
        {
            ...
            // cout << "Changed cost of preference " << preferences[vtu].name << " from " <<
            // preferences[vtu].cost << " to ";

            preferences[vtu].cost += wtu;

            if (metricDebug)
            {
                cout << "+ weight on preference " << preferences[vtu].name << ":" << vtu << " of " << wtu
                     << endl;
            }
