...
namespace Planner
{
    ...
    enum NNFNodeType
    {
        ...
        NNF_EQ_LITEND,
        NNF_EQ_NOTLITSTART,
        NNF_EQ_NOTLITOVERALL,
        NNF_EQ_NOTLITEND,
        NNF_EQ_NUMSTART,
        NNF_EQ_NUMOVERALL,
        NNF_EQ_NUMEND,
        NNF_EQ_NOTNUMSTART,
        NNF_EQ_NOTNUMOVERALL,
        NNF_EQ_NOTNUMEND,
        NNF_EQ_RPGNUMSTART,
        NNF_EQ_RPGNUMOVERALL,
        NNF_EQ_RPGNUMEND
    };
    ...
    public:
        ...
        {
            std::cerr << "Fatal internal error: this visitor expects RPG numerics to have been substituted "
                         "in\n";
            exit(1);
        }
    ...
    public:
        virtual void prefixForComparisons(list<pair<NNFNodeType, int>> &dest) const
        {
            ...
            for (; cItr != cEnd; ++cItr)
            {
                (*cItr)->prefixForComparisons(dest);
            }
        };
        ...
        virtual const list<NNFNode *> &getChildren() const
        {
            return children;
        };

        virtual void stealChildren(list<NNFNode *> &c)
        {
            c.clear();
            c.swap(children);
            csize = 0;
    ...
    {
    protected:
        virtual NNFNodeType getType() const
        {
            return NNF_EQ_AND;
        };
    ...
    public:
        ...
        virtual Literal *getLiteral() const
        {
            return lit;
        };

        virtual VAL::time_spec getTS() const
        {
            return ts;
        };
    ...
    public:
        ...
        virtual RPGBuilder::NumericPrecondition *getPre()
        {
            return pre;
        };
    ...
    public:
        ...
        NNFNode_NOT_Numeric(RPGBuilder::NumericPrecondition *p, const VAL::time_spec &t)
          : NNFNode_Numeric(p, t){};
    ...
    public:
        ...
        virtual const int &getPre() const
        {
            return pre;
        };

        virtual const VAL::time_spec &getTS() const
        {
            return ts;
        };
    ...
    public:
        NNF_Flat(const list<pair<int, int>> &usr, const list<pair<Cell, int>> &cellAndParent,
                 const list<bool> &r);
        ...
        inline void reset()
        {
            memcpy(unsat, unsatReset, resetSize);
            memset(fragilityTrue, 0, resetSize);
            memset(fragilityFalse, 0, resetSize);
        };

        inline void satisfy(const int &i)
        {
            for (currCell = parentID[i]; currCell != -1 && (--(unsat[currCell]) == 0);
        ...
        inline void satisfy(const int &i, vector<double> &earliestForNode, double now)
        {
            ...
            for (currCell = parentID[i]; currCell != -1; currCell = parentID[currCell])
            {
                ...
                else
                {
                    ...
                    if (earliestForNode[currCell] < now)
                    {  // the parent must need this, at least for the time being
                ...
                // if we get to here, either something has become satisfied for the first time, or has become
                // satisfied earlier
        ...
        {
            for (currCell = parentID[i]; currCell != -1 && (++(unsat[currCell]) == 1);
                 currCell = parentID[currCell])
                ;
        };
        ...
        template <typename T>
        void fillDependencyTables(vector<list<LiteralCellDependency<T>>> &litTable,
                                  vector<list<LiteralCellDependency<T>>> &negativeLitTable,
                                  vector<list<LiteralCellDependency<T>>> &numTable, list<int> &llist,
                                  list<int> &nllist, list<int> &nlist, const T &dest)
        {
            for (int i = 0; i < cellCount; ++i)
            {
                if (cells[i].lit)
                {
                    ...
                    {
                        negativeLitTable[cells[i].lit->getStateID()].push_back(
                            LiteralCellDependency<T>(dest, i));
    ...
    private:
        ...
        public:
            NNFBuildDep0(vector<list<LiteralCellDependency<T>>> &table, T &destIn)
              : depTable(table), dest(destIn){};

            virtual void visit_Literal(NNFNode_Literal *litNode)
            {
                int dummy = -1;
                depTable[litNode->getLiteral()->getStateID()].push_back(
                    LiteralCellDependency<T>(dest, dummy));
            }
        };
    ...
    public:
        ...
        static pair<NNFNode *, bool> pruneStaticLiterals(NNFNode *const root,
                                                         vector<pair<bool, bool>> &staticLiterals);
        static pair<NNFNode *, bool> pruneStaticNumerics(NNFNode *const root,
                                                         vector<pair<bool, bool>> &staticNumerics);
        static pair<NNFNode *, bool> pruneUnreachable(NNFNode *const root, LiteralSet &initialState);
        static pair<NNFNode *, bool>
        substituteRPGNumerics(NNFNode *const root, const bool &desirable, const bool &undesirable,
                              RPGBuilder::BuildingNumericPreconditionData &commonData);

        static void findFactsDefinitelyNeeded(NNFNode *const root, set<int> &mustBeTrue,
                                              set<int> &mustBeFalse);

        template <typename T>
        static void buildLiteralsToPreconditions(NNFNode *const root,
                                                 vector<list<LiteralCellDependency<T>>> &litTable, T &dest)
        {
        ...
        static void buildLiteralsToPreconditions(NNF_Flat *const root,
                                                 vector<list<LiteralCellDependency<T>>> &litTable,
                                                 vector<list<LiteralCellDependency<T>>> &negativeLitTable,
                                                 vector<list<LiteralCellDependency<T>>> &numTable,
                                                 list<int> &thisToLit, list<int> &thisToNegLit,
                                                 list<int> &thisToNum, const T &dest)
        {
            root->fillDependencyTables<T>(litTable, negativeLitTable, numTable, thisToLit, thisToNegLit,
                                          thisToNum, dest);
        };
    };
