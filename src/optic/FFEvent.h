...
namespace Planner
{
    ...
    struct StartEvent
    {
        ...
        StartEvent(const int &a, const int &da, const int &s, const double &mind, const double &maxd,
                   const double &e)
          : actID(a)
          , divisionsApplied(da)
          , stepID(s)
          , advancingDuration(mind)
          , minDuration(mind)
          , maxDuration(maxd)
          , elapsed(e)
          , minAdvance(DBL_MAX)
        ...
        {
            return (actID == e.actID && divisionsApplied == e.divisionsApplied && stepID == e.stepID &&
                    fabs(minDuration - e.minDuration) < 0.0005 &&
                    fabs(maxDuration - e.maxDuration) < 0.0005 && fabs(elapsed - e.elapsed) < 0.0005 &&
                    fabs(advancingDuration - e.advancingDuration) < 0.0005 &&
                    //          compulsaryEnd == e.compulsaryEnd &&
                    terminated == e.terminated && fanIn == e.fanIn && endComesBefore == e.endComesBefore);
        }

        void endMustComeAfter(const int &i)
        {
            assert(i >= 0);
            endComesAfter.insert(i);
        }

        void endMustComeAfterPair(const int &i)
        {
            assert(i >= 0);
            endComesAfterPair.insert(i);
        }

        void actionHasFinished(const int &i)
        {
            assert(i >= 0);
            endComesAfter.erase(i);
        }
    };
    ...
    public:
         ...
         * - <code>Planner::E_AT_START</code> if <code>action</code> is the start of an action, or an
         * instantaneous action
         * - <code>Planner::E_AT_END</code> if <code>action</code> is the start of an action
         * - <code>Planner::E_AT</code> for a timed-initial literal, where <code>divisionID</code> is the
         * index into <code>RPGBuilder::timedInitialLiteralsVector</code>
         * - <code>Planner::E_CONTINUOUS</code> for dummy events inserted to collate the facts relevant to the
         * status of non-temporal preferences.
         * - <code>Planner::E_DUMMY_TEMPORAL_TRIGGER</code> for dummy events inserted to collate the facts
         * relevant to the trigger of a temporal preference, with preference ID <code>divisionID</code>
         * - <code>Planner::E_DUMMY_TEMPORAL_GOAL</code> for dummy events inserted to collate the facts
         * relevant to the goal of a temporal preference, with preference ID <code>divisionID</code>
         * - <code>Planner::E_DUMMY_PREFERENCE_PRECONDITION</code> for dummy events inserted to hold soft
         * temporal constraints to meet preference preconditions.
         */
        ...
        /** @brief For end actions, if they've been applied yet; for dummy steps, whether to add their
         * temporal constraints.
         *
         * This serves two purposes:
         * - If <code>time_spec</code> is <code>Planner::E_AT_END</code>, then if this variable holds the
         * value <code>true</code>, the end of the action has been applied.  Otherwise, it's a placeholder.
         * - If this is a dummy event, then if this variable holds the value <code>true</code>, the temporal
         * constraints are safe to add (or must always be added) to the STP/LP.
         */
        bool getEffects;
        double lpTimestamp;
        double lpMinTimestamp;
        ...
        {
            if (time_spec == Planner::E_AT_END && pairWithStep != f.pairWithStep)
            {
                return false;
            }
            return (action == f.action && time_spec == f.time_spec && minDuration == f.minDuration &&
                    maxDuration == f.maxDuration && pairWithStep == f.pairWithStep &&
                    getEffects == f.getEffects && divisionID == f.divisionID);
        }

        bool isDummyStep() const
        {
            return (time_spec == Planner::E_CONTINUOUS || time_spec == Planner::E_DUMMY_TEMPORAL_GOAL_TRUE ||
                    time_spec == Planner::E_DUMMY_TEMPORAL_TRIGGER_TRUE ||
                    time_spec == Planner::E_DUMMY_TEMPORAL_GOAL_FALSE ||
                    time_spec == Planner::E_DUMMY_TEMPORAL_TRIGGER_FALSE ||
