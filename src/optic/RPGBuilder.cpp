...
namespace Planner
{
    ...
    public:
        ...
        GoalNumericCollector(list<RPGBuilder::NumericPrecondition> *ng, list<double> *ngD, list<Literal *> *g,
                             list<double> *gD, map<string, list<int>> &p, map<string, int> &pv,
                             const VAL::operator_ *o, FastEnvironment *f, VAL::TypeChecker *t = 0)
          : numericGoals(ng)
          , numericGoalDeadlines(ngD)
          , literalGoals(g)
          , literalGoalDeadlines(gD)
          , numToUse(numericGoals)
          , numDToUse(numericGoalDeadlines)
          , litToUse(literalGoals)
          , litDToUse(literalGoalDeadlines)
          , prefNameToID(p)
          , prefNameToNumberOfTimesDefinitelyViolated(pv)
          , tc(t)
          , adding(true)
          , op(o)
          , fe(f)
        ...
        virtual void visit_comparison(comparison *c)
        {
            ...
            numToUse->push_back(
                RPGBuilder::NumericPrecondition(c->getOp(), const_cast<VAL::expression *>(c->getLHS()),
                                                const_cast<VAL::expression *>(c->getRHS()), fe, tc));
        ...
        virtual void visit_simple_goal(simple_goal *p)
        {
            if (checkingIfConstraintCouldOnlyBeFalse)
            {
                ...
                {
                    VAL::LiteralParameterIterator<VAL::parameter_symbol_list::iterator> tmpBegin =
                        tmp.begin();
        ...
        virtual void visit_preference(preference *p)
        {
            ...
            if (!inConstraints)
            {
                ...
                else
                {
                    if (cr == -1)
                    {
                        // cout << "Warning: at-end preference " << p->getName() << " will always be violated,
                        // ignoring\n";
                        ++(prefNameToNumberOfTimesDefinitelyViolated.insert(make_pair(p->getName(), 0))
                               .first->second);
                    }
                    if (cr == 1)
                    {
        ...
        virtual void visit_constraint_goal(constraint_goal *cg)
        {
                ...
                removeFrom.back().cons != E_WITHIN && removeFrom.back().cons != E_ALWAYSWITHIN)
            {
                ...
                if (prefName != "anonymous")
                {
                    prefNameToID[prefName].push_back(-1);
                }
                removeFrom.pop_back();
            }
            else
                ...
                switch (cg->getCons())
                {
                    ...
                    case VAL::E_WITHIN:
                    {
                        ...
                        else if (cr == -1)
                        {
                            ...
                            ++(prefNameToNumberOfTimesDefinitelyViolated.insert(make_pair(prefName, 0))
                                   .first->second);
                        }
                        break;
                    }
                    ...
                    default:
                    {
                        cout << "Fatal internal error: should never get to this point without having matched "
                                "the preference type\n";
                        exit(1);
                    }
                }
    ...
    void postmortem_noConstraints(const bool unsupportedPref, const char *n)
    {
        ...
        else
        {
            ...
            if (!warned)
            {
                ...
                cerr << "The planner will continue, but will quietly ignore one or more constraints - "
    ...
    void postmortem_twoSimulataneousNumericEffectsOnTheSameVariable(const string &actName,
    ...
    void postmortem_noTimeSpecifierOnInstantNumericEffect(const string &actname, const string &effect,
                                                          const string &suggested, const bool &isAssign)
    {
        cerr << "A problem has been encountered with your domain/problem file.\n";
        cerr << "-------------------------------------------------------------\n";
        cerr << "Unfortunately, a bug has been encountered in your domain and problem file,\n";
    ...
    void postmortem_wrongNumberOfFluentArguments(const string &actname, const bool &haveActName,
                                                 const whereAreWe &w, const string &predname,
                                                 const string &lit, const int &givenArgs,
                                                 const set<int> &realargs)
    {
        cerr << "A problem has been encountered with your domain/problem file.\n";
        cerr << "-------------------------------------------------------------\n";
        cerr << "Unfortunately, a bug has been encountered in your domain and problem file,\n";
    ...
    void postmortem_wrongNumberOfPredicateArguments(const string &actname, const bool &haveActName,
                                                    const whereAreWe &w, const string &predname,
                                                    const string &lit, const int &givenargs,
                                                    const set<int> &realargs)
    {
    ...
    public:
        ...
        TimedPrecEffCollector(instantiatedOp *currIOP, const VAL::operator_ *o, map<string, list<int>> &p,
                              FastEnvironment *f, VAL::TypeChecker *t = 0)
          : thisIOP(currIOP)
          , tc(t)
          , adding(true)
          , op(o)
        ...
        virtual void visit_simple_goal(simple_goal *p)
        {
            ...
            if (!addLit)
            {
                ...
                else
                {
                    ...
                    if (debug)
                    {
                        cout << "\t\tNull, but don't care - is a negative pre\n";
                    }
                }
            }
            else
        ...
        virtual void visit_comparison(comparison *c)
        {
            if (visitingDuration)
            {
                ...
                RPGBuilder::NumericPrecondition *const newPre = new RPGBuilder::NumericPrecondition(
                    c->getOp(), const_cast<VAL::expression *>(c->getLHS()),
                    const_cast<VAL::expression *>(c->getRHS()), fe, tc, adding);
                if (newPre->valid)
                {
            ...
            {
                addToListNumeric->push_back(RPGBuilder::NumericPrecondition(
                    c->getOp(), const_cast<VAL::expression *>(c->getLHS()),
                    const_cast<VAL::expression *>(c->getRHS()), fe, tc, adding));
        ...
        virtual void visit_disj_goal_internal(const goal_list *gl)
        {
            ...
            if (!tautology)
            {
                if (conds.size() + negativeConds.size() + numericConds.size() > 1)
                {
                    postmortem_noADL();
                }
            }
        ...
        virtual void visit_timed_goal(timed_goal *p)
        {
            ...
            switch (p->getTime())
            {
                ...
                case Planner::E_OVER_ALL:
                    ...
                    break;
                default:
                    cout << "Error, unsupported precondition time specification in action schema (not start, "
                            "end, or over all)\n";
                    exit(0);
                    break;
            };
        ...
        virtual void visit_cond_effect(cond_effect *p)
        {
            ...
            literalAddTos.back()[0] =
                RPGBuilder::NoDuplicatePair(&(condEffs.back()->startPrec), &(condEffs.back()->startPrecSet));
            literalAddTos.back()[1] =
                RPGBuilder::NoDuplicatePair(&(condEffs.back()->inv), &(condEffs.back()->invSet));
            literalAddTos.back()[2] =
                RPGBuilder::NoDuplicatePair(&(condEffs.back()->endPrec), &(condEffs.back()->endPrecSet));

            literalNegativeAddTos.back()[0] = RPGBuilder::NoDuplicatePair(
                &(condEffs.back()->startNegPrec), &(condEffs.back()->startNegPrecSet));
            literalNegativeAddTos.back()[1] =
                RPGBuilder::NoDuplicatePair(&(condEffs.back()->negInv), &(condEffs.back()->negInvSet));
            literalNegativeAddTos.back()[2] = RPGBuilder::NoDuplicatePair(&(condEffs.back()->endNegPrec),
                                                                          &(condEffs.back()->endNegPrecSet));
            ...
            const pair<list<RPGBuilder::NumericEffect> *, set<int> *> oldAddEffToListNumeric =
                addEffToListNumeric;
            ...
            if (!isDurative)
            {
                ...
                addToListNegative = literalNegativeAddTos.back()[0];
                addToListNumeric = &(condEffs.back()->startPrecNumeric);
                addEffToList = RPGBuilder::NoDuplicatePair(&(condEffs.back()->startAddEff),
                                                           &(condEffs.back()->startAddEffSet));
                delEffToList = RPGBuilder::NoDuplicatePair(&(condEffs.back()->startDelEff),
                                                           &(condEffs.back()->startDelEffSet));
                addEffToListNumeric =
                    make_pair(&(condEffs.back()->startNumericEff), &(condEffs.back()->startNumericEffsOnVar));
            }
            else
            {
        ...
        virtual void visit_preference(preference *p)
        {
            ...
            const int tsAsInt =
                (preconditionTS == Planner::E_AT_START ? 0 : (preconditionTS == Planner::E_AT_END ? 2 : 1));
        ...
        virtual void visit_timed_effect(timed_effect *p)
        {
            ...
            switch (p->ts)
            {
                case Planner::E_AT_END:
                {
                    ...
                    if (visitingWhen)
                    {
                        addEffToList = RPGBuilder::NoDuplicatePair(&(condEffs.back()->endAddEff),
                                                                   &(condEffs.back()->endAddEffSet));
                        delEffToList = RPGBuilder::NoDuplicatePair(&(condEffs.back()->endDelEff),
                                                                   &(condEffs.back()->endDelEffSet));
                ...
                default:
                {
                    cout << "Error, unsupported effect time specification " << p->ts << " in action schema "
                         << *toBlame << " (not start or end, or continuous)\n";
                    exit(0);
                }
            };
            p->effs->visit(this);
            ...
            if (debug)
            {
                cout << "Effect visited; reverting to numeric effect list and set at "
                     << addEffToListNumeric.first << " and " << addEffToListNumeric.second << endl;
            }
        };
        ...
        virtual void visit_operator_(VAL::operator_ *p)
        {
            if (debug)
            {
                cout << "Going through preconditions\n";
            }
            adding = true;
            inpres = true;
            checkpos = true;
        ...
        virtual void visit_action(VAL::action *p)
        {
            ...
            literalAddTos.push_back(vector<RPGBuilder::NoDuplicatePair>(3));
            literalNegativeAddTos.push_back(vector<RPGBuilder::NoDuplicatePair>(3));
            numericAddTos.push_back(vector<list<RPGBuilder::NumericPrecondition> *>(
                3, (list<RPGBuilder::NumericPrecondition> *)0));
        ...
        virtual void visit_assignment(assignment *a)
        {
            ...
            {
                startNumericEff.push_back(RPGBuilder::NumericEffect(
                    a->getOp(), pne->getStateID(), const_cast<VAL::expression *>(a->getExpr()), fe, tc));
                ...
                for (; fItr != fEnd; ++fItr)
                {
                    if (fItr->numericOp == RPGBuilder::NE_FLUENT &&
                        (fItr->fluentValue == -2 || fItr->fluentValue == -18))
                    {
                        isCTS = true;
                        break;
                    }
                ...
                if (!isCTS)
                {
                    ...
                    postmortem_noTimeSpecifierOnInstantNumericEffect(actionname, effectdescription, suggested,
                                                                     isAssign);
                }
            }
            else
            {
                if (debug)
                {
                    // cout << "Using set at " << (addEffToListNumeric.second) << " to check for self-mutex
                    // effects\n";
                }
                ...
                addEffToListNumeric.first->push_back(RPGBuilder::NumericEffect(
                    a->getOp(), pne->getStateID(), const_cast<VAL::expression *>(a->getExpr()), fe, tc));
                assert(addEffToListNumeric.first->back().fluentIndex <
                       instantiatedOp::howManyNonStaticPNEs());
    ...
    void validatePNE(PNE *c)
    {
        ...
        if (expectedArgs.find(givenArgs) == expectedArgs.end())
        {
            ...
            postmortem_wrongNumberOfFluentArguments(actionname, actLabel, WhereAreWeNow, predname, lit,
                                                    givenArgs, expectedArgs);
        }
    };
    ...
    void validateLiteral(Literal *l)
    {
        ...
        if (expectedArgs.find(givenArgs) == expectedArgs.end())
        {
            ...
            postmortem_wrongNumberOfPredicateArguments(actionname, actLabel, WhereAreWeNow, predname, lit,
                                                       givenArgs, expectedArgs);
        }
    }
    ...
    public:
        ...
        InitialStateCollector(const VAL::operator_ *o, FastEnvironment *f, VAL::TypeChecker *t = 0,
                              vector<bool> *setDefinedVectorValues = 0)
          : tc(t)
          , adding(true)
          , op(o)
          , fe(f)
          ...
          , assignTo(-1)
          , TIL(false)
          , sawAValueFor(setDefinedVectorValues)
        {
        ...
        virtual void visit_simple_effect(simple_effect *p)
        {
            ...
            else
            {
                if (adding)
                {
                    ...
                    {
                        cout << "Internal error: cannot locate initial state fact " << l
                             << " in proposition look-up table\n";
                        exit(1);
                    }
                }
            }
        ...
        virtual void visit_timed_initial_literal(timed_initial_literal *p)
        {
            ...
            TIL = true;
            p->effs->visit(this);
            const map<double, RPGBuilder::FakeTILAction>::iterator lookup =
                timedInitialLiterals.find(time_stamp);
            if (lookup == timedInitialLiterals.end())
            {
                timedInitialLiterals.insert(pair<double, RPGBuilder::FakeTILAction>(
                    time_stamp, RPGBuilder::FakeTILAction(time_stamp, tilAddSet, tilDeleteSet)));
            }
            else
            {
        ...
        virtual void visit_plus_expression(plus_expression *e)
        {
            e->getLHS()->visit(this);
            e->getRHS()->visit(this);
            const double r = workingValues.back();
            workingValues.pop_back();
            const double l = workingValues.back();
            workingValues.pop_back();
        ...
        void visit_special_val_expr(const special_val_expr *v)
        {
            postmortem_mathsError("#t, ?duration or total-time",
                                  "Special values, such as these, cannot be used to define initial fluent "
                                  "values.\n",
                                  WhereAreWeNow);
        };
            ...
            else
            {
                ...
                if (realPNE->getHead()->getName() == "fake-duration")
                {
                    postmortem_mathsError("?duration",
                                          "Special values, such as this, cannot be used to define initial "
                                          "fluent values.\n",
                                          WhereAreWeNow);
                }
                else
                {
                    pair<bool, double> isSet =
                        EFT(realPNE->getHead())->getInitial(realPNE->begin(), realPNE->end());
                    if (isSet.first)
                    {
                    ...
                    else
                    {
                        ...
                        postmortem_mathsError(toPass,
                                              "Values within the formulae to define initial fluent values "
                                              "must only refer to\nconstant values.\n",
                                              WhereAreWeNow);
                    }
                }
    ...
    RPGBuilder::op_type checkIfRogue(TimedPrecEffCollector &c)
    {
        ...
        for (int pass = 0; pass < 3; ++pass)
        {
            list<RPGBuilder::NumericPrecondition *> &durList =
                (pass ? (pass == 2 ? c.maxDurationExpression : c.minDurationExpression) :
...

        {
            ...
            for (; llItr != llEnd; ++llItr)
            {
                if (!(*llItr))
                {
                    if (rogueDebug)
                    {
                        cout << "Start Precondition " << *llItr << " doesn't exist\n";
                    }
                    return RPGBuilder::OT_INVALID_ACTION;
                }
            }
...

        {
            ...
            for (; llItr != llEnd; ++llItr)
            {
                if (!(*llItr))
                {
                    if (rogueDebug)
                    {
                        cout << "Invariant " << *llItr << " doesn't exist\n";
                    }
                    return RPGBuilder::OT_INVALID_ACTION;
                }
            }
...

        {
            ...
            for (; llItr != llEnd; ++llItr)
            {
                if (!(*llItr))
                {
                    if (rogueDebug)
                    {
                        cout << "End Precondition " << *llItr << " doesn't exist\n";
                    }
                    return RPGBuilder::OT_INVALID_ACTION;
                }
            }
...

        {
            ...
            for (; llItr != llEnd; ++llItr)
            {
                if (!(*llItr))
                {
                    if (rogueDebug)
                    {
                        cout << "Start Add Effect " << *llItr << " doesn't exist\n";
                    }
                    return RPGBuilder::OT_INVALID_ACTION;
                }
            }
...

        {
            ...
            for (; llItr != llEnd; ++llItr)
            {
                if (!(*llItr))
                {
                    if (rogueDebug)
                    {
                        cout << "Start Delete Effect " << *llItr << " doesn't exist\n";
                    }
                    return RPGBuilder::OT_INVALID_ACTION;
                }
            }
...

        {
            ...
            for (; llItr != llEnd; ++llItr)
            {
                if (!(*llItr))
                {
                    if (rogueDebug)
                    {
                        cout << "End Add Effect " << *llItr << " doesn't exist\n";
                    }
                    return RPGBuilder::OT_INVALID_ACTION;
                }
            }
...

        {
            ...
            for (; llItr != llEnd; ++llItr)
            {
                if (!(*llItr))
                {
                    if (rogueDebug)
                    {
                        cout << "End Del Effect " << *llItr << " doesn't exist\n";
                    }
                    return RPGBuilder::OT_INVALID_ACTION;
                }
            }
    ...
    void RPGBuilder::initialise()
    {
        ...
        if (RPGdebug && Globals::globalVerbosity & 65536)
        {
        ...
        if (RPGdebug && Globals::globalVerbosity & 65536)
        {
            instantiatedOp::writeAllPNEs(cout);
        }
        ...
        achievedInLayer =
            vector<EpsilonResolutionTimestamp>(literalCount, EpsilonResolutionTimestamp::undefined());
        achievedInLayerReset =
            vector<EpsilonResolutionTimestamp>(literalCount, EpsilonResolutionTimestamp::undefined());
        achievedBy = vector<pair<int, Planner::time_spec>>(literalCount);
        achievedByReset = vector<pair<int, Planner::time_spec>>(
            literalCount, pair<int, Planner::time_spec>(-1, Planner::E_AT_START));

        negativeAchievedInLayer =
            vector<EpsilonResolutionTimestamp>(literalCount, EpsilonResolutionTimestamp::undefined());
        negativeAchievedInLayerReset =
            vector<EpsilonResolutionTimestamp>(literalCount, EpsilonResolutionTimestamp::zero());
        negativeAchievedBy = vector<pair<int, Planner::time_spec>>(literalCount);
        negativeAchievedByReset = vector<pair<int, Planner::time_spec>>(
            literalCount, pair<int, Planner::time_spec>(-1, Planner::E_AT_START));
        ...
        TimedPrecEffCollector::doInit();  // for robustness checking - set which predicate names are legal,
                                          // and how many parameters they have

        {
            ...
            for (int i = 0; tilItr != tilEnd; ++tilItr, ++i)
            {
                ...
                FakeTILAction *const currFake = normalTimedInitialLiteralsVector[i] =
                    &(timedInitialLiterals.back());
...

        {
            ...
            for (; lsItr != lsEnd; ++lsItr)
            {
                ...
                {
                    literals[i] = *lsItr;
                    // cout << "Literal " << i << " - " << *(*lsItr) << " with global ID " <<
                    // (*lsItr)->getGlobalID() << "\n";
                }
            }
        }
...

        {
            if (PNEdebug)
            {
                cout << "PNEs in RPG instantiation:\n";
            }
            ...
            for (; pneItr != pneEnd; ++pneItr)
            {
                ...
                if (PNEdebug)
                {
                    cout << *(*pneItr) << " with state ID " << sID << " and global ID "
                         << (*pneItr)->getGlobalID() << "\n";
                }
            }
            if (PNEdebug)
            {
                cout << "PNEs in operators:\n";
            }
        }
        ...
        for (; opsItr != opsEnd; ++opsItr)
        {
            ...
            if (RPGdebug)
            {
                cout << "Operator " << operatorID << " - " << *currOp << "\n";
            }
...
#ifdef ENABLE_DEBUGGING_HOOKS
                    Globals::eliminatedAction(operatorID, "Had one or more preconditions that were not "
                                                          "instantiated");
#endif
                }
                else
            ...
            else
            {
                ...
                if (RPGdebug)
                {
                    cout << "Operator " << operatorID << " start adds:";
                }
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    ...
                    effectsToActions[effID].push_back(
...

            {
                ...
                if (RPGdebug)
                {
                    cout << "Operator " << operatorID << " start deletes:";
                }
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    ...
                    currEffectsList.push_back(*effItr);
                    negativeEffectsToActions[effID].push_back(
                        pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
                }
                if (RPGdebug)
                {
...

            {
                ...
                if (RPGdebug)
                {
                    cout << "Operator " << operatorID << " end adds:";
                }
...

            {
                ...
                if (RPGdebug)
                {
                    cout << "Operator " << operatorID << " end deletes:";
                }
...

        {
            if (RPGdebug)
            {
                cout << "Operator requires at start:";
            }

            {
                ...
                for (; precItr != precEnd; ++precItr)
                {
                    ...
                    if (precID >= 0)
                    {
                        ...
                        preconditionsToActions[precID].push_back(
                            pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
                    }
                }
...

            {
                ...
                for (; precItr != precEnd; ++precItr)
                {
                    ...
                    if (precID >= 0)
                    {
                        ...
                        currPreconditionsList.push_back(*precItr);
                        negativePreconditionsToActions[precID].push_back(
                            pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
                    }
                }
            ...
            if (RPGdebug)
            {
                cout << "Operator requires as an invariant:";
            }
...

            {
                ...
                if (currPreconditionsList.empty() && actionsToEndPreconditions[operatorID].empty())
                {
                    if (RPGdebug)
                    {
                        cout << "Operator is preconditionless at the end\n";
                    }
            ...
            {
                actionsToStartNumericPreconditions[operatorID].insert(
                    actionsToStartNumericPreconditions[operatorID].end(), c.startPrecNumeric.begin(),
                    c.startPrecNumeric.end());
                actionsToNumericInvariants[operatorID].insert(actionsToNumericInvariants[operatorID].end(),
                                                              c.invNumeric.begin(), c.invNumeric.end());
                actionsToEndNumericPreconditions[operatorID].insert(
                    actionsToEndNumericPreconditions[operatorID].end(), c.endPrecNumeric.begin(),
                    c.endPrecNumeric.end());
            }
        }
        ...
        */
        ...
        {
            list<Literal *> &newStartPrecs =
                (pass ? actionsToProcessedStartNegativePreconditions[operatorID] =
                            actionsToStartNegativePreconditions[operatorID] :
                        actionsToProcessedStartPreconditions[operatorID] =
                            actionsToStartPreconditions[operatorID]);
                    ...
                    if ((*precItr)->getStateID() >= 0)
                    {
                        if (oldStartEffects.find(*precItr) == notFound)
                        {
                            if (RPGdebug)
                            {
                                if (pass)
                                {
                                    cout << "Negative invariant " << *(*precItr)
                                         << " is not met by start effects\n";
                                }
                                else
                                {
...

            {
                ...
                for (; precItr != precEnd; ++precItr)
                {
                    ...
                    if (pass)
                    {
                        processedNegativePreconditionsToActions[precID].push_back(
                            pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
                    }
                    else
                    {
                        processedPreconditionsToActions[precID].push_back(
                            pair<int, Planner::time_spec>(operatorID, Planner::E_AT_START));
                    }
                }
            }
            ...
            {
                list<Literal *> &currPreconditionsList =
                    (pass ? actionsToEndNegativePreconditions[operatorID] :
                            actionsToEndPreconditions[operatorID]);
...

{
    ...
    GoalNumericCollector c(&numericGoals, &numericGoalDeadlines, &literalGoals, &literalGoalDeadlines,
                           prefNameToID, prefNameToNumberOfTimesDefinitelyViolated, 0, &env, theTC);
    current_analysis->the_problem->visit(&c);
    ...
    eliminateDuplicateConstraints(c.builtPreferences, prefNameToID, preferences,
                                  /*&preferencesThatAreSoftDeadlines, */ theTC);
    ...
    for (int pass = 0; pass < 2; ++pass)
    {
        const list<pair<int, list<Constraint>>> &builtPreconditionPreferences =
            (pass == 1 ? builtEndPreconditionPreferences : builtStartPreconditionPreferences);
        ...
        {
            vector<int> &dest = (pass == 1 ? actionsToEndPreferences[actpItr->first] :
                                             actionsToStartPreferences[actpItr->first]);
...

{
    ...
    {
        const bool destroy =
            (plaItr->second == Planner::E_AT_START &&
             initialUnsatisfiedProcessedStartNumericPreconditions[plaItr->first]) ||
...
TemporalAnalysis::findGoalSoftDeadlines(factRelevantToWithinPreferences,
                                        negativeFactRelevantToWithinPreferences);
...
NumericAnalysis::findVariablesThatAreTickers();
{
    ...
    const map<int, list<IntegralContinuousEffect>>::iterator iceEnd =
...
bool RPGBuilder::stepNeedsToHaveFinished(const ActionSegment &act, const MinimalState &theState,
                                         set<int> &toBeNonMutex)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (saItr == theState.startedActions.end())
            {
                if (debug)
                {
                    cout << "Action " << *(RPGBuilder::getInstantiatedOp(actID))
        ...
        for (; fItr != fEnd; ++fItr)
        {
            ...
            if (invItr != theState.invariants.end())
            {
                ...
                if (!invariantFinishesWithAction)
                {
                    ...
                    {
                        cout << "Action " << *(RPGBuilder::getInstantiatedOp(actID))
                             << " not applicable: cannot skip past invariants on " << *(*fItr)
                             << " owned by executing non-compression-safe actions\n";
                    }
                    return false;
                }
            }

            const StateBFacts::const_iterator csInvItr =
                theState.firstAnnotations.find((*fItr)->getStateID());
    ...
    if (needs)
    {
        ...
        for (; fItr != fEnd; ++fItr)
        {
            ...
            {
                const StateBFacts::const_iterator invBItr =
                    theState.firstAnnotations.find((*fItr)->getStateID());
            ...
            if (debug)
            {
                cout << "Would need an invariant " << *(*fItr) << ", which is not currently true\n";
            }
    ...
    if (willDelete)
    {
        ...
        for (; fItr != fEnd; ++fItr)
        {
            const map<int, PropositionAnnotation>::const_iterator invItr =
                theState.first.find((*fItr)->getStateID());
            if (invItr != stateEnd)
            {
                map<StepAndBeforeOrAfter, bool>::const_iterator dfItr = invItr->second.deletableFrom.begin();
                const map<StepAndBeforeOrAfter, bool>::const_iterator dfEnd =
                    invItr->second.deletableFrom.end();

                for (; dfItr != dfEnd; ++dfItr)
                {
                    if (act.second == Planner::E_AT_END && dfItr->first.stepID == invariantExemption)
                    {
                        continue;
                    }
                    if (dfItr->second == UNSAFETOSKIP)
                    {
                        if (debug)
                        {
                            cout << "Cannot skip past invariants on " << *(*fItr) << " owned by step "
                                 << dfItr->first.stepID << endl;
                        }
                        return false;
                    }
            ...
            if (invItr != retiredStateEnd)
            {
                ...
                for (; dfItr != dfEnd; ++dfItr)
                {
                    ...
                    if (dfItr->second == UNSAFETOSKIP)
                    {
                        if (debug)
                        {
                            cout << "Cannot skip past negative invariants on " << *(*fItr)
                                 << " owned by step " << dfItr->first.stepID << endl;
                        }
                        return false;
                    }
                    toBeNonMutex.insert(dfItr->first.stepID);
        ...
        {
            {
                ...
                if (invItr != stateEnd)
                {
                    if (debug)
                    {
                        cout << "Need ¬" << *(*fItr) << " but it is currently true\n";
                    }
                    return false;
                }
            }
...
pair<double, double> RPGBuilder::getOpDuration(instantiatedOp *op, const int &div,
                                               const vector<double> &minFluents,
                                               const vector<double> &maxFluents)
{
    return getOpDuration(op->getID(), div, minFluents, maxFluents);
};

pair<double, double> RPGBuilder::getOpDuration(const int &a, const int &div, const vector<double> &minFluents,
                                               const vector<double> &maxFluents)
{
    ...
    for (int pass = 0; pass < 3; ++pass)
    {
        list<DurationExpr *> *const currDE = (pass ? (pass == 2 ? &(rpgDurationExpressions[a][div]->max) :
                                                                  &(rpgDurationExpressions[a][div]->min)) :
...
void RPGBuilder::getEffects(instantiatedOp *op, const bool &start, list<Literal *> &add, list<Literal *> &del,
                            list<NumericEffect> &numeric)
{
    ...
    {
        {
            ...
            add.clear();
            add.insert(add.end(), pList.begin(), pList.end());
        }
        {
            list<Literal *> &pList = actionsToStartNegativeEffects[actID];
            del.clear();
            del.insert(del.end(), pList.begin(), pList.end());
        }
        {
            list<NumericEffect> &nList = actionsToStartNumericEffects[actID];
            numeric.clear();
            numeric.insert(numeric.end(), nList.begin(), nList.end());
        }
    }
    else
    {
...

    {
        ...
        for (; sEffItr != sEffEnd; ++sEffItr) {
            if ((*sEffItr)->ts == toMatch) {
                ...
                for (int pass = 0; pass < 2; ++pass) {
                    VAL::pc_list<VAL::simple_effect *> & listToUse = (pass ? (*sEffItr)->effs->del_effects :
    (*sEffItr)->effs->add_effects); list<Literal*> & listToAddTo = (pass ? del : add);
    ...
    if (start) { // for instantaneous actions, just have a start action which has everything
        ...
        {
            numeric.insert(numeric.end(), actionsToNumericEffects[actID].begin(),
    actionsToNumericEffects[actID].end());
        }
    }*/
};

void RPGBuilder::getPrecInv(instantiatedOp *op, const bool &start, list<Literal *> &precs,
                            list<Literal *> &inv, list<NumericPrecondition> &numericPrec,
                            list<NumericPrecondition> &numericInv)
{
    ...
    inv.clear();
    inv.insert(inv.end(), invList.begin(), invList.end());
    {
        list<NumericPrecondition> &ninv = actionsToNumericInvariants[opIndex];
        numericInv.clear();
        numericInv.insert(numericInv.end(), ninv.begin(), ninv.end());
    }
    precs.clear();
    numericPrec.clear();
    if (start)
...
/*
void RPGBuilder::getCollapsedAction(instantiatedOp* op, list<Literal*> & pre, list<Literal*> & add,
list<Literal*> & del, list<NumericPrecondition> & numericPre, list<NumericEffect> & numericEff) {
    ...
    {
        numericPre.insert(numericPre.end(), actionsToNumericPreconditions[actID].begin(),
actionsToNumericPreconditions[actID].end());
    }
    {
        numericEff.insert(numericEff.end(), actionsToNumericEffects[actID].begin(),
actionsToNumericEffects[actID].end());
    }
        ...
        for (; ceItr != ceEnd; ++ceItr)
        {
            ...
            for (int pass = 0; pass < 3; ++pass)
            {
                ...
                switch (pass)
                {
                    ...
                    default:
                    {
                        cerr << "Time specifier on condition effects have to be one of either 'at start', "
                                "'over all' or 'at end'\n";
                        exit(1);
                    }
                }
                ...
                {
                    // For now, we can only handle propositions governed by TILs, for metric optimisation
                    // purposes
                    ...
                    for (; aItr != aEnd; ++aItr)
                    {
                        if (aItr->second != Planner::E_AT)
                        {  // if the achiever is not a TIL
                            postmortem_noADL();
                        }
                    }
...
void RPGBuilder::buildDurations(vector<list<NumericPrecondition *>> &fixedDurations,
                                vector<list<NumericPrecondition *>> &minDurations,
                                vector<list<NumericPrecondition *>> &maxDurations)
{
    ...
    for (int i = 0; i < lim; ++i)
    {
        if (!rogueActions[i])
        {
            ...
            bool durative =
                (!allDurations[0]->empty() || !allDurations[1]->empty() || !allDurations[2]->empty());
            ...
            if (durative)
            {
                ...
                for (int pass = 0; pass < 3; ++pass)
                {
                    ...
                    for (; dlItr != dlEnd; ++dlItr)
                    {
                        ...
                        {
                            if (!pass)
                            {  // special case for equals constraints

                                if (newEval.first > evalDurMax.first || newEval.first < evalDurMin.first)
                                {
#ifdef ENABLE_DEBUGGING_HOOKS
                                    {
                                        ostringstream s;
...
// can never be applied
                            ...
                            if (pass != 1)
                            {  // if it's a fixed or max constraint
                                if (evalDurMax.second)
                                {
                                    if (newEval.first < evalDurMax.first)
                                    {
                                        evalDurMax.first = newEval.first;
                                    }
                                }
                                else
                                {
                            ...
                            if (pass != 2)
                            {  // if it's a fixed or min constraint
                                if (evalDurMin.second)
                                {
                                    if (newEval.first > evalDurMin.first)
                                    {
                                        evalDurMin.first = newEval.first;
                                    }
                                }
                                else
                                {
                ...
                {
                    // For now we only support 0-duration durative actions if their duration is fixed, i.e.
                    // min=max=0.0 As such, we impose a lower bound of 0.001 on the duration of any action
                    // non-fixed-duration action
                    if (evalDurMax.first != 0.0)
                    {
                        if (evalDurMax.first < 0.001)
                        {
                            durationConflict = true;
#ifdef ENABLE_DEBUGGING_HOOKS
                            Globals::eliminatedAction(i, "Unless an action's duration is fixed to 0.0, it "
                                                         "must be at least 0.001 long");
#endif
                        }
                        else
                        {
                ...
                if (evalDurMax.first == 0.0)
                {
                    ...
                    {
#ifdef ENABLE_DEBUGGING_HOOKS
                        Globals::eliminatedAction(i, "Unless an action's duration is fixed to 0.0, it must "
                                                     "be at least 0.001 long");
#endif
                        durationConflict = true;
                    }
                }
                ...
                if (!durationConflict && (evalDurMin.first == 0.0 || evalDurMax.first == 0.0))
                {
                    ...
                    if (durationConflict)
                    {
                        ...
                        if (!issuedWarning)
                        {
                            ...
                            cout << ",\nbut also has:\n\n";
                            cout << diagnosis;
                            cout << "\nIn this case, correct handling of the action is unclear (e.g. at what "
                                    "point\n";
                            cout << "must the over all/at end conditions hold if there is no gap between the "
                                    "start\n";
                            cout << "and end of the action).  If the intention is for the action to be "
                                    "truly\n";
                            cout << "instantaneous, use a PDDL (:action rather than a (:durative-action.  "
                                    "For now,\n";
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (durDebug)
                        {
                            cout << "Duration of " << *getInstantiatedOp(i)
                                 << " is 0, making action non-temporal\n";
                        }
                        ...
                        {
                            list<Literal *> &currList =
                                (pol ? actionsToEndNegativeEffects[i] : actionsToEndEffects[i]);
                            list<Literal *> &destList =
                                (pol ? actionsToStartNegativeEffects[i] : actionsToStartEffects[i]);
                            ...
                            for (; eItr != eEnd; ++eItr)
                            {
                                ...
                                if (oItr == oEnd)
                                {
                                    ...
                                    if (pol)
                                    {
                                        negativeEffectsToActions[(*eItr)->getStateID()].push_back(
                                            make_pair(i, Planner::E_AT_START));
                                    }
                                    else
                                    {
                                        effectsToActions[(*eItr)->getStateID()].push_back(
                                            make_pair(i, Planner::E_AT_START));
                                    }
                                }
                                if (pol)
                                {
                                    negativeEffectsToActions[(*eItr)->getStateID()].remove(
                                        make_pair(i, Planner::E_AT_END));
                                }
                                else
                                {
                                    effectsToActions[(*eItr)->getStateID()].remove(
                                        make_pair(i, Planner::E_AT_END));
                                }
                            }
                            currList.clear();
...

                        {
                            ...
                            for (; eItr != eEnd; ++eItr) {
                                ...
                                if (oItr == oEnd) {
                                    destList.push_back(*eItr);
                                    rpgNumericEffectsToActions[*eItr].push_back(make_pair(i,
                            Planner::E_AT_START));
                                }
                        ...
                        {
                            list<ProtoConditionalEffect *>::iterator condEffItr =
                                actionsToRawConditionalEffects[i].begin();
                            const list<ProtoConditionalEffect *>::iterator condEffEnd =
                                actionsToRawConditionalEffects[i].end();
...
{
    bool operator()(const pair<Literal *, Planner::time_spec> &a,
                    const pair<Literal *, Planner::time_spec> &b) const
    {
...
{
    {
        ...
        propositionalConditions.insert(propositionalConditions.end(), newPropositionalConditions.begin(),
                                       newPropositionalConditions.end());
    }

    {
        ...
        numericPreconditions.insert(numericPreconditions.end(), newNumericPreconditions.begin(),
    ...
    for (int pass = 0; pass < 2; ++pass)
    {
        ...
        {
            list<pair<Literal *, Planner::time_spec>>::const_iterator pcItr =
                propositionalDeleteEffects.begin();
            const list<pair<Literal *, Planner::time_spec>>::const_iterator pcEnd =
                propositionalDeleteEffects.end();
    ...
    propositionalAddEffects.insert(propositionalAddEffects.end(), strongestAddEffects.begin(),
                                   strongestAddEffects.end());
    propositionalDeleteEffects.insert(propositionalDeleteEffects.end(), strongestDelEffects.begin(),
                                      strongestDelEffects.end());
}

RPGBuilder::LinearEffects *RPGBuilder::buildLE(list<RPGBuilder::NumericEffect> &effList,
                                               const string &whereEffectsAreFrom)
{
    ...
    while (elItr != elEnd)
    {
        ...
        if (isCTS)
        {
            if (!toReturn)
            {
                if (localDebug)
                {
                    cout << "Found CTS effects on " << whereEffectsAreFrom << "\n";
                }
            ...
            {
                if (weightedSum.second.size() == 1 && weightedSum.second.front() == -1 &&
                    weightedSum.first.front() == 0.0)
                {
                    if (localDebug)
                    {
                        cout << "\tIncreases " << *(pnes[elItr->fluentIndex]) << " at rate 0 - ignoring\n";
                    }
                }
                else
                {
                    if (weightedSum.second.size() != 1 ||
                        !(weightedSum.second.front() == -2 || weightedSum.second.front() == -18))
                    {
                    ...
                    if (localDebug)
                    {
                        cout << "\tIncreases " << *(pnes[elItr->fluentIndex]) << " at rate " << currGradient
                             << "\n";
                    }
                }
            }
...
pair<double, double> RPGBuilder::RPGNumericEffect::applyEffectMinMax(const vector<double> &minFluents,
                                                                     const vector<double> &maxFluents,
                                                                     const double &minDur,
                                                                     const double &maxDur)
{
