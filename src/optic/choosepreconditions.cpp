...
namespace Planner
{
    ...
    public:
        ...
        protected:
            ...
            virtual void visit_OR_internal(NNFContainerNode *orNode)
            {
                ...
                parent->chosenPropositions.back().insert(bestChosenPropositions.begin(),
                                                         bestChosenPropositions.end());
                parent->chosenNegativePropositions.back().insert(bestChosenNegativePropositions.begin(),
                                                                 bestChosenNegativePropositions.end());
            ...
            virtual void visit_RPGNumeric_internal(const int &preID)
            {
                const pair<map<int, pair<bool, double>>::iterator, bool> insPair =
                    parent->numericPreTrueAtTime.insert(make_pair(preID, make_pair(true, 0.0)));
                if (insPair.second)
                {
                    RPGBuilder::RPGNumericPrecondition &numPre = RPGBuilder::getNumericPreTable()[preID];
                    const bool isSatisfied = insPair.first->second.first =
                        numPre.isSatisfiedWCalculate(parent->theState.secondMin, parent->theState.secondMax);
                    if (chooseDebug)
                    {
                        if (isSatisfied)
                        {
                    ...
                    if (isSatisfied)
                    {
                        insPair.first->second.second = earliestPointForNumericPrecondition(
                                                           numPre, parent->theState, parent->minTimestamps)
                                                           .toDouble();
                    }
                }
            ...
            CommonSatisfactionVisitor(ChoosePreconditions *p) : parent(p)
            {
            }
        ...
        {
        public:
            SatisfyAPrecondition(ChoosePreconditions *p) : CommonSatisfactionVisitor(p)
            {
            }
        ...
        {
        public:
            UnsatisfyAPrecondition(ChoosePreconditions *p) : CommonSatisfactionVisitor(p)
            {
            }
    ...
    public:
...
#ifdef ENABLEDERIVED
                            ,
                            const vector<double> &datIn, const vector<double> &ndatIn, const vector<int> &dil
#endif
                            )
          : theState(s)
          ,
        ...
        pair<bool, double> collect(const list<Literal *> &pres, const list<Literal *> &negPres,
                                   set<int> &propositions, set<int> &negativePropositions, set<int> &numerics)
        {
                                   ...
                                   NNFPreconditionChooser::Supporters &chosen)
        {
            ...
            chosen.negativePropositions.insert(chosenNegativePropositions.back().begin(),
                                               chosenNegativePropositions.back().end());
                ...
                while (loItr != loEnd)
                {
                    ...
                    if (derivationPolarity == 0)
                    {
                        ...
                        for (; arItr != arEnd; ++arItr)
                        {
                            ...
                            const pair<bool, double> couldBeDerived = collect(
                                rule, true, candidateChosenPropositions, candidateChosenNegativePropositions,
                        ...
                        extraPropositions.insert(bestChosenPropositions.begin(),
                                                 bestChosenPropositions.end());
                        extraNegativePropositions.insert(bestChosenNegativePropositions.begin(),
                                                         bestChosenNegativePropositions.end());
                    ...
                    else
                    {
                        ...
                        for (; arItr != arEnd; ++arItr)
                        {
                            ...
                            const pair<bool, double> couldBeDerived =
                                collect(rule, false, bestChosenPropositions, bestChosenNegativePropositions,
            ...
            chosen.negativePropositions.insert(extraNegativePropositions.begin(),
                                               extraNegativePropositions.end());
    ...
    public:
        ...
        virtual void visit_NOT_Literal(NNFNode_NOT_Literal *litNode)
        {
            ...
            if (theState.first.find(litID) != theState.first.end())
            {
                if (collectDebug)
                {
                    cout << "Visited negative literal " << *(RPGBuilder::getLiteral(litID))
                         << " - not true as we have it\n";
                }
        ...
        virtual void visit_Literal(NNFNode_Literal *litNode)
        {
            ...
            if (theState.first.find(litID) == theState.first.end())
            {
                ...
                if (collectDebug)
                {
                    cout << "Visited literal " << *(RPGBuilder::getLiteral(litID))
                         << " - not true as we don't have it\n";
                }
            }
            else
            {
    ...
    {
        pair<bool, double> satisfyNNFEarly(MinimalState &s, const vector<double> &minTimestamps,
                                           NNFNode *toVisit, Supporters &chosen)
        {
            if (toVisit)
            {
        ...
        bool collectNNFDependencies(MinimalState &s, NNFNode *toVisit, Supporters &chosen)
        {
            if (toVisit)
            {
                if (collectDebug)
