...
void MILPSolverCLP::addCol(const vector<pair<int, double>> &entries, const double &lb, const double &ub,
...
void MILPSolverCLP::addRow(const vector<pair<int, double>> &entries, const double &lb, const double &ub)
{
    ...
    for (int e = 0; e < entries.size(); ++e)
    {
        ...
        {
            std::cerr << "Fatal internal error: adding a row that refers to column " << scratchI[e]
                      << ", which does not exist - is not in the range [0," << getNumCols() - 1 << "]\n";
            exit(1);
        }
    }
#endif
...
void MILPSolverCLP::setMatrixEntry(const int &row, const int &column, const double &coefficient)
{
    lp->modifyCoefficient(row, column, coefficient);
}
