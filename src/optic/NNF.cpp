...
namespace Planner
{
    ...
    {
        std::cerr << "Fatal internal error: once the final numeric preconditions have been substituted into "
                     "NNF, it should not be negated\n";
        exit(1);
        return 0;
    };
    ...
    public:
        ...
        virtual void visit_simple_goal(simple_goal *p)
        {
            ...
            if (nnfCollectDebug)
            {
                cout << string(nnfCollectDebugIndent, ' ') << "Literal: " << *l << " (" << p->getProp()->head
                     << "/" << current_analysis->pred_tab.symbol_get(p->getProp()->head->getName());
        ...
        {
            VAL::time_spec tsToUse = (currTS.empty() ? VAL::E_AT : currTS.back());
            justCreated.push_back(pair<NNFNode *, bool>(
                new NNFNode_Numeric(new RPGBuilder::NumericPrecondition(
        ...
        virtual void visit_qfied_goal(qfied_goal *p)
        {
            ...
            if (contradiction || (p->getQuantifier() == E_EXISTS && branchNode->empty()))
            {
                if (nnfCollectDebug)
                {
                    if (!contradiction)
                    {
                        cout << "Exists quantifier's children are all false - must be false\n";
                    }
                }
            ...
            if (tautology || (p->getQuantifier() == E_FORALL && branchNode->empty()))
            {
                if (nnfCollectDebug)
                {
                    if (branchNode->empty())
                    {
                        cout << "Forall quantifier has no potentially false children - must be true\n";
                    }
                }
                delete branchNode;
        ...
        virtual void visit_conj_goal(conj_goal *p)
        {
            ...
            if (contradiction)
            {
                if (nnfCollectDebug)
                {
                    cout << string(nnfCollectDebugIndent, ' ')
                         << "and has a child that must be false so must, itself, be false\n";
                }
                delete andNode;
                justCreated.push_back(pair<NNFNode *, bool>(0, false));
                return;
            ...
            if (andNode->empty())
            {
                ...
                {
                    cout << string(nnfCollectDebugIndent, ' ')
                         << "and has no possibly untrue children - must be true\n";
                }
                delete andNode;
                justCreated.push_back(pair<NNFNode *, bool>(0, true));
                return;
        ...
        virtual void visit_imply_goal(imply_goal *p)
        {
            ...
            if (!lhs)
            {
                if (lhsdeterminedToBe)
                {
                    ...
                    {
                        cout << string(nnfCollectDebugIndent, ' ')
                             << "Antecedent of implication is true: simplifying to just the consequent\n";
                    }
                ...
                else
                {
                    ...
                    {
                        cout << string(nnfCollectDebugIndent, ' ')
                             << "Antecedent of implication is false: ex falso quodlibet, so can skip "
                                "consequent and assume truth\n";
                    }
            ...
            if (!rhs)
            {
                if (rhsdeterminedToBe)
                {
                    ...
                    {
                        cout << string(nnfCollectDebugIndent, ' ')
                             << "Consequent of implication is true, so implication is true\n";
                    }
                    delete lhs;
                ...
                else
                {
                    ...
                    {
                        cout << string(nnfCollectDebugIndent, ' ')
                             << "Consequent of implication is false, so replacing with negation of the "
                                "antecedent\n";
                    }
                    NNFNode *negated = lhs->negate();
                    delete lhs;
                    justCreated.push_back(pair<NNFNode *, bool>(negated, true));  // anything => true  =
                                                                                  // !anything
                    return;
                }
            }
        ...
        virtual void visit_neg_goal(neg_goal *p)
        {
            ...
            else
            {
                ...
                {
                    cout << string(nnfCollectDebugIndent, ' ')
                         << "Flipping truth value of child with known value - is now ";
                    if (posBack.second)
                    {
                        cout << "false\n";
                    }
    ...
    public:
        ...
        NNFStaticReducer(vector<pair<bool, bool>> &lit, vector<pair<bool, bool>> &num)
          : staticLiterals(&lit), staticNumerics(&num){};
        ...
        virtual void visit_Literal(NNFNode_Literal *litNode)
        {
            ...
            else
            {
                ...
                if ((*staticLiterals)[currLitID].first)
                {
                    //            cout << "In NNF, " << *(litNode->getLiteral()) << " is static and "
                    //            <<staticLiterals[currLitID].second << "\n";
                    justCreated.push_back(pair<bool, bool>(true, (*staticLiterals)[currLitID].second));
                }
                else
                {
    ...
    public:
        NNFCollectRequiredFacts(set<int> &mbt, set<int> &mbf) : mustBeTrue(mbt), mustBeFalse(mbf)
        {
        }
        ...
        virtual void visit_OR(NNFNode_OR *orNode)
        {
            ...
            for (; childItr != childEnd; ++childItr)
            {
                ...
                std::set_intersection(localTrue.begin(), localTrue.end(), isectTrue.begin(), isectTrue.end(),
                                      std::insert_iterator<set<int>>(resTrue, resTrue.begin()));
                std::set_intersection(localFalse.begin(), localFalse.end(), isectFalse.begin(),
                                      isectFalse.end(),
                                      std::insert_iterator<set<int>>(resFalse, resFalse.begin()));
    ...
    public:
        ...
        virtual void processNumeric(RPGBuilder::NumericPrecondition &justOne, const VAL::time_spec &ts)
        {
            ...
            const bool evaluateOkay = RPGBuilder::processPrecondition(commonData, justOne, conjunctiveDest,
                                                                      disjunctiveDest, -3, Planner::E_AT);
    ...
    public:
        NNFMarkRPGNumericDesirable(RPGBuilder::BuildingNumericPreconditionData &c, const bool &d,
                                   const bool &u)
          : commonData(c), desirable(d), undesirable(u){};
    ...
    pair<NNFNode *, bool> NNFUtils::pruneStaticLiterals(NNFNode *const root,
                                                        vector<pair<bool, bool>> &staticLiterals)
    {
    ...
    pair<NNFNode *, bool> NNFUtils::pruneStaticNumerics(NNFNode *const root,
                                                        vector<pair<bool, bool>> &staticNumerics)
    {
    ...
    pair<NNFNode *, bool>
    NNFUtils::substituteRPGNumerics(NNFNode *const root, const bool &desirable, const bool &undesirable,
                                    RPGBuilder::BuildingNumericPreconditionData &commonData)
    {
    ...
    NNF_Flat::NNF_Flat(const list<pair<int, int>> &usr, const list<pair<Cell, int>> &cellAndParent,
                       const list<bool> &r)
      : unsatSize(usr.size())
      , unsatReset(new int[unsatSize])
      , unsat(new int[unsatSize])
      , fragilityTrue(new int[unsatSize])
      , fragilityFalse(new int[unsatSize])
      , cellIsAnd(new bool[unsatSize])
      , cellCount(cellAndParent.size() + unsatSize)
      , parentID(new int[cellCount])
      , cells(new Cell[cellCount])
      , resetSize(unsatSize * sizeof(int))
    {
    ...
    void NNF_Flat::write(ostream &o) const
    {
        ...
        while (!printStack.empty())
        {
            ...
            if (cells[cellIndex].isCell())
            {
                ...
                if (!cells[cellIndex].polarity)
                {
                    o << "¬";
                }
                if (cells[cellIndex].lit)
                {
                    o << *(cells[cellIndex].lit);
                }
                const int numID = cells[cellIndex].num;
                if (numID != -1)
                {
                    o << RPGBuilder::getNumericPreTable()[numID];
                }
                o << "\n";
            }
            else
    ...
    public:
        ...
        virtual void visit_NOT_Numeric(NNFNode_NOT_Numeric *r)
        {
            cout << "Error - NOT Numeric Nodes should have been subsituted with NOT RPG Numeric Nodes by "
                    "now\n";
            assert(false);
        }
