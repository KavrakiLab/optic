...
bool branchOnBinaryVariable(MILPSolver *const lp,
                            const vector<MILPSolver::Objective::const_iterator> &branchOver,
                            ...
                            vector<bool> &solution, const bool &maximise, double &bestSolutionCost)
{
    ...
    for (int pass = 0; pass < 2; ++pass)
    {
        ...
        if (currValue == 1)
        {
            ...
            map<int, double>::const_iterator quadraticTerms =
                branchOver[var]->second.nonLinearCoefficients.begin();
            const map<int, double>::const_iterator quadraticEnd =
                branchOver[var]->second.nonLinearCoefficients.end();
            ...
            if (lp->solve(true))
            {  // if the LP can be solved
                const bool wasBetter = branchOnBinaryVariable(
                    lp, branchOver, branchCount, var + 1, childCoeffs, childSolution, maximise, solnValue);
                if (wasBetter)
                {
            ...
            {
                assert(newCoeffsItr != newCoeffsEnd);  // we should never have lost coefficients - at worse,
                                                       // they will be zeroed
        ...
        else
        {
            ...
            if (lp->solve(true))
            {  // if the LP can be solved
                const bool wasBetter = branchOnBinaryVariable(lp, branchOver, branchCount, var + 1, coeffs,
                                                              childSolution, maximise, solnValue);
                if (wasBetter)
                {
                    solution = childSolution;
                }
...
void MILPSolver::mustSolve(const bool &b)
{
    static bool success;
    success = solve(b);
    if (!success)
    {
        std::cerr << "Fatal internal error: solve call should have succeeded\n";
        writeLp("failed.lp");
        exit(1);
    }
}
...
bool MILPSolver::quadraticPreSolve()
{
    ...
    for (; tItr != tEnd; ++tItr)
    {
        ...
        if (!tItr->second.nonLinearCoefficients.empty())
        {
            ...
            {
                std::cerr << "Warning: unsupported quadratic terms in MIQCP objective - must be of the form "
                             "<binary var> * <var> * <weight>,\n";
                std::cerr << "but column \"" << getColName(tItr->first) << "\" (" << tItr->first
                          << ") is not binary\n";
                std::cerr << "This suggests a bug in the code building the objective for scheduling the plan "
                             "according to the task metric\n";
                return false;
            }
    ...
    if (branchCount)
    {
        ...
        if (!branchOnBinaryVariable(this, branchOver, branchCount, 0, coeffs, solution,
                                    quadraticObjective.maximise(), bestSolutionQuality))
        {
        ...
        for (int bv = 0; bv < branchCount; ++bv)
        {
            if (solution[bv])
            {
                ...
                map<int, double>::const_iterator quadraticTerms =
                    branchOver[bv]->second.nonLinearCoefficients.begin();
                const map<int, double>::const_iterator quadraticEnd =
                    branchOver[bv]->second.nonLinearCoefficients.end();
