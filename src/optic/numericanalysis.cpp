...
namespace Planner
{
    ...
    {
        vector<list<RPGBuilder::ConditionalEffect>> &actionsToConditionalEffects =
            RPGBuilder::getEditableActionsToConditionalEffects();
        const int opCount = actionsToConditionalEffects.size();

        for (int op = 0; op < opCount; ++op)
        {
            if (RPGBuilder::rogueActions[op])
            {
                continue;
            }

            list<RPGBuilder::ConditionalEffect>::iterator eff = actionsToConditionalEffects[op].begin();
            const list<RPGBuilder::ConditionalEffect>::iterator effEnd =
                actionsToConditionalEffects[op].end();

            while (eff != effEnd)
            {
                list<pair<int, Planner::time_spec>>::iterator numEff =
                    eff->getEditableNumericEffects().begin();
                const list<pair<int, Planner::time_spec>>::iterator numEffEnd =
                    eff->getEditableNumericEffects().end();
                ...
                while (numEff != numEffEnd)
                {
                    ...
                    else if (NumericAnalysis::getDominanceConstraints()[currEff.fluentIndex] ==
    ...
    NumericAnalysis::dominance_constraint NumericAnalysis::preconditionDominanceInOneDirection(
        const int &varID, const map<int, list<int>> &numericPreconditionsThatAreInCondEffs)
    {
        static const bool localDebug = (Globals::globalVerbosity & 16384);

        const vector<RPGBuilder::RPGNumericPrecondition> &rpgNumericPreconditions =
            RPGBuilder::getNumericPreTable();
        const vector<RPGBuilder::ArtificialVariable> &rpgArtificialVariables =
            RPGBuilder::getArtificialVariableTable();
        ...
        for (int rnp = 0; rnp < rnpCount; ++rnp)
        {
            ...
            if (currRNP.LHSVariable == varID)
            {
                ...
                if (possibleAnswers[1])
                {
                    if (requiringNotFullOnlySupportsReplenishment(rnp) &&
                        preconditionOnlySupportsWorseMetricThanOtherOptions(
                            rnp, numericPreconditionsThatAreInCondEffs))
                    {
                        // do nothing in this case: v >= c just means we can either make v smaller again (with
                        // no gap) or that we can't have a metrically superior v < c option
                        if (localDebug)
                        {
                    ...
                    {
                        // we have a precondition v > or >= c, which doesn't just support resource
                        // replenishment, so smaller isn't preferable
                        if (localDebug)
                        {
                        ...
                        if (!possibleAnswers[0])
                        {
                            if (localDebug)
                            {
                                cout << COLOUR_light_cyan << "Cannot show any dominance on "
                                     << *(RPGBuilder::getPNE(varID)) << COLOUR_default << endl;
                            }
                ...
                if (possibleAnswers[0])
                {
                    if (requiringNotFullOnlySupportsReplenishment(rnp) &&
                        preconditionOnlySupportsWorseMetricThanOtherOptions(
                            rnp, numericPreconditionsThatAreInCondEffs))
                    {
                        // do nothing in this case: v <= c just means we can either make v bigger again (with
                        // no gap) or that we can't have a metrically superior v > c option
                        if (localDebug)
                        {
                            cout << "Precondition doesn't let us do better - ignoring it\n";
                    ...
                    {
                        // we have a precondition v < or <= c,  which doesn't just support resource
                        // replenishment, so bigger isn't preferable
            ...
            else if (currRNP.LHSVariable >= 2 * pneCount)
            {
                const RPGBuilder::ArtificialVariable &currAV =
                    RPGBuilder::getArtificialVariable(currRNP.LHSVariable);
                if (currAV.size == 1)
                {
                    ...
                    else if (currAV.fluents[0] == negativeI)
                    {
                        if (localDebug)
                        {
                            cout << "Found a precondition on " << *(RPGBuilder::getPNE(varID)) << ": "
                                 << currRNP << endl;
                        }
        ...
        {
            // cout << "Fatal internal error: is good to have bigger and smaller values of " <<
            // *(RPGBuilder::getPNE(varID)) << endl; assert(!(possibleAnswers[0] && possibleAnswers[1]));
            if (localDebug)
            {
                cout << COLOUR_light_cyan << "Shown irrelevance: " << *(RPGBuilder::getPNE(varID))
                     << COLOUR_default << endl;
            }
            return E_IRRELEVANT;
        }
        ...
        if (possibleAnswers[0])
        {
            if (localDebug)
            {
                cout << COLOUR_light_cyan << "Shown bigger is better for " << *(RPGBuilder::getPNE(varID))
                     << COLOUR_default << endl;
            }
        ...
        if (possibleAnswers[1])
        {
            if (localDebug)
            {
                cout << COLOUR_light_cyan << "Shown smaller is better for " << *(RPGBuilder::getPNE(varID))
                     << COLOUR_default << endl;
            }
    ...
    bool VarOpConst::implies(const VarOpConst &other) const
    {
        ...
        switch (op)
        {
            ...
            default:
            {
                cerr << "Fatal internal error: the = precondition should have been pre-processed into a "
                        ">=,<= pair\n";
                exit(1);
            }
        }
    ...
    void
    NumericAnalysis::MaskedVariableBounds::applyPreToBounds(const RPGBuilder::RPGNumericPrecondition &currPre)
    {
        bool success = false;
    ...
    struct ConditionBoundsAndOutcome
    {
        ...
        ConditionBoundsAndOutcome(const RPGBuilder::ConditionalEffect *o, const VarOpConst &lbIn,
                                  const VarOpConst &ubIn)
    ...
    bool NumericAnalysis::couldIncreaseVariable(const RPGBuilder::RPGNumericEffect &eff,
    ...
    bool NumericAnalysis::couldDecreaseVariable(const RPGBuilder::RPGNumericEffect &eff,
                                                const MaskedVariableBounds &variableBounds)
    {
        ...
        for (int s = 0; s < eff.size; ++s)
        {
            ...
            else
            {
                ...
                if (localDebug)
                {
                    ...
                    cout << "*" << eff.weights[s] << " = " << variableBounds[lVar].second << "*"
                         << eff.weights[s] << " = " << variableBounds[lVar].second * eff.weights[s] << endl;
                }
            }
        }
    ...
    bool NumericAnalysis::couldBeWorseThan(const RPGBuilder::ConditionalEffect *a,
                                           const RPGBuilder::ConditionalEffect *b,
                                           const MaskedVariableBounds &variableBounds)
    {
        typedef map<int, pair<const RPGBuilder::RPGNumericEffect *, const RPGBuilder::RPGNumericEffect *>>
            EffectMap;
        ...
        for (int pass = 0; pass < 2; ++pass)
        {
            ...
            MaskedVariableBounds &currBounds = (pass ? bBounds : aBounds);
            {
                ...
                {
                    EffectMap &writeTo =
                        (neItr->second == Planner::E_AT_START ? startEffectsByVar : endEffectsByVar);
                    ...
                    if (pass)
                    {
                        writeTo
                            .insert(make_pair(
                                currEff.fluentIndex,
                                pair<RPGBuilder::RPGNumericEffect *, RPGBuilder::RPGNumericEffect *>(0, 0)))
                            .first->second.second = &currEff;
                    }
                    else
                    {
                        writeTo
        ...
        for (int pass = 0; pass < 2; ++pass)
        {
            ...
            for (; mItr != mEnd; ++mItr)
            {
                ...
                if (localDebug)
                {
                    if (mtStatus->second)
                    {
                        cout << "For the metric " << *(RPGBuilder::getPNE(mItr->first))
                             << " should be bigger\n";
                    }
                    else
                    {
                ...
                if (mItr->second.first && !mItr->second.second)
                {
                    ...
                    if (mtStatus->second)
                    {
                        if (couldDecreaseVariable(*mItr->second.first, aBounds))
                        {
                            ...
                            if (localDebug)
                            {
                                cout << "+ a could be worse than b, as a could decrease "
                                     << *(RPGBuilder::getPNE(mItr->first)) << " and b has no effect on it\n";
                            }
                            return true;
                        }
                    }
                    else
                    {
                        if (couldIncreaseVariable(*mItr->second.first, aBounds))
                        {
                            ...
                            if (localDebug)
                            {
                                cout << "+ a could be worse than b, as a could increase "
                                     << *(RPGBuilder::getPNE(mItr->first)) << " and b has no effect on it\n";
                            }
                ...
                else if (!mItr->second.first && mItr->second.second)
                {
                    ...
                    if (mtStatus->second)
                    {
                        if (couldIncreaseVariable(*mItr->second.second, bBounds))
                        {
                            ...
                            if (localDebug)
                            {
                                cout << "+ a could be worse than b, as b could increase "
                                     << *(RPGBuilder::getPNE(mItr->first)) << " and a has no effect on it\n";
                            }
                            return true;
                        }
                    }
                    else
                    {
                        if (couldDecreaseVariable(*mItr->second.second, bBounds))
                        {
                            ...
                            if (localDebug)
                            {
                                cout << "+ a could be worse than b, as b could decrease "
                                     << *(RPGBuilder::getPNE(mItr->first)) << " and a has no effect on it\n";
                            }
                            return true;
                        }
                ...
                else
                {
...

                    {
                        ...
                        for (int s = 0; s < eff.size; ++s)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    varToWeightAB.insert(
                                        make_pair(lVar - pneCount, make_pair(-eff.weights[s], 0.0)));
                                }
                            }
                        }
                    }

                    {
                        ...
                        for (int s = 0; s < eff.size; ++s)
                        {
                            ...
                            if (lVar == -3)
                            {
                                varToWeightAB.insert(make_pair(-3, make_pair(0.0, 0.0)))
                                    .first->second.second = eff.weights[s];
                            }
                            else if (lVar == -19)
                            {
                                varToWeightAB.insert(make_pair(-3, make_pair(0.0, 0.0)))
                            ...
                            else
                            {
                                ...
                                if (lVar < pneCount)
                                {
                                    varToWeightAB.insert(make_pair(lVar, make_pair(0.0, 0.0)))
                                        .first->second.second = eff.weights[s];
                                }
                                else
                                {
                                    varToWeightAB.insert(make_pair(lVar - pneCount, make_pair(0.0, 0.0)))
                                        .first->second.second = -eff.weights[s];
                                }
                            }
                        }
                    ...
                    if (varToWeightAB.size() > 1)
                    {
                        if (localDebug)
                        {
                            cout << "+ maybe worse: effect on " << *(RPGBuilder::getPNE(mItr->first))
                                 << " involves " << varToWeightAB.size() << " variables\n";
                        }
                    ...
                    if (mtStatus->second)
                    {
                        ...
                        // to find if a could be worse than b, we try to see if the bounds on the outcome from
                        // a are definitely better than those from b

                        // for now, we only cope with single variables here

                        double aImprovementOverB =
                            mItr->second.first->constant - mItr->second.second->constant;
                        ...
                        if (aImprovementOverB < 0.001)
                        {
                            if (localDebug)
                            {
                                cout << "+ returning true - var should be bigger and improvement of a over b "
                                        "is = "
                                     << aImprovementOverB << endl;
                            }
                            return true;
                        }
                    }
                    else
                    {
                        ...
                        if (aCostIncreaseOverB > 0.001)
                        {
                            if (localDebug)
                            {
                                cout << "+ returning true - var should be smaller and saving of a over b is "
                                        "= "
                                     << aCostIncreaseOverB << endl;
                            }
                            return true;
                        }
                    }
    ...
    bool NumericAnalysis::preconditionOnlySupportsWorseMetricThanOtherOptions(
        const int &preID, const map<int, list<int>> &numericPreconditionsThatAreInCondEffs)
    {
        // This assumes that 'requiringNotFullOnlySupportsReplenishment' has already been called, to check
        // that the precondition isn't a goal, and its behaviour within preconditions (rather than conditional
        ...
        if (localDebug)
        {
            cout << "Looking at conditional effects that depend on "
                 << RPGBuilder::getNumericPreTable()[preID] << endl;
        }
        ...
        {
            // multi-variable numeric precondition - give up for now on trying to analyse if it's a range of
            // options
            if (localDebug)
            {
        ...
        for (; actItr != actEnd; ++actItr)
        {
            ...
            const list<RPGBuilder::ConditionalEffect> &condEffs =
                RPGBuilder::getActionsToConditionalEffects()[*actItr];
            ...
            for (; thisCondEff != ceEnd; ++thisCondEff)
            {
                ...
                {
                    list<pair<int, Planner::time_spec>>::const_iterator npItr =
                        thisCondEff->getNumericPreconditions().begin();
                    const list<pair<int, Planner::time_spec>>::const_iterator npEnd =
                        thisCondEff->getNumericPreconditions().end();
                    ...
                    for (; npItr != npEnd; ++npItr)
                    {
                        ...
                        bool thisPreCouldBeBuilt = false;
                        VarOpConst currPre(RPGBuilder::getNumericPreTable()[npItr->first],
                                           thisPreCouldBeBuilt);
                ...
                {
                    list<pair<int, Planner::time_spec>>::const_iterator neItr =
                        thisCondEff->getNumericEffects().begin();
                    const list<pair<int, Planner::time_spec>>::const_iterator neEnd =
                        thisCondEff->getNumericEffects().end();
            ...
            {
                // give up for now if all the comparisons to this var aren't just at either the start or end
                // of an action maybe at some point do something more clever?
                if (localDebug)
                {
            ...
            presWeKnowFromTheMasterCond.insert(withPre->getPropositionalConditions().begin(),
                                               withPre->getPropositionalConditions().end());

            {
                // for numerics, exclude anything depending on the current var
                list<pair<int, Planner::time_spec>>::const_iterator npItr =
                    withPre->getNumericPreconditions().begin();
                const list<pair<int, Planner::time_spec>>::const_iterator npEnd =
                    withPre->getNumericPreconditions().end();
            ...
            for (; ocItr != ocEnd; ++ocItr)
            {
                ...
                {
                    set<pair<Literal *, Planner::time_spec>> presFromThisOutcome(
                        ocItr->outcome->getPropositionalConditions().begin(),
                        ocItr->outcome->getPropositionalConditions().end());

                    set<pair<Literal *, Planner::time_spec>> notCovered;

                    std::set_difference(presFromThisOutcome.begin(), presFromThisOutcome.end(),
                                        presWeKnowFromTheMasterCond.begin(),
                                        presWeKnowFromTheMasterCond.end(),
                                        std::insert_iterator<set<pair<Literal *, Planner::time_spec>>>(
                                            notCovered, notCovered.begin()));
                    ...
                    if (!notCovered.empty())
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "   > One of the other outcomes has propositional preconditions not "
                                    "covered by those attached to the outcome under scrutiny\n";
                        }
                        return false;
                    }
                }

                {
                    ...
                    list<pair<int, Planner::time_spec>>::const_iterator npItr =
                        ocItr->outcome->getNumericPreconditions().begin();
                    const list<pair<int, Planner::time_spec>>::const_iterator npEnd =
                        ocItr->outcome->getNumericPreconditions().end();
                    ...
                    std::set_difference(numericPresFromThisOutcome.begin(), numericPresFromThisOutcome.end(),
                                        numericPresWeKnowFromTheMasterCond.begin(),
                                        numericPresWeKnowFromTheMasterCond.end(),
                                        std::insert_iterator<set<pair<int, Planner::time_spec>>>(
                                            notCovered, notCovered.begin()));
                    ...
                    if (!notCovered.empty())
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "   > One of the other outcomes has numeric preconditions not covered by "
                                    "those attached to the outcome under scrutiny\n";
                        }
...

            {
                ...
                while (visitFirst)
                {
                    ...
                    list<ConditionBoundsAndOutcome *>::const_iterator succItr =
                        visitFirst->successors.begin();
                    const list<ConditionBoundsAndOutcome *>::const_iterator succEnd =
                        visitFirst->successors.end();
...

            {
                ...
                if (lowest.lb.op == VAL::E_GREATEQ)
                {
                    ...
                    {
                        lbTmp = auto_ptr<ConditionBoundsAndOutcome>(new ConditionBoundsAndOutcome(
                            0, lbPre, VarOpConst(masterVar, lowest.lb.constant, VAL::E_LESS)));
                ...
                else
                {
                    ...
                    {
                        lbTmp = auto_ptr<ConditionBoundsAndOutcome>(new ConditionBoundsAndOutcome(
                            0, lbPre, VarOpConst(masterVar, lowest.lb.constant, VAL::E_LESSEQ)));
                        continuum.push_front(lbTmp.get());
                    }
                }
            }

            {
                ...
                if (highest.ub.op == VAL::E_LESSEQ)
                {
                    ...
                    {
                        ubTmp = auto_ptr<ConditionBoundsAndOutcome>(new ConditionBoundsAndOutcome(
                            0, VarOpConst(masterVar, highest.ub.constant, VAL::E_GREATER), ubPre));
                ...
                else
                {
                    ...
                    {
                        ubTmp = auto_ptr<ConditionBoundsAndOutcome>(new ConditionBoundsAndOutcome(
                            0, VarOpConst(masterVar, highest.ub.constant, VAL::E_GREATEQ), ubPre));
                        continuum.push_back(ubTmp.get());
                    }
                }
            }
            ...
            {
                if (couldBeWorseThan((*mustBeAtLeastAsGood)->outcome, (*conditionedOnThisPre)->outcome,
                                     actBounds))
                {
                    // found a worse outcome
                    if (localDebug)
                    {
                        cout << "Outcome from range {" << (*mustBeAtLeastAsGood)->lb << ","
                             << (*mustBeAtLeastAsGood)->ub
                             << "} cannot be worse than the output from the range defined by this pre in "
                                "order to make this pre not necessarily a good idea\n";
                    }
                    return false;
                }
                ...
                {
                    cout << "Outcome from range {" << (*mustBeAtLeastAsGood)->lb << ","
                         << (*mustBeAtLeastAsGood)->ub << "} is fine\n";
                }
            }
            ...
            for (; cannotBeBetter != cannotBeBetterEnd; ++cannotBeBetter)
            {
                if (couldBeWorseThan((*conditionedOnThisPre)->outcome, (*cannotBeBetter)->outcome, actBounds))
                {
                    ...
                    if (localDebug)
                    {
                        cout << "Outcome from range {" << (*cannotBeBetter)->lb << ","
                             << (*cannotBeBetter)->ub
                             << "} cannot be better than the output from the range defined by this pre in "
                                "order to make this pre not necessarily a good idea\n";
                    }
                ...
                if (localDebug)
                {
                    cout << "Outcome from range {" << (*cannotBeBetter)->lb << "," << (*cannotBeBetter)->ub
                         << "} is fine\n";
                }
            }
        }
        ...
        if (localDebug)
        {
            cout << RPGBuilder::getNumericPreTable()[preID]
                 << " only supports metrically worse options than the alternatives\n";
        }
    ...
    bool NumericAnalysis::requiringNotFullOnlySupportsReplenishment(const int &preID)
    {
        ...
        if (localDebug)
        {
            cout << "Seeing if " << RPGBuilder::getNumericPreTable()[preID]
                 << " only supports effects in the opposite direction\n";
        }
        ...
        const list<pair<int, Planner::time_spec>> &actionsWithThatPre =
            RPGBuilder::getRawRpgNumericPreconditionsToActions()[preID];

        {
            ...
            for (; ngItr != ngEnd; ++ngItr)
            {
                if (ngItr->first == preID)
                {
                    if (localDebug)
                    {
                        cout << "The precondition is a goal, so is definitely worth aiming for - returning "
                                "false\n";
                    }
                    return false;
                }
        ...
        if (actionsWithThatPre.empty())
        {
            // if no actions require the precondition, bail out here
            {
                if (localDebug)
                {
                    cout << "No actions have this as a precondition, and is not a goal, but it is not "
                            "desirable/undesirable for preferences - returning true\n";
                }
        ...
        if (varID >= 2 * RPGBuilder::getPNECount())
        {
            ...
            if (currAV.size != 1)
            {
                ...
                if (localDebug)
                {
                    cout << "Variable is in multi-variable AV, so no obvious increase/decrease resource "
                            "analogy - returning false\n";
                }
                return false;
            }
        ...
        for (; actItr != actEnd; ++actItr)
        {
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                const list<Literal *> &effList =
                    (pass ? RPGBuilder::getEndPropositionAdds()[actItr->first] :
                ...
                for (int fID; eItr != eEnd; ++eItr)
                {
                    ...
                    if (!RPGBuilder::getPresToActions()[fID].empty())
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "An action with this pre adds " << *(*eItr)
                                 << ", required by an action - returning false\n";
                        }
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                const list<int> &effs = (pass ? RPGBuilder::getEndEffNumerics()[actItr->first] :
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    ...
                    if (currEff.fluentIndex != varID)
                    {
                        ...
                        if (localDebug)
                        {
                            cout << *(RPGBuilder::getInstantiatedOp(actItr->first))
                                 << ", with this pre, affects some other variable, "
                                 << *(RPGBuilder::getPNE(currEff.fluentIndex)) << ", so returning false\n";
                        }
                        return false;
                    }
                    ...
                    if (currEff.size)
                    {
                        ...
                        const list<int> &simultaneousNumericPres =
                            (pass ? RPGBuilder::getEndPreNumerics()[actItr->first] :
                        ...
                        for (int s = 0; s < currEff.size; ++s)
                        {
                            ...
                            if (lVar == -3 || lVar == -19)
                            {
                                const vector<RPGBuilder::RPGDuration *> &des =
                                    RPGBuilder::getRPGDEs(actItr->first);
                                ...
                                for (int pass = 0; pass < 3; ++pass)
                                {
                                    ...
                                    const list<RPGBuilder::DurationExpr *>::const_iterator deEnd =
                                        currList.end();
                                    ...
                                    for (; deItr != deEnd; ++deItr)
                                    {
                                        ...
                                        if (lVar == currEff.fluentIndex)
                                        {
                                            if (pass == 0 || pass == 2)
                                            {
                                                // is ?duration = the var we're looking for times some
                                                // constant

                                                if (upperBoundsOnVariable.find(-3) !=
                                                    upperBoundsOnVariable.end())
                                                {
                                                    // duplicate bound defined
                                                    if (localDebug)
                                                    {
                                                        cout << COLOUR_light_red
                                                             << "An action with this pre has a complex "
                                                                "effect "
                                                             << currEff
                                                             << " with two upper bounds on ?duration - "
                                                                "returning false\n"
                                                             << COLOUR_default;
                                            ...
                                            if (pass == 0 || pass == 1)
                                            {
                                                if (lowerBoundsOnVariable.find(-3) !=
                                                    lowerBoundsOnVariable.end())
                                                {
                                                    // duplicate bound defined
                                                    if (localDebug)
                                                    {
                                                        cout << COLOUR_light_red
                                                             << "An action with this pre has a complex "
                                                                "effect "
                                                             << currEff
                                                             << " with two lower bounds on ?duration - "
                                                                "returning false\n"
                                                             << COLOUR_default;
                                                        // complex effect, not simple constant
                                                        return false;
                            ...
                            else
                            {
                                ...
                                {
                                    cout << COLOUR_light_red
                                         << "An action with this pre has a complex effect " << currEff
                                         << " depending on something other than ?duration - returning false\n"
                                         << COLOUR_default;
                                    // complex effect, not simple constant
                                }
                    ...
                    for (int s = 0; s < currEff.size; ++s)
                    {
                        ...
                        if (geq)
                        {
                            ...
                            // we want to see if we can get an effect such that x is assigned something less
                            // than c
                            if (lW < 0)
                            {
                                // negative weighted duration term: max out the duration
                                const map<int, pair<double, double>>::const_iterator boundItr =
                                    upperBoundsOnVariable.find(lVar);
                                ...
                                else
                                {
                                    ...
                                    rhsAsWeightedSum.insert(make_pair(currEff.fluentIndex, 0.0))
                                        .first->second += boundItr->second.first * lW;
                                }
                            }
                            else
                            {
                                ...
                                const map<int, pair<double, double>>::const_iterator boundItr =
                                    lowerBoundsOnVariable.find(lVar);
                        ...
                        {
                            // precondition is of the form x <= c
                            // we want to see if we can get an effect such that x is assigned something
                            // greater than c
                    ...
                    if (fiWeight != rhsAsWeightedSum.end())
                    {
                        if (fabs(fiWeight->second) > 0.00001)
                        {
                            ...
                            {
                                cout << COLOUR_light_red
                                     << "Reduction to assigning a constant failed: " << fiWeight->second
                                     << " is non-zero - returning false\n"
                                     << COLOUR_default;
                                // complex effect, not simple constant
                            }
                            return false;
                    ...
                    if (localDebug)
                    {
                        cout << COLOUR_yellow << "Have reduced " << currEff << " to, at most, (assign "
                             << *(RPGBuilder::getPNE(currEff.fluentIndex)) << " " << rhsConstant << ")\n"
                             << COLOUR_default;
                    }
                    ...
                    if (geq)
                    {
                        if (rhsConstant - constant < -0.000001)
                        {
                            if (localDebug)
                            {
                                cout << "Have a band-gap assignment: decreasing sufficiently allows us to "
                                        "exceed prior bound, getting to "
                                     << rhsConstant << " instead of " << constant << " - returning false\n";
                            }
                    ...
                    else
                    {
                        if (rhsConstant - constant > 0.000001)
                        {
                            if (localDebug)
                            {
                                cout << "Have a band-gap assignment: increasing sufficiently allows us to "
                                        "exceed prior bound, getting to "
                                     << rhsConstant << " instead of " << constant << " - returning false\n";
                            }
                    ...
                    if (!currEff.isAssignment) {
                        if (geq) {
                            if (currEff.constant < 0) {
                                if (localDebug) {
                                    cout << "An action with this pre can decrease the var - returning
                    false\n";
                                }
                                return false;
                            }
                            ...
                            if (currEff.constant > 0) {
                                if (localDebug) {
                                    cout << "An action with this pre can increase the var - returning
                    false\n";
                                }
                                return false;
                            }
                        ...
                        if (geq) {
                            if (currEff.constant - constant < -0.000001 ) {
                                if (localDebug) {
                                    cout << "Have a band-gap assignment: decreasing sufficiently allows us to
                    exceed prior bound - returning false\n";
                                }
                                // have a 'gap' situation: v >= c allows (assign v d), where d < c
                                return false;
                            }
                            ...
                            if (currEff.constant - constant > 0.000001 ) {
                                if (localDebug) {
                                    cout << "Have a band-gap assignment: increasing sufficiently allows us to
                    go lower than prior bound - returning false\n";
                                }
        ...
        if (localDebug)
        {
            cout << "All actions with this pre either push it in a 'less satisfied' direction, or assign it "
                    "to the same value it must be leq/geq than - returning true\n";
        }

        return true;
    }

    void NumericAnalysis::updateForDurationDependence(
        const int &v, const double &w, const int &durVar, const bool &pass,
            ...
            case E_SMALLERISBETTER:
            {
                if (w < 0.0)
                {
                               ...
                               workingDominanceConstraints[durVar] == E_SMALLERISBETTER)
                    {
                        if (localDebug)
                        {
                            cout << "This erodes any dominance on " << *(RPGBuilder::getPNE(durVar))
                                 << ", as for this effect it's better for the duration to be long\n";
                        }
                ...
                else if (w > 0.0)
                {
                               ...
                               workingDominanceConstraints[durVar] == E_BIGGERISBETTER)
                    {
                        ...
                        {
                            cout << "This erodes any dominance on " << *(RPGBuilder::getPNE(durVar))
                                 << ", as for this effect it's better for the duration to be short\n";
                        }
    ...
    void NumericAnalysis::considerCostsOfLiteralGoalsReachedByGoalOnlyOperators()
    {
        ...
        for (int gID = 0; gItr != gEnd; ++gItr, ++gID)
        {
                ...
                !RPGBuilder::getRawPresToActions()[(*gItr)->getStateID()].empty())
            {
                if (localDebug & 1)
                {
                    cout << COLOUR_light_blue << "For limits: literal goal index " << gID << ", fact "
                         << *(*gItr) << ", is static or a precondition" << COLOUR_default << endl;
                }

                continue;
            }

            const list<pair<int, Planner::time_spec>> &eta =
                RPGBuilder::getEffectsToActions((*gItr)->getStateID());

            set<int> achievedBy;

            if (localDebug & 4)
            {
                cout << COLOUR_light_red << "Looking for achievers for goal index " << gID << ", fact "
                     << *(*gItr) << " with fID " << (*gItr)->getStateID() << COLOUR_default << endl;
            }
            ...
            if (eItr != eEnd)
            {
                ...
                {
                    cout << COLOUR_light_blue << "For limits: literal goal index " << gID << ", fact "
                         << *(*gItr) << ", could be reached by a TIL" << COLOUR_default << endl;
                }
                continue;
            }
...

            {
                ...
                for (; safe && aItr != aEnd; ++aItr)
                {
                    ...
                    {
                        const list<Literal *> &adds = (pass ? RPGBuilder::getEndPropositionAdds()[*aItr] :
                                                              RPGBuilder::getStartPropositionAdds()[*aItr]);
                        ...
                        for (; aeItr != aeEnd; ++aeItr)
                        {
                            if (*aeItr != *gItr)
                            {
                                if (localDebug & 1)
                                {
                                    cout << COLOUR_light_blue << "For limits: literal goal index " << gID
                                         << ", fact " << *(*gItr) << ", could be achieved by operator ";
                                    cout << *(RPGBuilder::getInstantiatedOp(*aItr))
                                         << ", which has other interesting effects (including one on "
                                         << *(*aeItr) << " )" << COLOUR_default << endl;
                                }
                                safe = false;
                                break;
                            }
                    ...
                    if (localDebug & 1)
                    {
                        cout << "  Looking at numeric effects of " << *(RPGBuilder::getInstantiatedOp(*aItr))
                             << ": ";
                        cout << RPGBuilder::getStartEffNumerics()[*aItr].size() << " and "
                             << RPGBuilder::getEndEffNumerics()[*aItr].size() << endl;
                    }

                    for (int pass = 0; safe && pass < 2; ++pass)
                    {
                        const list<int> &numEffs = (pass ? RPGBuilder::getEndEffNumerics()[*aItr] :
                                                           RPGBuilder::getStartEffNumerics()[*aItr]);
                        ...
                        for (; safe && neItr != neEnd; ++neItr)
                        {
                                ...
                                if (currEff.size > 1 || currEff.isAssignment)
                                {
                                    if (localDebug & 1)
                                    {
                                        cout << COLOUR_light_blue << "For limits: literal goal index " << gID
                                             << ", fact " << *(*gItr) << ", could be achieved by operator ";
                                        cout << *(RPGBuilder::getInstantiatedOp(*aItr))
                                             << ", which has a non-trivial numeric effect " << currEff
                                             << COLOUR_default << endl;
                                    }
                            ...
                            switch (dominanceConstraints[currEff.fluentIndex])
                            {
                                case E_IRRELEVANT:
                                {
                                    cerr << "Internal error: this should never, ever happen, as it means the "
                                            "dominance constraints have changed mid-function\n";
                                    exit(1);
                                }
                                case E_METRICTRACKING:
                                {
                                ...
                                case E_SMALLERISBETTER:
                                {
                                    if (currEff.constant < 0.0)
                                    {
                                        ...
                                        if (localDebug & 1)
                                        {
                                            cout << COLOUR_light_blue << "For limits: literal goal index "
                                                 << gID << ", fact " << *(*gItr)
                                                 << ", could be achieved by operator ";
                                            cout << *(RPGBuilder::getInstantiatedOp(*aItr))
                                                 << ", which has a beneficial decrease effect " << currEff
                                                 << COLOUR_default << endl;
                                        }
                                    }
                                    anyNum = true;
                                    ...
                                    if (currEff.constant > 0.0)
                                    {
                                        ...
                                        if (localDebug & 1)
                                        {
                                            cout << COLOUR_light_blue << "For limits: literal goal index "
                                                 ...
                                                 << ", could be achieved by operator ";
                                            cout << *(RPGBuilder::getInstantiatedOp(*aItr))
                                                 << ", which has a beneficial increase effect " << currEff
                                                 << COLOUR_default << endl;
                                        }
                                    }
                                    anyNum = true;
                                ...
                                default:
                                {
                                    if (localDebug & 1)
                                    {
                                        cout << COLOUR_light_blue << "For limits: literal goal index " << gID
                                             << ", fact " << *(*gItr) << ", could be achieved by operator ";
                                        cout << *(RPGBuilder::getInstantiatedOp(*aItr))
                                             << ", which has a no-known-dominance effect " << currEff
                                             << COLOUR_default << endl;
                                    }
                                    safe = false;
                                }
                    ...
                    {
                        const list<RPGBuilder::ConditionalEffect> &condEffs =
                            RPGBuilder::getActionsToConditionalEffects()[*aItr];

                        if (localDebug & 1)
                        {
                            cout << "  No troublesome numerics - looking at conditional effects on "
                                 << *(RPGBuilder::getInstantiatedOp(*aItr)) << ", of which there are "
                                 << condEffs.size() << endl;
                        }
                        ...
                        for (; ceItr != ceEnd; ++ceItr)
                        {
                            ...
                            for (; neItr != neEnd; ++neItr)
                            {
                                const RPGBuilder::RPGNumericEffect &currEff =
                                    RPGBuilder::getNumericEff()[neItr->first];
                                ...
                                if (currEff.size > 1 || currEff.isAssignment)
                                {
                                    if (localDebug & 1)
                                    {
                                        cout << COLOUR_light_blue << "For limits: literal goal index " << gID
                                             << ", fact " << *(*gItr) << ", could be achieved by operator ";
                                        cout << *(RPGBuilder::getInstantiatedOp(*aItr))
                                             << ", which has a non-trivial conditional numeric effect "
                                             << currEff << COLOUR_default << endl;
                                    }
            ...
            if (safe && anyNum)
            {
                ...
                // ii) have numeric effects that are only ever a bad idea, so we'd never want to apply them in
                // their
                //     own right for that
                ...
                for (; aItr != aEnd; ++aItr)
                {
                    ...
                    for (int pass = 0; safe && pass < 2; ++pass)
                    {
                        ...
                        for (; safe && neItr != neEnd; ++neItr)
                        {
                            ...
                            const map<int, list<int>>::const_iterator lsItr =
                                variableIsInLimit.find(currEff.fluentIndex);
                ...
                {
                    cout << COLOUR_yellow << "To achieve goal index " << gID << ", fact " << *(*gItr)
                         << ", there is a definite effect on one or more limits:\n"
                         << COLOUR_default;

                    map<int, list<int>>::const_iterator leItr = goalHasIndependentCostOnLimit[gID].begin();
                    const map<int, list<int>>::const_iterator leEnd =
                        goalHasIndependentCostOnLimit[gID].end();
    ...
    void NumericAnalysis::considerConflictingDominanceThroughEffects(
        vector<dominance_constraint> &workingDominanceConstraints)
    {
        static const bool localDebug = (Globals::globalVerbosity & 16384);
        const int pneCount = RPGBuilder::getPNECount();
        ...
        for (int effID = 0; effID < effCount; ++effID)
        {
            ...
            /*{
                list<pair<int, Planner::time_spec> >::const_iterator actItr =
            RPGBuilder::getRpgNumericEffectsToActions()[effID].begin(); const list<pair<int,
            Planner::time_spec> >::const_iterator actEnd =
            RPGBuilder::getRpgNumericEffectsToActions()[effID].end();
                ...
                for (; actItr != actEnd; ++actItr) {
                    ...
                    // otherwise, during the action's execution, something else could change the value of the
            variable
                    // and hence mess up the analysis by making it actually do net-worse when we thought it
            would be net-better
                    ...
                    if (!mustBeGuarded.empty()) {
                        ...
                        {
                            list<Literal*>::const_iterator addItr =
            RPGBuilder::getStartPropositionAdds()[actID].begin(); const list<Literal*>::const_iterator addEnd
            = RPGBuilder::getStartPropositionAdds()[actID].end();
            ...
            RPGBuilder::getStartPropositionDeletes()[actID].begin(); const list<Literal*>::const_iterator
            delEnd = RPGBuilder::getStartPropositionDeletes()[actID].end();

                            for (; delItr != delEnd; ++delItr) {
                                const map<int, RPGBuilder::Guarded>::const_iterator sfItr =
            RPGBuilder::getSemaphoreFacts().find((*delItr)->stateID());

                                if (sfItr == RPGBuilder::getSemaphoreFacts().end()) {
                                    continue;
                                }

                                guarded.insert(sfItr->second.guardedEffectVars.begin(),
            sfItr->second.guardedEffectVars.end());
                            }
                        }

                        set<int> unguarded;

                        std::set_difference(mustBeGuarded.begin(), mustBeGuarded.end(), guarded.begin(),
            guarded.end(), std::insert_iterator<set<int> >(unguarded, unguarded.end()));
...

                    {
                        ...
                        for (; seItr != seEnd; ++seItr) {
                            ...
                            varToPresentStartEffect.insert(make_pair(rpgNumericEffects[*seItr].fluentIndex,
            *seItr));
...

                    {
                        ...
                        for (int vPass = 0; vPass < 2; ++vPass) {
                            ...
                            for (; gvItr != gvEnd; ++gvItr) {
                                ...
                                map<int, int>::const_iterator startEffItr =
            varToPresentStartEffect.find(*gvItr);

                                if (startEffItr != varToPresentStartEffect.end()) {

                                    const RPGBuilder::RPGNumericEffect & earlierEff =
            rpgNumericEffects[startEffItr->second];
                                    ...
                                    for (int s = 0; s < earlierEff.size; ++s) {
                                        if (earlierEff.variables[s] < 0) {
                                            if (earlierEff.variables[s] == -3) {
                                                dest.startWeights.insert(make_pair(-3,0.0)).second +=
            earlierEff.weights[s]; } else if (earlierEff.variables[s] == -19) {
                                                dest.startWeights.insert(make_pair(-3,0.0)).second -=
            earlierEff.weights[s]; } else { cerr << "Fatal internal error: non duration or var term " <<
            earlierEff.variables[s] << " in instantaneous effect " << earlierEff << endl; exit(1);
                                            }
                                        } else {
                                            if (earlierEff.variables[s] >= pneCount) {
                                                dest.startWeights.insert(make_pair(earlierEff.variables[s] -
            pneCount,0.0)).second -= earlierEff.weights[s]; } else {
                                                dest.startWeights.insert(make_pair(earlierEff.variables[s],0.0)).second
            += earlierEff.weights[s];
                                            }
                                        }
                                    }
            ...
            if (everIsolated)
            {
                    ...
                    case E_NODOMINANCE:
                    {
                        for (int vi = 0; vi < currEff.size; ++vi)
                        {
                            ...
                            if (v < 0)
                            {
                                ...
                                {
                                    cerr << "Internal error: effects can only depend on task variables, "
                                            "constant, or ?duration\n";
                                    exit(1);
                                }
                                continue;
                            }
                            if (v >= pneCount)
                            {
                                if (localDebug)
                                {
                                    ...
                                    {
                                        cout << "Effect " << currEff << " erodes any dominance on "
                                             << *(RPGBuilder::getPNE(v - pneCount)) << endl;
                                    }
                                }
                ...
                if (effectIsDurative != 0)
                {
                    ...
                    const list<pair<int, Planner::time_spec>> &eta =
                        RPGBuilder::getRpgNumericEffectsToActions()[effID];
                    ...
                    {
                        actionToDurationDependentEffects[etaItr->first].insert(
                            make_pair(effID, effectIsDurative == 1 ? true : false));
                        if (localDebug)
                        {
                            cout << " " << etaItr->first;
                        }
        ...
        for (int actID = 0; actID < actCount; ++actID)
        {
            ...
            if (DEs.empty())
            {
                cerr << "Fatal internal error: action " << actID << ", "
                     << *(RPGBuilder::getInstantiatedOp(actID)) << " has no duration constraints, but has ";
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                ...
                for (; vItr != vEnd; ++vItr)
                {
                    for (int cvID = 0; cvID < ctsVCount; ++cvID)
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "A continuous effect on " << *(RPGBuilder::getPNE(v)) << " by "
                                 << *(RPGBuilder::getInstantiatedOp(actID)) << " is made ";
                            if (pass)
                            {
                                cout << "smaller by ";
                            }
                    ...
                    if (ddEffs != actionToDurationDependentEffects.end() && ddEffs->first == actID)
                    {
                        ...
                        for (; effID != effIDEnd; ++effID)
                        {
                            ...
                            if (localDebug)
                            {
                                cout << "A duration-dependent effect on " << *(RPGBuilder::getPNE(v))
                                     << " by " << *(RPGBuilder::getInstantiatedOp(actID)) << " is made ";
                                if (pass)
                                {
                                    cout << "smaller by ";
                                }
                            ...
                            updateForDurationDependence(v, w, *vItr, (pass == 1),
                                                        workingDominanceConstraints);
                        }
                    }
                }
            }
    ...
    NumericAnalysis::NumericLimitDescriptor::NumericLimitDescriptor(const int &v, const double &w,
                                                                    const VAL::comparison_op &cOp,
                                                                    const double &lim)
    ...
    NumericAnalysis::NumericLimitDescriptor::NumericLimitDescriptor(const vector<int> &v,
                                                                    const vector<double> &w, const int &size,
                                                                    const VAL::comparison_op &cOp,
                                                                    const double &lim)
      : op(cOp), limit(lim), optimisationLimit(false)
    {
            ...
            if (varBounds.first == -DBL_MAX)
            {
                ...
                if (-weightsecond > 0.001)
                {
                    if (localDebug)
                    {
                        ...
                        cout << ", and weight is " << weightsecond
                             << ", so could lead to metric improvement\n";
                    }
                    return false;
                }
            }
    ...
    void
    NumericAnalysis::seeIfInducesALimit(const NumericAnalysis::NumericLimitDescriptor &hypothesisedLimit,
                                        ...
                                        map<NumericAnalysis::NumericLimitDescriptor, bool> &upperBounds)
    {
        ...
        const map<NumericAnalysis::NumericLimitDescriptor, bool>::iterator bItr =
            upperBounds.find(hypothesisedLimit);
        ...
        if (bItr != upperBounds.end())
        {
            if (bItr->second)
            {
                ...
                NumericAnalysis::NumericLimitDescriptor *const editableFirst =
                    const_cast<NumericAnalysis::NumericLimitDescriptor *>(&(bItr->first));
            ...
            // if we've just updated a previous constraint, we don't need to go on to one-way checking and
            // then deduce whether to keep the limit based on that
            return;
        }
            ...
            {
                {
                    ...
                    for (; actItr != actEnd; ++actItr)
                    {
                        if (localDebug)
                        {
                            cout << "- Attached to " << *(RPGBuilder::getInstantiatedOp(actItr->first))
                                 << endl;
                        }
                        ...
                        if (currEff.variables[0] == -3 || currEff.variables[0] == -19)
                        {
                            ...
                            /*if (localDebug) {
                                cout << "  - Tightened lower bound on duration to " <<
                            RPGBuilder::getOpMinDuration(actItr->first,0) << " => " << bounds[-3].first <<
                            endl;
                            }*/
                            bounds.tightenUpper(-3, RPGBuilder::getOpMaxDuration(actItr->first, 0));
                            /*if (localDebug) {
                                cout << "  - Tightened upper bound on duration to " <<
                            RPGBuilder::getOpMaxDuration(actItr->first,0) << " => " << bounds[-3].second <<
                            endl;
                            }*/
                        }
                        else
                        {
                            const list<int> &preList = (actItr->second == Planner::E_AT_START ?
                                                            RPGBuilder::getStartPreNumerics()[actItr->first] :
                ...
                if (allDecreasers)
                {
                    const map<int, list<int>>::const_iterator ceItr =
                        numericEffectsThatAreInConditionalEffects.find(effID);
                    if (ceItr != numericEffectsThatAreInConditionalEffects.end())
                    {
                        ...
                        for (; actItr != actEnd; ++actItr)
                        {
                            if (localDebug)
                            {
                                cout << "- Attached to a conditional effect of "
                                     << *(RPGBuilder::getInstantiatedOp(*actItr)) << endl;
                            }

                            const list<RPGBuilder::ConditionalEffect> &condEffs =
                                RPGBuilder::getActionsToConditionalEffects()[*actItr];
                            list<RPGBuilder::ConditionalEffect>::const_iterator currCEItr = condEffs.begin();
                            const list<RPGBuilder::ConditionalEffect>::const_iterator currCEEnd =
                                condEffs.end();

                            for (; currCEItr != currCEEnd; ++currCEItr)
                            {
                                const list<pair<int, Planner::time_spec>> &neMatchIn =
                                    currCEItr->getNumericEffects();

                                list<pair<int, Planner::time_spec>>::const_iterator neMatchItr =
                                    neMatchIn.begin();
                                const list<pair<int, Planner::time_spec>>::const_iterator neMatchEnd =
                                    neMatchIn.end();
                                ...
                                for (; neMatchItr != neMatchEnd; ++neMatchItr)
                                {
                                    ...
                                    {
                                        {
                                            const list<int> &preList =
                                                (neMatchItr->second == Planner::E_AT_START ?
                                                     RPGBuilder::getStartPreNumerics()[*actItr] :
                                            ...
                                            for (; npItr != npEnd; ++npItr)
                                            {
                                                actBounds.applyPreToBounds(
                                                    RPGBuilder::getNumericPreTable()[*npItr]);
                                            }
                                        }

                                        {
                                            const list<pair<int, Planner::time_spec>> &preListB =
                                                currCEItr->getNumericPreconditions();
                                            ...
                                            for (; npItr != npEnd; ++npItr)
                                            {
                                                ...
                                                actBounds.applyPreToBounds(
                                                    RPGBuilder::getNumericPreTable()[npItr->first]);
                                            }
                                        }
                                    }
    ...
    void NumericAnalysis::findGoalNumericUsageLimits()
    {
        ...
        {
            const vector<list<RPGBuilder::ConditionalEffect>> &condEffs =
                RPGBuilder::getActionsToConditionalEffects();
        ...
        if (Globals::optimiseSolutionQuality)
        {
            ...
            if (theMetric)
            {
                if (localDebug)
                {
                    cout << "Seeing if metric is defined in terms of task vars or a minimal value of "
                            "makespan\n";
                }
                ...
                if (metricOnlyUsesTaskVariables)
                {
                    ...
                    seeIfInducesALimit(hypothesisedLimit, numericEffectsThatAreInConditionalEffects,
                                       upperBounds);
                }
            }
        }
        ...
        for (; ngItr != ngEnd; ++ngItr)
        {
            for (int pass = 0; pass < 2; ++pass)
            {
                ...
                {
                    hypothesisedLimit =
                        NumericLimitDescriptor(currPre.LHSVariable, 1.0, currPre.op, currPre.RHSConstant);
                }
                else
                {
                    const RPGBuilder::ArtificialVariable &currAV =
                    ...
                    hypothesisedLimit =
                        NumericLimitDescriptor(currAV.fluents, currAV.weights, currAV.size, currPre.op,
                                               currPre.RHSConstant - currAV.constant);
                }
    ...
    void NumericAnalysis::findDominanceConstraintsAndMetricTrackingVariables()
    {
            ...
            if (theMetric)
            {
                ...
                for (; mvItr != mvEnd; ++mvItr, ++mwItr)
                {
                    if (*mvItr >= 0)
                    {
                        if (dominanceConstraints[*mvItr] == E_IRRELEVANT)
                        {
                            ...
                            {
                                cout << "Keeping " << *(RPGBuilder::getPNE(*mvItr))
                                     << " as a metric-tracking variable\n";
                            }
                        ...
                        if (theMetric->minimise)
                        {
                            // metric is to minimise: bigger vars will give a better metric iff they have
                            // negative weights
                            metricVarIsBetterBigger[*mvItr] = (*mwItr < 0);
                        }
                        else
                        {
                            // metric is to maximise: bigger vars will give a better metric iff they have
                            // positive weights
                            metricVarIsBetterBigger[*mvItr] = (*mwItr > 0);
                        }
                    }
                ...
                while (!knownToBeUseful.empty())
                {
                    ...
                    for (int effID = 0; effID < effCount; ++effID)
                    {
                        ...
                        {
                            // if (Globals::globalVerbosity & 32) {
                            //     cout << "Ignoring effects on " <<
                            //     *(RPGBuilder::getPNE(effs[effID].fluentIndex)) << " as it's not just been
                            //     labelled as being metric-tracking\n";
                            // }
                            //  ignore effects on irrelevant variables
                            continue;
                        }
                        ...
                        for (int i = 0; i < effs[effID].size; ++i)
                        {
                            ...
                            if (dominanceConstraints[currVar] == E_IRRELEVANT ||
                                dominanceConstraints[currVar] == E_METRICTRACKING)
                            {
                                // this variable is an input to an effect on a known-useful variable
                                // it isn't irrelvant any more...
                                dominanceConstraints[currVar] = E_NODOMINANCE;
                                if (Globals::globalVerbosity & 32)
                                {
                                    cout << "Promoting " << *(RPGBuilder::getPNE(currVar))
                                         << " to not being irrelevant\n";
                                }
                                inputToAnEffect[currVar] = true;
                                // ...and next time round, make sure the inputs to any effects on this
                                // variable are also not marked as irrelevant
                                newKnownToBeUseful.push_back(currVar);
                            }
                            else
                            {
                                if (Globals::globalVerbosity & 32)
                                {
                                    cout << "Not changing " << *(RPGBuilder::getPNE(currVar))
                                         << " as it's not previously considered to be irrelevant\n";
                                }
                            }
                        }
                    }
        ...
        for (int i = 0; i < pneCount; ++i)
        {
            if (workingDCs[i] == E_NODOMINANCE && !inputToAnEffect[i])
            {
                if (!definedNumericPreconditionsThatAreInCondEffs)
                {
                    ...
                    for (int act = 0; act < actCount; ++act)
                    {
                        ...
                        for (; ceItr != ceEnd; ++ceItr)
                        {
                            ...
                            const list<pair<int, Planner::time_spec>> &numPres =
                                ceItr->getNumericPreconditions();
                            list<pair<int, Planner::time_spec>>::const_iterator npItr = numPres.begin();
                            const list<pair<int, Planner::time_spec>>::const_iterator npEnd = numPres.end();
                ...
                if ((workingDCs[i] = preconditionDominanceInOneDirection(
                         i, numericPreconditionsThatAreInCondEffs)) != E_NODOMINANCE)
                {
                    anyCandidates = true;
                }
            }
        ...
        /*for (int i = 0; i < pneCount; ++i) {
            ...
            if (workingDCs[i] == E_BIGGERISBETTER) {
                cout << "Have identified that bigger values of " << *(RPGBuilder::getPNE(i)) << " might be
        preferable\n"; } else if (workingDCs[i] == E_SMALLERISBETTER) { cout << "Have identified that smaller
        values of " << *(RPGBuilder::getPNE(i)) << " might be preferable\n";
            }
        }*/
        ...
        for (int i = 0; i < pneCount; ++i)
        {
            if (workingDCs[i] == E_BIGGERISBETTER)
            {
                ...
                cout << "Have identified that bigger values of " << *(RPGBuilder::getPNE(i))
                     << " are preferable\n";
            }
            else if (workingDCs[i] == E_SMALLERISBETTER)
            {
                dominanceConstraints[i] = E_SMALLERISBETTER;
                cout << "Have identified that smaller values of " << *(RPGBuilder::getPNE(i))
                     << " are preferable\n";
            }
        }
        ...
        if (readBounds)
        {
            ...
            while (!boundsFile.eof())
            {
                ...
                {
                    cerr << "Error: " << setTo
                         << " has to be one of either E_BIGGERISBETTER or E_SMALLERISBETTER\n";
                    exit(1);
                }
    ...
    bool durationsAreConstantBounded(const list<pair<int, Planner::time_spec>> &actions)
    {
        ...
        for (; actItr != actEnd; ++actItr)
        {
            ...
            if (duration.empty())
            {
                cerr << "Fatal internal error: have a duration-dependent effect on "
                     << *(RPGBuilder::getInstantiatedOp(actItr->first)) << ", but no durations\n";
                assert(!duration.empty());  // would mean a non-temporal action has a duration-dependent
                                            // effect
            }
            ...
            for (de = 0; de < 3; ++de)
            {
                const list<RPGBuilder::DurationExpr *> &currList = (*(duration[0]))[de];
                {
                    ...
                    {
                        if (!(*exprItr)->variables.empty())
                        {  // if the duration constraint depends on other variables
                            if (localDebug)
                            {
                                cout << "- Duration of " << *(RPGBuilder::getInstantiatedOp(actItr->first))
                                     << " is not constant-bounded\n";
                            }
                            return false;
                        }
                    }
    ...
    void NumericAnalysis::findWhichVariablesHaveOrderIndependentEffects()
    {
        ...
        {
            if (numericEffects[i].isAssignment)
            {  // assignment effects cannot be ordered arbitrarily
                allEffectsAreOrderIndependent[numericEffects[i].fluentIndex] = E_ORDER_DEPENDENT;
                if (localDebug)
                {
                    cout << "Assignment numeric effect " << numericEffects[i] << " makes effects on "
                         << numericEffects[i].fluentIndex << " be order-dependent\n";
                }
            }
            else if (numericEffects[i].size > 1)
            {
                ...
                if (localDebug)
                {
                    cout << "Multi-variate effect " << numericEffects[i] << " makes effects on "
                         << numericEffects[i].fluentIndex << " be order-dependent\n";
                }
            }
            else if (numericEffects[i].size == 1)
            {
...
#ifdef POPF3ANALYSIS
                const map<int, double>::const_iterator tItr = tickerVariables.find(
                    numericEffects[i].variables[0] >= pneCount ? numericEffects[i].variables[0] - pneCount :
                ...
                if (numericEffects[i].variables[0] != -3  // -3 denotes the variable ?duration
                    && numericEffects[i].variables[0] != -19)
                {  // -19 denotes -?duration
                    if (localDebug)
                    {
                        cout << "Numeric effect " << numericEffects[i] << " makes effects on "
                             << numericEffects[i].fluentIndex << " be order-dependent\n";
                    }
                    allEffectsAreOrderIndependent[numericEffects[i].fluentIndex] = E_ORDER_DEPENDENT;
                    continue;
                ...
                if (!durationsAreConstantBounded(RPGBuilder::getRpgNumericEffectsToActions()[i]))
                {
                    if (localDebug)
                    {
                        cout << "Non-constant-bounded duration-dependent effect " << numericEffects[i]
                             << " makes effects on " << numericEffects[i].fluentIndex
                             << " be order-dependent\n";
                    }
                    allEffectsAreOrderIndependent[numericEffects[i].fluentIndex] = E_ORDER_DEPENDENT;
                }
        ...
        for (int op = 0; op < opCount; ++op)
        {
            ...
            for (int v = 0; v < vCount; ++v)
            {
                ...
                if (!constantDur)
                {                                    // if the duration is non constant
                    currStatus = E_ORDER_DEPENDENT;  // then the effect could be order-dependent
                }
                else if (currStatus == E_ORDER_INDEPENDENT)
                {  // otherwise, if it's currently order-independent
                    currStatus = E_ORDER_INDEPENDENT_AT_END;  // then the CTS effect is order-independent, but
                                                              // only if no actions are executing
                }
            }
        }
    }
    ...
    void NumericAnalysis::findWhichVariablesAreOnlyInAtStarts()
    {
        ...
        for (int np = 0; np < npCount; ++np)
        {
            ...
            if (depItr != depEnd)
            {  // then it was found in an invariant
                ...
                {
                    const RPGBuilder::ArtificialVariable &currAV =
                        RPGBuilder::getArtificialVariable(numPres[np].LHSVariable);
    ...
    void NumericAnalysis::findWhetherTheMetricIsMonotonicallyWorsening()
    {
        ...
        if (!metricIsMonotonicallyWorsening && RPGBuilder::getMetric() &&
            RPGBuilder::getMetric()->variables.empty())
        {
            metricIsMonotonicallyWorsening =
                true;  // TemporalAnalysis::goalSoftDeadlinesHaveMonotonicallyWorseningCosts();
        }
    }
    ...
    {
        {
            ...
            for (int actID = 0; actID < actCount; ++actID)
            {
                if (RPGBuilder::rogueActions[actID] != RPGBuilder::OT_PROCESS)
                {
                    continue;
                }
                if (!ctsEffs[actID])
                ...
                {
                    tickerVariables.insert(make_pair(ctsEffs[actID]->vars[i], 0.0)).first->second +=
                        ctsEffs[actID]->effects[0][i].constant;
                }
            }
        }
    ...
    void NumericAnalysis::findEarlierIsBetterTimeDependentRewards()
    {
        ...
        for (int effID = 0; effID < effCount; ++effID)
        {
            ...
            if (varIsInWhichLimitAndWithWhatWeight.find(numericEffects[effID].fluentIndex) !=
                varIsInWhichLimitAndWithWhatWeight.end())
            {
        ...
        for (int act = 0; act < actCount; ++act)
        {
            ...
            for (; ceItr != ceEnd; ++ceItr)
            {
                ...
                for (; npItr != npEnd; ++npItr)
                {
                    if (varIsInWhichLimitAndWithWhatWeight.find(numericEffects[npItr->first].fluentIndex) !=
                        varIsInWhichLimitAndWithWhatWeight.end())
                    {
        ...
        for (; actItr != actEnd; ++actItr)
        {
...

            {
                ...
                for (; pItr != pEnd; ++pItr)
                {
                    ...
                    if (success)
                    {
                        ...
                        if (tItr != tickerVariables.end() &&
                            dominanceConstraints[limit.var] == E_SMALLERISBETTER)
                        {
...

            {
                ...
                for (; eItr != eEnd; ++eItr)
                {
                    ...
                    if (varIsInWhichLimitAndWithWhatWeight.find(currEff.fluentIndex) !=
                        varIsInWhichLimitAndWithWhatWeight.end())
                    {
                        ...
                        if (currEff.size)
                        {
                            ...
                            if (lVar < 0)
                            {
                                if (localDebug)
                                {
                                    cout << "Found a duration-dependent effect, doesn't fit desired pattern "
                                            "of time-dependency\n";
                                }
                            ...
                            else
                            {
                                ...
                                if (tItr == tickerVariables.end() ||
                                    dominanceConstraints[lVar] != E_SMALLERISBETTER)
                                {
                                    if (localDebug)
                                    {
                                        if (tItr == tickerVariables.end())
                                        {
                                            cout << "Found an effect depending on a non-ticker variable, "
                                                    "doesn't fit desired pattern of time-dependency\n";
                                        }
                                        else
                                        {
                                            cout << "Found an effect depending on a ticker variable which "
                                                    "isn't better to be smaller\n";
                                        }
                                    }
                        ...
                        if (npItr->second == actItr->second)
                        {
                            ...
                            if (success)
                            {
                                ...
                                else
                                {
                                    if (localDebug)
                                    {
                                        cout << "Found a conditional outcome conditioned on something other "
                                                "than a ticker, doesn't fit desired pattern of "
                                                "time-dependency\n";
                                    }
                                    dependsOnAnythingElse = true;
                                }
                            ...
                            else
                            {
                                if (localDebug)
                                {
                                    cout << "Found a conditional outcome with a non-simple v >= c or v <= c "
                                            "condition, doesn't fit desired pattern of time-dependency\n";
                                }
                            ...
                            varIsInWhichLimitAndWithWhatWeight.end())
                        {
                            ...
                            if (currEff.size)
                            {
                                ...
                                if (lVar < 0)
                                {
                                    ...
                                    if (localDebug)
                                    {
                                        cout << "Found a conditional duration-dependent effect, doesn't fit "
                                                "desired pattern of time-dependency\n";
                                    }
                                }
                                else
                                {
                                        ...
                                        dominanceConstraints[lVar] != E_SMALLERISBETTER)
                                    {
                                        ...
                                        if (localDebug)
                                        {
                                            ...
                                            {
                                                cout << "Found an effect depending on a ticker variable "
                                                        "which isn't better to be smaller\n";
                                            }
                                        }
                                    }
                                }
            ...
            list<CostAtTime *> *&dest =
                (actItr->second == Planner::E_AT_START ? timeDependentStartCosts[actItr->first] :
            ...
            for (; oItr != oEnd; ++oItr)
            {
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    ...
                    if (limitEffsAsTimeEffs[*effItr].empty())
                    {
                        ...
                        EffectDeterminedByTime *const evTmp =
                            new EffectDeterminedByTime(currEff.fluentIndex, currEff.constant, 0.0);
                        if (currEff.size)
                        {
                            assert(currEff.size == 1);
                            int lVar = currEff.variables[0];
                        ...
                        const map<int, map<int, double>>::const_iterator depItr =
                            varIsInWhichLimitAndWithWhatWeight.find(currEff.fluentIndex);
                        ...
                        for (; glItr != glEnd; ++glItr)
                        {
                            ...
                            if (localDebug)
                            {
                                cout << "    + Affects limit " << eHere->y << " by " << eHere->c << " + "
                                     << eHere->m << ".t\n";
                            }
                    ...
                    forList->limitCosts.insert(forList->limitCosts.end(),
                                               limitEffsAsTimeEffs[*effItr].begin(),
                                               limitEffsAsTimeEffs[*effItr].end());
                    forList->varCosts.insert(forList->varCosts.end(), varEffsAsTimeEffs[*effItr].begin(),
                                             varEffsAsTimeEffs[*effItr].end());
                }
        ...
        if (anyTDRs)
        {
...

            {
                ...
                for (; fItr != fEnd; ++fItr)
                {
                    const list<pair<int, Planner::time_spec>> &achievers =
                        RPGBuilder::getEffectsToActions(fItr->first);

                    if (achievers.size() > 1)
                    {
                        cerr << "Implementation limitation: only supports single achievers for "
                                "time-dependent-reward marker facts, so ignoring "
                             << *(RPGBuilder::getLiteral(fItr->first)) << endl;
                        continue;
                    }
                    ...
                    for (; accItr != accEnd; ++accItr)
                    {
                        ...
                        const bool includeThisAction = ((timeDependentStartCosts[accItr->first] &&
                                                         !timeDependentStartCosts[accItr->first]->empty()) ||
                                                        (timeDependentEndCosts[accItr->first] &&
                                                         !timeDependentEndCosts[accItr->first]->empty()));
                        ...
                        for (int pass = 0; pass < 2; ++pass)
                        {
                            const list<Literal *> &preList =
                                (!pass ?
                                     RPGBuilder::getProcessedStartPropositionalPreconditions()[accItr
                                                                                                   ->first] :
                            ...
                            for (; pItr != pEnd; ++pItr)
                            {
                                ...
                                {  // skip non-addable facts, as we don't need to look for support
            ...
            timeDependentRewardFacts.insert(timeDependentRewardFacts.end(),
                                            factsSupportingIdentifiedTDRs.begin(),
                                            factsSupportingIdentifiedTDRs.end());
    ...
    double NumericAnalysis::bestAdditionalRewardFromHere(const MinimalState &theState)
    {
        ...
        for (int gID = 0; gItr != gEnd; ++gItr, ++gID)
        {
            ...
            {
                const list<Literal *> &preList =
                    (!pass ? RPGBuilder::getProcessedStartPropositionalPreconditions()[accItr->first] :
                             RPGBuilder::getEndPropositionalPreconditions()[accItr->second]);
                ...
                for (; pItr != pEnd; ++pItr)
                {
                    fID = (*pItr)->getStateID();
                    if (RPGBuilder::getEffectsToActions(fID).empty() &&
                        theState.first.find(fID) == theState.first.end())
                    {
                        beenRewarded = true;
                        break;
                    }
                    ...
                    if (indexed != factToTDRArrayIndex.end())
                    {
                        if (localDebug)
                        {
                            cout << "- Must come after " << *(RPGBuilder::getLiteral(indexed->first))
                                 << " at " << theState.lowerBoundOnTimeDependentRewardFacts[indexed->second]
                                 << endl;
                        }
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                const list<CostAtTime *> *const use =
                    (!pass ? timeDependentStartCosts[accItr->first] : timeDependentEndCosts[accItr->first]);
                ...
                if (use)
                {
                    ...
                    for (; costItr != costEnd; ++costItr)
                    {
                        ...
                        {
                            list<EffectDeterminedByTime *>::const_iterator limItr =
                                (*costItr)->limitCosts.begin();
                            const list<EffectDeterminedByTime *>::const_iterator limEnd =
                            ...
                            for (; limItr != limEnd; ++limItr)
                            {
                                if (goalNumericUsageLimits[(*limItr)->y].optimisationLimit)
                                {
                                    if (localDebug)
                                    {
                                        cout << "Reward in window [" << (*costItr)->start << ","
                                             << (*costItr)->end
                                             << "] = " << ((*limItr)->m * currTS.toDouble()) + (*limItr)->c
                                             << endl;
                                    }
                                    toReturn +=
                                        ((*limItr)->m * currTS.toDouble()) + (*limItr)->c;  // y = mx + c
                                }
                            }
                        }
                    }
    ...
    void NumericAnalysis::findVariableBounds()
    {
...

        {
            ...
            for (int actID = 0; actID < actCount; ++actID)
            {
                if (RPGBuilder::rogueActions[actID] == RPGBuilder::OT_INVALID_ACTION)
                {
                    continue;
                }
                if (!ctsEffs[actID])
...

        {
            ...
            for (int act = 0; act < actCount; ++act)
            {
                ...
                for (; uItr != uEnd; ++uItr)
                {
                    numericEffectsThatAreInConditionalEffects[uItr->first].push_back(
                        make_pair(act, uItr->second));
                }
            }
        }
        ...
        while (!needToVisit.empty())
        {
            ...
            for (; vItr != vEnd; ++vItr)
            {
                ...
                if (!definedYet[*vItr] || (variableBounds[*vItr].first != -DBL_MAX) ||
                    (variableBounds[*vItr].second != DBL_MAX))
                {
                    ...
                    for (int eff = 0; eff < effCount; ++eff)
                    {
                        ...
                        if (definedYet[*vItr] || numericEffs[eff].isAssignment)
                        {
                            if (numericEffs[eff].size)
                            {
                                if (localDebug)
                                {
                                    cout << "Looking at actions with multi-variable effect "
                                         << numericEffs[eff] << endl;
                                }

                                for (int condPass = 0; condPass < 2; ++condPass)
                                {
                                    if (condPass == 1 &&
                                        numericEffectsThatAreInConditionalEffects.find(eff) ==
                                            numericEffectsThatAreInConditionalEffects.end())
                                    {
                                        continue;
                                    }

                                    const list<pair<int, Planner::time_spec>> &acts =
                                        (condPass ?
                                             numericEffectsThatAreInConditionalEffects.find(eff)->second :
                                             RPGBuilder::getRpgNumericEffectsToActions()[eff]);

                                    list<pair<int, Planner::time_spec>>::const_iterator actItr = acts.begin();
                                    const list<pair<int, Planner::time_spec>>::const_iterator actEnd =
                                        acts.end();
                                    ...
                                    for (; actItr != actEnd; ++actItr)
                                    {
                                        ...
                                        {
                                            cout << "  Found on "
                                                 << *(RPGBuilder::getInstantiatedOp(actItr->first)) << ", ";
                                            if (actItr->second == Planner::E_AT_START)
                                            {
                                                cout << "start\n";
                                            }
                                        ...
                                        const list<int> &actPres =
                                            (actItr->second == Planner::E_AT_END ?
                                                 RPGBuilder::getEndPreNumerics()[actItr->first] :
                                                 RPGBuilder::getStartPreNumerics()[actItr->first]);
                                        ...
                                        {
                                            bool thisPreCouldBeBuilt = false;
                                            VarOpConst currPre(RPGBuilder::getNumericPreTable()[*apItr],
                                                               thisPreCouldBeBuilt);

                                            if (thisPreCouldBeBuilt)
                                            {
                                                if (currPre.op == VAL::E_GREATER ||
                                                    currPre.op == VAL::E_GREATEQ)
                                                {
                                                    localBounds.tightenLower(currPre.var, currPre.constant);
                                                    if (localDebug)
                                                    {
                                                        cout << "    Locally, the lower bound on "
                                                             << *(RPGBuilder::getPNE(currPre.var)) << " is "
                                                             << currPre.constant << endl;
                                                    }
                                                }
                                                else
                                                {
                                                    ...
                                                    if (localDebug)
                                                    {
                                                        cout << "    Locally, the upper bound on "
                                                             << *(RPGBuilder::getPNE(currPre.var)) << " is "
                                                             << currPre.constant << endl;
                                                    }
                                                }
                                        ...
                                        localBounds.tightenLower(
                                            -3, RPGBuilder::getOpMinDuration(actItr->first, -1));
                                        localBounds.tightenUpper(
                                            -3, RPGBuilder::getOpMaxDuration(actItr->first, -1));

                                        for (int pass = 0; pass < 2; ++pass)
                                        {
                                            double localBoundToUpdate = numericEffs[eff].constant;

                                            for (int term = 0; (localBoundToUpdate != DBL_MAX &&
                                                                localBoundToUpdate != -DBL_MAX) &&
                                                               term < numericEffs[eff].size;
                                                 ++term)
                                            {
                                                ...
                                                switch (numericEffs[eff].variables[term])
                                                {
                                                    case -3:
                                                    {
                                                        ...
                                                        relevantBound = numericEffs[eff].weights[term] *
                                                                        (pass ? localBounds[-3].second :
                                                                                localBounds[-3].first);
                                                        break;
                                                    }
                                                    case -19:
                                                    {
                                                        ...
                                                        relevantBound = numericEffs[eff].weights[term] *
                                                                        (pass ? -localBounds[-3].first :
                                                                                -localBounds[-3].second);
                                                        break;
                                                    }
                                                    default:
                                                    {
                                                        ...
                                                        {
                                                            relevantBound =
                                                                -numericEffs[eff].weights[term] *
                                                                (pass ? localBounds[numericEffs[eff]
                                                                                        .variables[term] -
                                                                                    pneCount]
                                                                            .first :
                                                                        localBounds[numericEffs[eff]
                                                                                        .variables[term] -
                                                                                    pneCount]
                                                                            .second);
                                                        }
                                                ...
                                                if (localDebug)
                                                {
                                                    cout << "   Contribution from term " << term << " = "
                                                         << relevantBound << endl;
                                                }
                                            ...
                                            if (pass)
                                            {
                                                if (localBoundToUpdate > rhsUB)
                                                {
                                                    ...
                                                    if (localDebug)
                                                    {
                                                        cout << "  UB of effect outcome pushed up to "
                                                             << rhsUB << endl;
                                                    }
                                                }
                                                else
                                            ...
                                            else
                                            {
                                                if (localBoundToUpdate < rhsLB)
                                                {
                                                    ...
                                                    if (localDebug)
                                                    {
                                                        cout << "  LB of effect outcome pushed down to "
                                                             << rhsLB << endl;
                                                    }
                                                }
                                                else
                        ...
                        if (numericEffs[eff].isAssignment)
                        {
                            if (definedYet[*vItr])
                            {
                                if (rhsUB > variableBounds[*vItr].second)
                                {
                                    ...
                                    if (defDebug)
                                    {
                                        cout << numericEffs[eff] << " increases UB on "
                                             << *(RPGBuilder::getPNE(*vItr)) << " to " << rhsUB << "]\n";
                                    }
                                }
                                else
                                {
                                ...
                                if (rhsLB < variableBounds[*vItr].first)
                                {
                                    ...
                                    if (defDebug)
                                    {
                                        cout << numericEffs[eff] << " decreases LB on "
                                             << *(RPGBuilder::getPNE(*vItr)) << " to " << rhsLB << "]\n";
                                    }
                                }
                                else
                                {
                            ...
                            else
                            {
                                if (defDebug)
                                {
                                    cout << numericEffs[eff] << " is the first assigner to "
                                         << *(RPGBuilder::getPNE(*vItr)) << ": [" << rhsLB << "," << rhsUB
                                         << "]\n";
                                }
                                variableBounds[*vItr].second = rhsUB;
                                variableBounds[*vItr].first = rhsLB;
                            ...
                            ((variableBounds[*vItr].second == DBL_MAX) && !previousEffectsLeftUBUnbounded))
                        {
                            for (int term = 0; term < numericEffs[eff].size; ++term)
                            {
                                ...
                                if (numericEffs[eff].variables[term] >= pneCount)
                                {
                                    revisitIfFiniteBounded[numericEffs[eff].variables[term] - pneCount]
                                        .insert(*vItr);
                                    if (localDebug)
                                    {
                                        cout << "- If "
                                             << *(RPGBuilder::getPNE(numericEffs[eff].variables[term] -
                                                                     pneCount))
                                             << " ever becomes finite bounded, revisit this\n";
                                    }
                                }
                                else
                                {
                        ...
                        previousEffectsLeftLBUnbounded =
                            (previousEffectsLeftLBUnbounded || (variableBounds[*vItr].first == -DBL_MAX));
                        previousEffectsLeftUBUnbounded =
                            (previousEffectsLeftUBUnbounded || (variableBounds[*vItr].second == DBL_MAX));
                ...
                if ((lbPreviouslyUnbounded && (variableBounds[*vItr].first != -DBL_MAX)) ||
                    (ubPreviouslyUnbounded && (variableBounds[*vItr].second != DBL_MAX)))
                {
        ...
        for (int v = 0; v < pneCount; ++v)
        {
            ...
            {
                cout << *(RPGBuilder::getPNE(v)) << " has finite bounds: [" << variableBounds[v].first << ","
                     << variableBounds[v].second << "]\n";
            }
            else if (variableBounds[v].first != -DBL_MAX)
            {
                cout << *(RPGBuilder::getPNE(v)) << " has a finite lower bound: [" << variableBounds[v].first
                     << ",inf]\n";
            }
            else if (variableBounds[v].second != DBL_MAX)
            {
                cout << *(RPGBuilder::getPNE(v)) << " has a finite upper bound: [-inf,"
                     << variableBounds[v].second << "]\n";
            }
        }
    }
    ...
    void NumericAnalysis::findFactsThatDefineAndFixVariables()
    {
        ...
        {
            const map<int, list<RPGBuilder::IntegralContinuousEffect>> &ice =
                RPGBuilder::getActionsToIntegralConditionalEffects();
            ...
            for (; iceMapItr != iceMapEnd; ++iceMapItr)
            {
                ...
                const list<RPGBuilder::IntegralContinuousEffect>::const_iterator icEnd =
                    iceMapItr->second.end();

                for (; icItr != icEnd; ++icItr)
                {
                    {
                        list<pair<int, double>>::const_iterator effItr = icItr->getLeftStartEffects().begin();
                        const list<pair<int, double>>::const_iterator effEnd =
                            icItr->getLeftStartEffects().end();

                        for (; effItr != effEnd; ++effItr)
                        {
                            iceLookup[effItr->first].push_back(
                                make_pair(iceMapItr->first, Planner::E_AT_START));
                        }
                    }

                    {
                        ...
                        const list<pair<int, double>>::const_iterator effEnd =
                            icItr->getLeftEndEffects().end();

                        for (; effItr != effEnd; ++effItr)
                        {
                            iceLookup[effItr->first].push_back(
                                make_pair(iceMapItr->first, Planner::E_AT_END));
                        }
                    }
                }
            }
        ...
        for (int effID = 0; effID < effCount; ++effID)
        {
            ...
            for (int iceOrNot = 0; iceOrNot < 2; ++iceOrNot)
            {
                const list<pair<int, Planner::time_spec>> &effList =
                    (iceOrNot ? iceLookup[effID] : RPGBuilder::getRpgNumericEffectsToActions()[effID]);
                ...
                for (; (first || !requiredThenDeletedWithThisEffect.empty()) && actItr != actEnd;
                     ++actItr, first = false)
                {
                    set<int> required;
                    {
                        const list<Literal *> &pres =
                            (actItr->second == Planner::E_AT_START ?
                                 RPGBuilder::getStartPropositionalPreconditions()[actItr->first] :
                                 RPGBuilder::getEndPropositionalPreconditions()[actItr->first]);
                    ...
                    }
                    {
                        const list<Literal *> &dels =
                            (actItr->second == Planner::E_AT_START ?
                                 RPGBuilder::getStartPropositionDeletes()[actItr->first] :
                                 RPGBuilder::getEndPropositionDeletes()[actItr->first]);
                        set<int> delSet;
                        ...
                        set<int> iSect;
                        std::set_intersection(required.begin(), required.end(), delSet.begin(), delSet.end(),
                                              std::insert_iterator<set<int>>(iSect, iSect.end()));
                        iSect.swap(required);
                    }
                    if (required.empty())
                    {
                    ...
                    else
                    {
                        ...
                        std::set_intersection(required.begin(), required.end(),
                                              requiredThenDeletedWithThisEffect.begin(),
                                              requiredThenDeletedWithThisEffect.end(),
                                              std::insert_iterator<set<int>>(iSect, iSect.end()));
...

                    {
                        const list<Literal *> &adds =
                            (actItr->second == Planner::E_AT_START ?
                                 RPGBuilder::getStartPropositionAdds()[actItr->first] :
                                 RPGBuilder::getEndPropositionAdds()[actItr->first]);
                    ...
                    {
                        set<int> iSect;
                        std::set_intersection(addSet.begin(), addSet.end(),
                                              definitelyAddedWithThisEffect.begin(),
                                              definitelyAddedWithThisEffect.end(),
                                              std::insert_iterator<set<int>>(iSect, iSect.end()));
            ...
            pair<map<int, pair<set<int>, set<int>>>::iterator, bool> beforeItr =
                requiredThenDeletedToGetValue.insert(
                    make_pair(currEff.fluentIndex,
                              make_pair(requiredThenDeletedWithThisEffect, definitelyAddedWithThisEffect)));
            ...
            if (!beforeItr.second)
            {
                ...
                std::set_intersection(
                    beforeItr.first->second.first.begin(), beforeItr.first->second.first.end(),
                    requiredThenDeletedWithThisEffect.begin(), requiredThenDeletedWithThisEffect.end(),
                    std::insert_iterator<set<int>>(iSect, iSect.end()));
                ...
                else
                {
                    ...
                    set<int> iSect2;
                    std::set_intersection(
                        beforeItr.first->second.second.begin(), beforeItr.first->second.second.end(),
                        definitelyAddedWithThisEffect.begin(), definitelyAddedWithThisEffect.end(),
                        std::insert_iterator<set<int>>(iSect2, iSect2.end()));
        ...
        for (; reqItr != reqEnd; ++reqItr)
        {
            ...
            {
                cout << *(RPGBuilder::getPNE(reqItr->first)) << " fixed and defined by at least "
                     << *(RPGBuilder::getLiteral(*(reqItr->second.second.begin()))) << endl;
    ...
    void NumericAnalysis::findOrphanedNumericEffects()
    {
        ...
        const map<int, list<RPGBuilder::IntegralContinuousEffect>> &actionsToIntegralConditionalEffects =
            RPGBuilder::getActionsToIntegralConditionalEffects();

        {
            map<int, list<RPGBuilder::IntegralContinuousEffect>>::const_iterator iceItr =
                actionsToIntegralConditionalEffects.begin();
            const map<int, list<RPGBuilder::IntegralContinuousEffect>>::const_iterator iceEnd =
                actionsToIntegralConditionalEffects.end();
            ...
            for (; iceItr != iceEnd; ++iceItr)
            {
                ...
                for (; icItr != icEnd; ++icItr)
                {
                    ...
                    for (; effItr != effEnd; ++effItr)
                    {
                        ...
                        {
                            cout << "Conditional effect on "
                                 << *(RPGBuilder::getInstantiatedOp(iceItr->first)) << " means we keep "
                                 << effTable[*effItr] << endl;
                        }
        ...
        const vector<list<RPGBuilder::ConditionalEffect>> &actionsToConditionalEffects =
            RPGBuilder::getActionsToConditionalEffects();

        {
            ...
            for (int actID = 0; actID < actCount; ++actID)
            {
                ...
                list<RPGBuilder::ConditionalEffect>::const_iterator ceItr =
                    actionsToConditionalEffects[actID].begin();
                const list<RPGBuilder::ConditionalEffect>::const_iterator ceEnd =
                    actionsToConditionalEffects[actID].end();

                for (; ceItr != ceEnd; ++ceItr)
                {
                    list<pair<int, Planner::time_spec>>::const_iterator effItr =
                        ceItr->getNumericEffects().begin();
                    const list<pair<int, Planner::time_spec>>::const_iterator effEnd =
                        ceItr->getNumericEffects().end();

                    for (; effItr != effEnd; ++effItr)
                    {
                        if (!effTable[effItr->first].canEverHappen)
                        {
                            cout << "Conditional effect on " << *(RPGBuilder::getInstantiatedOp(actID))
                                 << " means we keep " << effTable[effItr->first] << endl;
                        }
    ...
    void NumericAnalysis::findEndEffectsSafeToMoveToTheStart()
    {
        ...
        for (int eff = 0; eff < effCount; ++eff)
        {
            if (factsThatDefineAndFixVariables.find(effTable[eff].fluentIndex) ==
                factsThatDefineAndFixVariables.end())
            {
                continue;
            }
