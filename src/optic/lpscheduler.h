...
namespace Planner
{
    ...
    /** @brief  Class to build an LP to evaluate a plan, and find bounds on the variables in the state
     * reached. */
    class LPScheduler
    {
    public:
    ...
    protected:
        ...
        /** @brief A class to track the changes on a task numeric variable as the LP is built for the plan so
         * far.
         */
        ...
        public:
             ...
             *    <code>FluentTracking::orderIndependentValueTerms</code> and
             * <code>FluentTracking::orderIndependentValueConstant</code>, for eventual inclusion into the
             * objective function.
             */
            enum fluent_status
            {
                FS_NORMAL = 0,
                FS_IGNORE = 1,
                FS_ORDER_INDEPENDENT = 2
            };

            /** @brief The status of this fluent.
             *
             *  For metric-tracking variables, this is set to either <code>FS_IGNORE</code>
             *  or <code>FS_ORDER_INDEPENDENT</code> in the <code>LPScheduler</code> constructor, depending on
             * whether the plan is to be scheduled to the metric.  For all other variables, this is set to
             * <code>FS_NORMAL</code>.
             */
            ...
            /** @brief The LP variable (column) containing the value of the variable following the last effect
             * to act upon it.
             *
            ...
            /** @brief Constant term used in calculating the value of an order-independent metric-tracking
             * variable.
             *
            ...
            /** @brief Column indices after which 'now' must follow. */
            set<int> nowMustFollowCols;
            ...
            FluentTracking(const double &initial)
              : statusOfThisFluent(FS_NORMAL)
              , postLastEffectValue(initial)
              , lastEffectValueVariable(-1)
              , lastEffectTimestampVariable(-1)
              , activeGradient(0.0)
              , activeGradientCount(0)
              , everHadADurationDependentEffect(false)
             ...
             */
            FluentTracking()
            {  // unfortunately needed for vector<FluentTracking>
            }
        };

        class ValueOfVariablesAtSomeTime
        {
        private:
            map<int, pair<int, double>> variableToColumnOrValue;

        public:
            typedef map<int, pair<int, double>>::const_iterator const_iterator;
            typedef map<int, pair<int, double>>::iterator iterator;

            ValueOfVariablesAtSomeTime()
            {
            }

            void addLastRecordAsIs(const int &v, const FluentTracking &currT)
            {
                variableToColumnOrValue.insert(
                    make_pair(v, make_pair(currT.lastEffectValueVariable, currT.postLastEffectValue)));
            }

            const_iterator begin() const
            {
                return variableToColumnOrValue.begin();
            }

            const_iterator end() const
            {
                return variableToColumnOrValue.end();
            }

            const_iterator find(const int &v) const
            {
                return variableToColumnOrValue.find(v);
            }

            void variableValueIsInColumn(const int &v, const int &col)
            {
                variableToColumnOrValue[v] = make_pair(col, std::numeric_limits<double>::signaling_NaN());
            }

            void variableValueIsConstant(const int &v, const double &value)
            {
                variableToColumnOrValue[v] = make_pair(-1, value);
            }

            void copyVRecordFrom(const int &v, const ValueOfVariablesAtSomeTime &other)
            {
                const const_iterator itr = other.find(v);
                assert(itr != other.end());
                variableToColumnOrValue[v] = itr->second;
            }
        };
        ...
        double timestampToUpdateVarLB;
        map<int, double> timestampToUpdateVarEndLB;
        ...
        /** @brief New dummy steps, whose timestamps additionally need to be minimised.
         *
         *  Map keys are step IDs.  Values pair a timestamp variable with its minimised value.
         */
        map<int, pair<int, double>> newDummySteps;
        ...
        struct EndDetails
        {
            ...
            EndDetails(const int &min, const int &max, const int &cons)
              : imaginaryMin(min), imaginaryMax(max), lastToMin(cons){};
            EndDetails(){};
        };
         ...
         *  - <code>instantEffects[a][0]</code> contains the instantaneous numeric effects for the start of
         * action <code>a</code>
         *  - <code>instantEffects[a][1]</code> contains the instantaneous numeric effects for the end of
         * action <code>a</code>
         */
        static vector<vector<list<RPGBuilder::RPGNumericEffect *>>> instantEffects;

        /** @brief The instantaneous integral-conditional-effect-encoding numeric effects of each action.
         *
         * Each value of the map is itself a vector:
         *  - <code>instantEffects[a][0]</code> contains the instantaneous numeric effects for the start of
         * action <code>a</code>
         *  - <code>instantEffects[a][1]</code> contains the instantaneous numeric effects for the end of
         * action <code>a</code>
         */
        static map<int, list<vector<list<RPGBuilder::RPGNumericEffect *>>>> instantIntegralConditionalEffects;

        /** @brief The preconditions of each action.
         *
         * Each entry is itself a vector:
         *  - <code>constraints[a][0]</code> contains the conditions that must hold at the start of action
         * <code>a</code>
         *  - <code>constraints[a][1]</code> contains the conditions that must hold throughout the execution
         * of action <code>a</code>
         *  - <code>constraints[a][2]</code> contains the conditions that must hold at the end of action
         * <code>a</code>
         */
        static vector<vector<list<const Constraint *>>> constraints;

        /** @brief The left- and right-most instantaneous integral-conditional-effect-encoding preconditions
         * of each action.
         *
         * Each value of the map is a vector: the left-most and right-most constraints that must apply at that
         * time.
         */
        static map<int, list<vector<pair<const Constraint *, const Constraint *>>>>
            instantIntegralConstraints;

        /** @brief The numeric variables relevant to each point of each action.
         *
         * Each entry is a vector of size 3, where:
         *  - <code>interesting[a][0]</code> defines the variables relevant to the start of action
         * <code>a</code>
         *  - <code>interesting[a][1]</code> defines the variables relevant to the invariants of action
         * <code>a</code>
         *  - <code>interesting[a][2]</code> defines the variables relevant to the end of action
         * <code>a</code>
         *
         *  In all cases, if the boolean value paired with a variable is <code>true</code>, the action will
         * have an effect on the variable.  Otherwise, it only requires to inspect its value (for a
         * precondition).
         */
         ...
         *
         *  If an action has a conditional effect on a metric-tracking variable, where that condition depends
         * on a fact controlled by optimisation TILs (TILs relevant only to such conditional effects), then
         *  the mutexes between the action's start/end and the TILs must be respected.  Thus, for each
         ...
         *    with any TIL deleting <code>(fact)</code>
         *
         *  This vector contains the list of incompatible time points for each action, according to the
         * optimisation TILs.  Each entry is a vector of size 2, where:
         *  - <code>pointsThatWouldBeMutexWithOptimisationTILs[a][0]</code> contains the time points mutex
         * with the start of action <code>a</code>
         *  - <code>pointsThatWouldBeMutexWithOptimisationTILs[a][1]</code> contains the time points mutex
         * with the end of action <code>a</code>
         */
         ...
         *    - <code>boringAct[a][0].second</code> is <code>false</code> if the start of the action
         * introduces time-dependent change, in the case where metric tracking variables are included.
        ...
        /** @brief The facts in the initial state */
        static LiteralSet initialFacts;

        /** @brief The gradients on each variable due to processes active from the initial state */
        static vector<double> initialGradients;
        ...
        int generateEndDetails(const Planner::time_spec &currTS, const int &actID, const int &stepID,
                               FFEvent &currEvent, const vector<FFEvent *> &planAsAVector,
                               int &nextImaginaryEndVar, vector<EndDetails> &imaginaryMinMax);

        void addConstraintsToGetValuesOfVariablesNow(InterestingMap &currInterest, const int &stepID,
                                                     const int &currVar,
                                                     ValueOfVariablesAtSomeTime &beforeStep);

        static void collateRelevantVariablesAndInvariants(
         ...
         *  @param invariantsOnVariableI  Where the output from the function: for each variable index, the
         * constraints that depend on it.
         */
        static void recordVariablesInvolvedInThisStepsInvariants(
        ...
        /** @brief Add constraints to the LP to determine whether a list of conditional effect conditions are
         * satisfied.
         *
         ...
         *    - the second element, a <code>Planner::time_spec</code>, dictates when it has to be satisfied:
         *      <code>VAL::E_AT_START</code>, <code>VAL::E_OVER_ALL</code>, or <code>VAL::E_AT_END</code>.
         *  @param actStartAt     The LP variable denoting the start of the action from which the conditions
         * are taken
         *  @param actEndAt       The LP variable denoting the end of the action from which the conditions are
         * taken (or <code>-1</code> if the action is non-temporal)
         *  @param conditionVars  The LP columns created (one for each entry in
         * <code>propositionConditions</code>) are added to this list.
         *
         *  @return  <code>true</code> if the conditions can possibly be satisfied, <code>false</code>
         * otherwise.
         */
        bool addAnyNumericConstraints(const list<pair<int, Planner::time_spec>> &numericConditions,
                                      const int &actStartAt, const int &actEndAt, list<int> &conditionVars);

        /** @brief Add constraints to the LP to determine whether a list of conditional effect conditions are
         * satisfied by the TILs.
         *
         ...
         */
        bool
        addAnyTimeWindowConstraints(const list<pair<Literal *, Planner::time_spec>> &propositionalConditions,
                                    const int &actStartAt, const int &actEndAt, list<int> &conditionVars);
         ...
         * For temporal preferences, we may also need to re-check if they are still satisfied.
         *
         * @param[in] csEndLowerBound  If just started a compression-safe action: the min timestamp of its end
         * @param[in] rpLowerBounds  Any extra lower bounds known from the RPG heuristic
         *
         * @see timestampToUpdate
         */
        void pushTimestampToMin(vector<AutomatonPosition::Position> &preferenceStatus,
        ...
        static map<int, vector<pair<double, double>>> boundsForAbstractFactOccurrences;

        class AbstractFactConstraintBlock
        {
        protected:
            int factID;
            const bool onlyOneUserPerWindow;
            MILPSolver *lp;
            int lpOffsetToPlanStepVariables;

            int recordBoundsAtPlanStepID;
            int varControllingTheLatestPointAtWhichAnOccurenceMustBeUsed;
            vector<int> constraintsForOneUserPerOccurrence;
            vector<int> outcomeWasUsed;
            bool noLongerAvailable;

            const vector<pair<double, double>> &occurrenceBounds;

        public:
            AbstractFactConstraintBlock(const int &fID, MILPSolver *lpIn, const int &numVars,
                                        const vector<pair<double, double>> &bounds)
              : factID(fID)
              , onlyOneUserPerWindow(
                    TemporalAnalysis::timelineOnTILFact(fID)->isOnlyEverReferredToThenDeleted())
              , lp(lpIn)
              , lpOffsetToPlanStepVariables(numVars)
              , recordBoundsAtPlanStepID(-1)
              , varControllingTheLatestPointAtWhichAnOccurenceMustBeUsed(-1)
              , noLongerAvailable(false)
              , occurrenceBounds(bounds)
            {
            }

            void setPlanStepBoundsAreDefinedAt(const int &stepID, const bool &nla)
            {
                assert(recordBoundsAtPlanStepID == -1);
                recordBoundsAtPlanStepID = stepID;
                noLongerAvailable = nla;
            }

            void addUser(const int &stepID);
            void addOverAllUser(const int &startStepID, const int &endStepID, const double &startGap,
                                const double &endGap);

            const vector<int> *needToReviseBounds() const;

            const vector<pair<double, double>> &getOccurrenceBounds() const
            {
                return occurrenceBounds;
            }

            const int &getBoundsStep() const
            {
                return recordBoundsAtPlanStepID;
            }

            const bool &isNoLongerAvailable() const
            {
                return noLongerAvailable;
            }

            void markAsNoLongerAvailable()
            {
                noLongerAvailable = true;
            }
        };

        /** @brief  The plan step recording the minimum timestamp at which the given abstract fact is
         * available. */
        map<int, AbstractFactConstraintBlock> planStepForAbstractFact;

        void recalculateAbstractFactBounds();

        bool planCostsWereCalculated;
        bool planCostsNeededAnLP;
        bool reachableCostEqualsCurrentCost;

        vector<double> timestampsThatMetCurrentCost;
        double currentPlanCost;
        double currentChoosablePlanCost;

        double reachablePlanCost;
        double unavoidableReachablePlanCost;

        /** @brief Variable representing makespan.  Do not access this directly, use
         * <code>getDummyVarForMakespan</code>. */
        int dummyVarForMakespan;

        /** @brief Add a column to the LP constrained to come after all non-dummy plan steps. */
        int getDummyVarForMakespan();

        struct VarsForPreference
        {
            int reachableCostVar;
            int committedCostVar;
            int committedRow;

            VarsForPreference(const int &a) : reachableCostVar(a), committedCostVar(a), committedRow(-1)
            {
            }

            VarsForPreference(const int &reach, const int &commit)
              : reachableCostVar(reach), committedCostVar(commit), committedRow(-1)
            {
            }
        };

        map<int, VarsForPreference> varsForPreference;
        map<int, list<int>> varsForPreconditionPreference;

        /** @brief A variable to calculate the reachable plan cost.  Do not access directly: use
         * <code>getVarForReachableCost()</code> */
        int varForReachableCost;
        int getVarForReachableCost(vector<AutomatonPosition::Position> &preferenceStatus,
                                   const double &preconditionPrefViolations);

        void sortOutTheSolution(const bool &setObjectiveToMetric, const int &justAppliedStep,
                                vector<AutomatonPosition::Position> &preferenceStatus,
                                const double &preconditionPrefViolations,
                                const map<int, double> &rpLowerBounds);

    public:
        LPScheduler(const MinimalState &s, vector<AutomatonPosition::Position> &preferenceStatus,
                    list<FFEvent> &header, list<FFEvent> &now, const int &justAppliedStep,
                    list<StartEvent> &seq, ParentData *parentData,
                    map<int, list<list<StartEvent>::iterator>> &compulsaryEnds,
                    const vector<double> *prevStateMin, const vector<double> *prevStateMax,
         ...
         *          or preconditions.
         */
        inline static bool isBoring(const int &a, const int &s, const bool &includeMetric)
        {
            if (includeMetric)
            {
        ...
        void removeExpiredAbstractFacts(StateFacts &facts);
         ...
         *  @param  timeAtWhichValueIsDefined   If
         * <code>LPScheduler::workOutFactLayerZeroBoundsStraightAfterRecentAction</code> is <code>true</code>,
         * then for variables with a simple gradient acting upon them (nothing sophisticated involving
         * duration-dependent effects) this is updated to contain the time at which the given bounds held;
         * after which the variable changes according to the gradient in effect upon it. Entries of
         * <code>-1.0</code> correspond to variables with no gradient effects.
         */
         ...
         *
         *  If <code>LPScheduler::workOutFactLayerZeroBoundsStraightAfterRecentAction</code> is
         * <code>true</code>, then this function is used to extrapolate the state variable bounds were time
         * allowed to elapse.  These bounds are needed to allow the successor pruning in action applicability
         * testing to remain sound.
         *
         *  @see LPScheduler::workOutFactLayerZeroBoundsStraightAfterRecentAction
         *
         ...
         *  @param  max  The vector containing the upper bounds to be updated
         */
        void extrapolateBoundsAfterRecentAction(const list<StartEvent> *startEventQueue, vector<double> &min,
                                                vector<double> &max,
                                                const vector<double> &timeAtWhichValueIsDefined);

        /** @brief Update the LP based on the relaxed plan. */
        bool addRelaxedPlan(const list<StartEvent> &seq,
                            vector<AutomatonPosition::Position> &preferenceStatus,
                            double &preconditionPrefViolations, list<FFEvent> &header, list<FFEvent> &now,
        ...
        const bool &werePlanCostsCalculated() const
        {
            return planCostsWereCalculated;
        }

        const double &getCurrentPlanCost() const
        {
            return currentPlanCost;
        }

        const double &getReachablePlanCost() const
        {
            return reachablePlanCost;
        }
         ...
         *  @param open    The list of actions that have started, but not yet finished, following the supplied
         * plan.
         ...
         */
        static ParentData *prime(list<FFEvent> &header, const TemporalConstraints *const cons,
        ...
        /** @brief  If <code>true</code>, variable bounds are computed for moment immediately after the recent
         * action applied. */
    ...
    public:
        ...
        LPQueueSet(const int &i)
          : arrSize(i + 1)
          , qSet(new bool[arrSize])
          , UB(new bool[arrSize])
          , LB(new bool[arrSize])
          , UBP(new bool[arrSize])
          , LBP(new bool[arrSize])
          , NEW(new int[arrSize])
        {
            ++qSet;
            ++UB;
    ...
    {
    protected:
        map<int, pair<bool, double>> mustFollowThisD;
        map<int, pair<bool, double>> mustPrecedeThisD;

    public:
        const map<int, pair<bool, double>> &mustFollowThis() const
        {
            return mustFollowThisD;
        }

        const map<int, pair<bool, double>> &mustPrecedeThis() const
        {
            return mustPrecedeThisD;
        }

        bool addFollower(const int &a, const bool &b, const double &d)
        {
            if (Globals::globalVerbosity & 4096)
            {
                ...
                else
                {
                    if (d == 0.0)
                    {
                        cout << "Insisting that " << a << " is at least at or after this step\n";
                    }
                    else if (d > 0.0)
                    {
                        cout << "Insisting that " << a << " is at least " << d << " after this step\n";
                    }
                    else if (d < 0.0)
                    {
                        cout << "Insisting that " << a << " is no more than " << -d << " before this step\n";
                    }
                }
            }
            pair<bool, double> &orWith = mustFollowThisD.insert(make_pair(a, make_pair(b, d))).first->second;

            if (b)
            {
                if (orWith.second <= 0.0)
                {
                    assert(d == EPSILON);
                    orWith.first = true;
                    orWith.second = EPSILON;
                }
            }
            else
            {
        ...
        void initialisePredecessors(const map<int, pair<bool, double>> &p)
        {
            assert(mustPrecedeThisD.empty());
            mustPrecedeThisD = p;
        }

        bool addPredecessor(const int &a, const bool &b, const double &d)
        {
            if (Globals::globalVerbosity & 4096)
            {
                if (b)
                ...
                else
                {
                    if (d == 0.0)
                    {
                        cout << "Insisting that " << a << " is at least at or before this step\n";
                    }
                    else if (d > 0.0)
                    {
                        cout << "Insisting that " << a << " is at least " << d << " before this step\n";
                    }
                    else if (d < 0.0)
                    {
                        cout << "Insisting that " << a << " is no more than " << -d << " after this step\n";
                    }
                }
            }
            pair<bool, double> &orWith = mustPrecedeThisD.insert(make_pair(a, make_pair(b, d))).first->second;
    ...
    public:
        ParentData(const int &qSize, list<FFEvent> *h, const int &nt)
          ...
          , postStartGap(-1)
          , postEndGap(-1)
          , distFromZero(qSize, DBL_MAX)
          , distToZero(qSize, 0.0)
          , pairWith(qSize, -1)
          , eventsWithFakes(qSize, (FFEvent *)0)
          , softEdges(qSize, -1)
          , parentPlan(h)
          , nextTIL(nt){};
        ...
        const map<int, IncomingAndOutgoing> &getSoftEdges() const
        {
            return temporarySoftEdges;
        };
        ...
        inline const int &edgesAreNonSoft(const int &i)
        {
            return softEdges[i];
        }
        ...
        inline void postStartGapIsStep(const int &i)
        {
            postStartGap = i;
        };

        inline void postEndGapIsStep(const int &i)
        {
            postEndGap = i;
        };
        ...
        const int &whereIsPostStartGap() const
        {
            return postStartGap;
        };

        const int &whereIsPostEndGap() const
        {
            return postEndGap;
        };
         ...
         */
        ChildData *spawnChildData(list<StartEvent> &seq, list<FFEvent> &header, list<FFEvent> &succ,
                                  const bool &includeMetric, const TemporalConstraints *const cons,
                                  const int &stepID);
        ...
        void sanityCheck()
        {
            ...
            for (int i = 0; i < loopLim; ++i)
            {
                ...
                {
                    if (hItr->time_spec == Planner::E_AT && pairWith[i] != -2)
                    {
                ...
                {
                    if (eventsWithFakes[i] && eventsWithFakes[i]->time_spec == Planner::E_AT &&
                        pairWith[i] != -2)
                    {
