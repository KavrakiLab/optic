...
namespace Planner
{
    ...
    TemporalConstraints *TotalOrderTransformer::cloneTemporalConstraints(
    ...
    int
    TotalOrderTransformer::stepThatMustPrecedeUnfinishedActions(const TemporalConstraints *const cons) const
    {
    ...
    void TOTHelper_updateForInstantaneousEffects(MinimalState &theState, const unsigned int &stepID,
            ...
            for (; effItr != effEnd; ++effItr)
            {
...

                {
                    ...
                    if (stateItr != theState.firstAnnotations.end())
                    {
                        ...
                        {
                            set<int> &actionEndPoints =
                                (pass ? stateItr->second.second : stateItr->second.first);
                            ...
                            if (!actionEndPoints.empty())
                            {
                                if (applyDebug)
                                {
                                    ...
                                    {
                                        cout << "\t" << *(*effItr)
                                             << " was an invariant of some compression-safe actions, but has "
                                                "now been deleted\n";
                                    }
                                    else
                                    {
                                        cout << "\t" << *(*effItr)
                                             << " was a pending end-effect of compression-safe actions, but "
                ...
                theState.first.insert(litID);
                {
                    ...
                    if (stateItr != theState.firstAnnotations.end())
                    {
                        ...
                        if (!actionEndPoints.empty())
                        {
                            ...
                            {
                                cout << "\t" << *(*effItr)
                                     << " was a pending end-effect of compression-safe actions, but has now "
                                        "been added\n";
                            }
    ...
    void TOTHelper_meetPreconditions(MinimalState &theState, const int &stepID,
    ...
    void TOTHelper_applyNumericEffects(MinimalState &theState, const list<int> &numEffs, const double &minDur,
                                       const double &maxDur)
    {
        ...
        for (; effItr != effEnd; ++effItr)
        {
            ...
            updated.push_back(pair<int, pair<double, double>>(
                rpgEff.fluentIndex,
                rpgEff.applyEffectMinMax(theState.secondMin, theState.secondMax, minDur, maxDur)));
        }
    ...
    MinimalState *TotalOrderTransformer::applyAction(MinimalState &theStateHidden,
                                                     vector<double> &minTimestamps, const ActionSegment &a,
                                                     ...
                                                     const double &maxDur)
    {
        ...
        {  // applying a TIL
            ...
            {
                TOTHelper_updateForInstantaneousEffects(*workOn, workOn->planLength,
                                                        tilVec[workOn->nextTIL]->delEffects,
                                                        tilVec[workOn->nextTIL]->addEffects);
                ++workOn->planLength;
            }
            ...
            if (previousMostRecent != -1)
            {
                ...
                workOn->temporalConstraints->addOrdering(previousMostRecent, newMostRecent,
                                                         true);  // then impose the total ordering constraint
                if (Globals::globalVerbosity & 4096)
                {
                    cout << "TO constraint: " << previousMostRecent << " comes before " << newMostRecent
                         << std::endl;
                }
            }
        ...
        if (a.second == VAL::E_AT_START)
        {
            ...
            {  // non-temporal action
                ...
                TOTHelper_applyNumericEffects(*workOn, RPGBuilder::getStartEffNumerics()[actID], minDur,
                                              maxDur);
                ...
                if (previousMostRecent != -1)
                {
                    const int newMostRecent = workOn->temporalConstraints->getMostRecentStep();
                    workOn->temporalConstraints->addOrdering(previousMostRecent, newMostRecent,
                                                             true);  // then impose the total ordering
                                                                     // constraint
                    if (Globals::globalVerbosity & 4096)
                    {
                        cout << "TO constraint: " << previousMostRecent << " comes before " << newMostRecent
            ...
            if (skip == SAFETOSKIP)
            {
                if (applyDebug)
                {
                    cout << " * Requesting invariants for compression-safe action\n";
                }
                ...
                for (; pItr != pEnd; ++pItr)
                {
                    ...
                    workOn->firstAnnotations[(*pItr)->getStateID()].second.insert(
                        endStepID);  // associate the end of this action with an invariant on this fact
                }
                ...
                if (applyDebug)
                {
                    cout << " * Recording pending effects of a compression-safe action\n";
                }
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    ...
                    {
                        // cout << COLOUR_light_red << "Warning: compression-safe action end-adds a fact that
                        // is already true\n";
                    }
                    workOn->firstAnnotations[(*effItr)->getStateID()].first.insert(
                        endStepID);  // associate the end of this action with an effect on this fact
                    if (applyDebug)
                    {
                        cout << "   - " << *(*effItr) << endl;
                    }
            ...
            {
                {
                    if (applyDebug)
                    {
                        cout << " * Non-compression-safe action - recording invariants\n";
                    }
