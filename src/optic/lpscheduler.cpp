...
namespace Planner
{
    ...
    map<int, list<vector<list<RPGBuilder::RPGNumericEffect *>>>>
        LPScheduler::instantIntegralConditionalEffects;

    vector<vector<list<const LPScheduler::Constraint *>>> LPScheduler::constraints;
    map<int, list<vector<pair<const LPScheduler::Constraint *, const LPScheduler::Constraint *>>>>
        LPScheduler::instantIntegralConstraints;
    ...
    void nicerLPPrint(MILPSolver *lp)
    {
        ...
        {
            cout << "C" << ci << ", " << lp->getColName(ci) << ", has range [" << lp->getColLower(ci) << ","
                 << lp->getColUpper(ci) << "]\n";
        }
                     ...
                     map<int, IncomingAndOutgoing> &temporaryEdges)
    {
        ...
        if (e.implicit)
        {
            if (bfDebug)
            {
                cout << "\tImplicit edge - skipping update\n";
            }
        }
        else
        {
            if (bfDebug)
            {
                cout << "\tExplicit edge - storing\n";
            }
            if (e.to == -1)
            {  // adding a minimum timestamp back to zero
                if (bfDebug)
                {
                    cout << "\tEdge from node " << e.from << " to time zero\n";
                }
                if (e.min > events[e.from]->lpMinTimestamp)
                {
                    events[e.from]->lpMinTimestamp = e.min;
                }
                if (e.max < events[e.from]->lpMaxTimestamp)
                {
                    events[e.from]->lpMaxTimestamp = e.max;
                }
            }
            else if (e.from == -1)
            {  // adding a maximum timestamp from zero
                if (bfDebug)
                {
                    cout << "\tEdge from time zero to node " << e.to << "\n";
                }
                if (e.min > events[e.to]->lpMinTimestamp)
                {
                    ...
                    if (bfDebug)
                    {
                        cout << "\tMin timestamp of " << e.to << " updated to " << e.min << endl;
                    }
                }
                if (e.max < events[e.to]->lpMaxTimestamp)
                {
                    events[e.to]->lpMaxTimestamp = e.max;
                }
            }
            else
            {
                if (bfDebug)
                {
                    cout << "\tEdge from node " << e.from << " to " << e.to << "\n";
                }
                ...
                if (Globals::globalVerbosity & 4096)
                {
                    cout << "Have " << fromPair->mustFollowThis().size() << " recorded successors of "
                         << e.from << endl;
                    cout << "Have " << toPair->mustPrecedeThis().size() << " recorded predecessors of "
                         << e.to << endl;
                }
            }
                ...
                if (Q.UB[u])
                {
                    ...
                    if (d0u != DBL_MAX)
                    {
                        ...
                        if (u >= 0)
                        {
                            ...
                            if (v >= 0)
                            {  // and this has to be a sensible value, i.e. u isn't a TIL or instantaneous
                               // action
                                double w;
                                ...
                                else
                                {
                                    if (bfDebug)
                                    {
                                        cout << "\tTo the start of the action (vertex " << v;
                                        cout.flush();
                                    }
                            ...
                            if (teFrom != temporaryEdges.end())
                            {
                                const map<int, pair<bool, double>> &backEdges =
                                    teFrom->second.mustPrecedeThis();
                                ...
                                for (; beItr != beEnd; ++beItr)
                                {
                                    ...
                                    else
                                    {
                                        if (bfDebug)
                                        {
                                            cout << "One back to time zero, but we can't reduce d00\n";
                                        }
                                    }
                                }
                            }
                        ...
                        {  // doing edges out of the zero node - max ts on each node
                ...
                if (Q.LB[u])
                {
                    ...
                    if (du0 != DBL_MAX)
                    {
                        ...
                        if (u >= 0)
                        {
                            {  // If it's a durative action, we'll have an edge in from the end of the action
                                ...
                                if (v >= 0)
                                {
                                    ...
                                    if (u < v)
                                    {
                                        ...
                                        if (bfDebug)
                                        {
                                            cout << "\tFrom the corresponding end: " << w << "\n";
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        if (bfDebug)
                                        {
                                            cout << "\tFrom the corresponding start: " << w << "\n";
                                        }
                                    }
                                    ...
                                    if (w != DBL_MAX)
                                    {
                                        ...
                                        if (du0plusWvu < (dv0 - HALFEPSILON))
                                        {
                                            ...
                                            if (distFromZero[v] != DBL_MAX &&
                                                distFromZero[v] + dv0 < -HALFEPSILON)
                                            {
                                                if (bfDebug)
                                                {
                                                    cout << "\t\t... but distFromZero = " << distFromZero[v]
                                                         << " - cycle\n";
                                                }
                                                Q.cleanup(e.from, e.to);
                                                return false;
                                            ...
                                            {  // figure 2, lines 20a to 20f
                                                if (Q.NEW[v] == u)
                                                {
                                                    if (bfDebug)
                                                    {
                                                        cout << "\t\tVertex from " << v << " to " << u
                                                             << " was the new one, have revised lower "
                                                                "bound\n";
                                                    }
                                                    if (Q.LBP[u])
                                                    {
                                    ...
                                    else
                                    {
                                        if (bfDebug)
                                        {
                                            cout << "(i.e. edge weight is infinite)\n";
                                        }
                                    }
                                }
                            ...
                            if (teFrom != temporaryEdges.end())
                            {
                                const map<int, pair<bool, double>> &foreEdges =
                                    teFrom->second.mustFollowThis();
                        ...
                        {  // edges into zero: min timestamps for each action
                            const int loopLim = events.size();
                            for (int v = 0; v < loopLim; ++v)
                            {
                                if (!events[v])
    ...
    private:
        ...
        bool allHold(const IncomingAndOutgoing &localConstraints, const double &localDistToZero,
                     const TemporalConstraints *const cons)
        {
...

            {
                ...
                for (; edgeItr != edgeEnd; ++edgeItr)
                {
                    ...
                    {
                        mappedNodeIdx =
                            remapPreferenceEdge(*(eventsWithFakes[edgeItr->first]), edgeItr->first, cons);
                        if (mappedNodeIdx == -1)
                        {
                            continue;
                        }
        ...
        bool allHold(const IncomingAndOutgoing &localConstraints, const TemporalConstraints *const cons,
                     double &workingDistToZero, double &workingDistFromZero)
        {
...

            {
                ...
                for (; edgeItr != edgeEnd; ++edgeItr)
                {
                    ...
                    if (edgeItr->second.second >= 0.0)
                    {
                        ...
                        // means the eariest we can have is at least that plus the edge weight (which is
                        // negative, but stored positively)
                        ...
                        if (proposed < workingDistToZero)
                        {
                            ...
                            if (localDebug)
                            {
                                cout << "- Weight " << -edgeItr->second.second
                                     << " to something with dist-to-zero of " << distToZero[mappedNodeIdx]
                                     << ", so dist to zero now " << workingDistToZero << endl;
                            }
                        }
                        else
                        {
                            if (localDebug)
                            {
                                cout << "- Weight " << -edgeItr->second.second
                                     << " to something with dist-to-zero of " << distToZero[mappedNodeIdx]
                                     << ", dist to zero still " << workingDistToZero << endl;
                            }
                        }
                    }
                    ...
                    if (edgeItr->second.second <= 0.0)
                    {
                        // have a step which must come no later than a certain amount after the one we're
                        // looking at means the eariest we can have is at least that minus the edge weight
                        // (which is positive, but stored negatively)
...

            {
                ...
                for (; edgeItr != edgeEnd; ++edgeItr)
                {
                    if (edgeItr->first == -1)
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "Edge from zero to a soft node, weight " << -edgeItr->second.second
                                 << endl;
                        }
                    ...
                    if (localDebug)
                    {
                        ...
                        {
                            cout << "Step " << mappedNodeIdx << " (originally " << edgeItr->first
                                 << ") has no upper bound on how late it can come - ignoring it\n";
                        }
                    }
                    ...
                    if (edgeItr->second.second >= 0.0 && distFromZero[mappedNodeIdx] != DBL_MAX)
                    {
                        ...
                        // the latest we can come is then that minus the edge weight (which is negative, but
                        // stored positively)
                        ...
                        if (proposed < workingDistFromZero)
                        {
                            ...
                            {
                                cout << "- Weight " << -edgeItr->second.second
                                     << " from something with dist-from-zero of "
                                     << distFromZero[mappedNodeIdx] << ", so dist from zero now "
                                     << workingDistFromZero << endl;
                            }
                        }
                        else
                        {
                            if (localDebug)
                            {
                                cout << "- Weight " << -edgeItr->second.second
                                     << " from something with dist-from-zero of "
                                     << distFromZero[mappedNodeIdx] << ", so dist from zero stays at "
                                     << workingDistFromZero << endl;
                            }
                        }
                    }
                    ...
                    if (edgeItr->second.second <= 0.0 && distFromZero[mappedNodeIdx] != DBL_MAX)
                    {
                        // have a step which must come before this one, with this one being a maximum distance
                        // after it the latest we can come is then that step's max, plus the edge weight
                        // (which is positive, but stored negatively)
            ...
            if (localDebug)
            {
                cout << "About to return, comparing working dist from zero of " << workingDistFromZero
                     << " to working dist to zero of " << workingDistToZero << endl;
            }
            ...
            return (-workingDistToZero <= workingDistFromZero);  // the earliest point at which the dummy can
                                                                 // appear is at or before the latest by which
    ...
    public:
        ...
        ChildData(LPQueueSet *QIn, const vector<double> &dfz, const vector<double> &dtz,
                  const vector<int> &pw, const vector<FFEvent *> &ewf, const vector<int> &se,
          ...
          , softEdges(se)
          , temporaryEdges(tes)
          , temporarySoftEdges(tses)
          , needsLP(nlp)
          , copyTimestampsOnDestruction(true){};
        ...
        ~ChildData()
        {
            ...
            if (copyTimestampsOnDestruction)
            {
                ...
                if (cdDebug)
                {
                    cout << "Copying " << ewfCount << " timestamps back\n";
                }
                ...
                for (int e = 0; e < ewfCount; ++e)
                {
                    if (eventsWithFakes[e])
                    {
                        ...
                        if (cdDebug)
                        {
                            cout << "\t" << e << ": " << eventsWithFakes[e]->lpTimestamp << "\n";
                        }
                    }
                }
            }
        ...
        bool propagateNewEdges(const vector<AutomatonPosition::Position> &previousPreferenceStatus,
                               const TemporalConstraints *const cons, bool &preferencesWereAsExpected);
            ...
            for (int i = 0; i < ll; ++i)
            {
                ...
                if (f)
                {
                    ...
                    if (debug == i && (!f->action || !TemporalAnalysis::canSkipToEnd(f->action->getID())))
                    {
                        ...
                        {
                            cout << "LP would put event " << i << " at " << f->lpTimestamp
                                 << ", but STN puts it at " << w << "\n";
        ...
        void distsToLPMinStampsAndCheck(vector<FFEvent *> &withLPTimestamps)
        {
            ...
            for (int i = 0; i < ll; ++i)
            {
                ...
                if (f)
                {
                    ...
                    {
                        std::cerr << std::setprecision(6) << std::fixed << std::endl
                                  << "Error: step " << i << " has been given a timestamp of "
                                  << withLPTimestamps[i]->lpTimestamp
                                  << " by the LP, but the STP reported this had to be at least " << w << endl;
                        exit(1);
                    }
                    ...
                    if (withLPTimestamps[i]->lpTimestamp - distFromZero[i] > 0.0000001)
                    {
                        std::cerr << std::setprecision(6) << std::fixed << std::endl
                                  << "Error: step " << i << " has been given a timestamp of "
                                  << withLPTimestamps[i]->lpTimestamp
                                  << " by the LP, but the STP reported this could be no more than "
                                  << distFromZero[i] << endl;
                        exit(1);
                    }
#endif
    ...
    bool ChildData::checkItContainsAllTheseEdges(const TemporalConstraints *const cons) const
    {
        ...
        for (int i = 0; i < lim; ++i)
        {
            ...
            {
                // cout << "Have " << mItr->second.mustPrecedeThis().size() << " recorded predecessors of " <<
                // i << endl;
                map<int, pair<bool, double>>::const_iterator exItr = mItr->second.mustPrecedeThis().begin();
                const map<int, pair<bool, double>>::const_iterator exEnd =
                    mItr->second.mustPrecedeThis().end();
                ...
                while (exItr != exEnd && sItr != sEnd)
                {
                    ...
                    else if (sItr->first < exItr->first)
                    {
                        ...
                        {
                            // cout << "- Predecessor of " << sItr->first << " isn't in the data, but that
                            // might be okay\n";
                        }
                        ++sItr;
                    }
            ...
            {
                {
                    if (f->action && f->time_spec == Planner::E_AT_START)
                    {
                        ...
                        const vector<pair<double, double>> &tsBounds =
                            TemporalAnalysis::getActionTSBounds()[f->action->getID()];
                ...
                {
                    {
                        ...
                        {
                            map<int, pair<bool, double>>::const_iterator ntfItr =
                                teItr->second.mustPrecedeThis().begin();
                            const map<int, pair<bool, double>>::const_iterator ntfEnd =
                                teItr->second.mustPrecedeThis().end();
    ...
    LPScheduler::LPScheduler(const MinimalState &s, vector<AutomatonPosition::Position> &preferenceStatus,
                             list<FFEvent> &plan)
    {
    ...
    void LPScheduler::collateRelevantVariablesAndInvariants(
        InterestingMap &currInterest, CountedConstraintSet &activeInvariants, const int &stepID,
        const Planner::time_spec &currTS, const int &actID, vector<set<int>> &activeAncestorsOfStep,
        ...
        vector<map<int, ConstraintSet>> &invariantsThisStepStartsOnVariableI)
    {
        ...
        if (activeAncestors.empty())
        {
            if (lpDebug & 1024)
            {
                cout << COLOUR_yellow << "No active ancestors of this step to cause any invariants"
                     << COLOUR_default << endl;
            }
            return;
        }
        ...
        for (; ciItr != ciEnd; ++ciItr)
        {
            ...
            {
                if (correspondEndHasBeenInserted[*aaItr])
                {
                    continue;
                }

                const map<int, ConstraintSet>::iterator onThisVariable =
                    invariantsThisStepStartsOnVariableI[*aaItr].find(ciItr->first);

                if (onThisVariable == invariantsThisStepStartsOnVariableI[*aaItr].end())
                {
                    if (lpDebug & 1024)
                    {
                        cout << COLOUR_yellow << "No invariants on " << *(RPGBuilder::getPNE(ciItr->first))
                             << " started at step " << *aaItr << COLOUR_default << endl;
                    }
                    continue;
                }
                if (lpDebug & 1024)
                {
                    cout << COLOUR_yellow << "Adding invariants on " << *(RPGBuilder::getPNE(ciItr->first))
                         << " that started at step " << *aaItr << COLOUR_default << endl;
                }
    ...
    void LPScheduler::recordVariablesInvolvedInThisStepsInvariants(
        const list<const Constraint *> &invariants, map<int, ConstraintSet> &invariantsOnVariableI)
    {
        ...
        for (; invItr != invEnd; ++invItr)
        {
            ...
            for (int s = 0; s < lim; ++s)
            {
                if (lpDebug & 1024)
                {
                    cout << COLOUR_light_green << "Step has an invariant depending on "
                         << *(RPGBuilder::getPNE(vars[s])) << COLOUR_default << endl;
                }
    ...
    void LPScheduler::addConstraintsToGetValuesOfVariablesNow(
        InterestingMap &currInterest, const int &stepID, const int &currVar,
        LPScheduler::ValueOfVariablesAtSomeTime &beforeStep)
    {
        ...
        for (; sItr != sItrEnd; ++sItr)
        {
            ...
            if (tracker.statusOfThisFluent != FluentTracking::FS_NORMAL)
            {
                if (lpDebug & 1)
                {
                    cout << "Not adding constraint to get value of " << *(RPGBuilder::getPNE(sItr->first))
                         << ": it is a metric tracking fluent";
                    ...
                    {
                        cout << ", the effects on which are order-independent, and hence will be included "
                                "directly in the objective function if needed\n";
                    }
                }
                continue;
            }
            ...
            if (!tracker.activeGradientCount && tracker.lastEffectValueVariable == -1)
            {
                ...
                if (lpDebug & 1)
                {
                    cout << "Not adding constraint at " << colIdx << " to get value of "
                         << *(RPGBuilder::getPNE(sItr->first)) << " - we know it's "
                         << tracker.postLastEffectValue << endl;
                }
            ...
            else
            {
                if (lpDebug & 1)
                {
                    cout << "Adding constraint at " << colIdx << " to get value of "
                         << *(RPGBuilder::getPNE(sItr->first)) << " now";
                    if (sItr->second)
                    {
                ...
                if (tracker.activeGradientCount && tracker.activeGradient != 0.0)
                {
                    if (lpDebug & 1)
                    {
                        cout << "Active gradient = " << tracker.activeGradient << "\n";
                    }
                    if (tracker.lastEffectValueVariable != -1)
                    {
                        ...
                        if (assertions)
                        {
                            assert(entries[3].second != 0.0);
                        }
                        if (assertions)
                        {
                            assert(entries[2].second != 0.0);
                        }
                    ...
                    else
                    {
                        ...
                        lp->addRow(entries, tracker.postLastEffectValue,
                                   tracker.postLastEffectValue);  // syntax for EQ prev
                    }
                    ...
                    if (assertions)
                    {
                        assert((lp->getNumRows() - 1) == constrIdx);
                    }
    ...
    int LPScheduler::generateEndDetails(const Planner::time_spec &currTS, const int &actID, const int &stepID,
                                        FFEvent &currEvent, const vector<FFEvent *> &planAsAVector,
                                        int &nextImaginaryEndVar, vector<EndDetails> &imaginaryMinMax)
    {
        int dummyEnd = -1;
    ...
    protected:
        ...
        void addNormalEffect(RPGBuilder::RPGNumericEffect *const ceItr)
        {
            ...
            bool isStable = ((tracker.activeGradientCount == 0) &&
                             (!parent->finalNumericVars[currVar].everHadADurationDependentEffect));
            ...
            for (int s = 0; s < sLim; ++s)
            {
                ...
                if (varIdx >= 0)
                {
...

                    {
                        ...
                        {
                            cerr << "Fatal internal error: in " << *ceItr << ", trying to refer to "
                                 << *RPGBuilder::getPNE(varIdx)
                                 << ", but it hasn't been made available at this point\n";
                        }
#endif
                    ...
                    //                        if (outstandingCTS.find(varIdx) != outstandingCTS.end())
                    //                        stableVariable[currVar] = false;
                }
            ...
            if (entries.empty())
            {
                ...
                if (lpDebug & 1)
                {
                    cout << *(RPGBuilder::getPNE(currVar)) << " is now " << accruedConstant
                         << " due to effect " << *ceItr << endl;
                }
            }
            else
            {
                ...
                parent->lp->addRow(entries, ceItr->constant + accruedConstant,
                                   ceItr->constant + accruedConstant);
            ...
            {
                stableNextTime->push_back(currVar);
            }
        ...
        void addOrderIndependentMetricEffect(RPGBuilder::RPGNumericEffect *const ceItr)
        {
            ...
            for (int s = 0; s < sLim; ++s)
            {
                ...
                if (varIdx >= 0)
                {
...

                    {
                        ...
                        if (atItr->second.first != -1)
                        {
                            tracker.orderIndependentValueTerms.insert(make_pair(atItr->second.first, 0.0))
                                .first->second += thisW;
                        }
                        else
                        {
                ...
                else if (varIdx == -3 || varIdx == -19)
                {
                    ...
                    if (varIdx == -3)
                    {
                        tracker.orderIndependentValueTerms.insert(make_pair(endStep, 0.0)).first->second +=
                            thisW;
                        tracker.orderIndependentValueTerms.insert(make_pair(startStep, 0.0)).first->second -=
                            thisW;
                    }
                    else
                    {
                ...
                {
                    tracker.orderIndependentValueTerms.insert(make_pair(parent->timestampVars[stepID], 0.0))
                        .first->second += thisW;
                }
                else if (varIdx == -20)
                {
                ...
                {
                    cout << "Unprocessed continuous effect in LP scheduler: refers to variable " << varIdx
                         << ", aborting.\n";
                    assert(false);
                }
            }
            ...
            if (modifierCol != -1)
            {
                tracker.orderIndependentValueTerms.insert(make_pair(modifierCol, 0.0)).first->second +=
                    modifierWeight;
            }
    ...
    public:
        ...
        ConstraintAdder(LPScheduler *const parentIn, FFEvent &ev, const char *const labelIn,
                        const int &stepIDIn, ValueOfVariablesAtSomeTime &applyToIn)
          : parent(parentIn)
          , currEvent(ev)
          , stepID(stepIDIn)
          , effCounter(0)
          , untouched((InterestingMap *)0)
          , stableNextTime((list<int> *)0)
          , unstableNextTime((list<int> *)0)
          , modifierCol(-1)
          , modifierWeight(NaN)
          , label(labelIn)
          , suffix(0)
          , constraintsWereBroken(false)
          , applyTo(&applyToIn)
          , output((ValueOfVariablesAtSomeTime *)0)
        {
        }
        ...
        void supplyEffectData(ValueOfVariablesAtSomeTime &atStep, InterestingMap &unto, list<int> &snt,
                              list<int> &unt)
        {
            output = &atStep;
            untouched = &unto;
        ...
        void operator()(const Constraint *const csItr) const
        {
            ...
            if (LPScheduler::lpDebug & 1024)
            {
                ...
                for (int s = 0; s < cSize; ++s)
                {
                    ...
                    {
                        const ValueOfVariablesAtSomeTime::const_iterator valItr =
                            applyTo->find(csItr->variables[s]);
                        ...
                        {
                            cout << *(RPGBuilder::getPNE(csItr->variables[s])) << " {"
                                 << valItr->second.second << "}";
                        }
                    }
                }
            ...
            for (int s = 0; s < cSize; ++s)
            {
                if (csItr->variables[s] < 0)
                {
                    ...
                    {
                        std::cerr << "Fatal internal error: variable index " << csItr->variables[s]
                                  << " used\n";
                        exit(1);
                    }
                }
            ...
            if (modifierCol != -1)
            {
                entries.push_back(make_pair(modifierCol, -modifierWeight));  // negated weight, because it's
                                                                             // moved from the RHS to the LHS
            }
        ...
        void operator()(const Literal *const litEff)
        {
            ...
            const map<int, RPGBuilder::Guarded>::const_iterator sItr =
                RPGBuilder::getSemaphoreFacts().find(litEff->getStateID());
    ...
    struct LPScheduler::DurationAdder
    {
        ...
        DurationAdder(LPScheduler *const parentIn, FFEvent &ev, const int &stepIDIn,
                      ValueOfVariablesAtSomeTime &beforeStepIn, const vector<bool> &stableVariableIn)
          : parent(parentIn)
          , durationID(0)
          , currEvent(ev)
          , stepID(stepIDIn)
          , beforeStep(&beforeStepIn)
          , stableVariable(stableVariableIn)
          , startToUse(-1)
          , endToUse(-1)
          , durationType(VAL::E_EQUALS)
          , durationIsFixed(true)
        {
        }
        ...
        void operator()(RPGBuilder::DurationExpr *const currDE)
        {
                ...
                for (int v = 0; v < vSize; ++v)
                {
                    ...
                    if (vToUse != -1)
                    {
                        ...
                        if (bsItr == beforeStep->end())
                        {
                            ...
                            {
                                std::cerr << "Internal error: variable -"
                                          << *(RPGBuilder::getPNE(vToUse - RPGBuilder::getPNECount()))
                                          << " not defined in the LP at a step where it is needed for a "
                                             "duration constraint.\n";
                            }
                            else
                            {
                                std::cerr << "Internal error: variable " << *(RPGBuilder::getPNE(vToUse))
                                          << " not defined in the LP at a step where it is needed for a "
                                             "duration constraint.\n";
                            }
                    ...
                    {
                        std::cerr << "Internal error: negative variable " << vToUse
                                  << " in duration expression\n";
                        exit(1);
                    }
                }
            }
            ...
            switch (durationType)
            {
                case VAL::E_EQUALS:
                {
                    if ((lpDebug & 1) && !vSize)
                    {
                        cout << "Simple constant fixed duration: " << currDE->constant << " - "
                             << accruedConstant << " = " << DEconstant << "\n";
                    }
                    ...
                    if (parent->nameLPElements)
                    {
                        ...
                        ostringstream namestream;
                        namestream << "dur" << startToUse << "fixed" << durationID << ": v" << startToUse
                                   << " -> v" << endToUse;
                        string asString = namestream.str();
                        parent->lp->setRowName(constrIdx, asString);
                ...
                case VAL::E_GREATEQ:
                {
                    if ((lpDebug & 1) && !vSize)
                    {
                        cout << "Simple constant minimum duration: " << currDE->constant << " - "
                             << accruedConstant << " = " << DEconstant << "\n";
                    }
                    ...
                    if (parent->nameLPElements)
                    {
                        int constrIdx = parent->lp->getNumRows() - 1;
                        ostringstream namestream;
                        namestream << "dur" << startToUse << "min" << durationID << ": v" << startToUse
                                   << " -> v" << endToUse;
                        string asString = namestream.str();
                        parent->lp->setRowName(constrIdx, asString);
                ...
                case VAL::E_LESSEQ:
                {
                    if ((lpDebug & 1) && !vSize)
                    {
                        cout << "Simple constant maximum duration: " << currDE->constant << " - "
                             << accruedConstant << " = " << DEconstant << "\n";
                    }
                    ...
                    if (parent->nameLPElements)
                    {
                        int constrIdx = parent->lp->getNumRows() - 1;
                        ostringstream namestream;
                        namestream << "dur" << startToUse << "max" << durationID << ": v" << startToUse
                                   << " -> v" << endToUse;
                        string asString = namestream.str();
                        parent->lp->setRowName(constrIdx, asString);
                        durationIsFixed = false;
    ...
    void LPScheduler::AbstractFactConstraintBlock::addUser(const int &stepID)
    {
        ...
        if (!onlyOneUserPerWindow && occCount == 1)
        {
            ...
            {
                lp->setColLower(lpOffsetToPlanStepVariables + stepID,
                                EpsilonResolutionTimestamp(occurrenceBounds[0].first, true).toDouble());
            }

            if (lp->getColUpper(lpOffsetToPlanStepVariables + stepID) > occurrenceBounds[0].second)
            {
                lp->setColUpper(lpOffsetToPlanStepVariables + stepID,
                                EpsilonResolutionTimestamp(occurrenceBounds[0].second, true).toDouble());
            }
        ...
        if (onlyOneUserPerWindow && constraintsForOneUserPerOccurrence.empty())
        {
            ...
            for (int occ = 0; occ < occCount; ++occ)
            {
                ...
                if (debug)
                {
                    cout << "  - Outcome " << occ << " limited by row "
                         << constraintsForOneUserPerOccurrence[occ] << ", detect use with var "
                         << outcomeWasUsed[occ] << endl;
                }
            }
        ...
        for (int occ = 0; occ < occCount; ++occ)
        {
            ...
            {
                // if multiple users are allowed in the same window, we don't need to augment the <=1
                // constraints
                lp->addCol(emptyEntries, 0.0, 1.0, MILPSolver::C_BOOL);
            }
            ...
            if (occ < occCount - 1)
            {
                upperBound.push_back(make_pair(
                    currCol, (occurrenceBounds[occCount - 1].second - occurrenceBounds[occ].second)));
                if (debug)
                {
    ...
    void LPScheduler::AbstractFactConstraintBlock::addOverAllUser(const int &startStepID,
                                                                  const int &endStepID,
                                                                  ...
                                                                  const double &endGap)
    {
        ...
        if (!onlyOneUserPerWindow && occCount == 1)
        {
...

            {
                ...
                {
                    lp->setColLower(lpOffsetToPlanStepVariables + startStepID,
                                    EpsilonResolutionTimestamp(localOBFirst, true).toDouble());
                }

                if (lp->getColLower(lpOffsetToPlanStepVariables + endStepID) < localOBFirst + EPSILON)
                {
                    lp->setColLower(lpOffsetToPlanStepVariables + endStepID,
                                    EpsilonResolutionTimestamp(localOBFirst + EPSILON, true).toDouble());
                }
            }

            {
                ...
                {
                    lp->setColUpper(lpOffsetToPlanStepVariables + endStepID,
                                    EpsilonResolutionTimestamp(localOBSecond, true).toDouble());
                }

                if (lp->getColUpper(lpOffsetToPlanStepVariables + startStepID) > localOBSecond - EPSILON)
                {
                    lp->setColUpper(lpOffsetToPlanStepVariables + startStepID,
                                    EpsilonResolutionTimestamp(localOBSecond - EPSILON, true).toDouble());
                }
            }
        ...
        for (int occ = 0; occ < occCount; ++occ)
        {
            ...
            if (debug)
            {
                cout << "* Have col for over-all from step " << startStepID << " using outcome " << occ
                     << endl;
            }
            ...
            if (occ < occCount - 1)
            {
                ...
                if (debug)
                {
                    const double localOBSecond =
                        occurrenceBounds[occ].second + (endGap == EPSILON ? 0.0 : EPSILON);
        ...
        const double localOBSecond =
            occurrenceBounds[occCount - 1].second + (endGap == EPSILON ? 0.0 : EPSILON);
    ...
    struct VarsForRequirement
    {
        ...
        VarsForRequirement(const int &s) : stepID(s)
        {
        }
    };
    ...
    LPScheduler::LPScheduler(const MinimalState &theState,
                             vector<AutomatonPosition::Position> &preferenceStatus, list<FFEvent> &header,
                             list<FFEvent> &now, const int &justAppliedStep,
                             list<StartEvent> &startEventQueue, ParentData *parentData,
                             map<int, list<list<StartEvent>::iterator>> &compulsaryEnds,
                             const vector<double> *secondMin, const vector<double> *secondMax,
                             list<int> *tilComesBefore, const bool &setObjectiveToMetric)
      : cd(0)
    {
        if (!initialised)
        {
        ...
        {
            cd = parentData->spawnChildData(startEventQueue, header, now, setObjectiveToMetric,
                                            theState.temporalConstraints, justAppliedStep);

            bool preferencesWereAsExpected = true;

            if (!cd || !cd->propagateNewEdges(preferenceStatus, theState.temporalConstraints,
                                              preferencesWereAsExpected))
            {
                if (lpDebug & 1)
                {
                    cout << "STP proved LP unsolvable, skipping LP\n";
                }
            ...
            else if (!setObjectiveToMetric && !cd->doLPSolve())
            {
                if (preferencesWereAsExpected)
                {
                    if (lpDebug & 1)
                    {
                        cout << "No need to solve LP - STP is sufficient\n";
                    }
                    if (paranoia)
                    {
#ifndef NDEBUG
                ...
                else
                {
                    if (lpDebug & 1)
                    {
                        cout << "The STP could be scheduled, but not all preference dummy steps were "
                                "necessarily supported as expected - using a MIP\n";
                        if (paranoia)
                        {
                            shouldSucceed = true;
                            shouldFail = false;
        ...
        if (paranoia && lpDebug)
        {
            if (shouldSucceed)
            {
                cout << "*** Should now succeed ***\n";
            }
            if (shouldFail)
            {
                cout << "*** Should now fail ***\n";
            }
        }
         ...
         */
        const int stepThatMustPrecedeFutureEnds =
            MinimalState::getTransformer()->stepThatMustPrecedeUnfinishedActions(
                theState.temporalConstraints);
        ...
        int mustVisit = 0;
        {
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                ...
                for (; citr != cend; ++citr, ++stepID, ++mustVisit)
                {
                    ...
                    // if (citr->time_spec == Planner::E_AT_START &&
                    // TemporalAnalysis::canSkipToEnd(citr->action->getID())) ++imaginaryEnds;

                    if (pass && citr->isDummyStep())
                    {
                        newDummySteps.insert(
                            make_pair(stepID, make_pair(-1, std::numeric_limits<double>::signaling_NaN())));
                    }

                    const map<int, bool> *const stepsThatComeBeforeThisOne =
                        theState.temporalConstraints->stepsBefore(stepID);
                        ...
                        citr->pairWithStep != stepThatMustPrecedeFutureEnds)
                    {
                        ...
                        if (fanOut[stepThatMustPrecedeFutureEnds].insert(make_pair(stepID, true)).second)
                        {
                            if (lpDebug & 512)
                            {
                                cout << "Additionally, the TO constraint means "
                                     << stepThatMustPrecedeFutureEnds << " precedes " << stepID << endl;
                            }
                        ...
                        else
                        {
                            if (lpDebug & 512)
                            {
                                cout << "The TO constraint means " << stepThatMustPrecedeFutureEnds
                                     << " precedes " << stepID << ", as it already was doing\n";
                            }
                        }
                    }
                    ...
                    if (!fanIn[stepID])
                    {
                        if (lpDebug & 512)
                        {
                            cout << "No steps precede " << stepID << " - adding to open list\n";
                        }
                        openList.push_back(stepID);
                    }

                    if (citr->time_spec == Planner::E_AT_END && citr->getEffects == false &&
...

        {
            ...
            {
                if (!fanOut[ps].empty() && planAsAVector[ps]->time_spec == Planner::E_AT &&
                    planAsAVector[ps]->divisionID < 0)
                {
                    std::cerr << "Error: edges go back into " << ps << ", an abstracted TIL on "
                              << *(RPGBuilder::getLiteral(-1 - planAsAVector[ps]->divisionID)) << endl;
                    ...
                    assert(!(!fanOut[ps].empty() && planAsAVector[ps]->time_spec == Planner::E_AT &&
                             planAsAVector[ps]->divisionID < 0));
                }
            }
        }
        ...
        if (lpDebug & 1)
        {
            cout << "Making lp for " << tsVarCount << " events, " << imaginaryEnds
                 << " future max events, and " << numVars << " variables\n";
        }
        // if (lpDebug & 1) cout << "Making lp for " << tsVarCount << " events and " << numVars << "
        // variables\n";
        ...
        if (lpDebug & 1)
        {
            cout << "Objective function to minimise variable " << optVar << "\n";
        }
        ...
        for (int i = 0; i < numVars; ++i)
        {
            ...
            else if (NumericAnalysis::getDominanceConstraints()[i] == NumericAnalysis::E_METRICTRACKING)
            {
                ...
                {
                    if (NumericAnalysis::getDataOnWhichVariablesHaveOrderIndependentEffects()[i] !=
                        NumericAnalysis::E_ORDER_DEPENDENT)
                    {
                        ...
                        if (lpDebug & 1)
                        {
                            cout << "Objective set to metric: keeping variable " << *(RPGBuilder::getPNE(i))
                                 << ", but making it order-independent\n";
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (lpDebug & 1)
                    {
                        cout << "Objective not set to metric: ignoring metric-tracking variable "
                             << *(RPGBuilder::getPNE(i)) << "\n";
                    }
                }
            }
            ...
            if (finalNumericVars[i].statusOfThisFluent != FluentTracking::FS_IGNORE)
            {
                if (initialGradients[i] != 0.0)
                {
                    ...
                    if (lpDebug & 1)
                    {
                        cout << "Variable " << *(RPGBuilder::getPNE(i))
                             << " becomes unstable due to process; gradient is now " << initialGradients[i]
                             << "\n";
                    }
                }
            }
        ...
        if (lpDebug & 1)
        {
            cout << "Building LP for " << tsVarCount << " step vars: a plan comprising " << header.size()
                 << " header actions and " << now.size() << " new actions.\n";
        }
        ...
        if (!prefs.empty())
        {
            ...
            for (; ndItr != ndEnd; ++ndItr)
            {
                ...
                {
                    if (PreferenceHandler::getInitialAutomataPositions()[*ndItr] ==
                        AutomatonPosition::triggered)
                    {
                        // the master column that says the sometime-after is currently unsatisfied if any of
                        // the instances of it are
                        lp->addCol(emptyEntries, 0.0, 1.0, MILPSolver::C_BOOL);
                        const int currCol = lp->getNumCols() - 1;
                        const map<int, VarsForPreference>::iterator prefVars =
                            varsForPreference.insert(make_pair(*ndItr, VarsForPreference(-1, currCol))).first;
                        ...
                        partOneOfPrefWasAtStep[*ndItr].push_back(
                            VarsForTrigger(-1, localCol, lp->getNumRows() - 1));
                    }
                }
            }
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                ...
                for (; citr != cend; ++citr, ++stepID)
                {
                    ...
                    NNF_Flat *const currNNF =
                        RPGBuilder::getPreferences()[citr->divisionID].d->flattened[part];
                    assert(currNNF);  // must be true, or else the NNF node check above would have called
                                      // continue
                    ...
                    for (int c = 0; c < nodeCount; ++c)
                    {
                        if (cells[c].isCell())
                        {
                            ...
                            if (parentCell == -1)
                            {
                                ...
                                {
                                    ostringstream s;
                                    s << "s" << stepID << RPGBuilder::getPreferences()[citr->divisionID].name
                                      << "singleouter" << c;
                            ...
                            else
                            {
                                if (cells[c].lit)
                                {
                                    ...
                                    {
                                        dummyStepDependencies[stepID]
                                            .literals[cells[c].lit->getStateID()]
                                            .push_back(c);
                                    }
                                    else
                                    {
                                        dummyStepDependencies[stepID]
                                            .negativeLiterals[cells[c].lit->getStateID()]
                                            .push_back(c);
                                    }
                                }
                                else if (cells[c].num >= 0)
                                {
                                    const RPGBuilder::RPGNumericPrecondition &currPre =
                                        RPGBuilder::getNumericPreTable()[cells[c].num];
                                    ...
                                    {
                                        dummyStepDependencies[stepID]
                                            .numerics[currPre.LHSVariable - RPGBuilder::getPNECount()]
                                            .insert(c);
                                    }
                                    else
                                    {
                                        ...
                                        for (int s = 0; s < currAV.size; ++s)
                                        {
                                            ...
                                            {
                                                dummyStepDependencies[stepID]
                                                    .numerics[currAV.fluents[s]]
                                                    .insert(c);
                                            }
                                            else
                                            {
                                                dummyStepDependencies[stepID]
                                                    .numerics[currAV.fluents[s] - RPGBuilder::getPNECount()]
                                                    .insert(c);
                                            }
                                        }
                                    }
                                                    ...
                                                    !currNNF->cellIsAnAnd()[parentCell])
                                {
                                    ...
                                    // thus, if any child constraint is non-satisfied, the and must be
                                    // registered as being non-satisfied i.e. big * and-truth-value - c1 - c2
                                    // .... >= 0

                                    // so, for this, we need to add an entry to the LHS of this constraint
                                    vector<pair<int, double>> localEntries(2);
                                    localEntries[0] = make_pair(
                                        constraintForSettingInnerNodeFromChildren[parentCell], -1.0);

                                    // next, we have a constraint ensuring that if the and node is set to 1,
                                    // at least one of its children is set to 1, thereby ensuring its
                                    // falseness.  this constraint is of the form: c1 + c2 + c3 + c4 >=
                                    // and-truth-value

                                    localEntries[1] =
                                        make_pair(constraintForSettingChildrenFromInnerNode[parentCell], 1.0);
                                ...
                                else
                                {
                                    ...
                                    // i.e. assuming the or holds (value 0) means at least one of the children
                                    // holds
                                    ...
                                    vector<pair<int, double>> localEntries(
                                        1, make_pair(constraintForSettingInnerNodeFromChildren[parentCell],
                                                     1.0));
                                    lp->addCol(localEntries, 0, 1, MILPSolver::C_BOOL);
                                    toUpdate[c] = lp->getNumCols() - 1;
                                    ...
                                    const double newBound =
                                        lp->getRowUpper(
                                            constraintForSettingInnerNodeFromChildren[parentCell]) +
                                        1.0;
                                    lp->setRowUpper(constraintForSettingInnerNodeFromChildren[parentCell],
                                                    newBound);

                                    // second, quite simply:
                                    // child n >= or-truth-value
                                    // so if the or is unsatisfied (1), the child is unsatisfied (1);
                                    // otherwise, we don't care either way
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                constraintForSettingInnerNodeFromChildren[c] = lp->getNumRows() - 1;
                                constraintForSettingChildrenFromInnerNode[c] =
                                    -1;  // in the OR case, this has to be one constraint per OR--child
                                         // relationship
        ...
        while (!openList.empty())
        {
            ...
            //        const bool implicitEnd = (currTS == Planner::E_AT_START &&
            //        TemporalAnalysis::canSkipToEnd(actID));

            if (currTS == Planner::E_AT && divisionID < 0)
            {
                map<int, AbstractFactConstraintBlock>::iterator blockItr =
                    planStepForAbstractFact.find(-1 - divisionID);
                if (blockItr == planStepForAbstractFact.end())
                {
                    blockItr = planStepForAbstractFact
                                   .insert(make_pair(
                                       -1 - divisionID,
                                       AbstractFactConstraintBlock(
                                           -1 - divisionID, lp, numVars,
                                           boundsForAbstractFactOccurrences.find(-1 - divisionID)->second)))
                                   .first;
                }
                blockItr->second.setPlanStepBoundsAreDefinedAt(
                    stepID, planAsAVector[stepID]->lpMinTimestamp == DBL_MAX);
            }
            ...
            if (lpDebug & 1)
            {
                ...
                if (currEvent.isDummyStep())
                {
                    ...
                    {
                        cout << "that is a dummy for a preference precondition "
                             << RPGBuilder::getPreferences()[currEvent.divisionID].name << " on step "
                             << currEvent.divisionID << endl;
                    }
                    else if (currEvent.time_spec == Planner::E_DUMMY_TEMPORAL_TRIGGER_TRUE &&
                             RPGBuilder::getPreferences()[currEvent.divisionID].cons == VAL::E_ALWAYSWITHIN)
                    {
                        cout << "that is a dummy corresponding to a triggering of always-within preference "
                             << RPGBuilder::getPreferences()[currEvent.divisionID].name << ":"
                             << currEvent.divisionID << " by step " << currEvent.pairWithStep << endl;
                    }
                    else if (currEvent.time_spec == Planner::E_DUMMY_TEMPORAL_TRIGGER_TRUE)
                    {
                        cout << "that is a dummy for triggering preference "
                             << RPGBuilder::getPreferences()[currEvent.divisionID].name << ":"
                             << currEvent.divisionID << "\n";
                    }
                    else if (currEvent.time_spec == Planner::E_DUMMY_TEMPORAL_GOAL_FALSE)
                    {
                        cout << "that is a dummy for unsatisfying preference "
                             << RPGBuilder::getPreferences()[currEvent.divisionID].name << ":"
                             << currEvent.divisionID << "\n";
                    }
                    else
                    {
                        ...
                        cout << "that is a dummy for satisfying preference "
                             << RPGBuilder::getPreferences()[currEvent.divisionID].name << ":"
                             << currEvent.divisionID << "\n";
                    }
                }
                ...
                else
                {
                    ...
                    {
                        cout << " as a placeholder for the lower bounds on when an abstracted fact "
                             << *(RPGBuilder::getLiteral(-1 - divisionID)) << " can be obtained\n";
                    }
                }
                cout << COLOUR_default;
            }
            ...
            if (nameLPElements)
            {
                ...
                if (lpDebug & 1)
                {
                    cout << "C" << currVar << " becomes " << asString << "\n";
                }
            }
            ...
            if (!currEvent.isDummyStep() && currEvent.getEffects)
            {
                ...
                for (; sntItr != sntEnd; ++sntItr)
                {
                    if (lpDebug & 1)
                    {
                        cout << "--- Variable " << *(RPGBuilder::getPNE(*sntItr)) << " becomes stable ---\n";
                    }
            ...
            {
                if (currEvent.time_spec == Planner::E_DUMMY_PREFERENCE_PRECONDITION ||
                    preferenceStatus[currEvent.divisionID] != AutomatonPosition::unreachable)
                {
                    ...
                    // time for some magic - look at the temporal constraints on this step, and figure out why
                    // they're there

                    if (stepsThatComeBeforeThisOne &&
                        (pref.cons != VAL::E_ALWAYSWITHIN ||
                         currEvent.time_spec == Planner::E_DUMMY_TEMPORAL_GOAL_TRUE))
                    {
...

                        {
                            ...
                            const map<int, bool>::const_iterator beforeEnd =
                                stepsThatComeBeforeThisOne->end();
                        ...
                        backList.insert(backList.end(), stepsThatComeBeforeThisOne->begin(),
                                        stepsThatComeBeforeThisOne->end());

                        while (!backList.empty())
                        {
                            const map<int, bool> *const currentPredecessors =
                                theState.temporalConstraints->stepsBefore(backList.front().first);

                            if (currentPredecessors)
                            {
                                int &toIncrement =
                                    fanIn.insert(make_pair(backList.front().first, 0)).first->second;
                                ...
                                for (; beforeItr != beforeEnd; ++beforeItr)
                                {
                                    ...
                                    pair<map<int, set<int>>::iterator, bool> insPair =
                                        fanOut.insert(make_pair(beforeItr->first, set<int>()));
                                    if (insPair.second)
                                    {
                        ...
                        // now recreate the available facts/find the most recent step to touch a given numeric
                        // variable
                        ...
                        while (!visitForwards.empty())
                        {
                            ...
                            switch (evt->time_spec)
                            {
                                ...
                                {
                                    adds =
                                        &(RPGBuilder::getNonAbstractedTILVec()[evt->divisionID]->addEffects);
                                    dels =
                                        &(RPGBuilder::getNonAbstractedTILVec()[evt->divisionID]->delEffects);
                                    nums = 0;
                                    break;
                                }
                                default:
                            ...
                            if (nums)
                            {
                                ...
                                {
                                    fluentAfterStep[RPGBuilder::getNumericEff()[*neItr].fluentIndex] =
                                        make_pair(effStep, true);
                                }
                            }
                    ...
                    // now we have fact and numeric variable annotations, indicating after which step they
                    // should come NB if a fact was never added or deleted, we must check if it was in the
                    // initial state
                    ...
                    const bool wantToSatisfy =
                        (currEvent.time_spec == Planner::E_DUMMY_TEMPORAL_TRIGGER_TRUE ||
                    ...
                    {
                        NNF_Flat *const currNNF =
                            RPGBuilder::getPreferences()[currEvent.divisionID].d->flattened[part];
                        ...
                        for (int c = 0; c < nodeCount; ++c)
                        {
                            ...
                            if (cells[c].lit)
                            {
                                const map<int, pair<int, bool>>::const_iterator factItr =
                                    factAfterStep.find(cells[c].lit->getStateID());
                                if (factItr != factAfterStep.end())
                                {
                                    if (cells[c].polarity || !wantToSatisfy)
                                    {
                                        ...
                                        if (prefSupportDebug)
                                        {
                                            ...
                                            cout << " by ordering after "
                                                 << lp->getColName(factItr->second.first + numVars)
                                                 << COLOUR_default << endl;
                                        }
                                        canBeSatisfied = true;
                                    }
                                    else
                                    {
                                        if (prefSupportDebug)
                                        {
                                            ...
                                            cout << " due to any deletors being before adder "
                                                 << lp->getColName(factItr->second.first + numVars)
                                                 << COLOUR_default << endl;
                                        }
                                    }
                                ...
                                {
                                    const map<int, pair<int, bool>>::const_iterator negativeItr =
                                        negativeFactAfterStep.find(cells[c].lit->getStateID());

                                    if (negativeItr != negativeFactAfterStep.end())
                                    {
                                        if (!cells[c].polarity || !wantToSatisfy)
                                        {
                                            satisfiedByOrderingAfter.insert(negativeItr->second.first +
                                                                            numVars);
                                            if (prefSupportDebug)
                                            {
                                                cout << COLOUR_yellow << "¬"
                                                ...
                                                cout << " by ordering after "
                                                     << lp->getColName(negativeItr->second.first + numVars)
                                                     << COLOUR_default << endl;
                                            }
                                        ...
                                        else
                                        {
                                            if (prefSupportDebug)
                                            {
                                                ...
                                                cout << " due to any adders being before deletor "
                                                     << lp->getColName(negativeItr->second.first + numVars)
                                                     << COLOUR_default << endl;
                                            }
                                        }
                                    ...
                                    {
                                        // never been added, never been deleted - what about the initial
                                        // state...
                                        ...
                                        if (initialFacts.find(cells[c].lit) != initialFacts.end())
                                        {
                                            ...
                                            if (prefSupportDebug)
                                            {
                                                if (cells[c].polarity)
                                                {
                                                    ...
                                                    cout << " not touched since initial state, and still "
                                                            "true\n"
                                                         << COLOUR_default;
                                                }
                                                else
                            ...
                            else if (cells[c].num != -1)
                            {
                                ...
                                if (prefSupportDebug)
                                {
                                    cout << COLOUR_yellow << "Finding support for " << currPre
                                         << COLOUR_default << endl;
                                }
                                double LHSvalue = 0.0;
                                ...
                                if (currPre.LHSVariable < RPGBuilder::getPNECount())
                                {
                                    ...
                                    {
                                        std::cerr << "Fatal internal error: using temporal preferences that "
                                                     "depend on variable "
                                                  << *(RPGBuilder::getPNE(currPre.LHSVariable));
                                        std::cerr << ", stored in column "
                                                  << lp->getColName(finalNumericVars[currPre.LHSVariable]
                                                                        .lastEffectValueVariable);
                                    ...
                                    const int &vAfter =
                                        finalNumericVars[currPre.LHSVariable].lastEffectTimestampVariable;
                                    if (vAfter != -1)
                                    {
                                        ...
                                        if (prefSupportDebug)
                                        {
                                            cout << COLOUR_yellow
                                                 << *(RPGBuilder::getPNE(currPre.LHSVariable)) << " = "
                                                 << finalNumericVars[currPre.LHSVariable].postLastEffectValue
                                                 << ", ordered after " << lp->getColName(vAfter) << "\n"
                                                 << COLOUR_default;
                                        }
                                    }
                                    else
                                    {
                                        if (prefSupportDebug)
                                        {
                                            cout << COLOUR_yellow
                                                 << *(RPGBuilder::getPNE(currPre.LHSVariable)) << " = "
                                                 << finalNumericVars[currPre.LHSVariable].postLastEffectValue
                                                 << ", and unchanged since the initial state\n"
                                                 << COLOUR_default;
                                        }
                                    }
                                }
                                ...
                                {
                                    if (finalNumericVars[currPre.LHSVariable - RPGBuilder::getPNECount()]
                                            .lastEffectValueVariable != -1)
                                    {
                                        std::cerr << "Fatal internal error: using temporal preferences that "
                                                     "depend on variable "
                                                  << *(RPGBuilder::getPNE(currPre.LHSVariable -
                                    ...
                                    LHSvalue -=
                                        finalNumericVars[currPre.LHSVariable - RPGBuilder::getPNECount()]
                                            .postLastEffectValue;
                                    const int &vAfter =
                                        finalNumericVars[currPre.LHSVariable - RPGBuilder::getPNECount()]
                                            .lastEffectTimestampVariable;
                                    if (vAfter != -1)
                                    {
                                        satisfiedByOrderingAfter.insert(vAfter);
                                        if (prefSupportDebug)
                                        {
                                            cout << COLOUR_yellow
                                                 << *(RPGBuilder::getPNE(currPre.LHSVariable -
                                                                         RPGBuilder::getPNECount()))
                                                 << " = "
                                                 << finalNumericVars[currPre.LHSVariable -
                                                                     RPGBuilder::getPNECount()]
                                                        .postLastEffectValue
                                                 << ", ordered after " << lp->getColName(vAfter) << "\n"
                                                 << COLOUR_default;
                                ...
                                else
                                {
                                    ...
                                    for (int s = 0; s < currAV.size; ++s)
                                    {
                                        if (currAV.fluents[s] < RPGBuilder::getPNECount())
                                        {
                                            if (finalNumericVars[currAV.fluents[s]].lastEffectValueVariable !=
                                                -1)
                                            {
                                                std::cerr << "Fatal internal error: using temporal "
                                                             "preferences that depend on variable "
                                                          << *(RPGBuilder::getPNE(currAV.fluents[s]))
                                                          << ", which is subject to continuous change\n";
                                                exit(1);
                                            }
                                            LHSvalue +=
                                                finalNumericVars[currAV.fluents[s]].postLastEffectValue *
                                                currAV.weights[s];
                                            const int &vAfter = finalNumericVars[currAV.fluents[s]]
                                                                    .lastEffectTimestampVariable;
                                            if (vAfter != -1)
                                            {
                                                satisfiedByOrderingAfter.insert(vAfter);
                                                if (prefSupportDebug)
                                                {
                                                    cout << COLOUR_yellow
                                                         << *(RPGBuilder::getPNE(currAV.fluents[s])) << " = "
                                                         << finalNumericVars[currAV.fluents[s]]
                                                                .postLastEffectValue
                                                         << ", ordered after " << lp->getColName(vAfter)
                                                         << "\n"
                                                         << COLOUR_default;
                                        ...
                                        {
                                            if (finalNumericVars[currAV.fluents[s] -
                                                                 RPGBuilder::getPNECount()]
                                                    .lastEffectValueVariable != -1)
                                            {
                                                std::cerr << "Fatal internal error: using temporal "
                                            ...
                                            LHSvalue -= finalNumericVars[currAV.fluents[s] -
                                                                         RPGBuilder::getPNECount()]
                                                            .postLastEffectValue *
                                                        currAV.weights[s];
                                            const int &vAfter = finalNumericVars[currAV.fluents[s] -
                                ...
                                if (currPre.op == VAL::E_GREATEQ ? LHSvalue >= currPre.RHSConstant :
                                                                   LHSvalue > currPre.RHSConstant)
                                {
                                    if (prefSupportDebug)
                                    {
                                        if (wantToSatisfy)
                    ...
                    map<int, VarsForPreference>::iterator prefVars =
                        varsForPreference.find(currEvent.divisionID);
                    ...
                    if (currEvent.time_spec == Planner::E_DUMMY_PREFERENCE_PRECONDITION)
                    {
                        ...
                        varsForPreconditionPreference.insert(make_pair(currEvent.divisionID, list<int>()))
                            .first->second.push_back(varsForNNFCells[0]);
                        ...
                        switch (pref.cons)
                        {
                            case VAL::E_ALWAYS:
                            {
                                if (prefVars == varsForPreference.end())
                                {
                                    ...
                                    prefVars = varsForPreference
                                                   .insert(make_pair(currEvent.divisionID,
                                                                     VarsForPreference(lp->getNumCols() - 1)))
                                                   .first;
                                }

                                // var denoting the always is true >= var denoting this instance of the always
                                // is true therefore if this instance = 1, the always = 1
                                localPairOfEntries[0] = make_pair(prefVars->second.committedCostVar, 1.0);
                            ...
                            case VAL::E_ATEND:
                            {
                                std::cerr << "Fatal error: dummy steps should not be inserted for at-end "
                                             "preferences, it should only depend on whether they are true at "
                                             "the end or not\n";
                                exit(1);
                                break;
                            }
                            case VAL::E_SOMETIME:
                            {
                                if (prefVars == varsForPreference.end())
                                {
                                    ...
                                    prefVars = varsForPreference
                                                   .insert(make_pair(currEvent.divisionID,
                                                                     VarsForPreference(-1, currCol)))
                                                   .first;

                                    // create the initial basic constraint that this can only be true if any
                                    // one of the instances of it are true c1 + c2 + c3 + c4 -
                                    // sometime-truth-value <= number of children - 1
                            ...
                            {
                                // for things we don't want to be true, we pessimistically force them to be
                                // true, and leave it to back-tracking to resolve either making it false, or
                                // making the STP feasible for at-most-once, this means enforcing the entire
                                // trace
                            ...
                            case VAL::E_ALWAYSWITHIN:
                            {
                                ...
                                if (part == 1)
                                {
                                    ...
                                    {
                                        // must bind this dummy step to be at the same time as the one it's
                                        // paired with, i.e. its direct trigger

                                        localPairOfEntries[0] = make_pair(currVar, 1.0);
                                        localPairOfEntries[1] =
                                            make_pair(currEvent.pairWithStep + numVars, -1.0);
                                        lp->addRow(localPairOfEntries, 0.0, 0.0);
                                    ...
                                    {
                                        // for things we don't want to be true, we pessimistically force them
                                        // to be true, and leave it to back-tracking to resolve
                                        lp->setColUpper(varsForNNFCells[0], 0.0);
                                    }

                                    if (prefVars == varsForPreference.end())
                                    {
                                        // the master column that says the sometime-after is currently
                                        // unsatisfied if any of the instances of it are
                                        lp->addCol(emptyEntries, 0.0, 1.0, MILPSolver::C_BOOL);
                                        ...
                                        if (lpDebug & 1)
                                        {
                                            if (pref.cons == VAL::E_ALWAYSWITHIN)
                                            {
                                                cout << "Now have a variable to track the truth value of "
                                                        "always-within "
                                                     << pref.name << endl;
                                            }
                                            else
                                    ...
                                    partOneOfPrefWasAtStep[currEvent.divisionID].push_back(
                                        VarsForTrigger(stepID, localCol, lp->getNumRows() - 1));

                                    // for things we don't want to be true, we pessimistically force them to
                                    // be true, and leave it to back-tracking to resolve either making it
                                    // false, or making the STP feasible
                                }
                                else
                                {
                                    // if this is satisfied, all previous part ones are satisfied

                                    map<int, list<VarsForTrigger>>::const_iterator emItr =
                                        partOneOfPrefWasAtStep.find(currEvent.divisionID);
                                    ...
                                    if (pref.cons == VAL::E_SOMETIMEAFTER)
                                    {
                                        ...
                                        for (; eItr != eEnd; ++eItr)
                                        {
                                            // first, if the preference is presumed unsatisfied, force this to
                                            // break
                                            localPairOfEntries[0] = make_pair(varsForNNFCells[0], 1.0);
                                            ...
                                            // second, if this is satisfied, force the preference as a whole
                                            // to satisfied
                                    ...
                                    else
                                    {
                                        ...
                                        for (; eItr != eEnd; ++eItr)
                                        {
                                            ...
                                            // (varsForCells[0] == 0) && (not too much time has elapsed) =>
                                            // eItr->satisfactionVar == 0
                                            ...
                                            {
                                                ostringstream s;
                                                s << "aw" << pref.name << "s" << eItr->stepID << "e" << stepID
                                                  << "intime";
                                                lp->setColName(lp->getNumCols() - 1, s.str());
                                            }
                                            ...
                                            conditionalPrecedence[1] =
                                                make_pair(numVars + eItr->stepID, -1.0);
                                            ...
                                            {
                                                ostringstream s;
                                                s << "aw" << pref.name << "g" << stepID << "ontime"
                                                  << eItr->stepID;
                                                lp->setRowName(lp->getNumRows() - 1, s.str());
                                            }
                                            ...
                                            // finally, treat as sometime after, but with jointCol instead of
                                            // varsForNNFCells[0]
                            ...
                            case VAL::E_SOMETIMEBEFORE:
                            {
                                ...
                                if (part == 0)
                                {
                                    if (lpDebug & 1)
                                    {
                                        cout << COLOUR_yellow << "- Noting that step " << stepID
                                             << " is the requirement of sometime-before preference "
                                             << pref.name << COLOUR_default << endl;
                                    }
                                    partZeroOfPrefWasAtStep[currEvent.divisionID].push_back(
                                        VarsForRequirement(stepID));
                                }
                                else
                                {
                                    // one of the previous part zeros must be true if this is to be true

                                    map<int, list<VarsForRequirement>>::const_iterator vfrItr =
                                        partZeroOfPrefWasAtStep.find(currEvent.divisionID);
                                    ...
                                    if (vfrItr == partZeroOfPrefWasAtStep.end())
                                    {
                                        ...
                                        {
                                            cout << COLOUR_yellow << "- Noting that step " << stepID
                                                 << ", a trigger of sometime-before preference " << pref.name
                                                 << ", comes after no requirements, so is definitely violated"
                                                 << endl;
                                        }
                                    ...
                                    {
                                        // c1 + c2 + c3 + c4 - sometime-before-truth-value <= number of
                                        // earlier requirement opportunities - 1

                                        const int earlierRequirementOpportunities = vfrItr->second.size();

                                        vector<pair<int, double>> sometimeBeforeConstraint(
                                            earlierRequirementOpportunities + 1);

                                        sometimeBeforeConstraint[0] =
                                            make_pair(prefVars->second.committedCostVar, -1.0);

                                        list<VarsForRequirement>::const_iterator esItr =
                                            vfrItr->second.begin();
                                        const list<VarsForRequirement>::const_iterator esEnd =
                                            vfrItr->second.end();
                                        ...
                                        if (lpDebug & 1)
                                        {
                                            cout << COLOUR_yellow << "- Noting that step " << stepID
                                                 << ", a trigger of sometime-before preference " << pref.name
                                                 << ", must be after one of the earlier requirements to be "
                                                    "satisfied [";
                                        }

                                        for (int col = 1; esItr != esEnd; ++esItr, ++col)
                                        {
                                            sometimeBeforeConstraint[col] = make_pair(
                                                variablesCorrespondingToNNFNodesForDummyStep[esItr->stepID]
                                                                                            [0],
                                                1.0);
                                            if (lpDebug & 1)
                                            {
                                        ...
                                        lp->addRow(sometimeBeforeConstraint, -LPinfinity,
                                                   earlierRequirementOpportunities - 1);
                            ...
                            case VAL::E_WITHIN:
                            {
                                if (prefVars == varsForPreference.end())
                                {
                                    ...
                                    // create the initial basic constraint that this can only be true if any
                                    // one of the instances of it are true c1 + c2 + c3 + c4 -
                                    // within-truth-value <= number of children - 1
                                ...
                                // we've now reduced it to a sometime constraint, only with jointCol instead
                                // of varsForNNFCells[0]
                ...
                for (int predecessorPass = 0; predecessorPass < 2; ++predecessorPass)
                {
                    ...
                    if (currentPredecessors)
                    {
                        ...
                        for (; beforeItr != beforeEnd; ++beforeItr)
                        {
                            ...
                            if (lpDebug & 1)
                            {
                                cout << " " << beforeItr->first;
                            }
                            const int prevVar = beforeItr->first + numVars;

                            if (planAsAVector[beforeItr->first]->isDummyStep())
                            {
                                if (planAsAVector[beforeItr->first]->time_spec ==
                                        Planner::E_DUMMY_PREFERENCE_PRECONDITION ||
                                    preferenceStatus[planAsAVector[beforeItr->first]->divisionID] !=
                                        AutomatonPosition::unreachable)
                                {
                                    ...
                                    switch (currEvent.time_spec)
                                    {
                                        case Planner::E_AT_START:
                                        {
                                            adds = &(RPGBuilder::getStartPropositionAdds()[currEvent.action
                                                                                               ->getID()]);
                                            dels =
                                                &(RPGBuilder::getStartPropositionDeletes()[currEvent.action
                                                                                               ->getID()]);
                                            nums = &(
                                                RPGBuilder::getStartEffNumerics()[currEvent.action->getID()]);
                                            break;
                                        }
                                        case Planner::E_AT_END:
                                        {
                                            adds = &(RPGBuilder::getEndPropositionAdds()[currEvent.action
                                                                                             ->getID()]);
                                            dels = &(RPGBuilder::getEndPropositionDeletes()[currEvent.action
                                                                                                ->getID()]);
                                            nums =
                                                &(RPGBuilder::getEndEffNumerics()[currEvent.action->getID()]);
                                            break;
                                        }
                                        case Planner::E_AT:
                                        {
                                            adds =
                                                &(RPGBuilder::getNonAbstractedTILVec()[currEvent.divisionID]
                                                      ->addEffects);
                                            dels =
                                                &(RPGBuilder::getNonAbstractedTILVec()[currEvent.divisionID]
                                                      ->delEffects);
                                        ...
                                        default:
                                        {
                                            std::cerr << "Fatal internal error: should not have time "
                                                         "specifier "
                                                      << currEvent.time_spec << " at this point\n";
                                            exit(1);
                                        }
                                    }
                                    ...
                                    {
                                        list<Literal *>::const_iterator delItr =
                                            (apass ? adds->begin() : dels->begin());
                                        const list<Literal *>::const_iterator delEnd =
                                            (apass ? adds->end() : dels->end());

                                        for (; delItr != delEnd; ++delItr)
                                        {
                                            const map<int, list<int>>::const_iterator depItr =
                                                dummyStepDependencies[beforeItr->first].literals.find(
                                                    (*delItr)->getStateID());

                                            if (depItr !=
                                                dummyStepDependencies[beforeItr->first].literals.end())
                                            {
                                    ...
                                    if (nums)
                                    {
                                        ...
                                        for (; neItr != neEnd; ++neItr)
                                        {
                                            const map<int, set<int>>::const_iterator depItr =
                                                dummyStepDependencies[beforeItr->first].numerics.find(
                                                    RPGBuilder::getNumericEff()[*neItr].fluentIndex);
                                            if (depItr !=
                                                dummyStepDependencies[beforeItr->first].numerics.end())
                                            {
                                                cpCells.insert(depItr->second.begin(), depItr->second.end());
                                            }
                                        }
                                    ...
                                    {
                                        conditionalPrecedence[2] = make_pair(
                                            variablesCorrespondingToNNFNodesForDummyStep[beforeItr->first]
                                                                                        [*cellItr],
                                            BIG);
                                        lp->addRow(conditionalPrecedence, 0.0, LPinfinity);
                                        ...
                                        {
                                            ostringstream s;
                                            s << "ord" << stepID << "after" << beforeItr->first << "cell"
                                              << *cellItr;
                                            lp->setRowName(lp->getNumRows() - 1, s.str());
                                        }
                                    }
                            ...
                            else
                            {
                                if (currTS != Planner::E_AT_END || prevVar != currVar - 1)
                                {
                                    ...
                                    lp->addRow(entries, (beforeItr->second ? EPSILON : 0.0),
                                               LPinfinity);  // syntax for '>= 0 or EPSILON'
...

            {
                ...
                else if (currTS == Planner::E_AT)
                {
                    if (divisionID >= 0)
                    {
                        ...
                        if (lpDebug & 1)
                        {
                            cout << "- As it's a TIL, fixing column bounds of " << currVar << " to "
                                 << thisTIL << endl;
                        }
                ...
                else
                {
                    ...
                    const double initialWorkingLower =
                        actionTSBounds[actID][currTS == Planner::E_AT_START ? 0 : 1].first;
                    ...
                    {
                        {
                            ...
                            for (int ecID = 0; ecItr != ecEnd; ++ecItr, ++ecID)
                            {
                                ...
                                for (int s = 0; s < size; ++s)
                                {
                                    ...
                                    {
                                        map<int, set<int>>::const_iterator guardItr =
                                            NumericAnalysis::getFactsThatDefineAndFixVariables().find(
                                                (*ecItr)->variables[s]);
                                        if (guardItr !=
                                            NumericAnalysis::getFactsThatDefineAndFixVariables().end())
                                        {
                                            ...
                                            {
                                                const double outcomeA =
                                                    theState.secondMin[(*ecItr)->variables[s]] *
                                                    (*ecItr)->weights[s];
                                                const double outcomeB =
                                                    theState.secondMax[(*ecItr)->variables[s]] *
                                                    (*ecItr)->weights[s];
                                ...
                                if (canBeBounded && foundTime)
                                {
                                    ...
                                    for (int s = 0; s < size; ++s)
                                    {
                                        ...
                                        {
                                            const int col = finalNumericVars[(*ecItr)->variables[s]]
                                                                .lastEffectValueVariable;

                                            if (col == -1)
                                            {
                                                fixed += finalNumericVars[(*ecItr)->variables[s]]
                                                             .postLastEffectValue *
                                                         (*ecItr)->weights[s];
                                            }
                                            else
                                            {
                                                constraintOnOpenEnd.push_back(
                                                    make_pair(col, (*ecItr)->weights[s]));
                                            }
                                        }
                                    }
                                    ...
                                    if (constraintOnOpenEnd.size() > 1)
                                    {
                                        ...
                                        {
                                            lp->addRow(constraintOnOpenEnd, -LPinfinity,
                                                       (*ecItr)->upper - fixed);
                                            added = true;
                                        }
                                        else if ((*ecItr)->lower != -DBL_MAX && (*ecItr)->upper == DBL_MAX)
                                        {
                                            lp->addRow(constraintOnOpenEnd, (*ecItr)->lower - fixed,
                                                       LPinfinity);
                                            added = true;
                                        }
                                        else
                                    ...
                                    if (timeWeight > 0.0)
                                    {
                                        ...
                                        {
                                            double forcedTBound =
                                                ((*ecItr)->lower - boundsOnRest.second) / timeWeight;
                                            ...
                                            if (forcedTBound > boundForSuccessors)
                                            {
                                                ...
                                                if (lpDebug & 1)
                                                {
                                                    cout << "Pushed lower bound up to " << forcedTBound
                                                         << " due to a time-dependent precondition\n";
                                                }
                                            }
                                        }
                                        ...
                                        {
                                            double forcedTBound =
                                                ((*ecItr)->upper - boundsOnRest.first) / timeWeight;
                                            ...
                                            if (forcedTBound < overallWorkingUpper)
                                            {
                                                ...
                                                if (lpDebug & 1)
                                                {
                                                    cout << "Pushed upper bound down to " << forcedTBound
                                                         << " due to a time-dependent precondition\n";
                                                }
                                            }
                                        }
                        ...
                        if (!tilVec.empty())
                        {
                            ...
                            const list<Literal *> &instantPres =
                                RPGBuilder::getEndPropositionalPreconditions()[actID];

                            list<pair<TILTimeline::const_iterator, TILTimeline::const_iterator>>
                                relevantTimelinePointsAndTheirEnds;
                            ...
                            for (; pItr != pEnd; ++pItr)
                            {
                                ...
                                const TILTimeline *const currTimeline =
                                    TemporalAnalysis::timelineOnTILFact((*pItr)->getStateID());
...

                                {
                                    ...
                                    if (ub != currTimeline->end() &&
                                        ub->first - EPSILON < overallWorkingUpper)
                                    {
                                        if (lpDebug & 1)
                                        {
                                            cout << "- " << *(*pItr) << " pushes upper bound down to "
                                                 << ub->first - EPSILON
                                                 << ", i.e. before the last TIL deletor\n";
                                        }
                                        ...
                                        if (overallWorkingUpper < boundForSuccessors)
                                        {
                                            if (lpDebug & 1)
                                            {
                                                cout << "- This is before the lower bound of "
                                                     << boundForSuccessors
                                                     << ", so the LP would be unsolvable\n";
                                            }
                                            solved = false;
                                            return;
                                        }
                                ...
                                TILTimeline::const_iterator adder =
                                    currTimeline->firstAdderSuitableForTime(boundForSuccessors);
                                ...
                                if (adder == currTimeline->end())
                                {
                                    ...
                                    {
                                        cout << "- " << *(*pItr)
                                             << " means that the LP cannot ever be solved: there is no "
                                                "true-TIL-window that can contain "
                                             << boundForSuccessors << endl;
                                    }
                                    solved = false;
                                    return;
                                }
                                ...
                                if (adder->first + EPSILON > boundForSuccessors)
                                {
                                    ...
                                    {
                                        cout << "- " << *(*pItr)
                                             << " leads to pushing lower-bound of end point up to "
                                             << adder->first + EPSILON << endl;
                                    }
                                    ...
                                    if (overallWorkingUpper < boundForSuccessors)
                                    {
                                        if (lpDebug & 1)
                                        {
                                            cout << "- This is after the upper bound of "
                                                 << overallWorkingUpper
                                                 << ", so the LP would be unsolvable\n";
                                        }
                                        solved = false;
                                ...
                                else
                                {
                                    if (lpDebug & 1)
                                    {
                                        cout << "- " << *(*pItr) << " is okay for now: " << adder->first
                                             << " is below the known lower bound\n";
                                    }
                                }

                                relevantTimelinePointsAndTheirEnds.push_back(
                                    make_pair(adder, currTimeline->end()));
                            }
                            ...
                            for (int iCounter = 0; pushedForwards; ++iCounter)
                            {
                                ...
                                if (lpDebug & 1)
                                {
                                    cout << "- Revisiting TIL timelines from " << boundForSuccessors
                                         << ", pass " << iCounter << endl;
                                }

                                list<pair<TILTimeline::const_iterator, TILTimeline::const_iterator>>::iterator
                                    rtItr = relevantTimelinePointsAndTheirEnds.begin();
                                const list<pair<TILTimeline::const_iterator, TILTimeline::const_iterator>>::
                                    iterator rtEnd = relevantTimelinePointsAndTheirEnds.end();
                                ...
                                for (; rtItr != rtEnd; ++rtItr)
                                {
                                    ...
                                    TILTimeline::const_iterator lastDeletorBeforeBoundForSuccessors =
                                        rtItr->second;
                                    ...
                                    {
                                        // some deletor has occurred between the previously relied upon TIL
                                        // and now thus, the next adder must precede nowAfter - push forwards
                                        // the last deletor until we find an adding happening

                                        for (; lastDeletorBeforeBoundForSuccessors != rtItr->second &&
                                               !lastDeletorBeforeBoundForSuccessors->second.added;
                                             ++lastDeletorBeforeBoundForSuccessors)
                                            ;
                                        ...
                                        if (lastDeletorBeforeBoundForSuccessors == rtItr->second)
                                        {
                                            ...
                                            if (lpDebug & 1)
                                            {
                                                cout << "- There is no adder after a now-past deletor on "
                                                        "this timeline\n";
                                            }
                                            solved = false;
                                            return;
                                        ...
                                        if (rtItr->first->first + EPSILON > boundForSuccessors)
                                        {
                                            ...
                                            if (lpDebug & 1)
                                            {
                                                cout << "- To get past a deletor on this timeline, and then "
                                                        "be added again, the lower bound is pushed to "
                                                     << boundForSuccessors << endl;
                                            }
                                            if (overallWorkingUpper < boundForSuccessors)
                                            {
                    ...
                    if (currEvent.getEffects && !tilVec.empty() && theState.nextTIL < tilVec.size())
                    {
...

                        {
                            ...
                            const list<Literal *> &instantPres =
                                (currTS == Planner::E_AT_START ?
                                     RPGBuilder::getStartPropositionalPreconditions()[actID] :
                                     RPGBuilder::getEndPropositionalPreconditions()[actID]);
                            ...
                            for (; pItr != pEnd; ++pItr)
                            {
                                ...
                                bool allTILs = true;
                                const list<pair<int, Planner::time_spec>> &deletors =
                                    RPGBuilder::getNegativeEffectsToActions()[(*pItr)->getStateID()];
                                list<pair<int, Planner::time_spec>>::const_iterator dItr = deletors.begin();
                                const list<pair<int, Planner::time_spec>>::const_iterator dEnd =
                                    deletors.end();
                                ...
                                for (; dItr != dEnd; ++dItr)
                                {
                                    if (dItr->second == Planner::E_AT)
                                    {
#ifndef NDEBUG
                                        list<Literal *>::const_iterator delItr =
                                            tilVec[dItr->first]->delEffects.begin();
                                        const list<Literal *>::const_iterator delEnd =
                                            tilVec[dItr->first]->delEffects.end();
                                        ...
                                        if (delItr == delEnd)
                                        {
                                            std::cerr << "Fatal error: TIL " << dItr->first
                                                      << " does not delete fact " << *(pItr)
                                                      << ", as claimed\n";
                                ...
                                if (allTILs)
                                {
                                    if (lpDebug & 1)
                                    {
                                        ...
                                        {
                                            cout << "- " << *(*pItr)
                                                 << " does not limit the latest point at which the action "
                                                    "can occur\n";
                                        }
                                        else
                                        {
                                            cout << "- " << *(*pItr) << " limits it to " << workingUpper
                                                 << endl;
                                        }
                                    }
                        ...
                        {
                            cout << "Looking at negative TIL effects got working upper bound down to "
                                 << overallWorkingUpper << endl;
                        }
                    }
                    ...
                    {
                        double workingUpper =
                            actionTSBounds[actID][currTS == Planner::E_AT_START ? 0 : 1].second;
                        //                    if (TILupbo < DBL_MAX) {
                        //                        if (workingUpper > TILupbo - EPSILON) workingUpper = TILupbo
                        //                        - EPSILON;
                        //                    }
                        ...
                        {
                            if (workingUpper > currEvent.lpMaxTimestamp)
                            {
                                workingUpper = currEvent.lpMaxTimestamp;
                            }
                        }
                        ...
                        if (workingUpper < DBL_MAX)
                        {
                            if (workingUpper != LPinfinity)
                            {
                                ...
                                lp->setColUpper(currVar,
                                                EpsilonResolutionTimestamp(workingUpper, true).toDouble());
                            }
                        }
                    }
                    else
                    {
                        if (overallWorkingUpper < DBL_MAX)
                        {
                            ...
                            lp->setColUpper(currVar,
                                            EpsilonResolutionTimestamp(overallWorkingUpper, true).toDouble());
                        }
                    }
                }
                ...
                if (!currEvent.isDummyStep() && currEvent.time_spec != Planner::E_AT &&
                    boundForSuccessors > makespanVarMinimum)
                {
            ...
            {
                addConstraintsForTILMutexes(
                    currVar,
                    pointsThatWouldBeMutexWithOptimisationTILs[actID][currTS == Planner::E_AT_START ? 0 : 1]);
            }

            if (!boundsForAbstractFactOccurrences.empty() &&
                (currTS == Planner::E_AT_START || currTS == Planner::E_AT_END))
            {
                set<int> mergedWithOverAll;

                {
                    const map<int, list<int>> &lookIn =
                        TemporalAnalysis::getAbstractFactsUsedByActionOverAll();
                    ...
                    if (depItr != lookIn.end())
                    {
                        ...
                        if (currTS == Planner::E_AT_START)
                        {
                            ...
                            const map<int, list<int>> &lookInStart =
                                TemporalAnalysis::getAbstractFactsUsedByActionStart();
                            const map<int, list<int>> &lookInEnd =
...

                            {
                                ...
                                {
                                    offsetItr =
                                        offsets.insert(offsetItr, make_pair(*fItr, make_pair(0.0, 0.0)));
                                }
                            }
                            ...
                            {
                                const map<int, list<int>>::const_iterator startDepItr =
                                    lookInStart.find(actID);
                            ...
                            {
                                map<int, AbstractFactConstraintBlock>::iterator blockItr =
                                    planStepForAbstractFact.find(offsetItr->first);
                                if (blockItr == planStepForAbstractFact.end())
                                {
                                    blockItr = planStepForAbstractFact
                                                   .insert(make_pair(offsetItr->first,
                                                                     AbstractFactConstraintBlock(
                                                                         offsetItr->first, lp, numVars,
                                                                         boundsForAbstractFactOccurrences
                                                                             .find(-1 - divisionID)
                                                                             ->second)))
                                                   .first;
                                }
                                if (lpDebug & 1)
                                {
                                    cout << COLOUR_light_magenta
                                         << "Registering step as over-all user of abstract fact "
                                         << *(RPGBuilder::getLiteral(offsetItr->first)) << COLOUR_default
                                         << endl;
                                }
                                blockItr->second.addOverAllUser(stepID, currEvent.pairWithStep,
                                                                offsetItr->second.first,
                                                                offsetItr->second.second);
                            }
                        }
                    }
                }

                {
                    ...
                    if (depItr != lookIn.end())
                    {
                        ...
                        for (; fItr != fEnd; ++fItr)
                        {
                            ...
                            map<int, AbstractFactConstraintBlock>::iterator blockItr =
                                planStepForAbstractFact.find(*fItr);
                            if (blockItr == planStepForAbstractFact.end())
                            {
                                blockItr = planStepForAbstractFact
                                               .insert(make_pair(*fItr, AbstractFactConstraintBlock(
                                                                            *fItr, lp, numVars,
                                                                            boundsForAbstractFactOccurrences
                                                                                .find(-1 - divisionID)
                                                                                ->second)))
                            ...
                            if (lpDebug & 1)
                            {
                                cout << COLOUR_light_magenta << "Registering step as a user of abstract fact "
                                     << *(RPGBuilder::getLiteral(*fItr)) << COLOUR_default << endl;
                            }
            ...
            if (!currEvent.isDummyStep() && currEvent.getEffects)
            {
                        ...
                        for (int v = 0; v < numVars; ++v)
                        {
                            if (bsItr != bsEnd && (v == bsItr->first))
                            {
                                if (bsItr->second.first != -1)
                                {
                                    if (secondMin)
                                    {
                                        lp->setColLower(bsItr->second.first, (*secondMin)[v]);
                                    }
                                    if (secondMax)
                                    {
                                        lp->setColUpper(bsItr->second.first, (*secondMax)[v]);
                                    }
                                }
                                else
                                {
                            ...
                            else
                            {
                                ...
                                if (update != -1)
                                {
                                    if (secondMin)
                                    {
                                        lp->setColLower(update, (*secondMin)[v]);
                                    }
                                    if (secondMax)
                                    {
                                        lp->setColUpper(update, (*secondMax)[v]);
                                    }
                                }
                                else
                                {
                                    assert(!secondMin ||
                                           finalNumericVars[v].postLastEffectValue >= (*secondMin)[v]);
                                    assert(!secondMax ||
                                           finalNumericVars[v].postLastEffectValue <= (*secondMax)[v]);
                                }
                            }
                        }
                    }
                ...
                /*const int dummyEnd = */ generateEndDetails(currTS, actID, stepID, currEvent, planAsAVector,
                                                             nextImaginaryEndVar, imaginaryMinMax);

                ConstraintAdder adder(this, currEvent, "inv@t", stepID, beforeStep);

                const map<int, list<vector<pair<const Constraint *, const Constraint *>>>>::const_iterator
                    iceConds = (currTS != Planner::E_AT ? instantIntegralConstraints.find(actID) :
                ...
                {
                    map<int, list<vector<list<RPGBuilder::RPGNumericEffect *>>>>::const_iterator iceEffs =
                        instantIntegralConditionalEffects.find(actID);
                ...
                if (currTS == Planner::E_AT_START || currTS == Planner::E_AT_END)
                {
...

                    {
                        ...
                        if (ice)
                        {
                            map<int, list<RPGBuilder::IntegralContinuousEffect>>::const_iterator defMapItr =
                                RPGBuilder::getActionsToIntegralConditionalEffects().find(actID);

                            adder.changeLabel("icepre@t");

                            list<RPGBuilder::IntegralContinuousEffect>::const_iterator defItr =
                                defMapItr->second.begin();
                            list<vector<pair<const Constraint *, const Constraint *>>>::const_iterator icItr =
                                iceConds->second.begin();
                            const list<vector<pair<const Constraint *, const Constraint *>>>::const_iterator
                                icEnd = iceConds->second.end();
                            ...
                            for (; icItr != icEnd; ++icItr, ++defItr)
                            {
                                ...
                                if (lpDebug & 1)
                                {
                                    cout << COLOUR_yellow << "Created integer conditional effect variable "
                                         << lp->getColName(iceVars.back()) << " with range ["
                                         << lp->getColLower(iceVars.back()) << ","
                                         << lp->getColUpper(iceVars.back()) << "]\n"
                                         << COLOUR_default;
                                }
                                ...
                                adder.setRHSModifier(iceVars.back(),
                                                     defItr->getRightPreconditionDifference());
                    ...
                    list<RPGBuilder::RPGNumericEffect *> &currEffs =
                        instantEffects[actID][(currTS == Planner::E_AT_START ? 0 : 1)];
                    ...
                    if (ice)
                    {
                        ...
                        list<RPGBuilder::IntegralContinuousEffect>::const_iterator icItr =
                            defMapItr->second.begin();
                        const list<RPGBuilder::IntegralContinuousEffect>::const_iterator icEnd =
                            defMapItr->second.end();
                        ...
                        for (; icItr != icEnd; ++icItr)
                        {
                            ...
                            const list<pair<int, double>> &effs =
                                (currTS == Planner::E_AT_START ? icItr->getLeftStartEffects() :
                                                                 icItr->getLeftEndEffects());
                    ...
                    {
                        list<Literal *> &currPropEffs =
                            (currTS == Planner::E_AT_START ? RPGBuilder::getStartPropositionAdds()[actID] :
                                                             RPGBuilder::getEndPropositionAdds()[actID]);
...

                    {
                        ...
                        for (; unstItr != unstEnd; ++unstItr)
                        {
                            if (lpDebug & 1)
                            {
                                cout << "Variable " << *(RPGBuilder::getPNE(*unstItr))
                                     << " becomes unstable after an instantaneous effect\n";
                            }
...

                    {
                        ...
                        for (; sItr != sItrEnd; ++sItr, ++colIdx, ++constrIdx)
                        {
                            ...
                            if (sItr->second)
                            {
                                list<pair<int, RPGBuilder::LinearEffects::EffectExpression>> &geList =
                                    gradientEffects[actID][0];
                                list<pair<int, RPGBuilder::LinearEffects::EffectExpression>>::iterator geItr =
                                    geList.begin();
                                const list<pair<int, RPGBuilder::LinearEffects::EffectExpression>>::iterator
                                    geEnd = geList.end();
                ...
                if (currTS == Planner::E_AT_START)
                {
                    ...
                    if (lpDebug & 1024)
                    {
                        cout << COLOUR_light_green << "Recording invariants started at " << stepID
                             << COLOUR_default << endl;
                    }
                    recordVariablesInvolvedInThisStepsInvariants(constraints[actID][1],
                                                                 invariantsThisStepStartsOnVariableI[stepID]);
                }
                else if (currTS == Planner::E_AT_END)
                {
                ...
                if (currTS == Planner::E_AT_START && !RPGBuilder::getRPGDEs(actID).empty())
                {
                    ...
                    {
                        durAdder.setStartEnd(currVar,
                                             (currDuration->fixed.empty() ?
                                                  imaginaryMinMax[stepID].imaginaryMax :
                                                  imaginaryMinMax[stepID].imaginaryMin),
                                             VAL::E_LESSEQ);
                ...
                if (currTS == Planner::E_AT_START)
                {
                    ...
                    list<pair<int, RPGBuilder::LinearEffects::EffectExpression>>::iterator geItr =
                        geList.begin();
                    const list<pair<int, RPGBuilder::LinearEffects::EffectExpression>>::iterator geEnd =
                        geList.end();
                    for (; geItr != geEnd; ++geItr)
                    {
                        ...
                        if (currTracker.statusOfThisFluent != FluentTracking::FS_NORMAL)
                        {
                            if (lpDebug & 1)
                            {
                                cout << "Ignoring order-independent CTS effect on "
                                     << *(RPGBuilder::getPNE(geItr->first)) << endl;
                            }
                        ...
                        if (!durationIsFixed)
                        {
                            // If an action has a non-constant-valued duration AND a cts effect on a variable,
                            // then we need to mark that that variable is subject to duration-dependent
                            // change.  Otherwise, it will be considered stable once the action has ended;
                            // whereas, really, the time it ends at (and hence the cumulative effect) can
                            // vary.
                            if (lpDebug & 1)
                            {
                                cout << "Variable " << *(RPGBuilder::getPNE(geItr->first))
                                     << " becomes duration-dependent\n";
                            }
                        ...
                        {
                            cout << "Variable " << *(RPGBuilder::getPNE(geItr->first))
                                 << " becomes unstable due to CTS effect; gradient is now "
                                 << currTracker.activeGradient << "\n";
                        }
                    }
                }
                else if (currTS == Planner::E_AT_END)
                {
                    ...
                    for (; geItr != geEnd; ++geItr)
                    {
                        ...
                        if (currTracker.statusOfThisFluent == FluentTracking::FS_NORMAL)
                        {
                            if (!--(currTracker.activeGradientCount))
                            {
                                ...
                                if (!finalNumericVars[geItr->first].everHadADurationDependentEffect)
                                {
                                    ...
                                    {
                                        cout << "Variable " << *(RPGBuilder::getPNE(geItr->first))
                                             << " becomes stable next time: "
                                             << currTracker.activeGradientCount
                                             << " active gradients on it, and never a duration-dependent "
                                                "effect\n";
                                    }
                                }
                            }
                        ...
                        else if (currTracker.statusOfThisFluent == FluentTracking::FS_ORDER_INDEPENDENT)
                        {
                            ...
                            currTracker.orderIndependentValueTerms
                                .insert(make_pair(timestampVars[stepID], 0.0))
                                .first->second += geItr->second.constant;
                            currTracker.orderIndependentValueTerms
                                .insert(make_pair(timestampVars[correspondingStart[stepID]], 0.0))
                                .first->second -= geItr->second.constant;

                            if (lpDebug & 1)
                            {
                                cout << "Noting order-independent CTS effect on "
                                     << *(RPGBuilder::getPNE(geItr->first)) << endl;
                                cout << " = " << geItr->second.constant << " * ("
                                     << lp->getColName(timestampVars[stepID]);
                ...
                else
                {
                    for (; foItr != foEnd; ++foItr)
                    {
                        ...
                        if (!(--(fanIn[succ])))
                        {
                            ...
                            if (lpDebug & 512)
                            {
                                cout << "No predecessors remaining for " << succ
                                     << " - placing it on the open list\n";
                            }
                        }
                        else
                        {
                            if (lpDebug & 512)
                            {
                                cout << "Now only " << fanIn[succ] << " predecessors for step " << succ
                                     << endl;
                            }
                        }

                        // remembering to push forwards which invariants need to be checked at each successor
                        // step

                        activeAncestorsOfStep[succ].insert(ancestorsToPassOn.begin(),
                                                           ancestorsToPassOn.end());
        ...
        // Now to sort out 'now'.
        // First, find which variables are undergoing continuous change, and make a pair of placeholder
        // variables:
        // - one for the time at which we are going to read their value
        // - one containing the value itself
        //
        // This loop will add constraints for the latter, linking its value to the last thing that changed it,
        // and scheduling it after that After that, we add constraints to the former scheduling it before the
        // ends of open actions who are changing this variable
        ...
        for (int varID = 0; varID < numVars; ++varID)
        {
            ...
            // if no gradients are active, 'lastEffectValueVariable' is adequate as it can't have changed
            // since then
            if (!currTracker.activeGradientCount)
            {
                continue;
...

            {
                ...
                if (currTracker.lastEffectTimestampVariable == -1)
                {
                    ...
                    if (Globals::totalOrder)
                    {
                        ...
                        if (lpDebug & 1)
                        {
                            cout << "now for " << *(RPGBuilder::getPNE(varID)) << " must follow "
                                 << lp->getColName(totalOrderNowTimestamp) << endl;
                        }
                    }
                }
                else
                {
                    ...
                    if (!Globals::totalOrder)
                    {
                        ...
                        {
                            cout << "now for " << *(RPGBuilder::getPNE(varID)) << " must follow "
                                 << lp->getColName(currTracker.lastEffectTimestampVariable);
                        }
                    }
                    else
                    {
                        ...
                        {
                            cout << "now for " << *(RPGBuilder::getPNE(varID)) << " must follow "
                                 << lp->getColName(totalOrderNowTimestamp);
                        }
        ...
        for (int stepID = 0; stepID < tsVarCount; ++stepID)
        {
            ...
            if (currEvent.time_spec != Planner::E_AT_END)
            {
                continue;
            }
            if (currEvent.getEffects)
            {
                continue;  // if we don't get its effects yet, it must be the end of an open action
            }
...

        {
            ...
            for (; ceItr != ceEnd; ++ceItr)
            {
                ...
                for (; matchItr != matchEnd; ++matchItr)
                {
                        ...
                        for (; invItr != invEnd; ++invItr)
                        {
                            ...
                            if (LPScheduler::lpDebug & 1024)
                            {
                                ...
                                for (int s = 0; s < cSize; ++s)
                                {
                                    ...
                                    {
                                        cout << lp->getColName(finalNumericVars[(*invItr)->variables[s]]
                                                                   .lastEffectValueVariable);
                                    }
                                }
                            ...
                            for (int s = 0; s < cSize; ++s)
                            {
                                if ((*invItr)->variables[s] < 0)
                                {
                                    ...
                                    {
                                        std::cerr << "Fatal internal error: variable index "
                                                  << (*invItr)->variables[s] << " used\n";
                                        exit(1);
                                    }
                                }
                                else
                                {
                                    lastVar =
                                        finalNumericVars[(*invItr)->variables[s]].lastEffectValueVariable;
                                    if (lastVar >= 0)
                                    {
                                    ...
                                    else
                                    {
                                        fixedLHS +=
                                            (*invItr)->weights[s] *
                                            finalNumericVars[(*invItr)->variables[s]].postLastEffectValue;
                                    }
                                }
                            }
                    ...
                    /*if (!RPGBuilder::getSemaphoreFacts().empty() &&
                    !(constraints[(*matchItr)->actID][1].empty() &&
                    constraints[(*matchItr)->actID][2].empty())) {

                        set<int> blockedOtherEffectsOnVar;

                        {
                            const list<Literal*> & endAdds =
                    RPGBuilder::getEndPropositionAdds()[(*matchItr)->actID];
                            ...
                            for (int fID; aItr != aEnd; ++aItr) {
                                ...
                                const map<int, RPGBuilder::Guarded>::const_iterator gItr =
                    RPGBuilder::getSemaphoreFacts().find(fID); if (gItr !=
                    RPGBuilder::getSemaphoreFacts().end()) {

                                    // the end of this action will restore the ability to change to the given
                    variable
                        ...
                        if (!blockedOtherEffectsOnVar.empty()) {
                            ...
                            if (lpDebug & 1) {
                                cout << COLOUR_yellow << "The execution of " <<
                    *(RPGBuilder::getInstantiatedOp((*matchItr)->actID)) << " is blocking access to some
                    vars\n" << COLOUR_default;
                            }
                            // now see how many invariants or end conditions we can write in terms of
                    variables this action has exclusive effect-access to ConstraintSet checkInvs; map<int,int>
                    needVars;

                            for (int ppass = 1; ppass < 3; ++ppass) {
                                list<const Constraint*>::const_iterator invItr =
                    constraints[(*matchItr)->actID][ppass].begin(); const list<const
                    Constraint*>::const_iterator invEnd = constraints[(*matchItr)->actID][ppass].end();

                                for (; invItr != invEnd; ++invItr) {
                                    int s = 0;
                                    const int cSize = (*invItr)->weights.size();
                                    for (; s < cSize; ++s) {
                                        if (blockedOtherEffectsOnVar.find((*invItr)->variables[s]) ==
                    blockedOtherEffectsOnVar.end()) { break;
                                        }
                                    }
                                    if (s == cSize) {
...

                            {
                                ...
                                for (; nvItr != nvEnd; ++nvItr) {
                                    ...
                                    if (currTracker.activeGradientCount == 0 ||
                    fabs(currTracker.activeGradient) < 0.00001 ) {
                                        // if there are no gradients active, just take the var holding its
                    previous value nvItr->second = currTracker.lastEffectValueVariable;

                                        if (lpDebug & 1) {
                                            cout << "Future value of " << *(RPGBuilder::getPNE(nvItr->first))
                    << " is the same as the last value\n";
                                        ...
                                        if (lpDebug & 1) {
                                            cout << "Extrapolating future value of " <<
                    *(RPGBuilder::getPNE(nvItr->first)) << " based on active gradients\n";
                                        }

                                        // need to add a constraint to extrapolate the value of the variable
                    at the end of the action
                                        ...
                                        if (currTracker.lastEffectValueVariable != -1) {
                                            ...
                                            lp->addRow(entries, currTracker.postLastEffectValue,
                    currTracker.postLastEffectValue); // syntax for EQ prev

                                        }

                                        if (nameLPElements) {
                                            ostringstream namestream;
                                            namestream << "guard" <<  (*matchItr)->stepID + 1 << "delta-" <<
                    *(RPGBuilder::getPNE(nvItr->first)); string asString = namestream.str();
                                            lp->setRowName(lp->getNumRows() - 1 , asString);
                                        }
                                    }
                                }
                                    ...
                                    if (LPScheduler::lpDebug & 1024) {
                                        ...
                                        for (int s = 0 ; s < cSize; ++s) {
                                            ...
                                            if ((*invItr)->variables[s] == -3) {
                                                ...
                                                cout <<
                    lp->getColName(needVars.find((*invItr)->variables[s])->second);
                                            }
                                    ...
                                    for (int s = 0 ; s < cSize; ++s) {
                                        if ((*invItr)->variables[s] < 0) {
                                            if ((*invItr)->variables[s] == -3) {
                                                ...
                                                entries.push_back(make_pair(endStep, (*invItr)->weights[s]));
                                                entries.push_back(make_pair(startStep,
                    -(*invItr)->weights[s]));
                    ...
                    (*invItr)->variables[s] << " used\n"; exit(1);
                                            ...
                                            const map<int,int>::const_iterator nvItr =
                    needVars.find((*invItr)->variables[s]);

                                            assert(nvItr != needVars.end());

                                            if (nvItr->second == -1) {
                                                extraForLHS = (*invItr)->weights[s] *
                    finalNumericVars[(*invItr)->variables[s]].postLastEffectValue; } else {
                                                entries.push_back(make_pair(nvItr->second,
                    (*invItr)->weights[s])); if (assertions) assert(entries.back().second != 0.0);
                                            }
                                        }
                                    }

                                    lp->addRow(entries,
                                               ((*invItr)->lower != -DBL_MAX ? (*invItr)->lower - extraForLHS
                    : -LPinfinity),
                                               ((*invItr)->upper != DBL_MAX  ? (*invItr)->upper - extraForLHS
                    : LPinfinity ) );
...

            {
                ...
                for (; ceItr != ceEnd; ++ceItr)
                {
                    ...
                    for (; matchItr != matchEnd; ++matchItr)
                    {
                            ...
                            for (; orderItr != orderEnd; ++orderItr) {
                                ...
                                if (*orderItr == -1) {
                                    ...
                                    if (nameLPElements) {
                                        int constrIdx = lp->getNumRows() - 1;
                                        ostringstream namestream;
                                        namestream << imaginaryMinMax[(*matchItr)->stepID].imaginaryMin << "
                        ecb " << imaginaryMinMax[*orderItr].imaginaryMin; string asString = namestream.str();
                                        lp->setRowName(constrIdx, asString);
                                    }
                                }
                            }
                        ...
                        /*if (orderOptimised) {
                            ...
                            for (; orderItr != orderEnd; ++orderItr) {
                                ...
                                if (nameLPElements) {
                                    int constrIdx = lp->getNumRows() - 1;
                                    ostringstream namestream;
                                    namestream << imaginaryMinMax[(*matchItr)->stepID].imaginaryMin << " eca "
                        << imaginaryMinMax[*orderItr].imaginaryMin; string asString = namestream.str();
                                    lp->setRowName(constrIdx, asString);
                                }
                        ...
                        /*                    RPGBuilder::RPGDuration* currDuration =
                           RPGBuilder::getRPGDEs(actID)[(*matchItr)->divisionsApplied];
                                            ...
                                            if (currDuration->fixed) {
                                                ...
                                                for (int v = 0; v < vSize; ++v) {
                                                    ...
                                                    varScratch[2+v] =
                           fluentsAtStep[(*matchItr)->stepID][currDE->variables[v]];
                                                }
                                                weightScratch[0] = -1.0;
                                                weightScratch[1] = 1.0;

                                                const double rTerm = (currDE->constant == 0.0 ? 0.0 :
                           -currDE->constant); add_constraintex(lp, 2 + vSize, weightScratch, varScratch, GE,
                           rTerm);
                                                ++constrIdx;
                                                destList.push_back(pair<list<StartEvent>::iterator,
                           int>(*matchItr, constrIdx));
                           ...
                           -currDE->constant);
                                                ...
                                                add_constraintex(lp, 2 + vSize, weightScratch, varScratch, GE,
                           rTerm);
                                                ++constrIdx;
                                                destList.push_back(pair<list<StartEvent>::iterator,
                           int>(*matchItr, constrIdx)); } else {
                                                destList.push_back(pair<list<StartEvent>::iterator,
                           int>(*matchItr, -1));
                                            }
                        */
        ...
        if (lpDebug & 1)
        {
            cout << "LP complete, " << lp->getNumCols() << " columns, " << lp->getNumRows() << " rows\n";
        }
        ...
        if (!newDummySteps.empty())
        {
            if (lpDebug & 1)
            {
                cout << "Doing minimisations for dummy steps\n";
            }
            ...
            TemporalAnalysis::canSkipToEnd(planAsAVector[justAppliedStep]->action->getID()))
        {
            if (lpDebug & 1)
            {
                cout << "Doing minimisation for compression-safe end\n";
            }
            ...
            if (solved)
            {
                timestampToUpdateVarEndLB.insert(
                    make_pair(previousObjectiveVar - numVars, lp->getObjValue()));

                if (lpDebug & 1)
                {
                    cout << "- Earliest time is " << timestampToUpdateVarEndLB.find(optVar - numVars)->second
                         << ", recording as bound on " << lp->getColName(previousObjectiveVar) << endl;
                }
            }
            else
            {
        ...
        if (solved)
        {
            ...
            for (; ceItr != ceEnd; ++ceItr)
            {
                ...
                for (; matchItr != matchEnd; ++matchItr)
                {
                    ...
                    if (lpDebug & 1)
                    {
                        cout << "Doing minimisation for unfinished end of "
                             << *(RPGBuilder::getInstantiatedOp((*matchItr)->actID)) << endl;
                    }
                    ...
                    {
                        timestampToUpdateVarEndLB.insert(
                            make_pair(((*matchItr)->stepID + 1), lp->getObjValue()));

                        if (lpDebug & 1)
                        {
                            cout << "- Earliest time is "
                                 << timestampToUpdateVarEndLB.find(((*matchItr)->stepID + 1))->second
                                 << ", recording as bound on "
                                 << lp->getColName((*matchItr)->stepID + 1 + numVars) << endl;
                        }
                    }
                    else
                    {
        ...
        if (solved)
        {
            ...
            sortOutTheSolution(setObjectiveToMetric, justAppliedStep, preferenceStatus,
                               theState.prefPreconditionViolations, rpLowerBounds);
        }
        else
        {
            if (lpDebug)
    ...
    int LPScheduler::getVarForReachableCost(vector<AutomatonPosition::Position> &preferenceStatus,
                                            const double &preconditionPrefViolations)
    {
        ...
        for (; wItr != wEnd; ++wItr, ++vItr)
        {
            ...
            if (currFT.statusOfThisFluent == FluentTracking::FS_ORDER_INDEPENDENT)
            {
                ...
                if (!currFT.orderIndependentValueTerms.empty())
                {
                    if (lpDebug & 1)
                    {
                        cout << "Contribution to baseline current cost by " << *(RPGBuilder::getPNE(*vItr))
                             << " = " << currFT.orderIndependentValueConstant * *wItr
                             << " + some weighted sum\n";
                    }

                    map<int, double>::const_iterator oiTerms = currFT.orderIndependentValueTerms.begin();
                    const map<int, double>::const_iterator oiTermsEnd =
                        currFT.orderIndependentValueTerms.end();

                    for (; oiTerms != oiTermsEnd; ++oiTerms)
                    {
                        entries.insert(make_pair(oiTerms->first, 0.0)).first->second +=
                            oiTerms->second * *wItr;
                        if (lpDebug & 1)
                        {
                            cout << "\t\t+ " << lp->getColName(oiTerms->first) << " * "
                                 << oiTerms->second * *wItr << endl;
                        }
                    }
                }
                else
                {
                    ...
                    {
                        cout << "Contribution to baseline current cost by " << *(RPGBuilder::getPNE(*vItr))
                             << " = " << currFT.orderIndependentValueConstant * *wItr << endl;
                    }
                }
            }
            else
            {
                if (currFT.lastEffectValueVariable == -1)
                {
                    ...
                    {
                        cout << "Contribution to baseline current cost by " << *(RPGBuilder::getPNE(*vItr))
                             << " = " << currFT.postLastEffectValue * *wItr << endl;
                    }
                }
                else
                {
                    ...
                    {
                        cout << "Contribution to baseline current cost by " << *(RPGBuilder::getPNE(*vItr))
                             << " is determined by variable " << currFT.lastEffectValueVariable << endl;
                    }
                }
            }
        }
                ...
                for (; prefID < prefCount && prefItr != prefEnd; ++prefID)
                {
                    ...
                    if (currPrefCost == DBL_MAX)
                    {
                        if (prefID < prefItr->first)
                        {
                            ...
                            {
                                cout << "Constraint " << prefID
                                     << " excluded from contributing to cost: the plan has not interacted "
                                        "with it\n";
                            }
                        }
                        else
                    ...
                    else
                    {
                        if (prefID < prefItr->first)
                        {
                            if (PreferenceHandler::preferenceDebug && lpDebug & 1)
                            {
                                cout << "Contribution to baseline current pref cost by " << prefs[prefID].name
                                     << ":" << prefID << " = "
                                     << PreferenceHandler::getCurrentCost(prefID, preferenceStatus) << endl;
                            }
                            baselinePrefReachable +=
                                PreferenceHandler::getReachableCost(prefID, preferenceStatus);
                        }
                        else
                        {
                            if (prefItr->second.reachableCostVar >= 0)
                            {  // some preferences have zero reachable cost
                                entries.insert(make_pair(prefItr->second.reachableCostVar, 0.0))
                                    .first->second += currPrefCost;
                            }
                            ++prefItr;
                        }
                    }
        ...
        unavoidableReachablePlanCost = currentCostFromNonPreferenceParts + metric->constant +
                                       RPGBuilder::getPermanentViolationCost() + baselinePrefReachable;
    ...
    {
        {
            ...
            for (; factItr != factEnd; ++factItr)
            {
                ...
                if (!wasUsed)
                {
                    if (lpDebug & 1)
                    {
                        cout << COLOUR_yellow << "Abstract fact " << *(RPGBuilder::getLiteral(factItr->first))
                             << " - never used, so LB stays at ";
                ...
                {
                    planAsAVector[factItr->second.getBoundsStep()]->lpMinTimestamp =
                        factItr->second.getOccurrenceBounds()[occ].first;
                    planAsAVector[factItr->second.getBoundsStep()]->lpTimestamp =
                        factItr->second.getOccurrenceBounds()[occ].first;

                    if (lpDebug & 1)
                    {
                        cout << " so available from "
                             << planAsAVector[factItr->second.getBoundsStep()]->lpMinTimestamp << endl;
                    }
                }
    ...
    void LPScheduler::sortOutTheSolution(const bool &setObjectiveToMetric, const int &justAppliedStep,
                                         vector<AutomatonPosition::Position> &preferenceStatus,
                                         const double &preconditionPrefViolations,
                                         const map<int, double> &rpLowerBounds)
    {
        ...
        static const bool optimised = true;
        {
            ...
            for (int stepID = 0; stepID < loopLim; ++stepID)
            {
                ...
                if (lpDebug & 2 && !itr->isDummyStep())
                {
                    ...
                    if (itr->time_spec == Planner::E_AT_START)
                    {
                        cout << " start = ";
                    }
                    else
        ...
        /*if (lpDebug & 8 && !mutexCols.empty()) {
            cout << "Values of TIL mutex column variables:\n";
            {
                ...
                for (; mcItr != mcEnd; ++mcItr) {
                    cout << "\t" << lp->getColName(*mcItr) << " = " <<
        lp->getSingleSolutionVariableValue(*mcItr) << endl; assert(lp->isColumnBinary(*mcItr));
                }
            }
            ...
            cout << "Values of TIL mutex rows:\n";
            {
                ...
                for (; mcItr != mcEnd; ++mcItr) {
                    ...
                    cout << "\t" << lp->getRowName(*mcItr) << " = " << lp->getSingleSolutionRowValue(*mcItr)
        << endl;
                }
            }
        }*/
        ...
        if (Globals::optimiseSolutionQuality && RPGBuilder::getMetric())
        {
            ...
            for (; wItr != wEnd; ++wItr, ++vItr)
            {
                ...
                if (currFT.statusOfThisFluent == FluentTracking::FS_ORDER_INDEPENDENT)
                {
                    ...
                    if (!currFT.orderIndependentValueTerms.empty())
                    {
                        if (lpDebug & 1)
                        {
                            cout << "Contribution to baseline current cost by "
                                 << *(RPGBuilder::getPNE(*vItr)) << " = "
                                 << currFT.orderIndependentValueConstant * *wItr << " + some weighted sum\n";
                        }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            cout << "Contribution to baseline current cost by "
                                 << *(RPGBuilder::getPNE(*vItr)) << " is determined by variable "
                                 << currFT.lastEffectValueVariable << endl;
                        }
                    }
                }
            }
            ...
            if (!RPGBuilder::getPreferences().empty())
            {
                ...
                for (; ppMapItr != ppMapEnd; ++ppMapItr)
                {
                    ...
                    for (; ppVar != ppEnd; ++ppVar)
                    {
                        ...
                        if (prefTermDebug)
                        {
                            cout << "Set objective weight on " << lp->getColName(*ppVar) << " to " << weight
                                 << " due to precondition preference "
                                 << RPGBuilder::getPreferences()[ppMapItr->first].name << endl;
                        }
                    }
                }
                    ...
                    for (; prefID < prefCount && prefItr != prefEnd; ++prefID)
                    {
                        ...
                        if (currPrefCost == DBL_MAX)
                        {
                            if (prefID < prefItr->first)
                            {
                                ...
                                {
                                    cout << "Constraint " << prefID
                                         << " does not influence cost - the plan has not interacted with "
                                            "it\n";
                                }
                            }
                            else
                        ...
                        else
                        {
                            if (prefID < prefItr->first)
                            {
                                if (prefTermDebug || (PreferenceHandler::preferenceDebug && lpDebug & 1))
                                {
                                    cout << "Contribution to baseline current pref cost by "
                                         << prefs[prefID].name << ":" << prefID << " = "
                                         << PreferenceHandler::getCurrentCost(prefID, preferenceStatus)
                                         << endl;
                                    cout << "Contribution to baseline reachable pref cost by "
                                         << prefs[prefID].name << ":" << prefID << " = "
                                         << PreferenceHandler::getReachableCost(prefID, preferenceStatus)
                                         << endl;
                                }
                            ...
                            else
                            {
                                ...
                                {  // some preferences have zero reachable cost
                                    ...
                                    if (prefTermDebug)
                                    {
                                        cout << "Set objective weight on "
                                             << lp->getColName(prefItr->second.reachableCostVar) << " to "
                                             << prefs[prefID].cost << " due to preference "
                                             << prefs[prefID].name << ":" << prefID << endl;
                                    }
                                }
                                ++prefItr;
                            }
                    ...
                    for (; prefID < prefCount; ++prefID)
                    {
                        ...
                        if (currPrefCost != DBL_MAX)
                        {
                            if (prefTermDebug || (PreferenceHandler::preferenceDebug && lpDebug & 1))
                            {
                                ...
                                cout << "Contribution to baseline reachable pref cost by "
                                     << prefs[prefID].name << ":" << prefID << " = "
                                     << PreferenceHandler::getReachableCost(prefID, preferenceStatus) << endl;
                            }
                            baselinePrefReachable +=
                                PreferenceHandler::getReachableCost(prefID, preferenceStatus);
                            baselinePrefCurrent +=
            ...
            {
                reachablePlanCost = currentCostFromNonPreferenceParts + metric->constant +
                                    RPGBuilder::getPermanentViolationCost() + baselinePrefReachable;
                if (planCostsNeededAnLP)
                {
                    if (metric->minimise)
                ...
                if (lpDebug & 1)
                {
                    cout << COLOUR_light_cyan << "Calculated reachable plan cost as " << reachablePlanCost
                         << endl
                         << COLOUR_default;
                }
            }
            ...
            if (!RPGBuilder::getPreferences().empty())
            {
                ...
                for (; prefItr != prefEnd; ++prefItr)
                {
                    ...
                    if (prefItr->second.committedCostVar >= 0)
                    {
                        ...
                        {
                            cout << "Set objective weight on "
                                 << lp->getColName(prefItr->second.committedCostVar) << " to "
                                 << prefs[prefItr->first].cost << " due to preference "
                                 << prefs[prefItr->first].name << ":" << prefItr->first << endl;
                        }
                    }
                }
            }
            ...
            currentPlanCost = currentChoosablePlanCost + currentCostFromNonPreferenceParts +
                              RPGBuilder::getMetric()->constant + RPGBuilder::getPermanentViolationCost() +
                              baselinePrefCurrent;

            if (lpDebug & 1)
            {
                cout << COLOUR_light_cyan << "Calculated current plan cost as " << currentPlanCost << endl
                     << COLOUR_default;
            }
    ...
    void LPScheduler::removeExpiredAbstractFacts(StateFacts &facts)
    {
        ...
        for (; factItr != factEnd; ++factItr)
        {
            if (factItr->second.isNoLongerAvailable())
            {
                if (lpDebug & 1)
                {
                    cout << COLOUR_light_red << "Abstract fact " << *(RPGBuilder::getLiteral(factItr->first))
                         << " is no longer available, removing from state\n"
                         << COLOUR_default;
                }
            ...
            else
            {
                if (lpDebug & 1)
                {
                    cout << COLOUR_light_green << "Abstract fact "
                         << *(RPGBuilder::getLiteral(factItr->first)) << " stays in the state\n"
                         << COLOUR_default;
                }
    ...
    const LPScheduler::Constraint *LPScheduler::buildConstraint(RPGBuilder::RPGNumericPrecondition &pre)
    {
        if (lpDebug & 4)
        {
            cout << pre << " with op " << pre.op << " becomes:";
        }
        ...
        switch (pre.op)
        {
            case VAL::E_GREATER:
            {
                ...
                toReturn.lower =
                    pre.RHSConstant + 0.0001;  // HACK - some small value to fake greater-than using a GE
                if (lpDebug & 4)
                {
                    cout << " >= " << toReturn.lower;
                }
            ...
            case VAL::E_LESS:
            {
                ...
                toReturn.upper =
                    pre.RHSConstant - 0.0001;  // HACK - some small value to fake less-than using a LE
                if (lpDebug & 4)
                {
                    cout << " <= " << toReturn.upper;
                }
        ...
        {  // simple variable or duration &c
            ...
            if (lpDebug & 4)
            {
                cout << "Constraint on simple variable: \n";
            }
        }
        else
        {
            ...
            {  // negative variable
                ...
                if (lpDebug & 4)
                {
                    cout << "Constraint on negative variable: \n";
                }
                toReturn.weights = vector<double>(1);
                toReturn.variables = vector<int>(1);
    ...
    void LPScheduler::initialise()
    {
        ...
        initialGradients.resize(numVars, 0.0);  // set all initial gradients to zero, but have a look at
                                                // processes to refine that

        const map<int, double> &tickerVars = NumericAnalysis::getVariablesThatAreTickers();

        if (initDebug)
        {
            cout << "Initialising LP lookups for " << actCount << " actions\n";
        }
        ...
        for (int a = 0; a < actCount; ++a)
        {
            ...
            if (RPGBuilder::rogueActions[a] != RPGBuilder::OT_INVALID_ACTION)
            {
                ...
                if (discretisation)
                {
                    ...
                    {
                        vector<list<pair<int, RPGBuilder::LinearEffects::EffectExpression>>> &toFill =
                            gradientEffects[a] =
                                vector<list<pair<int, RPGBuilder::LinearEffects::EffectExpression>>>(
                                    divisions);
                        ...
                        for (int v = 0; v < varCount; ++v)
                        {
                            ...
                            {
                                toFill[d].push_back(pair<int, RPGBuilder::LinearEffects::EffectExpression>(
                                    currVar, discretisation->effects[d][v]));
                                if (RPGBuilder::rogueActions[a] == RPGBuilder::OT_PROCESS)
                                {
                                    initialGradients[currVar] += discretisation->effects[d][v].constant;
                                }
                            }
                            if (NumericAnalysis::getDominanceConstraints()[currVar] !=
                                    NumericAnalysis::E_METRICTRACKING &&
                                NumericAnalysis::getDominanceConstraints()[currVar] !=
                                    NumericAnalysis::E_IRRELEVANT)
                            {
                ...
                {
                    gradientEffects[a] =
                        vector<list<pair<int, RPGBuilder::LinearEffects::EffectExpression>>>(1);
                }
            }
            ...
            {
                {
                    vector<list<RPGBuilder::RPGNumericEffect *>> &toFill = instantEffects[a] =
                        vector<list<RPGBuilder::RPGNumericEffect *>>(2);

                    for (int pass = 0; pass < 2; ++pass)
                    {
                        list<int> &currList = (pass ? RPGBuilder::getEndEffNumerics()[a] :
                                                      RPGBuilder::getStartEffNumerics()[a]);
                        ...
                        for (; clItr != clEnd; ++clItr)
                        {
                            ...
                            if (initDebug)
                            {
                                if (iPoint == 2)
                                {
                                    cout << *(RPGBuilder::getInstantiatedOp(a)) << " end writes to "
                                         << *(RPGBuilder::getPNE(currEff.fluentIndex)) << "\n";
                                }
                                else
                                {
                                    cout << *(RPGBuilder::getInstantiatedOp(a)) << " start writes to "
                                         << *(RPGBuilder::getPNE(currEff.fluentIndex)) << "\n";
                                }
                            ...
                            for (int s = 0; s < currEff.size; ++s)
                            {
                                ...
                                else if (vv <= -2)
                                {
                                    ...
                                    if (NumericAnalysis::getDominanceConstraints()[currEff.fluentIndex] !=
                                            NumericAnalysis::E_IRRELEVANT &&
                                        NumericAnalysis::getDominanceConstraints()[currEff.fluentIndex] !=
                                            NumericAnalysis::E_METRICTRACKING)
                                    {
                ...
                const map<int, list<RPGBuilder::IntegralContinuousEffect>>::const_iterator iceItr =
                    RPGBuilder::getActionsToIntegralConditionalEffects().find(a);
                ...
                if (iceItr != RPGBuilder::getActionsToIntegralConditionalEffects().end())
                {
                    ...
                    list<vector<list<RPGBuilder::RPGNumericEffect *>>> &toFillOuter =
                        instantIntegralConditionalEffects[a];
                    ...
                    for (; icItr != icEnd; ++icItr)
                    {
                        ...
                        for (int pass = 0; pass < 2; ++pass)
                        {
                            const list<pair<int, double>> &currList =
                                (pass ? icItr->getLeftEndEffects() : icItr->getLeftStartEffects());
                            ...
                            for (; clItr != clEnd; ++clItr)
                            {
                                RPGBuilder::RPGNumericEffect &currEff =
                                    RPGBuilder::getNumericEff()[clItr->first];
                                ...
                                if (initDebug)
                                {
                                    if (iPoint == 2)
                                    {
                                        cout << *(RPGBuilder::getInstantiatedOp(a)) << " end ICE writes to "
                                             << *(RPGBuilder::getPNE(currEff.fluentIndex)) << "\n";
                                    }
                                    else
                                    {
                                        cout << *(RPGBuilder::getInstantiatedOp(a)) << " start ICE writes to "
                                             << *(RPGBuilder::getPNE(currEff.fluentIndex)) << "\n";
                                    }
                                    assert(interesting[a][iPoint].find(currEff.fluentIndex)->second);
                                }
                ...
                {
                    vector<list<const Constraint *>> &toFill = constraints[a] =
                        vector<list<const Constraint *>>(3);

                    for (int pass = 0; pass < 3; ++pass)
                    {
                        list<int> &currList = (pass ? (pass == 2 ? RPGBuilder::getEndPreNumerics()[a] :
                                                                   RPGBuilder::getInvariantNumerics()[a]) :
                                                      RPGBuilder::getStartPreNumerics()[a]);

                        /*if (pass == 1) {
                            cout << "In LP: " << *(RPGBuilder::getInstantiatedOp(a)) << " has " <<
                        currList.size() << " numeric invariants\n";
                        }*/
                        list<int>::iterator clItr = currList.begin();
                        const list<int>::iterator clEnd = currList.end();
                ...
                if (iceItr != RPGBuilder::getActionsToIntegralConditionalEffects().end())
                {
                    list<vector<pair<const Constraint *, const Constraint *>>> &toFillOuter =
                        instantIntegralConstraints[a];
                    ...
                    for (; icItr != icEnd; ++icItr)
                    {
                        ...
                        toFill[pass].first =
                            buildConstraint(RPGBuilder::getNumericPreTable()[icItr->getLeftPrecondition()]);
                        toFill[pass].second =
                            buildConstraint(RPGBuilder::getNumericPreTable()[icItr->getRightPrecondition()]);
...

                {
                    ...
                    const list<RPGBuilder::ConditionalEffect> &ceffs =
                        RPGBuilder::getActionsToConditionalEffects()[a];
                    ...
                    {
                        const list<pair<Literal *, Planner::time_spec>> &conds =
                            ceffItr->getPropositionalConditions();
                        ...
                        {
                            const list<pair<double, double>> *windows =
                                TemporalAnalysis::factIsVisibleInWindows(condItr->first);
                            ...
                            for (; wItr != wEnd; ++wItr)
                            {
                                ...
                                {
                                    pointsThatWouldBeMutexWithOptimisationTILs[a][0].push_back(
                                        wItr->second);  // start points cannot coincide with start conditions
                                                        // being deleted
                                }
                                else if (condItr->second == Planner::E_AT_END)
                                {
                                    pointsThatWouldBeMutexWithOptimisationTILs[a][1].push_back(
                                        wItr->second);  // end points cannot coincide with end conditions
                                                        // being deleted
                                }
                            }
                        }
        ...
        /*{
            ...
            for (; sfItr != sfEnd; ++sfItr) {
                ...
                for (; vItr != vEnd; ++vItr) {
                  list<pair<int, Planner::time_spec> >::const_iterator pItr =
        RPGBuilder::getEffectsToActions(sfItr->first).begin(); const list<pair<int, Planner::time_spec>
        >::const_iterator pEnd = RPGBuilder::getEffectsToActions(sfItr->first).end();
                  ...
                  for (; pItr != pEnd; ++pItr) {
                      if (pItr->second == Planner::E_AT_START) {
                          ...
                          std::cerr << "Fatal internal error: semaphores should only be manipulated by
        actions\n"; exit(1);
                      }
                  }
                }
...

        {
            ...
            for (; gItr != gEnd; ++gItr)
            {
                if (gItr->first != -1)
                {
                    goalConstraints.push_back(buildConstraint(RPGBuilder::getNumericPreTable()[gItr->first]));
                }
                if (gItr->second != -1)
                {
                    goalConstraints.push_back(
                        buildConstraint(RPGBuilder::getNumericPreTable()[gItr->second]));
                }
            }
        }
    ...
    void LPScheduler::addConstraintsForTILMutexes(const int &timestampVar,
                                                  const vector<double> &mutexTimestamps)
    {
        if (mutexTimestamps.empty())
        {
            return;
        }
        ...
        const pair<double, double> tsBounds =
            make_pair(lp->getColLower(timestampVar), lp->getColUpper(timestampVar));
    ...
    void LPScheduler::updateStateFluents(vector<double> &min, vector<double> &max,
                                         vector<double> &timeAtWhichValueIsDefined)
    {
        ...
        for (int s = 0; s < numVars; ++s)
        {
            if (tickers.find(s) != tickers.end())
            {
                if (lpDebug & 1)
                {
                    cout << "New bounds on " << *(RPGBuilder::getPNE(s))
                         << ": it's a ticker, so bounded in the range 0 to infinity\n";
                }
                min[s] = 0.0;
                max[s] = DBL_MAX;
                continue;
            ...
            if (finalNumericVars[s].statusOfThisFluent == FluentTracking::FS_ORDER_INDEPENDENT)
            {
                if (finalNumericVars[s].orderIndependentValueTerms.empty())
                {
                    ...
                    {
                        cout << "New bounds on " << *(RPGBuilder::getPNE(s))
                             << ", an order-independent variable are constant: "
                             << finalNumericVars[s].orderIndependentValueConstant << endl;
                    }
                ...
                else
                {
                    ...
                    {
                        cout << "New bounds on " << *(RPGBuilder::getPNE(s))
                             << ", an order-independent variable, evaluate to: ";
                    }
                    ...
                    map<int, double>::const_iterator oiTerms =
                        finalNumericVars[s].orderIndependentValueTerms.begin();
                    const map<int, double>::const_iterator oiTermsEnd =
                        finalNumericVars[s].orderIndependentValueTerms.end();
                     ...
                     (finalNumericVars[s].statusOfThisFluent == FluentTracking::FS_NORMAL))
            {
                if (lpDebug & 1)
                {
                    cout << "New bounds on " << *(RPGBuilder::getPNE(s)) << ", column "
                         << lp->getColName(finalNumericVars[s].lastEffectValueVariable) << " were [" << min[s]
                         << "," << max[s] << "] now:";
                }
                ...
                if (workOutFactLayerZeroBoundsStraightAfterRecentAction)
                {
                    if (!finalNumericVars[s].everHadADurationDependentEffect)
                    {
                        ...
                        pair<map<int, double>::iterator, bool> knownMin =
                            knownMinValueOfColumn.insert(make_pair(relevantTSVar, 0.0));
                        ...
                        if (knownMin.second)
                        {
                            ...
                            if (lpDebug & 16)
                            {
                                cout << "First, minimising " << lp->getColName(previousObjectiveVar)
                                     << "; old col upper was " << oldColUpper << endl;
                            }
                            ...
                            if (optimised)
                            {
                                // since we minimised the value of this variable, we can use this as its lower
                                // bound from hereon
                                lp->setColLower(
                                    previousObjectiveVar,
                                    EpsilonResolutionTimestamp(knownMin.first->second, true).toDouble());
                            }
                        }
                        else
                        {
                        ...
                        timeAtWhichValueIsDefined[s] =
                            EpsilonResolutionTimestamp(knownMin.first->second, true).toDouble();

                        // temporary additional constraint to make sure now is as early as it can be
                        if (lpDebug & 16)
                        {
                            cout << "Never had a duration-dependent effect on variable, so using new method "
                                    "- limiting "
                                 << lp->getColName(relevantTSVar) << " to " << knownMin.first->second
                                 << " instead of " << oldColUpper << endl;
                        }
                ...
                if (nonDDAmethod)
                {
                    ...
                    {
                        cout << "Reset upper on "
                             << lp->getColName(finalNumericVars[s].lastEffectTimestampVariable) << " to "
                             << oldColUpper << endl;
                    }
                    if (lpDebug & 16)
                    {
                    ...
                    if (lpDebug & 1)
                    {
                        cout << " [" << mvTwo << "," << mv << "] from t=" << timeAtWhichValueIsDefined[s]
                             << " onwards\n";
                    }
                    ...
                    if (Globals::totalOrder)
                    {
                        ...
                        if (lpDebug & 1)
                        {
                            cout << " ** Using a total-order, so pretending is defined from "
                                 << timeAtWhichValueIsDefined[s] << endl;
                        }
                    }
                }
                else
            ...
            else
            {
                if (lpDebug & 1)
                {
                    cout << "Skipping updating bounds on " << *(RPGBuilder::getPNE(s)) << ", remain at ["
                         << min[s] << "," << max[s] << "]\n";
                }
            }
        }
    ...
    void LPScheduler::extrapolateBoundsAfterRecentAction(const list<StartEvent> *startEventQueue,
    ...
    int LPScheduler::getDummyVarForMakespan()
    {
        ...
        {
            if (!planAsAVector[s]->isDummyStep() && ((planAsAVector[s]->time_spec == Planner::E_AT_START) ||
                                                     (planAsAVector[s]->time_spec == Planner::E_AT_END)))
            {
    ...
    bool LPScheduler::isSolution(const MinimalState &state, list<FFEvent> &header, list<FFEvent> &now)
    {
        ...
        if (!lp)
        {
            ...
            for (; ngItr != ngEnd; ++ngItr)
            {
                if (ngItr->first != -1)
                {
                    const RPGBuilder::RPGNumericPrecondition &currNP =
                        RPGBuilder::getNumericPreTable()[ngItr->first];
                    if (!currNP.isSatisfiedWCalculate(state.secondMin, state.secondMax))
                    {
                        return false;
                    }
                }
                ...
                {
                    const RPGBuilder::RPGNumericPrecondition &currNP =
                        RPGBuilder::getNumericPreTable()[ngItr->second];
                    if (!currNP.isSatisfiedWCalculate(state.secondMin, state.secondMax))
                    {
                        return false;
                    }
        ...
        for (; gItr != gEnd; ++gItr)
        {
            ...
            for (int s = 0; s < cSize; ++s)
            {
                ...
                else
                {
                    if (lpDebug & 1)
                    {
                        cout << "Value of " << (*gItr)->variables[s] << " is constant: "
                             << finalNumericVars[(*gItr)->variables[s]].postLastEffectValue << endl;
                    }

                    offset +=
                        ((*gItr)->weights[s] * finalNumericVars[(*gItr)->variables[s]].postLastEffectValue);
                }
            }
            if (entries.empty())
            {
                ...
                else
                {
                    if (lpDebug & 1)
                    {
                        cout << "Goal constraint evaluates to " << offset << ", which is in range ["
                             << (*gItr)->lower << "..\n";
                    }
                }
            }
        ...
        if (Globals::optimiseSolutionQuality && RPGBuilder::getMetric())
        {
            ...
            if (planCostsWereCalculated && planCostsNeededAnLP)
            {
                ...
                {
                    cout << COLOUR_light_magenta
                         << "Plan costs were calculated, and needed an LP - first solve to minimise cost\n"
                         << COLOUR_default;
                }
                ...
                for (; wItr != wEnd; ++wItr, ++vItr)
                {
                    ...
                    if (currFT.statusOfThisFluent == FluentTracking::FS_ORDER_INDEPENDENT)
                    {
                        ...
                        for (; oiTerms != oiTermsEnd; ++oiTerms)
                        {
                            objective.insert(make_pair(oiTerms->first, 0.0)).first->second +=
                                oiTerms->second * *wItr;
                        }
                    }
                    else if (currFT.lastEffectValueVariable != -1)
                    {
                        objective.insert(make_pair(currFT.lastEffectValueVariable, 0.0)).first->second +=
                            *wItr;
                    }
                }
...

                {
                    ...
                    for (; prefItr != prefEnd; ++prefItr)
                    {
                        if (prefItr->second.committedCostVar >= 0)
                        {
                            objective.insert(make_pair(prefItr->second.committedCostVar, 0.0))
                                .first->second += RPGBuilder::getPreferences()[prefItr->first].cost;
                        }
                    }
                }
                ...
                if (success)
                {
                    ...
                    if (metric->minimise)
                    {
                        lp->addRow(primaryObjectiveRow, -LPinfinity,
                                   (ceil(costLimit / Globals::numericTolerance) + 100) *
                                       Globals::numericTolerance);
                    }
                    else
                    {
                        lp->addRow(primaryObjectiveRow,
                                   (floor(costLimit / Globals::numericTolerance) - 100) *
                                       Globals::numericTolerance,
                                   LPinfinity);
                    }
        ...
        if (!success)
        {
            if (mustSucceed)
            {
                std::cerr << "Warning: numeric precision issues with the LP when optimising the cost of a "
                             "goal state\n";
                std::cerr << "Plan will not be cost-then-makespan-optimal\n";
                if (lpDebug & 1)
                {
                    cout << COLOUR_light_red << "Goal not reached, but pretending it was due to precision\n"
                         << COLOUR_default;
                }
                toReturn = true;
            }
        ...
        else
        {
            if (lpDebug & 1)
            {
                cout << "Goal reached by " << header.size() << " header steps along with " << now.size()
                     << " more\n";
            }
    ...
    bool LPScheduler::addAnyNumericConstraints(const list<pair<int, Planner::time_spec>> &numericConditions,
                                               const int &actStartAt, const int &actEndAt,
                                               list<int> &conditionVars)
    {
        ...
        {
            const RPGBuilder::RPGNumericPrecondition &currPre =
                RPGBuilder::getNumericPreTable()[condItr->first];
            ...
            if (localVar >= 2 * pneCount)
            {
                ...
                if (currAV.size != 1)
                {
                    cout << "Ignoring conditional effect dependent on " << currPre
                         << " for now, as is size other than 1\n";
                    return false;
                }
            ...
            if (currPre.LHSVariable < 0)
            {
                switch (currPre.LHSVariable)
                {
                    ...
                    default:
                    {
                        cout << "Ignoring conditional effect dependent on special var " << currPre
                             << " for now\n";
                        return false;
                    }
                }
            ...
            else
            {
                ...
                if (localVar < pneCount)
                {
                    ...
                    {
                        cout << "Ignoring conditional effect dependent on " << currPre << " for now, as "
                             << localVar << " isn't a ticker\n";
                        return false;
                    }
                }
                else
                {
                    ...
                    {
                        cout << "Ignoring conditional effect dependent on " << currPre << " for now, as "
                             << localVar - pneCount << " isn't a ticker\n";
                        return false;
                    }
                }
            }
...

            {
                ...
                switch (op)
                {
                    ...
                    default:
                    {
                        std::cerr << "Internal error: = precondition should have been preprocessed into a "
                                     ">=, <= pair\n";
                        exit(1);
                    }
                }
    ...
    bool LPScheduler::addAnyTimeWindowConstraints(
        const list<pair<Literal *, Planner::time_spec>> &propositionalConditions, const int &actStartAt,
        const int &actEndAt, list<int> &conditionVars)
    {
        static const vector<pair<int, double>> emptyEntries(0);
        static const bool debug = (Globals::globalVerbosity & 32);

        list<pair<Literal *, Planner::time_spec>>::const_iterator condItr = propositionalConditions.begin();
        const list<pair<Literal *, Planner::time_spec>>::const_iterator condEnd =
            propositionalConditions.end();
    ...
    bool LPScheduler::scheduleToMetric()
    {
        ...
        {
            cout << "; Warning: metric is just to optimise makespan, so post-hoc optimisation is redundant "
                    "unless being used as a partial-order lifter\n";
        }
        ...
        for (int t = 0; t < termCount; ++t, ++vItr, ++wItr)
        {
            ...
            if (varInfo.statusOfThisFluent == FluentTracking::FS_IGNORE)
            {
                continue;
            }
            ...
            if (varInfo.statusOfThisFluent == FluentTracking::FS_NORMAL)
            {
                if (lpDebug & 1)
                {
                    cout << *(RPGBuilder::getPNE(currVar)) << " is a normal variable, adding " << *wItr
                         << " times it to the objective\n";
                }
            ...
            else
            {
                if (lpDebug & 1)
                {
                    cout << *(RPGBuilder::getPNE(currVar))
                         << " is an order-independent metric variable, adding terms to objective:\n\t";
                }
        ...
        for (int act = 0; act < actCount; ++act)
        {
            ...
            const list<RPGBuilder::ConditionalEffect> &condEffs =
                RPGBuilder::getActionsToConditionalEffects()[currEvent->action->getID()];
            if (condEffs.empty())
            {
                continue;
            }
            ...
            const int actEndAt = (RPGBuilder::getRPGDEs(currEvent->action->getID()).empty() ?
                                      -1 :
                                      timestampVars[currEvent->pairWithStep]);
            ...
            for (int cep = 0; ceItr != ceEnd; ++ceItr, ++cep)
            {
                ...
                if (lpDebug & 1)
                {
                    cout << "Considering conditional effect " << cep << " on step " << act << ", "
                         << *(currEvent->action) << " start\n";
                    cout << "Start step var: " << actStartAt << ", end var: " << actEndAt << endl;
                }
                const bool success = addAnyTimeWindowConstraints(ceItr->getPropositionalConditions(),
                                                                 actStartAt, actEndAt, conditionVars);
                ...
                const bool success2 = addAnyNumericConstraints(ceItr->getNumericPreconditions(), actStartAt,
                                                               actEndAt, conditionVars);
                ...
                assert(colRepresentingAllConstraintsSatisfied >= 0 &&
                       colRepresentingAllConstraintsSatisfied < lp->getNumCols());
                ...
                MILPSolver::Objective::Coefficient &overallObjectiveWeightOfEffect =
                    newObjective.getTerm(colRepresentingAllConstraintsSatisfied);
                ...
                for (; effItr != effEnd; ++effItr)
                {
                    ...
                    map<int, double>::const_iterator wItr =
                        mapVariableToObjectiveWeight.find(currEff.fluentIndex);
                    ...
                    {
                        cout << "Warning: ignoring conditional metric effect " << currEff
                             << ", as for now only increase/decrease metric conditional effects are "
                                "supported\n";
                        continue;
                    }

                    {
                        ...
                        {
                            cout << "Warning: ignoring conditional metric effect " << currEff
                                 << ", as for now the conditional effects can only be in terms of constants, "
                                    "?duration, or tickers\n";
                            continue;
                        }
                    }
                    ...
                    for (int s = 0; s < currEff.size; ++s)
                    {
                        switch (currEff.variables[s])
                        {
                            case -3:
                            {
                                overallObjectiveWeightOfEffect.nonLinearCoefficients
                                    .insert(make_pair(actEndAt, 0.0))
                                    .first->second += (currEff.weights[s] * wItr->second);
                                overallObjectiveWeightOfEffect.nonLinearCoefficients
                                    .insert(make_pair(actStartAt, 0.0))
                                    .first->second -= (currEff.weights[s] * wItr->second);
                                break;
                            }
                            case -19:
                            ...
                            default:
                            {
                                ...
                                {
                                    const map<int, double>::const_iterator tvItr =
                                        tickerVars.find(currEff.variables[s]);
                                    ...
                                    if (effItr->second == Planner::E_AT_START)
                                    {
                                        overallObjectiveWeightOfEffect.nonLinearCoefficients
                                            .insert(make_pair(actStartAt, 0.0))
                                            .first->second +=
                                            (currEff.weights[s] * wItr->second) * tvItr->second;
                                    }
                                    else
                                    {
                                        overallObjectiveWeightOfEffect.nonLinearCoefficients
                                ...
                                {
                                    const map<int, double>::const_iterator tvItr =
                                        tickerVars.find(currEff.variables[s] - pneCount);
                                    assert(tvItr != tickerVars.end());
                                    if (effItr->second == Planner::E_AT_START)
                                    {
                                        overallObjectiveWeightOfEffect.nonLinearCoefficients
                ...
                if (colRepresentingAllConstraintsSatisfied != -1)
                {
                    ...
                    {
                        cout << "Adding term to objective: "
                             << overallObjectiveWeightOfEffect.linearCoefficient << "*"
                             << lp->getColName(colRepresentingAllConstraintsSatisfied) << endl;
                    }
                    if (!overallObjectiveWeightOfEffect.nonLinearCoefficients.empty())
                    {
                        if (Globals::globalVerbosity & 32)
                        {
                            cout << "Adding quadratic terms to objective: "
                                 << lp->getColName(colRepresentingAllConstraintsSatisfied) << " * (";
                            map<int, double>::const_iterator qtItr =
                                overallObjectiveWeightOfEffect.nonLinearCoefficients.begin();
                            const map<int, double>::const_iterator qtEnd =
                                overallObjectiveWeightOfEffect.nonLinearCoefficients.end();
        ...
        if (Globals::globalVerbosity & 32)
        {
            cout << "Set objective to minimise makespan variable " << lp->getColName(variableForMakespan)
                 << endl;
            lp->writeLp("final.lp");
        }
    ...
    void LPScheduler::pushTimestampToMin(vector<AutomatonPosition::Position> &preferenceStatus,
                                         const map<int, double> &csEndLowerBound,
                                         const map<int, double> &rpLowerBounds)
    {
        if (lpDebug & 1)
        {
            cout << COLOUR_light_red << "Min timestamp of new step is now " << timestampToUpdate->lpTimestamp
                 << ", rather than " << timestampToUpdate->lpMinTimestamp << COLOUR_default << endl;
        }
        ...
        {
            {
                ...
                for (; dsItr != dsEnd; ++dsItr)
                {
                    ...
                    {
                        std::cerr << "Internal error: the solution found by the LP should not violate the "
                                     "temporal constraints used by BF\n";
                        exit(1);
                    }
                }
            }
            ...
            {
                std::cerr << "Internal error: the solution found by the LP should not violate the temporal "
                             "constraints used by BF\n";
                exit(1);
            }
            ...
            lp->setColLower(timestampToUpdateVar,
                            EpsilonResolutionTimestamp(timestampToUpdate->lpMinTimestamp, true).toDouble());

            if (lpDebug & 1)
            {
                cout << "CD: Set lower bound on " << lp->getColName(timestampToUpdateVar) << " to "
                     << timestampToUpdate->lpMinTimestamp << ", is now "
                     << lp->getColLower(timestampToUpdateVar) << endl;
            }

            if (timestampToUpdatePartner)
            {
                lp->setColLower(
                    timestampToUpdatePartnerVar,
                    EpsilonResolutionTimestamp(timestampToUpdatePartner->lpMinTimestamp, true).toDouble());
            }

            {
                ...
                for (; dsItr != dsEnd; ++dsItr)
                {
                    const EpsilonResolutionTimestamp rounded(planAsAVector[dsItr->first]->lpMinTimestamp,
                                                             true);
                    ...
                    if (lpDebug & 1)
                    {
                        cout << "CD: Set lower bound on " << lp->getColName(dsItr->second.first) << " to "
                             << planAsAVector[dsItr->first]->lpMinTimestamp << ", is now "
                             << lp->getColLower(dsItr->second.first) << endl;
                    }
                }
            }
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                ...
                for (; rplItr != rplEnd; ++rplItr)
                {
                    const EpsilonResolutionTimestamp rounded(planAsAVector[rplItr->first]->lpMinTimestamp,
                                                             true);
                    lp->setColLower(rplItr->first + numVars, rounded.toDouble());

                    if (lpDebug & 1)
                    {
                        cout << "CD: Set lower bound on " << lp->getColName(rplItr->first + numVars) << " to "
                             << planAsAVector[rplItr->first]->lpMinTimestamp << ", is now "
                             << lp->getColLower(rplItr->first + numVars) << endl;
                    }
                }
            }
        }
        else
        {
            ...
            if (lpDebug & 1)
            {
                cout << "Set lower bound on " << lp->getColName(timestampToUpdateVar) << " to "
                     << timestampToUpdate->lpMinTimestamp << endl;
            }
            ...
            if (timestampToUpdatePartner)
            {
                if (timestampToUpdate->time_spec == Planner::E_AT_END)
                {
                    ...
                    if (maxDur < newMin)
                    {
                        ...
                        if (newMin.toDouble() > oldMin + 0.0005)
                        {
                            if (lpDebug & 1)
                            {
                                cout << COLOUR_light_red << "Min timestamp of corresponding start is now "
                                     << newMin << " rather than " << oldMin << COLOUR_default << endl;
                            }
                ...
                else
                {
                    ...
                    if (newMin.toDouble() > oldMin + 0.0005)
                    {
                        if (lpDebug & 1)
                        {
                            cout << COLOUR_light_red << "Min timestamp of corresponding end is now " << newMin
                                 << " rather than " << oldMin << COLOUR_default << endl;
                        }
...

            {
                ...
                for (; dsItr != dsEnd; ++dsItr)
                {
                    ...
                    if (lpDebug & 1)
                    {
                        cout << "DS: Set lower bound on " << lp->getColName(dsItr->second.first) << " to "
                             << planAsAVector[dsItr->first]->lpMinTimestamp << ", is now "
                             << lp->getColLower(dsItr->second.first) << endl;
                    }
                }
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                ...
                for (; rplItr != rplEnd; ++rplItr)
                {
                    ...
                    if (lpDebug & 1)
                    {
                        cout << "(csrp:" << pass << ") Set lower bound on "
                             << lp->getColName(rplItr->first + numVars) << " to "
                             << planAsAVector[rplItr->first]->lpMinTimestamp << endl;
                    }
                    ...
                    if (planAsAVector[rplItr->first]->action)
                    {
                        ...
                        if (!RPGBuilder::getRPGDEs(actID).empty())
                        {
                            ...
                            {
                                EpsilonResolutionTimestamp newMin(
                                    planAsAVector[rplItr->first]->lpMinTimestamp, true);
                                EpsilonResolutionTimestamp maxDur(planAsAVector[rplItr->first]->maxDuration,
                                                                  true);
                                if (maxDur < newMin)
                                {
                                    newMin -= maxDur;
                                    ...
                                    if (newMin.toDouble() > oldMin + 0.0005)
                                    {
                                        if (lpDebug & 1)
                                        {
                                            cout << COLOUR_light_red
                                                 << "Min timestamp of corresponding start is now " << newMin
                                                 << " rather than " << oldMin << COLOUR_default << endl;
                                        }
                            ...
                            else
                            {
                                EpsilonResolutionTimestamp newMin(
                                    planAsAVector[rplItr->first]->lpMinTimestamp, true);
                                newMin += EpsilonResolutionTimestamp(
                                    planAsAVector[rplItr->first]->minDuration, true);
                                double &oldMin = planAsAVector[rplItr->first + 1]->lpMinTimestamp;
                                if (newMin.toDouble() > oldMin + 0.0005)
                                {
                                    if (lpDebug & 1)
                                    {
                                        cout << COLOUR_light_red
                                             << "Min timestamp of corresponding end is now " << newMin
                                             << " rather than " << oldMin << COLOUR_default << endl;
                                    }
    ...
    bool LPScheduler::addRelaxedPlan(const list<StartEvent> &seq,
                                     vector<AutomatonPosition::Position> &preferenceStatus,
                                     double &preconditionPrefViolations, list<FFEvent> &header,
                                     list<FFEvent> &now,
                                     const list<pair<double, list<ActionSegment>>> &relaxedPlan,
                                     const int &justAppliedStep)
    {
        ...
        if (timestampToUpdateVar == -1)
        {
            if (localDebug)
            {
                cout << "addRelaxedPlan(): No timestamp to update var, so doing nothing with the refined "
                        "bounds on action ends\n";
            }
            return true;
        }
            ...
            if (!seq.empty())
            {
                ...
                for (; seqItr != seqEnd; ++seqItr)
                {
                    ...
                    if (lp->getColLower(actEndVar) < rounded.toDouble())
                    {
                        if (localDebug)
                        {
                            cout << "\tExisting bound of " << lp->getColLower(actEndVar) << " on "
                                 << *(RPGBuilder::getInstantiatedOp(seqItr->actID))
                                 << " is increased by the RP bound of " << endTime->second << endl;
                        }
                    ...
                    else
                    {
                        if (localDebug)
                        {
                            cout << "\tExisting bound of " << lp->getColLower(actEndVar) << " on "
                                 << *(RPGBuilder::getInstantiatedOp(seqItr->actID))
                                 << " is no worse than the RP bound of " << endTime->second << endl;
                        }
                    }
                }
            }
                ...
                for (; rpItr != rpEnd; ++rpItr)
                {
                    ...
                    for (; asItr != asEnd; ++asItr)
                    {
                        if (asItr->second != Planner::E_AT_START)
                        {
                            ...
                            const int divMatch =
                                (asItr->second == Planner::E_OVER_ALL ? asItr->divisionID :
                                                                        gradientEffects[actID].size() - 1);
                                ...
                                if (ceItr != compulsaryEnds.end())
                                {
                                    ...
                                    for (; matchItr != matchEnd; ++matchItr)
                                    {
                                        if (matchItr->first->divisionsApplied == divMatch)
                                        {
                                            ...
                                            if (lpDebug & 1)
                                            {
                                                cout << "Changed RHS of timestamp for " << *(asItr->first)
                                                     << " from " << tmp << " to " << tmp + offset << "\n";
                                            }
        ...
        if (recalculate && timestampToUpdateVar != -1)
        {
            if (lpDebug & 1)
            {
                cout << "Recalculating timestamps following relaxed plan\n";
            }
            ...
            if (success)
            {
                sortOutTheSolution(false, justAppliedStep, preferenceStatus, preconditionPrefViolations,
                                   rpLowerBounds);
            }
        }
        else
        {
            if (lpDebug & 1)
            {
                cout << "No need to recalculate timestamps following relaxed plan\n";
            }
        }
    ...
    ParentData *LPScheduler::prime(list<FFEvent> &header, const TemporalConstraints *const cons,
                                   list<StartEvent> &open, const bool &includeMetric)
    {
        if (!hybridBFLP)
        {
            return 0;
...

        {
            ...
            for (i = 0; hItr != hEnd; ++hItr, ++i)
            {
                ...
                if (hItr->time_spec == Planner::E_DUMMY_TEMPORAL_TRIGGER_TRUE)
                {
                    if (hItr->pairWithStep >= 0)
                    {
                        ...
                        if (pref.cons == VAL::E_ALWAYSWITHIN)
                        {
                            ...
                            {
                                toReturn->makeSoftEdgeListFor(i).addPredecessor(hItr->pairWithStep, false,
                                                                                -pref.deadline);
                                toReturn->makeSoftEdgeListFor(hItr->pairWithStep)
                                    .addFollower(i, false, -pref.deadline);
                            }
                            else
                            {
                                toReturn->makeEdgeListFor(hardEdgesForVertex)
                                    .addPredecessor(hItr->pairWithStep, false, -pref.deadline);
                                toReturn->makeEdgeListFor(hItr->pairWithStep)
                                    .addFollower(hardEdgesForVertex, false, -pref.deadline);
                            }
                        }
                    }
                }
                ...
                for (; ntfItr != ntfEnd; ++ntfItr)
                {
                    if (ntfItr->first == exclude)
                    {
                        continue;
                    }
                    if (ntfItr->first == localEdgesForVertex)
                    {
                        continue;
                    }
                    if ((earlier = toReturn->edgesAreNonSoft(ntfItr->first)) != -1)
                    {
                        if (hardEdgesForVertex == -1)
                        {
                            if (primeDebug)
                            {
                                cout << "\t\tStep " << localEdgesForVertex
                                     << " soft-needs to start after step " << earlier << "\n";
                            }
                            toReturn->makeSoftEdgeListFor(earlier).addFollower(
                                localEdgesForVertex, ntfItr->second, (ntfItr->second ? EPSILON : 0.0));
                        }
                        else
                        {
                            if (primeDebug)
                            {
                                cout << "\t\tStep " << hardEdgesForVertex << " needs to start after step "
                                     << earlier << "\n";
                            }
                            toReturn->makeEdgeListFor(earlier).addFollower(hardEdgesForVertex, ntfItr->second,
                                                                           (ntfItr->second ? EPSILON : 0.0));
                        }
                        forThisStep->addPredecessor(earlier, ntfItr->second,
                                                    (ntfItr->second ? EPSILON : 0.0));
                    }
                    else
                    {
                        ...
                        if (primeDebug)
                        {
                            cout << "\t\tStep " << localEdgesForVertex << " soft-needs to start after step "
                                 << ntfItr->first << "\n";
                        }

                        toReturn->makeSoftEdgeListFor(ntfItr->first)
                            .addFollower(localEdgesForVertex, ntfItr->second,
                                         (ntfItr->second ? EPSILON : 0.0));
                        forThisStepSoft->addPredecessor(ntfItr->first, ntfItr->second,
                                                        (ntfItr->second ? EPSILON : 0.0));
                    }
                }
            }
        ...
        for (; seqItr != seqEnd; ++seqItr)
        {
            ...
            if (mostRecentStep != -1)
            {
                if (mostRecentStep != startWasAt)
                {
                    if (primeDebug)
                    {
                        cout << "TO: Insisting that an unfinished action at " << i << " must follow "
                             << mostRecentStep << ", the most recent step\n";
                    }
            ...
            {
                {
                    ...
                    for (; ecaItr != ecaEnd; ++ecaItr)
                    {
                        ...
                        if (af >= 0)
                        {
                            if (primeDebug)
                            {
                                cout << "\t\tEnd of " << seqItr->stepID << " comes after end of " << af
                                     << ", therefore...\n";
                            }
...

                {
                    ...
                    for (; ecbItr != ecbEnd; ++ecbItr)
                    {
                        ...
                        if (af >= 0)
                        {
                            if (primeDebug)
                            {
                                cout << "\t\tEnd of " << seqItr->stepID << " comes before end of " << af
                                     << ", therefore...\n";
                            }
                            const int afw = toReturn->getPairWith()[af];
        ...
        if (Globals::paranoidScheduling)
        {
            ...
            for (int m = 1; hItr != hEnd; ++hItr, ++m)
            {
                ...
                if (hItr->isDummyStep())
                {
                    ...
                    if (noisy)
                    {
                        cout << "Edge from " << m - 1 << " to time zero - " << 0.0
                             << ", as it's a dummy pref step\n";
                    }
                }
                else if (hItr->time_spec == Planner::E_AT_START)
                {
                    const vector<pair<double, double>> &tsBounds =
                        TemporalAnalysis::getActionTSBounds()[hItr->action->getID()];
                    ...
                    if (noisy)
                    {
                        cout << "Edge from " << m - 1 << " to time zero - " << -startMin
                             << " due to earliest RPG start point of action\n";
                    }
                }
                else if (hItr->time_spec == Planner::E_AT_END)
                {
                    ...
                    if (noisy)
                    {
                        cout << "Edge from " << m - 1 << " to time zero - " << -endMin
                             << " due to earliest RPG end point of action\n";
                    }
                }
            }
            ...
            for (int m = 1; m < mSize; ++m)
            {
                ...
                if (!eventsWithFakes[m - 1]->isDummyStep())
                {
                    ...
                    if (thisPair >= 0)
                    {
                        if (eventsWithFakes[m - 1]->time_spec == Planner::E_AT_START)
                        {
                            ...
                            if (noisy)
                            {
                                cout << "Edge from " << m - 1 << " to " << thisPair << " - "
                                     << eventsWithFakes[m - 1]->maxDuration << " due to max duration" << endl;
                            }
                        }
                        else
                        {
                            ...
                            {
                                cout << "Edge from " << m - 1 << " to " << thisPair << " - "
                                     << -eventsWithFakes[m - 1]->minDuration << " due to min duration"
                                     << endl;
                            }
                        }
                    }
                    ...
                    {
                        const double tilTime =
                            LPScheduler::getTILTimestamp(eventsWithFakes[m - 1]->divisionID);
                        matrix[0][m] = tilTime;
                ...
                {
                    const double backEdge = (eventsWithFakes[m - 1]->lpMinTimestamp == 0.0 ?
                                                 0.0 :
                                                 -(eventsWithFakes[m - 1]->lpMinTimestamp));
                    if (backEdge < matrix[m][0])
                    {
                        matrix[m][0] = backEdge;
                        if (noisy)
                        {
                            cout << "Changing edge from " << m - 1 << " to time zero - " << backEdge
                                 << " due to known minimum timestamp for action\n";
                        }
                    }
                    else
                    {
                        if (noisy)
                        {
                            cout << "Not changing edge from " << m - 1
                                 << " to time zero due to known minimum timestamp for action\n";
                        }
                    }
                }
                else
                ...
                if (eventsWithFakes[m - 1]->lpMaxTimestamp != -1.0)
                {
                    if (eventsWithFakes[m - 1]->lpMaxTimestamp < matrix[0][m])
                    {
                        ...
                        if (noisy)
                        {
                            cout << "Changing edge from time zero to " << m - 1 << " to " << matrix[0][m]
                                 << " due to known maximum timestamp for action\n";
                        }
                    }
                    else
                    {
                        if (noisy)
                        {
                            cout << "Not changing edge from time zero to " << m - 1
                                 << " due to known maximum timestamp for action\n";
                        }
                    }
                }
                ...
                map<int, IncomingAndOutgoing>::const_iterator eItr =
                    toReturn->getTemporaryEdges().find(m - 1);
                if (eItr != toReturn->getTemporaryEdges().end())
                {
                    map<int, pair<bool, double>>::const_iterator pItr =
                        eItr->second.mustPrecedeThis().begin();
                    const map<int, pair<bool, double>>::const_iterator pEnd =
                        eItr->second.mustPrecedeThis().end();
                    ...
                    for (; pItr != pEnd; ++pItr)
                    {
                        ...
                        if (noisy)
                        {
                            cout << "Edge from " << m - 1 << " to " << pItr->first << " - "
                                 << -pItr->second.second << " due to recorded predecessors" << endl;
                        }
                    }
                }
            }
    ...
    void mergeEdges(map<int, bool> &mergedEdges, const list<const map<int, bool> *> &extraPrecedenceSets,
                    const int &exclude, const set<int> &ignoreEdgesBackTo)
    {
    ...
    ChildData *ParentData::spawnChildData(list<StartEvent> &seq, list<FFEvent> &header,
                                          list<FFEvent> &newActs, const bool &includeMetric,
                                          const TemporalConstraints *const cons, const int &stepID)
    {
        ...
        const int mustComeBeforeOpenEnds =
            MinimalState::getTransformer()->stepThatMustPrecedeUnfinishedActions(cons);
        ...
        if (startGap > stepID)
        {  // if we're only triggering an existing action within the plan

            if (spawnDebug)
            {
                cout << "Is an end action\n";
            }
            ...
            toReturn = new ChildData(&Q, distFromZero, distToZero, pairWith, eventsWithFakes, softEdges,
                                     temporaryEdges, temporarySoftEdges, needsLP);
            ...
            if (mustComeBeforeOpenEnds != -1)
            {
                ...
                {
                    if (seqFillItr->stepID == startLocation)
                    {
                        continue;
                    }
                    const int cbf = toReturn->getPairWith()[seqFillItr->stepID];
                    map<int, pair<bool, double>>::const_iterator oldEdge =
                        edgesForTheEndAction.mustPrecedeThis().find(cbf);
                    if (oldEdge != edgesForTheEndAction.mustPrecedeThis().end() &&
                        oldEdge->second.second > 0.0)
                    {
                        if (spawnDebug)
                        {
                            cout << "Previously required this action to end strictly after another that "
                                    "hasn't finished yet - STP inconsistent\n";
                        }
                        delete toReturn;
                        toReturn = 0;
                        return toReturn;
                    ...
                    const pair<map<int, pair<bool, double>>::iterator, bool> insPair =
                        nowAfter.insert(make_pair(cbf, make_pair(true, EPSILON)));
                    ...
                    {
                        if (insPair.first->second.second == 0.0 || EPSILON > insPair.first->second.second)
                        {
                    ...
                    if (tightened)
                    {
                        if (spawnDebug)
                        {
                            cout << "Adding epsilon separation edge from " << endLocation
                                 << " to some future end event " << cbf << "\n";
                        }
            ...
            {  // now see if any new edges are in cons that weren't present before
                ...
                {
                    map<int, pair<bool, double>>::const_iterator oldCbf =
                        edgesForTheEndAction.mustPrecedeThis().begin();
                    const map<int, pair<bool, double>>::const_iterator oldCbfEnd =
                        edgesForTheEndAction.mustPrecedeThis().end();
                            ...
                            if (newCbf->first != startLocation)
                            {
                                if ((earlier = toReturn->edgesAreNonSoft(newCbf->first)) != -1)
                                {
                                    if (spawnDebug)
                                    {
                                        if (newCbf->second)
                                        {
                                            cout << "New 'before' edge: adding epsilon separation edge from "
                                                 << earlier
                                                 << " to the end snap-action that has been applied\n";
                                        }
                                        else
                                        {
                                            cout << "New 'before' edge: adding zero separation edge from "
                                                 << earlier
                                                 << " to the end snap-action that has been applied\n";
                                        }
                                    }
                                    toReturn->addNewEdge(BFEdge(earlier, endLocation,
                                                                (newCbf->second ? 0.001 : 0.0), DBL_MAX));
                                }
                                else
                                {
                                    toReturn->makeSoftEdgeListFor(endLocation)
                                        .addPredecessor(newCbf->first, newCbf->second,
                                                        (newCbf->second ? 0.001 : 0.0));
                                    toReturn->makeSoftEdgeListFor(newCbf->first)
                                        .addFollower(endLocation, newCbf->second,
                                                     (newCbf->second ? 0.001 : 0.0));
                                }
                            }
                            ++newCbf;
                            ...
                            if (newCbf->first != startLocation)
                            {
                                if ((earlier = toReturn->edgesAreNonSoft(newCbf->first)) != -1)
                                {
                                    ...
                                    if (newValue > oldCbf->second.second)
                                    {
                                        if (spawnDebug)
                                        {
                                            cout << "Changed cost from " << -oldCbf->second.second
                                                 << " to -epsilon on the separation edge from " << earlier
                                                 << " to the end snap-action that has been applied\n";
                                        }
                    ...
                    for (; newCbf != newCbfEnd; ++newCbf)
                    {
                        if (newCbf->first != startLocation)
                        {
                            if ((earlier = toReturn->edgesAreNonSoft(newCbf->first)) != -1)
                            {
                                ...
                                toReturn->addNewEdge(
                                    BFEdge(earlier, endLocation, (newCbf->second ? 0.001 : 0.0), DBL_MAX));
                            }
                            else
                            {
                                toReturn->makeSoftEdgeListFor(endLocation)
                                    .addPredecessor(newCbf->first, newCbf->second,
                                                    (newCbf->second ? 0.001 : 0.0));
                                toReturn->makeSoftEdgeListFor(newCbf->first)
                                    .addFollower(endLocation, newCbf->second, (newCbf->second ? 0.001 : 0.0));
                            }
                        }
                    }
                }

                // then, things this has to come before - necessarily must be ends of actions yet to have been
                // applied but, better check...
                ...
                if (Globals::paranoidScheduling)
                {
                    ...
                    for (int cs = 0; cs < consSize; ++cs)
                    {
                        ...
                        if (!toReturn->getEventsWithFakes()[cs]->getEffects)
                        {
                            continue;
                        }
...

                {
                    ...
                    for (; seqFillItr != seqFillEnd; ++seqFillItr)
                    {
                        ...
                        map<int, bool>::const_iterator toThisItr =
                            eventsBeforeFutureAction->find(endLocation);

                        if (toThisItr != eventsBeforeFutureAction->end())
                        {
                            const double localValue = (toThisItr->second ? EPSILON : 0.0);
                            const pair<map<int, pair<bool, double>>::iterator, bool> insPair =
                                nowAfter.insert(make_pair(cbf, make_pair(toThisItr->second, localValue)));
                            ...
                            if (tightened)
                            {
                                if (spawnDebug)
                                {
                                    if (toThisItr->second)
                                    {
                                        cout << "Adding epsilon separation edge to the end snap-action "
                                             << cbf << " that has yet to be applied\n";
                                    }
                                    else
                                    {
                                        cout << "Adding zero separation edge to the end snap-action " << cbf
            ...
            if (!newActs.empty())
            {
                ...
                for (; newStep != newEnd; ++newStep, ++gapItr)
                {
                    ...
                    if (applyEdgesTo == *gapItr)
                    {
                        ...
                        if (preceding)
                        {
                            ...
                            for (; pItr != pEnd; ++pItr)
                            {
                                ...
                                const double w =
                                    -(toReturn->getDistToZero()[earlier] - (pItr->second ? 0.001 : 0.0));
                                if (localMin < w)
                                {
                                    localMin = w;
                                }
                    ...
                    if (newStep->time_spec == Planner::E_DUMMY_TEMPORAL_TRIGGER_TRUE)
                    {
                        ...
                        {
                            const RPGBuilder::Constraint &pref =
                                RPGBuilder::getPreferences()[newStep->divisionID];
                            if (pref.cons == VAL::E_ALWAYSWITHIN)
                            {
                                if (applyEdgesTo == -1)
                                {
                                    toReturn->makeSoftEdgeListFor(dummyGap).addPredecessor(
                                        newStep->pairWithStep, false, -pref.deadline);
                                    toReturn->makeSoftEdgeListFor(newStep->pairWithStep)
                                        .addFollower(dummyGap, false, -pref.deadline);
                                }
                                else
                                {
                                    ...
                                    toReturn->addNewEdge(
                                        BFEdge(applyEdgesTo, newStep->pairWithStep, 0.001, pref.deadline));
                                }
                            }
                        }
                    }
                    else if (newStep->time_spec == Planner::E_DUMMY_TEMPORAL_GOAL_TRUE)
                    {
                        ...
                        if (pref.cons == VAL::E_WITHIN)
                        {
                            ...
                            else
                            {
                                ...
                                toReturn->makeSoftEdgeListFor(-1).addPredecessor(dummyGap, false,
                                                                                 -pref.deadline);
                                toReturn->makeSoftEdgeListFor(dummyGap).addFollower(-1, false,
                                                                                    -pref.deadline);
                            }
                        }
                    }
                            ...
                            for (; ecaItr != ecaEnd; ++ecaItr)
                            {
                                ...
                                if ((earlier = toReturn->edgesAreNonSoft(afR)) != -1)
                                {
                                    if (spawnDebug)
                                    {
                                        if (ecaItr->second)
                                        {
                                            cout << "Due to recorded constraints: adding edge to denote that "
                                                    "the first preference checking step comes at least "
                                                    "epsilon after step "
                                                 << earlier << endl;
                                        }
                                        else
                                        {
                                            cout << "Due to recorded constraints: adding edge to denote that "
                                                    "the first preference checking step comes after (or "
                                                    "alongside) step "
                                                 << earlier << endl;
                                        }
                                    }
                                    ...
                                    toReturn->addNewEdge(
                                        BFEdge(earlier, dummyGap, (ecaItr->second ? 0.001 : 0.0), DBL_MAX));
                                }
                                else
                                {
                                    toReturn->makeSoftEdgeListFor(dummyGap).addPredecessor(
                                        afR, ecaItr->second, (ecaItr->second ? 0.001 : 0.0));
                                    toReturn->makeSoftEdgeListFor(afR).addFollower(
                                        dummyGap, ecaItr->second, (ecaItr->second ? 0.001 : 0.0));
                                }
                            }
                        }
                        else
                            ...
                            if (toThisItr != beforeFutureStep->end())
                            {
                                if (applyEdgesTo != -1)
                                {
                                    if (spawnDebug)
                                    {
                                        if (toThisItr->second)
                                        {
                                            cout << "Constraint to future step: adding epsilon separation "
                                                    "edge from "
                                                 << applyEdgesTo << " to some future end event " << cbf
                                                 << "\n";
                                        }
                                        else
                                        {
                                            cout << "Constraint to future step: adding zero separation edge "
                                                    "from "
                                                 << applyEdgesTo << " to some future end event " << cbf
                                                 << "\n";
                                        }
                                    ...
                                    toReturn->addNewEdge(BFEdge(applyEdgesTo, cbf,
                                                                (toThisItr->second ? 0.001 : 0.0), DBL_MAX));
                                }
                                else
                                {
                                    toReturn->makeSoftEdgeListFor(cbf).addPredecessor(
                                        dummyGap, toThisItr->second, (toThisItr->second ? 0.001 : 0.0));
                                    toReturn->makeSoftEdgeListFor(dummyGap).addFollower(
                                        cbf, toThisItr->second, (toThisItr->second ? 0.001 : 0.0));
                                }
                            }
                        }
                    }
        ...
        assert(newActs.front().time_spec == Planner::E_AT_START ||
               newActs.front().time_spec == Planner::E_AT);
        ...
        if (s.time_spec == Planner::E_AT_START)
        {
            ...
            if (spawnDebug)
            {
                ...
                else
                {
                    if (spawnDebug)
                    {
                        cout << "Is an instantaneous action, goes at " << startGap << "\n";
                    }
                    if (LPScheduler::isBoring(s.action->getID(), 0, includeMetric))
                    {
            ...
            if (newStep != newActs.end())
            {  // if we have dummy steps
                ...
                if (naSize > sgSize)
                {
                    // this ensures that there are enough gaps defined, with timestamps given the default
                    // range of [0..inf]
                ...
                for (; newStep != newActs.end(); ++newStep)
                {
                    ...
                    if (spawnDebug)
                    {
                        cout << "Allocating gap " << dummyGap << " to dummy step for "
                             << RPGBuilder::getPreferences()[newStep->divisionID].name << endl;
                    }
            ...
            const vector<pair<double, double>> &tsBounds =
                TemporalAnalysis::getActionTSBounds()[s.action->getID()];
                    ...
                    for (; pItr != pEnd; ++pItr)
                    {
                        ...
                        if ((earlier = toReturn->edgesAreNonSoft(pItr->first)) != -1)
                        {
                            if (earlier != startLocation &&
                                ignoreStartEdgesBackTo.find(earlier) == ignoreStartEdgesBackTo.end())
                            {
                                const double w =
                                    -(toReturn->getDistToZero()[earlier] - (pItr->second ? 0.001 : 0.0));
                                if (startMin < w)
            ...
            if (nonTemporal)
            {
                if (spawnDebug)
                {
                    cout << "Edges back to previous steps give earliest start position " << startMin << " vs "
                         << tsBounds[0].first << " from RPG\n";
                }
            ...
            {
                {
                    ...
                    if (preceding)
                    {
                        ...
                        for (; pItr != pEnd; ++pItr)
                        {
                            ...
                            {
                                if (earlier != startLocation &&
                                    ignoreEndEdgesBackTo.find(earlier) == ignoreEndEdgesBackTo.end())
                                {
                                    const double w =
                                        -(toReturn->getDistToZero()[earlier] - (pItr->second ? 0.001 : 0.0));
                                    if (endMin < w)
...

                {
                    ...
                    for (; psItr != psEnd; ++psItr)
                    {
                        ...
                        for (; pItr != pEnd; ++pItr)
                        {
                            ...
                            if ((earlier = toReturn->edgesAreNonSoft(pItr->first)) != -1)
                            {
                                if (earlier != endLocation && earlier != startLocation &&
                                    ignoreEndEdgesBackTo.find(earlier) == ignoreEndEdgesBackTo.end())
                                {
                                    const double w =
                                        -(toReturn->getDistToZero()[earlier] - (pItr->second ? 0.001 : 0.0));
                ...
                if (spawnDebug)
                {
                    cout << "Edges back to previous steps give earliest positions " << startMin << ";"
                         << endMin << " vs " << tsBounds[0].first << ";" << tsBounds[1].first
                         << " from RPG\n";
                }
            ...
            if (spawnDebug)
            {
                cout << "Initially putting at " << startMin;
            }
            ...
            if (mustComeBeforeOpenEnds != -1)
            {
                ...
                for (; seqFillItr != seqFillEnd; ++seqFillItr)
                {
                    ...
                    if (spawnDebug)
                    {
                        cout << "Adding total-order epsilon separation edge from " << startLocation
                             << " to some future end event " << cbf << "\n";
                    }
                ...
                for (; seqFillItr != seqFillEnd; ++seqFillItr)
                {
                    ...
                    if (toThisItr != beforeFutureStep->end())
                    {
                        if (spawnDebug)
                        {
                            if (toThisItr->second)
                            {
                                cout << "Adding epsilon separation edge from " << startLocation
                                     << " to some future end event " << cbf
                                     << ", due to recorded constraints\n";
                            }
                            else
                            {
                                cout << "Adding zero separation edge from " << startLocation
                                     << " to some future end event " << cbf
                                     << ", due to recorded constraints\n";
                            }
                        }
                        toReturn->addNewEdge(
                            BFEdge(startLocation, cbf, (toThisItr->second ? 0.001 : 0.0), DBL_MAX));
                    }
                }
            }
                    ...
                    for (; ecaItr != ecaEnd; ++ecaItr)
                    {
                        ...
                        if ((earlier = toReturn->edgesAreNonSoft(afR)) != -1)
                        {
                            if (spawnDebug)
                            {
                                if (ecaItr->second)
                                {
                                    cout << "Recorded constraints: adding edge to denote that the start of "
                                            "the new action comes at least epsilon after step "
                                         << earlier << endl;
                                }
                                else
                                {
                                    cout << "Recorded constraints: adding edge to denote that the start of "
                                            "the new action comes after (or alongside) step "
                                         << earlier << endl;
                                }
                            }

                            toReturn->addNewEdge(
                                BFEdge(earlier, startLocation, (ecaItr->second ? 0.001 : 0.0), DBL_MAX));
                        }
                        else
                        {
                            toReturn->makeSoftEdgeListFor(startLocation)
                                .addPredecessor(afR, ecaItr->second, (ecaItr->second ? 0.001 : 0.0));
                            toReturn->makeSoftEdgeListFor(afR).addFollower(startLocation, ecaItr->second,
                                                                           (ecaItr->second ? 0.001 : 0.0));
                        }
                    }
                }
            }
                        ...
                        for (; ecaItr != ecaEnd; ++ecaItr)
                        {
                            ...
                            if ((earlier = toReturn->edgesAreNonSoft(afR)) != -1)
                            {
                                ...
                                if (spawnDebug)
                                {
                                    if (ecaItr->second)
                                    {
                                        cout << "Due to recorded constraints: adding edge to denote that the "
                                                "end of the new action comes at least epsilon after step "
                                             << earlier << endl;
                                    }
                                    else
                                    {
                                        cout << "Due to recorded constraints: adding edge to denote that the "
                                                "end of the new action comes after (or alongside) step "
                                             << earlier << endl;
                                    }
                                }

                                toReturn->addNewEdge(
                                    BFEdge(earlier, endLocation, (ecaItr->second ? 0.001 : 0.0), DBL_MAX));
                            }
                            else
                            {
                                toReturn->makeSoftEdgeListFor(endLocation)
                                    .addPredecessor(afR, ecaItr->second, (ecaItr->second ? 0.001 : 0.0));
                                toReturn->makeSoftEdgeListFor(afR).addFollower(
                                    endLocation, ecaItr->second, (ecaItr->second ? 0.001 : 0.0));
                            }
                        }
                    }
                    else
                    ...
                    for (; seqFillItr != seqFillEnd; ++seqFillItr)
                    {
                        ...
                        if (toThisItr != beforeFutureStep->end())
                        {
                            if (spawnDebug)
                            {
                                if (toThisItr->second)
                                {
                                    cout << "Constraint to future step: adding epsilon separation edge from "
                                         << endLocation << " to some future end event " << cbf << "\n";
                                }
                                else
                                {
                                    cout << "Constraint to future step: adding zero separation edge from "
                                         << endLocation << " to some future end event " << cbf << "\n";
                                }
                            }
                            toReturn->addNewEdge(
                                BFEdge(endLocation, cbf, (toThisItr->second ? 0.001 : 0.0), DBL_MAX));
                        }
                    }
                }
                ...
                if (!ignoreABedges) {
...

                    {
                        ...
                        for (; ecaItr != ecaEnd; ++ecaItr) {
                            ...
                            if (af >= 0) {
                                if (spawnDebug) cout << "Adding edge to denote that the end of the new action
                comes after the end of that starting at " << af << ", i.e. after node " <<
                toReturn->getPairWith()[af] << "\n"; toReturn->addNewEdge(BFEdge(toReturn->getPairWith()[af],
                endLocation, 0.001, DBL_MAX));
                            }
                        }
                    }

                    {
                        ...
                        for (; ecbItr != ecbEnd; ++ecbItr) {
                            ...
                            if (af >= 0) {
                                if (spawnDebug) cout << "Adding edge to denote that the end of the action
                comes before the end of that starting at " << af << ", i.e. before node " <<
                toReturn->getPairWith()[af] << ", due to eca pair\n"; toReturn->addNewEdge(BFEdge(endLocation,
                toReturn->getPairWith()[af], 0.001, DBL_MAX));
                            }
                        }
                    }
...

                    {
                        ...
                        for (; ecbItr != ecbEnd; ++ecbItr) {
                            ...
                            if (af >= 0) {
                                if (spawnDebug) cout << "Adding edge to denote that the end action comes
                before the end of that starting at node " << af << ", i.e. node " <<
                toReturn->getPairWith()[af] << "\n"; toReturn->addNewEdge(BFEdge(endLocation,
                toReturn->getPairWith()[af], 0.001, DBL_MAX));
                            }
                        }
                    ...
                    if (workingFanIn) {
                        ...
                        for (; ecaItr != ecaEnd; ++ecaItr) {
                            ...
                            if (af >= 0) {
                                if (spawnDebug) cout << "Adding edge to denote that the end of the action
                comes after that which started at node " << *ecaItr << ", i.e. " <<
                toReturn->getPairWith()[*ecaItr] << ", due to it being on the back-end of an
                end-comes-before\n"; toReturn->addNewEdge(BFEdge(toReturn->getPairWith()[*ecaItr],
                endLocation, 0.001, DBL_MAX));
                            }
                        }
                    }
                ...
                if (newStep->time_spec == Planner::E_DUMMY_TEMPORAL_TRIGGER_TRUE)
                {
                    if (newStep->pairWithStep >= 0)
                    {
                        ...
                        if (pref.cons == VAL::E_ALWAYSWITHIN)
                        {
                            if (applyEdgesTo == -1)
                            {
                                toReturn->makeSoftEdgeListFor(dummyGap).addPredecessor(newStep->pairWithStep,
                                                                                       false, -pref.deadline);
                                toReturn->makeSoftEdgeListFor(newStep->pairWithStep)
                                    .addFollower(dummyGap, false, -pref.deadline);
                            }
                        ...
                        for (; ecaItr != ecaEnd; ++ecaItr)
                        {
                            ...
                            if ((earlier = toReturn->edgesAreNonSoft(afR)) != -1)
                            {
                                if (spawnDebug)
                                {
                                    ...
                                    {
                                        cout << "Due to recorded constraints: adding edge to denote that "
                                                "preference checking step "
                                             << dummyGap << " comes at least epsilon after step " << earlier
                                             << endl;
                                    }
                                    else
                                    {
                                        cout << "Due to recorded constraints: adding edge to denote that "
                                                "preference checking step "
                                             << dummyGap << " comes after (or alongside) step " << earlier
                                             << endl;
                                    }
                                }
                ...
                else if (applyEdgesTo == -1)
                {
                    ...
                    if (beforeDummyStep)
                    {
                        ...
                        for (; ecaItr != ecaEnd; ++ecaItr)
                        {
                            ...
                            toReturn->makeSoftEdgeListFor(dummyGap).addPredecessor(
                                afR, ecaItr->second, (ecaItr->second ? 0.001 : 0.0));
                            toReturn->makeSoftEdgeListFor(afR).addFollower(dummyGap, ecaItr->second,
                                                                           (ecaItr->second ? 0.001 : 0.0));
                        }
                    }
                }
                        ...
                        if (toThisItr != beforeFutureStep->end())
                        {
                            if (applyEdgesTo != -1)
                            {
                                if (spawnDebug)
                                {
                                    if (toThisItr->second)
                                    {
                                        cout << "Constraint to future step: adding epsilon separation edge "
                                                "from "
                                             << dummyGap << " to some future end event " << cbf << "\n";
                                    }
                                    else
                                    {
                                        cout << "Constraint to future step: adding zero separation edge from "
                                ...
                                toReturn->addNewEdge(
                                    BFEdge(dummyGap, cbf, (toThisItr->second ? 0.001 : 0.0), DBL_MAX));
                            }
                            else
                            {
                                toReturn->makeSoftEdgeListFor(cbf).addPredecessor(
        ...
        else
        {
            ...
            if (mustComeBeforeOpenEnds != -1)
            {
                ...
                for (; seqFillItr != seqFillEnd; ++seqFillItr)
                {
                    ...
                    if (spawnDebug)
                    {
                        cout << "Adding edge from time zero to a future step " << cbf
                             << " to set its min ts to " << tilTime + 0.001 << "\n";
                    }
                    toReturn->addNewEdge(BFEdge(-1, cbf, tilTime + 0.001, distFromZero[cbf]));

                    if (spawnDebug)
                    {
                        cout << "Adding epsilon separation edge from " << startGap
                             << " to some future end event " << cbf << "\n";
                    }
                ...
                if (beforeNewStep)
                {
                    ...
                    for (; ecaItr != ecaEnd; ++ecaItr)
                    {
                        ...
                        if ((earlier = toReturn->edgesAreNonSoft(afR)) != -1)
                        {
                            if (spawnDebug)
                            {
                                cout << "TIL comes after step " << earlier << endl;
                            }
                            ...
                            if (w > tilTime - 0.001)
                            {
                                if (spawnDebug)
                                {
                                    cout << "- Found a simple cycle: the earliest step " << earlier
                                         << " could go at was " << w << ", i.e. too late for a TIL at "
                                         << tilTime << endl;
                                }
                                delete toReturn;
                                return 0;
                            ...
                            if (distFromZero[earlier] > tilTime - 0.001)
                            {
                                if (spawnDebug)
                                {
                                    cout << "Adding edge from time zero to a preceding step " << earlier
                                         << " to set its max ts to " << tilTime - 0.001 << "\n";
                                }
                                toReturn->addNewEdge(BFEdge(-1, earlier, w, tilTime - 0.001));
                                if (spawnDebug)
                                {
                                    cout << "Adding epsilon separation edge from step " << earlier
                                         << " to TIL\n";
                                }
                            ...
                            else
                            {
                                if (spawnDebug)
                                {
                                    cout << "Latest step " << earlier << " could come at was "
                                         << distFromZero[earlier] << ", so it is unaffected\n";
                                }
                        ...
                        {
                            toReturn->makeSoftEdgeListFor(-1).addPredecessor(afR, false,
                                                                             -(tilTime - EPSILON));
                            toReturn->makeSoftEdgeListFor(afR).addPredecessor(-1, false,
                                                                              -(tilTime - EPSILON));
                        }
                    }
                }
                        ...
                        if (preceding)
                        {
                            ...
                            for (; pItr != pEnd; ++pItr)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        const double w = -(toReturn->getDistToZero()[pItr->first] -
                                                           (pItr->second ? 0.001 : 0.0));
                                        if (localMin < w)
                                        {
                                            localMin = w;
                                        }
                                ...
                                if ((earlier = toReturn->edgesAreNonSoft(afR)) != -1)
                                {
                                    if (spawnDebug)
                                    {
                                        if (ecaItr->second)
                                        {
                                            cout << "Due to recorded constraints: adding edge to denote that "
                                                    "preference checking step "
                                                 << dummyGap << " comes at least epsilon after step "
                                                 << earlier << endl;
                                        }
                                        else
                                        {
    ...
    bool ChildData::propagateNewEdges(const vector<AutomatonPosition::Position> &previousPreferenceStatus,
                                      const TemporalConstraints *cons, bool &preferencesWereAsExpected)
    {
        ...
        if (floydDoubleCheck)
        {
            ...
            for (int m = 1; m < mSize; ++m)
            {
                ...
                if (eventsWithFakes[m - 1]->isDummyStep())
                {
                    ...
                    {
                        cout << "Edge from " << m - 1
                             << " to time zero is 0.0, as it's a dummy preference-checking step\n";
                    }
                }
                else if (eventsWithFakes[m - 1]->time_spec == Planner::E_AT_START)
                {
                    const vector<pair<double, double>> &tsBounds =
                        TemporalAnalysis::getActionTSBounds()[eventsWithFakes[m - 1]->action->getID()];
                    const double startMin = tsBounds[0].first;
                    const double startMax = tsBounds[0].second;
            ...
            for (int m = 1; m < mSize; ++m)
            {
                ...
                if (pairWith[m - 1] >= 0)
                {
                    if (eventsWithFakes[m - 1]->time_spec == Planner::E_AT_START)
                    {
                        ...
                        if (noisy)
                        {
                            cout << "Edge from " << m - 1 << " to " << pairWith[m - 1] << " - "
                                 << eventsWithFakes[m - 1]->maxDuration << " due to max duration" << endl;
                        }
                    }
                    else
                    {
                        ...
                        {
                            cout << "Edge from " << m - 1 << " to " << pairWith[m - 1] << " - "
                                 << -eventsWithFakes[m - 1]->minDuration << " due to min duration" << endl;
                        }
                    }
                }
                ...
                if (eItr != temporaryEdges.end())
                {
                    ...
                    for (; pItr != pEnd; ++pItr)
                    {
                        ...
                        if (negatedValue < matrix[m][pItr->first + 1])
                        {
                            ...
                            {
                                cout << "Edge from " << m - 1 << " to " << pItr->first << " - "
                                     << negatedValue << " due to recorded outgoing edges" << endl;
                            }
                        }
                    }
                }
...

            {
                ...
                for (; aItr != aEnd; ++aItr)
                {
                    if (-aItr->second < matrix[aItr->first + 1][0])
                    {
                        ...
                        if (noisy)
                        {
                            cout << "Edge from " << aItr->first << " to time zero, weight " << -aItr->second
                                 << " due to it having to follow after actions with known timestamps\n";
                        }
                    }
                }
            }
        ...
        {
            const bool pResult =
                Propagation(*Q, *neItr, distFromZero, distToZero, pairWith, eventsWithFakes, temporaryEdges);
            ...
            if (floydDoubleCheck)
            {
...

                {
                    ...
                    for (; aItr != aEnd; ++aItr)
                    {
                        if (-aItr->second < matrix[aItr->first + 1][0])
                        {
                            ...
                            if (noisy)
                            {
                                cout << "Edge from " << aItr->first << " to time zero, weight "
                                     << -aItr->second
                                     << " due to it having to follow after actions with known timestamps\n";
                            }
                        }
                    ...
                    for (i = 0; i < mSize; ++i)
                    {
                        ...
                        for (j = 0; j < mSize; ++j)
                        {
                            ...
                            if (distKJ != DBL_MAX)
                            {
                                ...
                                {
                                    /*if (noisy) {
                                        cout << "Path from " << i-1 << " to " << j-1 << " is shorter via " <<
                                    k-1 << endl; if (distIJ == DBL_MAX) { cout << "Was infinite, now " <<
                                    distIK + distKJ << endl; } else { cout << "Was " << distIJ << ", now " <<
                                    distIK + distKJ << endl;
                                        }
                                    }*/
                ...
                if (isOkay)
                {
                    ...
                    for (int m = 1; m < mSize; ++m)
                    {
                        ...
                        if (noisy)
                        {
                            cout << std::setprecision(6) << (-1 * matrix[m][0]) << " vs "
                                 << -distToZero[m - 1] << " - step " << m - 1 << " ("
                                 << eventsWithFakes[m - 1]->lpMinTimestamp << ")" << endl;
...
#define CANBEVIOLATEDHERE(x)                                                                                 \
    {                                                                                                        \
        cerr << "Incorrectly think that preference " << RPGBuilder::getPreferences()[x].name << ":" << x     \
             << " cannot be satisfied in the STP.  Is in position "                                          \
             << positionName[previousPreferenceStatus[x]] << "\n";                                           \
        assert(false);                                                                                       \
    }
#else
#define CANBEVIOLATEDHERE(x)                                                                                 \
    {                                                                                                        \
        ...
        if (!RPGBuilder::getPreferences().empty())
        {
            ...
            map<int, map<int, double>> alwaysWithinOrSometimeAfterPrefOpenedAtStep;  // if sometime-after a b,
                                                                                     // then dummies for b
                                                                                     // only re-satisfy the
                                                                                     // preference if ater an
                                                                                     // a

            map<int, map<int, double>> sometimeBeforePrefSatisfiedAtStep;  // if sometime-before a b, then
                                                                           // dummies for a need to come after
                                                                           // dummies for b
            ...
            for (int p = 0; p < planSize; ++p)
            {
                ...
                switch (eventsWithFakes[p]->time_spec)
                {
                    ...
                    {
                        std::cerr << "Fatal internal error: uninitialised dummy step found when checking "
                                     "whether soft-edges are satisfied\n";
                        exit(1);
                        break;
                    }
                    case Planner::E_DUMMY_PREFERENCE_PRECONDITION:
                    {
                        ...
                        if (softEdges[p] == -1)
                        {
                            ...
                            const map<int, IncomingAndOutgoing>::const_iterator tseItr =
                                temporarySoftEdges.find(p);

                            if (tseItr != temporarySoftEdges.end())
                            {
                                // get the time step of the actual action event to which the preference is
                                // attached i.e. look at pair with step
                                const double negativeTimeOfStepWithPreference =
                                    distToZero[eventsWithFakes[p]->pairWithStep];
                        ...
                        if (!edgesHold)
                        {
                            accruedPreferencePreconditionViolations +=
                                RPGBuilder::getPreferences()[eventsWithFakes[p]->divisionID].cost;
                        }
                    ...
                    case Planner::E_DUMMY_TEMPORAL_GOAL_TRUE:
                    {
                        ...
                        const AutomatonPosition::Position &currPosition =
                            previousPreferenceStatus[eventsWithFakes[p]->divisionID];
                        if (currPosition == AutomatonPosition::unreachable ||
                            currPosition == AutomatonPosition::eternallysatisfied)
                        {
                            if (tgDebug)
                            {
                                cout << "Skipping update of goal of "
                                     << RPGBuilder::getPreferences()[eventsWithFakes[p]->divisionID].name
                                     << ":" << eventsWithFakes[p]->divisionID << " - is in position "
                                     << positionName[currPosition] << endl;
                            }
                            break;
                        }

                        const RPGBuilder::Constraint &pref =
                            RPGBuilder::getPreferences()[eventsWithFakes[p]->divisionID];
                        ...
                        if (softEdges[p] == -1)
                        {
                            ...
                            if (tseItr != temporarySoftEdges.end())
                            {
                                if (tgDebug)
                                {
                                    cout << "Updating goal of "
                                         << RPGBuilder::getPreferences()[eventsWithFakes[p]->divisionID].name
                                         << ":" << eventsWithFakes[p]->divisionID
                                         << " - checking edges into dummy step " << p << ")\n";
                                }
#ifndef NDEBUG
                                if (pref.cons == VAL::E_WITHIN)
                                {
                                    assert(tseItr->second.mustFollowThis().find(-1) !=
                                           tseItr->second.mustFollowThis().end());
                                }
#endif
                            ...
                            {
                                assert(pref.cons != VAL::E_WITHIN);  // withins should have at least edges
                                                                     // from zero
                            }
                        }
                        else
                        {
                            ...
                            if (tgDebug)
                            {
                                cout << "Updating goal of "
                                     << RPGBuilder::getPreferences()[eventsWithFakes[p]->divisionID].name
                                     << ":" << eventsWithFakes[p]->divisionID
                                     << ", dummy dist to zero = " << dummyDistToZero
                                     << ", dummy dist from zero = " << dummyDistFromZero << endl;
                            }
                        }

                        assert(eventsWithFakes[p]->time_spec == Planner::E_DUMMY_TEMPORAL_GOAL_TRUE ||
                               pref.cons == VAL::E_ATMOSTONCE);

                        if (!edgesHold)
                        {
                            if (pref.cons != VAL::E_SOMETIMEBEFORE && pref.cons != VAL::E_ALWAYSWITHIN &&
                                pref.cons != VAL::E_SOMETIMEAFTER && pref.cons != VAL::E_SOMETIME &&
                                pref.cons != VAL::E_WITHIN)
                            {
                        ...
                        else
                        {
                            ...
                            {
                                const map<int, bool>::iterator wiItr =
                                    satisfiedSometimes.find(eventsWithFakes[p]->divisionID);
                            ...
                            else if (pref.cons == VAL::E_WITHIN)
                            {
                                ...
                                {
                                    const map<int, bool>::iterator wiItr =
                                        satisfiedWithins.find(eventsWithFakes[p]->divisionID);
                            ...
                            {
                                sometimeBeforePrefSatisfiedAtStep[eventsWithFakes[p]->divisionID].insert(
                                    make_pair(p, dummyDistToZero));
                            }
                            else if (pref.cons == VAL::E_ALWAYSWITHIN || pref.cons == VAL::E_SOMETIMEAFTER)
                            {
                                // means we have the means of closing an always-within/sometime-after
                                // preference that was opened earlier see if it actually does so or not

                                const map<int, map<int, double>>::iterator awItr =
                                    alwaysWithinOrSometimeAfterPrefOpenedAtStep.find(
                                        eventsWithFakes[p]->divisionID);
                                ...
                                if (awItr != alwaysWithinOrSometimeAfterPrefOpenedAtStep.end())
                                {
                                    ...
                                    {
                                        if (dummyDistToZero <= stepItr->second  // in both cases, it's only
                                                                                // any good if the goal is at
                                                                                // or after the trigger
                                            && (pref.cons == VAL::E_SOMETIMEAFTER ||
                                                stepItr->second - pref.deadline <= dummyDistToZero))
                                        {  // for always-within: finished on time
                    ...
                    case Planner::E_DUMMY_TEMPORAL_TRIGGER_TRUE:
                    {
                        ...
                        else
                        {
                            if (softEdges[p] == -1)
                            {
                                ...
                                {
                                    edgesHold =
                                        allHold(tseItr->second, cons, dummyDistToZero, dummyDistFromZero);
                                }
                            }
                            else
                            {
                        ...
                        {
                            const map<int, map<int, double>>::iterator awItr =
                                sometimeBeforePrefSatisfiedAtStep.find(eventsWithFakes[p]->divisionID);
                            ...
                            if (awItr != sometimeBeforePrefSatisfiedAtStep.end())
                            {
                                ...
                                {
                                    if (dummyDistFromZero < stepItr->second)
                                    {  // if this step comes after that - great
                                        break;
                                    }
                                }

                                if (stepItr == stepEnd)
                                {  // haven't come after an earlier goal match
                                    preferencesWereAsExpected = false;
                                    CANBEVIOLATEDHERE(eventsWithFakes[p]->divisionID);
                                    return true;
                                }
                        ...
                        {
                            alwaysWithinOrSometimeAfterPrefOpenedAtStep[eventsWithFakes[p]->divisionID]
                                .insert(make_pair(p, dummyDistToZero));
                        }
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                ...
                for (; wiItr != wiEnd; ++wiItr)
                {
                    if (!wiItr->second)
                    {
                        ...
                        if (currPosn != AutomatonPosition::unsatisfied &&
                            currPosn != AutomatonPosition::unreachable)
                        {
                            // nothing ever satisfied this preference in time, and the automaton position for
                            // it suggests it's satisfied
                            preferencesWereAsExpected = false;
            ...
            {
                map<int, map<int, double>>::const_iterator awiItr =
                    alwaysWithinOrSometimeAfterPrefOpenedAtStep.begin();
                const map<int, map<int, double>>::const_iterator awiEnd =
                    alwaysWithinOrSometimeAfterPrefOpenedAtStep.end();

                for (; awiItr != awiEnd; ++awiItr)
                {
                    const AutomatonPosition::Position &currPosn = previousPreferenceStatus[awiItr->first];

                    if (currPosn == AutomatonPosition::eternallysatisfied ||
                        currPosn == AutomatonPosition::probablyeternallysatisfied ||
                        currPosn == AutomatonPosition::satisfied)
                    {
                        preferencesWereAsExpected = false;
    ...
    bool ChildData::updateLPMinTimestamp(const double &d, const int &stepID)
    {
        ...
        if (d <= w)
        {
            if (Globals::globalVerbosity & 4096)
            {
                cout << COLOUR_light_red << "Post LP, not changing minimum timestamp of node " << stepID
                     << ": is still " << w << COLOUR_default << endl;
            }
            return true;
        }
        ...
        if (Globals::globalVerbosity & 4096)
        {
            cout << COLOUR_light_red << "Post LP, setting minimum timestamp of node " << stepID << " to " << d
                 << " rather than " << w << COLOUR_default << endl;
        }
