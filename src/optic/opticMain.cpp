...
void usage(char *argv[])
{
    ...
    cout << "\t"
         << "-citation"
         << "\t"
         << "Display citation to relevant papers;\n";
    cout << "\t"
         << "-b"
         << "\t\t"
    ...
    cout << "\t"
         << "-c"
         << "\t\t"
         << "Enable the tie-breaking in RPG that favour actions that slot into the partial order earlier;\n";
    cout << "\t"
         << "-S"
         << "\t\t"
    ...
    cout << "\t"
         << "-r"
         << "\t\t"
         << "Read in a plan instead of planning;\n";
    cout << "\t"
         << "-T"
         << "\t\t"
    ...
    cout << "\t"
         << "-v<n>"
         << "\t\t"
         << "Verbose to degree n (n defaults to 1 if not specified).\n";
    cout << "\t"
         << "-L<n>"
         << "\t\t"
         << "LP verbose to degree n (n defaults to 1 if not specified).\n";
};
...
int main(int argc, char *argv[])
{
    ...
    while (argcount < argc && argv[argcount][0] == '-')
    {
        ...
        if (remainder == "citation")
        {
            ...
            cout << "\tbooktitle = \"Twenty Second International Conference on Automated Planning and "
                    "Scheduling (ICAPS 12)\",\n";
            cout << "\tyear = \"2012\",\n";
            ...
            cout << "\tauthor = \"K. Tierney, A. J. Coles, A. I. Coles, C. Kroer, A. Britt, and R. M. "
                    "Jensen\",\n";
            ...
            cout << "\tbooktitle = \"Twentieth International Conference on Automated Planning and Scheduling "
                    "(ICAPS 10)\",\n";
            cout << "\tyear = \"2010\",\n";
            cout << "\tpublisher = \"AAAI Press\",\n";
            cout << "\tmonth = \"May\"\n";
        ...
        else
        {
            switch (argv[argcount][1])
            {
                ...
                case 'W':
                {
                    ...
                    if (Warg.empty())
                    {
                        cerr << "Error: must specify weight after W, e.g. -W5, or specify a weight and "
                                "restart weight reduction, e.g. -W5,1\n";
                        usage(argv);
                        exit(1);
                    }
                    ...
                    else
                    {
                        const string W = Warg.substr(0, commaAt);
                        {
                            ...
                            if (!(conv >> FF::doubleU))
                            {
                                cerr << "Error: must specify weight after W, e.g. -W5, or specify a weight "
                                        "and restart weight reduction, e.g. -W5,1\n";
                                usage(argv);
                                exit(1);
                            }
                        }
    ...
    if (Globals::optimiseSolutionQuality && Globals::givenSolutionQualityDefined)
    {
        ...
        {
            cout << "Forcing the use of the given solution quality of " << Globals::givenSolutionQuality
                 << endl;
            if (RPGBuilder::getMetric()->minimise)
            {
                Globals::bestSolutionQuality =
                    (Globals::givenSolutionQuality == 0.0 ? 0.0 : -Globals::givenSolutionQuality);
            }
            else
            {
    ...
    else
    {
        ...
        times(&refReturn);
        double secs =
            ((double)refReturn.tms_utime + (double)refReturn.tms_stime) / ((double)sysconf(_SC_CLK_TCK));
...
void split(const int &insAt, list<FFEvent>::iterator insStart, const list<FFEvent>::iterator &insItr,
           const list<FFEvent>::iterator &insEnd)
{
    {
...
list<FFEvent> *readPlan(char *filename)
{
    ...
    for (int idebug = 0, i = 0; planItr != planEnd; ++planItr, ++i, ++idebug)
    {
        ...
        instantiatedOp *const currOp =
            instantiatedOp::findInstOp(currStep->op_sym, currStep->params->begin(), currStep->params->end());
        if (!currOp)
        {
            const instantiatedOp *const debugOp = instantiatedOp::getInstOp(
                currStep->op_sym, currStep->params->begin(), currStep->params->end());
            cout << "Exiting: step " << idebug << " in the input plan uses the action " << *(debugOp)
                 << ", which the instantiation code in the planner does not recognise.\n";
            exit(1);
        }
        const int ID = currOp->getID();

        if (RPGBuilder::getRPGDEs(ID).empty())
        {  // non-durative action
            FFEvent toInsert(currOp, 0.001, 0.001);
            const double ts = currStep->start_time;
            if (debug)
            {
                cout << "; input " << ts << ": " << *currOp << " (id=" << ID << "), non-temporal";
            }
        ...
        else
        {
            ...
            for (int pass = 0; pass < 2; ++pass)
            {
                ...
                FFEvent toInsert =
                    (pass ? FFEvent(currOp, startIdx, actDur, actDur) : FFEvent(currOp, actDur, actDur));
                toInsert.lpTimestamp = ts;
                toInsert.lpMinTimestamp = ts;
                ...
                for (; insItr != insEnd; ++insItr, ++insAt)
                {
                    if (ts < insItr->lpTimestamp)
                    {
                        ...
                        if (pass)
                        {
                            ...
                            if (debug)
                            {
                                cout << " putting at step " << insAt << ", pairing with " << startIdx << "\n";
                            }
                        }
                        else
                        {
