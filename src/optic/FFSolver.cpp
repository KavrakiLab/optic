...
namespace Planner
{
    ...
    Solution::Solution() : plan(0), constraints(0), quality(std::numeric_limits<double>::signaling_NaN())
    {
    }

    void Solution::update(const std::list<FFEvent> &newPlan,
                          const Planner::TemporalConstraints *const newCons, const double &newMetric)
    {
        delete plan;
        delete constraints;
    ...
    double FF::evaluateMetric(const MinimalState &theState, const list<FFEvent> &plan, const bool printMetric)
    {
        ...
        if (printMetric)
        {
            ...
            /*if (theState.statusOfTemporalPreferences) {
                cout << "; Cost accumulated by within preferences: " <<
            theState.statusOfTemporalPreferences->getCost() << endl; cout << "; Violations:" <<
            theState.statusOfTemporalPreferences->getWithinViolations() << endl;
            }*/
        ...
        for (; wItr != wEnd; ++wItr, ++vItr)
        {
            if (*vItr < 0)
            {
                if (printMetric)
                {
                    cout << "; Adding " << *wItr << " x makespan to the metric, i.e. adding "
                         << getMakespan(plan) * *wItr << endl;
                }
            ...
            else if (*vItr < pneCount)
            {
                ...
                if (printMetric)
                {
                    cout << "; Adding " << *wItr << " x " << *(RPGBuilder::getPNE(*vItr))
                         << " to the metric, i.e. adding " << value * *wItr << endl;
                }
            }
            else
            {
    ...
    double calculateAdmissibleCost(const MinimalState &theState, const double &makespan,
                                   const double &incomingBound, const bool &verbose)
    {
        RPGBuilder::Metric *const theMetric = RPGBuilder::getMetric();
        ...
        else
        {
            ...
            if (NumericAnalysis::theMetricIsMonotonicallyWorsening())
            {
                ...
                {
                    const double bestAdditionalRewardFromHere =
                        NumericAnalysis::bestAdditionalRewardFromHere(theState);
                ...
                for (; wItr != wEnd; ++wItr, ++vItr)
                {
                    if (*vItr < 0)
                    {
                        if (*wItr < 0.0)
                        {
                            if (verbose)
                            {
                                cout << "[Would need upper-bound on makespan]";
                                cout.flush();
                            }
                    ...
                    else if (*vItr < pneCount)
                    {
                        ...
                        if (localDebug)
                        {
                            cout << "+ " << value << "*" << *wItr << "  ; " << *(RPGBuilder::getPNE(*vItr))
                                 << " term\n";
                        }
                    }
                    else
                    {
                        ...
                        {
                            cout << "+ " << value << "*" << *wItr << "  ; "
                                 << *(RPGBuilder::getPNE(*vItr - pneCount)) << " term\n";
                        }
                    }
                }
            }
    ...
    bool admissibleCostExceedsBound(
        /*const MinimalState & theState, */ const double &precalculatedAdmissibleCost, const bool &verbose)
    {
        ...
        {
            // cout << "Bound exceeded: this state has cost " << toReturn << " vs best " <<
            // Globals::bestSolutionQuality << endl;
            if (Globals::globalVerbosity & 1)
            {
                // cout << COLOUR_light_green << "[" << gCost << "^vs" << Globals::bestSolutionQuality << "]"
                // << COLOUR_default;
    ...
    pair<bool, bool> FF::carryOnSearching(const MinimalState &theState, const list<FFEvent> &plan,
                                          const pair<bool, double> &currentCost, const double &gCost,
                                          bool &wasNewBestSolution)
    {
        wasNewBestSolution = false;
        if (!Globals::optimiseSolutionQuality)
        {
            workingBestSolution.update(plan, theState.temporalConstraints,
                                       evaluateMetric(theState, plan, false));
            if (Globals::globalVerbosity & 1)
            {
                cout << "g";
                cout.flush();
        ...
        if (currentCost.first)
        {
            ...
            {
                cout << "; Using cost from the LP of " << currentCost.second
                     << ", in case it was a continuous cost function" << endl;
            }
        ...
        if (metric > Globals::bestSolutionQuality + 0.01)
        {
            ...
            {
                workingBestSolution.update(plan, theState.temporalConstraints,
                                           evaluateMetric(theState, plan, true));
            }
            ...
            if (Globals::optimiseSolutionQuality)
            {
                cout << "; States pruned based on pre-heuristic cost lower bound: "
                     << statesDiscardedAsTooExpensiveBeforeHeuristic << endl;
            }
#endif
            ...
            else if (theMetric->minimise && theMetric->variables.size() == 1 &&
                     theMetric->variables.front() < 0)
            {
                metricIsMinimiseMakespan = true;
            }
    ...
    "\n"; const double newStartMin = lpMinTimestamp - toUpdate->maxDuration; if (newStartMin >
    toUpdate->lpMinTimestamp) { toUpdate->lpMinTimestamp = newStartMin;
        }
    ...
    namespace CSBase
    {
        ...
        int compareAnnotations(const map<int, PropositionAnnotation> &a,
                               const map<int, PropositionAnnotation> &b)
        {
        ...
        void skipTerminates(list<StartEvent>::const_iterator &itr,
                            const list<StartEvent>::const_iterator &itrEnd)
        {
            while (itr != itrEnd && itr->terminated)
            {
                ++itr;
            }
        }
        ...
        int compareVecs(const vector<double> &a, const vector<double> &b)
        {
            ...
            for (int i = 0; i < aSize; ++i)
            {
                if (!ignorableFluents[i])
                {
                    av = a[i];
                    bv = b[i];
                    // cout << *(RPGBuilder::getPNE(i)) << ": " << av << " vs " << bv << " in
                    // compareVecs()\n";
                    if (av < bv - 0.0005)
                    {
                        return 1;
        ...
        bool propAndNonDominatedVariableLessThan(const ExtendedMinimalState &ae,
                                                 const ExtendedMinimalState &be)
        {
    ...
    bool SecondaryExtendedStateLessThan::operator()(const ExtendedMinimalState *const ae,
                                                    const ExtendedMinimalState *const be) const
    {
        return operator()(*ae, *be);
    }

    bool SecondaryExtendedStateLessThan::operator()(const ExtendedMinimalState &ae,
                                                    const ExtendedMinimalState &be) const
    {
        return CSBase::secondaryLessThan(ae, be);
    }

    bool WeakExtendedStateLessThan::operator()(const ExtendedMinimalState *const ae,
                                               const ExtendedMinimalState *const be) const
    {
        return operator()(*ae, *be);
    }

    bool WeakExtendedStateLessThan::operator()(const ExtendedMinimalState &ae,
                                               const ExtendedMinimalState &be) const
    {
        return CSBase::baseLessThan(ae, be);
    }

    bool FullExtendedStateLessThan::operator()(const ExtendedMinimalState *const ae,
                                               const ExtendedMinimalState *const be) const
    {
        return operator()(*ae, *be);
    }

    bool FullExtendedStateLessThan::operator()(const ExtendedMinimalState &ae,
                                               const ExtendedMinimalState &be) const
    {
        return CSBase::fullLessThan(ae, be);
    }

    bool ExtendedStateLessThanOnPropositionsAndNonDominatedVariables::operator()(
        const ExtendedMinimalState *const ae, const ExtendedMinimalState *const be) const
    {
        return operator()(*ae, *be);
    }

    bool ExtendedStateLessThanOnPropositionsAndNonDominatedVariables::operator()(
        const ExtendedMinimalState &ae, const ExtendedMinimalState &be) const
    {
    ...
    public:
        ...
        SearchQueueItem() : internalState(0), ownState(false)
        {
#ifdef STATEHASHDEBUG
                    ...
                    else
                    {
                        ...
                        if (planItr->action)
                        {
                            cout << i << ": " << *(planItr->action) << ", "
                                 << (planItr->time_spec == Planner::E_AT_START ? "start" : "end");
                        }
    ...
    public:
        ...
        {
            list<SearchQueueItem *> &q =
                (category == 1 ? qOne[p->heuristicValue.qbreak] : qTwo[p->heuristicValue.qbreak]);
        ...
        void insert(SearchQueueItem *p, const int category = 1)
        {
            ...
            if (FF::WAStar)
            {
                ...
                {
                    const int pps = p->state()->getInnerState().planLength -
                                    p->state()->getInnerState().actionsExecuting -
                                    p->state()->getInnerState().nextTIL;
                    prim += pps;
                }
            }
            ...
            {
                const double pCost =
                    p->heuristicValue.admissibleCostEstimate;  // p->state()->getInnerState().calculateCost();
                list<SearchQueueItem *>::iterator qItr = q.begin();
                const list<SearchQueueItem *>::iterator qEnd = q.end();
                for (; qItr != qEnd; ++qItr)
                {
                    if (pCost < (*qItr)->heuristicValue.admissibleCostEstimate)
                    {  //  (*qItr)->state()->getInnerState().calculateCost()) {
                        q.insert(qItr, p);
                        return;
                    }
                }
    ...
    void populateTimestamps(vector<double> &minTimestamps, double &makespan, list<FFEvent> &header,
                            list<FFEvent> &now)
    {
        int stepID = 0;
    ...
    FF::HTrio FF::calculateHeuristicAndSchedule(
        ExtendedMinimalState &theState, ExtendedMinimalState *prevState, set<int> &goals,
        set<int> &goalFluents, ParentData *const incrementalData, list<ActionSegment> &helpfulActions,
        pair<bool, double> &currentCost, list<FFEvent> &header, list<FFEvent> &now, const int &stepID,
        bool considerCache, map<double, list<pair<int, int>>> *justApplied, double tilFrom)
    {
        ...
        //  LPScheduler tryToSchedule(header, now, theState.entriesForAction, (prevState ?
        //  &prevState->secondMin : 0), (prevState ? &prevState->secondMax : 0));
        ...
        LPScheduler tryToSchedule(theState.getInnerState(), theState.getEditableInnerState().preferenceStatus,
                                  header, now, stepID, theState.startEventQueue, incrementalData,
                                  theState.entriesForAction,
                                  (prevState ? &prevState->getInnerState().secondMin : 0),
                                  (prevState ? &prevState->getInnerState().secondMax : 0),
                                  &(theState.tilComesBefore), scheduleToMetric);

        if (scheduleToMetric)
        {
            HTrio toReturn(0, 0.0, 0.0,
                           theState.getInnerState().planLength - theState.getInnerState().actionsExecuting,
        ...
        LPScheduler tryToScheduleAgain(scheduleTwiceState, scheduleTwiceState.preferenceStatus, header, now,
                                       stepID, theState.startEventQueue, incrementalData,
                                       theState.entriesForAction,
                                       (prevState ? &prevState->getInnerState().secondMin : 0),
                                       (prevState ? &prevState->getInnerState().secondMax : 0),
                                       &(theState.tilComesBefore), scheduleToMetric);
        ...
        tryToSchedule.updateStateFluents(theState.getEditableInnerState().secondMin,
                                         theState.getEditableInnerState().secondMax,
                                         timeAtWhichValueIsDefined);

#ifdef SCHEDULETWICE
        vector<double> timeAtWhichValueIsDefinedAgain;
        tryToScheduleAgain.removeExpiredAbstractFacts(scheduleTwiceState.first);
        tryToScheduleAgain.updateStateFluents(scheduleTwiceState.secondMin, scheduleTwiceState.secondMax,
                                              timeAtWhichValueIsDefinedAgain);
#endif
        ...
        if (LPScheduler::workOutFactLayerZeroBoundsStraightAfterRecentAction)
        {
            ...
            tryToSchedule.extrapolateBoundsAfterRecentAction(&(theState.startEventQueue), extrapolatedMin,
                                                             extrapolatedMax, timeAtWhichValueIsDefined);
        }

#ifndef NDEBUG
        if (Globals::optimiseSolutionQuality && !RPGBuilder::getPreferences().empty() &&
            tryToSchedule.werePlanCostsCalculated())
        {
            if (NumericAnalysis::theMetricIsMonotonicallyWorsening())
            {
                if (RPGBuilder::getMetric()->minimise)
                {
                    if (!(tryToSchedule.getReachablePlanCost() >=
                          RPGBuilder::getMetric()->constant +
                              PreferenceHandler::getReachableCost(theState.getInnerState()) - 0.001))
                    {
                        std::cerr << "Fatal internal error: LP reachable cost estimate of "
                                  << tryToSchedule.getReachablePlanCost()
                                  << " should not be less than reachable preference cost of "
                                  << PreferenceHandler::getReachableCost(theState.getInnerState()) << endl;
                        std::cerr << "; Current violations: "
                                  << PreferenceHandler::getCurrentViolations(theState.getInnerState())
                                  << endl;

                        assert(tryToSchedule.getReachablePlanCost() >=
                               PreferenceHandler::getReachableCost(theState.getInnerState()) - 0.001);
                    }
                }
                else
                {
                    if (!(tryToSchedule.getReachablePlanCost() <=
                          RPGBuilder::getMetric()->constant +
                              PreferenceHandler::getReachableCost(theState.getInnerState()) + 0.001))
                    {
                        std::cerr << "Fatal internal error: LP reachable reward estimate of "
                                  << tryToSchedule.getReachablePlanCost()
                                  << " should not be more than reachable preference cost of "
                                  << PreferenceHandler::getReachableCost(theState.getInnerState()) << endl;
                        std::cerr << "; Current violations: "
                                  << PreferenceHandler::getCurrentViolations(theState.getInnerState())
                                  << endl;
        ...
        if (NumericAnalysis::theMetricIsMonotonicallyWorsening() && tryToSchedule.werePlanCostsCalculated() &&
            Globals::bestSolutionQuality != -DBL_MAX)
        {
            ...
            {
                cout << COLOUR_light_green << "Cost limit: " << costLimit << ", given reachable plan cost of "
                     << tryToSchedule.getReachablePlanCost() << " and incumbent of quality "
                     << Globals::bestSolutionQuality << COLOUR_default << endl;
            }
        }
...
#ifdef RELAXEDPLANTWICE
        {
            ...
            delete RPGBuilder::getHeuristic()->getRelaxedPlan(
                dummy, &(theState.startEventQueue), minTimestamps, dummyTimestamp, costLimit, extrapolatedMin,
                extrapolatedMax, timeAtWhichValueIsDefined,  // for colin-jair heuristic
        ...
        if (considerCache)
        {
            if (FFcache_upToDate)
            {
                ...
                helpfulActions.insert(helpfulActions.end(), FFcache_helpfulActions.begin(),
                                      FFcache_helpfulActions.end());
            ...
            else
            {
                ...
                h = auto_ptr<RPGHeuristic::EvaluationInfo>(RPGBuilder::getHeuristic()->getRelaxedPlan(
                    theState.getEditableInnerState(), &(theState.startEventQueue), minTimestamps,
                    theState.timeStamp, costLimit, extrapolatedMin, extrapolatedMax,
                    timeAtWhichValueIsDefined,  // for colin-jair heuristic
        ...
        if (h->getH() < 0)
        {
            return HTrio(-1.0, DBL_MAX, DBL_MAX, INT_MAX, "RPG heuristic detected a deadend");
        }
        ...
        else
        {
            ...
            {
                cycle = !tryToSchedule.addRelaxedPlan(
                    theState.startEventQueue, theState.getEditableInnerState().preferenceStatus,
                    theState.getEditableInnerState().prefPreconditionViolations, header, now, relaxedPlan,
                    stepID);
                if (!cycle)
                {
                    tryToSchedule.removeExpiredAbstractFacts(theState.getEditableInnerState().first);
...
#ifdef SCHEDULETWICE
                tryToScheduleAgain.addRelaxedPlan(
                    theState.startEventQueue, scheduleTwiceState.preferenceStatus,
                    scheduleTwiceState.prefPreconditionViolations, header, now, relaxedPlan, stepID);
                if (!cycle)
                {
                    tryToScheduleAgain.removeExpiredAbstractFacts(scheduleTwiceState.first);
                }
        ...
        if (h->goalState)
        {
            ...
            if (FF::relaxMIP)
            {
                ...
                mipModel = auto_ptr<LPScheduler>(new LPScheduler(
                    theState.getInnerState(), theState.getEditableInnerState().preferenceStatus, header, now,
                    stepID, theState.startEventQueue, incrementalData, theState.entriesForAction,
                    (prevState ? &prevState->getInnerState().secondMin : 0),
                    (prevState ? &prevState->getInnerState().secondMax : 0), &(theState.tilComesBefore),
                    scheduleToMetric));
        ...
        if (cycle)
        {
            ...
            return HTrio(-1.0, DBL_MAX, DBL_MAX, INT_MAX,
                         "Cycle detected when adding relaxed plan data to LP");
        }
        ...
        HTrio toReturn(h->getH(), makespan, makespanEstimate,
                       theState.getInnerState().planLength - theState.getInnerState().actionsExecuting,
                       "Evaluated Successfully", h->goalState);
        ...
        if (scheduleToUse->werePlanCostsCalculated())
        {
            ...
            if (NumericAnalysis::theMetricIsMonotonicallyWorsening())
            {
                if (RPGBuilder::getMetric()->minimise)
                {
                    ...
                    {
                        // cout << "LP boosted admissible cost estimate from " <<
                        // toReturn.admissibleCostEstimate << " to " << tryToSchedule.getReachablePlanCost()
                        // << endl;
                        toReturn.admissibleCostEstimate = scheduleToUse->getReachablePlanCost();
                    }
                    else
                    {
                        // cout << "LP could not do better than the existing admissible cost estimate of " <<
                        // toReturn.admissibleCostEstimate << endl;
                    }
                }
                else
                {
    ...
    FF::HTrio FF::calculateHeuristicAndCompressionSafeSchedule(
        ExtendedMinimalState &theState, ExtendedMinimalState *prevState, set<int> &goals,
        set<int> &goalFluents, list<ActionSegment> &helpfulActions, list<FFEvent> &header, list<FFEvent> &now,
        const int &stepID, map<double, list<pair<int, int>>> *justApplied, double tilFrom)
    {
        if (evaluationDiagnostics)
        {
        ...
        double makespanEstimate = 0.0;
        auto_ptr<RPGHeuristic::EvaluationInfo> h(RPGBuilder::getHeuristic()->getRelaxedPlan(
            theState.getEditableInnerState(), &(theState.startEventQueue), minTimestamps, theState.timeStamp,
            -DBL_MAX, theState.getInnerState().secondMin, theState.getInnerState().secondMax,
            timeAtWhichValueIsDefined,  // for colin-jair heuristic
            helpfulActions, relaxedPlan, makespanEstimate, justApplied, tilFrom));
        ...
        HTrio toReturn(h->getH(), makespan, makespanEstimate,
                       theState.getInnerState().planLength - theState.getInnerState().actionsExecuting,
                       "Compression-Safe Evaluated Successfully", h->goalState);
        toReturn.admissibleCostEstimate = h->admissibleReachablePrefCost;

        return toReturn;
    }

    ExtendedMinimalState *FF::applyActionToState(ActionSegment &actionToApply,
                                                 const ExtendedMinimalState &parent,
                                                 const list<FFEvent> &plan,
                                                 list<pair<int, FFEvent>> &newDummySteps)
    {
        ...
        assert(RPGBuilder::getHeuristic()->testApplicability(parent.getInnerState(), parent.timeStamp,
                                                             actionToApply, true));
...

        {
            ...
            if (actionToApply.second == Planner::E_AT)
            {
                ...
                if (s + extraForTILs * 2 >= tsVecSize)
                {  // *2 because we might need a dummy step after each TIL for preferences
            ...
            {
                if (s + 4 >= tsVecSize)
                {  // 4 because we might need a dummy step after each stage of the snap-action
                    tsVecSize += 14;
                    minTimestamps.resize(tsVecSize, 0.0);
                }
        ...
        {  // til actions
            ...
            ExtendedMinimalState *toReturn =
                parent.applyAction(actionToApply, minTimestamps, newDummySteps, constraintsSatisfied);
        ...
        RPGBuilder::LinearEffects *const lEffs =
            RPGBuilder::getLinearDiscretisation()[actionToApply.first->getID()];

        if (actionToApply.second != Planner::E_AT_END)
        {
            pair<double, double> actDur(RPGBuilder::getOpDuration(
                actionToApply.first->getID(),
                (actionToApply.second == Planner::E_OVER_ALL ? actionToApply.divisionID + 1 : 0),
                parent.getInnerState().secondMin, parent.getInnerState().secondMax));
            ...
            {
                cout << "- Calculated duration of new action as being in the range [" << minDur << ","
                     << maxDur << "]\n";
            }
        ...
        {
            map<int, list<list<StartEvent>::iterator>>::const_iterator tsiOld =
                parent.entriesForAction.find(actionToApply.first->getID());
        ...
        ExtendedMinimalState *const toReturn = parent.applyAction(actionToApply, minTimestamps, newDummySteps,
                                                                  constraintsSatisfied, minDur, maxDur);
        ...
        if (actionToApply.second == Planner::E_AT_START)
        {
            if (!nonTemporal)
            {
                ...
                {
                    toReturn->startEventQueue.push_back(
                        StartEvent(actionToApply.first->getID(), 0,
                                   toReturn->getInnerState().planLength - (2 + newDummySteps.size()), minDur,
                                   maxDur, 0.0));
                }
                else
                {
                    ...
                    // cout << "Now " << theState.startedActions[actionToApply.first->getID()][0] << "
                    // instances of " << *(actionToApply.first) << " going\n";
                }
            }
        }
        else if (actionToApply.second == Planner::E_AT_END)
        {
            map<int, list<list<StartEvent>::iterator>>::iterator efaItr =
                toReturn->entriesForAction.find(actionToApply.first->getID());
            ...
            if (lEffs)
            {
                ...
                {
                    if ((*efaMatch)->divisionsApplied == intToMatch)
                    {
                        break;
                    }
                }
                ...
                for (; niItr != niEnd; ++niItr)
                {
                    if (checked.insert(*niItr).second)
                    {
                        const RPGBuilder::RPGNumericPrecondition &currPre =
                            RPGBuilder::getNumericPreTable()[*niItr];
                        if (!currPre.isSatisfiedWCalculate(toReturn->getInnerState().secondMin,
                                                           toReturn->getInnerState().secondMax))
                        {
                            if (Globals::globalVerbosity & 8192)
                            {
                                cout << "Failed temporal soundness check: invariant " << currPre << " of "
    ...
    bool FF::stateHasProgressedBeyondItsParent(const ActionSegment &theAction,
                                               const ExtendedMinimalState &parent,
                                               const ExtendedMinimalState &child)
    {
        ...
        if (!numStartEffs.empty())
        {
            ...
            const vector<NumericAnalysis::dominance_constraint> &dom =
                NumericAnalysis::getDominanceConstraints();
        ...
        if (!RPGBuilder::getRPGDEs(theAction.first->getID()).empty())
        {
            ...
            if (RPGBuilder::getLinearDiscretisation()[theAction.first->getID()] &&
                !RPGBuilder::getLinearDiscretisation()[theAction.first->getID()]->vars.empty())
            {
                return true;
            }
        }
    ...
    void ExtendedMinimalState::deQueueFirstOf(const int &actID, const int &divID)
    {
        ...
        if (tsiOld->second.empty())
        {
            entriesForAction.erase(tsiOld);
        }
    ...
    void orderAllAppropriateBefore(TemporalConstraints *cons, const list<FFEvent> &header,
                                   const list<FFEvent> &nowList, const Planner::time_spec &time_spec,
                                   const int &prefID, const int &comeBeforeStep)
    {
        int stepID = 0;
    ...
    void FF::pairDummyStepsWithRealSteps(MinimalState &theState, int &currStepID, list<FFEvent> &header,
                                         list<FFEvent> &nowList,
                                         const list<pair<int, FFEvent>> &newDummySteps)
    {
            ...
            switch (pref.cons)
            {
                ...
                case VAL::E_SOMETIMEBEFORE:
                {
                    ...
                    if (nowList.back().time_spec == Planner::E_DUMMY_TEMPORAL_TRIGGER_TRUE)
                    {
                        ...
                        if (pairingDebug)
                        {
                            cout << COLOUR_light_green << "Ordering step " << stepID << ", dummy for "
                                 << pref.name << " after earlier requirement-meeting dummies:\n"
                                 << COLOUR_default;
                        }

                        orderAllAppropriateBefore(theState.temporalConstraints, header, nowList,
                                                  Planner::E_DUMMY_TEMPORAL_GOAL_TRUE, prefID, stepID);
                    }
                ...
                case VAL::E_ALWAYSWITHIN:
                {
                    ...
                    if (nowList.back().time_spec == Planner::E_DUMMY_TEMPORAL_GOAL_TRUE)
                    {
                        ...
                        {
                            cout << COLOUR_light_green << "Ordering step " << stepID << ", dummy for "
                                 << pref.name << " after earlier trigger-meeting dummies:\n"
                                 << COLOUR_default;
                        }

                        orderAllAppropriateBefore(theState.temporalConstraints, header, nowList,
                                                  Planner::E_DUMMY_TEMPORAL_TRIGGER_TRUE, prefID, stepID);
                    }
    ...
    void FF::evaluateStateAndUpdatePlan(auto_ptr<SearchQueueItem> &succ, ExtendedMinimalState &state,
                                        ExtendedMinimalState *prevState, set<int> &goals,
                                        set<int> &goalFluents, ParentData *const incrementalData,
                                        list<ActionSegment> &helpfulActionsExport,
                                        pair<bool, double> &currentCost, const ActionSegment &actID,
                                        list<FFEvent> &header, const list<pair<int, FFEvent>> &newDummySteps)
    {
#ifdef POPF3ANALYSIS

        if (Globals::optimiseSolutionQuality &&
            admissibleCostExceedsBound(succ->heuristicValue.admissibleCostEstimate, false))
        {
        ...
        if (actID.second == Planner::E_AT_START)
        {
            ...
            extraEvent = FFEvent(actID.first, state.startEventQueue.back().minDuration,
                                 state.startEventQueue.back().maxDuration);
            eventOneDefined = true;

            assert(extraEvent.time_spec == Planner::E_AT_START);
            makeJustApplied(actualJustApplied, tilFrom, state, true);
            if (!actualJustApplied.empty())
            {
                justApplied = &actualJustApplied;
            }
            ...
            {  // if it's not a non-temporal action
                ...
                extraEventTwo = FFEvent(actID.first, startStepID, state.startEventQueue.back().minDuration,
                                        state.startEventQueue.back().maxDuration);
        ...
        else if (actID.second == Planner::E_AT_END)
        {
            ...
            map<int, list<list<StartEvent>::iterator>>::iterator tsiOld =
                state.entriesForAction.find(actID.first->getID());
            ...
            if (tsiOld->second.empty())
            {
                state.entriesForAction.erase(tsiOld);
            }

            if (rawDebug || Globals::globalVerbosity & 1048576)
            {
                cout << "Pairing with start at step " << pairWith->stepID << ", so activating end at "
                     << pairWith->stepID + 1 << "\n";
            }
        ...
        pairDummyStepsWithRealSteps(state.getEditableInnerState(), currStepID, succ->plan, nowList,
                                    newDummySteps);
        ...
        {
            if (!durationManager->updateTimestampsOfNewPlanStep(
                    prevState->getInnerState(), state.getEditableInnerState(), succ->plan, reusedEndEvent, 0,
                    stepID, isAStochasticGoalState))
            {
        ...
        else
        {
            ...
            if (!durationManager->updateTimestampsOfNewPlanStep(
                    prevState->getInnerState(), state.getEditableInnerState(), succ->plan, local1, local2,
                    stepID, isAStochasticGoalState))
            {
                succ->heuristicValue = HTrio(-1.0, DBL_MAX, DBL_MAX, INT_MAX, "Exceeded stochastic deadline");
                return;
        ...
        {
            h1 = calculateHeuristicAndCompressionSafeSchedule(state, prevState, goals, goalFluents,
                                                              helpfulActions, succ->plan, nowList, stepID,
                                                              justApplied, tilFrom);
        }
        else
        {
            h1 = calculateHeuristicAndSchedule(state, prevState, goals, goalFluents, incrementalData,
                                               helpfulActions, currentCost, succ->plan, nowList, stepID, true,
                                               justApplied, tilFrom);
        }
        ...
        if (RPGBuilder::getMetric())
        {
            if (Globals::optimiseSolutionQuality)
            {
                ...
                {
                    succ->heuristicValue = HTrio(-1.0, DBL_MAX, DBL_MAX, INT_MAX,
                                                 "Cannot better the cost of the incumbent solution");
                    return;
                }
            }
        }
        ...
        {
            if (!RPGBuilder::getRPGDEs(actID.first->getID()).empty() &&
                TemporalAnalysis::canSkipToEnd(actID.first->getID()))
            {  // if it's a compression-safe temporal action

                const int endStepID = prevState->getInnerState().planLength + 1;

                state.getEditableInnerState().updateWhenEndOfActionIs(actID.first->getID(), endStepID,
                                                                      extraEventTwo.lpMinTimestamp);
            }
        }
#endif
#endif
    ...
    bool FF::precedingActions(ExtendedMinimalState &theState, const ActionSegment &actionSeg,
                              list<ActionSegment> &alsoMustDo, const int oldTIL, const double moveOn)
    {
        //  static const double EPSILON = 0.001;
            ...
            for (int i = tilCount - 1; i >= 0; --i)
            {
...

                {
                    ...
                    for (; effItr != effEnd; ++effItr)
                    {
                        ...
                        if (RPGBuilder::getEffectsToActions(currEffID).empty() &&
        ...
        if (ts == Planner::E_AT_START)
        {
            if (tsDebug)
            {
                cout << "Considering start of " << *(actionSeg.first) << "\n";
            }
                ...
                for (; tsiItr != tsiEnd; ++tsiItr)
                {
                    if (!tsiItr->terminated)
                    {
                        ...
                        if (wouldBe > tsiItr->maxDuration)
                        {
                            ...
                            {
                                alsoMustDo.push_back(
                                    ActionSegment(RPGBuilder::getInstantiatedOp(tsiItr->actID),
                                                  Planner::E_AT_END, -16, RPGHeuristic::emptyIntList));
                            ...
                            else
                            {
                                if (tsDebug)
                                {
                                    cout << "Now " << tsiItr->elapsed << " since start of "
                                         << *(RPGBuilder::getInstantiatedOp(tsiItr->actID)) << " pt. "
                                         << tsiItr->divisionsApplied << ", this exceeds duration\n";
                                }
                                return false;
                            }
                        }
        ...
        else if (ts == Planner::E_AT_END)
        {
            if (tsDebug)
            {
                cout << "Considering end of " << *(actionSeg.first) << "\n";
            }

            map<int, list<list<StartEvent>::iterator>>::iterator tsiOld =
                theState.entriesForAction.find(actionSeg.first->getID());
            assert(tsiOld != theState.entriesForAction.end());

            list<StartEvent>::iterator pairWith;

            {
                RPGBuilder::LinearEffects *leffs =
                    RPGBuilder::getLinearDiscretisation()[actionSeg.first->getID()];
                if (leffs)
                {
            ...
            if (tsDebug)
            {
                cout << "Started " << pairWith->elapsed << " ago, of duration [" << pairWith->minDuration
                     << "," << pairWith->maxDuration << "\n";
            }
            ...
            for (list<StartEvent>::iterator bqItr = theState.startEventQueue.begin(); bqItr != pairWith;
                 ++bqItr)
            {
                if (!bqItr->terminated)
                {
                    if (tsDebug)
                    {
                        cout << "Can advance " << *(RPGBuilder::getInstantiatedOp(bqItr->actID)) << " point "
                             << bqItr->divisionsApplied << "\n";
                    }
                    ...
                    if (wouldBe > bqItr->maxDuration)
                    {
                        ...
                        {
                            alsoMustDo.push_back(ActionSegment(RPGBuilder::getInstantiatedOp(bqItr->actID),
                                                               Planner::E_AT_END, -16,
                                                               RPGHeuristic::emptyIntList));
                        ...
                        else
                        {
                            if (tsDebug)
                            {
                                cout << "Now " << bqItr->elapsed << " since point " << bqItr->divisionsApplied
                                     << " of " << *(RPGBuilder::getInstantiatedOp(bqItr->actID))
            ...
            if (theState.getInnerState().nextTIL < tilCount)
            {
                if (tilStamps[theState.getInnerState().nextTIL] < theState.timeStamp + extraTime)
                {
                    if (tsDebug)
                    {
                        cout << "Too late for the next TIL: " << tilStamps[theState.getInnerState().nextTIL]
                             << "\n";
                    }
                    return false;
                }
                ...
                for (; theRestItr != theRestEnd; ++theRestItr)
                {
                    if (!theRestItr->terminated)
                    {
                        ...
                        if (wouldBe > theRestItr->maxDuration)
                        {
                            ...
                            {
                                alsoMustDo.push_back(
                                    ActionSegment(RPGBuilder::getInstantiatedOp(theRestItr->actID),
                                                  Planner::E_AT_END, -1, RPGHeuristic::emptyIntList));
                            ...
                            else
                            {
                                if (tsDebug)
                                {
                                    cout << "Now " << theRestItr->elapsed << " since point "
                                         << theRestItr->divisionsApplied << " of "
                                         << *(RPGBuilder::getInstantiatedOp(theRestItr->actID))
                                         << ", this exceeds duration\n";
                                }
                                return false;
                            }
    ...
    bool FF::checkTemporalSoundness(ExtendedMinimalState *prevState, ExtendedMinimalState &theState,
                                    const ActionSegment &actionSeg, const int oldTIL, const double moveOn)
    {
        ...
        if (ts == Planner::E_AT_START || ts == Planner::E_AT_END)
        {
            if (LPScheduler::optimiseOrdering)
            {
                ...
                for (; tsiItr != tsiEnd; ++tsiItr)
                {
                    if (!tsiItr->terminated && !tsiItr->ignore &&
                        !TemporalAnalysis::canSkipToEnd(tsiItr->actID))
                    {
                        if (!madeJustAdded)
                        {
                            for (int pass = 0; pass < 2; ++pass)
                            {
                                ...
                                const list<Literal *> &addList =
                                    (pass == 0 ?
                                         RPGBuilder::getStartPropositionAdds()[actionSeg.first->getID()] :
                        ...
                        const list<Literal *> &precs =
                            RPGBuilder::getEndPropositionalPreconditions()[tsiItr->actID];
                        ...
                        for (int fID; pItr != pEnd; ++pItr)
                        {
                            ...
                            {
                                // end of the action requires a fact that has just been added - force an
                                // ordering on this
                                ...
                                const StateBFacts::const_iterator stateBItr =
                                    theState.getInnerState().firstAnnotations.find(fID);
                                if (stateBItr != theState.getInnerState().firstAnnotations.end())
                                {
                                    ...
                                    for (; epItr != epEnd; ++epItr)
                                    {
                                        if (*epItr != stepID)
                                        {
                                            if (tsDebug)
                                            {
                                                cout << "Ordering future end of "
                                                     << *(RPGBuilder::getInstantiatedOp(tsiItr->actID))
                                                     << " after new fact "
                                                     << *(RPGBuilder::getLiteral(FACTA(ffItr))) << endl;
                                            }
                                            theState.getInnerState().temporalConstraints->addOrdering(
                                                *epItr, stepID, true);
                                        }
                                    }
                                }
                                ...
                                if (addedBy.stepID != stepID)
                                {
                                    if (tsDebug)
                                    {
                                        cout << "Ordering future end of "
                                             << *(RPGBuilder::getInstantiatedOp(tsiItr->actID))
                                             << " after new fact " << *(RPGBuilder::getLiteral(FACTA(ffItr)))
                                             << endl;
                                    }
                                    theState.getInnerState().temporalConstraints->addOrdering(addedBy.stepID,
                                                                                              stepID, true);
                                }
        ...
        if (ts == Planner::E_AT_START)
        {
            ...
            if (tsDebug)
            {
                cout << "Now " << moveOn << " since start of new action " << *(actionSeg.first) << "\n";
            }
            ...
            if (theState.getInnerState().nextTIL < tilCount)
            {
                ...
                const double earliestFinish =
                    theState.timeStamp +
                    (nonTemporal ? 0.0 : theState.startEventQueue.back().advancingDuration);
                ...
                {
                    if (tilStamps[tc] < earliestFinish)
                    {  // if TIL has to occur during the execution of this action
                        {
            ...
            if (!nonTemporal && TemporalAnalysis::canSkipToEnd(actionSeg.first->getID()) &&
                LPScheduler::optimiseOrdering)
            {
                ...
                if (!invs.empty())
                {
                    ...
                    for (; tsiItr != tsiEnd; ++tsiItr)
                    {
                        if (!tsiItr->terminated)
                        {
                            const list<Literal *> &openDels =
                                RPGBuilder::getEndPropositionDeletes()[tsiItr->actID];
                            ...
                            for (; odItr != odEnd; ++odItr)
                            {
                                ...
                                if (invSet.find((*odItr)->getStateID()) != invSet.end())
                                {
                                    ...
                                    if (tsDebug)
                                    {
                                        cout << COLOUR_light_green << "End of action just started ("
                                             << endOfNewStep << ") must come before " << tsiItr->stepID + 1
                                             << ", to avoid an invariant violation\n"
                                             << COLOUR_default;
                                    }
                        ...
                        for (; tsiItr != tsiEnd; ++tsiItr)
                        {
                            if (!tsiItr->terminated)
                            {
                                ...
                                if (consIncr < thisIncr && fabs(thisIncr - consIncr) > 0.0005)
                                {
                                    ...
                                    {
                                        cout << COLOUR_light_green << "End of action just started ("
                                             << endOfNewStep << ") must come after " << tsiItr->stepID + 1
                                             << ", due to long-inside-short avoidance\n"
                                             << COLOUR_default;
                                    }
                                }
                            }
                            // cout << "Now " << tsiItr->second << " since start of " <<
                            // *(RPGBuilder::getInstantiatedOp(tsiItr->first)) << "\n";
                        }
                        if (theState.getInnerState().nextTIL < tilCount)
                        {
                            const double consIncr =
                                tilStamps[theState.getInnerState().nextTIL] - theState.timeStamp;
                            if (consIncr < thisIncr && fabs(thisIncr - consIncr) > 0.0005)
                            {
                                ++thisFanIn;
                ...
                {
                    {
                        ...
                        if (!delEffs.empty())
                        {
                            ...
                            for (; tsiItr != tsiEnd; ++tsiItr)
                            {
                                ...
                                {
                                    bool precedes = false;
                                    const list<Literal *> &openInvs =
                                        RPGBuilder::getInvariantPropositionalPreconditions()[tsiItr->actID];
                                    ...
                                    for (; oiItr != oiEnd; ++oiItr)
                                    {
                                        if (deSet.find((*oiItr)->getStateID()) != deSet.end())
                                        {
                                            ...
                                            {
                                                cout << COLOUR_light_green << "End of action just started ("
                                                     << endOfNewStep << ") must come after "
                                                     << tsiItr->stepID + 1
                                                     << ", to avoid deleting one of its invariants\n"
                                                     << COLOUR_default;
                                            }
                                    ...
                                    if (!precedes)
                                    {
                                        const LiteralSet &thisEndOneShot =
                                            RPGBuilder::getEndOneShots(tsiItr->actID);
                                            ...
                                            else
                                            {
                                                ...
                                                if (tsDebug)
                                                {
                                                    cout << COLOUR_light_green
                                                         << "End of action just started (" << endOfNewStep
                                                         << ") must come after " << tsiItr->stepID + 1
                                                         << ", to avoid deleting one if its one-shot end "
                                                            "preconditions.\n"
                                                         << COLOUR_default;
                                                }
                                        ...
                                        if (oneShotEndPrecs.find(*odItr) != oneShotEndPrecs.end())
                                        {
                                            ...
                                            if (tsDebug)
                                            {
                                                cout << COLOUR_light_green << "End of action just started ("
                                                     << endOfNewStep << ") must come before "
                                                     << tsiItr->stepID + 1
                                                     << ", to avoid losing a one-shot end precondition\n"
                                                     << COLOUR_default;
                                            }
        ...
        {
            std::cerr << "Internal error: should not be able to apply actions with 'over all' time "
                         "specifier\n";
            exit(1);
        }
        else if (ts == Planner::E_AT_END)
        {
            ...
            const map<int, list<list<StartEvent>::iterator>>::iterator tsiOld =
                theState.entriesForAction.find(actionSeg.first->getID());
            assert(tsiOld != theState.entriesForAction.end());
            ...
            if (!pairWith->getEndComesBeforePair().empty())
            {
                if (tsDebug)
                {
                    cout << "ECB-pair list is not empty\n";
                }
                return false;
            }
            ...
            if (tsDebug)
            {
                cout << "State timestamp is now " << theState.timeStamp << "\n";
            }
            ...
            if (LPScheduler::optimiseOrdering)
            {
...

                {
                    ...
                    if (thisFanIn)
                    {
                        ...
                        for (list<StartEvent>::iterator bqItr = theState.startEventQueue.begin();
                             thisFanIn && bqItr != pairWith; ++bqItr)
                        {
                            if (!bqItr->terminated)
                            {
        ...
        {  // TIL
            ...
            for (int pass = oldTIL; pass <= actionSeg.divisionID; ++pass)
            {
                ...
                {
                    {
                        ...
                        if (!beforeList.empty())
                        {
                            ...
                            while (theRestItr != theRestEnd && theRestItr->stepID < theState.stepBeforeTIL)
                            {
                                ++theRestItr;
                            }
...

                    {
                        ...
                        if (thisFanIn)
                        {
                            ...
                            for (;
                                 thisFanIn && bqItr != bqEnd && bqItr->stepID < (theState.stepBeforeTIL - 1);
                                 ++bqItr)
                            {
                                if (!bqItr->terminated)
                                {
                ...
                // what about the case where the TIL thing is quite small but it has to end before the action
                // because it can't slide around so readily, i.e. no matter how far we shift an action to the
                // left, this is always going to go inside it. i.e. it's timestamp which we can back-infer +
                // min duration > timestamp + this til, we have an ordering.- think about this, it could be
                // good....
            }
        }
    ...
    /*
    bool FF::checkTSTemporalSoundness(RPGHeuristic* const rpg, ExtendedMinimalState & theState, const int &
    theAction, const Planner::time_spec & ts, const double & incr, const int oldTIL) {
    ...
    *(RPGBuilder::getInstantiatedOp(tsiItr->first)) << "\n";
            ...
            map<int, list<list<StartEvent>::iterator > >::iterator tsiOld =
    theState.entriesForAction.find(theAction); assert(tsiOld != theState.entriesForAction.end());

            if (localDebug) cout << theAction << " started " << tsiOld->second.front()->elapsed << " ago, of
    duration " << tsiOld->second.front()->maxDuration << "\n";
    ...
    */
        ...
        for (; itr != itrEnd; ++itr)
        {
            ...
            for (list<ActionSegment>::iterator hitr = hitrStart; hitr != hitrEnd; ++hitr, ++w)
            {
                if (itr->first == hitr->first && itr->second == hitr->second &&
                    itr->divisionID == hitr->divisionID)
                {
                    break;
                }
            }
    ...
    void FF::makeJustApplied(map<double, list<pair<int, int>>> &justApplied, double &tilFrom,
                             ExtendedMinimalState &state, const bool &lastIsSpecial)
    {
        ...
        double tilAdvance =
            (visitedTIL ? DBL_MAX : tilStamps[state.getInnerState().nextTIL] - state.timeStamp);
        ...
        while (seItr != seEnd || !visitedTIL)
        {
            if (!visitedTIL && (seItr == seEnd || seItr->stepID >= state.stepBeforeTIL))
            {
                ...
                if (!state.tilComesBefore.empty())
                {
                    ...
                    {
                        int &deref = *cbfItr;
                        while (seNext != seEnd && seNext->stepID != deref)
                        {
                            ++seNext;
                        }
                        if (seNext != seEnd)
                        {
                            if (seNext->minAdvance > maxIncr)
                            {
                                seNext->minAdvance = maxIncr;
                            }
                            ++seNext;
                        }
                    }
                ...
                {
                    const double newElapsed =
                        (state.getInnerState().nextTIL == 0 ?
                             state.timeStamp :
                             state.timeStamp - tilStamps[state.getInnerState().nextTIL - 1]) +
                        maxIncr;
                    const double equivalentDur =
                        tilStamps[state.getInnerState().nextTIL] -
                        (state.getInnerState().nextTIL == 0 ? 0.0 :
                                                              tilStamps[state.getInnerState().nextTIL - 1]);
            ...
            else
            {
                if (!seItr->ignore)
                {
...

                    {
                        ...
                        ///                 RPGBuilder::LinearEffects * const lEffs =
                        ///                 RPGBuilder::getLinearDiscretisation()[seItr->actID];
                        double remainingDur = 0.0;
                        ///                 if (lEffs && lEffs->divisions > 1) {
                        ///                     for (int d = seItr->divisionsApplied + 1; d <
                        ///                     lEffs->divisions; ++d) {
                        ...
                        {
                            justApplied[remainingDur].push_back(
                                pair<int, int>(seItr->actID, seItr->divisionsApplied));
                        }
                        else
                        {
                            ...
                            if (insLayer > EPSILON)
                            {
                                justApplied[insLayer].push_back(
                                    pair<int, int>(seItr->actID, seItr->divisionsApplied));
                                if (debugJA)
                                {
        ...
        if (lastIsSpecial)
        {
            ...
            ///     RPGBuilder::LinearEffects * const lEffs =
            ///     RPGBuilder::getLinearDiscretisation()[seEnd->actID];
            double remainingDur = 0.0;
            ///     if (lEffs && lEffs->divisions > 1) {
    ...
    public:
        ...
        virtual InsertIterator *insertState(SearchQueueItem *const s, StatesToDelete *const keptStates)
            __attribute__((warn_unused_result)) = 0;
        virtual InsertIterator *insertState(ExtendedMinimalState *const e)
            __attribute__((warn_unused_result)) = 0;
        virtual FindIterator *findState(ExtendedMinimalState *const e) const
            __attribute__((warn_unused_result)) = 0;
    ...
    public:
            ...
            __attribute__((warn_unused_result))
        {
            ...
            toReturn->outerInsertion =
                insert(make_pair(s->state(), make_pair(s->state()->timeStamp, emptyInner)));
            toReturn->innerInsertion = toReturn->outerInsertion.first->second.second.insert(
                make_pair(s->state(), s->state()->timeStamp));
        ...
        virtual InsertIterator *insertState(ExtendedMinimalState *const e) __attribute__((warn_unused_result))
        {
            ...
            toReturn->innerInsertion =
                toReturn->outerInsertion.first->second.second.insert(make_pair(e, e->timeStamp));
    ...
    typedef map<ExtendedMinimalState *, ParetoStatesAndOthers,
                ExtendedStateLessThanOnPropositionsAndNonDominatedVariables>
        DominanceMap;
    ...
    public:
        ...
        {
            const vector<NumericAnalysis::dominance_constraint> &dcs =
                NumericAnalysis::getDominanceConstraints();
        ...
        virtual InsertIterator *insertState(ExtendedMinimalState *const e) __attribute__((warn_unused_result))
        {
            ...
            if (toReturn->outerInsertion.second)
            {
                if (insDebug)
                {
                    cout << "No states previously at " << &(toReturn->outerInsertion.first->second) << ": "
                         << e << " is the first one" << endl;
                }

                // haven't seen anything with this combination of propositions and non-dominated variable
                // values before so, it's got to be new.
                toReturn->outerInsertion.first->second.paretoStates.push_front(make_pair(e, e->timeStamp));
                toReturn->paretoStateListIterator =
                    toReturn->outerInsertion.first->second.paretoStates.begin();
                toReturn->innerInsertion =
                    toReturn->outerInsertion.first->second.others.insert(make_pair(e, e->timeStamp));
            }
            else
            {
                ...
                list<pair<ExtendedMinimalState *, double>> &paretoStates =
                    toReturn->outerInsertion.first->second.paretoStates;

                if (insDebug)
                {
                    cout << "Checking " << paretoStates.size() << " pareto set of states at "
                         << &(toReturn->outerInsertion.first->second) << endl;
                }
                ...
                while (psItr != psEnd)
                {
                    ...
                    if (e->getInnerState().prefPreconditionViolations >
                        psItr->first->getInnerState().prefPreconditionViolations)
                    {
                        anythingWorse = true;
                    }
                    else if (e->getInnerState().prefPreconditionViolations <
                             psItr->first->getInnerState().prefPreconditionViolations)
                    {
                        anythingBetter = true;
                    }
                    ...
                    for (int i = 0; i < numberOfVarsWithDominanceConstraints; ++i)
                    {
                        ...
                        {
                            if (newStateMin[v] > oldStateMin[v] + 0.0005 ||
                                newStateMax[v] > oldStateMax[v] + 0.0005)
                            {
                                if (insDebug)
                                {
                                    cout << "New bounds on " << *(RPGBuilder::getPNE(v)) << " are better: ["
                                         << newStateMin[v] << ", " << newStateMax[v] << "] vs ["
                                         << oldStateMin[v] << ", " << oldStateMax[v] << "]\n";
                                }
                                anythingBetter = true;
                            }
                            if (newStateMin[v] < oldStateMin[v] - 0.0005 ||
                                newStateMax[v] < oldStateMax[v] - 0.0005)
                            {
                                if (insDebug)
                                {
                                    cout << "New bounds on " << *(RPGBuilder::getPNE(v)) << " are worse: ["
                                         << newStateMin[v] << ", " << newStateMax[v] << "] vs ["
                                         << oldStateMin[v] << ", " << oldStateMax[v] << "]\n";
                                }
                                anythingWorse = true;
...

                    {
                        ...
                        const vector<AutomatonPosition::Position> &existing =
                            psItr->first->getInnerState().preferenceStatus;
                        for (p = 0; p < tpCount; ++p)
                        {
                ...
                if (identicalToAnything || dominatedByAnything)
                {
                    if (insDebug)
                    {
                        cout << "As it's identical to something, or dominated by something, just doing a "
                ...
                else
                {
                    if (dominatesAnything || betterThanAnExistingStateInAtLeastOneWay)
                    {
                        if (insDebug)
                        {
                            cout << "As it dominates something, or is non-dominated, doing a primary "
                                    "insertion of "
                                 << e << " and adding to the pareto list\n";
                        }
                        ...
                        if (toReturn->innerInsertion.second)
                        {
                            toReturn->outerInsertion.first->second.paretoStates.push_front(
                                make_pair(e, e->timeStamp));
                            toReturn->paretoStateListIterator =
                                toReturn->outerInsertion.first->second.paretoStates.begin();
                        }
                        else
                        {
                            if (insDebug)
                            {
                                cout << "Actually just quietly doing a secondary insertion, as there looks "
                                        "to be a similar state from before\n";
                            }
                        }
        ...
        virtual FindIterator *findState(ExtendedMinimalState *const e) const
            __attribute__((warn_unused_result))
        {
            FindIterator *const toReturn =
                new FindIterator(this->DominanceMap::find(e), this->DominanceMap::end());
            if (toReturn->outerFind != end())
            {
                ...
                else
                {
                    ...
                    const list<pair<ExtendedMinimalState *, double>> &paretoStates =
                        toReturn->outerFind->second.paretoStates;

                    list<pair<ExtendedMinimalState *, double>>::const_iterator psItr = paretoStates.begin();
                    const list<pair<ExtendedMinimalState *, double>>::const_iterator psEnd =
                        paretoStates.end();
                    ...
                    for (; psItr != psEnd; ++psItr)
                    {
                        ...
                        if (anythingWorse)
                        {
                            ...
                            {
                                // something is worse but nothing is better - existing state dominates this
                                // one
                                dominatedByAnything = true;
                                break;
                            }
    ...
    {
        if (useDominanceConstraintsInStateHash &&
            (DominanceStateHash::countDominatedVariables() || !RPGBuilder::getPreferences().empty()))
        {
    ...
    list<FFEvent> *FF::doBenchmark(bool &reachedGoal, list<FFEvent> *oldSoln, const bool doLoops)
    {
        ...
        if (!initCSBase)
        {
            ...
            {
                CSBase::ignorableFluents[i] =
                    ((!Globals::optimiseSolutionQuality && dcs[i] == NumericAnalysis::E_METRICTRACKING) ||
                     (dcs[i] == NumericAnalysis::E_IRRELEVANT));
                CSBase::nonDominatedFluent[i] =
                    (!(CSBase::ignorableFluents[i]) && dcs[i] == NumericAnalysis::E_NODOMINANCE);
            }
        }
        ...
        {
            initialState.getEditableInnerState().preferenceStatus =
                PreferenceHandler::getInitialAutomataPositions();
            ...
            if (tdrFactCount)
            {
                double *toFill = initialState.getEditableInnerState().lowerBoundOnTimeDependentRewardFacts =
                    new double[tdrFactCount];
...

        {
            ...
            for (; gsItr != gsEnd; ++gsItr)
            {
                ...
                if (currStatic.first)
                {
                    ...
                    {
                        cout << "Static goal " << *(*gsItr)
                             << " resolves to false: no plan can solve this problem\n";
...
/*if (reprocessQualityBound != std::numeric_limits<double>::signaling_NaN() && RPGBuilder::getMetric()) {
    if (reprocessQualityBound == 0.0) {
...
#ifdef ENABLE_DEBUGGING_HOOKS
        {
            ...
            for (; rsItr != osEnd; ++rsItr)
            {
                ...
                {
                    Globals::remainingActionsInPlan.push_back(
                        ActionSegment((instantiatedOp *)0, Planner::E_AT, rsItr->divisionID, set<int>()));
                }
                else
                {
                    Globals::remainingActionsInPlan.push_back(
                        ActionSegment(rsItr->action, rsItr->time_spec, rsItr->divisionID, set<int>()));
                }
            }
        }
#endif
        ...
        if (!doLoops)
        {
            ...
            {
                h2.makespan = 0.0;
                h2.admissibleCostEstimate = calculateAdmissibleCost(
                    currSQI->state()->getInnerState(), 0.0,
                    (RPGBuilder::getMetric() && !RPGBuilder::getMetric()->minimise ? DBL_MAX : 0.0), false);
            ...
            else
            {
                ...
                if (FF::allowCompressionSafeScheduler)
                {
                    h1 = calculateHeuristicAndCompressionSafeSchedule(*(currSQI->state()), 0, goals,
                                                                      numericGoals, currSQI->helpfulActions,
                                                                      currSQI->plan, tEvent, -1);
                }
                else
                {
                    ...
                    h1 = calculateHeuristicAndSchedule(*(currSQI->state()), 0, goals, numericGoals,
                                                       (ParentData *)0, currSQI->helpfulActions, currentCost,
                                                       currSQI->plan, tEvent, -1);
                }
        ...
        for (int i = 1; osItr != osEnd; ++osItr, ++i)
        {
            ...
            ActionSegment actID(osItr->action, osItr->time_spec, osItr->divisionID,
                                RPGHeuristic::emptyIntList);
...

            {
                ...
                RPGBuilder::getHeuristic()->findApplicableActions(
                    currSQI->state()->getInnerState(), currSQI->state()->timeStamp, maybeApplicableActions);
                ...
                if (appItr == appEnd)
                {
                    cout << COLOUR_light_red
                         << "*** Action was not considered applicable by findApplicableActions()\n"
                         << COLOUR_default;
                    exit(1);
                }
            }
            ...
            auto_ptr<ParentData> pd(
                FF::allowCompressionSafeScheduler ?
                    0 :
            ...
            SearchQueueItem *succ = new SearchQueueItem(
                applyActionToState(actID, *(currSQI->state()), currSQI->plan, newDummySteps), true);

            succ->heuristicValue.makespan = currSQI->heuristicValue.makespan;

            if (!succ->state())
            {
                cout << "Takes us to a null state: trying to start an action whose min duration exceeds its "
                        "max\n";
            }
            ...
            if (!stateHasProgressedBeyondItsParent(actID, *(currSQI->state()), *(succ->state())))
            {
                cout << COLOUR_light_blue
                     << "*** Would prune the state, as it doesn't look to have improved on its parent\n"
                     << COLOUR_default;
            }

            if (!RPGBuilder::getPreferences().empty())
            {
                cout << COLOUR_yellow << "Current "
                     << PreferenceHandler::getCurrentViolations(succ->state()->getInnerState())
                     << " with precondition violations "
                     << succ->state()->getInnerState().prefPreconditionViolations;
                cout.flush();
                cout << ", can reach " << PreferenceHandler::getReachableCost(succ->state()->getInnerState())
                     << COLOUR_default << endl;
            }
            ...
            if (Globals::optimiseSolutionQuality)
            {
                succ->heuristicValue.admissibleCostEstimate =
                    calculateAdmissibleCost(succ->state()->getInnerState(), succ->heuristicValue.makespan,
                                            currSQI->heuristicValue.admissibleCostEstimate, false);
            ...
            if (!doLoops)
            {
                ...
                if (!FF::bestFirstSearch)
                {
                    ...
                    {
                        if (asItr->first == actID.first && asItr->second == actID.second &&
                            asItr->divisionID == actID.divisionID)
                        {
                            break;
                        }
                        assert(!(asItr->first == actID.first && asItr->second == actID.second &&
                                 asItr->second != Planner::E_AT));
                    }
                    if (asItr == asEnd)
                    {
                        cout << "--\nAction was not helpful when it should have been.  The helpful actions "
                                "were:\n";
                        ...
                        /*              list<ActionSegment> prevHelpful;
                                      ...
                                      HTrio h1(evaluateStateWRTSchedule(rpg, *(currSQI->state), 0, goals,
                           numericGoals, (ParentData*) 0, prevHelpful, currSQI->plan, tEvent, -1));
            ...
            if (actID.second == Planner::E_AT_START)
            {
                ...
                extraEvent = FFEvent(actID.first, succ->state()->startEventQueue.back().minDuration,
                                     succ->state()->startEventQueue.back().maxDuration);
                eventOneDefined = true;
                assert(extraEvent.time_spec == Planner::E_AT_START);
                if (!RPGBuilder::getRPGDEs(actID.first->getID()).empty())
                {  // if it's not a non-temporal action
                    ...
                    extraEventTwo =
                        FFEvent(actID.first, startStepID, succ->state()->startEventQueue.back().minDuration,
                                succ->state()->startEventQueue.back().maxDuration);
                    extraEvent.pairWithStep = endStepID;
                    eventTwoDefined = true;
            ...
            else if (actID.second == Planner::E_AT_END)
            {
                ...
                map<int, list<list<StartEvent>::iterator>>::iterator tsiOld =
                    succ->state()->entriesForAction.find(actID.first->getID());
                ...
                if (tsiOld->second.empty())
                {
                    succ->state()->entriesForAction.erase(tsiOld);
                }
            ...
            pairDummyStepsWithRealSteps(succ->state()->getEditableInnerState(), currStepID, succ->plan,
                                        nowList, newDummySteps);
            ...
            {
                if (!durationManager->updateTimestampsOfNewPlanStep(
                        currSQI->state()->getInnerState(), succ->state()->getEditableInnerState(), succ->plan,
                        reusedEndEvent, 0, stepID, isAStochasticGoalState))
                {
            ...
            else
            {
                ...
                if (!durationManager->updateTimestampsOfNewPlanStep(
                        currSQI->state()->getInnerState(), succ->state()->getEditableInnerState(), succ->plan,
                        local1, local2, stepID, isAStochasticGoalState))
                {
                    h1 = HTrio(-1.0, DBL_MAX, DBL_MAX, INT_MAX, "Exceeded stochastic deadline");
                    stochasticOkay = false;
                }
            ...
            if (doLoops)
            {
                for (int rep = 0; rep < 99; ++rep)
                {
                    ...
                    LPScheduler tryToSchedule(succ->state()->getInnerState(),
                                              succ->state()->getEditableInnerState().preferenceStatus,
                                              succ->plan, nowList, i - 1, succ->state()->startEventQueue,
                                              pd.get(), succ->state()->entriesForAction,
                                              &(currSQI->state()->getInnerState().secondMin),
                                              &(currSQI->state()->getInnerState().secondMax),
                                              &(succ->state()->tilComesBefore), false);

                    vector<double> timeAtWhichValueIsDefined;

                    tryToSchedule.updateStateFluents(succ->state()->getEditableInnerState().secondMin,
                                                     succ->state()->getEditableInnerState().secondMax,
                                                     timeAtWhichValueIsDefined);
                }
                ...
                LPScheduler tryToSchedule(
                    succ->state()->getInnerState(), succ->state()->getEditableInnerState().preferenceStatus,
                    succ->plan, nowList, i - 1, succ->state()->startEventQueue, pd.get(),
                    succ->state()->entriesForAction, &(currSQI->state()->getInnerState().secondMin),
            ...
            else
            {
                ...
                if (stochasticOkay)
                {
                    ...
                    if (FF::allowCompressionSafeScheduler)
                    {
                        h1 = calculateHeuristicAndCompressionSafeSchedule(
                            *(succ->state()), currSQI->state(), goals, numericGoals, succ->helpfulActions,
                            succ->plan, nowList, stepID, 0, 0.001);
                    }
                    else
                    {
                        pair<bool, double> currentCost(false, std::numeric_limits<double>::signaling_NaN());

                        h1 = calculateHeuristicAndSchedule(
                            *(succ->state()), currSQI->state(), goals, numericGoals, pd.get(),
            ...
            if (doLoops)
            {
                double secs = ((double)refReturn.tms_utime + (double)refReturn.tms_stime -
                               (double)before.tms_utime - (double)before.tms_stime) /
            ...
            if (actID.second == Planner::E_AT_START)
            {
                ...
                {  // if it's a compression-safe temporal action
                    ...
                    succ->state()->getEditableInnerState().updateWhenEndOfActionIs(
                        actID.first->getID(), endStepID, extraEventTwo.lpMinTimestamp);
                }
            }
#endif
#endif
            ...
            if (actID.second == Planner::E_AT_START && !RPGBuilder::getRPGDEs(actID.first->getID()).empty() &&
                TemporalAnalysis::canSkipToEnd(actID.first->getID()))
            {
...

            {
                ...
                else
                {
                    ...
                    if (Globals::optimiseSolutionQuality &&
                        NumericAnalysis::theMetricIsMonotonicallyWorsening())
                    {
                ...
                NumericAnalysis::theMetricIsMonotonicallyWorsening())
            {
                ...
                {
                    cout << COLOUR_light_green << succ->heuristicValue.admissibleCostEstimate
                         << " does not exceed incumbent solution quality of " << -Globals::bestSolutionQuality
                         << COLOUR_default << endl;
                }
            }
#endif
            ...
            if (rawDebug)
            {
                cout << "Now done up to " << currSQI->plan.size() << " steps\n";
            }
        }
            ...
            for (int i = 0; printItr != printEnd; ++printItr, ++i)
            {
                ...
                if (printItr->time_spec == Planner::E_AT_START)
                {
                    ...
                    if (!RPGBuilder::getRPGDEs(printItr->action->getID()).empty())
                    {
                        ...
                        cout << "\tnode" << i << " -> node" << printItr->pairWithStep
                             << " [label=\"maxduration=" << printItr->maxDuration << "\"];\n";
                        cout << "\tnode" << printItr->pairWithStep << " -> node" << i
                             << " [label=\"-minduration=" << -printItr->minDuration << "\"];\n";
                    }
                    else
                    {
                        cout << "\"]\n";
                ...
                const map<int, bool> *const stepsThatComeBeforeThisOne =
                    currSQI->state()->getInnerState().temporalConstraints->stepsBefore(i);
    ...
    void registerFinished(ExtendedMinimalState &theState, set<int> &haveFinished)
    {
        ...
        for (; hfItr != hfEnd; ++hfItr) {
            ...
            const map<int, pair<set<int>, set<int> > >::iterator fwfEnd =
        theState.factsIfWeFinishActions.end();
    ...
    Solution FF::search(bool &reachedGoal)
    {
...

        {
            ...
            //             initialState.getEditableInnerState().statusOfTemporalPreferences =
            //             PreferenceStatusArray::getInitialArray();
            //         }
            ...
            if (ffDebug)
            {
                cout << "Initial state has " << initialState.getInnerState().first.size()
                     << " propositional facts and " << tinitialFluents.size() << " non-static fluents\n";
            }
        }
        {
            initialState.getEditableInnerState().preferenceStatus =
...

        {
            ...
            if (Globals::optimiseSolutionQuality)
            {
                initialSQI->heuristicValue.admissibleCostEstimate = calculateAdmissibleCost(
                    initialSQI->state()->getInnerState(), initialSQI->heuristicValue.makespan,
                    (RPGBuilder::getMetric() && !RPGBuilder::getMetric()->minimise ? DBL_MAX : -DBL_MAX),
                    false);
            ...
            TemporalAnalysis::pullInAbstractTILs(initialSQI->state()->getEditableInnerState(),
                                                 stepsForInitialAbstractTILs);
            ...
            if (FF::allowCompressionSafeScheduler)
            {
                bestHeuristic = calculateHeuristicAndCompressionSafeSchedule(
                    initialState, 0, goals, numericGoals, initialSQI->helpfulActions, initialSQI->plan,
                    tEvent, -1);
            }
            else
            {
                bestHeuristic = calculateHeuristicAndSchedule(initialState, 0, goals, numericGoals,
                                                              (ParentData *)0, initialSQI->helpfulActions,
                                                              bestCurrentCost, initialSQI->plan, tEvent, -1);
            }
            ...
            if (RPGBuilder::getMetric())
            {
                if (Globals::optimiseSolutionQuality)
                {
                    if (RPGBuilder::getMetric()->minimise)
                    {
                        if (bestHeuristic.admissibleCostEstimate <
                            initialSQI->heuristicValue.admissibleCostEstimate)
                        {
                            bestHeuristic.admissibleCostEstimate =
                                initialSQI->heuristicValue.admissibleCostEstimate;
                        }
                    }
                    else
            ...
            if (bestHeuristic.goalsSatisfied)
            {
                reachedGoal = true;
                bool ignore;
                const pair<bool, bool> prognosis(
                    carryOnSearching(initialSQI->state()->getInnerState(), initialSQI->plan, bestCurrentCost,
                                     bestHeuristic.admissibleCostEstimate, ignore));
                if (!prognosis.first)
                {
        ...
        if (ffDebug || true)
        {
            cout << "Initial heuristic = " << bestHeuristic.heuristicValue << ", admissible cost estimate "
                 << bestHeuristic.admissibleCostEstimate << "\n";
        }
        ...
        auto_ptr<list<FFEvent>> bestPlan(new list<FFEvent>());
        {
...
#ifdef DOUBLESTATEHASH
            {
                ...
                tList.push_back(
                    pair<pair<HTrio, bool>, double>(pair<HTrio, bool>(bestHeuristic, false), 0.0));

                oldVisitedStates.insert(
                    pair<ExtendedMinimalState, list<pair<pair<HTrio, bool>, double>>>(*toHash, tList));
                oldZealousVisitedStates.insert(
                    pair<ExtendedMinimalState, list<pair<pair<HTrio, bool>, double>>>(*toHash, tList));
            }
#endif
        }
        ...
        while (!searchQueue.empty())
        {
            ...
            if (!foundBetter)
            {
                if (helpfulActions)
                {
                    ...
                    RPGBuilder::getHeuristic()->filterApplicableActions(currSQI->state()->getInnerState(),
                                                                        currSQI->state()->timeStamp,
                                                                        currSQI->helpfulActions);
                ...
                {
                    RPGBuilder::getHeuristic()->findApplicableActions(currSQI->state()->getInnerState(),
                                                                      currSQI->state()->timeStamp,
                                                                      maybeApplicableActions);
            ...
            const auto_ptr<ParentData> incrementalData(
                FF::allowCompressionSafeScheduler ?
                    0 :
                    LPScheduler::prime(currSQI->plan, currSQI->state()->getInnerState().temporalConstraints,
                                       currSQI->state()->startEventQueue, Globals::optimiseSolutionQuality));
            ...
            for (; helpfulActsItr != helpfulActsEnd; ++helpfulActsItr)
            {
                ...
                if (helpfulActsItr->second == Planner::E_AT)
                {
                    ...
                    succ = auto_ptr<SearchQueueItem>(new SearchQueueItem(
                        applyActionToState(tempSeg, *(currSQI->state()), currSQI->plan, newDummySteps),
                        true));
                    ...
                    if (tsSound)
                    {
                        ...
                        if (Globals::optimiseSolutionQuality)
                        {
                            succ->heuristicValue.admissibleCostEstimate = calculateAdmissibleCost(
                                succ->state()->getInnerState(), succ->heuristicValue.makespan,
                                currSQI->heuristicValue.admissibleCostEstimate, false);
                            if (admissibleCostExceedsBound(succ->heuristicValue.admissibleCostEstimate,
                                                           false))
                            {
                ...
                else
                {
                    ...
                    succ = auto_ptr<SearchQueueItem>(
                        new SearchQueueItem(applyActionToState(*helpfulActsItr, *(currSQI->state()),
                                                               currSQI->plan, newDummySteps),
                                            true));
                    if (succ->state())
                    {
                        tsSound = stateHasProgressedBeyondItsParent(*helpfulActsItr, *(currSQI->state()),
                                                                    *(succ->state()))  // it had some
                                                                                       // beneficial effects
                                  &&
                                  checkTemporalSoundness(currSQI->state(), *(succ->state()), *helpfulActsItr,
                                                         oldTIL);  // it didn't introduce a trivial cycle
                    }
                    else
                    {
                        tsSound = false;
                ...
                else
                {
                    ...
                    if (helpfulActsItr->second == Planner::E_AT)
                    {
                        ...
                        if (zealousEHC)
                        {
                            const auto_ptr<StateHash::FindIterator> lookup(
                                visitedStates->findState(succ->state()));
                        ...
                        if (visitTheState)
                        {
                            ...
                            {
                                set<int> needToFinish;  // = RPGBuilder::TILneedsToHaveFinished(tn,
                                                        // succ->state);
                                // registerFinished(toSolve->rpg, succ->state, needToFinish);
                                ...
                                pair<bool, double> currentCost(false,
                                                               std::numeric_limits<double>::signaling_NaN());

                                evaluateStateAndUpdatePlan(
                                    succ, *(succ->state()), TILparent->state(), goals, numericGoals,
                                    (incrementalIsDead ? (ParentData *)0 : incrementalData.get()),
                                    succ->helpfulActions, currentCost, tempSeg, TILparent->plan,
                                    newDummySteps);
                                ...
                                succ = auto_ptr<SearchQueueItem>(
                                    new SearchQueueItem(applyActionToState(tempSeg, *(TILparent->state()),
                                                                           TILparent->plan, newDummySteps),
                                                        true));

                                succ->heuristicValue.makespan = TILparent->heuristicValue.makespan;

                                if (!succ->state() ||
                                    !checkTemporalSoundness(TILparent->state(), *(succ->state()), tempSeg,
                                                            tn - 1))
                                {
...
#ifdef DOUBLESTATEHASH
                    map<ExtendedMinimalState, list<pair<pair<HTrio, bool>, double>>,
                        OldCompareStatesZealously>::iterator zvsItr;
                    map<ExtendedMinimalState, list<pair<pair<HTrio, bool>, double>>,
                        OldCompareStates>::iterator vsItr;
#endif
                    ...
                    if (!TILfailure)
                    {
                        ...
                        insResult = auto_ptr<StateHash::InsertIterator>(
                            visitedStates->insertState(succ.get(), statesKept.get()));
                        ...
                        if (zealousEHC)
                        {
                            zvsItr =
                                oldZealousVisitedStates
                                    .insert(pair<ExtendedMinimalState, list<pair<pair<HTrio, bool>, double>>>(
                                        *hunting, list<pair<pair<HTrio, bool>, double>>()))
                                    .first;
                            if (zvsItr->second.empty())
                            {
                                if (!visitTheState)
                                {
                                    ...
                                    {
                                        cout << "State is less than that in old hash, according to new "
                                                "comparator\n";
                                    }
                    ...
                    if (visitTheState)
                    {
                        ...
                        if (helpfulActsItr->second == Planner::E_AT)
                        {
                            evaluateStateAndUpdatePlan(
                                succ, *(succ->state()), TILparent->state(), goals, numericGoals,
                                (incrementalIsDead ? (ParentData *)0 : incrementalData.get()),
                                succ->helpfulActions, currentCost, *helpfulActsItr, TILparent->plan,
                                newDummySteps);
                        }
                        else
                        {
                            evaluateStateAndUpdatePlan(succ, *(succ->state()), currSQI->state(), goals,
                                                       numericGoals, incrementalData.get(),
                                                       succ->helpfulActions, currentCost, *helpfulActsItr,
                                                       currSQI->plan, newDummySteps);
                        }
                        ...
                        if (succ->heuristicValue.heuristicValue != -1.0)
                        {
                            ...
                            if (succ->heuristicValue.goalsSatisfied)
                            {
                                ...
                                const pair<bool, bool> prognosis(carryOnSearching(
                                    succ->state()->getInnerState(), succ->plan, currentCost,
                                    succ->heuristicValue.admissibleCostEstimate, forceRestart));
                                if (!prognosis.first)
                                {
                                    return workingBestSolution;
                                }
                            ...
                            if (zealousEHC)
                            {
                                zvsItr->second.push_back(pair<pair<HTrio, bool>, double>(
                                    pair<HTrio, bool>(succ->heuristicValue, false),
                                    succ->state()->timeStamp));
                            }
                            else
                            {
                                vsItr->second.push_back(pair<pair<HTrio, bool>, double>(
                                    pair<HTrio, bool>(succ->heuristicValue, false),
                                    succ->state()->timeStamp));
                            }
#endif
                            if (forceKeep ||
                                ((succ->heuristicValue.heuristicValue < bestHeuristic.heuristicValue) ||
                                 (FF::makespanTieBreak &&
                                  (succ->heuristicValue.heuristicValue == bestHeuristic.heuristicValue) &&
                                  (succ->heuristicValue.makespan < bestHeuristic.makespan))))
                            {
                                bestHeuristic = succ->heuristicValue;
                                cout << "b (" << bestHeuristic.heuristicValue << " | "
                                     << bestHeuristic.makespan << ")";
                                cout.flush();
                                // succ->printPlan();
                                searchQueue.clear();
        ...
        for (; aStarWeightItr != aStarWeightEnd; ++aStarWeightItr)
        {
...

            {
...

                {
                    ...
                    list<pair<pair<HTrio, bool>, double>> tList;
                    tList.push_back(
                        pair<pair<HTrio, bool>, double>(pair<HTrio, bool>(bestHeuristic, true), 0.0));
            ...
            while (!triggerRestart && !searchQueue.empty())
            {
                ...
                {
                    cout << "\n--\n";
                    cout << "Now visiting state with heuristic value of "
                         << currSQI->heuristicValue.heuristicValue << " | "
                         << currSQI->heuristicValue.makespan << "\n";
...
#ifdef POPF3ANALYSIS
                if (Globals::optimiseSolutionQuality &&
                    admissibleCostExceedsBound(currSQI->heuristicValue.admissibleCostEstimate, false))
                {
                    if (Globals::globalVerbosity & 2)
                    {
                        cout << "[" << -Globals::bestSolutionQuality << "<"
                             << currSQI->heuristicValue.admissibleCostEstimate << "]";
                        cout.flush();
                    }
                ...
                {
                    RPGBuilder::getHeuristic()->findApplicableActions(
                        currSQI->state()->getInnerState(), currSQI->state()->timeStamp, applicableActions);
                }
                ...
                const auto_ptr<ParentData> incrementalData(
                    FF::allowCompressionSafeScheduler ?
                        0 :
                        LPScheduler::prime(
                            currSQI->plan, currSQI->state()->getInnerState().temporalConstraints,
                            currSQI->state()->startEventQueue, Globals::optimiseSolutionQuality));
                ...
                for (; !triggerRestart && helpfulActsItr != helpfulActsEnd; ++helpfulActsItr)
                {
                    ...
                    if (helpfulActsItr->second == Planner::E_AT)
                    {
                        ...
                        {
                            tsSound =
                                checkTemporalSoundness(currSQI->state(), *(succ->state()), tempSeg, oldTIL);
                        }
                    ...
                    else
                    {
                        ...
                        {
                            tsSound =
                                (stateHasProgressedBeyondItsParent(*helpfulActsItr, *(currSQI->state()),
                                                                   *(succ->state())) &&
                                 checkTemporalSoundness(currSQI->state(), *(succ->state()), *helpfulActsItr));
                        }
                    ...
                    else
                    {
                        ...
                        if (helpfulActsItr->second == Planner::E_AT)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    const double &previousTS = lookup->previousTimestamp();
                                    if (fabs(previousTS - succ->state()->timeStamp) > 0.0005 &&
                                        (previousTS > succ->state()->timeStamp))
                                    {
                                        visitTheState = 2;
                                    }
                                }
                            ...
                            if (visitTheState)
                            {
                                ...
                                for (int tn = oldTIL + 1; tn <= helpfulActsItr->divisionID; ++tn)
                                {
                                    ActionSegment tempSeg(0, Planner::E_AT, tn - 1,
                                                          RPGHeuristic::emptyIntList);
                                    FFcache_upToDate = false;

                                    pair<bool, double> currentCost(
                                        false, std::numeric_limits<double>::signaling_NaN());
                                    ...
                                    newDummySteps.clear();
                                    succ = auto_ptr<SearchQueueItem>(new SearchQueueItem(
                                        applyActionToState(tempSeg, *(TILparent->state()), TILparent->plan,
                                                           newDummySteps),
                                        true));
                        ...
                        if (!TILfailure)
                        {
                            ...
                            int doubleVisitTheState = 0;
                            {
                                zvsItr =
                                    oldZealousVisitedStates
                                        .insert(
                                            pair<ExtendedMinimalState, list<pair<pair<HTrio, bool>, double>>>(
                                                *(succ->state()), list<pair<pair<HTrio, bool>, double>>()))
                                        .first;
                                vsItr =
                                    oldVisitedStates
                                        .insert(
                                ...
                                if (vsItr->second.empty() ||
                                    (fabs(vsItr->second.back().second - succ->state()->timeStamp) > 0.0005 &&
                                     (vsItr->second.back().second > succ->state()->timeStamp)))
                                {
                        ...
                        if (visitTheState)
                        {
                            ...
                            if (succ->heuristicValue.heuristicValue != -1.0)
                            {
                                ...
                                if (succ->heuristicValue.goalsSatisfied)
                                {
                                    ...
                                    // return make_pair(new list<FFEvent>(succ->plan), new
                                    // TemporalConstraints(*(succ->state()->getInnerState().temporalConstraints)));
                                }
                                insResult->setTimestampOfThisState(succ->state());

#ifdef DOUBLESTATEHASH
                                vsItr->second.push_back(pair<pair<HTrio, bool>, double>(
                                    pair<HTrio, bool>(succ->heuristicValue, true), succ->state()->timeStamp));
                                zvsItr->second.push_back(pair<pair<HTrio, bool>, double>(
                                    pair<HTrio, bool>(succ->heuristicValue, true), succ->state()->timeStamp));
#endif
                                if (keepState)
                                {
                                    if (succ->heuristicValue.heuristicValue < bestHeuristic.heuristicValue ||
                                        (FF::makespanTieBreak &&
                                         (succ->heuristicValue.heuristicValue ==
                                          ...
                                          succ->heuristicValue.makespan < bestHeuristic.makespan)))
                                    {
                                        ...
                                        {
                                            cout << "\t" << bestHeuristic.heuristicValue << " | "
                                                 << bestHeuristic.makespan << ", category " << visitTheState
                                                 << " - a new best heuristic value, with plan:\n";
                                            // succ->printPlan();
                                        }
                                        else
                                        {
                                    ...
                                    else
                                    {
                                        if (Globals::globalVerbosity & 2)
                                        {
                                            cout << "\t" << succ->heuristicValue.heuristicValue << " | "
                                                 << succ->heuristicValue.makespan << ", category "
                                                 << visitTheState << "\n";
                                        }
                                        else if (Globals::globalVerbosity & 1)
                                        {
    ...
    list<FFEvent> *FF::reprocessPlan(list<FFEvent> *oldSoln, TemporalConstraints *cons)
    {
        ...
        if (cons == 0)
        {
            ...
            {
                if ((osItr->time_spec == Planner::E_AT_END) &&
                    TemporalAnalysis::canSkipToEnd(osItr->action->getID()))
                {
                    continue;
                }
        ...
        else
        {
            ...
            for (; !openList.empty(); openList.pop_front())
            {
                ...
                if ((eventForStep[currStep]->time_spec != Planner::E_AT_END) ||
                    !TemporalAnalysis::canSkipToEnd(eventForStep[currStep]->action->getID()))
                {
...

        {
...
#ifdef ENABLE_DEBUGGING_HOOKS
            {
                ...
                for (; remainingSolnItr != oldSolnEnd; ++remainingSolnItr)
                {
                    ...
                    {
                        Globals::remainingActionsInPlan.push_back(ActionSegment(
                            (instantiatedOp *)0, Planner::E_AT, (*remainingSolnItr)->divisionID, set<int>()));
                    }
                    else
                    {
                        Globals::remainingActionsInPlan.push_back(
                            ActionSegment((*remainingSolnItr)->action, (*remainingSolnItr)->time_spec,
                                          (*remainingSolnItr)->divisionID, set<int>()));
                    }
                }
            }
#endif
            if (FF::allowCompressionSafeScheduler)
            {
                calculateHeuristicAndCompressionSafeSchedule(
                    initialState, 0, goals, numericGoals, currSQI->helpfulActions, currSQI->plan, tEvent, -1);
            }
            else
            {
                pair<bool, double> currentCost(false, std::numeric_limits<double>::signaling_NaN());
                calculateHeuristicAndSchedule(initialState, 0, goals, numericGoals, (ParentData *)0,
                                              currSQI->helpfulActions, currentCost, currSQI->plan, tEvent,
                                              -1);
            }
        }
        ...
        for (int stepID = 0; oldSolnItr != oldSolnEnd; ++oldSolnItr, ++stepID)
        {
            ...
            ActionSegment nextSeg((*oldSolnItr)->action, (*oldSolnItr)->time_spec, (*oldSolnItr)->divisionID,
                                  RPGHeuristic::emptyIntList);
            ...
            auto_ptr<SearchQueueItem> succ = auto_ptr<SearchQueueItem>(new SearchQueueItem(
                applyActionToState(nextSeg, *(currSQI->state()), currSQI->plan, newDummySteps), true));
            succ->heuristicValue.makespan = currSQI->heuristicValue.makespan;

            pair<bool, double> currentCost(false, std::numeric_limits<double>::signaling_NaN());

            evaluateStateAndUpdatePlan(succ, *(succ->state()), currSQI->state(), goals, numericGoals,
                                       incrementalData.get(), succ->helpfulActions, currentCost, nextSeg,
                                       currSQI->plan, newDummySteps);
