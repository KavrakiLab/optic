...
namespace Planner
{
    ...
    struct ActionFluentModification
    {
        ...
        ActionFluentModification(const int &a, const Planner::time_spec &startOrEnd, const bool &oe,
                                 const double &incr, const int &shots, const bool &ass)
    ...
    public:
        ...
        enum pointless_effect
        {
            PE_ADDED = 1,
            PE_DELETED = 2,
            PE_DELETED_THEN_ADDED = 3
        };
        ...
        struct Operand
        {
            ...
            PNE *durationVar;
            Operand(PNE *p)
              : numericOp(NE_STOCHASTIC_DURATION_TERM), fluentValue(-1), constantValue(NAN), durationVar(p){};
#endif
            ...
            Operand(const string &s)
              : numericOp(NE_VIOLATION), fluentValue(-1), constantValue(NAN), isviolated(s){};
        };
        ...
        public:
            ...
            Constraint()
              ...
              , cost(0.0)
              , neverTrue(false)
              , attachedToOperator(make_pair(-1, Planner::E_AT))
              , deadline(0.0)
              , from(0.0){};
            Constraint(const string &n)
              : name(n)
              , parsed_goal(0)
              , parsed_trigger(0)
              , d(0)
        ...
        public:
            ...
            NumericEffect(const VAL::assign_op &opIn, const int &fIn, VAL::expression *formulaIn,
                          VAL::FastEnvironment *f, VAL::TypeChecker *t = 0);
        ...
        public:
            ...
            NumericPrecondition(const VAL::comparison_op &opIn, VAL::expression *LHSformulaIn,
                                VAL::expression *RHSformulaIn, VAL::FastEnvironment *f,
                                VAL::TypeChecker *t = 0, const bool polarity = true);
        ...
        public:
            ...
            ArtificialVariable()
              : ID(-1), size(0), constant(0.0), maxNeed(-DBL_MAX), desirable(false), undesirable(false){};
            ArtificialVariable(const int &id, const int &s, const vector<double> &w, const vector<int> &f,
                               const double &d, const double &maxIn, const bool &des, const bool &undes)
              : ID(id)
              , size(s)
              , weights(w)
              , fluents(f)
            ...
            double evaluateWCalculate(const vector<double> &minFluentTable,
                                      const vector<double> &maxFluentTable, const int &pneCount) const;
        ...
        public:
            ...
            RPGNumericPrecondition(const int &i, const bool &d, const bool &ud, const int &lv,
                                   const double &lw, const VAL::comparison_op &o, const double &rw)
              : ID(i)
              , desirable(d)
              , undesirable(ud)
              , LHSVariable(lv)
            ...
            bool isSatisfied(const vector<double> &maxFluents) const
            {
                ...
                if (lv == std::numeric_limits<double>::signaling_NaN())
                {
                    return false;
                }
            ...
            bool isSatisfiedWCalculate(const vector<double> &minFluents,
                                       const vector<double> &maxFluents) const;
            bool canBeUnsatisfiedWCalculate(const vector<double> &minFluents,
                                            const vector<double> &maxFluents) const;
        ...
        public:
            ...
            RPGNumericEffect()
              : ID(-1), fluentIndex(-1), isAssignment(false), constant(NAN), size(-1), canEverHappen(true){};
            ...
            double evaluate(const vector<double> &maxFluents, const double &minDur,
                ...
                for (int i = 0; i < size; ++i)
                {
                    ...
                    else if (variables[i] == -19)
                    {
                        ...
                        if (lw < 0)
                        {
                            ...
                            {
                                std::cerr << "Warning: negative weight attached to negative duration term - "
                                             "is this intentional?\n";
                                printedMessage = true;
                            }
                        }
                    ...
                    {
                        std::cerr << "Error: asked to evaluate formula containing term with fluent ID "
                                  << variables[i]
                                  << ", which doesn't represent either a non-static fluent or the duration "
                                     "term -3 or -19\n";
                        assert(false);
                    }
                }
            ...
            double evaluateMin(const vector<double> &maxFluents, const double &minDur,
                               const double &maxDur) const
            {
            ...
            pair<double, double> applyEffectMinMax(const vector<double> &minFluents,
                                                   const vector<double> &maxFluents, const double &minDur,
                                                   const double &maxDur);
        ...
        public:
            ...
            /** The minimum durations of the action, those of the form <code>(>= ?duration ...)</code> or
             * <code>(> ?duration ...)</code> */
            list<DurationExpr *> min;

            /** The maximum durations of the action, those of the form <code>(<= ?duration ...)</code> or
             * <code>(< ?duration ...)</code> */
            ...
            const list<DurationExpr *> &operator[](const int &i) const
            {
                ...
                std::cerr << "Fatal internal error: duration expression index should be in the range 0 to 2, "
                             "inclusive\n";
                exit(1);
            }
        };
        ...
        public:
             ...
             */
            FakeTILAction(const double &dur, const LiteralSet &adds, const LiteralSet &dels) : duration(dur)
            {
                mergeIn(adds, dels);
            }
        };
        ...
        struct ShotCalculator
        {
            ...
            ShotCalculator(const int &v, const double &g, const double &d)
              : variable(v), greaterThan(g), decreaseBy(d){};
        };
        ...
        public:
            ...
            /** The IDs of the variables upon which this action has continuous effects.  @see
             * RPGBuilder::getPNE */
            vector<int> vars;
         ...
         *
         * This is used in preprocessing to eliminate duplicate constraints.  For each set of identical
         * constraints, one is kept, with its name set to the concatenation of all the constraints it
         * represents, separated by spaces. The <code>outputNameMap</code> maps all the exemplars to this, so
         * it subsequently accrues their sum cost.
         *
         * @param input  The list of constraints
         * @param outputNameMap  Return-by-reference: a mapping from constraint names to the IDs in the output
         * vector
         * @param output         Return-by-reference: list of constraints without duplicates.
         */
        static void eliminateDuplicateConstraints(
        ...
        private:
             ...
             * hold (either <code>VAL::E_AT_START</code>, <code>VAL::E_OVER_ALL</code> or
             * <code>VAL::E_AT_END</code>. For now, the literal must be controlled exclusively by timed
             * initial literals.
             */
             ...
             * entry denotes an entry in <code>RPGBuilder::rpgNumericPreconditions</code>; the second denotes
             * the time at which it must hold (either <code>VAL::E_AT_START</code>,
             * <code>VAL::E_OVER_ALL</code> or <code>VAL::E_AT_END</code>.
             */
             ...
             * Each entry is pair: the first is an index in <code>RPGBuilder::rpgNumericEffects</code>; the
             * second denotes the time at which it occurs (either <code>VAL::E_AT_START</code> or
             * <code>VAL::E_AT_END</code>).
             */
        ...
        public:
            ...
            void addCondition(Literal *const l, const Planner::time_spec &t,
                              const int &
#ifndef NDEBUG
                                  actID
#endif
            ...
            void addCondition(const int &p, const Planner::time_spec &t,
                              const int &
#ifndef NDEBUG
                                  actID
#endif
            ...
            void addNumericEffect(const int &p, const Planner::time_spec &t,
                                  const int &
#ifndef NDEBUG
                                      actID
#endif
        ...
        public:
            IntegralContinuousEffect(const int &act, const int &oc, const int &lp,
                                     const Planner::time_spec &ts, const double &lpd, const int &rp,
                                     const double &rpd)
              : parentAction(act)
              , minimalOutcome(0)
              , outcomes(oc)
              , leftPrecondition(lp)
              , leftPreconditionTS(ts)
              , leftPreconditionDifference(lpd)
              , rightPrecondition(rp)
              , rightPreconditionDifference(rpd)
        ...
        public:
            NoDuplicatePair() : first((list<Literal *> *)0), second((LiteralSet *)0)
            {
            }
    ...
    protected:
        ...
        struct BuildingNumericPreconditionData
        {
            ...
            BuildingNumericPreconditionData()
              : negOffset(pnes.size())
              , offset(negOffset * 2)
              , precCount(0)
              , avCount(offset)
              , localMaxNeed(offset, -DBL_MAX){};
        };
        ...
        // static void makeOneSided(pair<list<double>, list<int> > & LHSvariable, pair<list<double>, list<int>
        // > & RHSvariable, const int & negOffset); static void makeWeightedSum(list<Operand> & formula,
        // pair<list<double>, list<int> > & result);
        static bool processPrecondition(RPGBuilder::BuildingNumericPreconditionData &commonData,
                                        NumericPrecondition &currPre, list<int> &conjunctiveList,
        ...
        static bool processPreconditions(RPGBuilder::BuildingNumericPreconditionData &commonData,
                                         ...
                                         int &toIncrement, const int &i,
                                         const Planner::time_spec &passTimeSpec);

        static void buildDurations(vector<list<NumericPrecondition *>> &d,
                                   vector<list<NumericPrecondition *>> &e,
                                   vector<list<NumericPrecondition *>> &f);

        /** @brief Infer additional start preconditions, due to numeric invariants.
         *
         *  Each numeric invariants (<code>over all</code> conditions) leads to a numeric start precondition
         * in one of two ways:
         *  - if it refers to variables that are not affected by the actions start effects or continuous
         * numeric effects, then it is copied as-is to be a start numeric precondition
         *  - otherwise, the invariant is regressed through the actions' start effects (and an epsilon amount
         * of its continuous numeric) effects, and this new constraint added as a start precondition.
         *
         ...
         *  @param  commondata  Comomon data used when building up extra preconditions (shared across many
         * actions)
         *  @param  preResult   After the function, this holds the new numeric precondition formed.  It is a
         * pair, where:
         *                      - the first entry is a pointer to the precondition itself
         *                      - the second entry is true if the precondition was new (i.e. never seen before
         * on any action)
         *  @param  avResult    After the function, this holds the artificial variable that was created for
         * the new precondition.  It is a pair, where:
         *                      - the first entry is a pointer to the artificial variable itself (or
         * <code>0</code> if one was needed, i.e. the invariant only referred to a single variable)
         *                      - the second entry is true if the artificial variable was new (i.e. never seen
         * before in any other precondition)
         */
        static bool pushInvariantBackThroughStartEffects(
            const RPGNumericPrecondition &pre, list<int> &startEffs, LinearEffects *ctsEffs,
            InvData &commonData, pair<const RPGNumericPrecondition *, bool> &preResult,
            pair<ArtificialVariable *, bool> &avResult);

        static void handleNumericInvariants();
        static void findSelfMutexes();
        static void oneShotInferForTILs();
        static void kshotInferForAction(const int &i, MinimalState &refState, LiteralSet &maybeOneShotLiteral,
                                        vector<double> &initialFluents, const int &fluentCount);
         ...
         *  - (at start (p)) is coupled with an effect (at start (not (p))) and one of either (at end (p)) or
         * (at start (p))
         *  - (at end (p)) is coupled with an effect (at end (not (p))) and another (at end (p)).
         *  - there is no condition (over all (p))
         *
         *  They can also be goals, the rationale being that 'goals being satisfied' is an infinitely long
         * action at the end of a plan, requiring the semaphore before it can be applied, i.e. requiring no
         * actions to be executing (as follows from the PDDL semantics).
         *
         *  To make semaphore facts useful, the fluents they 'guard' are noted by this.  A semaphore fact
         * guards a fluent if, to refer to the value of that fluent, one must also have the requisite
         * semaphore fact.
         */
    ...
    public:
        ...
        static void simplify(pair<list<double>, list<int>> &s);
        static void makeOneSided(pair<list<double>, list<int>> &LHSvariable,
                                 pair<list<double>, list<int>> &RHSvariable, const int &negOffset);

#ifdef STOCHASTICDURATIONS
        static void simplify(pair<list<double>, list<pair<int, PNE *>>> &s);
        static void makeDurationWeightedSum(list<Operand> &formula,
        ...
        /** @return A reference to <code>rpgNumericPreconditions</code>, the preconditions used in the
         * problem, in LNF. */
        ...
        /** @return a reference to <code>rpgNumericEffectsToActions</code>: for each numeric effect, which
         * actions have it. */
        ...
        /** @return a reference to <code>rpgNumericPreconditionsToActions</code>: for each numeric
         * precondition, which actions have it. */
        ...
        /** @return a reference to <code>processedRPGNumericPreconditionsToActions</code>: for each numeric
         * precondition, after pushing invariants through start effects, which actions have it. */
        static const vector<list<pair<int, Planner::time_spec>>> &
        getProcessedRPGNumericPreconditionsToActions()
        {
        ...
        {
            return (!fixedDurationExpressions[i].empty() || !minDurationExpressions[i].empty() ||
                    !maxDurationExpressions[i].empty());
        }
        ...
        static void getEffects(instantiatedOp *op, const bool &start, list<Literal *> &add,
                               list<Literal *> &del, list<NumericEffect> &numeric);
        static void getPrecInv(instantiatedOp *op, const bool &start, list<Literal *> &precs,
                               list<Literal *> &inv, list<NumericPrecondition> &numericPrec,
                               list<NumericPrecondition> &numericInv);

        // static void getCollapsedAction(instantiatedOp* op, list<Literal*> & pre, list<Literal*> & add,
        ...
        static pair<double, double> getOpDuration(instantiatedOp *op, const int &div,
                                                  const vector<double> &minFluents,
                                                  const vector<double> &maxFluents);
        static pair<double, double> getOpDuration(const int &op, const int &div,
                                                  const vector<double> &minFluents,
                                                  const vector<double> &maxFluents);
         ...
         *
         * @see <code>RPGBuilder::findUninterestingnessCriteria</code> and
         * <code>RPGBuilder::findConcurrentRedundantActions</code>
         *
    ...
    extern EpsilonResolutionTimestamp earliestPointForNumericPrecondition(
        const RPGBuilder::RPGNumericPrecondition &p, const vector<double> *earliestNumericPOTimes);

    extern EpsilonResolutionTimestamp earliestPointForNumericPrecondition(
        const RPGBuilder::RPGNumericPrecondition &p, const MinimalState &s, const vector<double> &stepTimes);
    ...
    private:
        ...
        struct ActionViolationData
        {
            ...
            ActionViolationData()
              : precViolationCost(-1.0)
              , effViolationCost(-1.0)
              , overallViolationCost(-1.0)
              , canBeApplied(false)
            {
            }
    ...
    public:
        ...
        /** @brief If set to true, the RPG integrates continuous effects, so they occur in full at the start
         * of the action. */
        ...
        /** @brief If set to true, helpful actions are ordered according to the deadline to which they are
         * relevant, soonest first. */
        static bool orderByDeadlineRelevance;

        /** @brief If set to true, the TRPG is always expanded fully, thereby guaranteeing cost estimates are
         * admissible. */
        static bool alwaysExpandFully;

        static bool addTheMaxCosts;

        RPGHeuristic(const bool &b, vector<list<Literal *>> *atse, vector<list<Literal *>> *atee,
                     ...
                     vector<list<int>> *atnuep, vector<list<int>> *atpnuep, vector<int> *iusp,
        ...
        public:
            ...
            EvaluationInfo() : hValue(-1), admissibleReachablePrefCost(DBL_MAX), goalState(false)
            {
            ...
            {
                // cout << "Created evaluation info at " << this << "< by copying " << &e << ", with h = " <<
                // hValue << std::endl;
            }
            ...
            EvaluationInfo &operator=(const EvaluationInfo &e)
            {
                ...
                admissibleReachablePrefCost = e.admissibleReachablePrefCost;
                goalState = e.goalState;
                // cout << "Updated evaluation info at " << this << "by assignment, with h = " << hValue <<
                // std::endl;
                return *this;
            }
        ...
        EvaluationInfo *getRelaxedPlan(MinimalState &theState, const list<StartEvent> *startEventQueue,
                                       ...
                                       const double &currentCost, const vector<double> &extrapolatedMin,
                                       const vector<double> &extrapolatedMax,
                                       const vector<double> &timeAtWhichStateVariableBoundsHold,
                                       list<ActionSegment> &helpfulActions,
                                       list<pair<double, list<ActionSegment>>> &relaxedPlan,
                                       double &finalPlanMakespanEstimate,
                                       map<double, list<pair<int, int>>> *justApplied = 0,
                                       double tilFrom = 0.001) __attribute__((warn_unused_result));
        ...
        bool testApplicability(const MinimalState &theState, const double &stateTime,
                               const ActionSegment &actID, bool fail = false, bool ignoreDeletes = false);
    ...
    enum whereAreWe
    {
        ...
        PARSE_GOAL,
        PARSE_INITIAL,
        PARSE_CONDITIONALEFFECT,
        PARSE_CONTINUOUSEFFECT,
        PARSE_METRIC,
