...
namespace Planner
{
    ...
    public:
        ...
        struct AddedOrDeleted
        {
            ...
            AddedOrDeleted() : added(false), deleted(false)
            {
            }

            AddedOrDeleted(const bool &b) : added(b), deleted(!b)
            {
            }
        };
    ...
    public:
        TILTimeline()
          : onlyEverAddedByTILs(true)
          , onlyEverDeletedByTILs(true)
          , allReferencesToItDeleteIt(false)
          , closedEnded(false)
          , numberOfWindows(-1)
        {
        }
        ...
        const_iterator firstAdderSuitableForTime(const double &t) const
        {
            const_iterator toReturn =
                happenings.upper_bound(t);  // find the next point on the timeline after t
            if (toReturn != happenings.begin())
            {  // but actually we want the point before then, so roll backwards
                --toReturn;
            }
            for (; toReturn != happenings.end() && !toReturn->second.added; ++toReturn)
                ;  // roll forwards until we're after an adder
            return toReturn;
        }
    ...
    private:
        ...
        static void
        markCompressionSafetyConditions(const int &actID, const list<int> &effects,
                                        vector<set<int>> &actionsDependingOnVarBeingCompressionSafe);
        static void markAffectedVariablesAsNotCompressionSafe(
    ...
    public:
         ...
         * @param negativeFactRelevantToWithinPreferences[out] Negative facts mapped to relevant within
         * preference IDs
         */
        ...
        static void reboundSingleAction(const int &actID, const vector<pair<double, double>> &durationBounds,
                                        bool &keep, pair<double, double> &startBounds,
                                        pair<double, double> &endBounds, const bool workingGlobally = false);
        ...
        static bool canSkipToEnd(const int &i)
        {
            ...
            {
                std::cerr << "Fatal internal error: asked whether action index " << i
                          << " is start--end skippable.\n";
                std::cerr << "Size of relevant array: " << startEndSkip.size()
                          << ".  Number of actions: " << Inst::instantiatedOp::howMany() << ".\n";
                exit(1);
            }
#endif
         ...
         *
         * For action with duration-dependent effects on cost, then if cost is monotonically worsening, the
         * actions' durations are limited. This computes the limits, based on 'for an action to be executed,
         * it can itself consume no more than the remaining cost'.
         *
         * @param costLimit[in]  The limit on remaining cost.  As with numeric limits, everything is compiled
         * to reward.  Thus, if minimising cost, and we have an incumbent solution of cost 1000 and a new
         * partial solution of cost 900, the cost limit is -100: no action can decrease the plan reward (i.e.
         * increase its cost) by more than -100 (100).  If the cost limit is -DBL_MAX, the theoretical min and
         * max durations are return assuming cost is ignored.
         *
         * @param actionDurationBounds[out]  A table of action duration bounds.
         *
         */
        static void doDurationLimitingEffectAnalysis(const double &costLimit,
                                                     vector<pair<double, double>> &actionDurationBounds);
