...
namespace Planner
{
    ...
    struct ConditionAndPolarity
    {
        ...
        ConditionAndPolarity() : first(-1), second(false), polarity(true)
        {
        }

        ConditionAndPolarity(const int &a, const bool &b, const bool &c) : first(a), second(b), polarity(c)
        {
        }
    };
    ...
    protected:
        ...
        virtual void satisfyAtLowCost(const NNF_Flat *, const vector<list<CostedAchieverDetails>> &,
                                      const vector<EpsilonResolutionTimestamp> &,
                                      const vector<EpsilonResolutionTimestamp> *, list<list<Literal *>> &,
                                      list<list<Literal *>> &, list<list<int>> *);

    public:
        virtual const vector<vector<int>> *getComparisonData() = 0;

        virtual void getPreconditionsToSatisfy(
            list<Literal *> &pres, list<Literal *> &negativePres, list<int> *numericPres,
        ...
        virtual bool update(MinimalState *, const pair<bool, bool> &goalOrTriggerChanged,
        ...
        virtual void getCostsOfDeletion(const MinimalState &, map<int, set<int>> &,
                                        map<int, map<double, set<int>>> &)
        {
        }

        virtual void getCostsOfAdding(const MinimalState &, map<int, AddingConstraints> &,
                                      map<int, map<double, AddingConstraints>> &)
        {
        }

        /** @brief Called when a goal/trigger has become satisfied during RPG expansion
         *
         *  When a goal/trigger has become true, it may lead to the the costs of facts in the RPG being
         * reduced.  For instance, for the preference (sometime-before a b), if b has been satisfied, this
         * function is called; and <code>SometimeBeforeFSA::updateCosts</code> will note that adding the fact
         * a no longer violates that preference.
         */
        virtual void updateCosts(const MinimalState &, const bool &, vector<AutomatonPosition::Position> &,
                                 map<int, AddingConstraints> &, map<int, map<double, AddingConstraints>> &,
                                 list<pair<int, int>> &)
        {
        }

        /** @brief Get goals needed to satisfy this preference in RPG extraction */
        virtual void getDesiredGoals(
            list<list<Literal *>> & /* positiveLiterals*/, list<list<Literal *>> & /* negativeLiterals*/,
            list<list<int>> * /* positiveNumeric*/, const MinimalState &, const vector<vector<NNF_Flat *>> &,
            const vector<list<CostedAchieverDetails>> &,
            const vector<EpsilonResolutionTimestamp> & /* negativeLiteralAchievedInLayer*/,
            const vector<EpsilonResolutionTimestamp> *)
        {
        }

        virtual void becomesUnreachableAtTime(const MinimalState &,
                                              map<EpsilonResolutionTimestamp, list<int>> &)
        {
        }
    };
    ...
    public:
        ...
        virtual double GCost(const MinimalState *, const int &);
        virtual void getUnsatisfiedConditionCounts(const MinimalState &, vector<NNF_Flat *> &);
        virtual void getDesiredGoals(list<list<Literal *>> &desiredPositive,
                                     list<list<Literal *>> &desiredNegative, list<list<int>> *desiredNumeric,
    ...
    public:
        SometimeBeforeFSA(const int &i, RPGBuilder::Constraint *const p,
    ...
    public:
        SometimeAfterFSA(const int &i, RPGBuilder::Constraint *const p, AutomatonPosition::Position &initPosn,
                         const VAL::constraint_sort shouldBe = VAL::E_SOMETIMEAFTER);
    ...
    public:
        ...
        static double getCurrentCost(const int &specificPreference, const MinimalState &theState,
                                     const int flag = 3);
        static double getCurrentCost(const int &specificPreference,
                                     const vector<AutomatonPosition::Position> &positions,
                                     const int flag = 3);
        static double getCurrentCost(const double &prefPreconditionViolations,
                                     const vector<AutomatonPosition::Position> &positions,
                                     const int flag = 3);
        ...
        static double getReachableCost(const int &specificPreference, const MinimalState &theState,
                                       const int flag = 3);
        static double getReachableCost(const int &specificPreference,
                                       const vector<AutomatonPosition::Position> &positions,
                                       const int flag = 3);
        ...
        static void getUnsatisfiedConditionCounts(
            const MinimalState &, vector<vector<NNF_Flat *>> &,
            map<EpsilonResolutionTimestamp, list<int>> &withinPreferencesUnreachableAtTime);

        static void getCostsOfDeletion(const MinimalState &, map<int, set<int>> &prefCostOfDeletingFact,
                                       map<int, map<double, set<int>>> &prefCostOfChangingNumberA);
        static void getCostsOfAdding(const MinimalState &, map<int, AddingConstraints> &prefCostOfAddingFact,
                                     map<int, map<double, AddingConstraints>> &prefCostOfChangingNumberB);

        static void
        updateCostsAndPreferenceStatus(const MinimalState &, const pair<int, bool> &whatHasBeenSatisfied,
        ...
        static void getDesiredGoals(list<list<Literal *>> &desiredPositive,
                                    list<list<Literal *>> &desiredNegative, list<list<int>> *desiredNumeric,
                                    const MinimalState &startState,
                                    const vector<vector<NNF_Flat *>> &unsatisfiedPreferenceConditions,
        ...
        static void getPreconditionsToSatisfy(
            list<Literal *> &pres, list<Literal *> &negativePres, list<int> *numPres,
        ...
        static void collectRequiredFactsInNNF(const int &pref, const bool &wasTheTrigger,
                                              set<int> &requiredPositive, set<int> &requiredNegative);
        static void collectAllFactsInNNF(const int &pref, const bool &wasTheTrigger,
                                         set<int> &requiredPositive, set<int> &requiredNegative);
        ...
        static void
        substituteRPGNumericPreconditions(RPGBuilder::BuildingNumericPreconditionData &commonData);

        inline static int compareStatusOfPref(const int &prefIdx, const AutomatonPosition::Position &a,
                                              const AutomatonPosition::Position &b)
        {
