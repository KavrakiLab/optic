...
namespace AutomatonPosition
{
    enum Position
    {
        ...
        triggered = 2,
        unreachable = 3,
        eternallysatisfied = 4,
        seenoncealreadyandstillholds = 5,
        seenoncealready = 6,
        probablyeternallysatisfied = 7
    };
};
...
namespace Planner
{
    ...
    enum time_spec
    {
        ...
        E_CONTINUOUS,
        E_AT,
        E_DUMMY_TEMPORAL_TRIGGER_TRUE,
        E_DUMMY_TEMPORAL_GOAL_TRUE,
        E_DUMMY_PREFERENCE_PRECONDITION,
        E_DUMMY_TEMPORAL_TRIGGER_FALSE,
        E_DUMMY_TEMPORAL_GOAL_FALSE
    };
...
#define SMALL 0.001
    ...
    struct PreferenceSetAndCost
    {
        ...
        PreferenceSetAndCost(const bool isSatisfied = false) : cost(0.0), satisfied(isSatisfied)
        {
        }
    ...
    public:
        ...
        /** @brief The index of the TIL event (an index into
         * <code>RPGBuilder::allTimedInitialLiteralsVector</code>. */
        int divisionID;

        /** @brief Step IDs of actions that must have finished prior to this one.  Deprecated. */
        set<int> needToFinish;

        ActionSegment() : first(0), second(Planner::E_OVER_ALL), divisionID(-1)
        {
        }
    ...
    extern EpsilonResolutionTimestamp operator-(const EpsilonResolutionTimestamp &a,
                                                const EpsilonResolutionTimestamp &b);
    ...
    public:
        ...
        CostedAchieverDetails(const vector<double> &costsIn, const PreferenceSetAndCost &prefCostsIn,
                              const bool &costFreeIn)
          : layer(EpsilonResolutionTimestamp::zero())
          , costFree(costFreeIn)
          , costs(costsIn)
          , preferenceCosts(prefCostsIn)
        {
            achiever.first = -1;
        ...
        CostedAchieverDetails(const EpsilonResolutionTimestamp &layerTime,
                              ...
                              vector<double> &costDetails, const PreferenceSetAndCost &prefCostsIn,
                              const bool &costFreeIn)
          : layer(layerTime)
          , achiever(achieverDetails)
          , costFree(costFreeIn)
          , costs(costDetails)
          , preferenceCosts(prefCostsIn)
        {
    ...
    public:
         ...
         *  - 4096: provide information about the STP constraints used within the incremental Bellman-Ford
         * implementation
         *  - 8192: provide debugging information about trivial cycle checking (if doing totally ordered
         * search)
         ...
         *  - 1048576: provide information about the ordering constraints added to the partial order when
         * applying an action
         */
...
#define EPSILON 0.001
         ...
         *  three techniques, at every state: the LP, the incremental Bellman-Ford, and Floyd-Walshall.
         * Additionally, the latter is ran each time an edge is added to the incremental Bellman-Ford
         * algorithm to check the incremental updates are correct.
         *
         *  @see LPScheduler
         */
         ...
         *  both the LP and incremental Bellman-Ford, without the two being integrated.  The profile data
         * produced by gprof can then be used to ascertain the comparative performance of the two approaches:
         *  - Time taken for the STP is <code>LPScheduler::prime()</code> +
         * <code>ParentData::spawnChildData()</code>
         *  - Time taken for the LP is <code>LPScheduler::LPScheduler()</code> -
         * <code>ParentData::spawnChildData()</code> (as the latter is called from within the LP scheduler
         * constructor, but needs to be discounted as in profiling mode the two are not integrated.)
         *
         *  @see LPScheduler
         */
        ...
        /** @brief Read in <code>planFilename</code> and note that its actions must not be pruned in
         * preprocessing.
         *
         *  @see actionHasToBeKept
         */
         ...
         *  @param synopsis  A short reason for why the action was eliminated.  This is printed if the pruning
        ...
        static void checkActionBounds(const int &actID, const double &startBegin, const double &startEnd,
                                      const double &endBegin, const double &endEnd);
#endif
        ...
        /** @brief The amount by which a new solution must better the best found so far, above a nominal small
         * improvement. */
