...
namespace Planner
{
    ...
    void TemporalAnalysis::dummyDeadlineAnalysis()
    {
        ...
        actionTSBounds = vector<vector<pair<double, double>>>(
            actCount, vector<pair<double, double>>(2, pair<double, double>(0.0, DBL_MAX)));
    };
    ...
    void TemporalAnalysis::processTILDeadlines()
    {
        ...
        if (debug)
        {
            cout << "Performing TIL deadline analysis\n";
        }
...

        {
            ...
            for (int tilID = 0; tilID < tilCount; ++tilID)
            {
                ...
                if (debug)
                {
                    cout << "\tTIL " << tilID << " at " << tilItr->duration << "\n";
                }

                {
                    ...
                    for (; effItr != effEnd; ++effItr)
                    {
                        ...
                        const pair<map<int, bool>::iterator, bool> wItr =
                            windowable.insert(make_pair(litID, true));
                        if (wItr.second)
                        {
                            const list<pair<int, Planner::time_spec>> &etaList =
                                RPGBuilder::getEffectsToActions(litID);
                            ...
                            for (; etItr != etEnd; ++etItr)
                            {
                                if (etItr->second != Planner::E_AT)
                                {
                                    ...
                                    if (debug)
                                    {
                                        ...
                                        {
                                            cout << "\t" << *(RPGBuilder::getInstantiatedOp(etItr->first))
                                                 << " start\n";
                                        }
                        ...
                        if (wItr.first->second)
                        {
                            ...
                            const pair<map<int, double>::iterator, bool> laItr =
                                lastAppearance.insert(make_pair(litID, DBL_MAX));
                            ...
                            if (laItr.second)
                            {
                                if (dest.empty())
                                {
                                    if (initialState.find(*effItr) != initialState.end())
                                    {
                                        ...
                                        if (debug)
                                        {
                                            cout << " - true in the initial state";
                                        }
                                    }
                                    else
                                    {
                                        if (debug)
                                        {
                                            cout << " - not true in the initial state";
                                        }
                                    }
                                }
                                else
                                {
                                    if (debug)
                                    {
                                        cout << " - not reappeared since last delete";
                                    }
                                }
                            }
                            else
                            {
                                if (debug)
                                {
                                    cout << " - last appeared at " << startOfWindow;
                                }
                            }
...

                {
                    ...
                    for (; effItr != effEnd; ++effItr)
                    {
                        ...
                        if (wItr.first->second)
                        {
                            ...
                            const pair<map<int, double>::iterator, bool> laItr =
                                lastAppearance.insert(make_pair(litID, 0.0));

                            if (laItr.second)
                            {
                                double visibleFrom = tilItr->duration;
                                if (windows.find(litID) == windows.end())
                                {  // appeared, but no time-windows defined yet
                                    if (initialState.find(*effItr) != initialState.end())
                                    {
                                        visibleFrom = 0.0;
                                    }
                            ...
                            if (initialState.find(*effItr) == initialState.end() &&
                                RPGBuilder::getEffectsToActions((*effItr)->getStateID()).empty())
                            {
                                literalAppears.insert(
                                    pair<int, double>((*effItr)->getStateID(), tilItr->duration));
                            }
                        }
                        else
                        {
                            if (debug)
                            {
                                ...
                                const list<pair<int, Planner::time_spec>> &etaList =
                                    RPGBuilder::getEffectsToActions(litID);
                                list<pair<int, Planner::time_spec>>::const_iterator etItr = etaList.begin();
                                const list<pair<int, Planner::time_spec>>::const_iterator etEnd =
                                    etaList.end();
...

            {
                ...
                for (; restrictItr != restrictEnd; ++restrictItr)
                {
                    ...
                    list<pair<int, Planner::time_spec>> &affected =
                        RPGBuilder::getPresToActions()[restrictItr->first];
                    ...
                    for (; affItr != affEnd; ++affItr)
                    {
                        if (affItr->second == Planner::E_AT_START)
                        {
                            ...
                            if (epsilonOff > currMin)
                            {
                                ...
                                if (workingGlobally)
                                {
                                    Globals::checkActionBounds(affItr->first,
                                                               actionTSBounds[affItr->first][0].first,
                                                               actionTSBounds[affItr->first][0].second,
                                                               actionTSBounds[affItr->first][1].first,
                                                               actionTSBounds[affItr->first][1].second);
                                }
#endif

                                const double endOff =
                                    epsilonOff + globalActionDurationBounds[affItr->first].first;
                        ...
                        else
                        {
                            ...
                            if (epsilonOff > currMin)
                            {
                                ...
                                const double startOff =
                                    epsilonOff - globalActionDurationBounds[affItr->first].second;
...

            {
                ...
                for (; restrictItr != restrictEnd; ++restrictItr)
                {
                    if (debug)
                    {
                        cout << *(RPGBuilder::getLiteral(restrictItr->first)) << " disappears at "
                             << restrictItr->second << "\n";
                    }
                    ...
                    list<pair<int, Planner::time_spec>> &affected =
                        RPGBuilder::getRawPresToActions()[restrictItr->first];

                    if (debug)
                    {
                        cout << "Bounding " << affected.size() << " start/end points\n";
                    }
                    list<pair<int, Planner::time_spec>>::iterator affItr = affected.begin();
                    const list<pair<int, Planner::time_spec>>::iterator affEnd = affected.end();
                    ...
                    for (; affItr != affEnd; ++affItr)
                    {
                        ...
                        if (affItr->second == Planner::E_AT_START)
                        {
                            ...
                            if (epsilonOff < currMax)
                            {
                                ...
                                if (endOff < endMax)
                                {
                                    ...
                                    if (debug)
                                    {
                                        cout << " => end max down to " << epsilonOff << "+"
                                             << globalActionDurationBounds[affItr->first].second << " = "
                                             << endMax << ";";
                                        cout.flush();
                                    }
#ifdef ENABLE_DEBUGGING_HOOKS
                        ...
                        {  // invariant or at end
                            ...
                            if (epsilonOff < currMax)
                            {
                                ...
                                const double startOff =
                                    epsilonOff - RPGBuilder::getOpMinDuration(affItr->first, 0);
        ...
        for (int i = 0; i < actCount; ++i) {
            if (!RPGBuilder::rogueActions[i]) {
                ...
                // if (startMax < DBL_MAX) cout << "Start of " << *(RPGBuilder::getInstantiatedOp(i)) << " is
        applicable no later than " << startMax << "\n";
            }
        }*/
    };
    ...
    void TemporalAnalysis::findGoalDeadlines(list<Literal *> &goals, list<double> &dest)
    {
        ...
        for (int gid = 0; gItr != gEnd; ++gItr, ++dItr, ++gid)
        {
            ...
            {
                list<pair<int, Planner::time_spec>> &eta =
    ...
    void TemporalAnalysis::findGoalSoftDeadlines(
        std::map<int, std::list<int>> &factRelevantToWithinPreferences,
        ...
        for (int pID = 0; pID < prefCount; ++pID)
        {
...

            {
                ...
                for (; fItr != fEnd; ++fItr)
                {
                    ...
                    {
                        const list<pair<int, Planner::time_spec>> &adders =
                            RPGBuilder::getEffectsToActions()[*fItr];
                        ...
                        if (adders.size() == 1)
                        {
                            if (adders.front().second != Planner::E_AT)
                            {
                                const list<Literal *> &pres =
                                    RPGBuilder::getProcessedStartPropositionalPreconditions()[adders.front()
                                                                                                  .first];
                                ...
                                for (; pItr != pEnd; ++pItr)
                                {
                                    cout << "Only one action adds " << *(RPGBuilder::getLiteral(*fItr))
                                         << " so its precondition " << *(*pItr)
                                         << " is relevant to within preferences\n";
    ...
    // void TemporalAnalysis::findGoalSoftDeadlines(vector<map<EpsilonResolutionTimestamp, int > > &
    // softDeadlines,
    ...
    //
    //             if (tinitialState.find(RPGBuilder::getLiteral(gidItr->first)) != tinitialState.end())
    //             continue;
    //
    ...
    //                 std::cerr << "Fatal error when parsing input preference: two within preferences are
    //                 defined to act on the same fact, at the same time.\nMerge them into one and run the
    //                 planner again.\n"; exit(1);
    //             }
    //
    //
    ...
    //                     {
    //                         cout << "Soft-deadlines on goals do not have monotonically worsening cost due
    //                         to soft-deadline at " << sdItr->first << " on "; list<Literal*>::const_iterator
    //                         gItr = goals.begin(); for (int i = 0; i < gID; ++i, ++gItr) ; cout << *(*gItr)
    //                         << endl;
    //                     }
    //                     yesGoalSoftDeadlinesHaveMonotonicallyWorseningCosts = false;
    //                     return;
    ...
    //
    //                 (*(PreferenceStatusArray::getGlobalInitialArray()))[wpItr->first] =
    //                 AutomatonPosition::eternallysatisfied;
    //
    //             }
    //         }
    //     }
        ...
        for (int a = 0; a < actCount; ++a)
        {
            if (!RPGBuilder::rogueActions[a])
            {
                ...
                if (workingGlobally)
                {
                    ...
                    {
                        Globals::checkActionBounds(a, actionTSBounds[a][0].first,
                                                   actionTSBounds[a][0].second);
                    }
                    else
                    {
                        Globals::checkActionBounds(a, actionTSBounds[a][0].first, actionTSBounds[a][0].second,
                                                   actionTSBounds[a][1].first, actionTSBounds[a][1].second);
                    }
                }
#endif
                ...
                {
                    // cout << "Initial RPG bounds " << *(getInstantiatedOp(a)) << " no earlier than " <<
                    ...
                    if (earliestStart < (earliestEnd - maxActDur) - 0.0005)
                    {
                        ...
                        //                  cout << "Maximum act duration = " << maxActDur << ", so pulling
                        //                  start to " << earliestStart << "\n";
                    }
                    ...
                    if (isRogue)
                    {
                        cout << "Pruning " << *(RPGBuilder::getInstantiatedOp(a))
                             << " - temporal contradiction: start min/max = " << actionTSBounds[a][0].first
                             << "/" << actionTSBounds[a][0].second;
                        cout << "; end min/max = " << actionTSBounds[a][1].first << "/"
                             << actionTSBounds[a][1].second;
                        cout << "; duration min/max = " << globalActionDurationBounds[a].first << "/"
                             << globalActionDurationBounds[a].second << endl;
#ifdef ENABLE_DEBUGGING_HOOKS
                        Globals::eliminatedAction(a, "The earliest point the start could be applied was too "
                                                     "close to the latest point at which the end could be "
                                                     "applied");
#endif
                    }
                }
     ...
     *
     *  @retval <code>true</code>   There is no overlap between the effects list and the preconditions of
     * actions or goals
    ...
    {
        const map<int, set<int>>::const_iterator guardItr =
            NumericAnalysis::getFactsThatDefineAndFixVariables().find(v);
        if (guardItr != NumericAnalysis::getFactsThatDefineAndFixVariables().end())
        {
            set<int> isect;
            std::set_intersection(guardItr->second.begin(), guardItr->second.end(), definiteFacts.begin(),
                                  definiteFacts.end(), std::insert_iterator<set<int>>(isect, isect.begin()));
    ...
    bool endNumericPreconditionsOnlyReferToTime(const list<int> &pres, const set<int> &definiteFacts)
    {
        ...
        for (; preItr != preEnd; ++preItr)
        {
            ...
            {
                if (NumericAnalysis::getVariablesThatAreTickers().find(currPre.LHSVariable) !=
                    NumericAnalysis::getVariablesThatAreTickers().end())
                {
                    continue;
                }
            ...
            {
                if (NumericAnalysis::getVariablesThatAreTickers().find(currPre.LHSVariable - pneCount) !=
                    NumericAnalysis::getVariablesThatAreTickers().end())
                {
                    continue;
                }
            ...
            else
            {
                ...
                for (int s = 0; s < currAV.size; ++s)
                {
                    ...
                    {
                        if (NumericAnalysis::getVariablesThatAreTickers().find(currAV.fluents[s]) !=
                            NumericAnalysis::getVariablesThatAreTickers().end())
                        {
                            continue;
                        }
    ...
    {
        const vector<list<pair<int, Planner::time_spec>>> &preconditionsToActions =
            RPGBuilder::getRawPresToActions();
        const vector<list<pair<int, Planner::time_spec>>> &negativeEffectsToActions =
            RPGBuilder::negativeEffectsToActions;

        const int litCount = preconditionsToActions.size();

        // First, we exclude facts that could be added by a TIL during the action and then deleted again by
        // its end. In this case, moving the end delete to the start and scrubbing the invariant is not
        // equivalent.
        ...
        for (int lit = 0; lit < litCount; ++lit)
        {
            ...
            {
                if (*exItr == lit)
                {  // skip any facts manipulated by TILs
                    ++exItr;
                    if (Globals::globalVerbosity & 16384)
                    {
                        cout << *RPGBuilder::getLiteral(lit)
                             << ", if end-deleted, will break the action being compression safe, due to "
                                "potential TIL interactions\n";
                    }
            ...
            const list<pair<int, Planner::time_spec>>::const_iterator eEnd =
                negativeEffectsToActions[lit].end();
            ...
            const list<pair<int, Planner::time_spec>>::const_iterator pEnd =
                preconditionsToActions[lit].end();
            ...
            for (; pItr != pEnd; ++pItr)
            {
                switch (pItr->second)
                {
                    ...
                    default:
                    {
                        std::cerr << "Fatal internal error: precondition recorded for a time specifier other "
                                     "than at start, over all or at end.\n";
                        exit(1);
                    }
                }
            }
            ...
            std::set_difference(actionsWithFactAsAStartInvEndCondition[2].begin(),
                                actionsWithFactAsAStartInvEndCondition[2].end(),
                                actionsWithFactAsAStartInvEndCondition[1].begin(),
                                actionsWithFactAsAStartInvEndCondition[1].end(),
            ...
            if (!onlyAsEnd.empty())
            {
                ...
                {
                    cout << *RPGBuilder::getLiteral(lit)
                         << ", if end-deleted, will break the action being compression safe, as it can be "
                            "required only at the end of some action\n";
                }
                continue;
            }
            // Actions which only have the fact as an 'at start' precondition - these must also delete at
            // start

            set<int> onlyAsAStart;

            std::set_difference(actionsWithFactAsAStartInvEndCondition[0].begin(),
                                actionsWithFactAsAStartInvEndCondition[0].end(),
                                actionsWithFactAsAStartInvEndCondition[1].begin(),
                                actionsWithFactAsAStartInvEndCondition[1].end(),
            ...
            std::set_difference(onlyAsAStart.begin(), onlyAsAStart.end(), startEndDelete[0].begin(),
                                startEndDelete[0].end(),
                                insert_iterator<set<int>>(onlyAsStartButNotThenDeleted,
                                                          onlyAsStartButNotThenDeleted.begin()));

            if (!onlyAsStartButNotThenDeleted.empty())
            {
                // breaks the idiom: cannot move the end delete to the start of an action if something else
                // could use it only momentarily during its execution.
                if (Globals::globalVerbosity & 16384)
                {
                    cout << *RPGBuilder::getLiteral(lit)
                         << ", if end-deleted, will break the action being compression safe, as it can "
                            "needed at the start of an action, but isn't then deleted\n";
                }
            ...
            std::set_difference(
                actionsWithFactAsAStartInvEndCondition[1].begin(),
                actionsWithFactAsAStartInvEndCondition[1].end(), startEndDelete[1].begin(),
                startEndDelete[1].end(),
                insert_iterator<set<int>>(overAllButNotDeletedAtEnd, overAllButNotDeletedAtEnd.begin()));

            if (!overAllButNotDeletedAtEnd.empty())
            {
                // breaks the idiom: could potentially use and not delete the fact after the start but before
                // the end of an action which will delete the fact at its end
                if (Globals::globalVerbosity & 16384)
                {
                    cout << *RPGBuilder::getLiteral(lit)
                         << ", if end-deleted, will break the action being compression safe, as some action "
                         << *RPGBuilder::getInstantiatedOp(*(overAllButNotDeletedAtEnd.begin()))
                         << " needs it as an invariant, but doesn't then delete it\n";
                }
            ...
            {
                cout << *RPGBuilder::getLiteral(lit)
                     << " is end-deleted, but recognised as still being compression safe\n";
            }
        }
    }

#ifdef POPF3ANALYSIS
    bool TemporalAnalysis::endNumericEffectsAreCompressionSafe(
        const list<int> &effects, vector<bool> &allInteractionWithVarIsCompressionSafe)
    {
        ...
        for (; effItr != effEnd; ++effItr)
        {
            ...
            if (!allInteractionWithVarIsCompressionSafe[currEff.fluentIndex])
            {
                if (Globals::globalVerbosity & 16384)
                {
                    cout << "Interacts with non-compression safe variable "
                         << *(RPGBuilder::getPNE(currEff.fluentIndex)) << endl;
                }
                return false;
            }

            // third, the effect has to be only ever a good idea, with respect to the dominance constraints on
            // the variable
            switch (NumericAnalysis::getDominanceConstraints()[currEff.fluentIndex])
            {
                case NumericAnalysis::E_NODOMINANCE:
                {
                    if (Globals::globalVerbosity & 16384)
                    {
                        cout << "Interacts with variable with no dominance constraints, "
                             << *(RPGBuilder::getPNE(currEff.fluentIndex)) << endl;
                    }
                    return false;
                }
                case NumericAnalysis::E_METRICTRACKING:
                {
                    if (Globals::globalVerbosity & 16384)
                    {
                        cout << "Interacts with metric-tracking variable, "
                             << *(RPGBuilder::getPNE(currEff.fluentIndex)) << endl;
                    }
                    break;
                }
                case NumericAnalysis::E_IRRELEVANT:
                {
                    if (Globals::globalVerbosity & 16384)
                    {
                        cout << "Interacts with irrelevant variable, "
                             << *(RPGBuilder::getPNE(currEff.fluentIndex)) << endl;
                    }
                    break;
                }
                case NumericAnalysis::E_SMALLERISBETTER:
                {
                    ...
                    {  // if smaller is better, and this is an increase, it's not compression safe
                        if (Globals::globalVerbosity & 16384)
                        {
                            cout << "Has undesirable increase effect on "
                                 << *(RPGBuilder::getPNE(currEff.fluentIndex)) << endl;
                        }
                        return false;
                    }
                ...
                {
                    if (currEff.constant < 0.0)
                    {  // if bigger is better, and this is a decrease, it's not compression safe
                        if (Globals::globalVerbosity & 16384)
                        {
                            cout << "Has undesirable decrease effect on "
                                 << *(RPGBuilder::getPNE(currEff.fluentIndex)) << endl;
                        }
                        return false;
                    }
    ...
    void TemporalAnalysis::markCompressionSafetyConditions(
        const int &actID, const list<int> &effects,
    ...
    void TemporalAnalysis::markAffectedVariablesAsNotCompressionSafe(
        const list<int> &effects, vector<bool> &allInteractionWithVarIsCompressionSafe, set<int> &newlyUnsafe)
    {
        list<int>::const_iterator effItr = effects.begin();
        const list<int>::const_iterator effEnd = effects.end();
    ...
    struct TimeDependentLimit
    {
        ...
        TimeDependentLimit() : RHSconstant(0.0), durationWeight(0.0), timeWeight(0.0), maxValueOfRest(DBL_MAX)
        {
        }

        TimeDependentLimit(const double &rhs, const double &durWeight, const double &tWeight,
                           const double &lhsRest)
    ...
    void TemporalAnalysis::findCompressionSafeActions()
    {
        ...
        {
            allInteractionWithVarIsCompressionSafe[v] =
                (NumericAnalysis::getDominanceConstraints()[v] == NumericAnalysis::E_IRRELEVANT ||
                 ((NumericAnalysis::getDataOnWhichVariablesHaveOrderIndependentEffects()[v] ==
                   NumericAnalysis::E_ORDER_INDEPENDENT) &&
        ...
        for (int i = 0; i < actCount; ++i)
        {
            if (!RPGBuilder::rogueActions[i])
            {
                if (RPGBuilder::getRPGDEs(i).empty())
                {
                    continue;
                }
                ...
                if (Globals::globalVerbosity & 16384)
                {
...
#else
                    startEndSkip[i] = endNumericEffectsAreCompressionSafe(
                        RPGBuilder::actionsToRPGNumericEndEffects[i], allInteractionWithVarIsCompressionSafe);
#endif
#else
                    startEndSkip[i] = RPGBuilder::actionsToRPGNumericEndEffects[i].empty();
#endif
                    ...
                    if (!startEndSkip[i])
                    {
                        cout << "Action " << *(RPGBuilder::getInstantiatedOp(i))
                             << " cannot be compression safe - has unsafe end numeric effects\n";
                    }
                    else
                    {
                        startEndSkip[i] = RPGBuilder::actionsToRPGNumericInvariants[i].empty() &&
                                          endNumericPreconditionsOnlyReferToTime(
                                              RPGBuilder::actionsToRPGNumericEndPreconditions[i],
                                              factsWeDefinitelyHaveAfterTheStart) &&
                                          !RPGBuilder::linearDiscretisation[i];

                        if (!startEndSkip[i])
                        {
                            cout << "Action " << *(RPGBuilder::getInstantiatedOp(i))
                                 << " cannot be compression safe - has non-start numerics\n";
                        }
                        else
                        {
                            startEndSkip[i] =
                                firstIsSubsumedBySecond(RPGBuilder::actionsToEndPreconditions[i],
                                                        RPGBuilder::actionsToInvariants[i]) &&
                                firstIsSubsumedBySecond(RPGBuilder::actionsToEndNegativePreconditions[i],
                                                        RPGBuilder::actionsToNegativeInvariants[i]);
                            if (!startEndSkip[i])
                            {
                                cout << "Action " << *(RPGBuilder::getInstantiatedOp(i))
                                     << " cannot be compression safe - end pres aren't subsumed by "
                                        "invariants\n";
                            }
                            else
                            {
                                startEndSkip[i] = noOverlap(RPGBuilder::actionsToEndNegativeEffects[i],
                                                            RPGBuilder::preconditionsToActions,
                                                            PreferenceHandler::getPreconditionsToPrefs(),
                                                            goalsAsASet, okayToDeleteAtEnd);
                                if (!startEndSkip[i])
                                {
                                    cout << "Action " << *(RPGBuilder::getInstantiatedOp(i))
                                         << " cannot be compression safe - end delete effects intersect with "
                                            "needed preconditions\n";
                                }
                                else
                                {
                                    startEndSkip[i] =
                                        noOverlap(RPGBuilder::actionsToEndEffects[i],
                                                  RPGBuilder::negativePreconditionsToActions,
                                                  PreferenceHandler::getNegativePreconditionsToPrefs(),
                                                  negativeGoalsAsASet, okayToAddAtEnd);
                                    if (!startEndSkip[i])
                                    {
                                        cout << "Action " << *(RPGBuilder::getInstantiatedOp(i))
                                             << " cannot be compression safe - end add effects intersect "
                                                "with needed negative preconditions\n";
                                    }
                                }
                            }
                        }
                ...
                else
                {
...
#ifdef TOTALORDERSTATES
                    startEndSkip[i] =
                        RPGBuilder::actionsToRPGNumericEndEffects[i].empty()
#else
                    startEndSkip[i] =
                        endNumericEffectsAreCompressionSafe(RPGBuilder::actionsToRPGNumericEndEffects[i],
                    ...
                    startEndSkip[i] =
                        ...
                        firstIsSubsumedBySecond(RPGBuilder::actionsToEndNegativePreconditions[i],
                                                RPGBuilder::actionsToNegativeInvariants[i]) &&
                        noOverlap(
                            RPGBuilder::actionsToEndNegativeEffects[i], RPGBuilder::preconditionsToActions,
                            PreferenceHandler::getPreconditionsToPrefs(), goalsAsASet, okayToDeleteAtEnd) &&
                        noOverlap(RPGBuilder::actionsToEndEffects[i],
                                  RPGBuilder::negativePreconditionsToActions,
                                  PreferenceHandler::getNegativePreconditionsToPrefs(), negativeGoalsAsASet,
                                  okayToAddAtEnd);
                }
                ...
                if (startEndSkip[i])
                {
...

                        {
                            ...
                            for (; clItr != clEnd; ++clItr)
                            {
                                RPGBuilder::RPGNumericPrecondition &currPre =
                                    RPGBuilder::rpgNumericPreconditions[*clItr];
                                if (currPre.LHSVariable < -1)
                                {
                                    if (Globals::globalVerbosity & 16384)
                                    {
                                        cout << "Action " << *(RPGBuilder::getInstantiatedOp(i))
                                             << " is not compression safe: it has a duration-dependent start "
                                                "condition "
                                             << currPre << "\n";
                                    }
                                    ddes = true;
                                ...
                                {
                                    RPGBuilder::ArtificialVariable &currAV =
                                        RPGBuilder::getArtificialVariable(currPre.LHSVariable);
                                    for (int j = 0; j < currAV.size; ++j)
                                    {
                                        if (currAV.fluents[j] < -1)
                                        {
                                            ...
                                            {
                                                cout << "Action " << *(RPGBuilder::getInstantiatedOp(i))
                                                     << " is not compression safe: it has a "
                                                        "duration-dependent start condition "
                                                     << currPre << "\n";
                                            }
                            ...
                            for (; clItr != clEnd; ++clItr)
                            {
                                ...
                                for (int j = 0; j < currEff.size; ++j)
                                {
                                    if (currEff.variables[j] < -1)
                                    {
                                        ...
                                        {
                                            cout << "Action " << *(RPGBuilder::getInstantiatedOp(i))
                                                 << " is not compression safe: it has a duration-dependent "
                                                    "start effect "
                                                 << currEff << "\n";
                                        }
                    ...
                    if (startEndSkip[i])
                    {
                        ...
                        {
                            cout << *(RPGBuilder::getInstantiatedOp(i))
                                 << " is a candidate for start-end skipping\n";
                        }

#ifdef POPF3ANALYSIS
                        markCompressionSafetyConditions(i, RPGBuilder::actionsToRPGNumericEndEffects[i],
                                                        actionsDependingOnVarBeingCompressionSafe);
#endif
                    }
                    else
                    {
                        ...
                        markAffectedVariablesAsNotCompressionSafe(
                            RPGBuilder::actionsToRPGNumericEndEffects[i],
                            allInteractionWithVarIsCompressionSafe, newNonCompressionSafeVariables);
                        ...
                        while (!newNonCompressionSafeVariables.empty())
                        {
                            ...
                            for (; vItr != vEnd; ++vItr)
                            {
                                set<int>::const_iterator noncsActItr =
                                    actionsDependingOnVarBeingCompressionSafe[*vItr].begin();
                                const set<int>::const_iterator noncsActEnd =
                                    actionsDependingOnVarBeingCompressionSafe[*vItr].end();
                                ...
                                for (; noncsActItr != noncsActEnd; ++noncsActItr)
                                {
                                    if (startEndSkip[*noncsActItr])
                                    {
                                        ...
                                        ++reversed;
                                        markAffectedVariablesAsNotCompressionSafe(
                                            RPGBuilder::actionsToRPGNumericEndEffects[*noncsActItr],
                                            allInteractionWithVarIsCompressionSafe,
                                            newNonCompressionSafeVariables);
                                    }
                                }

                                if (Globals::globalVerbosity & 16)
                                {
                                    cout << "Shown that interaction with " << *(RPGBuilder::getPNE(*vItr))
                                         << " is not compression safe, actions affected: " << reversed
                                         << endl;
                                }
                ...
                if (Globals::globalVerbosity & 16384)
                {
                    if (startEndSkip[i])
                    {
                        cout << COLOUR_light_green << "Overall, action "
                             << *(RPGBuilder::getInstantiatedOp(i)) << " is compression safe\n"
                             << COLOUR_default;
                    }
                    else
                    {
                        cout << COLOUR_light_red << "Overall, action " << *(RPGBuilder::getInstantiatedOp(i))
                             << " is not compression safe\n"
                             << COLOUR_default;
                    }
                }
            }
        ...
        if (Globals::globalVerbosity & 16)
        {
            for (int v = 0; v < pneCount; ++v)
            {
                ...
                {
                    cout << "Detecting compression-safety of interaction with " << *(RPGBuilder::getPNE(v))
                         << " has allowed " << actionsDependingOnVarBeingCompressionSafe[v].size()
                         << " action(s) to be compression safe\n";
                }
            }
        }
#endif
        ...
        if (nonRogueCount)
        {
            ...
            {
                cout << "None of the ground temporal actions in this problem have been recognised as "
                        "compression-safe\n";
            }
            else
            {
                const int percentage =
                    (int)((double)compressionSafeActionCount / (double)nonRogueCount * 100.0);
                cout << percentage
                     << "% of the ground temporal actions in this problem are compression-safe\n";
            }
        }
    }
    ...
    void TemporalAnalysis::buildTimelinesOnTILs()
    {
        ...
        for (int til = 0; til < tilCount; ++til)
        {
            ...
            for (int addDeletePass = 0; addDeletePass < 2; ++addDeletePass)
            {
                list<Literal *>::const_iterator addItr =
                    (addDeletePass ? tilVec[til]->delEffects.begin() : tilVec[til]->addEffects.begin());
                const list<Literal *>::const_iterator addEnd =
                    (addDeletePass ? tilVec[til]->delEffects.end() : tilVec[til]->addEffects.end());
                ...
                for (; addItr != addEnd; ++addItr)
                {
                    ...
                    if (fItr == timelineOnTILs.end())
                    {
                        ...
                        {
                            fItr->second.happenings.insert(
                                make_pair(-EPSILON, TILTimeline::AddedOrDeleted(true)));
                        }

                        set<pair<int, Planner::time_spec>> requiredBy;
                        requiredBy.insert(RPGBuilder::getRawPresToActions()[fID].begin(),
                                          RPGBuilder::getRawPresToActions()[fID].end());

                        for (int pass = 0; pass < 2; ++pass)
                        {
                            const list<pair<int, Planner::time_spec>> &affectedBy =
                                (pass ? RPGBuilder::getNegativeEffectsToActions()[fID] :
                                        RPGBuilder::getEffectsToActions(fID));
                        ...
                        fItr->second.allReferencesToItDeleteIt =
                            (requiredBy.empty() && abstractTILsWherePossible &&
                             fItr->second.onlyEverAddedByTILs);
                    }

                    if (addDeletePass)
                    {
                        fItr->second.happenings
                            .insert(make_pair(tilVec[til]->duration, TILTimeline::AddedOrDeleted(false)))
                            .first->second.deleted = true;
                    }
                    else
                    {
                        fItr->second.happenings
                            .insert(make_pair(tilVec[til]->duration, TILTimeline::AddedOrDeleted(true)))
                            .first->second.added = true;
                    }
                }
            }
        }
...

        {
            ...
            for (; tilFactItr != tilFactEnd; ++tilFactItr)
            {
                ...
                {
                    cout << "(ii) Abstracting out TIL " << *(RPGBuilder::getLiteral(tilFactItr->first))
                         << ": is a single window [" << LB << "," << UB << "]\n";
                }
                else
                {
                    cout << "(ii) Abstracting out TIL " << *(RPGBuilder::getLiteral(tilFactItr->first))
                         << ": appears in " << windows << " windows, bounded overall by [" << LB << "," << UB
                         << "]\n";
                }
        ...
        for (int til = 0; til < tilCount; ++til)
        {
            ...
            if (!tilVec[til]->addEffects.empty() || !tilVec[til]->delEffects.empty())
            {
                ...
                {
                    {
                        list<Literal *> &effList =
                            RPGBuilder::nonAbstractedTimedInitialLiteralsVector.back()->addEffects;
                        ...
                        {
                            RPGBuilder::effectsToActions[(*elItr)->getStateID()].remove(
                                make_pair(til, Planner::E_AT));
                            RPGBuilder::effectsToActions[(*elItr)->getStateID()].push_back(
                                make_pair(newTilID, Planner::E_AT));
                        }
                    }
                    {
                        list<Literal *> &effList =
                            RPGBuilder::nonAbstractedTimedInitialLiteralsVector.back()->delEffects;
                        ...
                        for (; elItr != elEnd; ++elItr)
                        {
                            RPGBuilder::negativeEffectsToActions[(*elItr)->getStateID()].remove(
                                make_pair(til, Planner::E_AT));
                            RPGBuilder::negativeEffectsToActions[(*elItr)->getStateID()].push_back(
                                make_pair(newTilID, Planner::E_AT));
                        }
                    }
                }
        ...
        assert(abstractTILsWherePossible ||
               RPGBuilder::getNormalTILVec() == RPGBuilder::getNonAbstractedTILVec());
        ...
        for (; tilItr != tilEnd; ++tilItr)
        {
            ...
            list<pair<int, Planner::time_spec>> &dependents =
                RPGBuilder::getRawPresToActions()[tilItr->first];

            assert((tilItr->second.onlyEverAddedByTILs && tilItr->second.allReferencesToItDeleteIt &&
                    tilItr->second.closedEnded) ||
                   (tilItr->second.numberOfWindows >= 1 && tilItr->second.closedEnded &&
                    tilItr->second.onlyEverAddedByTILs && tilItr->second.onlyEverDeletedByTILs));
    ...
    void TemporalAnalysis::reboundActionsGivenTILTimelines()
    {
...

        {
            for (int np = 0; np < numPreCount; ++np)
            {
                    ...
                    for (int s = 0; s < currAV.size; ++s)
                    {
                        ...
                        else if (v < pneCount)
                        {
                            ...
                            if (tItr != tickers.end())
                            {
                                ...
                                if (lpDebug & 1)
                                {
                                    cout << "- Found it: " << *(RPGBuilder::getPNE(v)) << "*" << timeWeight
                                         << endl;
                                }
                            }
                            else
                ...
                if (lpDebug & 1)
                {
                    cout << "Found a precondition containing weighted " << timeWeight << " time: " << currPre
                         << endl;
                    cout << "RHS is " << currPre.RHSConstant << endl;
                    cout << "The rest of the LHS evaluates to " << valueOfRest.first << ","
                         << valueOfRest.second << endl;
                }

                if (valueOfRest.second != DBL_MAX)
                {
                    boundsImpliedByCondition[np] = TimeDependentLimit(currPre.RHSConstant, durationWeight,
                                                                      timeWeight, valueOfRest.second);

                    if (lpDebug & 1)
                    {
                        if (boundsImpliedByCondition[np].timeWeight > 0.0)
                        {
                            cout << "Lower bound implied by condition: ("
                                 << boundsImpliedByCondition[np].RHSconstant -
                                        boundsImpliedByCondition[np].maxValueOfRest
                                 << " - " << durationWeight << " * ?duration) / "
                                 << boundsImpliedByCondition[np].timeWeight << endl;
                        }
                        if (boundsImpliedByCondition[np].timeWeight < 0.0)
                        {
                            cout << "Upper bound implied by condition: ("
                                 << -boundsImpliedByCondition[np].RHSconstant +
                                        boundsImpliedByCondition[np].maxValueOfRest
                                 << " + " << durationWeight << " * ?duration) / "
                                 << -boundsImpliedByCondition[np].timeWeight << endl;
        ...
        for (int actID = 0; actID < actCount; ++actID)
        {
            ...
            if (lpDebug & 1)
            {
                cout << "Seeing where " << *(RPGBuilder::getInstantiatedOp(actID))
                     << " sits on the TIL timelines\n";
            }
            ...
            {
                const list<int> &instantPres = (pass ? RPGBuilder::getEndPreNumerics()[actID] :
                ...
                {
                    const double impliedLB = boundsImpliedByCondition[*ipItr].getLower(
                        globalActionDurationBounds[actID].first, globalActionDurationBounds[actID].second);
                    const double impliedUB = boundsImpliedByCondition[*ipItr].getUpper(
                        globalActionDurationBounds[actID].first, globalActionDurationBounds[actID].second);
                    ...
                    if (pass)
                    {
                        if (impliedLB > endBounds.first)
                        {
                            if (lpDebug & 1)
                            {
                                cout << "- Pushed end lower bound up to " << impliedLB << " because of "
                                     << RPGBuilder::getNumericPreTable()[*ipItr] << endl;
                            }
                        ...
                        if (impliedUB < endBounds.second)
                        {
                            if (lpDebug & 1)
                            {
                                cout << "- Pushed end upper bound down to " << impliedUB << " because of "
                                     << RPGBuilder::getNumericPreTable()[*ipItr] << endl;
                            }
                    ...
                    else
                    {
                        if (impliedLB > startBounds.first)
                        {
                            if (lpDebug & 1)
                            {
                                cout << "- Pushed start lower bound up to " << impliedLB << " because of "
                                     << RPGBuilder::getNumericPreTable()[*ipItr] << endl;
                            }
                        ...
                        if (impliedUB < startBounds.second)
                        {
                            if (lpDebug & 1)
                            {
                                cout << "- Pushed start upper bound down to " << impliedUB << " because of "
                                     << RPGBuilder::getNumericPreTable()[*ipItr] << endl;
                            }
            ...
            reboundSingleAction(actID, globalActionDurationBounds, keep, startBounds, endBounds,
                                workingGlobally);
        ...
        if (firstCall)
        {
            ...
            {
#ifdef ENABLE_DEBUGGING_HOOKS
                Globals::eliminatedAction(*pruneItr, "TIL timelines cannot find a point at which it can "
                                                     "happen");
#endif
                cout << "Pruning " << *(RPGBuilder::getInstantiatedOp(*pruneItr))
                     << " - the TIL timelines never allow it to be applied\n";
    ...
    struct RelevantTimelinePointsAndOffset
    {
        ...
        RelevantTimelinePointsAndOffset(const TILTimeline::const_iterator &a,
                                        const TILTimeline::const_iterator &b, const double &so,
                                        const double &eo)
    ...
    void TemporalAnalysis::reboundSingleAction(const int &actID,
                                               const vector<pair<double, double>> &durationBounds, bool &keep,
                                               pair<double, double> &startBounds,
                                               pair<double, double> &endBounds, const bool workingGlobally)
    {
        static const int lpDebug = 0;
        ...
        if (keep)
        {
            ...
            if (startBounds.first > startBounds.second)
            {
                if (lpDebug & 1)
                {
                    cout << "- Pruning action: start lower bound of " << startBounds.first
                         << " exceeds upper bound of " << startBounds.second << endl;
                }
                keep = false;
            }
            ...
            if (endBounds.first > endBounds.second)
            {
                if (lpDebug & 1)
                {
                    cout << "- Pruning action: end lower bound of " << endBounds.first
                         << " exceeds upper bound of " << endBounds.second << endl;
                }
                keep = false;
            }
        }
        ...
        if (workingGlobally)
        {
            ...
            {
                Globals::checkActionBounds(actID, startBounds.first, startBounds.second, endBounds.first,
                                           endBounds.second);
            }
        }
#endif
        ...
        for (int pass = 0; keep && pass < 2; ++pass)
        {
            const map<int, list<int>> &pointLookIn =
                (pass ? actionEndUsesAbstractedFact : actionStartUsesAbstractedFact);
            ...
            for (int offsetPass = 0; keep && offsetPass < 2; ++offsetPass)
            {
                const map<int, list<int>> &lookIn =
                    (offsetPass ? actionOverAllUsesAbstractedFact : pointLookIn);
                ...
                if (tdItr != lookIn.end())
                {
                    ...
                    for (; keep && pItr != pEnd; ++pItr)
                    {
...

                        {
                            ...
                            if (ub != currTimeline->end() && ub->first - UBoffset < overallWorkingUpper)
                            {
                                if (lpDebug & 1)
                                {
                                    if (offsetPass == 0)
                                    {
                                        cout << "- " << *(RPGBuilder::getLiteral(*pItr))
                                             << " pushes upper bound " << pass << " down to "
                                             << ub->first - UBoffset
                                    ...
                                    {
                                        cout << "- " << *(RPGBuilder::getLiteral(*pItr))
                                             << " being over all pushes upper bound " << pass << " down to "
                                             << ub->first - UBoffset
                                             << ", i.e. alonside the last TIL deletor\n";
                                    }
                                }
                                ...
                                if (overallWorkingUpper < actualBoundForSuccessors)
                                {
                                    if (lpDebug & 1)
                                    {
                                        cout << "- This is before the lower bound " << pass << " of "
                                             << actualBoundForSuccessors
                        ...
                        {
                            cout << "- " << *(RPGBuilder::getLiteral(*pItr))
                                 << " - looking for suitable adder for time "
                                 << actualBoundForSuccessors + canTolerateLaterAddBy << endl;
                        }
                        TILTimeline::const_iterator adder = currTimeline->firstAdderSuitableForTime(
                            actualBoundForSuccessors + canTolerateLaterAddBy);
                        ...
                        if (adder->first + LBoffset > actualBoundForSuccessors)
                        {
                            if (lpDebug & 1)
                            {
                                ...
                                {
                                    cout << "- " << *(RPGBuilder::getLiteral(*pItr))
                                         << " leads to pushing lower-bound " << pass << " up to "
                                         << adder->first + LBoffset << endl;
                                }
                                else
                                {
                                    cout << "- " << *(RPGBuilder::getLiteral(*pItr))
                                         << " being over all leads to pushing lower-bound " << pass
                                         << " up to " << adder->first << " + " << LBoffset << " = "
                                         << LBoffset << endl;
                                }
                            }

                            pushedForwards = (!relevantTimelinePointsAndTheirEnds[0].empty() ||
                                              !relevantTimelinePointsAndTheirEnds[1].empty());
                            actualBoundForSuccessors = adder->first + LBoffset;
                            if (overallWorkingUpper < actualBoundForSuccessors)
                            {
                                if (lpDebug & 1)
                                {
                                    cout << "- This is after the upper bound " << pass << " of "
                                         << overallWorkingUpper << ", so the LP would be unsolvable\n";
                                }
                                keep = false;
#ifdef ENABLE_DEBUGGING_HOOKS
                                if (workingGlobally)
                                {
                                    if (RPGBuilder::getRPGDEs(actID).empty())
                                    {
                                        Globals::checkActionBounds(actID, startBounds.first,
                                                                   startBounds.second);
                                    }
                                    else
                                    {
                                        Globals::checkActionBounds(actID, startBounds.first,
                                                                   startBounds.second, endBounds.first,
                                                                   endBounds.second);
                                    }
                                }
#endif
                        ...
                        else
                        {
                            ...
                            {
                                cout << "- " << *(RPGBuilder::getLiteral(*pItr))
                                     << " is okay for now: " << adder->first
                                     << " is below the known lower bound " << pass << "\n";
                            }
                        }
                        ...
                        if (pass)
                        {
                            if (offsetPass)
                            {
                                relevantTimelinePointsAndTheirEnds[pass].push_back(
                                    RelevantTimelinePointsAndOffset(adder, currTimeline->end(), EPSILON,
                                                                    EPSILON));
                            }
                            else
                            {
                                relevantTimelinePointsAndTheirEnds[pass].push_back(
                                    RelevantTimelinePointsAndOffset(adder, currTimeline->end(), 0.0,
                                                                    EPSILON));
                            }
                        }
                        else
                        {
                            ...
                            {
                                relevantTimelinePointsAndTheirEnds[pass].push_back(
                                    RelevantTimelinePointsAndOffset(adder, currTimeline->end(), 0.0, 0.0));
                            }
                            else
                            {
                                relevantTimelinePointsAndTheirEnds[pass].push_back(
        ...
        if (keep)
        {
            ...
            if (startBounds.first > startBounds.second)
            {
                ...
                if (workingGlobally)
                {
                    ...
                    {
                        Globals::checkActionBounds(actID, startBounds.first, startBounds.second,
                                                   endBounds.first, endBounds.second);
                    }
                }
#endif
            }
        ...
        for (int iCounter = 0; keep && pushedForwards; ++iCounter)
        {
            ...
            for (int pass = 0; keep && pass < 2; ++pass)
            {
                ...
                if (lpDebug & 1)
                {
                    cout << "- Revisiting TIL timelines from " << boundForSuccessors << ", pass " << iCounter
                         << ":" << pass << endl;
                    if (pushedForwards)
                    {
                ...
                list<RelevantTimelinePointsAndOffset>::iterator rtItr =
                    relevantTimelinePointsAndTheirEnds[pass].begin();
                const list<RelevantTimelinePointsAndOffset>::iterator rtEnd =
                    relevantTimelinePointsAndTheirEnds[pass].end();
                ...
                for (; rtItr != rtEnd; ++rtItr)
                {
                    ...
                    if (lastDeletorBeforeBoundForSuccessors != rtItr->itrEnd)
                    {
                        ...
                        // thus, the next adder must precede nowAfter - push forwards the last deletor until
                        // we find an adding happening

                        for (; lastDeletorBeforeBoundForSuccessors != rtItr->itrEnd &&
                        ...
                        if (rtItr->adder->first + EPSILON - rtItr->startOffset > boundForSuccessors)
                        {
                            ...
                            if (lpDebug & 1)
                            {
                                cout << "- To get past a deletor on this timeline, and then be added again "
                                     << EPSILON - rtItr->startOffset
                                     << " before, the lower bound is pushed to " << boundForSuccessors
                                     << endl;
                            }
                            if (overallWorkingUpper < boundForSuccessors)
                            {
                                if (lpDebug & 1)
                                {
                                    cout << "- This is after the upper bound of " << overallWorkingUpper
                                         << ", so the LP would be unsolvable\n";
                                }
                                keep = false;
                                break;
    ...
    void TemporalAnalysis::recalculateDurationsFromBounds(const bool workingGlobally)
    {
        ...
        for (int actID = 0; actID < actCount; ++actID)
        {
            ...
            if (!durs.empty())
            {
                ...
                {
                    const double timelineMax =
                        actionTSBounds[actID][1].second - actionTSBounds[actID][0].first;
                    if (timelineMax < globalActionDurationBounds[actID].second)
                    {
#ifdef ENABLE_DEBUGGING_HOOKS
                        if (workingGlobally)
                        {
                            Globals::checkActionDurationBounds(actID, globalActionDurationBounds[actID].first,
                                                               timelineMax);
                        }
#endif
                        ...
                        if (localDebug)
                        {
                            std::cerr << "Action " << *(RPGBuilder::getInstantiatedOp(actID))
                                      << " in fact can take at most ";
                ...
                {
                    const double timelineMin =
                        actionTSBounds[actID][1].first - actionTSBounds[actID][1].second;
                    if (timelineMin > globalActionDurationBounds[actID].first)
                    {
#ifdef ENABLE_DEBUGGING_HOOKS
                        if (workingGlobally)
                        {
                            Globals::checkActionDurationBounds(actID, timelineMin,
                                                               globalActionDurationBounds[actID].second);
                        }
#endif
    ...
    void TemporalAnalysis::pullInAbstractTILs(MinimalState &theState, list<FFEvent> &planToAddDummyStepsTo)
    {
        ...
        for (; tilItr != tilEnd; ++tilItr)
        {
            ...
            if (firstAdder->first >= 0.0)
            {  // not an initial-state add
            ...
            else
            {
                ...
                {
                    std::cerr << "Fatal error: TIL-manipulated fact "
                              << *(RPGBuilder::getLiteral(tilItr->first))
                              << " is down as being usable from epsilon after " << firstAdder->first
                              << ", but it isn't in the initial state\n";
    ...
    void TemporalAnalysis::doDurationLimitingEffectAnalysis(
        const double &costLimit, vector<pair<double, double>> &actionDurationBounds)
    {
        ...
        static const int pneCount = RPGBuilder::getPNECount();
        static const bool minimisationMetric =
            (RPGBuilder::getMetric() ? RPGBuilder::getMetric()->minimise : true);
        ...
        if (!isolatedDurationLimitingEffects)
        {
                ...
                NumericAnalysis::theMetricIsMonotonicallyWorsening())
            {
                ...
                if (debug)
                {
                    cout << "Number of metric variables: " << RPGBuilder::getMetric()->variables.size()
                         << endl;
                }
                ...
                for (; mvItr != mvEnd; ++mvItr, ++mwItr)
                {
                    ...
                    if (*mvItr < pneCount)
                    {
                        if (RPGBuilder::getMetric()->minimise)
                        {
                            ...
                            if (debug)
                            {
                                cout << "Metric weight on " << *(RPGBuilder::getPNE(*mvItr)) << " = "
                                     << -*mwItr << endl;
                            }
                        }
                        else
                    ...
                    else
                    {
                        ...
                        if (RPGBuilder::getMetric()->minimise)
                        {
                            ...
                            if (debug)
                            {
                                cout << "Metric weight on " << *(RPGBuilder::getPNE(*mvItr - pneCount))
                                     << " = " << -*mwItr << endl;
                            }
                        }
                        else
        ...
        for (; effItr != effEnd; ++effItr)
        {
            ...
            if (debug)
            {
                cout << currEff << ": gives a fixed cost of " << fixedCost << " and cost gradient of "
                     << costGradient << endl;
            }

            const list<pair<int, Planner::time_spec>> &deps =
                RPGBuilder::getRpgNumericEffectsToActions()[*effItr];
            ...
            if (costGradient > 0.0)
            {
                ...
                for (; depItr != depEnd; ++depItr)
                {
                    if (minDur > actionDurationBounds[depItr->first].first)
                    {
                        if (debug)
                        {
                            cout << "Increased min duration on "
                                 << *(RPGBuilder::getInstantiatedOp(depItr->first)) << " from "
                                 << actionDurationBounds[depItr->first].first << " to " << minDur << endl;
                        }
            ...
            else
            {
                ...
                for (; depItr != depEnd; ++depItr)
                {
                    if (maxDur < actionDurationBounds[depItr->first].second)
                    {
                        if (debug)
                        {
                            cout << "Decreased max duration on "
                                 << *(RPGBuilder::getInstantiatedOp(depItr->first)) << " from "
                                 << actionDurationBounds[depItr->first].second << " to " << maxDur << endl;
                        }
