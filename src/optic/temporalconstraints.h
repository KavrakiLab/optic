...
namespace Planner
{
    ...
    extern pair<map<int, int>::iterator, bool> INVARIANTINSERT(map<int, int> &dest,
                                                               const pair<int, int> &toInsert,
                                                               const int &bound);
...
#define INVARIANTERASE(x, y, z) x.erase(y)
    ...
    /** @brief A record of the most recent and current interactions between a specific numeric variable, and
     * the steps in the plan.
     */
    struct FluentInteraction
    {
        /** @brief The plan step index of the last action to have a discrete numeric effect on this
         * variable.*/
        ...
        /** @brief A set of plan steps, each the start of an action with a continuous numeric effect on this
         * variable.*/
        set<int> activeCTSEffects;

        /** @brief A record of which steps have invariants acting on this variable.  The keys of the map
         * denote the end steps of actions; the values the corresponding start step. */
        map<int, int> activeInvariants;  // end step maps to start step

        /** @brief Default constructor.  <code>lastInstantaneousEffect</code> is set to -1, i.e. no step has
         * yet affected this variable.*/
    ...
    protected:
        /**
         *  The vector of temporal constraints, for each step in the plan.  Each entry <code>i</code> in the
         * vector consists of a map, the keys of which denote the steps that must precede <code>i</code>, and
    ...
    public:
         ...
         */
        virtual void addOrdering(const unsigned int &comesFirst, const unsigned int &comesSecond,
                                 const bool &epsilon);
         ...
         *             the corresponding keys, if <code>true</code>, denote that separation of at least
         * epsilon is needed.
         */
        ...
        /**
         * Returns the value of <code>mostRecentStep</code>, the index of the step most recently added to the
         * plan.
         *
        ...
        /** Updates the value of <code>mostRecentStep</code>, the index of the step most recently added to the
         * plan.
         *
