...
namespace Planner
{
    ...
    public:
         ...
         *   - <code>E_METRICTRACKING<code> denotes that the task variable never appears as a precondition -
         * only in the task metric.
         *   - <code>E_SMALLERISBETTER</code> denotes that smaller values of the variable are preferable to
         * larger values (e.g. all preconditions are of the form <code>(<= v c)</code>, and <code>v</code> can
         * only be increased).
         *   - <code>E_BIGGERISBETTER</code> denotes that smaller values of the variable are preferable to
         * larger values (e.g. all preconditions are of the form <code>(>= v c)</code>, and <code>v</code> can
         * only be decreased).
         */
         ...
         */
        enum order_independence
        {
            E_ORDER_DEPENDENT,
            E_ORDER_INDEPENDENT_AT_END,
            E_ORDER_INDEPENDENT
        };
         ...
         *
         * This struct is used to hold the output from
         * <code>NumericAnalysis::findGoalNumericUsageLimits()</code>. The limit is defined on the given
         * variable according to <code>op</code>:
         * - if <code>op = VAL::E_GREATER</code>, then the limit is <code>var > limit</code>, i.e.
         * <code>var</code> must not be decreased to this point or the relevant goal is unreachable.
         * - if <code>op = VAL::E_GREATEQ</code>, then the limit is <code>var >= limit</code>, i.e.
         * <code>var</code> must not be decreased beyond this point or the relevant goal is unreachable.
         */
        struct NumericLimitDescriptor
        {
            ...
            NumericLimitDescriptor(const int &v, const double &w, const VAL::comparison_op &cOp,
                                   const double &lim);

            NumericLimitDescriptor(const vector<int> &v, const vector<double> &w, const int &size,
                                   const VAL::comparison_op &cOp, const double &lim);
        ...
        public:
            ...
            void tightenUpper(const int &v, const double &u)
            {
                ...
                {
                    const map<int, pair<double, double>>::iterator mItr =
                        mask.insert(make_pair(v, NumericAnalysis::masterVariableBounds[v])).first;
    ...
    protected:
        ...
        /** @brief For each task variable, this defines whether the effects on that variable are
         * order-independent. */
...
#ifdef POPF3ANALYSIS
        /** @brief Ascertain whether it is only ever better for preconditions to have bigger/smaller values of
         * a given fact.
         *
         *  This function is used internally by
         * <code>findDominanceConstraintsAndMetricTrackingVariables()<code>.
         *
         *  @param varID  Variable ID to analyse to find if bigger/better values are preferable
         *  @param numericPreconditionsThatAreInCondEffs  A map from numeric preconditions to which actions'
         * conditional effects they are in
         *
         ...
         */
        static dominance_constraint preconditionDominanceInOneDirection(
        ...
        static bool couldIncreaseVariable(const RPGBuilder::RPGNumericEffect &eff,
                                          const MaskedVariableBounds &bounds);
        static bool couldDecreaseVariable(const RPGBuilder::RPGNumericEffect &eff,
                                          const MaskedVariableBounds &bounds);

        /** @brief Find if one conditional outcome could be worse than another.
         *
         * This is pessimistic: it returns <code>true</code> if there's any conceivable way that
         * <code>a</code> could be worse than <code>b</code>.
         *
         ...
         */
        static bool couldBeWorseThan(const RPGBuilder::ConditionalEffect *a,
         ...
         * lower cost (greater reward) then having a smaller x is preferable; so even though there is a
         * precondition making it look like having a larger value of x enables something, it only enables an
         * inferior option.
         *
         *  @param preID  The precondition ID to look at the dependents of
         *  @retval <code>true</code>  The precondition only supports an option inferior to what would be
         * available were it violated.
         *  @retval <code>false</code>  Cannot prove this: precondition might be good to have satisfied.
         */
        static bool preconditionOnlySupportsWorseMetricThanOtherOptions(
         ...
         *
         * This function is used internally by
         * <code>findDominanceConstraintsAndMetricTrackingVariables()<code>. If a variable is only in
         * preconditions of the form <code>p {>=,>} c</code>, it not necessarily always be preferable for it
         * to be smaller if there is an effect <code>q -= p</code> on a variable which, according to the
         * preconditions, is better to be bigger.  This function considers how the effects might change the
         * dominance rules determined from just the preconditions.  @see preconditionDominanceInOneDirection
         *
         * @param workingDominanceConstraints  The dominance constraints to be updated based on the effects
         */
        static void
        considerConflictingDominanceThroughEffects(vector<dominance_constraint> &workingDominanceConstraints);
         ...
         * If a variable causes an action's duration to increase, and that action has a continuous
         * effect/duration-dependent effect on another variable, then this affects the dominance constraints:
         ...
         * - if the effect is on a variable for which neither smaller nor bigger values are clearly
         * preferable, then there is no clear preference for the duration.
         *
         * In all cases, the preference for the duration is used update the dominance constraints on the
         * variables affecting the duration:
         * - if increasing a variable causes the duration to increase/decrease, and that is not what would be
         * preferred, then bigger values of that variable can no longer be considered better;
         * - if decreasing a variable causes the duration to increase/decrease, and that is not what would be
         * preferred, then smaller values of that variable can no longer be considered better.
         *
         ...
         * @param asIncreasedCausesDurationToDecrease  If <code>true</code>, increasing the variable's value
         * causes the duration of the effect to increase; otherwise, it causes it to decrease.
         * @param workingDominanceConstraints          The current dominance constraints, to be updated as
         * above.
         */
        static void updateForDurationDependence(const int &v, const double &w, const int &durVar,
                                                const bool &asIncreasedCausesDurationToDecrease,
        ...
        /** @brief Map (keys are goal indices) to store results of
         * <code>NumericAnalysis::considerCostsOfLiteralGoalsReachedByGoalOnlyOperators()</code>. */
    ...
    public:
         ...
         * and adding their contribution the maximum positive/negative gradient (for increase/decrease
         * effects, respectively) of v.  If an action can self-overlap arbitrarily many times, this causes the
         * gradient to become infinite.
         */
        ...
        /** @brief Return the results of the analysis to find which literal goals have direct-achivement costs
        ...
        /** @brief Return the results of the analysis to find whether variables are only ever linearly
         * modified by processes.
         *
         * @see NumericAnalysis::findVariablesThatAreTickers
         * @return A map from <code>PNE::getStateID()</code> values to doubles, denoting the continuous effect
         * on that due to processes.
         */
         ...
         *
         *  @return A const reference to <code>NumericAnalysis::dominanceConstraints</code>, the vector of
         * dominance constraints on each task variable.
         *
         */
        ...
        static const map<int, list<pair<int, Planner::time_spec>>> &
        getNumericEffectsThatAreInConditionalEffects()
        {
