...
namespace Planner
{
                                                     ...
                                                     const double &maxDur)
    {
        ...
        MinimalState *toReturn = t.applyAction(theStateHidden, minTimestamps, newDummySteps,
                                               constraintsSatisfied, a, inPlace, minDur, maxDur);

        if (previousMostRecent != -1)
        {  // if this isn't the first step in the plan
            const int newMostRecent = toReturn->temporalConstraints->getMostRecentStep();
            toReturn->temporalConstraints->addOrdering(previousMostRecent, newMostRecent,
...
#ifdef POPF3ANALYSIS
    void TotalOrderTransformer::updateWhenEndOfActionIs(MinimalState &s, const int &actID, const int &stepID,
                                                        const double &newTS)
    {
