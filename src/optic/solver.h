...
public:
...
#ifndef NDEBUG
    class MapWithNoNegativeIndices : public map<int, double>
    {
    public:
        pair<map<int, double>::iterator, bool> insert(const pair<int, double> &toInsert)
        {
            assert(toInsert.first >= 0);
            return map<int, double>::insert(toInsert);
        }

        map<int, double>::iterator insert(const map<int, double>::iterator &insItr,
                                          const pair<int, double> &toInsert)
        {
            assert(toInsert.first >= 0);
            return map<int, double>::insert(insItr, toInsert);
        }
    };
#else
    typedef map<int, double> MapWithNoNegativeIndices;
#endif
    ...
    public:
        struct Coefficient
        {
            ...
            MapWithNoNegativeIndices nonLinearCoefficients;

            Coefficient() : linearCoefficient(0.0)
            {
            }
        };
    ...
    public:
        ...
        Objective(const bool maxObj = false) : _maximise(maxObj)
        {
        }
...
public:
    ...
    virtual void addCol(const vector<pair<int, double>> &entries, const double &lb, const double &ub,
                        const ColumnType &type) = 0;
    ...
    virtual void setMatrixEntry(const int &row, const int &column, const double &coefficient) = 0;
    ...
    void setObjCoeffs(_InputIterator begin, const _InputIterator &end)
    {
        for (; begin != end; ++begin)
        {
            setObjCoeff(begin->first, begin->second);
        }
    }
    ...
    virtual void mustSolve(const bool &skipPresolve);
...
extern bool alwaysLPRelaxation;
